...
namespace PX.Objects.PM.ChangeRequest.GraphExtensions
{
	public class ChangeOrderEntryExt : PXGraphExtension<PX.Objects.PM.ChangeOrderEntry>
	{
		...
		{
			IncreaseRevenueBudget(-1);
			return Base.reject.Press(adapter);
		}
		...
		{
			if (Base.Document.Current.Status != ChangeOrderStatus.Rejected)
			{
				IncreaseRevenueBudget(-1);
			}
			return Base.coCancel.Press(adapter);
		}
		...
		{
			if (Base.Document.Current.Status == ChangeOrderStatus.Rejected || Base.Document.Current.Status == ChangeOrderStatus.Canceled)
			{
				IncreaseRevenueBudget(1);
			}

			return Base.hold.Press(adapter);
		}
		...
		public virtual void AddSelectedChangeRequests()
		{
			...
			foreach (PMChangeRequest cr in ChangeRequests.Select())
			{
				if (cr.RefNbr != null)
				{
					existing.Add(cr.RefNbr);
				}
			}
			...
			foreach (PMChangeRequest cr in AvailableChangeRequests.Cache.Updated)
			{
				...
				if (existing.Contains(cr.RefNbr)) continue;
		...
		protected virtual PMChangeOrderRevenueBudget ExtractRevenueBudget(PMChangeRequestLine line)
		{
			...
			filter.InventoryID = line.RevenueInventoryID;
		...
		protected virtual void _(Events.RowDeleted<PMChangeRequest> e)
		{
			...
			foreach (PMChangeRequestLine line in select.Select(e.Row.RefNbr))
			{
				if (e.Row.CostChangeOrderNbr == Base.Document.Current.RefNbr)
				{
					...
					if (costFromLine != null)
					{
						...
						if (cost != null)
						{
							...
							if ((Base.Document.Current.Status == ChangeOrderStatus.Rejected || Base.Document.Current.Status == ChangeOrderStatus.Canceled))
							{
								costFromLine.Rate = line.UnitPrice;
								Base.IncreaseDraftBucket(costFromLine, 1);
							}
						}
					}
				...
				if (Base.ChangeOrderClass.Current.IsRevenueBudgetEnabled == true)
				{
					...
					if (revenueFromLine != null)
					{
						...
						if (revenue != null)
						{
							...
							if ((Base.Document.Current.Status == ChangeOrderStatus.Rejected || Base.Document.Current.Status == ChangeOrderStatus.Canceled))
							{
								revenueFromLine.Rate = line.UnitPrice;
								Base.IncreaseDraftBucket(revenueFromLine, 1);
							}
						}
					}
				...
				else if ((Base.Document.Current.Status == ChangeOrderStatus.Rejected || Base.Document.Current.Status == ChangeOrderStatus.Canceled))
				{
					IncreaseChangeRequestRevenueBudget(e.Row, 1);
				}
			}
			...
			{
				IncreaseChangeRequestMarkup(e.Row, 1);
			}
		...
		protected virtual void _(Events.RowSelected<PMChangeOrder> e)
		{
			if(e.Row != null)
			{
				var changeOrderIsClosed = e.Row.Status == ChangeOrderStatus.Closed;

				if (changeOrderIsClosed)
				{
					ChangeRequests.AllowDelete = false;
					ChangeRequests.AllowUpdate = false;
					ChangeRequests.AllowInsert = false;
				}
				...
				addChangeRequests.SetEnabled(!changeOrderIsClosed && e.Row.Status != ChangeRequestStatus.Rejected && e.Row.Status != ChangeRequestStatus.Canceled);
			}
		}
			...
			if (Base.ChangeOrderClass.Current != null && Base.ChangeOrderClass.Current.IsRevenueBudgetEnabled == true)
			{
				...
				foreach (PMChangeRequestLine line in select.Select(crRefNbr))
				{
					if (Base.Project.Current.BudgetLevel == BudgetLevels.Task)
					{
						if (line.RevenueTaskID == null || line.RevenueAccountGroupID == null)
						{
							throw new PXSetPropertyException(Messages.ChangeRequestCannotBeAdded_Task, PXErrorLevel.RowError);
						}
					}
					else if (Base.Project.Current.BudgetLevel == BudgetLevels.Item)
					{
						...
						{
							throw new PXSetPropertyException(Messages.ChangeRequestCannotBeAdded_TaskItem, PXErrorLevel.RowError);
						}
					}
					else if (Base.Project.Current.BudgetLevel == BudgetLevels.CostCode)
					{
						...
						{
							throw new PXSetPropertyException(Messages.ChangeRequestCannotBeAdded_TaskCostCode, PXErrorLevel.RowError);
						}
					}
		...
		private void IncreaseRevenueBudget(int mult = 1)
		{
			if (Base.ChangeOrderClass.Current.IsRevenueBudgetEnabled != true)
			{
				var select = new PXSelect<PMChangeRequest,
				Where<PMChangeRequest.changeOrderNbr, Equal<Current<PMChangeOrder.refNbr>>, Or<PMChangeRequest.costChangeOrderNbr, Equal<Current<PMChangeOrder.refNbr>>>>>(Base);

				foreach (PMChangeRequest changeRequest in select.Select())
				{
					IncreaseChangeRequestMarkup(changeRequest, mult);
					IncreaseChangeRequestRevenueBudget(changeRequest, mult);
				}
			}
		}

		private void IncreaseChangeRequestMarkup(PMChangeRequest changeRequest, int mult = 1)
		{
			var markups = new PXSelect<PMChangeRequestMarkup, Where<PMChangeRequestMarkup.refNbr, Equal<Required<PMChangeRequest.refNbr>>>>(Base);
			foreach (PMChangeRequestMarkup markup in markups.Select(changeRequest.RefNbr))
			{
				PMChangeOrderRevenueBudget revenue = ExtractRevenueBudget(markup);
				if (revenue != null)
				{
					revenue.Rate = 0;
					Base.IncreaseDraftBucket(revenue, mult);
				}
			}
		}
		private void IncreaseChangeRequestRevenueBudget(PMChangeRequest changeRequest, int mult = 1)
		{
			var changeRequestLines = new PXSelect<PMChangeRequestLine, Where<PMChangeRequestLine.refNbr, Equal<Required<PMChangeRequest.refNbr>>>>(Base);
			foreach (PMChangeRequestLine changeRequestLine in changeRequestLines.Select(changeRequest.RefNbr))
			{
				PMChangeOrderRevenueBudget revenueBudget = ExtractRevenueBudget(changeRequestLine);
				if (revenueBudget != null)
				{
					revenueBudget.Rate = changeRequestLine.UnitPrice;
					Base.IncreaseDraftBucket(revenueBudget, mult);
				}
			}
		}
	}
