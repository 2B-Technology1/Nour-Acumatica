	...
	public class PTOHelper
	{
		...
		public static bool IsFirstBankProcessOfPTOYear(PXGraph graph, DateTime targetDate, int employeeID, IPTOBank bank, PRPayment paymentToSkip = null)
		{
			...
			var records = SelectFrom<PRPaymentPTOBank>
				...
				.Where<PRPayment.employeeID.IsEqual<P.AsInt>
					...
					.And<PREarningDetail.date.IsBetween<P.AsDateTime.UTC, P.AsDateTime.UTC>>
		...
		private static List<IPTOBank> GetAllEmployeeBanksQuery(PXGraph graph, PRPayment payment, Func<IPTOBank, bool> condition)
		{
			return SelectFrom<PREmployeePTOBank>
				...
				.View.Select(graph, payment.EmployeeID).AsEnumerable()
		...
		public static DateTime GetPTOStartDate(IPTOBank bank, DateTime ptoYearStart, int day)
		{
			if (bank.StartDate.Value.Year != ptoYearStart.Year)
			{
				if (bank.StartDate.Value.Year > ptoYearStart.Year)
				{
					return bank.StartDate.GetValueOrDefault();
				}
				else
				{
			...
			if (ptoYearStart.DayOfYear > bank.StartDate.Value.DayOfYear)
			{
				return new DateTime(ptoYearStart.Year, ptoYearStart.Month, ptoYearStart.Day);
			}

			return new DateTime(ptoYearStart.Year, bank.StartDate.Value.Month, bank.StartDate.Value.Day);
		}
