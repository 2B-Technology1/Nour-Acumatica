...
namespace PX.Objects.PR
{
	public class PRSetupMaint : PXGraph<PRSetupMaint>
	{
		...
		protected virtual void _(Events.RowPersisting<PRTransactionDateException> e)
		{
			...
			if (((e.Operation & PXDBOperation.Command) == PXDBOperation.Update || (e.Operation & PXDBOperation.Command) == PXDBOperation.Insert) &&
				new SelectFrom<PRTransactionDateException>
					.Where<PRTransactionDateException.date.IsEqual<P.AsDateTime.UTC>
		...
		protected virtual void _(Events.FieldVerifying<PRSetup, PRSetup.updateGL> e)
		{
			...
			PRPayment nonReleasedPayments = SelectFrom<PRPayment>.Where<PRPayment.released.IsNotEqual<True>>.View.SelectSingleBound(this, null);

			if (nonReleasedPayments != null)
			{
				throw new PXSetPropertyException(Messages.CannotModifyUpdateGLSetting);
			}
		}

		public virtual void _(Events.RowPersisting<PRSetup> e)
		{
			if (e.Row == null || e.Operation == PXDBOperation.Delete || !PXAccess.FeatureInstalled<FeaturesSet.projectModule>())
			{
				return;
			}
