...
{
	/// <summary>
	/// Used only in Canadian Payroll. Stores the information about the paid time off accrual calculation related to a paycheck. The information will be displayed on the Paychecks and Adjustments (PR302000) form.
	/// </summary>
	...
	public class PRPTODetail : IBqlTable, IPaycheckExpenseDetail<string>
	{
		...
		{
			public static PRPTODetail Find(PXGraph graph, int? recordID, PKFindOptions options = PKFindOptions.None) => FindBy(graph, recordID, options);
		}
		...
		{
			public static PRPTODetail Find(PXGraph graph, int? branchID, string paymentDocType, string paymentRefNbr, int? bankID, int? projectID, int? projectTaskID, int? laborItemID, string earningTypeCD, int? costCodeID, PKFindOptions options = PKFindOptions.None) =>
				FindBy(graph, branchID, paymentDocType, paymentRefNbr, bankID, projectID, projectTaskID, laborItemID, earningTypeCD, costCodeID, options);
		}
		...
		public static class FK
		{
			...
			public class ProjectTask : PMTask.PK.ForeignKeyOf<PRPTODetail>.By<projectID, projectTaskID> { }
