...
{
	/// <summary>
	/// Stores information related to a batch of employees that are paid at the same frequency. Contains summary information about the batch.
	...
	public class PRBatch : IBqlTable
	{
		...
		{
			public static PRBatch Find(PXGraph graph, string batchNbr, PKFindOptions options = PKFindOptions.None) => FindBy(graph, batchNbr, options);
		}
		...
		[PXSelector(typeof(SearchFor<PRBatch.batchNbr>.Where<MatchWithPayGroup<PRBatch.payGroupID>>))]
