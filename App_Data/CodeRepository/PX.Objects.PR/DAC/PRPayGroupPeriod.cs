...
{
	/// <summary>
	/// Stores the information about the pay schedule of each pay group for all the created pay periods. The information will be displayed on the Pay Groups (PR205000) form.
	/// </summary>
	[Serializable]
	...
	public partial class PRPayGroupPeriod : IBqlTable, IPeriod
	{
		...
		{
			public static PRPayGroupPeriod Find(PXGraph graph, string payGroupID, string finPeriodID, PKFindOptions options = PKFindOptions.None) =>
				FindBy(graph, payGroupID, finPeriodID, options);
		}
		...
		[PXUIField(DisplayName = "Financial Year", Visibility = PXUIVisibility.Visible, Visible = false, Enabled = false)]
