...
namespace PX.Objects.PR
{
	...
	public class PRPayBatchEntry : PXGraph<PRPayBatchEntry, PRBatch>, PXImportAttribute.IPXPrepareItems
	{
		public PRPayBatchEntry()
		{
			...
			{
				this.RowSelected.AddHandler<PRBatch>(FetchTransactionsFromCBAPIRowSelected);
			}
		}
		...
		protected virtual IEnumerable transactions()
		{
			...
			List<PRBatchEmployee> result = new List<PRBatchEmployee>();
			bool voidPayCheckExist = false;

			int totalRows = 0;
			int startRow = PXView.StartRow;
			foreach (PRBatchEmployee batchEmployee in query.Select(PXView.Currents, PXView.Parameters, PXView.Searches, PXView.SortColumns,
			...
			PXUIFieldAttribute.SetVisible<PRBatchEmployee.voidPaymentDocAndRef>(Transactions.Cache, null, voidPayCheckExist);
		...
		[Branch(
			typeof(Parent<PRBatchEmployee.branchID>),
			typeof(SearchFor<BranchWithAddress.branchID>),
			IsDetail = false)]
		[PXRestrictor(typeof(Where<PRBatchEmployee.employeeCountryID.FromCurrent.IsNull.Or<BranchWithAddress.addressCountryID.IsEqual<PRBatchEmployee.employeeCountryID.FromCurrent>>>),
		...
		protected virtual void _(Events.RowSelected<PRBatch> e)
		{
			...
			CopySelectedEmployeeEarningDetailLine.SetEnabled(allowBatchModification);
		...
		protected virtual void FetchTransactionsFromCBAPIRowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			this.RowSelected.RemoveHandler<PRBatch>(FetchTransactionsFromCBAPIRowSelected);
			Transactions.Select();
			this.RowSelected.AddHandler<PRBatch>(FetchTransactionsFromCBAPIRowSelected);
		}
		...
		{
			PRBatchEmployee currentBatchEmployee = e.Row as PRBatchEmployee;
			...
			if (currentBatchEmployee?.HasNegativeHoursEarnings == true)
			{
				...
				PXUIFieldAttribute.SetWarning<PRBatchEmployee.hourQty>(e.Cache, e.Row, error);
			}
			else
			{
				PXUIFieldAttribute.SetWarning<PRBatchEmployee.hourQty>(e.Cache, e.Row, null);
			}

			if (Preferences.Current?.EnablePieceworkEarningType == false && currentBatchEmployee?.HourQty > 0m && currentBatchEmployee?.Rate == 0m)
			{
				PXUIFieldAttribute.SetWarning<PRBatchEmployee.rate>(e.Cache, e.Row, Messages.PayRateNotFound);
			}
			else
			{
				PXUIFieldAttribute.SetWarning<PRBatchEmployee.rate>(e.Cache, e.Row, null);
			}
		}
		...
		protected virtual void _(Events.FieldUpdated<PREarningDetail.employeeID> e)
		{
			...
			if (CreatePayrollBatchEmployee(e.NewValue as int?))
				e.Cache.SetDefaultExt<PREarningDetail.branchID>(e.Row);
		}
		...
		protected virtual void _(Events.FieldUpdating<PRBatchEmployee.employeeID> e)
		{
			if (IsImport && e.NewValue != null && e.NewValue is string acctCD)
			{
				...
				CreatePayrollBatchEmployee(employeeID);
				if (employeeID != null)
					e.NewValue = employeeID;
			}
		}

		protected virtual void _(Events.FieldUpdated<PRBatchEmployee.employeeID> e)
		{
			if (IsImport && e.NewValue is int employeeID && (Transactions.Current == null || Transactions.Current.EmployeeID != employeeID))
			{
				Transactions.Current = Transactions.Search<PRBatchEmployee.employeeID>(employeeID).TopFirst;
			}
		}
		...
		protected virtual bool CheckEmployeeForImport(string acctCD, out string errorMessage)
		{
			...
			CreatePayrollBatchEmployee(payrollEmployee.BAccountID);
			return true;
		}
		...
		protected virtual bool CreatePayrollBatchEmployee(int? employeeID)
		{
			if (employeeID == null)
				return false;
			...
			PRBatchEmployee newPRBatchEmployeeRecord = new PRBatchEmployee();
		...
		protected virtual bool TrySelectPayrollBatchEmployee(int? employeeID, out string errorMessage)
		{
			...
			PRBatchEmployee existingPRBatch = GetPayrollBatchesForCurrentPeriod(new[] { employeeID }).FirstOrDefault();
			if (existingPRBatch != null)
			{
				errorMessage = Messages.EmployeeAlreadyAddedToAnotherBatch;
				return false;
			}

			PRPayment existingPRPayment = GetPaymentsForCurrentPeriod(new[] { employeeID }).FirstOrDefault();
			if (existingPRPayment != null)
			{
				errorMessage = GetExistingPRPaymentErrorMessage(existingPRPayment);
				return false;
			}
		...
		{
			List<PRBatchEmployee> employeesInCurrent = Transactions.Select().FirstTableItems.ToList();
			List<PRPayment> existingPayments = GetPaymentsForCurrentPeriod(employeesInCurrent.Select(x => x.EmployeeID).ToArray());
		...
		private List<PRPayment> GetPaymentsForCurrentPeriod(int?[] employeeIDs)
		{
			PXView query = new SelectFrom<PRPayment>.
				Where<PRPayment.payGroupID.IsEqual<PRBatch.payGroupID.FromCurrent>.
					And<PRPayment.payPeriodID.IsEqual<PRBatch.payPeriodID.FromCurrent>>.
					And<PRPayment.voided.IsNotEqual<True>>.
					And<PRPayment.employeeID.IsIn<P.AsInt>>>.View(this).View;
			...
			return query.SelectMulti(employeeIDs).RowCast<PRPayment>().ToList();
		}

		private List<PRBatchEmployee> GetPayrollBatchesForCurrentPeriod(int?[] employeeIDsInPayGroup)
		{
			PXView query = new SelectFrom<PRBatchEmployee>.
				...
				Where<Brackets<PRBatch.batchNbr.IsNotEqual<PRBatch.batchNbr.FromCurrent>.
						Or<PRBatch.batchNbr.FromCurrent.IsNull>>.
								And<PRBatch.open.IsEqual<False>>.
					And<PRBatch.closed.IsEqual<False>>.
					...
					And<PRBatchEmployee.employeeID.IsIn<P.AsInt>>>.View(this).View;

			if (Document.Current.PayrollType == PayrollType.Regular)
			{
				query.WhereAnd<Where<PRBatch.payrollType.IsEqual<PayrollType.regular>>>();
			}

			return query.SelectMulti(employeeIDsInPayGroup).RowCast<PRBatchEmployee>().ToList();
		}
		...
		{
			PRBatchEmployee batchEmployeeExt = Transactions.Current;
		...
		private void PrepareEmployeesPanel(bool retainClassAndTypeFilters, AddEmployeeFilter addEmployeeFilter = null)
		{
			...
			List<PRBatchEmployee> existingBatchesWithSamePayPeriodForEmployeesNotInBatch = GetPayrollBatchesForCurrentPeriod(employeeIDsInPayGroupAndNotInBatch);
			List<PRPayment> existingPaymentsWithSamePayPeriodForEmployeesNotInBatch = GetPaymentsForCurrentPeriod(employeeIDsInPayGroupAndNotInBatch);
			foreach (PREmployee employee in employeesInPayGroup)
			{
				...
				{
					PRBatchEmployee existingPRBatch = existingBatchesWithSamePayPeriodForEmployeesNotInBatch.FirstOrDefault(x => x.EmployeeID == employeeID);
					if (existingPRBatch != null)
					{
						allowEmployeeSelection = false;
						message = Messages.EmployeeAlreadyAddedToAnotherBatch;
					}

					if (allowEmployeeSelection)
					{
						PRPayment existingPayment = existingPaymentsWithSamePayPeriodForEmployeesNotInBatch.FirstOrDefault(x => x.EmployeeID == employeeID);
						if (existingPayment != null)
						{
							allowEmployeeSelection = false;
							message = GetExistingPRPaymentErrorMessage(existingPayment);
						}
					}
				}
		...
		protected string GetExistingPRPaymentErrorMessage(PRPayment payment)
		{
			return payment.DocType == PayrollType.Regular ?
				string.Format(Messages.EmployeeAlreadyAddedToRegularPaycheck, payment.PaymentDocAndRef)
					: string.Format(Messages.EmployeeAlreadyAddedToNonRegularPaycheck, payment.PaymentDocAndRef);
		}
		...
		private void AddEarningDetailsForSelectedEmployees(List<PREmployee> selectedEmployees)
		{
			...
			{
				PRBatchEmployee newPRBatchEmployeeRecord = null;

				try
				{
					newPRBatchEmployeeRecord = new PRBatchEmployee();
				...
				catch (PXException exception)
				{
					...
					if (exception.MessageNoPrefix == Messages.ActivityNotReleased)
						throw new PXException(Messages.EarningDetailsCreationFailedBecauseOfTimeActivity);

					if (exception.MessageNoPrefix == Messages.EmployeeWasNotEmployed)
						throw new PXException(Messages.EarningDetailsCreationFailedBecauseOfDates);
				}
			}
		...
		private void AddQuickPayHolidayEarningDetails(EarningDetailUpdateParameters parameters, out decimal totalHolidayHours)
		{
			PXResultset<CSCalendarExceptions> calendarExceptions = SelectFrom<CSCalendarExceptions>.
				Where<CSCalendarExceptions.calendarID.IsEqual<P.AsString>.
					And<CSCalendarExceptions.date.IsGreaterEqual<P.AsDateTime.UTC>>.
					And<CSCalendarExceptions.date.IsLessEqual<P.AsDateTime.UTC>>>.View.
		...
		private Dictionary<DateTime, decimal> GetStandardWorkingHours(EarningDetailUpdateParameters parameters)
		{
			...
			while (dateToCheck <= parameters.BatchEndDate.Date)
			{
				...
				CSCalendarExceptions holidayCheck = SelectFrom<CSCalendarExceptions>.
					Where<CSCalendarExceptions.calendarID.IsEqual<P.AsString>.
						And<CSCalendarExceptions.date.IsEqual<P.AsDateTime.UTC>>>.View.ReadOnly.
			...
			out string errorMessage)
		{
			...
			PRBatchEmployee existingPRBatch = GetPayrollBatchesForCurrentPeriod(new[] { (int?)employeeID }).FirstOrDefault();
			if (existingPRBatch != null)
			{
				errorMessage = Messages.EmployeeAlreadyAddedToAnotherBatch;
			...
			PRPayment existingPRPayment = GetPaymentsForCurrentPeriod(new[] { (int?)employeeID }).FirstOrDefault();
			if (existingPRPayment != null)
			{
				errorMessage = GetExistingPRPaymentErrorMessage(existingPRPayment);
				allowTimeActivitySelection = false;
		...
		protected virtual void AddTimeActivities()
		{
			...
			{
				CreatePayrollBatchEmployee(timeActivity.EmployeeID);
		...
		[Obsolete(Common.Messages.ItemIsObsoleteAndWillBeRemoved2023R1)]
		private List<PRBatchEmployee> GetExistingPayrollBatches(int?[] employeeIDsInPayGroup)
		{
			return GetPayrollBatchesForCurrentPeriod(employeeIDsInPayGroup);
		}

		[Obsolete(Common.Messages.ItemIsObsoleteAndWillBeRemoved2023R1)]
		private List<PRPayment> GetExistingPayments(int?[] employeeIDs)
		{
			return GetPaymentsForCurrentPeriod(employeeIDs);
		}
		#endregion
	}
	...
	public sealed class PRBatchEmployeeExt : PRBatchEmployee
	{
		...
		public new class paymentRefNbr : BqlString.Field<paymentRefNbr> { }
		[PXString]
		public new string PaymentRefNbr { get; set; }
		#endregion

		#region PaymentDocAndRef
		public new abstract class paymentDocAndRef : BqlString.Field<paymentDocAndRef> { }
		[PXString]
		[PXUIField(DisplayName = "Paycheck Ref", Enabled = false)]
		public new string PaymentDocAndRef { get; set; }
		#endregion

		#region VoidPaymentDocAndRef
		public new abstract class voidPaymentDocAndRef : BqlString.Field<voidPaymentDocAndRef> { }
		[PXString]
		[PXUIField(DisplayName = "Void Paycheck Ref", Visible = false)]
		public new string VoidPaymentDocAndRef { get; set; }
		#endregion

		#region HasNegativeHoursEarnings
		[PXBool]
		[PXUnboundDefault(false)]
		public new bool? HasNegativeHoursEarnings { get; set; }
		public new abstract class hasNegativeHoursEarnings : BqlBool.Field<hasNegativeHoursEarnings> { }
		#endregion
	}
