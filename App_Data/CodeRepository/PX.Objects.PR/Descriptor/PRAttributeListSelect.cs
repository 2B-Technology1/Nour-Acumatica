...
namespace PX.Objects.PR
{
		...
		where TTypeMeta : SettingDefinitionListAttribute
	{
		...
		protected IEnumerable viewHandler()
		{
			...
			foreach (ISettingDefinition definition in definitionList)
			{
				...
				if (foundRecord != null)
				{
					...
					if (foundRecord.CanadaReportMapping != definition.CanadaReportMapping)
					{
						foundRecord.CanadaReportMapping = definition.CanadaReportMapping;
						setUpdatedStatus = true;
					}

					bool isEncryptionRequired = (definition.IsEncryptionRequired == true);
					if (foundRecord.CanadaReportMapping != null && foundRecord.IsEncryptionRequired != isEncryptionRequired)
					{
						foundRecord.IsEncryptionRequired = isEncryptionRequired;
						setUpdatedStatus = true;
					}
		...
		private TEntity CreateDacRecord(PXCache cache, ISettingDefinition definition, Dictionary<TaxSettingAdditionalInformationKey, PRTaxSettingAdditionalInformation> infoDefinitionDict)
		{
			...
			attr.CanadaReportMapping = definition.CanadaReportMapping;
			if (definition.CanadaReportMapping != null)
			{
				attr.IsEncryptionRequired = definition.IsEncryptionRequired;
			}
		...
		where TPrimaryViewType : class, IBqlTable
	{
		...
		public void SetSettingNameForDescription(TEntity row)
		{
			if (!string.IsNullOrEmpty(row.Description) && row is IStateSpecific stateSpecificRow && !string.IsNullOrEmpty(stateSpecificRow.State))
			{
				...
				row.CanadaReportMapping = definition.CanadaReportMapping;
				if (definition.CanadaReportMapping != null)
				{
					row.IsEncryptionRequired = definition.IsEncryptionRequired;
				}
