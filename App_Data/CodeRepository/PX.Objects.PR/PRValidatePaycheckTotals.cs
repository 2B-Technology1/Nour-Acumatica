...
namespace PX.Objects.PR
{
	...
	public class PRValidatePaycheckTotals : PXGraph<PRValidatePaycheckTotals>
	{
		...
		protected virtual void ValidateHeader(bool tryCorrectTotalsOnDiscrepancy)
		{
			...
			copy.DetailLinesCount = EarningDetails.Select().Count + BenefitDetails.Select().Count + DeductionDetails.Select().Count + TaxDetails.Select().Count;
			...
			CheckValues(copy.DetailLinesCount, Document.Cache, Document.Current, nameof(Document.Current.DetailLinesCount), tryCorrectTotalsOnDiscrepancy,
				Messages.HeaderDetailLinesCountDoesntMatch, Document.Current.DetailLinesCount, copy.DetailLinesCount);
		...
		{
			if (!tryCorrectTotalsOnDiscrepancy && Document.Current.Calculated != true || Document.Current.NetAmount <= 0)
			{
				return;
			}
			...
			if (tryCorrectTotalsOnDiscrepancy)
			{
				if (directDepositSum != Document.Current.NetAmount)
				{
					...
					var calcEngineDirectDepositSum = calculationEngine.GetDirectDepositSum(Document.Current);
					if (calcEngineDirectDepositSum != Document.Current.NetAmount)
					{
