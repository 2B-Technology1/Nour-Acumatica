...
namespace PX.Objects.PR
{
	public class PRDedBenCodeMaint : PXGraph<PRDedBenCodeMaint, PRDeductCode>
	{
		...
		public SelectFrom<PREmployeeDeduct>
			.Where<PREmployeeDeduct.codeID.IsEqual<PRDeductCode.codeID.FromCurrent>>.View EmployeeDeductions;
		...
		public SelectFrom<PRPaymentDeduct>
			.Where<PRPaymentDeduct.codeID.IsEqual<PRDeductCode.codeID.FromCurrent>>.View PaymentDeductions;
		...
		protected virtual void _(Events.FieldVerifying<PRDeductCode.affectsTaxes> e)
		{
			if (EmployeeDeductions.SelectSingle() != null || PaymentDeductions.SelectSingle() != null)
			{
				throw new PXSetPropertyException<PRDeductCode.affectsTaxes>(Messages.DedBenCodeInUse);
			}
		}

		protected virtual void _(Events.FieldVerifying<PRDeductCode, PRDeductCode.certifiedReportType> e)
		{
			...
			bool isDeductCodeAssociatedToCertProject = SelectFrom<PRProjectFringeBenefitRateReducingDeduct>
				.Where<PRProjectFringeBenefitRateReducingDeduct.deductCodeID.IsEqual<P.AsInt>>.View.Select(e.Cache.Graph, e.Row.CodeID).Any();

			if (isDeductCodeAssociatedToCertProject)
			{
				throw new PXSetPropertyException<PRDeductCode.certifiedReportType>(Messages.DeductCodeAssociatedToACertProject);
			}
		}
