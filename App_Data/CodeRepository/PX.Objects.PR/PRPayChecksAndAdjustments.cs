...
namespace PX.Objects.PR
{
	...
	{
		public static readonly IReadOnlyDictionary<WebDialogResult, string> ConfirmCancelButtons = new Dictionary<WebDialogResult, string>
			{
				{ WebDialogResult.Yes, Messages.ConfirmLabel },
				{ WebDialogResult.No, Messages.CancelLabel }
			};
		...
		public PRPayChecksAndAdjustments()
		{
			...
			Action.AddMenuAction(PutOnHold);
			Action.AddMenuAction(RemoveFromHold);
		...
		[PXRestrictor(typeof(Where<PRPayment.countryID.FromCurrent.IsNull.Or<BranchWithAddress.addressCountryID.IsEqual<PRPayment.countryID.FromCurrent>>>),
			Messages.PaymentBranchCountryNotMatching, new[] { typeof(PRPayment.countryID), typeof(BranchWithAddress.addressCountryID) })]
		protected virtual void _(Events.CacheAttached<PRDeductionDetail.branchID> e) { }
		...
		[PXRestrictor(typeof(Where<PRPayment.countryID.FromCurrent.IsNull.Or<BranchWithAddress.addressCountryID.IsEqual<PRPayment.countryID.FromCurrent>>>),
			Messages.PaymentBranchCountryNotMatching, new[] { typeof(PRPayment.countryID), typeof(BranchWithAddress.addressCountryID) })]
		protected virtual void _(Events.CacheAttached<PRBenefitDetail.branchID> e) { }
		...
		[PXRestrictor(typeof(Where<PRPayment.countryID.FromCurrent.IsNull.Or<BranchWithAddress.addressCountryID.IsEqual<PRPayment.countryID.FromCurrent>>>),
			Messages.PaymentBranchCountryNotMatching, new[] { typeof(PRPayment.countryID), typeof(BranchWithAddress.addressCountryID) })]
		protected virtual void _(Events.CacheAttached<PRTaxDetail.branchID> e) { }
		...
		[PXRestrictor(typeof(Where<PRPayment.countryID.FromCurrent.IsNull.Or<BranchWithAddress.addressCountryID.IsEqual<PRPayment.countryID.FromCurrent>>>),
			Messages.PaymentBranchCountryNotMatching, new[] { typeof(PRPayment.countryID), typeof(BranchWithAddress.addressCountryID) })]
		protected virtual void _(Events.CacheAttached<PRPTODetail.branchID> e) { }
		...
		[PXUIField(DisplayName = "Add to Payment Batch", MapEnableRights = PXCacheRights.Update, MapViewRights = PXCacheRights.Update)]
		[PXButton(Category = "Processing")]
		...
		public PXAction<PRPayment> PutOnHold;
		[PXUIField(DisplayName = Messages.PutOnHoldAction, MapEnableRights = PXCacheRights.Update, MapViewRights = PXCacheRights.Update)]
		[PXProcessButton(Category = "Corrections")]
		...
		public PXAction<PRPayment> RemoveFromHold;
		[PXUIField(DisplayName = Messages.RemoveFromHoldAction, MapEnableRights = PXCacheRights.Update, MapViewRights = PXCacheRights.Update)]
		[PXProcessButton(Category = "Corrections")]
		public virtual IEnumerable removeFromHold(PXAdapter adapter)
		{
			if (Document.Current != null)
			{
		...
		protected virtual void AddTimeActivities()
		{
			foreach (PMTimeActivityExt timeActivity in TimeActivities.Select().FirstTableItems.Where(item => item.Selected == true))
			{
				...
				earningDetail.Hours = Math.Round((decimal)timeActivity.TimeSpent / 60, 6);
		...
		public virtual IEnumerable release(PXAdapter adapter)
		{
			...
			{
				if (HasAssociatedTaxFormBatch(payment))
				{
					if (Document.View.Ask(null, Messages.ConfirmationHeader, Messages.TaxFormToBeGenerated, MessageButtons.YesNo, ConfirmCancelButtons, MessageIcon.Question) == WebDialogResult.No)
					{
						return list;
					}
				}
				...
				try
				{
					...
					if (pmt.DocType != PayrollType.VoidCheck)
					{
						if (pmt.DocType == PayrollType.Adjustment && pmt.Calculated != true)
						{
							...
							calculationEngine.SetDirectDepositSplit(pmt);
					...
					if (pmt.DocType == PayrollType.VoidCheck)
					{
						UpdateRecordOfEmploymentStatus(pmt);
					}
		...
		[PXUIField(DisplayName = "View Deductions and Benefits", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		[PXButton]
		...
		public virtual void checkTaxUpdateTimestamp()
		{
			...
			{
				UpdateTaxesPopupView.Current.Message = PXMessages.LocalizeNoPrefix(Messages.TaxUpdateNeeded);
		...
		protected void _(Events.RowSelected<PRPayment> e)
		{
			...
			ProcessPayment.SetEnabled(e.Row.Hold == false && e.Row.Paid == false && e.Row.PaymentBatchNbr == null
				&& (e.Row.Calculated == true || (e.Row.DocType == PayrollType.Adjustment && IsReleaseActionEnabled(e.Row, PRSetup.Current.UpdateGL ?? false)))
				&& e.Row.NetAmount > 0 && PRSetup.Current.UpdateGL == true && e.Row.Released == false
			);
			...
			PutOnHold.SetVisible(e.Row.Hold == false);
			PutOnHold.SetEnabled(e.Row.Hold == false && e.Row.Released == false && (e.Row.Paid == false || e.Row.DocType == PayrollType.VoidCheck));
			RemoveFromHold.SetVisible(e.Row.Hold == true);
			RemoveFromHold.SetEnabled(e.Row.Hold == true);			
		...
		public void _(Events.FieldDefaulting<PRPayment, PRPayment.empType> e)
		{
			var row = e.Row as PRPayment;
			if (row == null || row.DocType == null || row.EmployeeID == null)
			{
				return;
			}

			if (row.DocType != PayrollType.Adjustment)
			{
				var employee = PREmployee.PK.Find(this, row.EmployeeID);
				if (employee?.EmpType != null)
				{
					e.NewValue = employee.EmpType;
				}
			}
			else
			{
				e.NewValue = EmployeeType.Hourly;
			}
		}
		...
		protected virtual void _(Events.FieldVerifying<PRPayment.employeeID> e)
		{
			...
			CheckLatestReleasedPayment(this, e);
		}
		...
		public void _(Events.FieldUpdated<PRPayment.cashAccountID> e)
		{
			Document.Current.Calculated = false;
		}
		...
		{
			if (IsCopyPasteContext && e.NewRow != null && Document.Current.IsWeeklyOrBiWeeklyPeriod == false && (e.NewRow.RuleType == PROvertimeRuleType.Weekly || e.NewRow.RuleType == PROvertimeRuleType.Consecutive))
			{
		...
		#region Helpers
		public static bool HasAssociatedTaxFormBatch(PRPayment payment)
		{
			...
			PRPrepareTaxFormsMaint taxFormGraph = PXGraph.CreateInstance<PRPrepareTaxFormsMaint>();

			PXAccess.MasterCollection.Branch branch = PXAccess.GetBranch(payment.BranchID);

			PREmployeeTaxForm taxForm = SelectFrom<PREmployeeTaxForm>
				...
				.Where<PREmployeeTaxForm.employeeID.IsEqual<P.AsInt>
					...
					.And<PRTaxFormBatch.year.IsEqual<P.AsString>>>.View.Select(taxFormGraph, payment.EmployeeID, branch.BAccountID, branch.Organization.BAccountID, payment.TransactionDate.Value.Year.ToString()).FirstOrDefault();
		...
		public static void DeleteEmptySummaryEarnings(PXView summaryEarningsView, PXCache earningDetailCache)
		{
			...
			{
				if (row.Hours == 0 && row.Amount == 0 && row.MTDAmount == 0 && row.QTDAmount == 0 && row.YTDAmount == 0 &&
			...
			if (row.EmployeeID != null)
			{
				...
				foreach (IPTOBank bank in banksToInsert)
				{
					...
					foreach ((DateTime, DateTime) yearSpan in ptoYears)
					{
						...
						var effectiveStartDate = PTOHelper.GetPTOStartDate(bank, ptoYearStart, day);
		...
		private void CheckExistingPaychecksAndBatches<F>(Events.FieldVerifying<F> e, int? employeeID, string payPeriodID) where F : class, IBqlField
		{
			...
			if (existingPayment != null && existingPayment.PayPeriodID == payPeriodID)
			{
				if (IsCopyPasteContext)
				{
					e.Cancel = true;
		...
		private static void CheckLatestReleasedPayment<F>(PXGraph graph, Events.FieldVerifying<F> e) where F : class, IBqlField
		{
			PRPayment payment = (PRPayment)e.Row;

			if (payment.DocType == PayrollType.Final && payment.PayPeriodID != null)
			{
				var latestReleasedPayment = new SelectFrom<PRPayment>
					.Where<PRPayment.employeeID.IsEqual<PRPayment.employeeID.FromCurrent>
						.And<PRPayment.docType.IsNotEqual<PayrollType.voidCheck>>
						.And<PRPayment.released.IsEqual<False>
							.Or<PRPayment.voided.IsEqual<False>>>>
					.OrderBy<PRPayment.payPeriodID.Desc>.View(graph).SelectSingle();

				if (latestReleasedPayment != null && string.Compare(latestReleasedPayment.PayPeriodID, payment.PayPeriodID) > 0)
				{
					throw new PXSetPropertyException<PRPayment.payPeriodID>(Messages.FinalPaycheckCantBeReleased, payment.PaymentDocAndRef);
				}
			}
		}
			...
			PXView query = new SelectFrom<PRPayment>.
					Where<PRPayment.refNbr.IsNotEqual<P.AsString>.
						And<PRPayment.payPeriodID.IsEqual<P.AsString>.
							Or<PRPayment.payPeriodID.IsLess<P.AsString>.
							And<PRPayment.released.IsEqual<False>>>>.
						And<PRPayment.payGroupID.IsEqual<P.AsString>>.
						And<PRPayment.employeeID.IsEqual<P.AsInt>>.
						And<PRPayment.voided.IsNotEqual<True>>>.View(this).View;

			if (Document.Current.DocType == PayrollType.Regular)
			{
				query.WhereAnd<Where<PRPayment.docType.IsEqual<PayrollType.regular>.Or<PRPayment.docType.IsNotEqual<PayrollType.voidCheck>.And<PRPayment.released.IsEqual<False>>>>>();
			}
			else
			{
				query.WhereAnd<Where<PRPayment.docType.IsNotEqual<PayrollType.voidCheck>.And<PRPayment.released.IsEqual<False>>>>();
			}

			return (PRPayment)query.SelectSingle(Document.Current.RefNbr, payPeriodID, payPeriodID, payGroupID, employeeID);
		}
		...
		private PRBatch GetExistingPayrollBatch(int? employeeID, string payPeriodID, string payGroupID)
		{
			...
			PXView query = new SelectFrom<PRBatch>.
					...
					Where<PRBatch.open.IsNotEqual<True>.
						...
						And<PRBatch.payPeriodID.IsEqual<P.AsString>.
							Or<PRBatch.payPeriodID.IsLess<P.AsString>.
							And<PRBatch.open.IsEqual<False>>.And<PRBatch.closed.IsEqual<False>>>>.
						And<PRBatch.payGroupID.IsEqual<P.AsString>>.
						And<PRBatchEmployee.employeeID.IsEqual<P.AsInt>>>>.View(this).View;


			if (Document.Current.DocType == PayrollType.Regular)
			{
				query.WhereAnd<Where<PRBatch.payrollType.IsEqual<PayrollType.regular>.And<PRBatch.open.IsEqual<False>>.And<PRBatch.closed.IsEqual<False>>>>();
			}
			else
			{
				query.WhereAnd<Where<PRBatch.payrollType.IsNotEqual<PayrollType.voidCheck>.And<PRBatch.open.IsEqual<False>>.And<PRBatch.closed.IsEqual<False>>>>();
			}

			return (PXResult<PRBatch, PRBatchEmployee>)query.SelectSingle(payPeriodID, payPeriodID, payGroupID, employeeID);
		}
		...
		private void SetCostAssignmentType(PRPayment payment)
		{
			...
			bool noCostAssigned = PRSetup.Current.ProjectCostAssignment == ProjectCostAssignmentType.NoCostAssigned;
			...
			CostAssignmentSetting earningSetting = new CostAssignmentSetting(noCostAssigned,
				earningsAssignedToProject, earningsAssignedToLaborItem, earningsAssignedToEarningType);
			...
			CostAssignmentSetting benefitSetting = new CostAssignmentSetting(noCostAssigned,
				benefitsAssignedToProject, benefitsAssignedToLaborItem, benefitsAssignedToEarningType);
			...
			CostAssignmentSetting taxSetting = new CostAssignmentSetting(noCostAssigned,
				taxesAssignedToProject, taxesAssignedToLaborItem, taxesAssignedToEarningType);
			...
			CostAssignmentSetting ptoSetting = new CostAssignmentSetting(noCostAssigned,
				ptoAssignedToProject, ptoAssignedToLaborItem, ptoAssignedToEarningType);
		...
		public virtual void InsertRecordOfEmployment(PXGraph graph, PRPayment payment)
		{
			...
			roe.CRAPayrollAccountNumber = PRTaxReportingAccount.PK.Find(graph, bAccount.BAccountID)?.CRAPayrollAccountNumber;
			...
			var insurableEarnings = SelectFrom<PRPaymentTax>
					...
					.Where<PRPayGroupPeriod.finPeriodID.IsLessEqual<P.AsString>
						.And<PRPayment.employeeID.IsEqual<P.AsInt>>
					...
					.OrderBy<Desc<PRPayment.transactionDate>>
					.View.Select(graph, payment.PayPeriodID, payment.EmployeeID);

			int numRecords = PayPeriodType.GetROENumberOfPeriods(roe.PeriodType);

			decimal totalInsurableHours = 0m;
			decimal totalInsurableEarnings = 0m;
			using (var insurableEarningsEnumerator = insurableEarnings.GetEnumerator())
			{
				for (int i = 0; i < numRecords && insurableEarningsEnumerator.MoveNext(); i++)
				{
						PXResult<PRPaymentTax, PRPayment, PRPayGroupPeriod> record = (PXResult<PRPaymentTax, PRPayment, PRPayGroupPeriod>)insurableEarningsEnumerator.Current;
		...
		public virtual void UpdateRecordOfEmploymentStatus(PRPayment payment)
		{
			foreach (PRRecordOfEmployment recordOfEmployment in SelectFrom<PRRecordOfEmployment>
							.Where<PRRecordOfEmployment.origRefNbr.IsEqual<P.AsString>>
							.OrderBy<PRRecordOfEmployment.refNbr.Desc>.View.Select(this, payment.RefNbr))
			{
		...
		public virtual void VoidCheckProc(PRPayment doc)
		{
					...
					And<PREarningDetail.paymentRefNbr.IsEqual<P.AsString>>>.View.Select(this, doc.DocType, doc.RefNbr))
			{
				...
				Earnings.Cache.SetDefaultExt<PREarningDetail.recordID>(copy);
				Earnings.Cache.SetStatus(copy, PXEntryStatus.Inserted);
