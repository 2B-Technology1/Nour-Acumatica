...
{
	public abstract class ShopifyPaymentsDataProvider : OrderRestDataProvider
	{
		protected BCSyncStatus OriginalBCSyncStatus { get; set; } = null;
		protected OrderData OriginalOrderData { get; set; } = null;
				
		protected ShopifyPaymentsDataProvider(IShopifyRestClient restClient) : base(restClient)
		{}
		...
		public static CCTranStatus GetCCTranStatus(OrderTransaction transaction)
		{
			...
			if ((transaction.Kind == TransactionType.Authorization || transaction.Kind == TransactionType.EmvAuthorization)
		...
		protected void SetTranType(TransactionData tranData, string PCtranType, TransactionStatus? PCtranStatus)
		{
		...
		public static bool GetCCTranTypeByAuthTran(string input, out CCTranType output)
		{
			bool res = true;
			output = 0;
			switch (input)
