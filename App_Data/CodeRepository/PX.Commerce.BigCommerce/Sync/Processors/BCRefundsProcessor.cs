...
namespace PX.Commerce.BigCommerce
{
	...
	public class BCRefundsProcessor : BCOrderBaseProcessor<BCRefundsProcessor, BCRefundsBucket, MappedRefunds>
	{
		...
		public override  async Task<EntityStatus> GetBucketForImport(BCRefundsBucket bucket, BCSyncStatus syncstatus, CancellationToken cancellationToken = default)
		{
			...
			orderData.Refunds = orderRefundsRestDataProvider.GetAll(orderData.Id.ToString(), cancellationToken)?.ToList() ?? new List<OrderRefund>();
			...
			orderData.OrdersCoupons = orderCouponsRestDataProvider.GetAll(syncstatus.ExternID, cancellationToken)?.ToList() ?? new List<OrdersCouponData>();
			orderData.OrderProducts = orderProductsRestDataProvider.GetAll(syncstatus.ExternID, cancellationToken)?.ToList() ?? new List<OrdersProductData>();
			orderData.Taxes = orderTaxesRestDataProvider.GetAll(syncstatus.ExternID, cancellationToken).ToList()?.ToList() ?? new List<OrdersTaxData>();
			orderData.Transactions = orderTransactionsRestDataProvider.GetAll(syncstatus.ExternID, cancellationToken)?.ToList() ?? new List<OrdersTransactionData>();
		...
		public override  async Task MapBucketImport(BCRefundsBucket bucket, IMappedEntity existing, CancellationToken cancellationToken = default)
		{
			...
			else if (mappedRefunds?.Local?.Status?.Value == PX.Objects.SO.Messages.Completed)
			{
				bucket.Refunds.Local = new SalesOrder();
				CreateRefundPayment(bucket, mappedRefunds);
				CreateRefundOrders(bucket, mappedRefunds, cancellationToken);
		...
		public virtual void CreateRefundOrders(BCRefundsBucket bucket, MappedRefunds existing, CancellationToken cancellationToken = default)
		{
			var currentBinding = GetBinding();
			var currentBindingExt = GetBindingExt<BCBindingExt>();
			...
			foreach (OrderRefund data in refunds)
			{
					...
					if (data.TotalTax > 0 && orderData.Taxes.Count > 0)// if acumatica original SO has tax and Refunds has tax then process tax
				{
					...
					foreach (OrdersTaxData tax in orderData.Taxes)
					{
						...
						var taxAmount = GetHelper<BCHelper>().RoundToStoreSetting(taxable * tax.Rate / 100);
						if (inserted == null)
						{
				...
				else
				{
					if (existing.Local.TaxDetails?.Count > 0 && data.TotalTax > 0 && orderData.Taxes.Count == 0)// In case of full refunds order taxes count become zero
					{
						...
						foreach (TaxDetail tax in existing.Local.TaxDetails)
						{
							...
							var taxAmount = GetHelper<BCHelper>().RoundToStoreSetting((decimal)(data.TotalTax * tax.TaxRate.Value / taxRateTotal)); // just get tax amount based on totaltax inrefund and taxrate from Acumatica salesorder
							if (inserted == null)
							{
								impl.TaxDetails.Add(new TaxDetail()
								{
				...
				if (order.RefundOrders != null)
				{
					foreach (var refundOrder in order.RefundOrders)
					{
						...
						if (refundOrder.Id == null)
						{
							...
							#region Taxes
							GetHelper<BCHelper>().ValidateTaxes(obj.SyncID, impl, refundOrder);
							#endregion
						}
