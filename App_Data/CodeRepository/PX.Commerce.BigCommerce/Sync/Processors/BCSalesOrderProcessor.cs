...
namespace PX.Commerce.BigCommerce
{
	...
	public class BCSalesOrderProcessor : BCOrderBaseProcessor<BCSalesOrderProcessor, BCSalesOrderBucket, MappedOrder>
	{
		...
		public override async Task<PullSimilarResult<MappedOrder>> PullSimilar(ILocalEntity entity, CancellationToken cancellationToken = default)
		{
			...
			OrderData data = orderDataProvider.GetByID(uniqueFieldValue);
			if (data == null) return null;
			...
			if (mapped is MappedOrder)
			{
				...
				if (operation == BCSyncOperationAttribute.ExternChanged && !order.IsNew && order?.Extern != null && status?.PendingSync == false)
				{
					...
					if (order.Extern.ItemsShipped > 0 && order.Extern.ItemsShipped < order.Extern.ItemsTotal)
					{
						...
						foreach (OrdersShipmentData shipment in orderShipmentsDataProvider.GetAll(order.ExternID, cancellationToken)?.ToList() ?? new List<OrdersShipmentData>())
						{
							DateTime? orderdate = order.Extern.DateModifiedUT.ToDate();
		...
		public override async Task<EntityStatus> GetBucketForImport(BCSalesOrderBucket bucket, BCSyncStatus syncstatus, CancellationToken cancellationToken = default)
		{
			...
			data.OrderProducts = orderProductsRestDataProvider.GetAll(syncstatus.ExternID, cancellationToken)?.ToList() ?? new List<OrdersProductData>();
			data.Shipments = orderShipmentsDataProvider.GetAll(syncstatus.ExternID, cancellationToken)?.ToList() ?? new List<OrdersShipmentData>();
			data.OrdersCoupons = orderCouponsRestDataProvider.GetAll(syncstatus.ExternID, cancellationToken)?.ToList() ?? new List<OrdersCouponData>();
			data.Taxes = orderTaxesRestDataProvider.GetAll(syncstatus.ExternID,cancellationToken).ToList() ?? new List<OrdersTaxData>();
			data.OrderShippingAddresses = orderShippingAddressesRestDataProvider.GetAll(syncstatus.ExternID, cancellationToken)?.ToList() ?? new List<OrdersShippingAddressData>();
			data.Transactions = orderTransactionsRestDataProvider.GetAll(syncstatus.ExternID,cancellationToken)?.ToList() ?? new List<OrdersTransactionData>();
			data.MetaFields = orderMetaFieldDataProvider.GetAll(null, data.Id.ToString(), cancellationToken).ToList() ?? new List<OrdersMetaFieldData>();

			FilterCustomers filter = new FilterCustomers { Include = "addresses" };
			data.Customer = data.CustomerId != 0 ? customerDataProviderV3.GetByID(data.CustomerId?.ToString(), filter, cancellationToken) : null;

			if (data.StatusId == (int)OrderStatuses.Refunded || data.StatusId == (int)OrderStatuses.PartiallyRefunded || data.RefundedAmount > 0)
				data.Refunds = orderRefundsRestDataProvider.GetAll(data.Id.ToString(), cancellationToken)?.ToList() ?? new List<OrderRefund>();
		...
		public override async Task SaveBucketImport(BCSalesOrderBucket bucket, IMappedEntity existing, String operation, CancellationToken cancellationToken = default)
		{
			...
			else
			{
				...
				#region Taxes
				GetHelper<BCHelper>().ValidateTaxes(obj.SyncID, impl, obj.Local);
		...
		public override async Task SaveBucketExport(BCSalesOrderBucket bucket, IMappedEntity existing, String operation, CancellationToken cancellationToken = default)
		{
			...
			if (obj.ExternID != null && existing != null && obj.Local.ExternalOrderOriginal?.Value == true) // Update status only if order is unmodified
			{
				...
				data = orderDataProvider.Update(data, obj.ExternID);
			}
			else
			{
				...
				{
					data = orderDataProvider.Update(obj.Extern, obj.ExternID);
				}
				...
				List<OrdersProductData> products = orderProductsRestDataProvider.GetAll(data.Id?.ToString(), cancellationToken)?.ToList() ?? new List<OrdersProductData>();
				DetailInfo[] oldDetails = obj.Details.ToArray(); obj.ClearDetails();
				if (data.ShippingAddressCount > 0)
				{
					var ShippingAddresses = orderShippingAddressesRestDataProvider.GetAll(data.Id?.ToString(), cancellationToken)?.ToList() ?? new List<OrdersShippingAddressData>();
			...
			if (GetBindingExt<BCBindingExt>().SyncOrderNbrToStore == true)
			{
				obj.Extern.MetaFields = orderMetaFieldDataProvider.GetAll(new FilterOrderMetaField { Key = BCObjectsConstants.ImportedInAcumatica, NameSpace = BCObjectsConstants.Namespace_Global }, data.Id.ToString(), cancellationToken).ToList() ?? new List<OrdersMetaFieldData>();
