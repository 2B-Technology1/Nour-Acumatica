...
namespace PX.Objects.Localizations.GB
{
	...
	// Copy done due to need to use the same functionality as is those classes, except for mandatory padding of text up to line size.
	// Also, there should not be an empty line at the end of the file.
	// TODO: on merge the classes should be refactored to avoid copying of the code

	/// <summary>
	/// Data provider class for the BACS export scenario specific to the HSBC bank in the UK.
	/// </summary>
	public class BACSHSBCProvider : BaseSchemaPaymentSYProvider, IPXSYProvider
	{
		...
		public override void Export(string objectName, PXSYTable table, bool breakOnError, Action<SyProviderRowResult> callback)
		{
			...
			if (table.Count > 0 || table.Columns.Contains(FileID))
			{
				...
				if (fileIndex < 0)
				{
					throw new PXException(PX.DataSync.Messages.ACHColumnNotFound, FileID);
				}
	...
	public class BACSWriter: BaseWriter
	{
		...
		public virtual string Extensiton
		{
			get;
		}
