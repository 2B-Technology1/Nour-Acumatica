...
namespace PX.Objects.Localizations.GB.HMRC
{
    static class Helper
    {
        ...
        public static string GetVendorIPAddress()
        {
			...
			{
				PXTrace.WriteInformation("The HTTP used insted of HTTPS because of the error retured by the HTTPS endpoint:" + e.Message);
				ipaddress = new WebClient().DownloadString("http://api.ipify.org");
			}

			IPAddress address;
			bool isValidAddress = IPAddress.TryParse(ipaddress, out address) && address.ToString() == ipaddress;
