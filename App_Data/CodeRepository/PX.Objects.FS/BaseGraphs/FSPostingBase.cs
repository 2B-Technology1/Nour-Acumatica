...
namespace PX.Objects.FS
{
        ...
        where TGraph : PXGraph
    {
		...
		{
			DocGenerationHelper.ValidateAutoNumbering(Base, numberingID);
		}
		#endregion
                                            ...
                                            bool isService)
        {
            ...
            if ((tranARRow != null && tranARRow.AccountID != null) || (tranAPRow != null && tranAPRow.AccountID != null) || (tranSORow != null && tranSORow.SalesAcctID != null))
            {
				int? siteID = tranARRow?.SiteID ?? tranAPRow?.SiteID ?? tranSORow?.SiteID;

				SharedClasses.SubAccountIDTupla subAcctIDs = SharedFunctions.GetSubAccountIDs(graph, fsSrvOrdTypeRow, inventoryID, branchID, customerLocationID, branchLocationID, salesPersonID, siteID);
                                                                      ...
                                                                      FSPostRegister postRegister)
        {
            ...
            if (entityType == FSPostRegister.entityType.Values.APPOINTMENT)
            {
				...
				appt.SkipExternalTaxCalculation = true;
