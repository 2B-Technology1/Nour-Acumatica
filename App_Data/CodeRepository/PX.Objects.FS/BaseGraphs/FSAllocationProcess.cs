...
namespace PX.Objects.FS
{
    ...
    public class FSAllocationProcess : PXGraph<FSAllocationProcess>
    {
        public static FSAllocationProcess SingleFSDeallocateProcess =>
            (FSAllocationProcess)PXContext.GetSlot<FSAllocationProcess>() ??
            PXContext.SetSlot<FSAllocationProcess>(CreateInstance<FSAllocationProcess>());
        ...
        public virtual void ReallocateItemIntoCurrentLineSplit(ServiceOrderEntry graph, AllocationInfo allocationInfo, int recursionLevel = 0)
        {
            ...
            {
				 FSSODet fsSoDet = graph.ServiceOrderDetails.Select().RowCast<FSSODet>().Where(d => d.RefNbr == allocationInfo.RefNbr
																				&& d.SrvOrdType == allocationInfo.SrvOrdType
																				&& d.SODetID == allocationInfo.SODetID)
																		.FirstOrDefault();

                if ((recursionLevel == 0 && allocationInfo.LotSerialNbr == null) || (fsSoDet != null && fsSoDet.IsInventoryItem != true))
                {
                    return;
                }
                else
        ...
        public virtual void CreateAndAssignPlan(PXCache splitCache, FSSODetSplit openSplit)
        {
            ...
            INItemPlan plan = splitCache.Graph.FindImplementation<FSSODetSplitPlan>().DefaultValues(openSplit);
