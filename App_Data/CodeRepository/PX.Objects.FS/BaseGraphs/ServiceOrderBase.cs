...
namespace PX.Objects.FS
{
        ...
        where TPrimary : class, PX.Data.IBqlTable, new()
    {
        ...
        public PXSetup<FSBillingCycle,
			InnerJoin<FSCustomerBillingSetup, On<FSCustomerBillingSetup.billingCycleID, Equal<FSBillingCycle.billingCycleID>>,
			CrossJoin<FSSetup>>,
			Where<FSCustomerBillingSetup.customerID, Equal<Current<FSServiceOrder.billCustomerID>>,
				And<Where2<
					Where<FSSetup.customerMultipleBillingOptions, Equal<True>,
						And<FSCustomerBillingSetup.srvOrdType, Equal<Current<FSServiceOrder.srvOrdType>>>>,
					Or<Where<FSSetup.customerMultipleBillingOptions, Equal<False>,
						And<FSCustomerBillingSetup.srvOrdType, IsNull>>>>>>> BillingCycleRelated;
        #endregion
                                                                ...
                                                                bool forceAppointmentCheckings)
        {
            ...
            if (e.Operation == PXDBOperation.Insert || e.Operation == PXDBOperation.Update)
            {
				...
				var projectTasksChanged = serviceOrderDetails.Select().RowCast<FSSODet>().Any(x => x.ProjectTaskID != (int?)serviceOrderDetails.Cache.GetValueOriginal<FSSODet.projectTaskID>(x));
				var projectChanged = fsServiceOrderRow.ProjectID != (int?)cacheServiceOrder.GetValueOriginal<FSServiceOrder.projectID>(fsServiceOrderRow);
				var branchChanged = fsServiceOrderRow.BranchID != (int?)cacheServiceOrder.GetValueOriginal<FSServiceOrder.branchID>(fsServiceOrderRow);
				var curyChanged =  fsServiceOrderRow.CuryID != (string)cacheServiceOrder.GetValueOriginal<FSServiceOrder.curyID>(fsServiceOrderRow);
				var billServiceContractChanged = fsServiceOrderRow.BillServiceContractID != (int?)cacheServiceOrder.GetValueOriginal<FSServiceOrder.billServiceContractID>(fsServiceOrderRow);

				if ((curyChanged || projectChanged || branchChanged || projectTasksChanged || billServiceContractChanged || detailsForUpdate.Any()) && serviceOrderAppointments.Any())
                    {
                        ...
                        {
						if (fsAppointmentRowBeingSaved?.AppointmentID == fsAppointmentRow.AppointmentID)
                            {
							continue;
						}
                                ...
                                if (curyChanged)
								{
									fsAppointmentRow_local.CuryID = fsServiceOrderRow.CuryID;
									graphAppointmentEntry.AppointmentRecords.Update(fsAppointmentRow_local);
								}

						fsAppointmentRow_local.ProjectID = fsServiceOrderRow.ProjectID;
						fsAppointmentRow_local.DfltProjectTaskID = null;
                                fsAppointmentRow_local.BranchID = fsServiceOrderRow.BranchID;
                                graphAppointmentEntry.AppointmentRecords.Update(fsAppointmentRow_local);

						graphAppointmentEntry.AppointmentRecords.Cache.SetValueExtIfDifferent<FSAppointment.billServiceContractID>(fsAppointmentRow_local, fsServiceOrderRow.BillServiceContractID);

						var relatedAppointmentDetails = graphAppointmentEntry.AppointmentDetails.Select().RowCast<FSAppointmentDet>().Where(x => x.AppointmentID == fsAppointmentRow.AppointmentID);
						foreach (FSAppointmentDet relatedAppointmentDetail in relatedAppointmentDetails)
						{
							relatedAppointmentDetail.FSSODetRow = serviceOrderDetails.Select().Single(x => x.Record.SODetID == relatedAppointmentDetail.SODetID);
								}

						graphAppointmentEntry.UpdateDetailsFromProjectTaskID(fsServiceOrderRow);
                ...
                UpdateServiceCounts(fsServiceOrderRow, fsSODetList.Where(x => x.IsService == true));
		...
		protected bool HaveAnyBilledAppointmentsInServiceOrder(PXGraph graph, int? SOID )
		{
			return PXSelect<FSAppointment,
				...
				And<FSAppointment.status, Equal<AppointmentStatus.billed>>>>.
				Select(graph, SOID).Any();
		}
                ...
                where DAC : class, IBqlTable, IFSSODetBase, new()
        {
            ...
            FSAppointmentDet fsAppointmentDetRow = null;
			bool disableUnitCost = false;

			if (dacType == typeof(FSAppointmentDet))
            {
                fsAppointmentDetRow = (FSAppointmentDet)eRow;
                calledFromAPP = true;
				disableUnitCost = Caches[typeof(TPrimary)].Current != null &&
									(((FSAppointment)Caches[typeof(TPrimary)].Current).Status == ListField.AppointmentStatus.Canceled ||
									((FSAppointment)Caches[typeof(TPrimary)].Current).Status  == ListField.AppointmentStatus.Closed);
			}
            else
            {
                ...
                calledFromSO = true;
				disableUnitCost = Caches[typeof(TPrimary)].Current != null &&
									(((FSServiceOrder)Caches[typeof(TPrimary)].Current).Status == ListField.ServiceOrderStatus.Canceled ||
									((FSServiceOrder)Caches[typeof(TPrimary)].Current).Status == ListField.ServiceOrderStatus.Closed ||
									((FSServiceOrder)Caches[typeof(TPrimary)].Current).Mem_Invoiced == true ||
									((FSServiceOrder)Caches[typeof(TPrimary)].Current).AllowInvoice == true);
			}
			...
			SetEnabledAndPersistingCheck<FSSODet.manualDisc>(cache, eRow, eventType, isEnabled, persistingCheck: null);
            ...
            SetEnabledAndPersistingCheck<FSSODet.estimatedQty>(cache, eRow, eventType, isEnabled || IsImport, persistingCheck: null);
            ...
            if (PXAccess.FeatureInstalled<FeaturesSet.inventory>())
            {
                ...
                SetEnabledAndPersistingCheck<FSSODet.siteLocationID>(cache, eRow, eventType, isStockItem && isEnabled,
					persistingCheck: isEnabled && row.LineType == ID.LineType_ALL.INVENTORY_ITEM ? PXPersistingCheck.NullOrBlank : PXPersistingCheck.Nothing );
            }
            ...
            SetEnabledAndPersistingCheck<FSSODet.acctID>(cache, eRow, eventType, isEnabled || IsImport, persistingCheck);
			...
			isEnabled = isEnabled || (!disableUnitCost && (row.LineType == ID.LineType_ALL.NONSTOCKITEM || row.LineType == ID.LineType_ALL.SERVICE));

			SetEnabledAndPersistingCheck<FSSODet.curyUnitCost>(cache, eRow, eventType, isEnabled || IsImport, persistingCheck: null);
            ...
            where CuryUnitPrice : class, IBqlField
        {
            ...
            {
				bool alwaysFromBaseCurrency = ARSalesPriceMaint.SingleARSalesPriceMaint.GetAlwaysFromBaseCurrencySetting(cache);
                                                   ...
                                                   bool allowCustomerChange = false)
        {
			...
			bool enableContractPeriod = enableServiceContract && billingBy == ID.Billing_By.SERVICE_ORDER;
            ...
            bool enableAllowInvoice = billingBy == ID.Billing_By.SERVICE_ORDER
                                        && fsSrvOrdTypeRow.Behavior != FSSrvOrdType.behavior.Values.InternalAppointment
        ...
        public virtual object Get_IFSSODetBase_SubID_DefaultValue(PXCache cache, IFSSODetBase fsSODetRow, FSServiceOrder fsServiceOrderRow, FSSrvOrdType fsSrvOrdTypeRow, FSAppointment fsAppointmentRow = null)
        {
            ...
            try
            {
                value = SubAccountMaskAttribute.MakeSub<FSSrvOrdType.combineSubFrom>(
                            ...
                            new Type[] { typeof(FSBranchLocation.subID), typeof(Location.cMPSalesSubID), typeof(InventoryItem.salesSubID), typeof(Location.cSalesSubID), typeof(INPostClass.salesSubID), typeof(SalesPerson.salesSubID), typeof(FSSrvOrdType.subID), typeof(INSite.salesSubID)});
        ...
        public virtual void CreatePrepaymentDocument(FSServiceOrder fsServiceOrderRow, FSAppointment fsAppointmentRow, out PXGraph target, string paymentType = ARPaymentType.Payment)
        {
            ...
            {
                arPaymentRow.CuryOrigDocAmt = fsServiceOrderRow.SOCuryUnpaidBalanace ?? 0m;
            }
            else
            {
                arPaymentRow.CuryOrigDocAmt = graphARPaymentEntry
                    ...
                    .CuryConvCury(fsServiceOrderRow.SOUnpaidBalanace.Value);
            }
		...
		public virtual void UpdateServiceOrderUnboundFields(FSServiceOrder fsServiceOrderRow, FSAppointment fsAppointmentRow, bool disableServiceOrderUnboundFieldCalc)
		{
			string billingBy = GetBillingMode(fsServiceOrderRow);
			ServiceOrderEntry.UpdateServiceOrderUnboundFields(this, fsServiceOrderRow, fsAppointmentRow, billingBy, disableServiceOrderUnboundFieldCalc, true);
		}

		public virtual string GetBillingMode(FSServiceOrder fsServiceOrderRow)
		{
			FSSrvOrdType srvOrdType;
			if (this is ServiceOrderEntry)
			{
				var graph = this as ServiceOrderEntry;
				srvOrdType = graph.ServiceOrderTypeSelected.Current;
			}
			else
			{
				var graph = this as AppointmentEntry;
				srvOrdType = graph.ServiceOrderTypeSelected.Current;
			}

			string result = ServiceOrderEntry.GetBillingMode(this, BillingCycleRelated.Current, srvOrdType, fsServiceOrderRow);
			return result;
		}
                                                                ...
                                                                bool enableByLineType)
        {
            ...
            bool enableActualDuration = enableByLineType && enableByTimeBehavior;
                                                              ...
                                                              FSAppointment fsAppointmentRow)
        {
			...
			bool isProjectSpecified = fsAppointmentRow?.ProjectID != null && !ProjectDefaultAttribute.IsNonProject(fsAppointmentRow?.ProjectID);
            ...
            PXUIFieldAttribute.SetEnabled<FSAppointmentLog.projectTaskID>(cache, fsLogRow, noEmployee == false && isProjectSpecified);
		...
		{
			UpdateStaffRelatedUnboundFields(fsAppointmentDetServiceRow, null, logView, null);
		}
		...
		public virtual void ValidateContact(FSServiceOrder row)
		{
			...
			string errorMessage = string.Format(TX.Error.OperationCannotPerformDueToContactDeactivated, contact?.DisplayName);

			if (contact?.ContactType == ContactTypesAttribute.Employee)
			{
				BAccount employee = SelectFrom<BAccount>
					.Where<BAccount.parentBAccountID.IsEqual<@P.AsInt>
						.And<BAccount.defContactID.IsEqual<@P.AsInt>>>
					.View.Select(this, contact.BAccountID, contact.ContactID)
					.SingleOrNull();

				if (employee?.VStatus != ContactStatus.Active)
				{
					throw new PXException(errorMessage, PXErrorLevel.Error);
				}
			}
			else if (contact?.Status != ContactStatus.Active)
				throw new PXException(errorMessage, PXErrorLevel.Error);
		}
        ...
        public virtual List<FSProfitability> ProfitabilityRecords_INItems(PXGraph graph, FSServiceOrder serviceOrder, FSAppointment appointment)
        {
            ...
            { 
                ...
                {
                    result.Add(PrepareProfitabilityRow(row, billingBy));
                }
            }
