...
namespace PX.Objects.FS
{
        ...
        where TWhere : class, IBqlWhere, new()
    {
        ...
        public PXSelectJoin<FSSalesPrice,
                        ...
                        InnerJoin<FSServiceContract,
                            On<FSServiceContract.serviceContractID, Equal<FSSalesPrice.serviceContractID>>>>,
                        Where<FSServiceContract.serviceContractID, Equal<Current<FSServiceContract.serviceContractID>>>> SalesPriceLines;
            ...
            )
        {
            ...
            if (!string.IsNullOrEmpty(reportID))
            {
                ...
                foreach (FSServiceContract contract in list)
                {
					...
					actualReportID = new NotificationUtility(this).SearchCustomerReport(reportID, contract.CustomerID, contract.BranchID);
        ...
        public virtual IEnumerable CopyContract(PXAdapter adapter)
        {
            ...
            if ((dialogResult == WebDialogResult.OK || (this.IsContractBasedAPI && dialogResult == WebDialogResult.Yes)) && CopyContractFilter.Current.StartDate != null)
            {
				...
				delegate ()
				{
					...
					try
					{
						this.CopyContractProc(contract, CopyContractFilter.Current?.StartDate, CopyContractFilter.Current?.RefNbr);
					}
					finally
					{
					...
					foreach (FSServiceContract order in orders)
					{
						...
						try
						{
							...
							if (scEntry != null && orders[0].RecordType == ID.RecordType_ServiceContract.SERVICE_CONTRACT)
							{
								...
								scEntry.Value.GetExtension<ServiceContractEntryBase_ActivityDetailsExt>().SendNotification("Contract", notificationCD, order.BranchID, parameters);
							...
							else if (rcEntry != null && orders[0].RecordType == ID.RecordType_ServiceContract.ROUTE_SERVICE_CONTRACT)
							{
								...
								rcEntry.Value.GetExtension<ServiceContractEntryBase_ActivityDetailsExt>().SendNotification("Contract", notificationCD, order.BranchID, parameters);
		...
		public virtual void EnableDisable_Document(PXCache cache, FSServiceContract fsServiceContractRow)
        {
			...
			bool enableInsertUpdateAndImport = enableInsertUpdate || IsImport;

			PXUIFieldAttribute.SetEnabled<FSContractPeriodFilter.actions>(this.ContractPeriodFilter.Cache, this.ContractPeriodFilter.Current, enableInsertUpdateAndImport);
            PXUIFieldAttribute.SetEnabled<FSContractPeriodFilter.postDocRefNbr>(this.ContractPeriodFilter.Cache, this.ContractPeriodFilter.Current, enableInsertUpdateAndImport);
            PXUIFieldAttribute.SetEnabled<FSContractPeriodFilter.standardizedBillingTotal>(this.ContractPeriodFilter.Cache, this.ContractPeriodFilter.Current, enableInsertUpdateAndImport);
            PXUIFieldAttribute.SetEnabled<FSContractPeriodFilter.contractPeriodID>(this.ContractPeriodFilter.Cache, this.ContractPeriodFilter.Current, enableInsertUpdateAndImport);

			PXUIFieldAttribute.SetVisible<FSCopyContractFilter.refNbr>(this.CopyContractFilter.Cache, this.CopyContractFilter.Current, RefNbrRequired());
		...
		public virtual void DeleteScheduledAppSO(FSServiceContract fsServiceContractRow, DateTime? cancelationDate)
        {
            if (fsServiceContractRow != null && cancelationDate != null)
            {
                ...
                foreach (FSServiceOrder fsServiceOrderRow in fsServiceOrderRows)
                {
					...
					var billingBy = graphServiceOrderEntry.GetBillingMode(serviceOrder);
                ...
                foreach (FSAppointment fsAppointmentRow in fsAppointmentRows)
                {
					...
					var billingBy = graphAppointmentEntry.GetBillingMode(graphAppointmentEntry.ServiceOrderRelated.Current);
		...
		{
			bool refNbrRequired = RefNbrRequired();

			if (refNbrRequired && !RefNbrIsValid(refNbr, out string errorMessage))
				throw new PXException(errorMessage);
					...
					if (sourceContract.SourcePrice == ID.SourcePrice.CONTRACT)
					{
						...
						foreach (FSSalesPrice sourcePeriodPrice in sourcePeriodPrices)
						{
							foreach (FSSalesPrice salesPrice in SalesPriceLines.Select().RowCast<FSSalesPrice>().Where(x => x.InventoryID == sourcePeriodPrice.InventoryID))
							{
								...
								contractPrice.tstamp = this.TimeStamp;
		...
		{
			if (SetupRecord.Current != null &&
				SetupRecord.Current.ServiceContractNumberingID != null)
			{
				PX.Objects.CS.Numbering numbering = (PX.Objects.CS.Numbering)PXSelect<PX.Objects.CS.Numbering,
							Where<PX.Objects.CS.Numbering.numberingID, Equal<Required<PX.Objects.CS.Numbering.numberingID>>>>
							.Select(this, SetupRecord.Current.ServiceContractNumberingID);

				return numbering.UserNumbering == true;
			}
		...
		private bool RefNbrIsValid(string refNbr, out string errorMessage)
		{
			...
			{
				var displayName = PXUIFieldAttribute.GetDisplayName<FSCopyContractFilter.refNbr>(CopyContractFilter.Cache);
				errorMessage = PXLocalizer.LocalizeFormat(TX.Error.RefNbrCannotBeEmpty, displayName);
				return false;
			}

			if (ServiceContractSelected.Current != null &&
				refNbr == ServiceContractSelected.Current.RefNbr)
			{
				errorMessage = TX.Error.RefNbrCannotBeTheSame;
				return false;
			}

			var serviceContractExists = PXSelect<FSServiceContract,
				Where<FSServiceContract.refNbr, Equal<Required<FSServiceContract.refNbr>>>>
				.Select(this, refNbr)
				.Any();
			...
			{
				errorMessage = TX.Error.RefNbrAlreadyExists;
				return false;
			}
		...
		public class ServiceContractEntryBase_ActivityDetailsExt : ActivityDetailsExt<ServiceContractEntryBase<TGraph, TPrimary, TWhere>, FSServiceContract, FSServiceContract.noteID>
		{
			...
			public override string GetPrimaryRecipientFromContext(NotificationUtility utility, string type, object row, NotificationSource source)
			{
				...
				var contract = Base.Caches[typeof(FSServiceContract)].Current as FSServiceContract;

				if (contract == null || source == null || string.IsNullOrEmpty(contract.EmailNotificationCD))
					return null;

				NotificationSetup setup = new NotificationUtility(Base).SearchSetup(type, contract.EmailNotificationCD);

				PXResultset<NotificationSetupRecipient> notificationRecipientSet
					= PXSelect<
							NotificationSetupRecipient,
						Where<
							NotificationSetupRecipient.setupID, Equal<Required<NotificationSetupRecipient.setupID>>,
							And<NotificationSetupRecipient.active, Equal<True>>>>
						.Select(Base, setup.SetupID);
				...
				foreach (NotificationSetupRecipient recipient in notificationRecipientSet)
				{
					...
					switch (recipient.ContactType)
					{
						case FSContractContactType.Contact:
							...
							contact = PXSelect<Contact, Where<Contact.contactID, Equal<Required<Contact.contactID>>>>.Select(Base, contract.CustomerContactID);
						...
						case FSContractContactType.Billing:
							...
							contact = PXSelectJoin<
								...
								Where<
									Customer.bAccountID, Equal<Required<Customer.bAccountID>>>>
								.Select(Base, contract.BillCustomerID);
						...
						case FSContractContactType.Customer:
							...
							contact = PXSelectJoin<
								...
								Where<
									Customer.bAccountID, Equal<Required<Customer.bAccountID>>>>
								.Select(Base, contract.CustomerID);
						...
						case FSContractContactType.Salesperson:
							...
							PXResult<SalesPerson, EPEmployee, BAccount, Contact> bqlResult = (PXResult<SalesPerson, EPEmployee, BAccount, Contact>)
								PXSelectJoin<
									SalesPerson,
								...
								.Select(Base, contract.SalesPersonID);
						...
						case FSContractContactType.Employee:
							...
							contact = PXSelect<Contact, Where<Contact.contactID, Equal<Required<Contact.contactID>>>>.Select(Base, recipient.ContactID);
					...
					return PXDBEmailAttribute.ToString(recipients);
				}
