...
namespace PX.Objects.FS
{
    public static class SharedFunctions
    {
        ...
        public static bool ShowWarningScheduleNotProcessed(PXCache cache, FSSchedule fsScheduleRow)
        {
            ...
            if (anyGenerationProcess == false)
            {
                cache.RaiseExceptionHandling<FSSchedule.refNbr>(fsScheduleRow,
                                                                ...
                                                                new PXSetPropertyException(TX.Warning.SCHEDULE_WILL_NOT_AFFECT_SYSTEM_UNTIL_GENERATION_OCCURS, PXErrorLevel.Warning));
            }
        ...
        {
			UpdateEquipmentFields(graph, cache, row, inventoryID, null, updateQty);
		}

		public static void UpdateEquipmentFields(PXGraph graph, PXCache cache, Object row, int? inventoryID, FSSrvOrdType fsSrvOrdTypeRow, bool updateQty = true)
        {
            if (!PXAccess.FeatureInstalled<FeaturesSet.equipmentManagementModule>())
            {
                return;
            ...
            SetEquipmentActionFromInventory(graph, row, inventoryID, fsSrvOrdTypeRow);
        ...
        {
			SetEquipmentActionFromInventory(graph, row, inventoryID, null);
		}

		public static void SetEquipmentActionFromInventory(PXGraph graph, object row, int? inventoryID, FSSrvOrdType fsSrvOrdTypeRow)
        {
            ...
            else 
            {
                ...
                {
                    equipmentAction = getEquipmentModelAction(graph, fsxEquipmentModelRow, fsSrvOrdTypeRow);
                }
            }
        ...
        {
			return getEquipmentModelAction(graph, fsxEquipmentModelRow, null);
		}

		public static string getEquipmentModelAction(PXGraph graph, FSxEquipmentModel fsxEquipmentModelRow, FSSrvOrdType fsSrvOrdTypeRow)
        {
		...
		[Obsolete("This method is replaced by GetSubAccountIDs overload method with new siteID parameter")]
        public static SharedClasses.SubAccountIDTupla GetSubAccountIDs(PXGraph graph, FSSrvOrdType fsSrvOrdTypeRow, int? inventoryID, int? branchID, int? locationID, int? branchLocationID, int? salesPersonID, bool isService = true)
        {
			...
			return GetSubAccountIDsTuple(graph, fsSrvOrdTypeRow, branchID, locationID, branchLocationID, salesPersonID, inventoryItemRow, inventoryItemRow?.DfltSiteID);
		}

		public static SharedClasses.SubAccountIDTupla GetSubAccountIDs(PXGraph graph, FSSrvOrdType fsSrvOrdTypeRow, int? inventoryID, int? branchID, int? locationID, int? branchLocationID, int? salesPersonID, int? siteID)
		{
			InventoryItem inventoryItemRow = SharedFunctions.GetInventoryItemRow(graph, inventoryID);

			return GetSubAccountIDsTuple(graph, fsSrvOrdTypeRow, branchID, locationID, branchLocationID, salesPersonID, inventoryItemRow, siteID);
		}

		private static SharedClasses.SubAccountIDTupla GetSubAccountIDsTuple(PXGraph graph, FSSrvOrdType fsSrvOrdTypeRow, int? branchID, int? locationID, int? branchLocationID, int? salesPersonID, InventoryItem inventoryItemRow, int? inSiteID)
		{
            INSite inSite = null;
		...
		public static List<SM_ARReleaseProcess.ItemInfo> GetDifferentItemList(PXGraph graph, ARTran arTran, bool createDifferentEntriesForQtyGreaterThan1)
		{
			...
			else if (arTran.SOShipmentType != null && arTran.SOShipmentNbr != null
			...
			{
				IEnumerable<SOShipLineSplit> lotSerialSplits;
				if (arTran.SOShipmentLineNbr != null)
				{
					lotSerialSplits = PXSelect<SOShipLineSplit,
				...
				{
					lotSerialSplits = SelectFrom<SOShipLineSplit>
						.Where<SOShipLineSplit.shipmentNbr.IsEqual<P.AsString>
							.And<SOShipLineSplit.inventoryID.IsEqual<P.AsInt>>
							.And<SOShipLineSplit.origOrderType.IsEqual<P.AsString>>
							.And<SOShipLineSplit.origOrderNbr.IsEqual<P.AsString>>
							.And<SOShipLineSplit.origLineNbr.IsEqual<P.AsInt>>>
						.View
						.Select(graph,
			...
			else if (arTran.SOOrderType != null && arTran.SOOrderNbr != null && arTran.SOOrderLineNbr != null)
			{
				...
				foreach (SOLineSplit soLineSplit in lotSerialSplits.RowCast<SOLineSplit>())
				{
					if (string.IsNullOrEmpty(soLineSplit.LotSerialNbr) &&
						(soLineSplit.POCreate == true && soLineSplit.POSource != INReplenishmentSource.DropShipToOrder))
					{
						continue;
					}
