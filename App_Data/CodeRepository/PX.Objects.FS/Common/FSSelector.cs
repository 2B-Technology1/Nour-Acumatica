...
namespace PX.Objects.FS
{
    ...
    public class FSSelectorAppointmentSODetIDAttribute : PXSelectorAttribute
    {
		...
		public override void SubstituteKeyFieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
		{
			...
			FSAppointmentDet detail = PXSelect<FSAppointmentDet,
						Where<FSAppointmentDet.appointmentID, Equal<Current<FSAppointment.appointmentID>>,
							And<Where<FSAppointmentDet.lineRef, Equal<Required<FSAppointmentDet.lineRef>>>>>
					>
				.SelectWindowed(sender.Graph, 0, 1, value);

			if (detail != null)
			{
				e.ReturnValue = detail.LineRef;
			}
			else if (e.Row != null)
			{
				e.ReturnValue = null;
    ...
    public class FSSelectorExtensionMaintenanceEquipmentAttribute : PXSelectorAttribute
    {
		...
		public FSSelectorExtensionMaintenanceEquipmentAttribute(Type customerID, Type withNonActiveRecords)
            : base(BqlTemplate.OfCommand<
                    ...
                    Or<
						Where2<
							Where<FSEquipment.status, Equal<FSEquipment.status.Active>, Or<BqlPlaceholder.A, Equal<True>>>,
                        And<
                            Where2<
                                Where<FSEquipment.ownerType, Equal<FSEquipment.ownerType.Customer>,
										And<FSEquipment.ownerID, Equal<Current<BqlPlaceholder.B>>>>,
                                Or<Where<FSEquipment.ownerType, Equal<FSEquipment.ownerType.OwnCompany>,
										And<FSEquipment.customerID, Equal<Current<BqlPlaceholder.B>>>>>>>>>>>>
					.Replace<BqlPlaceholder.A>(withNonActiveRecords)
					.Replace<BqlPlaceholder.B>(customerID)
                    .ToType(),
        SelectorBase_Equipment.selectorColumns)
        {
            SubstituteKey = typeof(FSEquipment.refNbr);
    ...
    public class FSSelectorEquipmentLineRefSOInvoiceAttribute : PXSelectorAttribute
    {
		...
		public FSSelectorEquipmentLineRefSOInvoiceAttribute(Type withNonActiveRecords)
            : base(
				BqlTemplate.OfCommand<Search2<FSEquipmentComponent.lineNbr,
                       InnerJoin<InventoryItem,
                            On<InventoryItem.inventoryID, Equal<FSEquipmentComponent.inventoryID>>>,
                       Where2<
						   Where2<
							   Where<FSEquipmentComponent.status, Equal<FSEquipmentComponent.status.Active>, Or<BqlPlaceholder.A, Equal<True>>>,
    ...
    public class FSINLotSerialNbrAttribute : PXCustomSelectorAttribute
    {
        ...
        public class ApptINLotSerialStatus : PX.Data.IBqlTable, IStatus
        {
            ...
            {
                public static ApptINLotSerialStatus Find(PXGraph graph, int? inventoryID, int? subItemID, int? siteID, int? locationID, string lotSerialNbr, int? sODetID, PKFindOptions options = PKFindOptions.None)
                    => FindBy(graph, inventoryID, subItemID, siteID, locationID, lotSerialNbr, sODetID, options);
            }
            ...
            public static ApptINLotSerialStatus GetApptINLotSerialStatus(PXCache inCache, INLotSerialStatusByCostCenter inLotSerialStatus, int? soDetID, PXCache apptCache, ref List<string> lotSerialStatusFields)
            {
        ...
        public virtual void AddLotSerialsFromIN(FSAppointmentDet apptLine, List<ApptINLotSerialStatus> lotSerialList)
        {
            ...
            var lotSerials = PXSelectJoin<INLotSerialStatusByCostCenter,
                        InnerJoin<INSiteLotSerial, On<
                            INSiteLotSerial.inventoryID, Equal<INLotSerialStatusByCostCenter.inventoryID>,
                        ...
                        Where<
                            INLotSerialStatusByCostCenter.inventoryID, Equal<Required<INLotSerialStatusByCostCenter.inventoryID>>,
                            ...
                            And<Where<
                                    INLotSerialStatusByCostCenter.locationID, Equal<Required<INLotSerialStatusByCostCenter.locationID>>,
                                    Or<Required<INLotSerialStatusByCostCenter.locationID>, IsNull>>>>>>>.
                        Select(_Graph,
            ...
            foreach (PXResult<INLotSerialStatusByCostCenter, INSiteLotSerial> result in lotSerials)
            {
                var inLotSerialStatus = (INLotSerialStatusByCostCenter)result;
                var inSiteLotSerial = (INSiteLotSerial)result;

                if (existingLotSerials.Contains(inLotSerialStatus.LotSerialNbr, StringComparer.OrdinalIgnoreCase) == true)
                {
                    continue;
                }

                var lotSerialRow = ApptINLotSerialStatus.GetApptINLotSerialStatus(_Graph.Caches[typeof(INLotSerialStatusByCostCenter)], inLotSerialStatus, -1, ApptINLotSerialStatusView.Cache, ref LotSerialStatusFields);
        ...
        public virtual void AddLotSerialsFromSrvOrdSplit(FSAppointmentDet apptLine, List<ApptINLotSerialStatus> lotSerialList)
        {
            ...
            var lotSerials = PXSelectJoin<INLotSerialStatusByCostCenter,
                        ...
                        InnerJoin<FSSODetSplit, On<
                            FSSODetSplit.inventoryID, Equal<INLotSerialStatusByCostCenter.inventoryID>,
                            And<FSSODetSplit.siteID, Equal<INLotSerialStatusByCostCenter.siteID>,
                            And<FSSODetSplit.lotSerialNbr, Equal<INLotSerialStatusByCostCenter.lotSerialNbr>>>>>>,
                        Where<
                            INLotSerialStatusByCostCenter.inventoryID, Equal<Required<INLotSerialStatusByCostCenter.inventoryID>>,
                            And<INLotSerialStatusByCostCenter.subItemID, Equal<Required<INLotSerialStatusByCostCenter.subItemID>>,
                            And2<
            ...
            foreach (PXResult<INLotSerialStatusByCostCenter, INSiteLotSerial, FSSODetSplit> result in lotSerials)
            {
                var inLotSerialStatus = (INLotSerialStatusByCostCenter)result;
                var inSiteLotSerial = (INSiteLotSerial)result;
                var fsSODetSplit = (FSSODetSplit)result;

                var lotSerialRow = ApptINLotSerialStatus.GetApptINLotSerialStatus(_Graph.Caches[typeof(INLotSerialStatusByCostCenter)], inLotSerialStatus, relatedSrvOrdLine.SODetID, ApptINLotSerialStatusView.Cache, ref LotSerialStatusFields);
    ...
    public class FSSiteAvailAttribute : SiteAvailAttribute
    {
        ...
        public FSSiteAvailAttribute(Type InventoryType, Type SubItemType)
            : base(InventoryType, SubItemType, typeof(CostCenter.freeStock))
        {
        }
        #endregion
        ...
        public override void FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
        {
            ...
            {
				var oldValue = sender.GetValue(e.Row, _FieldName);
				if (oldValue == null)
