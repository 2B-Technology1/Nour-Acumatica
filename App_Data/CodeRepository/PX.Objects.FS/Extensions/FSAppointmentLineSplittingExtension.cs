...
namespace PX.Objects.FS
{
	...
	public class FSAppointmentLineSplittingExtension : LineSplittingExtension<AppointmentEntry, FSAppointment, FSAppointmentDet, FSApptLineSplit>
	{
		...
		protected override void EventHandlerInternal(ManualEvent.Row<FSAppointmentDet>.Updated.Args e)
		{
			...
			else
			{
				...
				if (string.IsNullOrEmpty(e.Row.LotSerialNbr) == false
						&& !string.Equals(e.Row.LotSerialNbr, e.OldRow.LotSerialNbr, StringComparison.OrdinalIgnoreCase)
		...
		protected override void EventHandler(ManualEvent.Row<FSAppointmentDet>.Persisting.Args e)
		{
			...
			)
			{
				...
				if (lotSerTrack.IsIn(INLotSerTrack.SerialNumbered, INLotSerTrack.LotNumbered))
				{
					...
					existingSplitTotalQty = INUnitAttribute.ConvertFromTo<FSAppointmentDet.inventoryID>
					(e.Cache, e.Row, inventoryItem.BaseUnit, e.Row.UOM, existingSplitTotalQty, INPrecision.QUANTITY);
		...
		{
			if (!string.Equals(e.Row.LotSerialNbr, e.OldRow.LotSerialNbr, StringComparison.OrdinalIgnoreCase) && e.Row.LotSerialNbr != null && e.Row.Operation == SO.SOOperation.Issue)
		...
		protected override void AppendSerialStatusCmdWhere(PXSelectBase<INLotSerialStatusByCostCenter> cmd, FSAppointmentDet apptLine, INLotSerClass lotSerClass)
		{
				...
				foreach (FSSODetSplit soSplit in soSplitsWithLotSerial)
				{
					...
					if (soSplitBalance > 0m)
					{
						apptSplit = existingSplits.Find(x =>
							...
							string.IsNullOrEmpty(soSplit.LotSerialNbr) == false ||
		...
		protected virtual void UpdateLotSerialSplitsBasedOnLineLotSerial(FSAppointmentDet apptLine, string lotSerTrack, bool? lotSerTrackExpiration)
		{
			...
			{
				if (string.Equals(split.LotSerialNbr, apptLine.LotSerialNbr, StringComparison.OrdinalIgnoreCase))
				{
