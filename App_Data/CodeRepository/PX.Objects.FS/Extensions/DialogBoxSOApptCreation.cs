...
namespace PX.Objects.FS
{
        ...
        where TMain : class, IBqlTable, new()
    {
            ...
            bool createAppointment)
        {
			...
			bool isSourceDocumentCROpportunity = header.sourceDocument is CROpportunity;
			bool skipAddressReassigningForOpportunity = false;

			if (PXAccess.FeatureInstalled<FeaturesSet.projectAccounting>())
			{
				ProjectSettingsManager settings = new ProjectSettingsManager();
				skipAddressReassigningForOpportunity = settings.CalculateProjectSpecificTaxes && isSourceDocumentCROpportunity;
			}
			...
			var numID = srvOrdGraph.ServiceOrderTypeSelected.SelectSingle()?.SrvOrdNumberingID;
			DocGenerationHelper.ValidateAutoNumbering(Base, numID);
			...
			if (isSourceDocumentCROpportunity)
			{
				serviceOrderRow.AllowOverrideContactAddress = ((CROpportunity)header.sourceDocument).AllowOverrideContactAddress;
			}
			else
			{
            ...
            #endregion
            #region Address
			if (header.Address != null && !skipAddressReassigningForOpportunity) 
            {
            ...
            foreach (DBoxDetails line in details)
            {
                if (line.InventoryID != null)
                {
					...
					if (line.EstimatedDuration != null)
					{
                    ...
                    srvOrdLine.POCreate = line.POCreate == true ? GetPOCreate(Base, line, fsSrvOrdTypeRow) : false;
		...
		{
			if (fsSrvOrdTypeRow.PostTo != ID.SrvOrdType_PostTo.NONE && fsSrvOrdTypeRow.PostTo != ID.SrvOrdType_PostTo.ACCOUNTS_RECEIVABLE_MODULE)
			{
				return true;
			}
