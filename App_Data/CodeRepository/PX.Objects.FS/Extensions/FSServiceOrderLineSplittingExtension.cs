	...
	public class FSServiceOrderLineSplittingExtension : LineSplittingExtension<ServiceOrderEntry, FSServiceOrder, FSSODet, FSSODetSplit>
	{
		...
		protected virtual void EventHandler(ManualEvent.Row<FSSODetSplit>.Selected.Args e)
		{
			if (e.Row != null)
			{
				...
				PXUIFieldAttribute.SetEnabled<FSSODetSplit.isAllocated>(e.Cache, e.Row, isLineTypeInventory && isIssue && !isCompleted && parent?.POCreate != true);
		...
		public override PXSelectBase<INLotSerialStatusByCostCenter> GetSerialStatusCmd(FSSODet line, PXResult<InventoryItem, INLotSerClass> item)
		{
			PXSelectBase<INLotSerialStatusByCostCenter> cmd = new
			...
			switch (item.GetItem<INLotSerClass>().LotSerIssueMethod)
			{
				...
				case INLotSerIssueMethod.Expiration:
					cmd.OrderByNew<OrderBy<Asc<INLocation.pickPriority, Asc<INLotSerialStatusByCostCenter.expireDate, Asc<INLotSerialStatusByCostCenter.lotSerialNbr>>>>>();
					break;
