...
namespace PX.Objects.FS
{
    public class SM_APInvoiceEntry : FSPostingBase<APInvoiceEntry>
    {
		...
		[PXOverride]
		public virtual IEnumerable ReverseInvoice(PXAdapter adapter, Func<PXAdapter, IEnumerable> baseMethod)
		{
			if (Base.Document.Current.DocType == APDocType.DebitAdj)
			{
				if (Base.Transactions.Select().RowCast<APTran>().Select(Base.Transactions.Cache.GetExtension<FSxAPTran>).Any(_ =>
				_.RelatedDocNoteID != null
				|| !string.IsNullOrEmpty(_.RelatedEntityType)
				|| !string.IsNullOrEmpty(_.AppointmentRefNbr)
				|| !string.IsNullOrEmpty(_.ServiceOrderRefNbr)
					))
					throw new PXException(AP.Messages.DebitAdjustmentReferecesFSEntities);
			}
