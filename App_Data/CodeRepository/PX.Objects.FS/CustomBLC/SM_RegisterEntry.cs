...
namespace PX.Objects.FS
{
	public class SM_RegisterEntry : FSPostingBase<RegisterEntry>
	{
		...
		public override void CreateInvoice(PXGraph graphProcess, List<DocLineExt> docLines, short invtMult, DateTime? invoiceDate, string invoiceFinPeriodID, OnDocumentHeaderInsertedDelegate onDocumentHeaderInserted, OnTransactionInsertedDelegate onTransactionInserted, PXQuickProcess.ActionFlow quickProcessFlow)
		{
			...
			IEnumerable<FSLog> logRows = fsAppointmentRow != null ?
				PXSelect<FSLog,
				Where<FSLog.docID, Equal<Required<FSAppointment.appointmentID>>>,
				OrderBy<Asc<FSLog.dateTimeBegin>>>
				.Select(Base, fsAppointmentRow.AppointmentID)
				.RowCast<FSLog>()
				:
				Enumerable.Empty<FSLog>();
			...
			var docLinesBillable = docLines
				.Select(l => new
				{
					LineType = (l.fsAppointmentDet?.LineType ?? l.fsSODet.LineType),
					Line = l
				})
				.Where(l => l.LineType.IsIn(ID.LineType_ALL.SERVICE, ID.LineType_ALL.NONSTOCKITEM, ID.LineType_ALL.INVENTORY_ITEM))
				.ToList();

			if (docLinesBillable.Count == 0)
				return;
			...
			foreach (var billableLine in docLinesBillable)
			{
				var docLineExt = billableLine.Line;
				...
				{
					var relatedLogs = logRows.Where(x => x.DetLineRef == fsAppointmentDetRow.LineRef);
					pmTranRow.Date = relatedLogs.Any() ? relatedLogs.First().DateTimeBegin : fsAppointmentRow.ActualDateTimeBegin;
				}
				else
				{
				...
				bool isStockLine = billableLine.LineType == ID.LineType_ALL.INVENTORY_ITEM;
				bool isServiceLine = billableLine.LineType == ID.LineType_ALL.SERVICE;
				...
				pmTranRow.Billable = !isStockLine;
				...
				pmTranRow.Qty = isStockLine ? 0.0M : docLine.GetQty(FieldType.BillableField);
				...
				pmTranRow.TranCuryAmount = isStockLine ? 0.0M : docLine.GetTranAmt(FieldType.BillableField) * invtMult;
				pmTranRow.AccountGroupID = fsSrvOrdTypeRow.AccountGroupID;
				pmTranRow.Description = isStockLine ? TX.Messages.DummyProjectTransactionDescription : docLine.TranDesc;

				bool lineHasOnlyOneStaffMember = fsAppointmentDetRow?.StaffID != null;
				bool isProject = fsAppointmentRow != null && ProjectDefaultAttribute.IsProject(Base, fsAppointmentRow.ProjectID);
				bool employeeRestrictionVerified = true;

				if (isProject && isServiceLine && lineHasOnlyOneStaffMember)
				{
					employeeRestrictionVerified = VerifyEmployeeRestriction(fsAppointmentRow.ProjectID, fsAppointmentDetRow.StaffID);
				}

				if (isServiceLine &&
					lineHasOnlyOneStaffMember &&
					(!isProject || employeeRestrictionVerified))
				{
					pmTranRow.ResourceID = fsAppointmentDetRow.StaffID;
				}
		...
		private bool VerifyEmployeeRestriction(int? projectID, int? staffID)
		{
			...
			if (project.RestrictToEmployeeList == true)
			{
				EPEmployeeContract employeeContract = SelectFrom<EPEmployeeContract>
													.Where<EPEmployeeContract.contractID.IsEqual<@P.AsInt>
													.And<EPEmployeeContract.employeeID.IsEqual<@P.AsInt>>>
													.View
													.Select(Base, projectID, staffID);
