...
namespace PX.Objects.FS
{
    public class SM_POOrderEntry : PXGraphExtension<POOrderEntry>
    {
		...
		public class FSSODetSplitPlanSyncOnly : IN.GraphExtensions.ItemPlanSyncOnly<POOrderEntry, FSSODetSplit>
		{
        ...
        protected virtual void _(Events.RowPersisted<POOrder> e)
        {
            ...
            if (e.TranStatus == PXTranStatus.Open && e.Operation == PXDBOperation.Update)
            {
                ...
                if (poOrderOldStatus != poOrderRow.Status)
                {
					...
					if (poOrderRow.Status == POOrderStatus.Open)
					{
						FSPOReceiptProcess.UpdateSrvOrdApptLineUnitCost(cache.Graph, poOrderRow);
					}
				}
            }
        }
		#endregion
		#region INItemPlan
		protected virtual void _(Events.RowDeleted<INItemPlan> e)
		{
			if (e.Row == null)
			{
				return;
			...
			if (inItemPlanRow.PlanType == INPlanConstants.PlanF7 || inItemPlanRow.PlanType == INPlanConstants.PlanF8)
			{
				...
				if (inItemPlanRow != null && inItemPlanRow.SupplyPlanID != null)
				{
					...
					// Acuminator disable once PX1043 SavingChangesInEventHandlers [Changes are not persisted via this call for POOrderEntry graph]
					FSPOReceiptProcess.UpdatePOReferenceInSrvOrdLine(Base, FSSODetFixedDemand,
						...
						null, false, e.Cache, inItemPlanRow, true);
				}
			}
		}
		...
		public void _(Events.CacheAttached<FSSODetSplit.planID> e) { }
	}
}
