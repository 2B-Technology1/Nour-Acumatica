...
namespace PX.Objects.FS
{
    public class SM_INReleaseProcess : PXGraphExtension<INReleaseProcess>
    {
        ...
        public virtual void AddNewSODet(List<SrvOrder> srvOrderList, int? sOID, string srvOrdType, int? sODetID, string serialNbr, decimal? curyUnitCost)
        {
            ...
            if (current != null)
            {
                ...
                else 
                {
                    ...
                    {
                        SODetSplit soDetSplit = currentDet.Splits.Find(x => string.Equals(x.LotSerialNbr, serialNbr, StringComparison.OrdinalIgnoreCase));
		...
		public class FSSODetSplitPlanSyncOnly : IN.GraphExtensions.ItemPlanSyncOnly<INReleaseProcess, FSSODetSplit>
		{
		...
		/// <summary>
		/// Overrides <see cref="INReleaseProcess.ReleaseDocProc(JournalEntry, INRegister, bool)"/>
		/// </summary>
        [PXOverride]
        public virtual void ReleaseDocProc(JournalEntry je, INRegister doc, bool releaseFromHold, ReleaseDocProcDelegate del)
        {
            ...
            if (fsPostRegisterRow.PostedTO == ID.Batch_PostTo.SI)
            {
                if (fsPostRegisterRow.EntityType == ID.PostDoc_EntityType.SERVICE_ORDER)
                {
                    var soInvoiceINTranRows = PXSelectJoin<INTran,
                                        ...
                                        LeftJoin<FSPostDet,
                                            On<FSPostDet.postID, Equal<FSSODet.postID>>>>>,
                                    Where<
            ...
            { 
                ...
                foreach (INTranProcess inTranProcessRow in inTranProcessList) 
                {
                    ...
                    if (fsSODetRow != null && fsSODetRow.SODetID != null)
                    {
                        ...
                        {
                            if (SharedFunctions.IsLotSerialRequired(soGraph.ServiceOrderDetails.Cache, split.InventoryID) == false
								|| string.Equals(split.LotSerialNbr, inTranRow.LotSerialNbr, StringComparison.OrdinalIgnoreCase)
                                || (inTranSplitRow != null && string.Equals(split.LotSerialNbr, inTranSplitRow.LotSerialNbr, StringComparison.OrdinalIgnoreCase)))
                            {
                    ...
                    { 
                        ...
                        {
                            if (SharedFunctions.IsLotSerialRequired(appGraph.AppointmentDetails.Cache, split.InventoryID) == false
								|| string.Equals(split.LotSerialNbr, inTranRow.LotSerialNbr, StringComparison.OrdinalIgnoreCase)
                                || (inTranSplitRow != null && string.Equals(split.LotSerialNbr, inTranSplitRow.LotSerialNbr, StringComparison.OrdinalIgnoreCase)))
                            {
                                split.UnitCost = inTranRow.UnitCost;
            ...
            foreach (SrvOrder srvOrder in srvOrderList)
            {
                ...
                foreach (SODet detail in srvOrder.Details)
                {
                                                        ...
                                                        .Select(soGraph, fsSODetRow.SrvOrdType, fsSODetRow.RefNbr, fsSODetRow.LineNbr)) 
                    {
                        ...
                        {
                            if (detail.Splits != null && detail.Splits.Exists(x => string.Equals(x.LotSerialNbr, fsSODetSplitRow.LotSerialNbr, StringComparison.OrdinalIgnoreCase)))
                            {
                                fsSODetSplitRow.CuryUnitCost = detail.Splits.Find(x => string.Equals(x.LotSerialNbr, fsSODetSplitRow.LotSerialNbr, StringComparison.OrdinalIgnoreCase)).CuryUnitCost;
