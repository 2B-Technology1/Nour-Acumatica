...
using PX.Data.BQL;
using System.Linq;
using Messages = SP.Objects.Messages;
...
namespace SP.Objects.AR
{
	...
	public class ARDocumentEnqExt : PXGraphExtension<ARDocumentEnq>
	{
		...
		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXCustomizeBaseAttribute(typeof(PXUIFieldAttribute), nameof(PXUIFieldAttribute.DisplayName), "Currency Document Total")]
		protected virtual void _(Events.CacheAttached<ARDocumentEnq.ARDocumentResult.curyOrigDocAmt> e)
		{
		}
		#endregion
		#region CuryDocBal
		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXCustomizeBaseAttribute(typeof(PXUIFieldAttribute), nameof(PXUIFieldAttribute.DisplayName), "Currency Open Balance")]

		protected virtual void _(Events.CacheAttached<ARDocumentEnq.ARDocumentResult.curyDocBal> e)
		{
		}
		#endregion
		#region CuryDocBal
		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXCustomizeBaseAttribute(typeof(PXUIFieldAttribute), nameof(PXUIFieldAttribute.Required), false)]

		protected virtual void _(Events.CacheAttached<ARDocumentEnq.ARDocumentResult.curyID> e)
		{
		}
		#endregion
		#endregion
		...
		public override void Initialize()
		{
			...
			Base.Actions["ARAgedPastDueMCReportportal"].SetEnabled(customerExists);
		}
		...
		[PXButton(Category = Messages.ReportActionCategory)]
		...
		public PXAction<ARDocumentEnq.ARDocumentFilter> aRAgedPastDueMCReportportal;
		[PXUIField(DisplayName = "Aging Report MC", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select, FieldClass = "Multicurrency")]
		[PXButton(Category = Messages.ReportActionCategory)]
		public virtual IEnumerable ARAgedPastDueMCReportportal(PXAdapter adapter)
		{
			ARDocumentEnq.ARDocumentFilter filter = Base.Filter.Current;
			if (filter != null)
			{
				...
				parameters["CustomerID"] = currentCustomer.AcctCD;
				parameters["OrgBAccountID"] = bAccount?.AcctCD;
				throw new PXReportRequiredException(parameters, "AR631100", PX.Objects.AR.Messages.ARAgedPastDueReportMC);
			}
			return adapter.Get();
		}
		...
		protected virtual void ARDocumentFilter_RowSelected(PXCache sender, PXRowSelectedEventArgs e, PXRowSelected sel)
		{
			...
			rowExt.CuryOpenInvoiceAndCharge = 0;
			rowExt.CreditMemosandUnappliedPayment = 0;
			rowExt.CuryCreditMemosandUnappliedPayment = 0;
			...
			foreach (ARDocumentEnq.ARDocumentResult record in Base.Documents.Select())
			{
					...
					record.DocType == ARDocType.CreditMemo)
				{
					...
					rowExt.CuryOpenInvoiceAndCharge += record.CuryDocBal;
				}
				else if (record.DocType == ARDocType.Payment || record.DocType == ARDocType.Prepayment)
				{
					...
					rowExt.CuryCreditMemosandUnappliedPayment += record.CuryDocBal;
				}
			}
			...
			if (Base.Filter.Current != null)
			{
				...
				rowExt.CuryNetBalance = Base.Filter.Current.CuryBalanceSummary;
			}
			if (currentCustomer != null)
			{
			...
			bool isNotBaseCurrency = row.CuryID != null && row.CuryID != currentCustomer?.BaseCuryID;
			PXUIFieldAttribute.SetVisible<ARDocumentFilterExt.curyCreditMemosandUnappliedPayment>(sender, null, isNotBaseCurrency);
			PXUIFieldAttribute.SetVisible<ARDocumentFilterExt.curyOpenInvoiceAndCharge>(sender, null, isNotBaseCurrency);
			PXUIFieldAttribute.SetVisible<ARDocumentFilterExt.curyNetBalance>(sender, null, isNotBaseCurrency);
