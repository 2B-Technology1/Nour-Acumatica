    ...
    public class InventoryCardMaint : PXGraph<InventoryCardMaint>
    {
        ...
        public void CalculateDiscountPriceCard(PortalCardLines row, decimal price)
        {
	        ...
	        DiscountEngine.SetLineDiscountOnly(DocumentDetails.Cache, row,
                                new DiscountLineFields
									...
									DiscountLineFields.skipLineDiscounts>(DocumentDetails.Cache, row),
                                    row.BaseDiscountID,
                                    price,
                                    row.Qty * price,
                                    row.Qty,
        ...
        public void CalculateInfo(PortalCardLine currentcart)
        {
            ...
            if (currentcart != null)
            {
				...
				string curyID = currentCustomer.CuryID ?? currentCustomer.BaseCuryID;
				if (curyID == null)
				{
					var branch = PX.Objects.GL.Branch.PK.Find(this, PortalSetup.Current.SellingBranchID);
					curyID = branch?.BaseCuryID;
				}

				currentcart.CurrencyStatus = curyID;
