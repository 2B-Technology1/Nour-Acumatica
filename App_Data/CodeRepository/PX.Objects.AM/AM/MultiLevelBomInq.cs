...
namespace PX.Objects.AM
{
    ...
    public class MultiLevelBomInq : PXGraph<MultiLevelBomInq>
    {
        ...
        protected virtual AMMultiLevelBomData CreateDetailRow(AMBomMatl amBomMatl, AMBomOper amBomOper,
            ...
            AMMultiLevelBomFilter filter, string levelBomid, string levelRevisionID, AMBomMatlCury bomMatlCury)
        {
            ...
            var row = new AMMultiLevelBomData
            {
                ...
                ExtCost = qtyRequired * bomMatlCury?.UnitCost ?? 0,
                TotalExtCost = totalQtyRequired * bomMatlCury?.UnitCost ?? 0,
        ...
        public virtual void LoadDataRecords(string levelBomid, string levelRevisionID, int? level, decimal? totalQtyReq, AMBomItem parentBomItem, List<AMMultiLevelBomData> multiLevelBomRecs)
        {
			...
			if (costRollGraph == null)
				costRollGraph = CreateInstance<BOMCostRoll>();
            ...
            foreach (PXResult<AMBomMatl, AMBomItem, AMBomOper, InventoryItem, INItemCost, INItemSite, AMBomMatlCury> result in PXSelectJoin<
                AMBomMatl,
                InnerJoin<AMBomItem,
                ...
                .Select(this, levelBomid, levelRevisionID))
            {
				...
				this.Caches<INItemCost>().Hold(itemCost);
				...
				{
					var newUnitCost = costRollGraph.GetUnitCost(invItem, itemSite, itemSite.SiteID);
					// when tran unit cost is zero (most likely avg or fifo with no qty on hand) the above method will return zero.
					row.UnitCost = (newUnitCost == 0m && itemCost.TranUnitCost == null ? bomMatlCury?.UnitCost : newUnitCost) ?? 0m;
				}
        ...
        public virtual List<AMMultiLevelBomData> RollCostUpdate(List<AMMultiLevelBomData> multiLevelBomRecs)
        {
			...
			var multiLevelBomResults = new List<MultiLevelBomResult>();
...
                    continue;
				...
				var inventoryItem = InventoryItem.PK.Find(this, multiLevelRecord.InventoryID);
				var multiLevelBomResult = new MultiLevelBomResult(multiLevelRecord.Level.GetValueOrDefault(), bomItem, inventoryItem, null);
                multiLevelBomResults.Add(multiLevelBomResult);
            }
            ...
            costRollGraph.RollCosts(multiLevelBomResults);
            ...
            foreach (var mutliLevelRecord in multiLevelBomRecs)
            {
                ...
                if (!string.IsNullOrWhiteSpace(mutliLevelRecord.ManufacturingBOMID) && !string.IsNullOrWhiteSpace(mutliLevelRecord.ManufacturingRevisionID))
                {
                    bomCostRec = (AMBomCost)costRollGraph.Caches<AMBomCost>().Locate(new AMBomCost
                    {
						...
						CuryID = costRollGraph.Accessinfo.BaseCuryID
					});
                }
