...
namespace PX.Objects.AM
{
    ...
    {
        public PXSelectJoin<AMDisassembleBatch,
			InnerJoinSingleTable<Branch, On<AMDisassembleBatch.branchID, Equal<Branch.branchID>>>,
			Where<AMDisassembleBatch.docType, Equal<AMDocType.disassembly>,
				And<Branch.baseCuryID, Equal<Current<AccessInfo.baseCuryID>>>>> Document;
        ...
        {
            GL.OpenPeriodAttribute.SetValidatePeriod<AMDisassembleBatch.finPeriodID>(Document.Cache, null, GL.PeriodValidation.DefaultSelectUpdate);
        ...
        protected virtual void AMDisassembleBatch_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
        {
            ...
            if (!sender.ObjectsEqual<AMDisassembleTran.tranDate>(e.Row, e.OldRow))
            {
                foreach (AMDisassembleTran result in MaterialTransactionRecords.Select())
                {
                    MaterialTransactionRecords.Cache.MarkUpdated(result);
                }
            }
        }
