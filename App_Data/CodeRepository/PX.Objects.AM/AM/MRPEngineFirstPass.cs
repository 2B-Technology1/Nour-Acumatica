    ...
    public class MRPEngineFirstPass
    {
        ...
        public virtual void LoadInventoryStockAdjustments(MRPProcessCache.MrpItemDictionary mrpItemDictionary)
        {
            ...
            foreach (PXResult<INLocationStatusByCostCenter, INLocation, InventoryItem, INSite> result in PXSelectJoinGroupBy<INLocationStatusByCostCenter,
                    ...
                    Sum<INLocationStatusByCostCenter.qtyOnHand>>>>>>.Select(MrpGraph))
            {
        ...
        {
			IEnumerable<string> orderTypes = SelectFrom<SOOrderType>.Where<SOOrderType.behavior.IsEqual<SOBehavior.bL>
				.And<SOOrderType.active.IsEqual<True>>>
				.View.Select(MrpGraph)
				.RowCast<SOOrderType>().Select(o => o.OrderType);

			var hasBlanket = orderTypes != null && !orderTypes.Empty_();
                ...
                >.Select(MrpGraph))
            {
                ...
                foreach (Tuple<DateTime, DateTime> interval in intervals)
                {
                    ...
                    {
                        row.OrderQtyConsumed = ForecastQtyConsumed(forecast, beginDate, endDate) + (hasBlanket ? ForecastBlanketOrderQtyConsumed(forecast, beginDate, endDate) : 0m);
                    }
		...
		public virtual void LoadBlanketSalesOrders()
		{
			...
			if (orderTypes == null || orderTypes.Empty_())
			{
				return;
			}

			foreach (PXResult<SimpleBlanketLineSplit, InventoryItem, INSite> result in SelectFrom<SimpleBlanketLineSplit>
				.InnerJoin<InventoryItem>.On<InventoryItem.inventoryID.IsEqual<SimpleBlanketLineSplit.inventoryID>>
				...
				.And<InventoryItem.planningMethod.IsEqual<INPlanningMethod.dRP>.Or<InventoryItem.planningMethod.IsEqual<INPlanningMethod.mRP>>>
				...
				.View.Select(MrpGraph))
			{
				...
				{
					InsertBlanketException(simpleBlanketLineSplit, inventoryItem, MRPExceptionType.Delete);
					continue;
				}

				if (simpleBlanketLineSplit.SchedShipDate == null)
				{
					InsertBlanketException(simpleBlanketLineSplit, inventoryItem, MRPExceptionType.OrderWithoutSchedDate);
					continue;
				}
        ...
        [PXProjection(typeof(Select2<INItemPlan,
            ...
            Where<InventoryItem.stkItem, Equal<True>,
                And<Where<InventoryItem.planningMethod, Equal<INPlanningMethod.dRP>,
                    Or<InventoryItem.planningMethod, Equal<INPlanningMethod.mRP>>>>>>), Persistent = false)]
        [PXHidden]
        [Serializable]
		...
		[PXHidden]
		[Serializable]
		public class SimpleBlanketLineSplit : IBqlTable
		{
			#region Keys
			...
			[PXFormula(typeof(Sub<Sub<SimpleBlanketLineSplit.qty, SimpleBlanketLineSplit.shippedQty>, SimpleBlanketLineSplit.qtyOnOrders>))]
			...
			[PXDBQuantity(BqlField = typeof(SOLineSplit.qtyOnOrders))]
