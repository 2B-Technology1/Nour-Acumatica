            ...
            using (new DisableSelectorValidationScope(Base.Caches<AMBomOper>()))
            {
                ...
                foreach (AMBomOper fromRow in fromRows)
                {
					...
					foreach(PXResult<AMBOMCurySettings> fromCury in SelectFrom<AMBOMCurySettings>.Where<AMBOMCurySettings.bOMID.IsEqual<@P.AsString>
						...
						.And<AMBOMCurySettings.lineType.IsEqual<BOMCurySettingsLineType.operation>>>.View.Select(Base, fromRow.BOMID, fromRow.RevisionID, fromRow.OperationID))
					{
						var toCury = PXCache<AMBOMCurySettings>.CreateCopy(fromCury);
						toCury.BOMID = newBOMID;
						toCury.RevisionID = newRevisionID;
						Base.Caches<AMBOMCurySettings>().Insert(toCury);
					}
				}
            }
        }
                ...
                .Select(Base, sourceBOM.BOMID, sourceBOM.RevisionID))
            {
                ...
                try
                {
					...
					foreach (PXResult<AMBomMatlCury> fromCury in SelectFrom<AMBomMatlCury>.Where<AMBomMatlCury.bOMID.IsEqual<@P.AsString>
						...
						.And<AMBomMatlCury.lineType.IsEqual<BOMCurySettingsLineType.material>>>.View.Select(Base, fromRow.BOMID, fromRow.RevisionID, fromRow.OperationID, fromRow.LineID))
					{
						var toCury = PXCache<AMBomMatlCury>.CreateCopy(fromCury);
						toCury.BOMID = newBOMID;
						toCury.RevisionID = newRevisionID;
						Base.Caches<AMBomMatlCury>().Insert(toCury);
					}
				}
        ...
        protected virtual void CopyBomTool(AMBomItem sourceBOM, string newBOMID, string newRevisionID, bool copyNotes)
        {
            ...
            foreach (AMBomTool fromRow in fromRows)
            {
				...
				foreach (PXResult<AMBomToolCury> fromCury in SelectFrom<AMBomToolCury>.Where<AMBomToolCury.bOMID.IsEqual<@P.AsString>
					...
					.And<AMBomToolCury.lineType.IsEqual<BOMCurySettingsLineType.tool>>>.View.Select(Base, fromRow.BOMID, fromRow.RevisionID, fromRow.OperationID, fromRow.LineID))
				{
					var toCury = PXCache<AMBomToolCury>.CreateCopy(fromCury);
					toCury.BOMID = newBOMID;
					toCury.RevisionID = newRevisionID;
					Base.Caches<AMBomToolCury>().Insert(toCury);
				}
			}
        }
