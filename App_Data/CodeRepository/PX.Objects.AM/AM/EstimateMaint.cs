...
namespace PX.Objects.AM
{
    ...
    public class EstimateMaint : PXRevisionableGraph<EstimateMaint, AMEstimateItem, AMEstimateItem.estimateID, AMEstimateItem.revisionID>
    {
            ...
            string newRevisionID, bool newRevIsPrimary)
        {
            ...
            try
            {
				...
				TransferCache<AMEstimateStep>(copyGraph, toGraph);
				...
				toGraph.Documents.Current.SubcontractCost = copyGraph.Documents.Current.SubcontractCost.GetValueOrDefault();
            }
            finally
            {
        ...
        {
            foreach (PXResult<AMProdOper, AMWC, AMWCCury> operResult in PXSelectJoin<
                AMProdOper,
                LeftJoin<AMWC,
                    On<AMProdOper.wcID, Equal<AMWC.wcID>>,
				LeftJoin<AMWCCury,
					On<AMWCCury.wcID, Equal<AMWC.wcID>,
						And<AMWCCury.curyID, Equal<Current<AccessInfo.baseCuryID>>>>>>,
                ...
                .Select(this, prodItem.OrderType, prodItem.ProdOrdID))
            {
				...
				var WCCury = (AMWCCury)operResult;
				...
				newEstOper.WorkCenterStdCost = WCCury?.StdCost ?? 0;
        ...
        {
            foreach (PXResult<AMBomOper, AMWC, AMWCCury> operResult in PXSelectJoin<
                AMBomOper,
                LeftJoin<AMWC, On<AMBomOper.wcID, Equal<AMWC.wcID>>,
                ...
                .Select(this, bomItem.BOMID, bomItem.RevisionID))
            {
				...
				var bomWCCury = (AMWCCury)operResult;
                ...
                newEstOper.WorkCenterStdCost = bomWCCury?.StdCost ?? 0;
        ...
        {
            foreach (PXResult<AMBomMatl, AMBomOper, InventoryItem, AMBomMatlCury> result in PXSelectJoin<AMBomMatl,
                InnerJoin<AMBomOper,
				...
				LeftJoin<AMBomMatlCury, On<AMBomMatlCury.bOMID, Equal<AMBomMatl.bOMID>,
                ...
                .Select(this, bomItem.BOMID, bomItem.RevisionID))
            {
                ...
                var inventoryItem = (InventoryItem) result;
				var bomMatlCury = (AMBomMatlCury)result;
                ...
                var newEstMatl = new AMEstimateMatl
                {
                    ...
                    UOM = bomMatl.UOM,
                    UnitCost = bomMatlCury?.UnitCost ?? 0,
        ...
        {
            foreach (PXResult<AMBomTool, AMBomOper, AMBomToolCury> result in PXSelectJoin<AMBomTool,
                InnerJoin<AMBomOper,
				...
				LeftJoin<AMBomToolCury, On<AMBomToolCury.bOMID, Equal<AMBomTool.bOMID>,
					And<AMBomToolCury.revisionID, Equal<AMBomTool.revisionID>,
					And<AMBomToolCury.operationID, Equal<AMBomTool.operationID>,
					And<AMBomToolCury.lineID, Equal<AMBomTool.lineID>,
					And<AMBomToolCury.curyID, Equal<Current<AccessInfo.baseCuryID>>>>>>>>>,
                ...
                .Select(this, bomItem.BOMID, bomItem.RevisionID))
            {
                ...
                var bomOper = (AMBomOper) result;
				var bomToolCury = (AMBomToolCury)result;
                ...
                var newEstRow = new AMEstimateTool
                {
                    ...
                    UnitCost = bomToolCury?.UnitCost ?? 0
                };
                SetCurrentEstimateOperation(newEstRow);
                EstimateOperToolRecords.Insert(newEstRow);
            }
        ...
        {
            foreach (PXResult<AMProdOvhd, AMProdOper, AMOverhead, AMOverheadCurySettings> result in PXSelectJoin<AMProdOvhd,
                InnerJoin<AMProdOper,
        ...
        {
            foreach (PXResult<AMBomOvhd, AMBomOper, AMOverhead, AMOverheadCurySettings> result in PXSelectJoin<
                AMBomOvhd,
                InnerJoin<AMBomOper,
			...
			if (CreateProductionOrderFilter.AskExt() == WebDialogResult.OK)
            {
                ...
                PXLongOperation.StartOperation(this, delegate
                {
                    ...
                    PXTrace.WriteInformation("Estimate '{0}' Revision '{1}' Created production order {2} {3}",
                        curDocument.EstimateID.TrimIfNotNullEmpty(),
                        curDocument.RevisionID.TrimIfNotNullEmpty(),
                        prodItem.OrderType.TrimIfNotNullEmpty(),
                        prodItem.ProdOrdID.TrimIfNotNullEmpty());

					InsertEstimateHistory(curDocument.EstimateID,curDocument.RevisionID,
						Messages.GetLocal(Messages.EstimateCreatedProdOrder,
                                prodItem.ProdOrdID.TrimIfNotNullEmpty(),
                                prodItem.OrderType,
								curDocument.RevisionID.TrimIfNotNullEmpty()));
            ...
            >.Select(this, estimateItem.EstimateID, estimateItem.RevisionID))
            {
                ...
                >.Select(this, estimateItem.EstimateID, estimateItem.RevisionID, estimateOper.OperationID))
                {
                    try
                    {
						...
						bomMaintGraph.BomMatlCurySelected.Insert(new AMBomMatlCury
						{
        ...
        protected virtual AMBomItem CreateBomItem(AMEstimateItem estimateItem, BOMMaint bomMaintGraph)
        {
            ...
            if (estimateItem.SiteID == null && CreateBomItemFilter.Current.SiteID == null && !MBCEnabled)
            {
		...
		protected virtual void _(Events.RowUpdated<CurrencyInfo> e, PXRowUpdated baseMethod)
		{
			var oldRow = e.OldRow;
			var row = e.Row;
			var newSign = row.CuryMultDiv;
			if(!e.Cache.ObjectsEqual<
				CurrencyInfo.curyRateTypeID,
				CurrencyInfo.curyRate,
				CurrencyInfo.recipRate>(row, oldRow))
			{
				var flippedSign = newSign == "D" ? "M" : "D";
				row.CuryMultDiv = flippedSign;
			}	
			baseMethod.Invoke(e.Cache, e.Args);
			row.CuryMultDiv = newSign;
		}
		...
		protected virtual void _(Events.FieldUpdated<AMEstimateItem.inventoryID> e)
		{
			if (e.NewValue == null) return;
			e.Cache.SetDefaultExt<AMEstimateItem.orderQty>(e.Row);
		}

		protected virtual void _(Events.FieldVerifying<AMEstimateItem.orderQty> e)
		{
			if (e.NewValue == null) return;
			decimal newQty = (decimal)e.NewValue;
			if (newQty % 1 == 0) return;

			AMEstimateItem estimateItem = (AMEstimateItem)e.Row;
			if (!InventoryHelper.IsItemLotSerialTracked(this, estimateItem.InventoryID)) return;

			e.NewValue = Math.Round(newQty, MidpointRounding.AwayFromZero);
			e.Cache.RaiseExceptionHandling<AMEstimateItem.orderQty>(e.Row, e.NewValue,
				new PXSetPropertyException(IN.Messages.SerialItem_LineQtyUpdated, PXErrorLevel.Warning));
		}
		...
		public sealed override void Configure(PXScreenConfiguration config) =>
			Configure(config.GetScreenConfigurationContext<EstimateMaint, AM.AMEstimateItem>());
		protected static void Configure(WorkflowContext<EstimateMaint, AM.AMEstimateItem> context)
		{
        ...
        public virtual IEnumerable reportQuote(PXAdapter adapter)
        {
                ...
                !string.IsNullOrWhiteSpace(EstimateReferenceRecord?.Current.QuoteNbr))
            {
                ...
                throw new PXReportRequiredException(parameters, Reports.SOQuoteReportParams.ReportID,
                    PXBaseRedirectException.WindowMode.New, Reports.SOQuoteReportParams.ReportName, OrganizationLocalizationHelper.GetCurrentLocalization(this));
            }
    ...
    public class AddToOrderFilter : IBqlTable
    {
        ...
        [PX.Objects.SO.SO.RefNbr(typeof(Search2<SOOrder.orderNbr,
			LeftJoin<Customer, On<SOOrder.customerID, Equal<Customer.bAccountID>, And<Where<Match<Customer, Current<AccessInfo.userName>>>>>>,
			Where<SOOrder.orderType, Equal<Current<AddToOrderFilter.orderType>>, And<SOOrder.completed, Equal<False>, And<SOOrder.cancelled, Equal<False>,
    ...
    public class CreateProdOrderFilter : IBqlTable
    {
        ...
        protected String _OrderType;
        [PXDefault(typeof(Search<AMEstimateSetup.defaultOrderType>))]
        [AMOrderTypeField]
