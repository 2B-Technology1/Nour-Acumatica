    ...
    public class ProductionCostCalculator
    {
        ...
        public static PlannedCosts GetPlannedFixedOverheadCost(PXGraph graph, AMProdOper amProdOper, decimal? productionQty)
        {
            ...
            >.Select(graph, amProdOper.OrderType, amProdOper.ProdOrdID, amProdOper.OperationID))
            {
                ...
                totalCost += amProdOvhd.OFactor.GetValueOrDefault() * ovhdCury.CostRate ?? 0;
            }
            ...
            PXResultset<AMProdOvhd> productionOperations)
        {
            ...
            foreach (PXResult<AMProdOvhd, AMOverhead, AMOverheadCurySettings> result in productionOperations)
            {
                var amProdOvhd = (AMProdOvhd)result;
                var amOverhead = (AMOverhead)result;
				var ovhdCury = (AMOverheadCurySettings)result;
                ...
                var calcOvdCost = amProdOvhd.OFactor.GetValueOrDefault() * ovhdCury?.CostRate ?? 0;
...
#if DEBUG
                AMDebug.TraceWriteMethodName($"{amOverhead.OvhdID} {OverheadType.GetTypeDesc(amOverhead.OvhdType)}: operOvdCalc={operOvdCalc}; TotActCost={amProdOvhd.TotActCost}; OFactor={amProdOvhd.OFactor}; CostRate={ovhdCury?.CostRate ?? 0}");
#endif
            }
        ...
        public static PlannedCosts GetPlannedMachineCost(PXGraph graph, AMProdOper amProdOper, decimal? productionQty, decimal? machineHours)
        {
            return GetPlannedMachineCost(
               SelectFrom<AMWCMach>.
				...
				.View.Select(graph, amProdOper?.WcID)
        ...
        public static PlannedCosts GetPlannedMachineCost(PXResultset<AMWCMach> machines, AMProdOper amProdOper, decimal? productionQty, decimal? machineHours)
        {
            ...
            foreach (PXResult<AMWCMach, AMWCMachCury, AMMach, AMMachCurySettings> result in machines)
            {
				...
				decimal standardCost = wcMachine.MachineOverride.GetValueOrDefault() ? wcMachCury?.StdCost ?? 0 : machineCury?.StdCost ?? 0;				
        ...
        {
            return GetPlannedLaborCost(graph, amProdOper, productionQty, remainingProductionQty, totalHours, UomHelper.CostDecimalPrecision, laborTime, laborCost);
        }

		/// <summary>
		/// Get the planned labor unit and total cost for the given operation
		/// </summary>
		public static PlannedCosts GetPlannedLaborCost(PXGraph graph, AMProdOper amProdOper, decimal? productionQty, decimal? remainingProductionQty, decimal? totalHours, int decimalPrecision)
		{
			return GetPlannedLaborCost(graph, amProdOper, productionQty, remainingProductionQty, totalHours, decimalPrecision, amProdOper?.ActualLaborTime, amProdOper?.ActualLabor);
		}

		/// <summary>
		/// Get the planned labor unit and total cost for the given operation
		/// </summary>
		public static PlannedCosts GetPlannedLaborCost(PXGraph graph, AMProdOper amProdOper, decimal? productionQty, decimal? remainingProductionQty, decimal? totalHours, int decimalPrecision, int? laborTime, decimal? laborCost)
        {
			...
			var hoursRemaining = (totalHours.GetValueOrDefault() - actualLaborHours).NotLessZero();

            var planLaborRate = ShiftDiffType.GetShiftDifferentialCost(graph, amProdOper.WcID);
			var laborTotalCost = Math.Round(laborCost.GetValueOrDefault() + (planLaborRate * hoursRemaining), decimalPrecision, MidpointRounding.AwayFromZero);
			var laborUnitCost = productionQty == 0 ? laborTotalCost : Math.Round((decimal)(laborTotalCost / productionQty), 9, MidpointRounding.AwayFromZero);
#if DEBUG
			if(productionQty.GetValueOrDefault() != 0)
            {
        ...
        public static PlannedCosts GetPlannedToolCost(PXGraph graph, AMProdOper amProdOper, decimal? productionQty)
        {
            ...
            >.Select(graph, amProdOper.OrderType, amProdOper.ProdOrdID, amProdOper.OperationID))
            {
				...
				var toolCost = amProdTool?.UnitCost ?? 0;
                ...
                operToolUnitCost += amProdTool.QtyReq.GetValueOrDefault() * toolCost;
                operToolTot += amProdTool.TotActCost.GetValueOrDefault() + (toolCost * operToolQtyRem.GetValueOrDefault());
#if DEBUG
        ...
        {
			if (releasedTransactions == null || ammTran.BaseQty.GetValueOrDefault() == 0)
            {
                return operCostResults;
            }
			...
			operCostResults.UnitAmount = Math.Abs(operCostResults.TotalAmount) / tranReturningQty;
            return operCostResults;
        }
