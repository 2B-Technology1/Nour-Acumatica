...
namespace PX.Objects.AM
{
    public class BOMCompareInq : PXGraph<BOMCompareInq>
    {
		...
		private void ApplyCurySettings(AMBomMatl row)
		{
			var matlCury = AMBomMatlCury.PK.Find(this, row.BOMID, row.RevisionID, row.OperationID, row.LineID, Accessinfo.BaseCuryID);
			row.UnitCost = matlCury?.UnitCost;
			row.SiteID = matlCury?.SiteID;
			row.LocationID = matlCury?.LocationID;
		}

		private void ApplyCurySettings(AMBomTool row)
		{
			var toolCury = AMBomToolCury.PK.Find(this, row.BOMID, row.RevisionID, row.OperationID, row.LineID, Accessinfo.BaseCuryID);
			row.UnitCost = toolCury?.UnitCost;
		}
        ...
        protected virtual IEnumerable bomToolRecords([PXString]string detailLineNbr1, [PXString]string detailLineNbr2)
        {
            ...
            if (type1 == OperCategories.TOOL.ToString() && type2 == OperCategories.TOOL.ToString())
            {
                ...
                {
					var bomTool = (AMBomTool)row;
					bomTool.Selected = bomTool.NoteID.ToString() == note2;
					ApplyCurySettings(bomTool);
                    list.Add(row);
                }
            }
            ...
            else if (type2 == OperCategories.TOOL.ToString())
            {
				...
				var bomTool = (AMBomTool)row2;
				bomTool.Selected = true;
				ApplyCurySettings(bomTool);
				list.Add(row2);
            }
		...
		/// <summary>
		/// The required filter for displaying the information on the BOM Compare (AM41000) form (corresponding to the <see cref="BOMCompareInq"/> graph).
		/// </summary>
		[Serializable]
