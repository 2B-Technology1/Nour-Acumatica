...
namespace PX.Objects.AM
{
    ...
    public class CreateProductionOrdersProcess : PXGraph<CreateProductionOrdersProcess>
    {
        ...
        public PXFilteredProcessingJoin<AMFixedDemand, ProductionOrdersCreateFilter,
			...
			InnerJoin<INSite,
				On<AMFixedDemand.siteID, Equal<INSite.siteID>>,
			InnerJoin<Branch,
			...
			And2<Where<Where2<Where<AMFixedDemand.planType, Equal<INPlanConstants.planM8>,
				And2<Where<AMFixedDemand.hold, Equal<False>, And<Where<SOOrderTypeExt.aMProductionOrderEntryOnHold, IsNull, Or<SOOrderTypeExt.aMProductionOrderEntryOnHold, Equal<True>>>>>,
				Or<SOOrder.approved, Equal<True>, And<Where<SOOrderTypeExt.aMProductionOrderEntry, IsNull, Or<SOOrderTypeExt.aMProductionOrderEntry, Equal<True>>>>>>>,
			Or<Where<AMFixedDemand.planType, Equal<INPlanConstants.planMA>, And<AMFixedDemand.hold, Equal<False>>>>>>,
			And2<Where<AMFixedDemand.supplyPlanID, Equal<True>, Or<AMFixedDemand.supplyPlanID, IsNull>>,
			And2<Where<SOOrder.completed, IsNull, Or<SOOrder.completed, Equal<False>>>,
			And<Branch.baseCuryID, Equal<Current<AccessInfo.baseCuryID>>>>>>>>>>>>>>>>>> FixedDemand;
        ...
        {
			PXResultset<AMFixedDemand> fixedDemands = SelectFromFixedDemandView();
			return EnumerateAndPrepareFixedDemands(fixedDemands);
        }

		public virtual PXResultset<AMFixedDemand> SelectFromFixedDemandView()
		{
			PXView query = new PXView(this, false, FixedDemand.View.BqlSelect);

			var fixedDemands = new PXResultset<AMFixedDemand>();
			...
			if (PXView.MaximumRows == 1 && PXView.SortColumns != null && PXView.Searches != null
				&& Array.FindIndex(PXView.SortColumns,
					s => s.Equals(nameof(AMFixedDemand.planID), StringComparison.OrdinalIgnoreCase))
					is int planIDIndex
				&& planIDIndex >= 0
				&& PXView.Searches[planIDIndex] != null)
			{
				long planID = Convert.ToInt64(PXView.Searches[planIDIndex]);
				query.WhereAnd<Where<AMFixedDemand.planID.IsEqual<@P.AsLong>>>();
				parameters = parameters.Append(planID);
			}

			using (new PXFieldScope(query, GetFixedDemandFieldScope()))
			{
				foreach (PXResult<AMFixedDemand> demand in query.Select(PXView.Currents, parameters,
		...
		{
			yield return typeof(AMFixedDemand);
			yield return typeof(SOOrderType);
			...
			yield return typeof(SOOrder.cancelled);
			yield return typeof(SOOrder.completed);
			yield return typeof(SOOrder.hold);
			yield return typeof(SOOrder.approved);
			...
			yield return typeof(SOLine.tranDesc);
			yield return typeof(SOLineSplit.orderType);
			yield return typeof(SOLineSplit.orderNbr);
			yield return typeof(SOLineSplit.lineNbr);
			yield return typeof(SOLineSplit.splitLineNbr);
			...
			yield return typeof(InventoryItem.parentItemClassID);
			yield return typeof(INItemClass.itemClassID);
			yield return typeof(INItemClass.itemClassCD);
			yield return typeof(INItemClass.descr);
			yield return typeof(Branch);
			yield return typeof(INSite.branchID);
			yield return typeof(INSite.baseCuryID);
			yield return typeof(INSite.descr);
			yield return typeof(AMProdMatl);
			yield return typeof(AMProdMatlSplit.orderType);
			yield return typeof(AMProdMatlSplit.prodOrdID);
			yield return typeof(AMProdMatlSplit.operationID);
			yield return typeof(AMProdMatlSplit.lineID);
			yield return typeof(AMProdMatlSplit.splitLineNbr);
		}
        ...
        {
			foreach (PXResult<AMFixedDemand> rec in fixedDemands)
			{
				TryPrepareFixedDemandRow(rec);
				yield return rec;
			}
		}

		public virtual bool TryPrepareFixedDemandRow(PXResult<AMFixedDemand> rec)
		{
			var demand = (AMFixedDemand)rec;
			var line = PXResult.Unwrap<SOLine>(rec);
			var matlSplit = PXResult.Unwrap<AMProdMatlSplit>(rec);
			var matl = PXResult.Unwrap<AMProdMatl>(rec);
        ...
        public virtual void CreateProductionOrders(List<AMFixedDemand> list, ProductionOrdersCreateFilter filter, ProdMaint prodMaintGraph)
        {
            ...
            for (var i = 0; i < list.Count; i++)
            {
                ...
                try
                {
                    ...
                    if (isSO2Production)
                    {
						soLine = SOLine.PK.Find(prodMaintGraph, demand.SOOrderType, demand.SOOrderNbr, demand.SOLineNbr);
                        if (soLine != null)
                        {
		...
		[Obsolete("Moved the validation logic to FixedDemand BQL query")]
		...
		/// <summary>
		/// Optional filter processing options on the 'Create Production Orders' (AM510000) form (corresponding to the <see cref="CreateProductionOrdersProcess"/> graph).
		/// </summary>
		[Serializable]
        ...
        public class ProductionOrdersCreateFilterProjectionAttribute : PX.TM.OwnedFilter.ProjectionAttribute
        {
            ...
            typeof(Select2<,,>),
				...
				LeftJoin<AMProdMatlSplit, On<AMProdMatlSplit.planID, Equal<INItemPlan.planID>>,
	...
	/// <summary>
	/// A projection that uses the "Mark for Production" data in the <see cref="INItemPlan"/> class for processing data on the Create Production Orders (AM510000) form (corresponding to the <see cref="CreateProductionOrdersProcess"/> graph).
	/// </summary>
    ...
    public class AMFixedDemand : INItemPlan
    {
        ...
        protected String _SOOrderNbr;
        [PXDBString(15, IsUnicode = true, BqlField = typeof(SOLine.orderNbr), InputMask = "")]
        ...
        [PXDBString(2, IsFixed = true, BqlField = typeof(AMProdMatlSplit.orderType))]
        ...
        [PXDBString(15, IsUnicode = true, BqlField = typeof(AMProdMatlSplit.prodOrdID), InputMask = "")]
        ...
        [PXDBInt(BqlField = typeof(AMProdMatlSplit.operationID))]
        ...
        [PXDBInt(BqlField = typeof(AMProdMatlSplit.lineID))]
