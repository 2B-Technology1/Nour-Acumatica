...
namespace PX.Objects.AM.Attributes
{
    ...
    {
        public AMLotSerialNbrAttribute(Type InventoryType, Type SubItemType, Type LocationType) : base(InventoryType, SubItemType, LocationType, typeof(CostCenter.freeStock))
        {
        }

        public AMLotSerialNbrAttribute(Type InventoryType, Type SubItemType, Type LocationType, Type ParentLotSerialNbrType) : base(InventoryType, SubItemType, LocationType, ParentLotSerialNbrType, typeof(CostCenter.freeStock))
        {
        }
        ...
        public static bool MaterialLotSerialNbrIssuedToOrder(PXGraph graph, AMMTran returningAMMTran, string lotSerialNbr)
        {
			...
			var matlLotSerial = AMProdMatlLotSerial.PK.Find(graph,
				returningAMMTran.OrderType,
				returningAMMTran.ProdOrdID,
				returningAMMTran.OperationID,
				returningAMMTran.MatlLineId,
				lotSerialNbr,
				returningAMMTran.ParentLotSerialNbr?? string.Empty);

			if(matlLotSerial == null || matlLotSerial.QtyIssued <=0 ) return false;
