...
namespace PX.Objects.AM
{
    ...
    {
		[PXFilterable]
        public PXSelectJoin<AMRPDetail,
			InnerJoin<Branch, On<AMRPDetail.branchID, Equal<Branch.branchID>>,
        ...
        [PXUIField(DisplayName = "Manufacture", FieldClass = Features.MANUFACTURINGFIELDCLASS, Enabled = true)]
        [PXButton(CommitChanges = true)]
        ...
        public void Transfer(List<AMRPDetail> list)
        {
			...
			foreach (AMRPDetail detail in list)
            {
				...
				if (transferSite == null)
					transferSite = detail.TransferSiteID;
				if (sameFromSite)
				{
					if (transferSite != detail.TransferSiteID)
						sameFromSite = false;
				}
			...
			if (sameFromSite)
				PlanTransferOrderFilter.Current.FromSiteID = transferSite;
        ...
        public static void CreateProductionOrders(ProdMaint prodMaint, List<AMOrderCrossRef> list)
        {
            ...
            {
				var retryCount = 10;
				for (var attemptCount = 0; attemptCount <= retryCount; attemptCount++)
				{
                try
                {
					...
					{
						// Only retry on estimate INLotSerClassLotSerNumVal lock
						if (lve.Table == typeof(INLotSerClassLotSerNumVal) && attemptCount < retryCount)
						{
							continue;
						}
						else
						{
							...
							PXTrace.WriteError($"Unable to create production order for AMRPDetail Record ID {recordNbrs}. {lve.Message}");
						}
                }
		...
		public sealed override void Configure(PXScreenConfiguration config) =>
			Configure(config.GetScreenConfigurationContext<MRPDetail, AM.AMRPDetail>());
		protected static void Configure(WorkflowContext<MRPDetail, AM.AMRPDetail> context)
		{
					...
					.WithActions(actions =>
					{
						...
						actions.AddNew("ShowMRPDetailInquiry", a => a
							...
							.IsSidePanelScreen(sp => sp
								...
								.WithAssignments(ass =>
								{
									ass.Add(nameof(AMRPDetail.InventoryID), e => e.SetFromField<AMRPDetail.inventoryID>());
									ass.Add(nameof(AMRPDetail.SiteID), e => e.SetFromField<AMRPDetail.siteID>());
								})));
						#endregion
					});
			});
	...
	public class MRPDisplayFilters : IBqlTable
	{
		...
		[PXSelector(typeof(Search<AMRPDetailRelatedDocument.refNbr>))]
		...
		[PXSelector(typeof(Search<AMRPDetailRelatedParentDocument.parentRefNbr>))]
		...
		[PXSelector(typeof(Search<AMRPDetailRelatedProductDocument.productRefNbr>))]
