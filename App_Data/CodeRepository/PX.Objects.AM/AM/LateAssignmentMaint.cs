...
namespace PX.Objects.AM
{
    public class LateAssignmentMaint : PXGraph<LateAssignmentMaint>
    {
        ...
        public PXSelect<AMProdItemSplitPreassign,
			Where<AMProdItemSplitPreassign.orderType, Equal<Optional<AMProdItemSplitPreassign.orderType>>,
				And<AMProdItemSplitPreassign.baseCuryID, Equal<Current<AccessInfo.baseCuryID>>>>> ProdItemSplits;
		...
		protected virtual void _(Events.RowSelected<AMProdItemSplitPreassign> e)
		{
			...
			var enabled = e.Row?.Released == true &&
				(e.Row?.Hold != true && e.Row?.Closed != true && e.Row?.Canceled != true && e.Row?.Locked == false);

			SetScreenEneabled(enabled);
		}
