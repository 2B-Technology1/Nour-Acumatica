...
namespace PX.Objects.AM
{
    ...
    public class ConfigurationSelect : PXSelect<AMConfigurationResults>, IRuleProcessor, IRuleConditionValidator
    {
        ...
        private void RegisterHandlers(PXGraph graph)
        {
			...
			graph.FieldUpdated.AddHandler<AMConfigurationResults.revision>(AMConfigurationResults_Revision_FieldUpdated);
        ...
        protected virtual void AMConfigurationResults_KeyID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
        {
			...
			if (sender.Graph.IsImport || sender.Graph.IsContractBasedAPI)
				AddProdMaintHandlers(sender.Graph);
...
#pragma warning restore PX1044
		...
		protected virtual void AMConfigurationResults_Revision_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			CreateResultsTemplate(sender, (AMConfigurationResults)e.Row);
		}
				...
				foreach (AMConfiguration config in Configuration.Select())
				{
					...
					using (var scope = new AMRuleFormulaScope(this))
					{
						using (new UseCurrentParentScope(ResultOptions.Cache, true, typeof(AMConfigResultsOption.configResultsID), typeof(AMConfigResultsOption.featureLineNbr)))
						{
							...
							{
								ConfigRules.Current = configRule;
        ...
        {
			var newResultsRule = new AMConfigResultsRule
			{
				ConfigurationID = configRule.ConfigurationID,
				Revision = configRule.Revision,
			...
			newResultsRule = ResultRules.Insert(newResultsRule);
        }
		...
		public void AddProdMaintHandlers(PXGraph graph)
		{
			graph.FieldUpdating.AddHandler<AMConfigurationResults.keyID>(AMConfigurationResults_KeyID_FieldUpdating);
			graph.FieldUpdated.AddHandler<AMConfigurationResults.keyID>(AMConfigurationResults_KeyID_FieldUpdated);
		}
        ...
        public static bool TryGetDefaultConfigurationID(PXGraph graph, int? inventoryID, int? siteID, out string configurationID)
        {
            ...
            configurationID = graph.GetService<IConfigurationIDManager>().GetID(inventoryID, siteID);
