...
{
	/// <summary>
	/// The configuration of available features that are used when maintaining a product configurator configuration. Maintains data on the Features (AM203500) form (corresponding to the <see cref="FeatureMaint"/> graph).
	/// </summary>
	[Serializable]
    [PXCacheName("Feature")]
    ...
    public class AMFeature : IBqlTable, INotable
    {
        ...
        {
            public static AMFeature Find(PXGraph graph, string featureID, PKFindOptions options = PKFindOptions.None)
                => FindBy(graph, featureID, options);
        }
        #endregion
