...
{
	/// <summary>
	/// Master table for a vendor shipment created on the Vendor Shipments (AM310000) form (corresponding to the <see cref="VendorShipmentEntry"/> graph).
	/// </summary>
	[Serializable]
    ...
    public class AMVendorShipment : IBqlTable, INotable
    {
        ...
        {
            public static AMVendorShipment Find(PXGraph graph, string shipmentNbr, PKFindOptions options = PKFindOptions.None) => FindBy(graph, shipmentNbr, options);
        }
		...
		[PXSelector(typeof(Search2<AMVendorShipment.shipmentNbr,
			InnerJoin<Branch, On<AMVendorShipment.branchID, Equal<Branch.branchID>>>,
        ...
        [AMSite(DescriptionField = typeof(INSite.descr))]
