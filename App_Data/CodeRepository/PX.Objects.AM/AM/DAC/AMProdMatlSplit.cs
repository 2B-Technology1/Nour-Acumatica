...
{
	/// <summary>
	/// Line details for a material on a production order. The line details are created on the Production Order Details (AM209000) form (corresponding to the <see cref="ProdDetail"/> graph).
	/// Parent: <see cref="AMProdItem"/>
	/// </summary>
	[System.Diagnostics.DebuggerDisplay("{DebuggerDisplay,nq}")]
    [Serializable]
    [PXCacheName(Messages.ProductionMatlSplit)]
    public class AMProdMatlSplit : IBqlTable, ILSDetail, IProdOper, IItemPlanSource
	{
#if DEBUG
        ...
        {
            public static AMProdMatlSplit Find(PXGraph graph, string orderType, string prodOrdID, int? operationID, int? lineID, int? splitLineNbr, PKFindOptions options = PKFindOptions.None) 
                => FindBy(graph, orderType, prodOrdID, operationID, lineID, splitLineNbr, options);
	...
	[PXProjection(typeof(Select2<
		AMProdMatlSplit, InnerJoin<
			AMProdMatl, On<
				AMProdMatl.orderType, Equal<AMProdMatlSplit.orderType>, And<
				AMProdMatl.prodOrdID, Equal<AMProdMatlSplit.prodOrdID>, And<
				AMProdMatl.operationID, Equal<AMProdMatlSplit.operationID>, And<
				AMProdMatl.lineID, Equal<AMProdMatlSplit.lineID>>>>
				>
			>
		>))]
