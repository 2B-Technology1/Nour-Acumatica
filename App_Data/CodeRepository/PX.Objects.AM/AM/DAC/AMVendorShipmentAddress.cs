...
{
	/// <summary>
	/// Address information for a vendor shipment created on the Vendor Shipments (AM310000) form (corresponding to the <see cref="VendorShipmentEntry"/> graph).
	/// Parent: <see cref="AMVendorShipment"/>
	/// </summary>
	[Serializable]
    [PXCacheName(Messages.VendorShipmentAddress)]
    public class AMVendorShipmentAddress : IBqlTable, IAddress
    {
        #region Keys
        public class PK : PrimaryKeyOf<AMVendorShipmentAddress>.By<addressID>
        {
            public static AMVendorShipmentAddress Find(PXGraph graph, int? addressID, PKFindOptions options = PKFindOptions.None) => FindBy(graph, addressID, options);
        }
        #endregion
        ...
        [PX.Objects.CR.State(typeof(AMVendorShipmentAddress.countryID))]
        ...
        [PXZipValidation(typeof(Country.zipCodeRegexp), typeof(Country.zipCodeMask), countryIdField: typeof(AMVendorShipmentAddress.countryID))]
        [PXPersonalDataField]
