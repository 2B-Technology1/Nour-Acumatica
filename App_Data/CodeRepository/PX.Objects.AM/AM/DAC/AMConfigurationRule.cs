...
{
	/// <summary>
	/// The maintenance table for the configuration of the product configurator rules under a configuration revision on the Configuration Maintenance (AM207500) form (corresponding to the <see cref="ConfigurationEntry"/> graph).
	/// The <see cref="AMConfigurationRule.RuleSource"/> graph indicates that the rule is for a feature or an attribute.
	/// Parent: <see cref="AMConfiguration"/>
	/// </summary>
    ...
    public class AMConfigurationRule : IBqlTable, IProductConfiguration
    {
        ...
        {
            public static AMConfigurationRule Find(PXGraph graph, string configurationID, string revision, string ruleSource, int? sourceLineNbr, int? lineNbr, PKFindOptions options = PKFindOptions.None) 
                => FindBy(graph, configurationID, revision, ruleSource, sourceLineNbr, lineNbr, options);
        }
	...
	/// <summary>
	/// Projection for the <see cref="AMConfigurationRule"/> class for only the attribute rules.
	/// Parent: <see cref="AMConfigurationAttribute"/>
	/// </summary>
	...
	/// <summary>
	/// Projection for the <see cref="AMConfigurationRule"/> class for only the feature rules.
	/// Parent: <see cref="AMConfigurationFeature"/>
	/// </summary>
	[System.Diagnostics.DebuggerDisplay("[{ConfigurationID}:{Revision}] SourceLineNbr={SourceLineNbr}; LineNbr={LineNbr}")]
    [Serializable]
    ...
    public class AMConfigurationFeatureRule : IBqlTable
    {
        ...
        public abstract class sourceOptionLineNbr : PX.Data.BQL.BqlInt.Field<sourceOptionLineNbr> { }
