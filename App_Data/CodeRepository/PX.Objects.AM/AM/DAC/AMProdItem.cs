...
using static PX.Objects.AM.AMMTran.FK;
using PX.Objects.CR;
...
namespace PX.Objects.AM
{
	...
	/// <list type="bullet">
	/// <item><description>Create Production Orders (AM510000) (corresponding to the <see cref="CreateProductionOrdersProcess"/> graph)</description></item>
	/// <item><description>Production Order Maintenance (AM201500) (corresponding to the <see cref="ProdMaint"/> graph)</description></item>
	/// </list>
	/// </remarks>
	[Serializable]
    ...
    public class AMProdItem : IBqlTable, ILSPrimary, INotable, IProdOrder
    {
        ...
        {
            public static AMProdItem Find(PXGraph graph, string orderType, string prodOrdID, PKFindOptions options = PKFindOptions.None) 
		...
		public abstract class hasTransactions : PX.Data.BQL.BqlBool.Field<hasTransactions> { }

		protected Boolean? _HasTransactions;
		[PXDBBool]
		[PXDefault(false)]
		[PXUIField(DisplayName = "HasTransactions", Visible = false, Enabled = false)]
		public virtual Boolean? HasTransactions
		{
			get
			{
		...
		[PXDBBool]
		[PXDefault(false)]
		[PXUIField(DisplayName = "Completed", Visible = false, Enabled = false)]
		...
		[PXDBBool]
		[PXDefault(false)]
		[PXUIField(DisplayName = "Closed", Visible = false, Enabled = false)]
		...
		[PXDBBool]
		[PXDefault(false)]
		[PXUIField(DisplayName = "Canceled", Visible = false, Enabled = false)]
		...
		[PXDBBool]
		[PXDefault(false)]
		[PXUIField(DisplayName = "IsOpen", Visible = false, Enabled = false)]
		...
		[PXRestrictor(typeof(Where<AMProdItem.prodOrdID.IsNotEqual<AMProdItem.prodOrdID.FromCurrent>
			.And<AMProdItem.orderType.IsEqual<AMProdItem.orderType.FromCurrent>>
			.Or<AMProdItem.orderType.IsNotEqual<AMProdItem.orderType.FromCurrent>>>), Messages.UnableToSelectOrderToLink, typeof(AMProdItem.prodOrdID))]
        [ProductionOrderSelector(typeof(AMProdItem.parentOrderType), includeAll: true)]
		[PXFormula(typeof(Default<AMProdItem.parentOrderType>))]
...
#pragma warning restore PX1030
		...
		[PXFormula(typeof(Default<AMProdItem.productOrderType>))]
        ...
        [INLotSerialNbr(typeof(AMProdItem.inventoryID), typeof(AMProdItem.subItemID), typeof(AMProdItem.locationID), typeof(CostCenter.freeStock), PersistingCheck = PXPersistingCheck.Nothing, Visible = false)]
        ...
        [AMSite(DisplayName = "Scrap Warehouse")]
		...
		#region Locked
		public abstract class locked : PX.Data.BQL.BqlBool.Field<locked> { }
		/// <summary>
		/// Production Order Locked Status
		/// </summary>
		[PXDBBool]
		[PXDefault(false)]
		[PXUIField(DisplayName = "Locked", Visible = false, Enabled = false)]
		...
		public abstract class customerLocationID : PX.Data.BQL.BqlBool.Field<customerLocationID> { }

		protected int _CustomerLocationID;
		[LocationActive(typeof(Where<CR.Location.bAccountID, Equal<Current<AMProdItem.customerID>>,
			...
			Where<BAccountR.bAccountID, Equal<Current<AMProdItem.customerID>>,
			And<CR.Location.isActive, Equal<True>,
			And<MatchWithBranch<CR.Location.cBranchID>>>>>,
			Search<CR.Location.locationID,
			Where<CR.Location.bAccountID, Equal<Current<AMProdItem.customerID>>,
			And<CR.Location.isActive, Equal<True>, And<MatchWithBranch<CR.Location.cBranchID>>>>>>), PersistingCheck = PXPersistingCheck.Nothing)]
					[PXForeignReference(typeof(CompositeKey<Field<AMProdItem.customerID>.IsRelatedTo<CR.Location.bAccountID>,
						Field<AMProdItem.customerLocationID>.IsRelatedTo<CR.Location.locationID>>))]	
		...
		#region
		public class Events : PXEntityEvent<AMProdItem>.Container<Events>
		{
			public PXEntityEvent<AMProdItem> UpdateStatus;
		}
		#endregion
	}
