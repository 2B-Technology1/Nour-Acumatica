...
{
	/// <summary>
	/// The maintenance table for the configuration of the product configurator options under a configuration revision on the Configuration Maintenance (AM207500) form (corresponding to the <see cref="ConfigurationEntry"/> graph).
	/// Parent: <see cref="AMConfiguration"/>, <see cref="AMConfigurationFeature"/>
	/// </summary>
    ...
    public class AMConfigurationOption : IBqlTable, IConfigOption, IOperation, IProductConfiguration
	{
        ...
        {
            public static AMConfigurationOption Find(PXGraph graph, string configurationID, string revision, int? featureLineNbr, int? lineNbr, PKFindOptions options = PKFindOptions.None) 
                => FindBy(graph, configurationID, revision, featureLineNbr, lineNbr, options);
        }
