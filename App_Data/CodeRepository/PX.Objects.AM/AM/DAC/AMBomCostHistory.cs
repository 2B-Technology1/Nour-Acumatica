...
{
	/// <summary>
	/// History of the <see cref="AMBomCost"/> graph that was created when a user clicked "Archive" on the Cost Roll (AM508000) form, and the <see cref="BOMCostRoll"/> graph.
	/// </summary>
	[Serializable]
    ...
    public class AMBomCostHistory : IBqlTable, IBomRevision
    {
        ...
        {
            public static AMBomCostHistory Find(PXGraph graph, string bOMID, string revisionID, DateTime startDate, PKFindOptions options = PKFindOptions.None)
                => FindBy(graph, bOMID, revisionID, startDate, options);
