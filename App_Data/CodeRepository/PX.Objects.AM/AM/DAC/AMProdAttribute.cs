...
{
	/// <summary>
	/// Attributes of a production order that are maintained on the Production Order Maintenance (AM201500) form (corresponding to the <see cref="ProdMaint"/> graph).
    ...
    public class AMProdAttribute : IBqlTable, IProdOper
    {
        ...
        {
            public static AMProdAttribute Find(PXGraph graph, string orderType, string prodOrdID, int? lineNbr, PKFindOptions options = PKFindOptions.None) 
                => FindBy(graph, orderType, prodOrdID, lineNbr, options);
		...
		[PXDependsOnFields(typeof(AMProdAttribute.attributeID))]
