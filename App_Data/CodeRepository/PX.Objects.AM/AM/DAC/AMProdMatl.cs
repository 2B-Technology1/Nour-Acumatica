...
{
	/// <summary>
	/// Maintenance table for the materials on a production order operation on the Production Order Details (AM209000) form (corresponding to the <see cref="ProdDetail"/> graph).
    ...
    public class AMProdMatl : IBqlTable, ISortOrder, ILSPrimary, IProdOper, IPhantomBomReference, INotable
    {
        ...
        {
            public static AMProdMatl Find(PXGraph graph, string orderType, string prodOrdID, int? operationID, int? lineID, PKFindOptions options = PKFindOptions.None) 
        ...
        [PXDefault(ProductionOrderStatus.Planned, typeof(Search<
            ...
            Where<AMProdItem.orderType, Equal<Current<AMProdMatl.orderType>>,
                And<AMProdItem.prodOrdID, Equal<Current<AMProdMatl.prodOrdID>>>>>))]
        [PXUIField(DisplayName = "Material Status", Enabled = false)]
        ...
        [INLotSerialNbr(typeof(AMProdMatl.inventoryID), typeof(AMProdMatl.subItemID), typeof(AMProdMatl.locationID), typeof(CostCenter.freeStock), PersistingCheck = PXPersistingCheck.Nothing, Visible = false)]
