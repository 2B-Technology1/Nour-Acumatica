...
{
	/// <summary>
	/// Master table with the product configurator results. The data is based on the configuration maintenance data from the <see cref="AMConfiguration"/> class with the configuration totals and order references. New data is stored for each new configuration result.
	/// </summary>
	[System.Diagnostics.DebuggerDisplay("{DebuggerDisplay,nq}")]
    [Serializable]
    [PXCacheName(Messages.ConfigurationResult)]
	[PXCopyPasteHiddenFields(typeof(AMConfigurationResults.curyID), typeof(AMConfigurationResults.curyInfoID))]
    public class AMConfigurationResults : IBqlTable, IProductConfigResult
    {
        ...
        {
            public static AMConfigurationResults Find(PXGraph graph, int? configResultsID, PKFindOptions options = PKFindOptions.None) 
                => FindBy(graph, configResultsID, options);
		...
		/// </summary>
		[PXBool]
		[PXDependsOnFields(typeof(AMConfigurationResults.prodOrderType), typeof(AMConfigurationResults.prodOrderNbr))]
		public virtual bool? IsProductionReferenced => ProdOrderType != null && ProdOrderNbr != null;
	}
}
