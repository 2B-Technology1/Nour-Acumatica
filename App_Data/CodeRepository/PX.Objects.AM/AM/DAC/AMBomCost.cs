...
{
	/// <summary>
	/// Table with results when processing the Cost Roll (AM508000) form and the <see cref="BOMCostRoll"/> graph.
	/// </summary>
	[Serializable]
    ...
    public class AMBomCost : IBqlTable, IBomRevision
	{
		...
		public class PK : PrimaryKeyOf<AMBomCost>.By<bOMID, revisionID, userID, curyID>
        {
            public static AMBomCost Find(PXGraph graph, string bOMID, string revisionID, Guid userID, PKFindOptions options = PKFindOptions.None)
				=> Find(graph, bOMID, revisionID, userID, graph.Accessinfo.BaseCuryID, options);

			public static AMBomCost Find(PXGraph graph, string bOMID, string revisionID, Guid userID, string curyID, PKFindOptions options = PKFindOptions.None)
				=> FindBy(graph, bOMID, revisionID, userID, curyID, options);
