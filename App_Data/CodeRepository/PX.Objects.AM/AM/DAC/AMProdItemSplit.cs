...
{
	/// <summary>
	/// Line details for a production order. The line details are maintained on the Production Order Maintenance (AM201500) form (corresponding to the <see cref="ProdMaint"/> graph).
	/// Parent: <see cref="AMProdItem"/>
	/// </summary>
	[PXCacheName(Messages.ProductionItemSplit)]
    [Serializable]
    [System.Diagnostics.DebuggerDisplay("{DebuggerDisplay,nq}")]
    public class AMProdItemSplit : IBqlTable, ILSDetail, IProdOrder, IItemPlanSource
	{
        ...
        {
            public static AMProdItemSplit Find(PXGraph graph, string orderType, string prodOrdID, int? splitLineNbr, PKFindOptions options = PKFindOptions.None) 
                => FindBy(graph, orderType, prodOrdID, splitLineNbr, options);
        ...
        [PXDefault(INTranType.Receipt, typeof(AMProdItem.tranType))]
        [INTranType.List]
        ...
        [INLotSerialNbrAttribute(typeof(AMProdItemSplit.inventoryID), typeof(AMProdItemSplit.subItemID), typeof(AMProdItemSplit.locationID), typeof(AMProdItem.lotSerialNbr), typeof(CostCenter.freeStock), 
	...
	/// <summary>
	/// Projection of <see cref="AMProdItem"/> and <see cref="AMProdItemSplit"/> to show preassigned lot/serial numbers for a production order. 
	/// Parent: <see cref="AMProdItem"/>
	/// </summary>
    ...
    public class AMProdItemSplitPreassign : IBqlTable,IProdOrder
    {
        ...
        protected String _LotSerialNbr;
        [PXDBString(INLotSerialStatusByCostCenter.lotSerialNbr.Length, IsUnicode = true, InputMask = "", IsKey = true, BqlField = typeof(AMProdItemSplit.lotSerialNbr))]
		...
		[PXDBString(BqlField = typeof(Branch.baseCuryID))]
		...
		[PXDBBool(BqlField = typeof(AMProdItem.released))]
		[PXUIField(Enabled = false, Visible = false)]
		...
		[PXDBBool(BqlField = typeof(AMProdItem.hasTransactions))]
		[PXUIField(Enabled = false, Visible = false)]
		...
		[PXDBBool(BqlField = typeof(AMProdItem.closed))]
		[PXUIField(Enabled = false, Visible = false)]
		...
		[PXDBBool(BqlField = typeof(AMProdItem.canceled))]
		[PXUIField(Enabled = false, Visible = false)]
		...
		/// Production Order Lock state
		/// </summary>
		[PXDBBool(BqlField = typeof(AMProdItem.locked))]
		[PXUIField(Enabled = false, Visible = false)]
