...
{
	/// <summary>
	/// The maintenance table for the configuration of the product configurator attributes under a configuration revision on the Configuration Maintenance (AM207500) form (corresponding to the <see cref="ConfigurationEntry"/> graph).
    ...
    public class AMConfigurationAttribute : IBqlTable, IProductConfiguration
    {
        ...
        {
            public static AMConfigurationAttribute Find(PXGraph graph, string configurationID, string revision, int? lineNbr, PKFindOptions options = PKFindOptions.None) 
                => FindBy(graph, configurationID, revision, lineNbr, options);
        }
		...
		[PXDependsOnFields(typeof(AMConfigurationAttribute.attributeID))]
