...
{
	/// <summary>
	/// Setup record for the Engineering Change Request (AM210000) form that is related to the approval process. Configured on the BOM Preferences (AM101000) form with the <see cref="BOMSetup"/> graph.
	/// Requires the <see cref="CS.FeaturesSet.manufacturingECC"/> feature to be enabled.
	/// </summary>
	[Serializable]
	[PXCacheName(Messages.AMECRSetupApproval)]
    public partial class AMECRSetupApproval : IBqlTable, IAssignedMap
    {
        #region ApprovalID
        ...
        [PXSelector(
			typeof(Search<
				...
				Where<
					EPAssignmentMap.entityType, Equal<Attributes.AssignmentMapType.AssignmentMapTypeAMECR>,
        ...
        [PXDBBool]
		[PXDefault(typeof(Search<AMBSetup.eCRRequestApproval>), PersistingCheck = PXPersistingCheck.Nothing)]
