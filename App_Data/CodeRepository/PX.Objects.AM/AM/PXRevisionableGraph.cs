...
namespace PX.Objects.AM
{
	...
	/// </summary>
	public abstract class PXRevisionableGraph<TGraph, TPrimary, TKeyField, TRevisionField> : PXRevisionableGraphBase<TGraph, TPrimary, TKeyField, TRevisionField>
		where TGraph : PXRevisionableGraphBase<TGraph, TPrimary, TKeyField, TRevisionField>, new()
	...
	/// Manufacturing Revisionable base graph
	/// </summary>
	public abstract class PXRevisionableGraphBase<TGraph, TPrimary, TKeyField, TRevisionField> : PXGraph<PXRevisionableGraphBase<TGraph, TPrimary, TKeyField, TRevisionField>>
        where TGraph : PXRevisionableGraphBase<TGraph, TPrimary, TKeyField, TRevisionField>, new()
        where TPrimary : class, IBqlTable, new()
        where TKeyField : class, IBqlField
        where TRevisionField : class, IBqlField
        ...
        where TRevisionField : class, IBqlField
    {
        ...
        protected override IEnumerable Handler(PXAdapter adapter)
        {
            ...
            var searchesOrig = adapter.Searches;
            ...
            if (adapter.Searches != null)
            {
                ...
                else if (adapter.Searches.Length == 1 && !isAutoNumber)
                {
                    //Revision or BOMID is empty
                    if (adapter.SortColumns[0] == nameof(AMBomItem.BOMID))
                    {
                        currentKey = searchesOrig[0].ToString();
                        currentRev = null;
                    }
                    else
                    {
                        currentKey = null;
                        currentRev = searchesOrig[0].ToString();
                    }
        ...
        where TRevisionField : class, IBqlField
    {
        ...
        protected override IEnumerable Handler(PXAdapter adapter)
        {
            _Graph.Clear();
            _Graph.SelectTimeStamp();
			BqlCommand cmd = adapter.View.BqlSelect.OrderByNew<OrderBy<Asc<TKeyField, Desc<TRevisionField>>>>();
			TPrimary rec = (TPrimary)cmd.CreateView(_Graph).SelectSingle();
        ...
        where TRevisionField : class, IBqlField
    {
        ...
        protected override IEnumerable Handler(PXAdapter adapter)
        {
            ...
            if (goToLast)
            {
				BqlCommand cmd = adapter.View.BqlSelect.OrderByNew<OrderBy<Desc<TKeyField, Desc<TRevisionField>>>>();
				rec = (TPrimary)cmd.CreateView(_Graph).SelectSingle(); 
                if (rec != null)
                {
                    return new object[] { rec };
                }
