...
{
    public class ProdDetail : ProdMaintBase<ProdDetail>
    {
		public class AMProdMatlSplitPlan : AMProdMatlSplitPlan<ProdDetail> { }
		public class AMProdItemSplitPlan : AMProdItemSplitPlan<ProdDetail> { }
        ...
        public SelectFrom<AMProdItem>
			...
			.Where<AMProdItem.orderType.IsEqual<AMProdItem.orderType.AsOptional>
				.And<Branch.baseCuryID.IsEqual<AccessInfo.baseCuryID.FromCurrent>>>.View ProdItemRecords; 
        ...
        [PXImport(typeof(AMProdItem))]
        public SelectFrom<AMProdOvhd>
			.InnerJoin<AMOverhead>.On<AMProdOvhd.ovhdID.IsEqual<AMOverhead.ovhdID>>
			.LeftJoin<AMOverheadCurySettings>.On<AMProdOvhd.ovhdID.IsEqual<AMOverheadCurySettings.ovhdID>
				.And<AMOverheadCurySettings.curyID.IsEqual<AccessInfo.baseCuryID.FromCurrent>>>
			.Where<AMProdOvhd.orderType.IsEqual<AMProdOper.orderType.FromCurrent>
				.And<AMProdOvhd.prodOrdID.IsEqual<AMProdOper.prodOrdID.FromCurrent>
				.And<AMProdOvhd.operationID.IsEqual<AMProdOper.operationID.FromCurrent>>>>
			.View ProdOvhdRecords;
        [PXHidden]
		...
		public override void Persist()
        {
            ...
            if (currentProdItem != null && reschedule && !ProdItemRecords.Cache.IsRowDeleted(currentProdItem))
            {
				...
				var lastPrevious = currentProdItem.LastOperationID ?? null;
				currentProdItem.FirstOperationID = firstLastOperations?.Item1;
				currentProdItem.LastOperationID = firstLastOperations?.Item2;

				if (lastOperChange)
				{
					if (lastPrevious != null && ProdItemRecords.Cache.GetStatus(currentProdItem) != PXEntryStatus.Inserted)
					{
						var lastPreviousOper = ProdOperRecords.Cache.Cached.RowCast<AMProdOper>().Where(x => x.OrderType == currentProdItem.OrderType
									&& x.ProdOrdID == currentProdItem.ProdOrdID && x.OperationID == lastPrevious).FirstOrDefault();

						if (lastPreviousOper != null)
						{
							foreach (AMMTran tran in PXSelectReadonly<AMMTran,
									Where<AMMTran.orderType, Equal<Required<AMMTran.orderType>>,
										...
										.Select(this, lastPreviousOper.OrderType, lastPreviousOper.ProdOrdID, lastPreviousOper.OperationID,
											new string[] { AMDocType.Move, AMDocType.Labor }))
							{
								throw new PXException(Messages.GetLocal(Messages.MoveTransactionAlreadyExists, lastPreviousOper.OperationCD,
												AMDocType.GetDocTypeDesc(tran.DocType), tran.BatNbr));
							}
						}
					}
                ...
                if(currentProdItem.HasTransactions == true
                    || currentProdItem.Completed == true)
                {
            ...
            {
                if (ProdItemRecords.Current != null && ProdItemRecords.Current.Released == false)
                {
                    try
                    {
            ...
            reschedule = reschedule && (ProdItemRecords.Current.HasTransactions == false);
        ...
        protected bool IsApiEnabled
        {
            get
            {
                ...
                return ((IsImport || IsContractBasedAPI) && (prodItem.Released == true && prodItem.Completed == false || prodItem.Canceled == false));
            }
        }
        ...
        protected virtual void AMProdItem_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
        {
            ...
            CreateVendorShipment.SetEnabled(amProdItem.IsOpen == true);
        }
        ...
        protected virtual void AMProdOper_StatusID_FieldDefaulting(PXCache cache, PXFieldDefaultingEventArgs e)
        {
            ...
            e.NewValue = prodItem.Released == false
        ...
        protected virtual bool CanOperationCDChange(AMProdItem prodItem, AMProdOper prodOper, string newOperationCD, out PXSetPropertyException ex)
        {
            ...
            if(prodItem.HasTransactions != true)
            {
                return true;
            }
        ...
        protected virtual void _(Events.RowSelected<AMProdOper> e)
        {
			...
			if (!IsImport && !IsContractBasedAPI && ProdItemRecords?.Current?.LastOperationID != null && ProdItemRecords.Current.LastOperationID == e?.Row?.OperationID)
			{
        ...
        {
			var newValue = Boolean.Parse(e.NewValue.ToString());
            var prodItem = ProdItemRecords.Current;
		...
		{
			if (e?.Row == null || e.Cache.ObjectsEqual<AMProdMatl.qtyReq, AMProdMatl.qtyRoundUp, AMProdMatl.batchSize>(e.Row, e.OldRow))
			{
				return;
			}

			var matlStatus = GetMaterialStatus(ProdItemRecords.Current, e.Row);
			if (matlStatus != null && matlStatus != e.Row.StatusID)
			{
				e.Cache.SetValueExt<AMProdMatl.statusID>(e.Row, matlStatus);
			}
		}
		...
		public virtual void _(Events.FieldUpdated<AMProdMatl, AMProdMatl.pOCreate> e)
		{
			if (e.Row.POCreate == true)
			{
				e.Cache.SetDefaultExt<AMProdMatl.vendorID>(e.Row);
			}
			else
			{
				e.Cache.SetValueExt<AMProdMatl.vendorID>(e.Row, null);
			}
		}
        #endregion
        ...
        public void AMProdTool_ToolID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
			...
			PXResult<AMToolMst, AMToolMstCurySettings> result = (PXResult<AMToolMst, AMToolMstCurySettings>)SelectFrom<AMToolMst>
				.LeftJoin<AMToolMstCurySettings>.On<AMToolMst.toolID.IsEqual<AMToolMstCurySettings.toolID>
					.And<AMToolMstCurySettings.curyID.IsEqual<@P.AsString>>>
				.Where<AMToolMst.toolID.IsEqual<@P.AsString>>
				.View.Select(this, Accessinfo.BaseCuryID, prodTool.ToolID).FirstOrDefault_();

			var amToolMst = (AMToolMst)result;
			var toolCury = (AMToolMstCurySettings)result;
				

            if (amToolMst != null)
            {
                prodTool.Descr = amToolMst.Descr;
                prodTool.UnitCost = toolCury?.UnitCost ?? 0;
            }
            else
            {
            ...
            string poReceiptType, string poReceiptNbr)
        {
            ...
            foreach (var newsplit in insertedSchedules)
            {
                ...
                {
					foreach(var deleted in deletedPlans)
					{
						var deletedOrig = (INItemPlan)initemplan.Cache.GetOriginal(deleted);
						var parentScehduleOrig = (AMProdMatlSplit)prodMatlSplit.Cache.GetOriginal(parentschedule);
						if (deletedOrig != null && parentScehduleOrig != null && deletedOrig.PlanID != parentScehduleOrig.PlanID) continue;
                    ...
                    parentschedule = PXCache<AMProdMatlSplit>.CreateCopy(parentschedule);
						var demand = PXCache<INItemPlan>.CreateCopy(deletedOrig);
        ...
        public static void ProcessPOOrder(PXGraph graph, POOrder poOrder)
        {
                ...
                And<AMProdMatlSplit.qtyReceived, Less<AMProdMatlSplit.qty>, And<AMProdMatlSplit.pOCancelled, NotEqual<boolTrue>, And<AMProdMatlSplit.completed, NotEqual<boolTrue>>>>>>>>.Select(graph, poOrder.OrderType, poOrder.OrderNbr))
            {
				...
				//canceled po lines just need supplyplanid cleared
                if (parentschedule.Completed != true && parentschedule.POCancelled != true && parentschedule.BaseQty > parentschedule.BaseQtyReceived
					&& poline.Cancelled != true
					&& !(poline.LineType == POLineType.GoodsForDropShip && poline.BaseReceivedQty != 0m))
                {
            ...
            AMProdMatlSplit parentschedule, PXSelect<AMProdItem> prodOrder, INItemPlan demand)
        {
            ...
            newPlan.PlanType = prodOrder.Current != null && (prodOrder.Current.Hold == true || prodOrder.Current.Released == false) ? INPlanConstants.PlanM5 : INPlanConstants.PlanM6;
                ...
                if (supply.Selected == true && supply.AMProdMatlSplitLineNbr == null)
                {
                    ...
                    for (int i = 0; i < ProdMatlSplits.Count; i++)
                    {
                        ...
                        if (string.IsNullOrEmpty(split.POOrderNbr) && split.IsAllocated == false && split.Completed == false && split.BaseQty > 0m)
                        {
                            ...
                            {
								if (supply.LineType == POLineType.NonStock)
								{
									supply.LineType = POLineType.NonStockForManufacturing;
								}
								else
								{
