...
namespace PX.Objects.AM
{
	...
	public class WCMaint : PXGraph<WCMaint, AMWC>
	{
		...
		public SelectFrom<AMWCCury>.Where<AMWCCury.wcID.IsEqual<AMWC.wcID.FromCurrent>
			.And<AMWCCury.curyID.IsEqual<AccessInfo.baseCuryID.AsOptional>>>.View WCCurySelected;
		...
		public class WCCurySettings : CurySettingsExtension<WCMaint, AMWC, AMWCCury>
		{
			...
			protected override List<Type> ComposeCommand()
			{
				List<Type> list = new List<Type>(15)
				{
					...
					typeof(AMWCCury),
					typeof(Where<,,>),
					typeof(AMWCCury.wcID),
					typeof(Equal<>),
					typeof(Current<>),
					typeof(AMWC.wcID),
					typeof(And<,,>),
					typeof(AMWCCury.detailID),
					typeof(Equal<DetailType.wc>),
					typeof(And<,>),
					typeof(AMWCCury.curyID),
					typeof(Equal<>),
					typeof(Optional<>),
		...
		public class WCMachCurySettings : CurySettingsExtension<WCMaint, AMWCMach, AMWCMachCury>
		{
			public static bool IsActive() => true;
			...
			protected override List<Type> ComposeCommand()
			{
				List<Type> list = new List<Type>(15)
				{
					...
					typeof(AMWCMachCury),
					typeof(Where<,,>),
					typeof(AMWCMachCury.wcID),
					typeof(Equal<>),
					typeof(Current<>),
					typeof(AMWCMach.wcID),
					typeof(And<,,>),
					typeof(AMWCMachCury.detailID),
					typeof(Equal<>),
					typeof(Current<>),
					typeof(AMWCMach.machID),
					typeof(And<,>),
					typeof(AMWCMachCury.curyID),
					typeof(Equal<>),
					typeof(Optional<>),
					typeof(AccessInfo.baseCuryID)
				};
		...
		protected virtual IEnumerable wCMachines()
		{
			this.Caches[typeof(AMWCMachCury)].ClearQueryCache();

			foreach (PXResult<AMWCMach, AMWCMachCury> result in SelectFrom<AMWCMach>
				.LeftJoin<AMWCMachCury>.On<AMWCMachCury.wcID.IsEqual<AMWCMach.wcID>
					.And<AMWCMachCury.detailID.IsEqual<AMWCMach.machID>>
					.And<AMWCMachCury.curyID.IsEqual<AccessInfo.baseCuryID.FromCurrent>>>
				.Where<AMWCMach.wcID.IsEqual<AMWC.wcID.FromCurrent>>
				.View.Select(this))
			{
				var master = (AMWCMach)result;
				var cury = (AMWCMachCury)result;
				if (master == null)
					yield break;				

				master.StdCost = cury?.StdCost ?? 0;
				yield return master;
			}
		}
        ...
        protected virtual void AMWCMach_MachID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            ...
            PXResult<AMMach, AMMachCurySettings> result =
				(PXResult<AMMach, AMMachCurySettings>)PXSelectJoin<AMMach,
				...
				Where<AMMach.machID, Equal<Required<AMMach.machID>>>>.Select(this,
                    workCenterMachine.MachID);

			var machineMaster = (AMMach)result;
            ...
            {
				workCenterMachine.StdCost = machCury?.StdCost ?? 0;
        ...
        protected virtual void AMWCMach_MachineOverride_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            ...
            if (machineMaster != null && !string.IsNullOrEmpty(workCenterMachine.MachID))
            {
				var machineCuryMaster = AMMachCurySettings.PK.Find(this, machineMaster.MachID, Accessinfo.BaseCuryID);
                workCenterMachine.StdCost = machineCuryMaster?.StdCost ?? 0;
                workCenterMachine.MachAcctID = machineMaster.MachAcctID;
                workCenterMachine.MachSubID = machineMaster.MachSubID;
            }
        }
