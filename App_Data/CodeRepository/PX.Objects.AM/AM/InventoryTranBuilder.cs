...
namespace PX.Objects.AM
{
    ...
    public class InventoryTranBuilder
    {
        ...
        private INRegisterEntryBase _inGraph;
        ...
        public virtual void UpdateInRegister()
        {
            ...
            if (_transactionType == TransactionType.Adjustment && ((INAdjustmentEntry)_inGraph).adjustment.Current != null)
            {
                ...
                register.FinPeriodID = PostPeriod;
                register.Hold = true;
				register.IgnoreAllocationErrors = true;
		...
		public IEnumerable<INTran> GetTransactions()
		{
			if (_transactionType == TransactionType.IssueReturn)
			{
				return ((INIssueEntry)_inGraph)?.transactions?.Select()?.RowCast<INTran>();
			}
			if (_transactionType == TransactionType.Receipt)
			{
				return ((INReceiptEntry)_inGraph)?.transactions?.Select()?.RowCast<INTran>();
			}
			return ((INAdjustmentEntry)_inGraph)?.transactions?.Select()?.RowCast<INTran>();
		}

		public void DeleteAllTranLines()
		{
			foreach (var row in GetTransactions())
			{
				if (row?.Released == true)
				{
					continue;
				}
				_inGraph.Caches[typeof(INTran)].Delete(row);
			}
		}
        ...
        protected virtual void CreateScrapINLines(AMMTran scrapTran, List<AMMTranSplit> scrapSplits, AMMTran wipTran, decimal qty, decimal inTranAmount)
        {
			...
			_inGraph.CostCenterDispatcherExt?.SetCostLayerType(line);
            line = UpdateLine(line);
        ...
        protected virtual INTran CreateStockItemINAdjustmentLine(AMMTran ammTran, decimal tranQty, decimal unitCost)
        {
			...
			line.UnitCost = GetAdjustmentLineUnitCost(line, ammTran.UOM, unitCost);
		...
		private decimal GetAdjustmentLineUnitCost(INTran inTran, string uom, decimal unitCost)
		{
			return inTran != null && !string.IsNullOrWhiteSpace(inTran.OrigRefNbr) && inTran?.Qty < 0
				? (inTran?.UnitCost ?? 0m)
				: GetUomUnitCost(uom, inTran, unitCost);
		}
        ...
        protected virtual AMMTran CreateStockItemINLine(AMMTran ammTran, List<AMMTranSplit> splits, decimal? qty, decimal? unitCost)
        {
            ...
            if (_transactionType == TransactionType.IssueReturn)
            {
                ...
                if (tranQty < 0 && ammTran.TranType == AMTranType.Return)
                {
					...
					line.TranCost = Math.Abs(ammTran.TranAmt.GetValueOrDefault());
					line.ExactCost = true;
                }
            ...
            if (splits != null && splits.Count > 0)
            {
				var prodItem = AMProdItem.PK.Find(_inGraph, ammTran.OrderType, ammTran.ProdOrdID);
				var demandPlans = GetDemandPlans(ammTran, prodItem);
                ...
                foreach (PXResult<INTranSplit, INItemPlan> mergedResult in mergedResults)
                {
                    ...
                    if (inItemPlan?.PlanID != null)
                    {
						...
						//if prod order was completed short of SO demand, 
						//attach SO demand to this new plan record
						var origDemand = INItemPlan.PK.Find(_inGraph, calcPlanType.DemandPlanID);
						if (origDemand != null && (prodItem?.Completed == true || inItemPlan.PlanQty >= prodItem?.QtyRemaining)
							&& inItemPlan.PlanQty < origDemand.PlanQty)
						{
							origDemand.SupplyPlanID = inItemPlan.PlanID;
							_inGraph.Caches<INItemPlan>().Update(origDemand);
						}
			...
			line = (INTran)_inGraph.Caches<INTran>().Locate(line);
			if(line != null)
			{				
				if (Math.Abs((decimal)ammTran.Qty) != line.Qty)
			{
		...
		{
			return GetDemandPlans(line, AMProdItem.PK.Find(_inGraph, line.OrderType, line.ProdOrdID));
		}


		protected virtual List<PXResult<INItemPlan, INPlanType>> GetDemandPlans(AMMTran line, AMProdItem prodItem)
        {
