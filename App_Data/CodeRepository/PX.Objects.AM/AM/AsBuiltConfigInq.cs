...
namespace PX.Objects.AM
{
        ...
        protected virtual IEnumerable tree([PXString]string parentID, [PXString]string matlLine, [PXInt]int? level)
        {
            ...
            if(level == null)
            {
                var filterResults = SelectFrom<AMProdItem>
                    ...
                    .And<AMProdItem.canceled.IsNotEqual<True>>
        ...
        protected virtual IEnumerable prodLotSerialRecs([PXString]string selectedValue)
        {
            ...
            {                
                ...
                foreach(PXResult<AMProdMatlLotSerial, AMProdItem, AMProdMatl, InventoryItem> prodMatl in prodMatls)
                {
					...
					var filter = Filter.Current;
					if (filter != null && (filter.LotSerialNbr != null && string.Equals(filter.LotSerialNbr, matlLot.ParentLotSerialNbr, StringComparison.OrdinalIgnoreCase) || filter.ProdOrdID != null ||
	...
	/// <summary>
	/// The required filter for displaying the information on the As-Built Configuration (AM401700) form (corresponding to the <see cref="AsBuiltConfigInq"/> graph).
	/// </summary>
	[Serializable]
    ...
    public class AsBuiltConfigFilter : IBqlTable
    {
        ...
        [PXString]
        [PXSelector(typeof(Search5<AMProdItemSplit.lotSerialNbr,
			InnerJoin<AMProdItem, On<AMProdItem.orderType, Equal<AMProdItemSplit.orderType>,
				And<AMProdItem.prodOrdID, Equal<AMProdItemSplit.prodOrdID>>>,
			InnerJoin<Branch, On<AMProdItem.branchID, Equal<Branch.branchID>>>>,
			Where<AMProdItemSplit.lotSerialNbr, IsNotNull,
				And<Branch.baseCuryID, Equal<Current<AccessInfo.baseCuryID>>>>,
			Aggregate<GroupBy<AMProdItemSplit.lotSerialNbr>>>))]
        ...
        [PXSelector(typeof(Search<SOOrder.orderNbr, Where<SOOrder.curyID, Equal<Current<AccessInfo.baseCuryID>>>>))]
        ...
        [PXString]
        [PXSelector(typeof(Search2<AMProdItem.prodOrdID,
			InnerJoin<Branch, On<AMProdItem.branchID, Equal<Branch.branchID>>>,
