...
namespace PX.Objects.AM
{
    ...
    public class ReferenceDeleteGraph : PXGraph<ReferenceDeleteGraph>
    {
        ...
        protected virtual void PersistAMDeletes()
        {
            ...
            var amWIPBatNBrs = new UniqueStringCollection();
            foreach (AMBatch row in this.Caches[typeof(AMBatch)].Deleted)
            {
                ...
                if (row.DocType == AMDocType.WipAdjust)
                {
                    amWIPBatNBrs.Add(row.BatNbr);
                }
            }

            var materialTranList = new List<AMMTran>();
            var costTranList = new List<AMMTran>();
            var wipTranList = new List<AMMTran>();
            ...
            PersistWithProductionWIPAdjustmentEntry(amWIPBatNBrs, wipTranList);
        }
        ...
        protected virtual void PersistWithMaterialEntry(UniqueStringCollection amMatlBatNBrs, List<AMMTran> ammTranList)
        {
            ...
            foreach (var amMatlBatNBr in amMatlBatNBrs)
            {
                ...
                {
					using (new DisableDecrementOnDeleteLineNbrScope(amMaterialEntryGraph.transactions.Cache, typeof(AMMTran.lineNbr)))
					using (new DisableDecrementOnDeleteLineNbrScope(amMaterialEntryGraph.splits.Cache, typeof(AMMTranSplit.splitLineNbr)))
					{
        ...
        protected virtual void PersistWithProductionCostEntry(UniqueStringCollection amCostBatNBrs, List<AMMTran> ammTranList)
        {
            ...
            foreach (var amCostBatNbrs in amCostBatNBrs)
            {
                ...
                if (DeleteMode == DeleteModeType.Batch)
                {
					using (new DisableDecrementOnDeleteLineNbrScope(amProductionCostEntryGraph.transactions.Cache, typeof(AMMTran.lineNbr)))
					{
        ...
        protected virtual void PersistWithProductionWIPAdjustmentEntry(UniqueStringCollection amWIPBatNBrs, List<AMMTran> ammTranList)
        {
            if (amWIPBatNBrs == null || !amWIPBatNBrs.HasValues)
            {
                return;
            }

            var wipAdjustmentEntryGraph = CreateInstance<WIPAdjustmentEntry>();
            wipAdjustmentEntryGraph.batch.AllowDelete = true;
            wipAdjustmentEntryGraph.transactions.AllowDelete = true;

            if (wipAdjustmentEntryGraph.ampsetup.Current != null)
            {
                wipAdjustmentEntryGraph.ampsetup.Current.RequireControlTotal = false;
            }
            
            foreach (var wipBatNbrs in amWIPBatNBrs)
            {
                wipAdjustmentEntryGraph.Clear();

                AMBatch amBatch = PXSelect<AMBatch,
                    Where<AMBatch.docType, Equal<AMDocType.wipAdjust>,
                        And<AMBatch.batNbr, Equal<Required<AMBatch.batNbr>>>>>.Select(wipAdjustmentEntryGraph, wipBatNbrs);
                ...
                wipAdjustmentEntryGraph.batch.Current = amBatch;

                if (DeleteMode == DeleteModeType.Batch)
                {
					using (new DisableDecrementOnDeleteLineNbrScope(wipAdjustmentEntryGraph.transactions.Cache, typeof(AMMTran.lineNbr)))
					{
						wipAdjustmentEntryGraph.transactions.Select();
						wipAdjustmentEntryGraph.batch.Delete(wipAdjustmentEntryGraph.batch.Current);
						wipAdjustmentEntryGraph.Persist();
					}
                    continue;
                }
                ...
                foreach (var ammTran in ammTranList)
                {
                    wipAdjustmentEntryGraph.transactions.Delete(ammTran);
                }

                if (wipAdjustmentEntryGraph.IsDirty)
                {
                    wipAdjustmentEntryGraph.Persist();
                }
            }
        ...
        protected virtual void PersistGLDeletes()
        {
            ...
            foreach (var glBatchNbr in glBatchNbrs)
            {
                ...
                if (DeleteMode == DeleteModeType.Batch)
                {
					using (new DisableDecrementOnDeleteLineNbrScope(journalEntry.GLTranModuleBatNbr.Cache, typeof(GLTran.lineNbr)))
					{
        ...
        protected static void PersistINReceiptDeletes(INReceiptEntry graph, INRegister inRegister)
        {
			...
			using (new DisableDecrementOnDeleteLineNbrScope(graph2.transactions.Cache, typeof(INTran.lineNbr)))
			using (new DisableDecrementOnDeleteLineNbrScope(graph2.splits.Cache, typeof(INTranSplit.splitLineNbr)))
			{
