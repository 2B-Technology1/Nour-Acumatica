...
{
    public class ConfigurationMaint : PXRevisionableGraph<ConfigurationMaint, AMConfiguration, AMConfiguration.configurationID, AMConfiguration.revision>, PXImportAttribute.IPXPrepareItems, PXImportAttribute.IPXProcess
    {
        #region Views
		...
		protected virtual IEnumerable featureOptions()
		{
			this.Caches[typeof(AMConfigurationOption)].ClearQueryCache();
			this.Caches[typeof(AMConfigurationOptionCurySettings)].ClearQueryCache();

			bool itVar1 = false;
			IEnumerator enumerator = this.FeatureOptions.Cache.Inserted.GetEnumerator();
			while(enumerator.MoveNext())
			{
				AMConfigurationOption master = (AMConfigurationOption)enumerator.Current;
				if (master.ConfigFeatureLineNbr == ConfigurationFeatures.Current?.LineNbr)
				{
				itVar1 = true;
				yield return master;
			}
			...
			if (!itVar1)
			{
				foreach (PXResult<AMConfigurationOption, AMConfigurationOptionCurySettings> result in PXSelectJoin<AMConfigurationOption,
				LeftJoin<AMConfigurationOptionCurySettings, On<AMConfigurationOptionCurySettings.configurationID, Equal<AMConfigurationOption.configurationID>,
					And<AMConfigurationOptionCurySettings.revision, Equal<AMConfigurationOption.revision>,
					And<AMConfigurationOptionCurySettings.configFeatureLineNbr, Equal<AMConfigurationOption.configFeatureLineNbr>,
					And<AMConfigurationOptionCurySettings.lineNbr, Equal<AMConfigurationOption.lineNbr>,
					And<AMConfigurationOptionCurySettings.curyID, Equal<Current<AccessInfo.baseCuryID>>>>>>>>,
				Where<AMConfigurationOption.configurationID,
					Equal<Current<AMConfigurationFeature.configurationID>>,
				And<AMConfigurationOption.revision,
					Equal<Current<AMConfigurationFeature.revision>>,
				...
				OrderBy<Asc<AMConfigurationOption.configurationID,
						...
						Asc<AMConfigurationOption.configFeatureLineNbr,
						Asc<AMConfigurationOption.sortOrder,
						Asc<AMConfigurationOption.lineNbr>>>>>>>.Select(this))
				{
					var option = (AMConfigurationOption)result;
					var optionCury = (AMConfigurationOptionCurySettings)result;
					if (option == null)
						yield break;

					option.SiteID = optionCury?.SiteID;
					option.LocationID = optionCury?.LocationID;
					yield return option;
				} 
			}
		...
		public class CurySettings : CurySettingsExtension<ConfigurationMaint, AMConfigurationOption, AMConfigurationOptionCurySettings>
		{
			...
			protected override List<Type> ComposeCommand()
			{
				...
				{
					typeof(Select<,>),
					typeof(AMConfigurationOptionCurySettings),
					typeof(Where<,,>),
					typeof(AMConfigurationOptionCurySettings.configurationID),
					typeof(Equal<>),
					typeof(Current<>),
					typeof(AMConfigurationOption.configurationID),
					typeof(And<,,>),
					typeof(AMConfigurationOptionCurySettings.revision),
					typeof(Equal<>),
					typeof(Current<>),
					typeof(AMConfigurationOption.revision),
					typeof(And<,,>),
					typeof(AMConfigurationOptionCurySettings.configFeatureLineNbr),
					typeof(Equal<>),
					typeof(Current<>),
					typeof(AMConfigurationOption.configFeatureLineNbr),
					typeof(And<,,>),
					typeof(AMConfigurationOptionCurySettings.lineNbr),
					typeof(Equal<>),
					typeof(Current<>),
					typeof(AMConfigurationOption.lineNbr),
					typeof(And<,>),
					typeof(AMConfigurationOptionCurySettings.curyID),
					typeof(Equal<>),
					typeof(Optional<>),
        ...
        public override void CopyRevision(ConfigurationMaint fromGraph, ConfigurationMaint toGraph, string keyValue, string revisionValue)
        {
            ...
            foreach (AMConfigurationFeature feature in fromGraph.ConfigurationFeatures.Select())
            {
                ...
                foreach (AMConfigurationOption option in fromGraph.FeatureOptions.Select())
                {
                    ...
                    if (copyOption.OperationID != null && AMBomOper.PK.Find(toGraph, toGraph.Documents.Current.BOMID,
                            toGraph.Documents.Current.BOMRevisionID, copyOption.OperationID) == null)
                    {
                        copyOption.OperationID = null;
                    }
                    copyOption.ConfigurationID = keyValue;
                    copyOption.Revision = revisionValue;
                    toGraph.FeatureOptions.Insert(copyOption);
                }
        ...
        protected virtual void _(Events.RowSelected<AMConfiguration> e)
        {
            var row = e.Row;
            if (row == null)
            {
            ...
            PXUIFieldAttribute.SetVisible<AMConfiguration.keyEquation>(e.Cache, row, enableFormula);
            PXUIFieldAttribute.SetRequired<AMConfiguration.keyEquation>(e.Cache, enableFormula);
            PXDefaultAttribute.SetPersistingCheck<AMConfiguration.keyEquation>(e.Cache, row, enableFormula ? PXPersistingCheck.NullOrBlank : PXPersistingCheck.Nothing);
            PXUIFieldAttribute.SetVisible<AMConfiguration.keyNumberingID>(e.Cache, row, enableNumSequence);
            PXUIFieldAttribute.SetEnabled<AMConfiguration.keyNumberingID>(e.Cache, row, enableNumSequence);
            PXUIFieldAttribute.SetRequired<AMConfiguration.keyNumberingID>(e.Cache, enableNumSequence);
            PXDefaultAttribute.SetPersistingCheck<AMConfiguration.keyNumberingID>(e.Cache, row, enableNumSequence ? PXPersistingCheck.NullOrBlank : PXPersistingCheck.Nothing);

            bool isPersisted = Documents.Cache.GetStatus(row) != PXEntryStatus.Inserted;

            // Price Tab
            PXUIFieldAttribute.SetEnabled<AMConfiguration.priceRollup>(e.Cache, row, ConfiguratorSetup.Current.AllowRollupOverride == true);
            PXUIFieldAttribute.SetEnabled<AMConfiguration.priceCalc>(e.Cache, row, ConfiguratorSetup.Current.AllowCalculateOverride == true);
            ...
            FeatureRules.AllowDelete = row.Status == ConfigRevisionStatus.Pending;
            ...
            PXUIFieldAttribute.SetEnabled<AMConfiguration.bOMID>(e.Cache, row, row.Status == ConfigRevisionStatus.Pending && !isPersisted);
        }
...
#pragma warning restore PX1073
            ...
            if (!FeatureOptions.Select().Any())
            {
                                                                ...
                                                                Equal<Required<AMFeatureOption.featureID>>>>.Select(this, row.FeatureID))
                {
                    var insertedOption = FeatureOptions.Insert(new AMConfigurationOption
                    {
						...
						ConfigFeatureLineNbr = row.LineNbr
                    });
        ...
        protected virtual void _(Events.RowPersisting<AMConfigurationOption> e)
        {
            var row = e.Row;
            if (row == null)
            {
            ...
            PXDefaultAttribute.SetPersistingCheck<AMConfigurationOption.operationID>(e.Cache, row,
                row.InventoryID != null && row.OperationID == null && row.MaterialType != AMMaterialType.Supplemental
            ...
            {
                e.Cache.RaiseExceptionHandling<AMConfigurationOption.inventoryID>(
                    row,
                    row.InventoryID,
            ...
            {
                var inventoryItem = (InventoryItem)PXSelectorAttribute.Select<AMConfigurationOption.inventoryID>(e.Cache, row);
                if (inventoryItem?.StkItem != true)
                {
                    return;
                }

                e.Cache.RaiseExceptionHandling<AMConfigurationOption.subItemID>(
                        row,
                        row.SubItemID,
        ...
        public sealed override void Configure(PXScreenConfiguration config) =>
            Configure(config.GetScreenConfigurationContext<ConfigurationMaint, AMConfiguration>());
        protected static void Configure(WorkflowContext<ConfigurationMaint, AMConfiguration> context)
        {
		...
		public bool PrepareImportRow(string viewName, IDictionary keys, IDictionary values)
		{
			...
			if (string.Compare(viewName, AttributeRules.View.Name, true) == 0)
			{
				PXCache attributeRuleCache = AttributeRules.Cache;
				AMConfigurationAttributeRule item = (AMConfigurationAttributeRule)(AttributeRules.Current ?? attributeRuleCache.CreateInstance());
				try
				{
					attributeRuleCache.SetValueExt<AMConfigurationAttributeRule.targetFeatureLineNbr>(item, GetImportedValue<AMConfigurationAttributeRule.targetFeatureLineNbr>(Views[viewName], values, true));
					attributeRuleCache.SetValueExt<AMConfigurationAttributeRule.targetOptionLineNbr>(item, GetImportedValue<AMConfigurationAttributeRule.targetOptionLineNbr>(Views[viewName], values, true));
					attributeRuleCache.Update(item);
				}
				catch (Exception e)
				{
		...
		private object GetImportedValue<Field>(PXView view, IDictionary values, bool isRequired) where Field : IBqlField
		{
			var item = view.Cache.CreateInstance();
			string displayName = PXUIFieldAttribute.GetDisplayName<Field>(view.Cache);
			...
			view.Cache.RaiseFieldUpdating<Field>(item, ref value);
		...
		/// <summary>
		/// An indicator for storing the Configuration ID by the selected levels as configured on pop-up panel on the Configuration Maintenance (AM207500) form (corresponding to the <see cref="ConfigurationMaint"/> graph).
		/// </summary>
		[Serializable]
