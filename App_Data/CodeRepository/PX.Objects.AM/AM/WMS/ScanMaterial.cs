	...
	public class ScanMaterial : WMSBase
	{
		...
		public sealed class MaterialMode : ScanMode
		{
			...
			protected override IEnumerable<ScanState<ScanMaterial>> CreateStates()
			{
				...
				yield return new ProdOrdState().Intercept.IsStateSkippable.ByDisjoin(basis => basis.ProdOrdID != null);
				yield return new OperationState().Intercept.IsStateSkippable.ByDisjoin(basis => basis.OperationID != null);
				...
				yield return new LotSerialState().Intercept.IsStateActive.ByConjoin(basis => basis.IsLotSerialRequired());
				yield return new ExpireDateState().Intercept.IsStateActive.ByConjoin(basis => basis.IsExpirationDateRequired());
			...
			protected override void ResetMode(bool fullReset = false)
			{
				...
				Clear<ProdOrdState>(when: fullReset);
				Clear<OperationState>(when: fullReset);
				...
				public class Logic : ScanExtension
				{
					...
					protected virtual bool HasSplitLotSerialError(string lotSerialNbr, out FlowStatus error)
					{
						if (!string.IsNullOrEmpty(lotSerialNbr) &&
							Basis.Graph.splits.SelectMain().Any(s => !string.Equals(s.LotSerialNbr, lotSerialNbr, StringComparison.OrdinalIgnoreCase)))
						{
						...
						if (!string.IsNullOrEmpty(Basis.ParentLotSerialNbr))
						{
					...
					{
						var errorInfo = Basis.Graph.ItemAvailabilityExt.GetCheckErrors(tran, CostCenter.FreeStock).FirstOrDefault();
						if (errorInfo != null)
						{
