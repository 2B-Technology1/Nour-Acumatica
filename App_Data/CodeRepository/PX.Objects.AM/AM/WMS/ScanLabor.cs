	...
	public class ScanLabor : WMSBase
	{
		...
		public sealed class LaborMode : ScanMode
		{
			...
			protected override IEnumerable<ScanState<ScanLabor>> CreateStates()
			{
                ...
                yield return new LotSerialState().Intercept.IsStateActive.ByConjoin(basis => basis.ItemHasLotSerial && basis.SelectedLotSerialClass.LotSerAssign != INLotSerAssign.WhenUsed && basis.IsLastOperation() && basis.IsLotSerialRequired());
                yield return new ExpireDateState().Intercept.IsStateActive.ByConjoin(basis => basis.IsLastOperation() && basis.IsExpirationDateRequired());
			...
			public sealed class ConfirmState : ConfirmationState
			{
				...
				public class Logic : ScanExtension
				{
					...
					protected virtual bool CanConfirm(out FlowStatus error)
					{
						...
						if (Basis.CurrentMode.HasActive<LotSerialState>() && !Basis.IsIndirectLabor && Basis.SelectedLotSerialClass.LotSerTrack == INLotSerTrack.SerialNumbered && Basis.Qty != 1)
						{
