...
namespace PX.Objects.AM
{
    public class ConfigurationCopyEngine
    {
		...
		protected virtual void RecalculateFeatureTotalQty(List<AMConfigResultsFeature> featureResults)
		{
			...
			{
				featureResult.TotalQty = 0;
				PXUnboundFormulaAttribute.CalcAggregate<AMConfigResultsOption.actualQty>(_graph.Caches[typeof(AMConfigResultsOption)], featureResult);
				_graph.Caches[typeof(AMConfigResultsFeature)].Update(featureResult);
			}
		}
        ...
        protected virtual void UpdateOptions(AMConfigurationResults toConfigResult, AMConfigurationResults fromConfigResult)
        {
            ...
            using(new RemoveMultiFormulaAggregrateScope<PXUnboundFormulaAttribute>(_graph.Caches[typeof(AMConfigResultsOption)], typeof(AMConfigResultsOption.actualQty)))
			{
			...
			if (optionsUpdated)
			{
				RecalculateFeatureTotalQty(PXSelect<AMConfigResultsFeature,
							Where<AMConfigResultsFeature.configResultsID, Equal<Required<AMConfigResultsFeature.configResultsID>>>>
				.Select(_graph, toConfigResult.ConfigResultsID).ToFirstTableList());
			}
		}
