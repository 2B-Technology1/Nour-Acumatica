...
namespace PX.Objects.AM
{
    ...
    public class MaterialTranBuilder
    {
        ...
        public MaterialTranBuilder(PXGraph graph)
        {
            ...
            _StatusByLotSerial = true;
        }

		private bool _QueryByAllocated { get; set; }
            ...
            INLotSerClass lotSerClass, decimal totalIssueQty, string issueUom, int? siteID, int? locationID, out string exception)
        {
			...
			var remainingIssueBaseQty = ConvertToBaseUOM<AMProdMatl.inventoryID>(_graph.Caches<AMProdMatl>(), prodMatl, inventoryItem,
					remainingIssueQty, issueUom);
			if (remainingIssueBaseQty <= 0)
			{
				return retList;
			}
                ...
                .Select(_graph, prodMatl.OrderType, prodMatl.ProdOrdID, prodMatl.OperationID, prodMatl.LineID))
            {
                ...
                var returnBaseQty = Math.Min(Math.Min(rtBaseQty, remainingIssueBaseQty), Math.Abs(splitBaseQty));
        ...
        public virtual List<AMMTran> BuildMaterialIssueTransactions(AMProdMatl prodMatl, InventoryItem inventoryItem, INLotSerClass lotSerClass, decimal totalIssueQty, string issueUom, int? siteID, int? locationID, out string exception)
        {
            ...
            if (inventoryItem.StkItem.GetValueOrDefault())
            {
                ...
                var isLotSerialItem = StatusByLotSerial
                    && lotSerClass.LotSerTrack != INLotSerTrack.NotNumbered
                    && lotSerClass.LotSerAssign != INLotSerAssign.WhenUsed;
				var isUserEnterableLotSerial = isLotSerialItem && lotSerClass.LotSerIssueMethod == INLotSerIssueMethod.UserEnterable;
				var lotSerialAllocatedSplits = (isLotSerialItem && splits != null)
					? (string.IsNullOrWhiteSpace(prodMatl?.LotSerialNbr)
						? splits.Where(x => x.IsAllocated == true && !string.IsNullOrWhiteSpace(x.LotSerialNbr))
						: splits.Where(x => x.IsAllocated == true && string.Equals(x.LotSerialNbr, prodMatl.LotSerialNbr, StringComparison.OrdinalIgnoreCase)))
					: null;
				var hasCheckedAllocatedByLotSerialNbr = isLotSerialItem && lotSerialAllocatedSplits != null && lotSerialAllocatedSplits.Any();
				if (hasCheckedAllocatedByLotSerialNbr)
				{
					_QueryByAllocated = true;
					foreach (var allocatedSplit in lotSerialAllocatedSplits)
					{
					...
					_QueryByAllocated = false;
				}

				if (isLotSerialItem && remainingQty > 0 && !isUserEnterableLotSerial)
                {
					// Possible to have allocations not restricted by Lot/Serial Nbr
					if (splits != null)
					{
						foreach (var allocatedSplit in splits.Where(x => x.IsAllocated == true && string.IsNullOrWhiteSpace(x.LotSerialNbr)))
						{
							var allocatedLocation = allocatedSplit.LocationID != null
									? allocatedSplit.LocationID
									: (allocatedSplit.SiteID == prodMatl.SiteID ? prodMatl.LocationID : null);
							CreateAMMTranLotSerial(_graph.Caches<AMProdMatl>(), prodMatl, splits, inventoryItem, lotSerClass, issueUom, siteID,
								allocatedLocation, null, ref ammTrans, ref remainingQty);
						} 
					}
                ...
                {
					if (remainingQty > 0 && isUserEnterableLotSerial && IsBackflush)
					{
						var prodOper = AMProdOper.PK.Find(_graph, prodMatl.OrderType, prodMatl.ProdOrdID, prodMatl.OperationID);
						exception = Messages.GetLocal(Messages.BackflushUserEnterableNotAllocated,
							inventoryItem.InventoryCD.TrimIfNotNullEmpty(),
							prodOper?.OperationCD ?? $"({prodMatl.OperationID})",
							prodMatl.OrderType,
							prodMatl.ProdOrdID);
						PXTrace.WriteInformation(exception);
						return new List<AMMTran>();
					}
					...
					if (splits != null)
                    {
                        ...
                        {
							if(hasCheckedAllocatedByLotSerialNbr && !string.IsNullOrWhiteSpace(allocatedSplit.LotSerialNbr))
							{
								// has mixed allocations
								continue;
							}
							var allocatedLocation = allocatedSplit.LocationID != null
										? allocatedSplit.LocationID
										: (allocatedSplit.SiteID == prodMatl.SiteID ? prodMatl.LocationID : null);
							CreateAMMTranLocation(_graph.Caches<AMProdMatl>(), prodMatl, splits, inventoryItem, issueUom, siteID, allocatedLocation, ref ammTrans, ref remainingQty);
                        }
						_QueryByAllocated = false;
					}
            ...
            {
                var prodOper = AMProdOper.PK.Find(_graph, prodMatl.OrderType, prodMatl.ProdOrdID, prodMatl.OperationID);
                exception = Messages.GetLocal(IsBackflush ? Messages.BackFlushMaterialShortage : Messages.BuildingTransactionMaterialShortage,
            ...
            int? siteId, int? locationId, ref List<AMMTran> ammTrans, ref decimal remainingQty)
        {
            ...
            foreach (INLocationStatusByCostCenter locationStatus in GetLocationStatus(prodMatl.InventoryID, prodMatl.SubItemID, siteId, locationId))
            {
        ...
        protected virtual AMMTran CreateAMMTranLocation(PXCache cache, AMProdMatl prodMatl, InventoryItem inventoryItem, INLocationStatusByCostCenter locationStatus, decimal issueQty, string issueUom)
        {
            ...
            int? siteId, int? locationId, string lotSerialNbr, ref List<AMMTran> ammTrans, ref decimal remainingQty)
        {
            ...
            foreach (var result in GetLotSerialStatus(lotSerClass, prodMatl.InventoryID, prodMatl.SubItemID, siteId, locationId, lotSerialNbr))
            {
                var lotSerialStatus = result.GetItem<INLotSerialStatusByCostCenter>();
                var itemLotSerial = result.GetItem<INItemLotSerial>();
                var lotSplitAlloc = splits?.Where(x => x.IsAllocated == true && string.Equals(x.LotSerialNbr, itemLotSerial.LotSerialNbr, StringComparison.OrdinalIgnoreCase)).Sum(x => x.BaseQty) ?? 0m;
                var processedQty = GetExcludedBaseQty(ammTrans, prodMatl, lotSerialStatus);

                if (lotSerialStatus != null && itemLotSerial != null)
                {
					if (lotSplitAlloc > 0 && !string.IsNullOrWhiteSpace(lotSerialNbr))
					{
						lotSerialStatus.QtyHardAvail = lotSplitAlloc - processedQty;
					}
					else
					{
        ...
        /// <param name="issueUom">Transaction unit of measure</param>
        /// <returns>new built transaction line</returns>
        protected virtual AMMTran CreateAMMTranLotSerial(PXCache cache, AMProdMatl prodMatl, InventoryItem inventoryItem, INLotSerialStatusByCostCenter lotSerialStatus, decimal issueQty, string issueUom)
        {
            if(inventoryItem?.InventoryID == null)
            {
                throw new ArgumentNullException(nameof(inventoryItem));
        ...
        protected static AMMTran CreateAMMTran(AMProdMatl prodMatl, INLocationStatusByCostCenter locationStatus, decimal tranQty, string tranUOM)
        {
            return CreateAMMTran(prodMatl, locationStatus, tranQty, tranUOM, null);
        }

        protected static AMMTran CreateAMMTran(AMProdMatl prodMatl, INLocationStatusByCostCenter locationStatus, decimal tranQty, string tranUOM, decimal? baseQty)
        {
        ...
        protected static AMMTran CreateAMMTran(AMProdMatl prodMatl, INLotSerialStatusByCostCenter lotSerialStatus, decimal tranQty, string tranUOM)
        {
            return CreateAMMTran(prodMatl, lotSerialStatus, tranQty, tranUOM, null);
        }

        protected static AMMTran CreateAMMTran(AMProdMatl prodMatl, INLotSerialStatusByCostCenter lotSerialStatus, decimal tranQty, string tranUOM, decimal? baseQty)
        {
            var ammTran = CreateAMMTran(prodMatl, tranQty, tranUOM, baseQty);
        ...
        protected virtual PXResultset<INLocationStatusByCostCenter> GetLocationStatus(int? inventoryID, int? subitemID, int? siteID, int? locationID)
        {
            return _QueryByAllocated
				? InventoryHelper.GetLocationStatusProductionAllocatedQty(_graph, inventoryID, subitemID, siteID, locationID, null, true, true)
                : InventoryHelper.GetLocationStatus(_graph, inventoryID, subitemID, siteID, locationID, null, true, true);
        }

        protected virtual PXResultset<INLotSerialStatusByCostCenter> GetLotSerialStatus(INLotSerClass lotSerClass, int? inventoryID, int? subitemID, int? siteID, int? locationID, string lotSerialNbr)
        {
            return _QueryByAllocated
				? InventoryHelper.GetLotSerialStatusProductionAllocatedQty(_graph, lotSerClass, inventoryID, subitemID, siteID, locationID, lotSerialNbr, null, true, true)
                : InventoryHelper.GetLotSerialStatus(_graph, lotSerClass, inventoryID, subitemID, siteID, locationID, lotSerialNbr, null, true, true);
        }
        ...
        protected static decimal GetExcludedBaseQty(List<AMMTran> ammTrans, AMProdMatl prodMatl, INLotSerialStatusByCostCenter lotSerialStatus)
        {
        ...
        protected static decimal GetExcludedBaseQty(List<AMMTran> ammTrans, AMProdMatl prodMatl, INLocationStatusByCostCenter locationStatus)
        {
        ...
        public static void CreateMaterialTransaction(MaterialEntry materialEntry, List<AMMTran> newTransactions, AMMTran origAmmTran)
        {
			...
			var currentIsImport = materialEntry.IsImport == true;
			materialEntry.IsImport = true;
			...
			materialEntry.IsImport = currentIsImport;
		}
