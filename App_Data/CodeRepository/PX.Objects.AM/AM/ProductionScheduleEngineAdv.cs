...
namespace PX.Objects.AM
{
    ...
    public class ProductionScheduleEngineAdv : ProductionScheduleEngine
    {
        ...
        public virtual void UpdateProdItem(AMSchdItem schdItem)
        {
			...
			if (amProdItem.ScheduleStatus != schdItem.ScheduleStatus)
			{
				update = true;
            ...
            ref DateTime currentSchedDateTime)
        {
            ...
            while (remainingOperationSchdTime > 0)
            {
                ...
                {
                    ProcessWorkCenterSchedule(workCenterCalendar, scheduleDirection, ref schdOper, ref currentSchedDateTime, ref remainingOperationSchdTime, isFirstAttempt);
        ...
        private void ProcessWorkCenterSchedule(WorkCenterCalendarHelper workCenterCalendar, ReadDirection scheduleDirection, ref AMSchdOper schdOper, ref DateTime currentSchedDateTime, ref int remainingSchdTime, bool isFirstAttempt)
        {
			...
			foreach (var amShift in shifts)
            {
				if (isFirstAttempt && amShift.ShiftCD == lastShift.ShiftCD)
				{
					isFirstAttempt = false;
				}
                ...
                var runTimeEfficency = (useCrewEff
                    ...
                    : runTimeOnly);
				var shiftRemainingSchdTime = Math.Min(runTimeEfficency, remainingSchdTime) + finishTime;

				int shiftSchdTimePassedIn = shiftRemainingSchdTime;

				ProcessOperationShift(schdOper, amShift, wcDateInfo.ShiftDateInfos[amShift.ShiftCD],
                    workCenterCalendar, scheduleDirection, ref currentSchedDateTime, ref shiftRemainingSchdTime, isFirstAttempt);

				if (shiftSchdTimePassedIn != shiftRemainingSchdTime)
				{
					isFirstAttempt = false;
				}
		...
		/// <exception cref="SchedulingOrderDatesException"></exception>
        private void ProcessOperationShift(AMSchdOper schdOper, AMShift wcShift, AMDateInfo shiftDateInfo, WorkCenterCalendarHelper workCenterCalendar, 
            ReadDirection scheduleDirection, ref DateTime currentSchedDateTime, ref int remainingSchdTime, bool isFirstAttempt)
        {
            if (schdOper == null)
            {
                throw new ArgumentNullException(nameof(schdOper));
            ...
            if ((checkWorkCenterSchd == null) || (checkWorkCenterSchd.AvailableBlocks.GetValueOrDefault() == 0 && !isFirstAttempt))
            {
				throw new SchedulingOrderDatesException(shiftDateInfo.Date, shiftDateInfo.Date.AddDays(addDays),
					PXMessages.LocalizeFormat(Messages.AvailableTimeForWorkCenterAndShiftCouldNotBeFound, wcShift.WcID.TrimIfNotNullEmpty(), wcShift.ShiftCD.TrimIfNotNullEmpty(), shiftDateInfo.Date));
            }
        ...
        protected List<AMWCSchdDetail> UpdateWCSchdDetailBreakTimes(AMWCSchd workCenterSchd, List<AMWCSchdDetail> schdDetail, AMDateInfo dateInfo)
        {
            ...
            all.AddRange(CreateBreakTimeDetail(workCenterSchd, calendarBreakTimes, dateInfo));
        ...
        private SchdDateDetail<AMWCSchdDetail> WorkCenterSchdDateDetail(AMWCSchd workCenterSchd, AMDateInfo wcShiftDateInfo)
        {
			...
			var unavailableSchedDetails = UpdateWCSchdDetailBreakTimes(workCenterSchd, GetWCSchdDetail(workCenterSchd), wcShiftDateInfo)
				.Where(d => d.RunTime.GetValueOrDefault() > 0).OrderBy(d => d.OrderByDate.GetValueOrDefault()).ToList();

			if(unavailableSchedDetails != null && unavailableSchedDetails.Any())
			{
				AMWCSchdDetail earliestUsed = unavailableSchedDetails[0];
				var earliestUsedQueueTime = earliestUsed.QueueTime.Value;
				DateTime dayStartTime = BlockRoundedDate(new DateTime(1900, 1, 1) + wcShiftDateInfo.StartTime, BlockSize, false);
				DateTime earliestUsedStartTime = earliestUsed.StartTime.GetValueOrDefault(dayStartTime);
				DateTime earliestUnavailStartTime = earliestUsedStartTime.AddMinutes(-earliestUsedQueueTime);

				if (dayStartTime.CompareTo(earliestUnavailStartTime) == 0 && earliestUsedQueueTime > 0)
				{
					var newUnavailBlock = earliestUsed.Copy();
					newUnavailBlock.StartTime = dayStartTime;
					newUnavailBlock.EndTime = BlockRoundedDate(earliestUsedStartTime, BlockSize, true);
					newUnavailBlock.OrderByDate = dayStartTime;
					unavailableSchedDetails.Add(newUnavailBlock);
				}
			}
			return new SchdDateDetail<AMWCSchdDetail>(BlockSize, wcShiftDateInfo, unavailableSchedDetails);
        }
        ...
        private void CreateBreakTimeDetail(AMWCSchd workCenterSchd, AMDateInfo dateInfo)
        {
            ...
            CreateBreakTimeDetail(workCenterSchd, dateInfo.BreakTimes, dateInfo);
        }

        private List<AMWCSchdDetail> CreateBreakTimeDetail(AMWCSchd workCenterSchd, List<AMCalendarBreakTime> breakTimes, AMDateInfo dateInfo)
        {
            var newBreakTimes = new List<AMWCSchdDetail>();
			bool isOvernight = dateInfo.OvernightTime;			
            ...
            foreach (var calendarBreakTime in breakTimes) 
			{
				...
				if (isOvernight && calendarBreakTime.StartTime.GetValueOrDefault().ToString("tt") == "AM")
				{
					dateAdd = 1;
				}
				...
				newBreakTimes.Add(WorkCenterSchdDetailRecs.Insert(new AMWCSchdDetail
				{
					...
					SchdDate = workCenterSchd.SchdDate.GetValueOrDefault(),
					...
					RunTime = schdTime,
					SchdTime = schdTime,
					OrderByDate = workCenterSchd.SchdDate.GetValueOrDefault().Date.AddDays(dateAdd) + startTime.GetValueOrDefault().TimeOfDay
				})); 
            }
            ...
            AMProdMatl prodMatl, InventoryItem inventoryItem, AMProdOper prodOper, DateTime itemPlanDate)
        {
            ...
            var itemStatus = InventoryHelper.GetSiteStatusSum(graph, prodMatl?.InventoryID, prodMatl?.SubItemID, prodMatl?.SiteID);
