...
namespace PX.Objects.AM
{
    ...
    public class SchedulePlanBomCopy : ProductionBomCopyBase
    {
        ...
        protected virtual AMSchdOper CreateScheduleDetail(AMSchdOper amSchdOper, AMBomOper sourceAmBomOper, decimal phtmpriorlevelqty)
        {
			...
			var useCrewEff = Setup.Current?.UseShiftCrewSize == true && amWorkCenter != null && amWorkCenter.WcBasis == AMWC.BasisForCapacity.CrewSize;
			...
			if (useCrewEff)
			{
				totalTime = ProductionScheduleEngine.CalcTotalEfficiencyTime(totalTime, amShift.CrewSize, amShift.ShftEff);
			}
        ...
        protected virtual List<AMPlanMaterial> GetOrderMaterial(DateTime? materialDateTime)
        {
            ...
            foreach (var bomRevision in uniqueBomRevisions)
            {
				...
				foreach (PXResult<AMBomMatl, AMBomItem, AMBomMatlCury> result in SelectFrom<AMBomMatl>
					.InnerJoin<AMBomItem>
						.On<AMBomMatl.bOMID.IsEqual<AMBomItem.bOMID>
							.And<AMBomMatl.revisionID.IsEqual<AMBomItem.revisionID>>>
					.LeftJoin<AMBomMatlCury>
						.On<AMBomMatl.bOMID.IsEqual<AMBomMatlCury.bOMID>
							.And<AMBomMatl.revisionID.IsEqual<AMBomMatlCury.revisionID>>
							.And<AMBomMatl.operationID.IsEqual<AMBomMatlCury.operationID>>
							.And<AMBomMatl.lineID.IsEqual<AMBomMatlCury.lineID>>>
					.Where<AMBomMatl.bOMID.IsEqual<@P.AsString>
						.And<AMBomMatl.revisionID.IsEqual<@P.AsString>>
						.And<
							Brackets<AMBomMatlCury.curyID.IsNull>
								.Or<AMBomMatlCury.curyID.IsEqual<@P.AsString>>>>
					.View.Select(ProcessingGraph, bomRevision.Item1, bomRevision.Item2, ProcessingGraph.Accessinfo.BaseCuryID))
				{
					...
					bomMatl.SiteID = bomMatlCury?.SiteID;
					bomMatl.LocationID = bomMatlCury?.LocationID;
        ...
        {
			if (bomMatl?.MaterialType == null
				|| bomMatl.MaterialType == AMMaterialType.Phantom
				|| (bomMatl.MaterialType == AMMaterialType.Subcontract
					&& (bomMatl.SubcontractSource == AMSubcontractSource.DropShip
						|| bomMatl.SubcontractSource == AMSubcontractSource.VendorSupplied))
            ...
            var planMaterial = new AMPlanMaterial
            {
                ...
                UOM = bomMatl.UOM,
                SiteID = bomMatl.SiteID ?? Order.Current.SiteID,
