...
namespace PX.Objects.AM
{
    ...
    public class ProductionConfigurationCopy : ProductionBomCopy
    {
        ...
        protected virtual void CopyMatlInsertedRow(AMConfigurationOption option, AMConfigResultsOption resultOption)
        {
			...
			prodMatl.SubcontractSource = option.SubcontractSource;
        ...
        protected virtual void LoadConfigPhantoms(AMBomOper amBomOper, int currentIndex)
        {
			...
			if (ConfigResults?.Current?.ConfigResultsID < 0)
			{
				...
				.Where(r => r.ConfigResultsID == ConfigResults?.Current?.ConfigResultsID && r.Included == true))
				{
					...
					foreach (AMConfigurationOption option in PXSelect<AMConfigurationOption,
						...
						OrderBy<Asc<AMConfigurationOption.sortOrder, Asc<AMConfigurationOption.lineNbr>>>>.Select(ProcessingGraph, resultOption.ConfigurationID,
						resultOption.Revision, resultOption.FeatureLineNbr, resultOption.OptionLineNbr, amBomOper.OperationID))
					{
						BuildOperationByConfigPhantoms(resultOption, option, amBomOper, currentIndex);
					}
				}
				return;
			}
