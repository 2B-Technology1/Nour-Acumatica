...
namespace PX.Objects.AM
{
    ...
    public class GenerateForecastProcess : PXGraph<GenerateForecastProcess>
    {
        ...
        public PXSelectJoin<AMForecastStaging,
			InnerJoin<Branch, On<AMForecastStaging.branchID, Equal<Branch.branchID>>>,
			Where<AMForecastStaging.userID, Equal<Current<AccessInfo.userID>>,
				And<Branch.baseCuryID, Equal<Current<AccessInfo.baseCuryID>>>>> SelectedRecs;
        ...
        protected IEnumerable ProcessRecords(PXAdapter adapter)
        {
            ...
            PXLongOperation.StartOperation(this, () =>
            {
                ...
                {
                    if (SelectedRecs.Cache.GetStatus(forecastRec).IsDeleted() || !forecastRec.Selected.GetValueOrDefault())
                    {
                        continue;
                    }
        ...
        protected IEnumerable ProcessAllRecords(PXAdapter adapter)
        {
            ...
            PXLongOperation.StartOperation(this, () =>
            {
                foreach (AMForecastStaging forecastRec in SelectedRecs.Cache.Cached)
                {
					if (SelectedRecs.Cache.GetStatus(forecastRec).IsDeleted())
					{
						continue;
					}
        ...
        public void ProcessHistory()
        {
            ...
            try
            {
				using (new DisableFormulaCalculationScope(SelectedRecs.Cache, typeof(AMForecastStaging.changeUnits), typeof(AMForecastStaging.percentChange)))
				{
					DeleteSessionData();
        ...
        {
			var forecastStagingRecord = SetAMForecastStaging(bucket, forecastQty, lastYearQty, new AMForecastStaging());
			var isUpdate = false;
			var located = (AMForecastStaging)SelectedRecs.Cache.Locate(forecastStagingRecord);
			if (located != null)
			{
				forecastStagingRecord = SetAMForecastStaging(bucket, forecastQty, lastYearQty, PXCache<AMForecastStaging>.CreateCopy(located));
				var rowStatus = SelectedRecs.Cache.GetStatus(forecastStagingRecord);
				isUpdate = rowStatus == PXEntryStatus.Deleted || rowStatus == PXEntryStatus.Updated;
				if (rowStatus == PXEntryStatus.InsertedDeleted || rowStatus == PXEntryStatus.Deleted)
				{
					SelectedRecs.Cache.Remove(forecastStagingRecord);
				}
			}
			...
			if(isUpdate)
			{
				SelectedRecs.Update(forecastStagingRecord);
				return;
			}
		...
		protected virtual AMForecastStaging SetAMForecastStaging(Bucket bucket, decimal forecastQty, decimal lastYearQty, AMForecastStaging row)
		{
			if(row == null)
			{
				row = new AMForecastStaging();
			}

			row.UserID = Accessinfo.UserID;
			row.InventoryID = bucket.InventoryID;
			row.SubItemID = bucket.SubItemID;
			row.SiteID = bucket.SiteID;
			row.BeginDate = bucket.BeginDate;
			row.EndDate = bucket.EndDate;
			row.Seasonality = bucket.ReplenishmentPolicyID;
			row.UOM = bucket.BaseUOM;
			row.ForecastQty = forecastQty;
			row.LastYearBaseQty = lastYearQty;
			row.LastYearSalesQty = lastYearQty;
			row.Dependent = Settings?.Current?.Dependent;
			row.CustomerID = bucket.BAccountID ?? 0;
        ...
        protected virtual void ForecastSettings_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
        {
			...
			this.Settings.Cache.IsDirty = false;
        }
