...
namespace PX.Objects.AM
{
    ...
    public class EstimateCopy : PXGraph<EstimateCopy, AMEstimateItem>
    {
        ...
        protected virtual void CopyAllEstimateChildren(AMEstimateItem fromEstimateItem, AMEstimateItem toEstimateItem, bool resetNoteID)
        {
			...
			InsertListToCache(CopyAsNewStep(fromEstimateItem, toEstimateItem));
		}
		...
		public virtual List<AMEstimateStep> CopyAsNewStep(AMEstimateItem fromEstimateItem, AMEstimateItem toEstimateItem)
		{
			var list = new List<AMEstimateStep>();
			foreach (AMEstimateStep estimateStep in PXSelect<AMEstimateStep,
				Where<AMEstimateStep.estimateID, Equal<Required<AMEstimateStep.estimateID>>,
					...
					>.Select(this, fromEstimateItem.EstimateID, fromEstimateItem.RevisionID))
			{
				var newRow = PXCache<AMEstimateStep>.CreateCopy(estimateStep);
				newRow.NoteID = null;
				newRow.RevisionID = toEstimateItem.RevisionID;
				newRow.EstimateID = IsValidEstimateID(toEstimateItem) ? toEstimateItem.EstimateID : null;
				list.Add(newRow);
        ...
        protected virtual void AMEstimateOper_WorkCenterID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            ...
            if (amwc != null)
            {
                ...
                foreach (PXResult<AMWCMach, AMMach, AMMachCurySettings> result in PXSelectJoin<AMWCMach,
					InnerJoin<AMMach, On<AMWCMach.machID, Equal<AMMach.machID>>,
                    InnerJoin<AMMachCurySettings, On<AMWCMach.machID, Equal<AMMachCurySettings.machID>,
