...
namespace PX.Objects.AM
{
    public class ManufacturingDiagram : PXGraph<ManufacturingDiagram>
    {
        ...
        private void RefillProductionOrderResourceCache()
        {
            ...
            foreach (PXResult<AMSchdItem> record in
                SelectFrom<AMSchdItem>
			        ...
			        .InnerJoin<INSite>.On<AMSchdItem.FK.Site.And<INSite.baseCuryID.IsEqual<AccessInfo.baseCuryID.FromCurrent>>>					
                    ...
                    .View.Select(this))
            {
                ...
                var item = new ProductionOrderResource
                {
					...
					ReadOnly = (isReadOnly || prodItem.Released == true)
                };
	        ...
	        var cnt = 1;
         	foreach (PXResult<AMSchdOper> record in
		        SelectFrom<AMSchdOper>
					...
					.InnerJoin<Branch>.On<AMProdItem.branchID.IsEqual<Branch.branchID>.And<Branch.baseCuryID.IsEqual<AccessInfo.baseCuryID.FromCurrent>>>
			        .Where<
        ...
        private bool? CalculateLackOfMaterialsSign(AMProdOper prodOrder, AMProdItem amProdItem)
        {
            ...
            foreach (PXResult<AMProdMatl> record in
                SelectFrom<AMProdMatl>
                    .InnerJoin<INSiteStatusByCostCenter>.On<AMProdMatl.inventoryID.IsEqual<INSiteStatusByCostCenter.inventoryID>
                    ...
                    .View.Select(this, prodOrder.OrderType, prodOrder.ProdOrdID, prodOrder.OperationID))
            {
                ...
                var inSiteStatus = record.GetItem<INSiteStatusByCostCenter>();
        ...
        public virtual Int32? InventoryID { get; set; }
            ...
            #region Warehouse
            [AMSite(DisplayName = "Warehouse", DescriptionField = typeof(INSite.descr))]
			...
			[PXRestrictor(typeof(Where<AMProdItem.completed, Equal<False>, And<AMProdItem.canceled, Equal<False>>>),
