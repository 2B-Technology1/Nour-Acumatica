    ...
    public class CTPProcess : PXGraph<CTPProcess>
    {
        ...
        protected virtual void RejectCTPProcess(SOOrderEntry orderEntry, ProdMaint prodGraph, CTPLine ctpLine)
        {
            ...
            var prodItem = SelectFrom<AMProdItem>.Where<AMProdItem.orderType.IsEqual<@P.AsString>.
                And<AMProdItem.prodOrdID.IsEqual<@P.AsString>>>.View.Select(prodGraph, ctpLine.AMCTPOrderType, ctpLine.ProdOrdID);
            if (prodItem != null)
            {
        ...
        protected virtual AMProdItem CopyCTPtoRegularProdOrder(ProdMaint prodGraph, CTPLine ctpLine, CTPFilter filter)
        {
            ...
            if (ctpOrder != null)
            {
                ...
                prodGraph.ProdMaintRecords.Current = prodGraph.ProdMaintRecords.Update(ctpOrder);
		...
		/// <summary>
		/// Processing filter for the Process Capable to Promise (AM515000) form (corresponding to the <see cref="CTPProcess"/> graph).
		/// </summary>
		...
		/// <summary>
		/// Projection of the <see cref="SOLine"/> class used as the processing data for the Process Capable to Promise (AM515000) form (corresponding to the <see cref="CTPProcess"/> graph).
		/// </summary>
		...
		/// <summary>
		/// The filter for the "Quantity Available" pop-up panel that is opened when a user clicks the hyperlink in the OpenQty field on the Critical Materials (AM401000) form (corresponding to the <see cref="CriticalMaterialsInq"/> graph).
		/// </summary>
		[Serializable]
