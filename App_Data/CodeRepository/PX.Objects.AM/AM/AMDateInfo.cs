...
namespace PX.Objects.AM
{
    ...
    public struct AMDateInfo
    {
		...
		public static int GetDateSeconds(DateTime startDate, DateTime endDate)
		{
			int seconds = Convert.ToInt32(Math.Ceiling((endDate - startDate).TotalSeconds));

			if (seconds < 0 && seconds >= MINUTES_PER_DAY_INT * 60 * -1
				&& DateTime.Compare(endDate, DateTime.MaxValue.AddDays(-1)) <= 0)
			{
				return Convert.ToInt32(Math.Ceiling((endDate.AddDays(1) - startDate).TotalSeconds));
			}
        ...
        {
			return RemoveSeconds(Common.Dates.BeginOfTimeDate + dateTime.TimeOfDay);
        }
