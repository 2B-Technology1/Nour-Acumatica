...
namespace PX.Objects.AM.GraphExtensions
{
        ...
        where TPrimary : class, IBqlTable, new()
    {
		...
		protected virtual void _(Events.FieldUpdated<OpportunityDocument,OpportunityDocument.bAccountID> e)
		{
			if (e?.Row == null || e.NewValue == e.OldValue) return;
			foreach (AMEstimateReference estimateReference in OpportunityEstimateRecords.Select())
			{
				if (estimateReference.BAccountID == e.Row.BAccountID) continue;

				estimateReference.BAccountID = e.Row.BAccountID;
				OpportunityEstimateRecords.Update(estimateReference);
			}
		}
        ...
        public virtual void CopyConfigurationsToNewQuote(Guid? quoteId, QuoteMaint graph)
        {
            ...
            foreach (CROpportunityProducts product in graph.Products.Select())
            {
				...
				AMConfiguration amConfigResult = PXSelect<
					AMConfiguration,
						Where<AMConfiguration.inventoryID, Equal<Required<AMConfigurationResults.inventoryID>>,
					...
					OrderBy<
						Desc<AMConfiguration.createdDateTime>>>
					.SelectSingleBound(graph, null, fromConfigResult.InventoryID, productExt.AMConfigurationID);

				if (amConfigResult == null)
				{
					SetActiveConfigurationID(graph, fromConfigResult, productExt);
				}
		...
		private void SetActiveConfigurationID(QuoteMaint graph, AMConfigurationResults fromConfigResult, CROpportunityProductsExt productExt)
		{
			if (fromConfigResult.InventoryID != null)
			{
				if (fromConfigResult.SiteID != null)
				{
					INItemSite itemSite = PXSelect<
						INItemSite,
						Where<INItemSite.inventoryID, Equal<Required<INItemSite.inventoryID>>,
							And<INItemSite.siteID, Equal<Required<INItemSite.siteID>>>>>
						.SelectSingleBound(graph, null, fromConfigResult.InventoryID, fromConfigResult.SiteID);
					if (itemSite != null)
					{
						INItemSiteExt itemSiteExt = PXCache<INItemSite>.GetExtension<INItemSiteExt>(itemSite);
						if (itemSiteExt?.AMConfigurationID != null)
						{
							productExt.AMConfigurationID = itemSiteExt.AMConfigurationID;
							return;
						}
					}
				}
				...
				InventoryItem inventoryItem = PXSelect<
					...
					.SelectSingleBound(graph, null, fromConfigResult.InventoryID);
				if (inventoryItem != null)
				{
					InventoryItemExt itemExt = PXCache<InventoryItem>.GetExtension<InventoryItemExt>(inventoryItem);
					if (itemExt?.AMConfigurationID != null)
					{
						productExt.AMConfigurationID = itemExt.AMConfigurationID;
						return;
					}
				}
			}
            ...
            AMConfigurationResults fromConfigResult, QuoteMaint graph)
        {
            ...
            if (ItemConfiguration.IsDocumentValid(out var errorMessage))
            {
				...
				graph.Caches[typeof(AMConfigurationResults)].Update(locatedConfig);
