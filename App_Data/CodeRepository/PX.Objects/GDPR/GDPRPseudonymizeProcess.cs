	...
	public class GDPRPseudonymizeProcessBase: GDPRPersonalDataProcessBase
	{
		...
		protected virtual void WipeAudit(PXGraph processingGraph, Type childTable, IEnumerable<PXPersonalDataFieldAttribute> fields, IEnumerable<object> childs)
		{
			foreach (object child in childs)
			{
				...
				PXDatabase.Delete<VisitedRecord>(new PXDataFieldRestrict<VisitedRecord.refNoteID>(processingGraph.Caches[childTable].GetValue(child, nameof(INotable.NoteID)) as Guid?));
			}
		}
