...
namespace PX.Objects.Common.Discount
{
	...
	{
		protected virtual bool IsDiscountFeatureEnabled(PXGraph graph, DiscountCalculationOptions calculationOptions = DiscountCalculationOptions.CalculateAll)
		{
			...
			if (calculationOptions.HasFlag(DiscountCalculationOptions.DisableAPDiscountsCalculation))
			{
				return FeatureInstalled<FeaturesSet.customerDiscounts>(graph);
			}
			else if (calculationOptions.HasFlag(DiscountCalculationOptions.DisableARDiscountsCalculation))
			{
				return FeatureInstalled<FeaturesSet.vendorDiscounts>(graph);
			}
			return true;
		}

		protected bool IsDiscountCalculationEnabled(PXGraph graph, DiscountCalculationOptions options)
		{
			return (!graph.IsImport || options.HasFlag(DiscountCalculationOptions.CalculateDiscountsFromImport));
		}
		...
		{
			if (branchID == null || locationID == null || date == null || !IsDiscountFeatureEnabled(cache.Graph, discountCalculationOptions))
				return;
			...
			if ((dLine.AutomaticDiscountsDisabled != true || discountCalculationOptions.HasFlag(DiscountCalculationOptions.ExplicitlyAllowToCalculateAutomaticLineDiscounts)) &&
				...
				(IsDiscountCalculationEnabled(cache.Graph, discountCalculationOptions) || cache.Graph.IsMobile))
			{
				bool skipLineDiscounts = cache.Fields.Contains(nameof(dLine.SkipLineDiscounts))
					? dLine.SkipLineDiscounts == true
					: GetUnitPrice(cache, line, locationID, curyID, date.Value).skipLineDiscount;

				if (!skipLineDiscounts)
				{
		...
		protected override void SetLineDiscountOnlyImpl(
			...
			bool needDiscountID)
		{
			if (locationID == null || date == null || !IsDiscountFeatureEnabled(cache.Graph))
				return;
			...
			HashSet<DiscountSequenceKey> applicableDiscountSequences = SelectApplicableEntityDiscounts(cache.Graph, entities, DiscountType.Line, !needDiscountID);
			...
			bool needDiscountID)
		{
			...
			HashSet<DiscountSequenceKey> applicableDiscountSequences = SelectApplicableEntityDiscounts(cache.Graph, entities, DiscountType.Line, !needDiscountID, true);
			...
			DiscountCalculationOptions discountCalculationOptions)
		{
			try
			{
				...
				if (recalcFilter.RecalcTarget == RecalcDiscountsParamFilter.AllLines && (recalcFilter.RecalcDiscounts == true || recalcFilter.RecalcUnitPrices == true))
				{
				...
				else if (currentLine != null && (recalcFilter.RecalcDiscounts == true || recalcFilter.RecalcUnitPrices == true))
				{
			...
			DiscountCalculationOptions discountCalculationOptions)
		{
			...
			var recalcFilter = new RecalcDiscountsParamFilter
			{
				...
				RecalcDiscounts = IsDiscountFeatureEnabled(cache.Graph, discountCalculationOptions),
		...
		protected virtual void RecalculatePricesAndDiscountsOnLine(PXCache cache, TLine line, RecalcDiscountsParamFilter recalcFilter, DiscountCalculationOptions discountCalculationOptions)
		{
			...
			cache.MarkUpdated(line, assertError: true);
		}
		}
			...
			RecalcDiscountsParamFilter recalcFilter = null)
		{
			...
			if (branchID == null || locationID == null || date == null || dLine.AutomaticDiscountsDisabled == true || !IsDiscountFeatureEnabled(cache.Graph, discountCalculationOptions))
				return;
		...
		protected virtual void SetManualLineDiscount(PXCache cache, HashSet<KeyValuePair<object, string>> entities, TLine line, DateTime date)
		{
			...
			if (extPrice != null && qty != null)
			{
				...
				HashSet<DiscountSequenceKey> discountSequencesByDiscountID = SelectDiscountSequences(discountID);
				HashSet<DiscountSequenceKey> applicableDiscountSequences = SelectApplicableEntityDiscounts(cache.Graph, entities, DiscountType.Line, false);
		...
		protected virtual void ApplyDiscountToLine(PXCache sender, TLine line, decimal? qty, decimal? curyUnitPrice, decimal? curyExtPrice, DiscountLineFields dLine, DiscountResult discountResult, int multInv)
		{
			...
			else if (!sender.Graph.IsImport && !sender.Graph.IsImportFromExcel)
			{
			...
			RecalcDiscountsParamFilter recalcFilter = null)
		{
			...
			if (documentDetails.Count != 0)
			{
				...
				if (discountCalculationOptions.HasFlag(DiscountCalculationOptions.DisableAllAutomaticDiscounts))
				{
					...
					{
						List<TDiscountDetail> trace = GetDiscountDetailsByType(cache, discountDetails, DiscountType.Group).Where(x => x.IsOrigDocDiscount != true).ToList();
			...
			string discountType)
		{
			...
			foreach (TLine line in documentDetails)
			{
				...
				ushort[] discountsAppliedToLine = discountedLine.DiscountsAppliedToLine;

				foreach (TDiscountDetail discountLine in discountLines)
				{
					if (discountLine.LineNbr != null && discountsAppliedToLine != null && discountsAppliedToLine.Contains(discountLine.LineNbr ?? 0))
					{
		...
		protected HashSet<DiscountSequenceKey> FilterManualDiscountDetails(ConcurrentDictionary<string, DiscountCode> cachedDiscountTypes, IList<TDiscountDetail> typedDiscountDetails)
		{
			var result = new HashSet<DiscountSequenceKey>();

			foreach (TDiscountDetail discountDetail in typedDiscountDetails)
			{
				var discountSequenceKey = new DiscountSequenceKey(discountDetail.DiscountID, discountDetail.DiscountSequenceID);

				if (cachedDiscountTypes[discountDetail.DiscountID].IsManual && discountDetail.IsOrigDocDiscount == false)
				{
					result.Add(discountSequenceKey);
				}
			}
			...
			bool skipManual = false)
		{
			...
			var manualDiscountDetails = FilterManualDiscountDetails(cachedDiscountTypes, groupDiscountDetails);
			...
			foreach (TLine line in documentDetails)
			{
				...
				HashSet<DiscountSequenceKey> applicableGroupDiscounts = SelectApplicableEntityDiscounts(cache.Graph, GetDiscountEntitiesDiscounts(cache, line, locationID, true), DiscountType.Group, skipManual);

				applicableGroupDiscounts = RemoveManualDiscounts(cache, applicableGroupDiscounts, manualDiscountDetails);
			...
			bool ignoreExistingDocumentDiscounts)
		{
			...
			foreach (TLine line in documentDetails)
			{
				...
				ushort[] discountsAppliedToLine = discountFields.DiscountsAppliedToLine;
				...
				foreach (TDiscountDetail discount in newDiscountDetails)
				{
					...
					if ((discount.IsOrigDocDiscount == true || (discount.Type == DiscountType.Group && !ignoreExistingGroupDiscounts) || (discount.Type == DiscountType.Document && !ignoreExistingDocumentDiscounts)) 
						&& discountsAppliedToLine != null && discountsAppliedToLine.Contains(discount.LineNbr ?? 0))
					{
		...
		public virtual TwoWayLookup<TDiscountDetail, TLine> GetListOfLinksBetweenDiscountsAndDocumentLines(PXCache cache, List<TLine> documentDetails, List<TDiscountDetail> discountDetails)
		{
			...
			foreach (TLine line in documentDetails)
			{
				...
				{
					if ((discountsAppliedToLine != null && discountsAppliedToLine.Contains(dDetail.LineNbr ?? 0)) ||
		...
		public virtual TwoWayLookup<TDiscountDetail, TLine> GetListOfDiscountsAppliedToLine(PXCache cache, TLine line, List<TDiscountDetail> discountDetails)
		{
			...
			if (discountFields.AutomaticDiscountsDisabled != true && discountsAppliedToLine != null)
			{
				foreach (TDiscountDetail dDetail in discountDetails)
				{
					if (discountsAppliedToLine.Contains(dDetail.LineNbr ?? 0) ||
						(dDetail.IsOrigDocDiscount != true && dDetail.Type.IsIn(DiscountType.Document, DiscountType.ExternalDocument)))
					{
						discountCodesWithApplicableLines.Link(dDetail, line);
					}
		...
		{
			CalculateGroupDiscountRate(cache, lines, currentLine, discountCodesWithApplicableLines, false, recalculateTaxes, documentLinesSelectParams, recalcAll, forceFormulaCalculation);
		}
		...
		public virtual void CalculateGroupDiscountRate(
			...
			bool forceFormulaCalculation = false)
		{
			List<TLine> documentDetails = GetDocumentDetails(cache, lines, useLegacyDetailsSelectMethod, documentLinesSelectParams);
			...
			bool forceFormulaCalculation = false)
		{
			...
			foreach (TLine line in documentDetails)
			{
				...
				UpdateGroupDiscountRate(lineAmt, CalculateGroupDiscountRateOnSpecificLine(cache, line, discountCodesWithApplicableLines, false, recalcAll));
		...
		{
			var oldDiscountRate = aLine.GroupDiscountRate;
			if (aLine.GroupDiscountRate == discountRate)
				return false;

			aLine.GroupDiscountRate = discountRate;
			aLine.RaiseFieldUpdated<AmountLineFields.groupDiscountRate>(oldDiscountRate);
			return true;
		}
		...
		private void ApplyDocumentDiscountRatesToLine(PXCache cache, TwoWayLookup<TDiscountDetail, TLine> discountsLinkedToLine, List<TLine> documentDetails, TLine currentLine, bool forceFormulaCalculation = false)
		{
			...
			foreach (TLine line in documentDetails)
			{
				...
				UpdateDocumentDiscountRate(lineAmtFields, 1 - RoundDiscountRate(totalDocumentDiscountRate));
		...
		{
			var oldDiscountRate = aLine.DocumentDiscountRate;
			if (aLine.DocumentDiscountRate == discountRate)
				return false;

			aLine.DocumentDiscountRate = discountRate;
			aLine.RaiseFieldUpdated<AmountLineFields.documentDiscountRate>(oldDiscountRate);
			return true;
		}
				...
				if (!skipAutomaticDocumentDiscounts && !discountCalculationOptions.HasFlag(DiscountCalculationOptions.CalculateExternalDocumentDiscountsOnly))
				{
					...
					if (!recalculateAllFromAction)
					{
						...
						foreach (TDiscountDetail manualDiscount in manualDiscounts)
						{
							...
							HashSet<DiscountSequenceKey> applicableDocumentDiscounts = SelectApplicableEntityDiscounts(cache.Graph, entities, DiscountType.Document, false);
					...
					{
						List<TDiscountDetail> trace = GetDiscountDetailsByType(cache, discountDetails, DiscountType.Document).Where(x => x.IsOrigDocDiscount != true).ToList();
			...
			DiscountCalculationOptions discountCalculationOptions)
		{
			...
			if (cachedDiscountTypes.ContainsKey(manualDiscountID) && documentDetails.Count != 0)
			{
				...
				if (cachedDiscountTypes[manualDiscountID].Type == DiscountType.Group)
				{
					...
					foreach (TLine line in documentDetails)
					{
						HashSet<DiscountSequenceKey> applicableGroupDiscounts = SelectApplicableEntityDiscounts(cache.Graph, GetDiscountEntitiesDiscounts(cache, line, locationID, true), cachedDiscountTypes[manualDiscountID].Type, false);
				...
				{
					HashSet<DiscountSequenceKey> applicableDiscounts = SelectApplicableEntityDiscounts(cache.Graph, GetDiscountEntitiesDiscounts(cache, documentDetails.First(), locationID, false, branchID), cachedDiscountTypes[manualDiscountID].Type, false);
		...
		public virtual void SetTotalDocDiscount(
			...
			DiscountCalculationOptions discountCalculationOptions)
		{
			if (curyDiscountTotal == null || IsDiscountFeatureEnabled(cache.Graph, discountCalculationOptions))
				return;
			...
			DiscountCalculationOptions discountCalculationOptions)
		{
			...
			if ((cache.Graph.IsCopyPasteContext || currentDiscountDetailLine.CuryDiscountAmt != oldDiscountDetailLine.CuryDiscountAmt) && 
				...
				(oldDiscountDetailLine.CuryDiscountAmt != null || !currentDiscountDetailLine.CuryDiscountAmt.IsNullOrZero()))
		...
		protected virtual UnitPriceVal GetUnitPrice(PXCache cache, TLine line, int? locationID, string curyID, DateTime date)
		{
			...
			if (eFields.CustomerID != null && eFields.InventoryID != null)
			{
				...
				unitPriceVal.skipLineDiscount = salesPrice.SkipLineDiscounts;
			}
			return unitPriceVal;
		}
		...
		protected virtual HashSet<DiscountSequenceKey> RemoveManualDiscounts(PXCache cache, PXSelectBase<TDiscountDetail> discountDetails, HashSet<DiscountSequenceKey> allApplicableDiscounts, string type)
		{
			...
			List<TDiscountDetail> typedDiscountDetails = GetDiscountDetailsByType(cache, discountDetails, type);
			return RemoveManualDiscounts(cache, allApplicableDiscounts, typedDiscountDetails);
		}

		/// <summary>
		/// Removes manual discounts from allApplicableDiscounts. Manual discounts that already applied to the document will be retained.
		/// </summary>
		protected virtual HashSet<DiscountSequenceKey> RemoveManualDiscounts(PXCache cache,
			...
			HashSet<DiscountSequenceKey> manualDiscountDetails)
		{
			...
			foreach (DiscountSequenceKey discountSequence in allApplicableDiscounts)
			{
				if (cachedDiscountTypes[discountSequence.DiscountID].IsManual)
				{
					if (manualDiscountDetails.Contains(discountSequence))
						applicableDiscounts.Add(discountSequence);
				}
				else
				{
		...
		{
			if (FeatureInstalled<FeaturesSet.matrixItem>(cache.Graph))
			{
		...
		public class UnsortedView : PXView
		{
			public UnsortedView(PXGraph graph, bool IsReadOnly, BqlCommand select)
				: base(graph, IsReadOnly, select)
			{
			}
			protected override void SortResult(List<object> list, PXSearchColumn[] sorts, bool reverseOrder)
			{
			}
		}
		...
		{
			return GetDocumentDetails(cache, documentDetails, false, parameters);
		}

		//Returns list of Document Details lines
		private List<TLine> GetDocumentDetails(PXCache cache, PXSelectBase<TLine> documentDetails, bool useLegacyDetailsSelectMethod, object[] parameters = null)
		{
			Func<TLine, bool> where = (detail) =>
			{
			...
			else
			{
				...
				var viewkey = this.GetType();
				PXView detailview;
				if (!graph.TypedViews.TryGetValue(viewkey, out detailview))
			{
					detailview = graph.TypedViews[viewkey] = new UnsortedView(cache.Graph, false, documentDetails.View.BqlSelect);
			}

			int startRow = 0;
			int totalRows = 0;
				return detailview.Select(null, parameters, null, null, null, null, ref startRow, 0, ref totalRows)
				.RowCast<TLine>()
					.Where(where)
				.ToList();
		}
