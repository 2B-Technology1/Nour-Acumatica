	...
	public abstract class DiscountEngine : PXGraph<DiscountEngine>
	{
			...
			DateTime date)
		{
			...
			return entities != null
				? SelectApplicableDiscounts(cache, dLine, SelectApplicableEntityDiscounts(cache.Graph, entities, discountType, true), curyAmount, quantity, discountType, date).FirstOrDefault()
			...
			bool appliedToDR = false)
		{
			...
			bool customerDiscountsEnabled = FeatureInstalled<FeaturesSet.customerDiscounts>(graph);
			bool vendorDiscountsEnabled = FeatureInstalled<FeaturesSet.vendorDiscounts>(graph);
			...
			foreach (KeyValuePair<object, string> entity in entities)
			{
				...
				if (isARDiscount && customerDiscountsEnabled)
				{
					applicableEntityDiscounts = GetApplicableEntityARDiscounts(entity);
				}
				else if (vendorDiscountsEnabled)
				{
		...
		[PXHidden]
		public class DiscountFeatures: IBqlTable
		{
		...
		protected static bool FeatureInstalled<FeatureType>(PXGraph graph)
			where FeatureType : IBqlField
		{
			PXCache cache = graph.Caches[typeof(DiscountFeatures)];
			if (cache.Current == null)
			{
				cache.Current = cache.CreateInstance();
			}

			bool? value;
			if ((value = (bool?)cache.GetValue<FeatureType>(cache.Current)) == null)
			{
				value = PXAccess.FeatureInstalled<FeatureType>();
				cache.SetValue<FeatureType>(cache.Current, value);
			}
			return value.Value;
		}
		...
		public static void UpdateEntityCache() => PXContext.SetSlot(PXDatabase.GetSlot<DECache>(typeof(DiscountSequence).Name, typeof(DiscountSequence)));
		...
		protected class DECache : IPrefetchable
		{
			...
			{
				var cachedDiscountEntities = GetEntityDiscountsSlot<TTable, TEntityType>();
		...
		public static void PutEntityDiscountsToSlot<TTable, TKeyType>(ICollection<KeyValuePair<TKeyType, HashSet<DiscountSequenceKey>>> items)
			where TTable : IBqlTable
		{
			var cachedDiscountEntities = GetEntityDiscountsSlot<TTable, TKeyType>();
			foreach (var pair in items)
				cachedDiscountEntities[pair.Key] = pair.Value.ToImmutableHashSet();
		}

		private static TSlot GetSlot<TSlot>(string key)
			where TSlot : class, new()
		{
			var cache = PXContext.GetSlot<TSlot>(key);
			if (cache == null)
				PXContext.SetSlot(key, cache = PXDatabase.GetSlot<TSlot>(key, typeof(DummyTable)));
			return cache;
		}

		protected static ConcurrentDictionary<TKeyType, ImmutableHashSet<DiscountSequenceKey>> GetEntityDiscountsSlot<TTable, TKeyType>()
			where TTable : IBqlTable
			=> GetSlot<ConcurrentDictionary<TKeyType, ImmutableHashSet<DiscountSequenceKey>>>(typeof(TTable).Name);

		protected static ConcurrentDictionary<int, string> GetInventoryPriceClassesSlot()
			=> GetSlot<ConcurrentDictionary<int, string>>(InventoryPriceClassesSlotName);

		protected static ConcurrentDictionary<Tuple<int, int>, string> GetCustomerPriceClassesSlot()
			=> GetSlot<ConcurrentDictionary<Tuple<int, int>, string>>(CustomerPriceClassIDSlotName);
		...
		private static ConcurrentDictionary<Object, ImmutableHashSet<DiscountSequenceKey>> CachedUnconditionalDiscounts
			=> GetSlot<ConcurrentDictionary<object, ImmutableHashSet<DiscountSequenceKey>>>(AR.Messages.Unconditional);
		...
		public static void RemoveFromCachedInventoryPriceClasses(int? inventoryID)
		{
			...
			var cachedInventoryPriceClasses = GetInventoryPriceClassesSlot();
		...
		public static void RemoveFromCachedCustomerPriceClasses(int? bAccountID)
		{
			...
			var cachedCustomerPriceClasses = GetCustomerPriceClassesSlot();
			...
			bool single)
		{
			...
			PXView bestDiscountView = new PXView(PXGraph.CreateInstance<PXGraph>(), false, command);
			List<PXResult<DiscountSequence, DiscountSequenceDetail>> applicableDiscountDetails = bestDiscountView.SelectMulti(requiredParams.ToArray()).Cast<PXResult<DiscountSequence, DiscountSequenceDetail>>().ToList();

			List<PXResult<DiscountSequence, DiscountSequenceDetail>> result = new List<PXResult<DiscountSequence, DiscountSequenceDetail>>();
			if (applicableDiscountDetails.Count() > 0)
			{
				List<PXResult<DiscountSequence, DiscountSequenceDetail>> filteredDiscountDetails = applicableDiscountDetails.
				//group selected discount details by IsLast value
				//group with IsLast != true (current discounts) have priority over the group with IsLast = true (outdated discounts)
				GroupBy
				(_ => ((DiscountSequenceDetail)_).IsLast,
					(keyIsLast, groupIsLast) => groupIsLast.OrderBy(_ => keyIsLast).
					//group ordered discount details in each groupIsLast group by DiscountID and DiscountSequenceID
					//to be able to select the best discount detail for each discount later
					GroupBy
					(_ => new
					{
						((DiscountSequenceDetail)_).DiscountID,
						((DiscountSequenceDetail)_).DiscountSequenceID
					},
						//sort discount details in each (DiscountID, DiscountSequenceID) group by LastDate
						//discount details that have LastDate closest to the current date should go first in the group:
						//newest discounts should go first if the group has IsLast != true
						//oldest discounts should go first if the group has IsLast = true
						(keyDiscountID, groupDiscountID) => (((DiscountSequenceDetail)groupDiscountID.FirstOrDefault())?.IsLast != true ?
							groupDiscountID.OrderByDescending(_ => ((DiscountSequenceDetail)_).LastDate) :
							groupDiscountID.OrderBy(_ => ((DiscountSequenceDetail)_).LastDate)).
							//we only need one best matching discount detail from each groupDiscountID group
							FirstOrDefault()
					//order selected discount details by Discount field to be able select the one that gives the best discount
					).OrderByDescending(_ => ((DiscountSequenceDetail)_).Discount)
				//flatten the result. Top one record is the best discount (it will be selected when best Line or Document discount is requested), all other - applicable and can be used for group discount selection
				//outdated discounts that have 0 or null discount are filtered out
				).SelectMany(_ => _.ToList().Where(_ => ((DiscountSequenceDetail)_).IsLast != true || (((DiscountSequenceDetail)_).Discount != null && ((DiscountSequenceDetail)_).Discount != 0m))).ToList();

				if (single)
				{
					result.Add(filteredDiscountDetails.FirstOrDefault());
				}
				else
				{
					result.AddRange(filteredDiscountDetails);
				}
			}
		...
		public virtual (decimal groupDiscountTotal, decimal documentDiscountTotal ,decimal discountTotal) GetDiscountTotals<TDiscountDetail>(PXSelectBase<TDiscountDetail> discountDetails)
			where TDiscountDetail : class, IBqlTable, IDiscountDetail, new()
		{
			(decimal groupDiscountTotal, decimal documentDiscountTotal, decimal discountTotal) discountTotals = (0m, 0m, 0m);
			foreach (TDiscountDetail discount in discountDetails.Select())
			{
				if (discount.SkipDiscount != true)
				{
					switch (discount.Type)
						{
						case DiscountType.Group:
							discountTotals.groupDiscountTotal += discount.CuryDiscountAmt ?? 0m;
							break;
						case DiscountType.Document:
						case DiscountType.ExternalDocument:
							discountTotals.documentDiscountTotal += discount.CuryDiscountAmt ?? 0m;
							break;
					}
					discountTotals.discountTotal += discount.CuryDiscountAmt ?? 0m;
				}
			}
		...
		internal static ApplyQuantityDiscountByBaseUOMOption ApplyQuantityDiscountByBaseUOM(PXGraph graph)
		{
				...
				FeatureInstalled<FeaturesSet.vendorDiscounts>(graph) &&
				CurrentConfiguration.ActualAPSetup.ApplyQuantityDiscountBy ==
					ApplyQuantityDiscountType.BaseUOM,

				FeatureInstalled<FeaturesSet.customerDiscounts>(graph) &&
