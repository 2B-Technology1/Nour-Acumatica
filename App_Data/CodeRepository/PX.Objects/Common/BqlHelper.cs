...
namespace PX.Objects.Common
{
	public static class BqlHelper
	{
		...
		{
			var sortFields = new Stack<IBqlSortColumn>(sortingFields);

			Type orderByClause = null;
			while (sortFields.Any())
			{
				var sorting = sortFields.Pop();
				if (orderByClause == null)
					orderByClause = sorting.GetType();
				else if (sorting.IsDescending)
					orderByClause = BqlCommand.Compose(typeof(Desc<,>), sorting.GetReferencedType(), orderByClause);
				else
					orderByClause = BqlCommand.Compose(typeof(Asc<,>), sorting.GetReferencedType(), orderByClause);
			}
			orderByClause = BqlCommand.Compose(typeof(OrderBy<>), orderByClause);
