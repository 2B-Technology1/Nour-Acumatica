...
namespace PX.Objects.Common.Utility
{
	public class PXResultMapper
	{
		...
		public PXResultMapper(PXView view, Dictionary<Type, Type> fieldMap, params Type[] resultDefinition)
					...
					if (bqlField != null)
					{
						...
						else
						{
							if (fieldMap.TryGetValue(bqlField, out var result))
							{
								...
								{
									map.DataField = GetMapFieldName(result);
								}
							}
						}
					}
		...
		{
			if (view != null && view.CacheType() == result.DeclaringType)
			{
				return result.Name;
			}
			else
