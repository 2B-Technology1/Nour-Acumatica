...
namespace PX.Objects.Common
{
		...
		where TSubledgerClosedFlagField : IBqlField
	{
		...
		protected virtual IPXResultset GetResultset(BqlCommand command, int? organizationID, string fromPeriodID, string toPeriodID) =>
			GetResultset(command, organizationID, fromPeriodID, toPeriodID, null);

		protected IPXResultset GetResultset(BqlCommand command, int? organizationID, string fromPeriodID, string toPeriodID, string[] sortAsImplicitColumns) =>
			new PXReportResultset(new PXView(this, true, command).SelectMultiBoundSortAsImplicit(new object[]
			{
