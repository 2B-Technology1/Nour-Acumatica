...
namespace PX.Objects.Common.Extensions
{
	...
	public static class PXGraphClassExtensions
	{
		...
		public static TTable LiteUpdate<TTable>(this PXGraph graph, TTable row, Action<PXCache, TTable> update, bool skipRowUpdatedEvent = false)
			where TTable : class, IBqlTable, new()
		{
			return graph.Caches<TTable>().LiteUpdate(row, update, skipRowUpdatedEvent);
		}

		public static TTable LiteUpdate<TTable>(this PXGraph graph, TTable row, Action<ValueSetter<TTable>> update, bool skipRowUpdatedEvent = false)
			where TTable : class, IBqlTable, new()
		{
			return graph.Caches<TTable>().LiteUpdate(row, update, skipRowUpdatedEvent);
		}

		public static TTable LiteUpdate<TTable>(this PXGraph graph, TTable row, Action<TTable> update, bool skipRowUpdatedEvent = false)
			where TTable : class, IBqlTable, new()
		{
			return graph.Caches<TTable>().LiteUpdate(row, update, skipRowUpdatedEvent);
		}

		public static bool TryGetScreenIdFor<T>(this PXGraph graph, out string screenID)
			where T : PXGraph
		{
			var node = PXSiteMap.Provider.FindSiteMapNode(typeof(T));
