...
namespace PX.Objects.Common.Attributes
{
	public class CopyChildLinkAttribute : PXUnboundFormulaAttribute
	{
		...
		protected virtual void OnRowDeleted(PXCache sender, object row)
		{
			...
			//in some cases we need to check another parent row which can be different type from _parentType
			if (IsParentDeleted(sender, row))
				return;
		...
		{
			var parentRow = PXParentAttribute.SelectParent(childCache, childRow, _parentType);
