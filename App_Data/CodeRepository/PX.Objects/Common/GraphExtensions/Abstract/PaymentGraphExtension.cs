...
namespace PX.Objects.Common.GraphExtensions.Abstract
{
        ...
        where TTran : class, IBqlTable, CM.IDocumentTran, new()
    {
        ...
        public virtual void CalcBalances<T>(TAdjust adj, T voucher, bool isCalcRGOL, bool DiscOnDiscDate, TTran tran)
		...
		protected virtual void _(Events.RowSelecting<TAdjust> e)
		{
			...
			{
				bool PPDwasReset = false;
				if (e.Row.AdjdHasPPDTaxes == true)
				{
					PPDwasReset = true;
					e.Row.AdjdHasPPDTaxes = false;
				}
				try
				{
					CalcBalancesFromAdjustedDocument(e.Row, false, DiscOnDiscDate);
