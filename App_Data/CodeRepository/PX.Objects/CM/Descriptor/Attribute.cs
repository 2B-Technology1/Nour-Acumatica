...
namespace PX.Objects.CM
{
	public static class CurrencyCollection
	{
			...
			public void Prefetch()
			{
				...
				this.Currency = PXDatabase
					.SelectMulti<Currency>(
						...
						new PXAliasedDataField<Currency.translationGainAcctID>(),
						new PXAliasedDataField<Currency.translationGainSubID>(),
						new PXAliasedDataField<Currency.translationLossAcctID>(),
						new PXAliasedDataField<Currency.translationLossSubID>()
						)
		...
		public static Currency GetCurrency(PXCache sender, Type sourceCuryID, object row)
		{
			...
			return GetCurrency(cache.GetValue(source, sourceCuryID.Name) as string);
		}
		
		public static long? MatchBaseCuryInfoId(CurrencyInfo info)
		{
			Currency cury = GetCurrency(info.BaseCuryID);
		...
		private static Definition slot
		{
			get
			{
				Definition result = PXContext.GetSlot<Definition>();
				if (result != null) return result;
				result = PXDatabase.GetSlot<Definition>(nameof(CurrencyCollection),
				new Type[] { typeof(Currency), typeof(CurrencyList), typeof(Company), typeof(Branch) });
				PXContext.SetSlot<Definition>(result);
				return result;
			}
		}
	}
	...
	public class CurrencyInfoAttribute : PXAggregateAttribute, IPXRowInsertingSubscriber, IPXRowPersistingSubscriber, IPXRowPersistedSubscriber, IPXRowUpdatingSubscriber, IPXReportRequiredField, IPXDependsOnFields
	{
		...
		protected class CurrencyInfoDefaultAttribute : CurrencyInfoDBDefaultAttribute
		{
			...
			{
				valueBeforePersisting = (long?)sender.GetValue(e.Row, _FieldOrdinal);
			...
			public override void RowPersisted(PXCache sender, PXRowPersistedEventArgs e)
			{
				...
				if (e.TranStatus == PXTranStatus.Aborted)
					if (valueBeforePersisting != null)
					{
						sender.SetValue(e.Row, _FieldOrdinal, valueBeforePersisting);
						valueBeforePersisting = null;
					}
			}
		}
		...
		{
			if (!sender.IsDirty) _oldRow = e.Row;
	...
	public class CurrencyPrecision : PXDefaultAttribute
	{
		...
		public CurrencyPrecision(short defaultValue, Type curyID)
			:base(defaultValue,
				CurrencyCollection.CombiteSearchPrecision(curyID))
		{
		...
		public override void FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			e.NewValue = CurrencyCollection.GetCurrency(sender, sourceCuryID, e.Row)?.DecimalPlaces;
	...
	public class PXCurrencyAttribute : PXDecimalAttribute, IPXFieldVerifyingSubscriber, IPXRowInsertingSubscriber
	{
		...
		public class PXCurrencyHelper
		{
			...
			protected bool _KeepResultValue = false;
			...
			{
				get => _KeepResultValue;
				set => _KeepResultValue = value;
			}
				...
				if (newRow != null && (oldRow == null || newRow.CuryRate != oldRow.CuryRate || newRow.CuryMultDiv != oldRow.CuryMultDiv))
				{
					...
					if (siblings != null && newRow.CuryRate != null)
					{
						...
						foreach (object data in siblings.SelectMultiBound(new object[] { e.Row }))
						{
							...
							if (_KeepResultValue)
								RecalculateFieldValue(cache, item, rate, mult);
							else
								RecalculateResultFieldValue(cache, item, rate, mult);
						}
					}
				}
			}

			protected virtual void RecalculateResultFieldValue(PXCache cache, object item, decimal rate, bool mult)
			{
			...
			protected virtual void RecalculateFieldValue(PXCache cache, object item, decimal rate, bool mult)
			{
				object fieldValue = null;
				var resultFieldValue = (decimal?)cache.GetValue(item, _ResultOrdinal);
				if (resultFieldValue != null)
					fieldValue = mult ? resultFieldValue.Value / rate : resultFieldValue.Value * rate;
				cache.RaiseFieldUpdating(FieldName, item, ref fieldValue);

				var oldFieldValue = cache.GetValue(item, _FieldOrdinal);
				if (oldFieldValue != fieldValue)
				{
					var oldItem = cache.CreateCopy(item);

					cache.SetValue(item, _FieldOrdinal, fieldValue);
					cache.RaiseFieldUpdated(FieldName, item, oldFieldValue);

					cache.RaiseRowUpdated(item, oldItem);
				}
			}
	...
	public class PXDBCurrencyAttribute : PXDBDecimalAttribute, IPXFieldVerifyingSubscriber, IPXRowInsertingSubscriber
	{
		...
		public bool KeepResultValue
		{
			get => _helper.KeepResultValue;
			set => _helper.KeepResultValue = value;
		}
		...
		public virtual void FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			if (e.NewValue != null)
			{
				// AC-286686: PXDBCurrencyAttribute implements IPXFieldVerifyingSubscriber,
				// which is not implemented in the base class, PXDBDecimalAttribute.
				// Therefore, for FieldVerifying event, PXDBCurrencyAttribute itself should
				// take responsibility of precision actualization.
	...
	public class PXDBBaseCuryAttribute : PXDBDecimalAttribute
	{
		...
		protected override void _ensurePrecision(PXCache sender, object row)
		{
			if (_Type != null)
			{
				_Precision = CurrencyCollection.GetCurrency(sender, _Type, row)?.DecimalPlaces;
				if(_Precision == null)
	...
	#region PXDBBaseCuryMaxPrecisionAttribute
	/// <summary>
	/// Extends <see cref="PXDBDecimalAttribute"/> by defaulting the precision property.
	/// If BranchID or CuryID is supplied than Precision is taken from the currency, corresponded to the Branch.BaseCuryID or Currency.CuryID.
	...
	public class PXDBBaseCuryMaxPrecisionAttribute : PXDBDecimalAttribute
	{
		protected override void _ensurePrecision(PXCache sender, object row)
		{
			_Precision = CurrencyCollection.GetBaseCurrencies()
				.Select(_ => CurrencyCollection.GetCurrency(_))
				.Max(_ => _.DecimalPlaces);
	...
	#region PXBaseCuryMaxPrecisionAttribute
	/// <summary>
	/// Extends <see cref="PXDecimalAttribute"/> by defaulting the precision property.
	/// If BranchID or CuryID is supplied than Precision is taken from the currency, corresponded to the Branch.BaseCuryID or Currency.CuryID.
	/// Otherwise system searchef for the max Precision for base currencies, set for Companies.
	/// </summary>
	public class PXBaseCuryMaxPrecisionAttribute : PXDecimalAttribute
	{
		protected override void _ensurePrecision(PXCache sender, object row)
		{
			_Precision = CurrencyCollection.GetBaseCurrencies()
	...
	public class PXDBCuryAttribute : PXDBDecimalAttribute
	{
		...
		protected override void _ensurePrecision(PXCache sender, object row)
		{
			...
			{
				if (sourceCuryID != null)
				{
					_Precision = CurrencyCollection.GetCurrency(sender, sourceCuryID, row)?.DecimalPlaces;
				}
				else
				{
					if (branchID != null)
					{
						_Precision = CurrencyCollection.GetCurrency(PXAccess.GetBranch((int?)GetSourceID(sender, row, branchID))?.BaseCuryID)?.DecimalPlaces;
					}

					if (branchID == null || _Precision == null)
					{
						_Precision = CurrencyCollection.GetCurrency(sender.Graph.Accessinfo.BaseCuryID)?.DecimalPlaces;
					}
				}
			...
			foreach (object ret in adapter.Get())
			{
				if (!anyDiff)
				{
					...
					else
					{
						...
						{
							foreach (string field in cache.Fields.Where(Extensions.CurrencyInfoAttribute.IsDifferenceEssential))
							{
	...
	public sealed class ContactGISelector : PXCustomSelectorAttribute
	{
		public ContactGISelector()
		 : base(typeof(Search3<GIDesign.designID, OrderBy<Asc<GIDesign.name>>>),
			typeof(GIDesign.name), typeof(GIDesign.sitemapTitle), typeof(GIDesign.sitemapScreenID))
		{
		...
		{
			var graph = PXGraph.CreateInstance<GenericInquiryDesigner>();
			return PXGenericInqGrph
				   .Def
				   .Where(d =>
					   d.Tables.Any(t =>
						   t.Name.IsIn(typeof(Contact).FullName, typeof(CRLead).FullName)
						   && (d.Relations.Any(x =>
								   x.ParentTable == t.Alias || x.ChildTable == t.Alias || x.ParentTable == null)
							   || !d.Relations.Any())))
				   .Select(d =>
				   {
					   var design = PXCache<GIDesign>.CreateCopy(d.Design);
					   if (graph.ScreenToSiteMapAddHelper.FindNodes(design).FirstOrDefault() is { } node)
					   {
						   design.SitemapScreenID = node.ScreenID;
						   design.SitemapTitle = node.Title;
					   }
					   return design;
				   });
		}
