...
namespace PX.Objects.CM
{
	...
	public class TranslationProcess : PXGraph<TranslationProcess>
	{
		...
		[PXFilterable]
		public PXFilteredProcessing<CurrencyRate, TranslationParams> TranslationCurrencyRateRecords;
		...
		protected virtual void TranslateItems(List<CurrencyRate> list, TranslationParams parameters)
		{
			...
			Ledger destinationLedger = (Ledger)PXSelectorAttribute.Select<TranslationParams.destLedgerId>(TranslationParamsFilter.Cache, parameters);
			Currency destinationCurrency = CurrencyCollection.GetCurrency(destinationLedger.BaseCuryID);
			...
			TranslHistCreate(parameters, list);
			return;
		}
		...
		protected virtual IEnumerable translationCurrencyRateRecords()
		{
			...
			if (!rateList.Any() && TranslationParamsFilter.Current.SourceCuryID == TranslationParamsFilter.Current.DestCuryID)
			{
				CurrencyRate same = new CurrencyRate
				{
					FromCuryID = TranslationParamsFilter.Current.SourceCuryID,
					ToCuryID = TranslationParamsFilter.Current.SourceCuryID,
					CuryRate = 1m,
					CuryMultDiv = "M"
				};
				rateList.Add(same);
		...
		protected static void TranslHistCreate(TranslationParams parameters, List<CurrencyRate> rateList)
		{
			...
			foreach (KeyValuePair<int?, decimal?> pair in totalAmount)
			{
				if (Math.Abs((decimal)pair.Value) > 0)
				{
					...
					PXProcessing.SetCurrentItem(histdet);
			...
			if (showResult)
			{
				using (new PXTimeStampScope(null))
				{
					...
					throw new PXRedirectRequiredException(TransHistGraph, false, "Translation Record");
				}
			}
		}
		public TranslationProcess()
		{
			...
			TranslationCurrencyRateRecords.SetProcessVisible(false);
			TranslationCurrencyRateRecords.SetProcessEnabled(false);
			TranslationCurrencyRateRecords.SetProcessAllCaption(Messages.CreateTranslation);
		...
		protected virtual void TranslationParams_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			TranslationCurrencyRateRecords.SetProcessDelegate(
				delegate (List<CurrencyRate> list)
				{
					TranslationProcess procGraph = CreateInstance<TranslationProcess>();
					procGraph.TranslateItems(list, translationParameters);
				}
			);
			TranslationCurrencyRateRecords.SetProcessAllEnabled(
		...
		protected virtual void CurrencyRate_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetEnabled<CurrencyRate.selected>(cache, rate, true);
		}
	...
	public class TranslationHistoryMaint : PXGraph<TranslationHistoryMaint>
	{
		...
		public void CreateBatch(TranslationHistory thist, bool setError)
		{
			...
			foreach (TranslationHistoryDetails histDet in PXSelect<TranslationHistoryDetails,
					...
					OrderBy<Asc<TranslationHistoryDetails.lineNbr, Asc<TranslationHistoryDetails.referenceNbr, Asc<
						TranslationHistoryDetails.branchID, Asc<TranslationHistoryDetails.accountID,
							Asc<TranslationHistoryDetails.subID, Asc<TranslationHistoryDetails.lineType>>>>>>>>
				.Select(grapth, thist.ReferenceNbr))
			{
