...
namespace PX.TM
{
    ...
    public class CompanyTreeMaint : PXGraph<CompanyTreeMaint>
    {
        ...
        public virtual void _(Events.RowPersisting<EPCompanyTreeMaster> e)
        {
            var row = e.Row;
            if (row == null) return;

            if (row.Description == PX.Objects.CR.Messages.New || row.Description == Messages.EmptyData)
            {
                throw new PXSetPropertyException(Messages.DublicateEmptyData, "Description");
            }

            foreach (EPCompanyTreeMaster item in SelectFrom<EPCompanyTreeMaster>
                                                .Where<EPCompanyTreeMaster.description.IsEqual<@P.AsString>>
                                                .View
                                                .Select(this, row.Description))
            {
                if (item != null && item.WorkGroupID != row.WorkGroupID && item.Description.ToLower() == row.Description)
                {
