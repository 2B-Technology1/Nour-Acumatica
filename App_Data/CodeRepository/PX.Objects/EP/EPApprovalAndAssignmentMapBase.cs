...
namespace PX.Objects.EP
{
	public class EPApprovalAndAssignmentMapBase<TGraph> : PXGraph<TGraph> where TGraph : PXGraph
	{
		...
		protected virtual IEnumerable nodesTree([PXDBGuid] Guid? ruleID)
		{
			...
			foreach (EPRule rule in list)
			{
				var node = new EPRuleTree
				{
					...
					AllowReassignment = rule.AllowReassignment,
		...
		[PXUIField(DisplayName = " ", MapEnableRights = PXCacheRights.Update, Enabled = true)]
		...
		protected virtual void EPRuleCondition_Value_FieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
		{
			...
			{
				PXFieldState state = sender.GetStateExt<EPRuleCondition.fieldName>(row) as PXFieldState;
		...
		/// <summary>
		/// Validates employee formula from OwnerSource field and raises an Exception if employee formula is invalid.
		/// </summary>
		protected virtual void _(Events.FieldVerifying<EPRule, EPRule.ownerSource> e)
		{
			try
			{
				var source = e.NewValue as string;

				if ((e.Row.RuleType != EPRuleType.Document) || string.IsNullOrEmpty(source))
					return;

				var hierarchyKey = new Regex(@"(?<=^\(\()[\S]{1,}(?=\)\)$)").Match(source).Value;
				if (string.IsNullOrEmpty(hierarchyKey))
					throw new Exception(Messages.EmployeeFormulaIsInvalid_Field);

				var parentKey = (hierarchyKey.Contains(".") ? hierarchyKey.Substring(0, hierarchyKey.LastIndexOf(".")) : hierarchyKey);
				if (string.IsNullOrEmpty(parentKey))
					throw new Exception(Messages.EmployeeFormulaIsInvalid);

				var childKey = (hierarchyKey.Contains(".")
					? hierarchyKey.Substring(hierarchyKey.LastIndexOf(".") + 1)
					: string.Empty);
				var pointIndex = parentKey.IndexOf('.'); // why not show user raw view of path string with '>' symbols and without brackets?
				if (pointIndex > 0)
				{
					parentKey = parentKey.Substring(0, pointIndex) + '.' + parentKey.Substring(pointIndex + 1).Replace('.', '>');
				}
				if (this.EntityItems.Select(new[] { parentKey }).Select(c => c[0] as CacheEntityItem).Any(c => c.SubKey == childKey))
					return;
			...
			{
				var args = new[] { e.NewValue.ToString().Truncate(20) };

				PXTrace.WriteWarning(Messages.EmployeeFormulaIsInvalid_Wrapper, args);

				throw new PXSetPropertyException(ex, PXErrorLevel.Warning, Messages.EmployeeFormulaIsInvalid_Wrapper, args);
			}
		}
		...
		protected PXFieldState CreateFieldStateForFieldName(object returnState, Type entityType, Type gType, string viewName, string cacheName, int? fieldLength, EPRuleBaseCondition condition)
		{
            ...
            if (entityType != null)
			{
				...
				{
					if (!CheckFieldValid(item)) continue;
		...
		protected PXFieldState CreateFieldStateForFieldValue(object returnState, string entityType, string cacheName, string fieldName)
		{
			...
			if (type != null)
			{
				...
				if ((state as PXStringState)?.AllowedValues == null)
				{
				...
				{
					state = PXStringState.CreateInstance(
						(state as PXStringState).Value, state.Length, false, state.Name, state.PrimaryKey
						,state.Required == true ? 1 : state.Required == null ? 0 : -1
						,(state as PXStringState)?.InputMask
						,(state as PXStringState)?.AllowedValues
						,(state as PXStringState)?.AllowedLabels
						,true
						,(string)(state as PXStringState)?.DefaultValue);
				}
		...
		public virtual bool CheckFieldValid(KeyValuePair<string, string> item) => item.Key.EndsWith("_Attributes") || !item.Key.Contains('_');
		#endregion
	}
}
