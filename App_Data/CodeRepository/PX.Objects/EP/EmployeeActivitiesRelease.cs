...
namespace PX.Objects.EP
{
	public class EmployeeActivitiesRelease : PXGraph<EmployeeActivitiesRelease>
	{
		...
		[PXCustomizeBaseAttribute(typeof(PXUIFieldAttribute), nameof(PXUIFieldAttribute.DisplayName), "Case Status")]
		...
		protected static void ReleaseActivities(List<EPActivityApprove> activities)
		{
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				if (!RecordCostTrans(registerEntry, useActivities, out activityAdded) && !activityAdded)
				{
					costTransError = true;
				}
		...
		public static bool RecordCostTrans(RegisterEntry registerEntry, List<EPActivityApprove> activities, out bool activityAdded)
		{
			...
			for (int i = 0; i < activities.Count; i++)
			{
				//Check the UNMERGED state of released as it could be set to true just now by the calling code.
				EPActivityApprove activity = PXSelect<EPActivityApprove>.Search<EPActivityApprove.noteID>(registerEntry, activities[i].NoteID);

				//Activity can be released to PM via time card prior to releasing the case.
				...
				try
				{
					...
					if (epsetup != null)
					{
						...
						//Cost transactions are created only if project is set.
						if (activity.ProjectTaskID != null && postingOptions != EPPostOptions.DoNotPost)
						{
					...
					{
						PXUpdate<
							Set<CRActivity.isLocked, True>, CRActivity,
							Where<CRActivity.noteID, Equal<Required<CRActivity.noteID>>>>.Update(registerEntry, activity.RefNoteID);
					}
					activityCache.Update(activity);
				}
				catch (Exception e)
