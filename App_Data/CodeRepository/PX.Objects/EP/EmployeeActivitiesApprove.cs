...
namespace PX.Objects.EP
{
		...
		public override void Persist()
		{
			...
			using (var ts = new PXTransactionScope())
			{
				if (groups.Count > 0)
				{
					...
					{
						maint.Clear();
						EPTimeCard card = maint.Document.Search<EPTimeCard.timeCardCD>(group.Key);
						maint.Document.Current = card;
						if (card.Status == EPTimeCardStatusAttribute.ApprovedStatus)
						{
							card.Status = EPTimeCardStatusAttribute.RejectedStatus;
							card.IsApproved = false;
							card.IsRejected = true;
							maint.Document.Update(card);
						}
						else
						{
	...
	public class EPActivityApprove : PMTimeActivity
	{
		...
		[PXUnboundDefault(typeof(Search<
			...
			Where<PMProject.contractID, Equal<Current<EPActivityApprove.projectID>>>>))]
		...
		#region ProjectTaskID
		/// <summary>
		/// Identifier of the <see cref="PX.Objects.PM.PMTask.TaskID">TaskID</see>.
		/// </summary>
		...
		[PXUnboundDefault(typeof(Search<
			PMTask.description,
			Where<PMTask.projectID, Equal<Current<EPActivityApprove.projectID>>, And<PMTask.taskID, Equal<Current<EPActivityApprove.projectTaskID>>>>>), PersistingCheck = PXPersistingCheck.Nothing)]
		...
		/// </summary>
		[PXUIField(DisplayName = "Cost Code Description", IsReadOnly = true, Visible = false, FieldClass = CostCodeAttribute.COSTCODE)]
		[PXString]
		[PXFieldDescription]
		[PXUnboundDefault(typeof(Search<
			PMCostCode.description,
			Where<PMCostCode.costCodeID, Equal<Current<EPActivityApprove.costCodeID>>>>))]
