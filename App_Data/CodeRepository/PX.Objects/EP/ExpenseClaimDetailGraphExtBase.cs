...
namespace PX.Objects.EP
{
		...
		where TGraph : PXGraph
	{
		...
		protected virtual decimal? GetUnitCostByExpenseItem(PXCache cache, EPExpenseClaimDetails receipt)
		{
			...
			CurrencyInfo currencyInfo = PXSelect<CurrencyInfo, Where<CurrencyInfo.curyInfoID, Equal<Required<EPExpenseClaimDetails.curyInfoID>>>>.Select(cache.Graph, receipt.CuryInfoID);

			decimal curyStdCost;
			if (item != null && currencyInfo != null && currencyInfo.CuryRate != null)
			{
		...
		public virtual void RefNbrUpdated(PXCache receiptsCache, EPExpenseClaim claim, EPExpenseClaimDetails receipt, string oldRefNbr)
		{
			...
			{
				CurrencyInfo info = PXSelect<CurrencyInfo, Where<CurrencyInfo.curyInfoID, Equal<Required<EPTaxTran.curyInfoID>>>>.Select(receiptsCache.Graph, copy.CuryInfoID);
				receiptsCache.Graph.Caches[typeof(CurrencyInfo)].RaiseFieldDefaulting<CurrencyInfo.curyPrecision>(info, out object newValue);
			...
			where TaskSubField : IBqlField
		{
			...
			Contract contract = PXSelect<Contract, Where<Contract.contractID, Equal<Required<Contract.contractID>>>>.Select(receiptCache.Graph,
																															receipt.ContractID);
			PMTask task = PMTask.PK.FindDirty(receiptCache.Graph, receipt.ContractID, receipt.TaskID);
