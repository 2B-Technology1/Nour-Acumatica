...
namespace PX.Objects.EP
{
	public class EmployeeMaint : PXGraph<EmployeeMaint>
	{
		...
		[PXCopyPasteHiddenFields(typeof(Location.vPaymentMethodID))]
		...
		public PXSelectJoin<VendorPaymentMethodDetail, 
                    ...
                    Where<VendorPaymentMethodDetail.bAccountID, Equal<Optional<Location.bAccountID>>,
			            And<VendorPaymentMethodDetail.locationID, Equal<Optional<Location.locationID>>,
    			            And<VendorPaymentMethodDetail.paymentMethodID, Equal<Optional<Location.vPaymentMethodID>>>>>,
		...
		public EmployeeMaint()
		{
			...
			AssigmentAndApprovalMaps.AllowSelect = PXAccess.FeatureInstalled<FeaturesSet.approvalWorkflow>();
		}
		...
		protected virtual void _(Events.CacheAttached<Contact.title> e)
		{
		...
		protected virtual void _(Events.FieldUpdated<EPEmployeePosition, EPEmployeePosition.startDate> e)
		{
			...
			DateTime? probationPeriodEndDate = GetProbationPeriodEndDate(row.StartDate.GetValueOrDefault(), row.ProbationPeriodEndDate, row.StartReason);
			e.Cache.SetValue<EPEmployeePosition.probationPeriodEndDate>(row, probationPeriodEndDate);
		}
		...
		{
			EPEmployeePosition row = e.Row;
			if (row == null || e.NewValue == null || e.OldValue == e.NewValue ||
				(row.StartReason != EPStartReason.New && row.StartReason != EPStartReason.Rehire))
			{
				return;
			}

			e.Cache.RaiseExceptionHandling<EPEmployeePosition.probationPeriodEndDate>(row, null, new PXSetPropertyException(Messages.ProbationEndDateUpdated, PXErrorLevel.Warning));
		}

		protected virtual void _(Events.FieldVerifying<EPEmployeePosition, EPEmployeePosition.startReason> e)
		{
			EPEmployeePosition row = e.Row;
			if (row == null || row.StartDate == null || e.OldValue == e.NewValue ||
				((string)e.NewValue != EPStartReason.New && (string)e.NewValue != EPStartReason.Rehire))
			{
				return;
			}
		...
		protected virtual void _(Events.FieldVerifying<EPEmployeePosition, EPEmployeePosition.probationPeriodEndDate> e)
		{
			...
			if ((DateTime?)e.NewValue != null && (DateTime?)e.NewValue < row.StartDate)
			{
				throw new PXSetPropertyException(Messages.ProbationEndDateEarlierThanStartDate);
			}
		}
		...
		{
			Contact defContact = Contact.SelectSingle();
			EPEmployeePosition activePosition = ActivePosition.Select();

			if (activePosition != null)
			{
				EPPosition activePositionDetails = PXSelect<EPPosition,
					Where<EPPosition.positionID, Equal<Required<EPPosition.positionID>>>>.Select(this, activePosition.PositionID);

				defContact.Salutation = activePositionDetails.Description;
				Contact.Update(defContact);
			}
		}
		...
		protected virtual void _(Events.FieldSelecting<VendorPaymentMethodDetail.detailValue> e) => PaymentMethodDetailHelper.VendorDetailValueFieldSelecting(this, e);
		...
		public virtual void ClearPaymentDetails(Location location, string paymentTypeID, bool clearNewOnly)
		{
		...
		public virtual DateTime? GetProbationPeriodEndDate(DateTime startDate, DateTime? probationPeriodEnd, string startReason)
		{
			if (startReason == EPStartReason.New || startReason == EPStartReason.Rehire)
			{
				int months = this.EmployeeClass.Current.ProbationPeriodMonths.GetValueOrDefault();
        ...
        // Acuminator disable once PX1016 ExtensionDoesNotDeclareIsActiveMethod extension should be constantly active
        public class EmployeeMaint_ActivityDetailsExt_Actions : ActivityDetailsExt_Actions<EmployeeMaint_ActivityDetailsExt, EmployeeMaint, EPEmployee, EPEmployee.noteID> { }

        // Acuminator disable once PX1016 ExtensionDoesNotDeclareIsActiveMethod extension should be constantly active
        public class EmployeeMaint_ActivityDetailsExt : ActivityDetailsExt<EmployeeMaint, EPEmployee, EPEmployee.noteID>
        {
            public override Type GetBAccountIDCommand() => typeof(EPEmployee.bAccountID);
        }
