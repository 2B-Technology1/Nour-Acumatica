...
namespace PX.Objects.EP
{
	public class EmployeeActivitiesEntry : PXGraph<EmployeeActivitiesEntry>
	{
		...
		[PXImport(typeof(PMTimeActivityFilter))]
		public PXSelectJoin<
				EPActivityApprove,
		...
		public EmployeeActivitiesEntry()
		{
			...
			EPActivityType activityType = EPActivityType.PK.Find(this, Setup.Current.DefaultActivityType);
		...
		[PXDBBool()]
		[PXDefault(typeof(Coalesce<Search<PMProject.certifiedJob, Where<PMProject.contractID, Equal<Current<PMTimeActivityFilter.projectID>>>>,
			Search<PMProject.certifiedJob, Where<PMProject.contractID, Equal<Current<PMTimeActivity.projectID>>>>,
		...
		{
			PXDBDateAndTimeAttribute.SetTimeVisible<EPActivityApprove.date>(Activity.Cache, null, Setup.Current.RequireTimes == true);
		...
		protected virtual void EPActivityApprove_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			{
				PXUIFieldAttribute.SetEnabled(sender, row, false);
				PXDBDateAndTimeAttribute.SetTimeEnabled<EPActivityApprove.date>(sender, row, IsImport);
				PXDBDateAndTimeAttribute.SetDateEnabled<EPActivityApprove.date>(sender, row, IsImport);
		...
		{
			EPActivityApprove row = (EPActivityApprove)e.Row;
			if (row == null || Setup.Current == null)
				return;
		...
		protected virtual void EPActivityApprove_TimeBillable_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
		...
		protected virtual void EPActivityApprove_Type_FieldDefaulting(PXCache cache, PXFieldDefaultingEventArgs e)
		{
			...
			{
				e.NewValue = Setup.Current.DefaultActivityType;
				e.Cancel = true;
			}
		}
