...
namespace PX.Objects.EP
{
	public class ExpenseClaimEntry : PXGraph<ExpenseClaimEntry, EPExpenseClaim>, PXImportAttribute.IPXPrepareItems
	{
		...
		{
			foreach (var rec in adapter.Get())
			{
				TryValidateRequireUniqueVendorRefWhenCardInvolved();

				EPExpenseClaim claim = rec as EPExpenseClaim;

				if (claim == null)
					claim = (PXResult<EPExpenseClaim>)rec;
		...
		public virtual IEnumerable ViewAPInvoice(PXAdapter adapter)
		{
			if (ExpenseClaimDetails.Current != null)
			{
				RedirectionToOrigDoc.TryRedirect(ExpenseClaimDetails.Current.APDocType, ExpenseClaimDetails.Current.APRefNbr, BatchModule.AP, preferPrimaryDocForm: true);
			}
			return adapter.Get();
		}
		#endregion
        ...
        protected virtual void EPExpenseClaim_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
        {
            ...
            if (employee != null)
            {
                ...
                if (!allowEdit)
                {
                    EPWingmanForExpenses wingMan = PXSelectJoin<EPWingmanForExpenses,
                                                    InnerJoin<EPEmployee, On<EPWingmanForExpenses.wingmanID, Equal<EPEmployee.bAccountID>>>,
                                                    Where<EPWingmanForExpenses.employeeID, Equal<Required<EPWingmanForExpenses.employeeID>>,
		...
		protected virtual void _(Events.FieldUpdated<EPExpenseClaim, EPExpenseClaim.curyDocBal> e)
		{
			foreach (EPApproval approval in Approval.Select())
			{
				Approval.Cache.SetDefaultExt<EPApproval.curyTotalAmount>(approval);
				Approval.Cache.SetDefaultExt<EPApproval.totalAmount>(approval);
				Approval.Cache.MarkUpdated(approval);
			}
		}
		...
		public virtual void ValidateProjectAndProjectTask(EPExpenseClaimDetails info)
		{
			if (info != null)
			{
				...
				if (info.TaskID != null)
				{
					PMTask projectTask = PMTask.PK.FindDirty(this, info.ContractID, info.TaskID);
		...
		protected virtual void EPExpenseClaimDetails_RowPersisting(PXCache cache, PXRowPersistingEventArgs e)
		{
			...
			if (row != null && e.Operation != PXDBOperation.Delete)
			{
				...
				{
					PMTask task = PMTask.PK.FindDirty(this, row.ContractID, row.TaskID);
		...
		protected virtual void EPExpenseClaimDetails_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			if (row != null)
			{
				...
				PXUIFieldAttribute.SetEnabled<EPExpenseClaimDetails.curyTranAmtWithTaxes>(cache, row, false);
				PXUIFieldAttribute.SetEnabled<EPExpenseClaimDetails.claimCuryTranAmtWithTaxes>(cache, row, false);
				PXUIFieldAttribute.SetEnabled<EPExpenseClaimDetails.status>(cache, row, false);
				PXUIFieldAttribute.SetEnabled<EPExpenseClaimDetails.aRRefNbr>(cache, row, false);
				PXUIFieldAttribute.SetEnabled<EPExpenseClaimDetails.aPRefNbr>(cache, row, false);
				PXUIFieldAttribute.SetEnabled<EPExpenseClaimDetails.curyNetAmount>(cache, row, false);
				PXUIFieldAttribute.SetEnabled<EPExpenseClaimDetails.taxZoneID>(cache, row, false);
				PXUIFieldAttribute.SetEnabled<EPExpenseClaimDetails.curyTaxTotal>(cache, row, false);
		...
		protected virtual void _(Events.FieldUpdated<EPExpenseClaimDetails, EPExpenseClaimDetails.uOM> e)
		{
			...
			if (!string.IsNullOrEmpty(oldUom)
				&& !string.IsNullOrEmpty(e.Row.UOM)
				&& oldUom != e.Row.UOM
				&& curyUnitCost != 0m)
			{
		...
		[PXSelector(typeof(Search2<CACorpCard.corpCardID,
						InnerJoin<EPEmployeeCorpCardLink,
							On<EPEmployeeCorpCardLink.employeeID, Equal<Current2<EPExpenseClaimDetails.employeeID>>,
						...
						InnerJoin<GL.Account,
							On<GL.Account.accountID, Equal<CashAccount.accountID>>>>>>),
			typeof(CACorpCard.corpCardCD), typeof(CACorpCard.name), typeof(CACorpCard.cardNumber), typeof(GL.Account.curyID),
			SubstituteKey = typeof(CACorpCard.corpCardCD),
			DescriptionField = typeof(CACorpCard.name))]
		protected virtual void _(Events.CacheAttached<EPExpenseClaimDetails.corpCardID> e) { }
		#endregion
