...
namespace PX.Objects.EP
{
	...
	public class TimeCardMaint : PXGraph<TimeCardMaint, EPTimeCard>, PXImportAttribute.IPXPrepareItems
	{
		...
		public PXSetup<EPEmployee>.Where<EPEmployee.bAccountID.IsEqual<EPTimeCard.employeeID.AsOptional>> Employee;
		...
		public PXSelectJoin<EPTimeCard,
			InnerJoin<EPEmployee, 
				On<EPEmployee.bAccountID, Equal<EPTimeCard.employeeID>>>,
		...
		public virtual IList<EPTimeCardSummaryWithInfo> SelectSummaryRecords(EPTimeCard document, bool autocorrect)
		{
			...
			/*
			* List all summary records that are in the database.
		...
		public PXSelect<EPTimecardDetail,
			Where<EPTimecardDetail.ownerID, Equal<Current<EPEmployee.defContactID>>,
				...
				And<Where<EPTimecardDetail.timeCardCD, IsNull, 
					Or<EPTimecardDetail.timeCardCD, Equal<Current<EPTimeCard.timeCardCD>>>>>>>>>,
		...
		public virtual IEnumerable activities()
		{
			...
			if (PXView.Currents != null)
				foreach (object current in PXView.Currents)
				{
					if (current is EPTimeCard currentDocument)
						document = currentDocument;
				}
		...
		protected bool skipProjectDefaultingFromEarningType = false;
		...
		public string ExpenseAccountSource
		{
			get
			{
				return string.IsNullOrEmpty(PmSetup.Current?.ExpenseAccountSource) ? PM.PMExpenseAccountSource.InventoryItem : PmSetup.Current.ExpenseAccountSource;
			}
		}
		...
		public string ExpenseSubMask
		{
			get
			{
				return string.IsNullOrEmpty(PmSetup.Current?.ExpenseSubMask) ? null : PmSetup.Current.ExpenseSubMask;
			}
		}
		...
		public string ExpenseAccrualSubMask
		{
			get
			{
				return string.IsNullOrEmpty(PmSetup.Current?.ExpenseAccrualSubMask) ? null : PmSetup.Current.ExpenseAccrualSubMask;
			}
		}
		...
		public string ActivityTimeUnit
		{
			get
			{
				return String.IsNullOrEmpty(EpSetup.Current?.ActivityTimeUnit) ? EPSetup.Minute : EpSetup.Current.ActivityTimeUnit;
			}
		}
		...
		public string EmployeeRateUnit
		{
			get
			{
				return String.IsNullOrEmpty(EpSetup.Current?.EmployeeRateUnit) ? EPSetup.Hour : EpSetup.Current.EmployeeRateUnit;
			}
		}
		...
		public bool ShowActivityTime
		{
			get
			{
				return EpSetup.Current?.RequireTimes == true;
			}
		}
			...
			foreach (EPTimeCardSummaryWithInfo item in summarySelect.Select(previous.TimeCardCD))
			{
					...
					item.EarningType != EpSetup.Current.VacationsType)
				{
					...
					foreach (EPTimeCardSummaryWithInfo existing in Summary.Select())
					{
						...
						PMTask task = PMTask.PK.FindDirty(this, item.ProjectID, item.ProjectTaskID);
						{
		...
		public virtual void PreloadHolidays()
		{
			...
			for (int i = 0; i < 7; i++)
			{
				...
				if (CalendarHelper.IsHoliday(this, Employee.Current.CalendarID, day))
			...
			Summary.Cache.SetDefaultExt<EPTimeCardSummaryWithInfo.projectDescription>(summary);
			if (Document.Cache.GetStatus(Document.Current) != PXEntryStatus.Inserted)
				this.Save.Press();
		}
		...
		public virtual void NormalizeTimecard()
		{
			...
			int numberOfSummaryRecords = 0;
			int? summaryLineNbr = null;
			...
			foreach (EPTimeCardSummaryWithInfo item in Summary.Select())
			{
				...
				if (item.EarningType.Trim() == EpSetup.Current.RegularHoursType.Trim() && item.IsBillable == false && item.ProjectID == nonProjectID)
				{
					numberOfSummaryRecords++;
					summaryLineNbr = item.LineNbr;
				}
			}
			...
			summary.Description = PXMessages.LocalizeNoPrefix(Messages.NormalizationDesc);
				summary.ProjectID = nonProjectID;
			...
			try
			{
				...
				summary = Summary.Insert(summary);
					summaryLineNbr = summary.LineNbr;
			}
			finally
			{
			...
			if (monQuota > 0)
			{
				...
				detailNormalize_Mon.ProjectID = nonProjectID;
				detailNormalize_Mon.IsBillable = false;
				if (summaryLineNbr != null) detailNormalize_Mon.SummaryLineNbr = summaryLineNbr;
			...
			if (delta >= 0)
			{
				...
				if (tueQuota > 0)
				{
					...
					detailNormalize_Tue.ProjectID = nonProjectID;
					detailNormalize_Tue.IsBillable = false;
					if (summaryLineNbr != null) detailNormalize_Tue.SummaryLineNbr = summaryLineNbr;
			...
			if (delta >= 0)
			{
				...
				if (wedQuota > 0)
				{
					...
					detailNormalize_Wed.ProjectID = nonProjectID;
					detailNormalize_Wed.IsBillable = false;
					if (summaryLineNbr != null) detailNormalize_Wed.SummaryLineNbr = summaryLineNbr;
			...
			if (delta >= 0)
			{
				...
				if (thuQuota > 0)
				{
					...
					detailNormalize_Thu.ProjectID = nonProjectID;
					detailNormalize_Thu.IsBillable = false;
					if (summaryLineNbr != null) detailNormalize_Thu.SummaryLineNbr = summaryLineNbr;
			...
			if (delta >= 0)
			{
				...
				if (friQuota > 0)
				{
					...
					detailNormalize_Fri.ProjectID = nonProjectID;
					detailNormalize_Fri.IsBillable = false;
					if (summaryLineNbr != null) detailNormalize_Fri.SummaryLineNbr = summaryLineNbr;
		...
		protected virtual void EPTimeCard_RowDeleted(PXCache sender, PXRowDeletedEventArgs e)
		{
			...
			EPTimeCard timeCard = PXSelect<
				EPTimeCard,
				Where<EPTimeCard.employeeID, Equal<Current<EPTimeCard.employeeID>>,
					And<EPTimeCard.weekId, Greater<Current<EPTimeCard.weekId>>,
					And<Where<EPTimeCard.timeCardCD, Equal<Current<EPTimeCard.origTimeCardCD>>,
						Or<Current<EPTimeCard.origTimeCardCD>, IsNull>>>>>>.SelectWindowed(this, 0, 1);
		...
		protected virtual void EPTimeCard_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (Employee.Current != null)
			{
				...
				{
					UserPreferences pref = UserPreferences.PK.Find(sender.Graph, Employee.Current.UserID);
					...
					else if (Employee.Current.CalendarID != null)
					{
						CSCalendar cal = CSCalendar.PK.Find(this, Employee.Current.CalendarID);
			...
			ValidateTotals(row, out string errorMsg);
			...
			if ((bool?)e.NewValue == true)
			{
				...
				foreach (PXResult res in activities())
				{
						...
						act.ApprovalStatus != ActivityStatusListAttribute.Canceled)
					{
						...
						Activities.Cache.RaiseExceptionHandling<EPTimecardDetail.timeSpent>(act, act.TimeSpent, new PXSetPropertyException(Messages.ActivityIsNotApproved, PXErrorLevel.RowError));
		...
		protected virtual void EPTimeCardSummaryWithInfo_RowInserting(PXCache sender, PXRowInsertingEventArgs e)
		{
			...
			EPEarningType earningType = EPEarningType.PK.Find(this, row.EarningType);
			if (earningType != null && !skipProjectDefaultingFromEarningType)
			{
				...
				{
					PMTask defTask = PMTask.PK.FindDirty(this, earningType.ProjectID, earningType.TaskID);
		...
		protected virtual void EPTimeCardSummaryWithInfo_ProjectID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			PMTask currentTask = PMTask.PK.FindDirty(this, row.ProjectID, row.ProjectTaskID);
			if (currentTask != null)
			{
				PMTask newTask = PMTask.UK.Find(this, row.ProjectID, currentTask.TaskCD);
				if (newTask != null)
				{
		...
		protected virtual void EPTimeCardSummaryWithInfo_ProjectTaskID_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			if (e.NewValue != null && e.NewValue is int newValue)
			{
				PMTask task = PMTask.PK.Find(sender.Graph, row.ProjectID, newValue);
				if (task != null)
				{
		...
		protected virtual void EPTimecardDetail_RowInserting(PXCache sender, PXRowInsertingEventArgs e)
		{
			...
			if (row.OrigNoteID != null)
				return;//inserting correction transaction - do not init just insert it as is.
			...
			row.OwnerID = Employee.Current.With(_ => _.DefContactID);
			row.Billed = false;
			row.TrackTime = true;
		...
		protected virtual void EPTimecardDetail_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (!dontSyncSummary)
			{
				...
				if (summary != null && row.SummaryLineNbr == null)
				{
					row.SummaryLineNbr = summary.LineNbr;
				}
		...
		protected virtual void EPTimecardDetail_RowDeleted(PXCache sender, PXRowDeletedEventArgs e)
		{
			...
			try
			{
				...
				if (row.SummaryLineNbr == null)//adjust activity is deleted
					return;
		...
		{
			if (Document.Current == null || Document.Current.WeekID == null)
				return;
		...
		{
			EPTimecardDetail row = e.Row as EPTimecardDetail;
			if (row == null || e.ExternalCall != true)
				return;
		...
		protected virtual void EPTimecardDetail_StartDate_Date_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			if (e.NewValue is string value
				&& DateTime.TryParse(value, sender.Graph.Culture, System.Globalization.DateTimeStyles.None, out DateTime valFromString))
			{
		...
		protected virtual void EPTimecardDetail_StartDate_Date_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (valuePending != PXCache.NotSetValue)
			{
						...
						oldValue != newValue && ((int)oldValue.Value.TimeOfDay.TotalMinutes == (int)newValue.Value.TimeOfDay.TotalMinutes)))
				{
					...
					{
						CalendarHelper.CalculateStartEndTime(this, calendarId, newValue.Value.Date, out DateTime start, out DateTime end);
		...
		public virtual void ProcessRegularTimecard(PM.RegisterEntry releaseGraph, EPTimeCard timecard)
		{
			...
			var emp = EPEmployee.PK.Find(releaseGraph, timecard.EmployeeID);
			if (emp != null)
			{
		...
		public virtual EPEarningType InitEarningType(EPTimecardDetail row)
		{
			...
			EPEarningType earningType = EPEarningType.PK.Find(this, row.EarningTypeID);
		...
		public virtual bool IsFit(EPTimeCardSummary summary, EPTimecardDetail activity)
		{
			...
			if (activity.ProjectID.GetValueOrDefault(ProjectDefaultAttribute.NonProject().GetValueOrDefault(0)) != summary.ProjectID.GetValueOrDefault(ProjectDefaultAttribute.NonProject().GetValueOrDefault(0)))
				return false;

			if (activity.ProjectTaskID != null || summary.ProjectTaskID != null)
			{
				if (activity.ProjectTaskID != summary.ProjectTaskID)
					return false;
			}
			...
			if (activity.IsBillable.GetValueOrDefault() != summary.IsBillable.GetValueOrDefault())
				return false;

			if (activity.CertifiedJob.GetValueOrDefault() != summary.CertifiedJob.GetValueOrDefault())
				return false;

			if (activity.ParentTaskNoteID != null)
			{
				if (activity.ParentTaskNoteID != summary.ParentNoteID)
					return false;
			}
			else
			{
			...
			{
				if (activity.JobID != summary.JobID)
					return false;
			}
			else
			{
			...
			{
				if (activity.ShiftID != summary.ShiftID)
					return false;
			}
			else
			{
			...
			{
				if (activity.CostCodeID != summary.CostCodeID)
					return false;
			}
			else
			{
			...
			{
				if (summary.UnionID != null)
					return false;
			}

			if (activity.LabourItemID != null)
			{
				if (activity.LabourItemID != summary.LabourItemID)
					return false;
			}
			else
			{
				if (summary.LabourItemID != null)
					return false;
			}
		...
		{
			if (summary.LineNbr == duplicate.LineNbr)
				return false;

			if (duplicate.ProjectID.GetValueOrDefault(ProjectDefaultAttribute.NonProject().GetValueOrDefault(0)) != summary.ProjectID.GetValueOrDefault(ProjectDefaultAttribute.NonProject().GetValueOrDefault(0)))
				return false;

			if (duplicate.ProjectTaskID != null || summary.ProjectTaskID != null)
			{
				if (duplicate.ProjectTaskID != summary.ProjectTaskID)
					return false;
			}

			if (duplicate.EarningType != summary.EarningType)
				return false;

			if (duplicate.IsBillable.GetValueOrDefault() != summary.IsBillable.GetValueOrDefault())
				return false;

			if (duplicate.CertifiedJob.GetValueOrDefault() != summary.CertifiedJob.GetValueOrDefault())
				return false;

			if (duplicate.ParentNoteID != null)
			{
				if (duplicate.ParentNoteID != summary.ParentNoteID)
					return false;
			}
			else
			{
			...
			{
				if (duplicate.JobID != summary.JobID)
					return false;
			}
			else
			{
			...
			{
				if (duplicate.ShiftID != summary.ShiftID)
					return false;
			}
			else
			{
			...
			{
				if (duplicate.CostCodeID != summary.CostCodeID)
					return false;
			}
			else
			{
			...
			{
				if (duplicate.LabourItemID != summary.LabourItemID)
					return false;
			}
			else
			{
		...
		{
			CSCalendar calendar = CSCalendar.PK.Find(this, Employee.Current?.CalendarID);
			if (calendar != null)
			{
				switch (date.Value.DayOfWeek)
				{
					case DayOfWeek.Monday: return PXDBDateAndTimeAttribute.CombineDateTime(date, calendar.MonStartTime);
					case DayOfWeek.Tuesday: return PXDBDateAndTimeAttribute.CombineDateTime(date, calendar.TueStartTime);
					case DayOfWeek.Wednesday: return PXDBDateAndTimeAttribute.CombineDateTime(date, calendar.WedStartTime);
					case DayOfWeek.Thursday: return PXDBDateAndTimeAttribute.CombineDateTime(date, calendar.ThuStartTime);
					case DayOfWeek.Friday: return PXDBDateAndTimeAttribute.CombineDateTime(date, calendar.FriStartTime);
					case DayOfWeek.Saturday: return PXDBDateAndTimeAttribute.CombineDateTime(date, calendar.SatStartTime);
					case DayOfWeek.Sunday: return PXDBDateAndTimeAttribute.CombineDateTime(date, calendar.SunStartTime);
				}
			}
		...
		public virtual void ValidateProjectAndProjectTask(EPTimecardDetail timeCardDetail)
		{
			if (timeCardDetail != null && Document.Current != null && Document.Current.IsReleased != true)
			{
				...
				{
					PMTask projectTask = PMTask.PK.FindDirty(this, timeCardDetail.ProjectID, timeCardDetail.ProjectTaskID);
		...
		public virtual bool ValidateTotals(EPTimeCard timecard, out string errorMsg)
		{
			...
			EPEmployee employee = EPEmployee.PK.Find(this, timecard.EmployeeID);
		...
		{
			InventoryItem nsItem = InventoryItem.PK.Find(this, record.InventoryID);
			if (nsItem == null)
			...
			Contract contract = Contract.PK.Find(this, record.ProjectID);
			...
			int? branchID = null;
			EP.EPEmployee emp = EPEmployee.PK.Find(this, employeeID);
			if (emp != null)
			{
			...
			if (contract.BaseType == CT.CTPRType.Project)//contract do not record money only usage.
			{
				...
				{
					PMTask task = PMTask.PK.FindDirty(this, record.ProjectID, record.TaskID);
			...
			if (EpSetup.Current != null && EPSetupMaint.GetPostToOffBalance(this, EpSetup.Current, employeeID))
			{
				accountGroupID = EPSetupMaint.GetOffBalancePostingAccount(this, EpSetup.Current, employeeID);
				accountID = null;
		...
		/// <summary>
		/// Specialized version of <see cref="PMTimeActivity"/>.<br></br>
		...
		public class EPTimecardDetail : PMTimeActivity
		{
			...
			/// <summary>
			/// The <see cref="PX.Objects.CR.CRActivityClass">task</see> associated with this activity.<br></br>
			...
			/// <summary>
			/// The <see cref="PX.Objects.EP.EPTimeCard">Time Card</see> associated with this activity.<br></br>
			/// </summary>
			[PXDBString(10)]
			[PXUIField(Visible = false)]
			...
			/// <summary>
			/// The <see cref="PX.Objects.PM.PMProject">project</see> on which the employee worked.<br></br>
			...
			/// <summary>
			/// The <see cref="PX.Objects.PM.PMTask">task</see> on which the employee worked. <br></br>
			...
			/// <summary>
			/// The identifier of the <see cref="PMCostCode">cost code</see> associated with the activity.
			/// </summary>
			/// <value>
			...
			/// </summary>
			/// <value>
			/// The identifier of the <see cref="PX.Objects.PM.PMUnion">union local</see> associated with the activity.
			/// </value>
			...
			/// </summary>
			/// <value>
			/// The value of this field corresponds to the value of the <see cref="PX.Objects.EP.EPWeekRaw.Description" /> field.
			/// </value>
			[PXDBInt()]
			...
			/// <summary>
			/// The identifier of the <see cref="PX.Objects.IN.InventoryItem">labor item</see> associated with the activity.
			/// </summary>
			...
			/// </summary>
			/// <value>
			/// The <see cref="PX.Objects.CR.PMTimeActivity.WorkCodeID">work code</see> associated with this activity.
			/// </value>
			...
			/// <summary>
			/// The work time reported for the selected date.
			/// </summary>
			[PXTimeList]
			[PXDefault(0)]
			[PXDBInt()]
			...
			/// <summary>
			/// Indicates if the work hours are billable.
			/// </summary>
			[PXDBBool()]
			...
			/// <summary>
			/// The billable time for the employee.
			/// </summary>
			[PXTimeList]
			[PXDefault(0)]
			[PXDBInt()]
			...
			/// The default date is defined in <see cref="PX.Objects.EP.EPEmployeeClass"/>.
			/// </summary>
			...
			/// The default shift code is defined in <see cref="PX.Objects.EP.EPEmployee"/>.<br></br>
			...
			/// <summary>
			/// The user-friendly unique identifier of the <see cref="PX.Objects.CR.CRCase">Case</see> associated with this activity.
			/// </summary>
			...
			/// <summary>
			/// The user-friendly unique identifier of the <see cref="PX.Objects.CT.Contract">Contract</see> associated with this activity.
			/// </summary>
			...
			/// <summary>
			/// Indicates whether this work activity is associated with an overtime earning type, as defined by the <see cref="PX.Objects.CR.PMTimeActivity.EarningTypeID"/> property.<br></br>
			...
			/// <summary>
			/// The billable overtime for the employee.
			/// </summary>
			...
			/// </summary>
			/// <value>
			/// For overtime items, the value is taken from the <see cref="PX.Objects.EP.TimeCardMaint.EPTimecardDetail.TimeSpent"/> property.
			/// </value>
			[PXTimeList]
			[PXInt]
		...
		[PXProjection(typeof(Select2<EPTimeCardSummary,
			...
			LeftJoin<PMTask,
				On<EPTimeCardSummary.projectID, Equal<PMTask.projectID>, And<EPTimeCardSummary.projectTaskID, Equal<PMTask.taskID>>>>>>>), new Type[] { typeof(EPTimeCardSummary)})]
		public class EPTimeCardSummaryWithInfo : EPTimeCardSummary
		{
			...
			/// </summary>
			[PXDefault(typeof(Search<PMTask.taskID, Where<PMTask.projectID, Equal<Current<projectID>>, And<PMTask.isDefault, Equal<True>>>>), PersistingCheck = PXPersistingCheck.Nothing)]
			[PXRestrictor(typeof(Where<PMTask.isCancelled, NotEqual<True>>), Messages.CancelledTask)]
			...
			[PXDefault(typeof(Search<
				PMTask.description,
				Where<PMTask.projectID, Equal<Current<projectID>>, And<PMTask.taskID, Equal<Current<projectTaskID>>>>>), PersistingCheck = PXPersistingCheck.Nothing)]
			...
			#region CostCodeDescription
			/// <summary>
			/// Cost Code's description field
			/// </summary>
			public abstract class costCodeDescription : PX.Data.BQL.BqlString.Field<costCodeDescription> { }
			/// <summary>
			/// Gets sets Cost Code's description
			/// </summary>
			[PXUIField(DisplayName = "Cost Code Description", IsReadOnly = true, FieldClass = CostCodeAttribute.COSTCODE)]
			[PXString]
			[PXFieldDescription]
			[PXUnboundDefault(typeof(Search<
				PMCostCode.description,
				Where<PMCostCode.costCodeID, Equal<Current<costCodeID>>>>))]
		...
		/// <summary>
		/// Extends the Contract DAC for use with Time Cards.
		/// </summary>
		[Serializable]
		public class ContractEx : Contract
		{
			...
			/// </summary>
			/// <value>
			/// The field can have one of the values described in <see cref="CTPRType.ListAttribute"/>.
			/// </value>
			...
			/// <summary>
			/// Identifier of the <see cref="PX.Objects.AR.Customer"/> to which the contract belongs.
			/// </summary>
			/// <value>
			/// Corresponds to the <see cref="PX.Objects.AR.Customer.PK"/> field.
			/// </value>
			...
			/// </summary>
			/// <value>
			/// The field can have one of the values described in <see cref="Contract.status.ListAttribute"/>.
			/// </value>
	...
	public class TimecardApprovalAction : EPApprovalAutomation<EPTimeCard, EPTimeCard.isApproved, EPTimeCard.isRejected, EPTimeCard.isHold, EPSetupTimeCardApproval>
	{
		...
		{
			var items = this._Pending.SelectMulti(GetSourceNoteID(source));
			...
			foreach (EPApproval item in items)
			{
				if (ValidateApprovalAccess(item) || IsTaskApprover(item.RefNoteID, item.OwnerID))
				{
					return true;
				}
			}
		...
		{
			return new SelectFrom<EPTimeCard>
				.InnerJoin<EPTimeCardSummary>.On<EPTimeCardSummary.FK.Timecard>
				.InnerJoin<PMTask>.On<EPTimeCardSummary.FK.ProjectTask>
				.Where<EPTimeCard.noteID.IsEqual<@P.AsGuid>
					.And<PMTask.approverID.IsEqual<@P.AsInt>>>.View(_Graph).Select(refNoteID, ownerID).Any();
		}
