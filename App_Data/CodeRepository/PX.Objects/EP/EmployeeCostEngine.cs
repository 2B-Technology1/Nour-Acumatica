...
namespace PX.Objects.EP
{
    ...
    public class EmployeeCostEngine
	{
		...
		{
			return GetEmployeeRate(laborItemID, projectID, projectTaskID, certifiedJob, unionID, employeeId, date, null);
		}

		public virtual Rate GetEmployeeRate(int? laborItemID, int? projectID, int? projectTaskID, bool? certifiedJob, string unionID, int? employeeId, DateTime? date, string curyID)
		{
			if (date == null)
				return null;
			...
			var paramList = new List<object>() { laborItemID, employeeId, projectID, projectTaskID };
			if(curyID != null)
			{ 
				select.WhereAnd<Where<PMLaborCostRate.curyID, Equal<Required<PMLaborCostRate.curyID>>>>();
				paramList.Add(curyID);
			}
			PXResultset<PMLaborCostRate> resultset = select.Select(paramList.ToArray());
