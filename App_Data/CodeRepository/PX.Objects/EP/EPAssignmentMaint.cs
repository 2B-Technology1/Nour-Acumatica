...
namespace PX.Objects.EP
{
	...
	{		
		...
		public PXSelectJoin<EPAssignmentRoute,
			LeftJoin<EPEmployee, On<EPEmployee.defContactID, Equal<EPAssignmentRoute.ownerID>>>,
		...
		{
			return PXSiteMap.Provider.GetNodes()
					.Where(x => this.GetEntityTypeScreens().Contains(x.ScreenID))
		...
		private IEnumerable<PXSiteMapNode> GraphTypeList()
		{
			...
			if (route != null && aMap.EntityType != null)
			{
				...
				{
					foreach (var node in GetEntityTypeNodes())
					{
						yield return node;
					}
				}
		...
		protected virtual void EPAssignmentMap_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (row != null)
			{
				...
				var list = GraphTypeList().ToList().OrderBy(o => o.Title);
