...
namespace PX.Objects.RQ
{
	...
	public class RQBiddingProcess : PXGraph<RQBiddingProcess>
	{
		...
		private RQRequisition DoChooseVendor(RQRequisition item, RQBiddingVendor vendor)
		{
			...
			{
				CurrencyInfo vendorCuryInfo = currencyinfo.SelectWindowed(0, 1,vendor.CuryInfoID);
				CurrencyInfo copyInfo = PXCache<CurrencyInfo>.CreateCopy(currencyinfo.SelectWindowed(0, 1, upd.CuryInfoID));

				if (!currencyinfo.Cache.ObjectsEqual<CurrencyInfo.curyID, CurrencyInfo.curyRateTypeID, CurrencyInfo.curyRate, CurrencyInfo.curyEffDate>(copyInfo, vendorCuryInfo))
				{
                    ...
                    try
                    {
						...
						if (sharedID == copyInfo.CuryInfoID)
						{
							...
							upd.CuryInfoID = copyInfo.CuryInfoID;
		...
		public virtual IEnumerable process(PXAdapter adapter)
		{
			...
			foreach (RQRequisition rec in adapter.Get<RQRequisition>())
			{
				...
				if(!hasError)
				{
					RQRequisition requisition = reqEntry.Value.Document.Search<RQRequisition.reqNbr>(item.ReqNbr);
					RQRequisition originalrequisition = PXCache<RQRequisition>.CreateCopy(requisition);

					requisition.CompleteBidding(reqEntry.Value);

					item = PXCache<RQRequisition>.CreateCopy(item);

					foreach (var diffField in reqEntry.Value.Document.Cache.GetDifference(originalrequisition, requisition))
