...
namespace PX.Objects.RQ
{
	public class RQRequisitionEntry : PXGraph<RQRequisitionEntry, RQRequisition>, IGraphWithInitialization
	{
		...
		public virtual IEnumerable CreatePOOrder(PXAdapter adapter)
		{
			...
			{
				var graph = PXGraph.CreateInstance<RQRequisitionEntry>();

				foreach (RQRequisition item in list)					
					graph.ProcessToPOOrder(item, massProcess);
			});
		...
		protected virtual void ProcessToPOOrder(RQRequisition item, bool massProcess)
		{
			...
			{
				if (massProcess)
					PXProcessing<RQRequisition>.SetCurrentItem(item);
				...
				Document.Current = Document.Search<RQRequisition.reqNbr>(item.ReqNbr);
						...
						if (oldOrder == null)
						{
							...
							if (item.VendorID != null && item.VendorLocationID != null)
							{
									...
									.SelectMultiBound(this, new object[] { item }))
								{
									...
									Lines.Cache.RaiseFieldDefaulting<RQRequisitionLine.lineType>(line, out object objDefaultLineType);
									string defaultLineType = (string)objDefaultLineType;
									...
									if (item.POType == POOrderType.DropShip)
									{
										lineType = inv?.StkItem == true ? POLineType.GoodsForDropShip
											...
											: line.LineType; // Should be impossible due to the error: "Only items for which both 'Require Receipt' and 'Require Shipment' are selected can be drop shipped.".
									}
									else
									{
										lineType = inv?.StkItem == true ? POLineType.GoodsForSalesOrder : defaultLineType; // POLinkDialog can replace NonStock with NonStockForSalesOrder
									}
									...
									InsertPOLine(graph, line, minQty, bid.CuryQuoteUnitCost, bid, defaultLineType, biddier?.PromisedDate);

									qty -= minQty;

									if (qty > 0)
									{
										InsertPOLine(graph, line, qty, line.CuryEstUnitCost, line, defaultLineType, biddier?.PromisedDate);
									}
								}
							}
							else
							{
								...
								OrderBy<Asc<RQBidding.vendorID, Asc<RQBidding.vendorLocationID, Asc<RQRequisitionLine.lineNbr>>>>>.Select(this, item.ReqNbr))
								{
									...
									if (custQty > 0)
									{
										string lineType = defaultLineType;
										InventoryItem inv = InventoryItem.PK.Find(graph, line.InventoryID);
										if (item.POType == POOrderType.DropShip)
										{
											lineType = inv?.StkItem == true ? POLineType.GoodsForDropShip
									...
									if (qty > custQty)
										InsertPOLine(graph, line, qty - custQty, bidding.CuryQuoteUnitCost, bidding, defaultLineType, biddier?.PromisedDate);
				...
				if (requisition.CustomerID != null && req == null)
				{
					...
					using (PXTransactionScope scope = new PXTransactionScope())
					{
						try
						{
							if (sograph.IsDirty)
							{
								bool oldRecalculateExternalTaxesSync = sograph.RecalculateExternalTaxesSync;
								try
								{
								...
								{
									sograph.RecalculateExternalTaxesSync = oldRecalculateExternalTaxesSync;
								}
							}
		...
		protected virtual void CreateQTOrderAndLines(SOOrderEntry sograph, RQRequisition requisition, RQRequisitionLine rqLine, InventoryItem inventoryItem, List<SOOrder> newSOOrders)
		{
			...
			if (sograph.Document.Current == null)
			{
				...
				order = PXCache<SOOrder>.CreateCopy(sograph.Document.Insert(order));
				...
				var soExternalTaxExt = sograph.FindImplementation<SOOrderEntryExternalTax>();
				bool oldSkipExternalTaxCalcOnSave = false;
				try
				{
					if (soExternalTaxExt != null)
					{
						oldSkipExternalTaxCalcOnSave = soExternalTaxExt.skipExternalTaxCalcOnSave;
						soExternalTaxExt.skipExternalTaxCalcOnSave = true;
					}
				sograph.Save.Press();
				}
				finally
				{
					if (soExternalTaxExt != null)
					{
						soExternalTaxExt.skipExternalTaxCalcOnSave = oldSkipExternalTaxCalcOnSave;
					}
				}
		...
		public virtual IEnumerable chooseVendor(PXAdapter adapter)
		{
			foreach (RQRequisition item in adapter.Get<RQRequisition>())
			{
					...
					(item.Status == RQRequisitionStatus.Bidding || item.Status == RQRequisitionStatus.Hold))
				{
					...
					upd.RemitAddressID = this.Vendors.Current.RemitAddressID < 0L ? upd.RemitAddressID : this.Vendors.Current.RemitAddressID;
					upd.RemitContactID = this.Vendors.Current.RemitContactID < 0L ? upd.RemitContactID : this.Vendors.Current.RemitContactID;
				...
				foreach (RQBiddingVendor vndr in source ?? Vendors.Select(order.ReqNbr))
				{
					...
					if (currentVendor == true || vndr.Status != true)
					{
						...
						try
						{
							...
							this.GetExtension<RQRequisitionEntry_ActivityDetailsExt>().SendNotification(APNotificationSource.Vendor, notificationCD, order.BranchID, parameters, adapter.MassProcess);
						}
						catch (Exception e)
						{
							if (currentVendor)
		...
		protected virtual void RQRequisition_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			this.validateAddresses.SetEnabled(row.Released == false && row.Cancelled == false && FindAllImplementations<IAddressValidationHelper>().RequiresValidation());
		...
		protected virtual void RQRequisition_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (row.VendorID != newRow.VendorID && PXAccess.FeatureInstalled<FeaturesSet.multicurrency>())
			{
				...
				if (this._overrideVendorCuryOrRate == false && rVendor != null)
				{
					CurrencyInfo copyInfo = PXCache<CurrencyInfo>.CreateCopy(currencyinfo.Select(row.CuryInfoID));
					...
					CurrencyInfo vendorCuryInfo = bidder != null ? currencyinfo.Select(bidder.CuryInfoID) : null;
					bool update = false;

					if(vendorCuryInfo == null)
					{
						newRow.CuryID = vendor.Current.CuryID ?? company.Current.BaseCuryID;

						try
					{
							PXDBCurrencyAttribute.SetBaseCalc<RQRequisitionLine.curyEstUnitCost>(this.Lines.Cache, null, false);
							copyInfo = CurrencyInfoAttribute.SetDefaults<RQRequisition.curyInfoID>(sender, newRow);
					}
						finally
					{
					...
					else if(!currencyinfo.Cache.ObjectsEqual<CurrencyInfo.curyID, CurrencyInfo.curyRateTypeID, CurrencyInfo.curyRate, CurrencyInfo.curyEffDate>(copyInfo, vendorCuryInfo))
					{ 
						var curyInfoID =  copyInfo.CuryInfoID;

						PXCache<CurrencyInfo>.RestoreCopy(copyInfo, vendorCuryInfo);

						copyInfo.CuryInfoID = curyInfoID;
						...
						try
						{
							...
							if (sharedID == copyInfo.CuryInfoID)
							{
								copyInfo.CuryInfoID = null;
								copyInfo = currencyinfo.Insert(copyInfo);

								sender.SetValueExt<RQRequisition.curyInfoID>(newRow, copyInfo.CuryInfoID);
								sender.SetValueExt<RQRequisition.curyID>(newRow, copyInfo.CuryID);
							}
							else
							{
								currencyinfo.Update(copyInfo);

								newRow.CuryID = copyInfo.CuryID;
							}
						}
						finally
						{
					...
					if (update)
					{
						string message = PXUIFieldAttribute.GetError<CurrencyInfo.curyEffDate>(currencyinfo.Cache, copyInfo);
		...
		protected virtual void RQRequisitionLine_Availability_FieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
		{
			...
			INSiteStatusByCostCenter availability = INSiteStatusByCostCenter.PK.Find(this, row.InventoryID, row.SubItemID, row.SiteID, CostCenter.FreeStock);
		...
		public override void CopyPasteGetScript(bool isImportSimple, List<Command> script, List<Container> containers)
		{
			...
			var bidVendorCuryInfoMappingIds = script.SelectIndexesWhere(s => s.ObjectName.StartsWith("_RQBiddingVendor_CurrencyInfo_"));
			foreach (var i in bidVendorCuryInfoMappingIds.Reverse())
			{
				script.RemoveAt(i);
		...
		public class RQRequisitionEntryShippingAddressCachingHelper : AddressValidationExtension<RQRequisitionEntry, POShipAddress>
		{
		...
		public class RQRequisitionEntryRemitAddressCachingHelper : AddressValidationExtension<RQRequisitionEntry, PORemitAddress>
		{
		...
		#region ActivityDetailsExt

		// Acuminator disable once PX1016 ExtensionDoesNotDeclareIsActiveMethod extension should be constantly active
		public class RQRequisitionEntry_ActivityDetailsExt : ActivityDetailsExt<RQRequisitionEntry, RQRequisition, RQRequisition.noteID>
		{
			public override Type GetBAccountIDCommand() => typeof(Select<Vendor, Where<Vendor.bAccountID, Equal<Current<RQBiddingVendor.vendorID>>>>);

			public override Type GetEmailMessageTarget() => typeof(Select2<VendorContact,
