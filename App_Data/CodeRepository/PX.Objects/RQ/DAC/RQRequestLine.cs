...
using PX.Objects.EP;
	...
	public partial class RQRequestLine : PX.Data.IBqlTable
	{
		...
		{
			public static RQRequestLine Find(PXGraph graph, string orderNbr, int? lineNbr, PKFindOptions options = PKFindOptions.None) => FindBy(graph, orderNbr, lineNbr, options);
		}
		...
		[SubAccount(typeof(RQRequestLine.expenseAcctID), typeof(RQRequestLine.branchID), DisplayName = "Sub.", Visibility = PXUIVisibility.Visible, Filterable = true, DescriptionField = typeof(Sub.description), Required = false)]
		[PXDefault(PersistingCheck = PXPersistingCheck.Nothing)]
