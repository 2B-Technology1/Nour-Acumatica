...
namespace PX.Objects.RQ
{
	...
	{
		public class RQBiddingEntryRemitAddressCachingHelper : AddressValidationExtension<RQBiddingEntry, PORemitAddress>
		{
		...
		protected virtual RQRequisitionLineBidding PrepareRQRequisitionLineBiddingInViewDelegate(PXResult<RQRequisitionLineBidding, RQBidding> item)
		{
			...
			if (Lines.Cache.GetStatus(rqLineBidding) != PXEntryStatus.Updated)
			{
				...
				if (bidding.LineID == null)
					...
					Lines.Cache.MarkUpdated(rqLineBidding, assertError: true);
			}
		...
		{
			this.Vendor.Cache.MarkUpdated(this.Vendor.Current, assertError: true);
        ...
        protected virtual void RQBiddingVendor_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			this.validateAddresses.SetEnabled(FindAllImplementations<IAddressValidationHelper>().RequiresValidation());
		}
