            ...
            foreach (RQRequestLine line in lines)
            {
                ...
                if (line.OpenQty > 0)
                {
                    if (!graph.Lines.Cache.IsDirty && req.CuryID != requisition.CuryID)
                    {
						...
						CM.CurrencyInfo info = graph.currencyinfo.Select();
						PXCache<CM.CurrencyInfo>.RestoreCopy(info, CM.CurrencyInfo.PK.Find(graph, req.CuryInfoID));
						info.CuryInfoID = requisition.CuryInfoID;
						requisition.CuryID = info.CuryID;
