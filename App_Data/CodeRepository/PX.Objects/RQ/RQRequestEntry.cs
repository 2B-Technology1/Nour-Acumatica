...
namespace PX.Objects.RQ
{
	public class RQRequestEntry : PXGraph<RQRequestEntry, RQRequest>, IGraphWithInitialization
	{
		...
		public
			...
			InnerJoin<RQRequisitionLineReceived, On<
				RQRequisitionLineReceived.reqNbr, Equal<RQRequisitionContent.reqNbr>,
			...
			Where<
				RQRequisitionContent.orderNbr, Equal<Optional<RQRequestLine.orderNbr>>,
				And<RQRequisitionContent.lineNbr, Equal<Optional<RQRequestLine.lineNbr>>>>>
			Contents;
		...
		*/
		...
		protected virtual void RQRequest_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			OpenPeriodAttribute.SetValidatePeriod<RQRequest.finPeriodID>(sender, null, budgetValidation ? PeriodValidation.DefaultUpdate : PeriodValidation.Nothing);
			PXDefaultAttribute.SetPersistingCheck<RQRequest.departmentID>(sender, row, persistingCheck);
			PXDefaultAttribute.SetPersistingCheck<RQRequest.finPeriodID>(sender, null, persistingCheck);
			...
			PXUIFieldAttribute.SetEnabled<RQRequest.finPeriodID>(sender, row, budgetValidation);
			sender.Adjust<FinPeriodIDAttribute>().For<RQRequest.finPeriodID>(a => a.RedefaultOnDateChanged = budgetValidation);
			...
			this.validateAddresses.SetEnabled(row.Cancelled == false && FindAllImplementations<IAddressValidationHelper>().RequiresValidation());
		...
		protected virtual void RQRequest_ReqClassID_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			if (reqClassCur.RestrictItemList == true)
			{
				...
				{
					this.Lines.Cache.MarkUpdated(line, assertError: true);
		...
		public class RQRequestEntryShippingAddressCachingHelper : AddressValidationExtension<RQRequestEntry, POShipAddress>
		{
		...
		public class RQRequestEntryRemitAddressCachingHelper : AddressValidationExtension<RQRequestEntry, PORemitAddress>
		{
		...
		public class RQRequestEntry_ActivityDetailsExt : ActivityDetailsExt<RQRequestEntry, RQRequest, RQRequest.noteID> { }
	...
	[PXProjection(typeof(
		...
		LeftJoin<INSiteStatusByCostCenter,
			On2<INSiteStatusByCostCenter.FK.InventoryItem,
			...
			if (recalcPricesFilter.AskExt() == WebDialogResult.OK)
			{
				...
				foreach (TDetail line in DetailSelect.Select())
				{
								...
								&& recalcPricesFilter.Current.OverrideManualPrices == false)))
					{
						...
						detailCache.MarkUpdated(line, assertError: true);
					} 
				}
			}
			return adapter.Get();
