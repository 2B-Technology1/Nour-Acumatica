...
namespace PX.Objects.GL
{
	...
	public class SummaryPostingController
	{
		...
		private bool IsSummaryPostingAllowed(GLTran tran)
		{
			...
			{
				var account = Account.PK.Find(_journalEntry, tran.AccountID);
		...
		public void AddSummaryTransaction(GLTran tran)
		{
			...
			if (_summaryTransactionsGroups.TryGetValue(tran, out Dictionary<GLTran, GLTran> summaryTransGroup))
			{
				if (summaryTransGroup.Count == 0) //this mean, we add back previously deleted Summary Tran
				{
					GLTran oldObject = _summaryTransactionsGroups.Keys.Single(_ => _summaryTransactionsGroups.Comparer.Equals(_, tran));
					if (oldObject.LineNbr == tran.LineNbr) //in this case they are same from cache's point of view as well
						tran.TranID = oldObject.TranID;
				}
		...
		private void NormalizeIfNeeded()
		{
			...
			{
				if (newSummaryTransactionsGroups.TryGetValue(summaryTransGroupKvp.Key, out Dictionary<GLTran, GLTran> group))
				{
					group.AddRange(summaryTransGroupKvp.Value);
				}
				else
				{
		...
		public static bool TryAggregateToTran(PXCache sender, GLTran tran, GLTran summ_tran)
		{
			...
			{
				sender.RestoreCopy(sender.Delete(copy_tran), copy_tran);
			}
			else
			{
