...
namespace PX.Objects.GL
{
	...
	public partial class AccountByPeriodFilter : IBqlTable
	{
		...
		[PXSelector(typeof(Search<Ledger.ledgerID, Where<Ledger.balanceType, NotEqual<LedgerBalanceType.budget>>>), 
	...
	[PXProjection(typeof(Select2<GLTran,
		LeftJoin<ADL.Batch, On<GLTran.module, Equal<ADL.Batch.module>,
			And<GLTran.batchNbr, Equal<ADL.Batch.batchNbr>>>>>))]
	...
	public class AccountByPeriodEnq : PXGraph<AccountByPeriodEnq>
	{
		...
		protected virtual PXSelectBase<GLTranR> Command
		{
			get
			{
				...
				PXSelectBase<GLTranR> cmd = PXAccess.FeatureInstalled<FeaturesSet.rowLevelSecurity>()
					? (PXSelectBase<GLTranR>)
					new PXSelectJoin<GLTranR,
						...
						Where<GLTranR.ledgerID, Equal<Current<AccountByPeriodFilter.ledgerID>>,
								And<GLTranR.accountID, Equal<Current<AccountByPeriodFilter.accountID>>>>>(this)
					:
					new PXSelectJoin<GLTranR,
							LeftJoin<ADL.Sub, On<GLTranR.subID, Equal<Sub.subID>>,
							LeftJoin<ADL.Batch, On<GLTranR.module, Equal<ADL.Batch.module>,
									And<GLTranR.batchNbr, Equal<ADL.Batch.batchNbr>>>>>,
						Where<GLTranR.ledgerID, Equal<Current<AccountByPeriodFilter.ledgerID>>,
				...
				{
					cmd.WhereAnd<Where<ADL.Batch.voided, NotEqual<True>,
				...
				{
					cmd.WhereAnd<Where<GLTranR.branchID, InsideBranchesOf<Required<AccountByPeriodFilter.orgBAccountID>>,
				...
				if (filter.ShowSummary == true)
				{
					...
					.Replace<BqlPlaceholder.A>(filter?.UseMasterCalendar == true 
						...
						.ToType();
					cmd.View = new PXView(this, true, cmd.View.BqlSelect.AggregateNew(aggregate));
				}
				return cmd;
			}
		}
		...
		public virtual IEnumerable RetrieveGLTran()
		{
			...
			using (new PXReadBranchRestrictedScope(null, branchIDs) { SpecificBranchTable = typeof(GLTran).Name })
			{
				if (filter.ShowSummary == false)
				{
					...
					{
						records = ExecuteCommandForRetrieveGLTran(isSyncPosition).Select(result =>
						{
							...
							{
								ResetAmounts(result.GetItem<ReclassifyingGLTranAggregate>(), tran);
							}
		...
		private void PrepareDetailRow(ref decimal runningBalance, ref decimal? runningCuryBalance, bool isSyncPosition, List<GLTranR> satisfiedUpdated, GLTranR tran, Account account)
		{
			if (isSyncPosition)
			{
				return;
				...
				if (account != null)
				{
					...
					using (new PXReadBranchRestrictedScope(null, branchIDs) { SpecificBranchTable = typeof(GLTran).Name })
					{
						...
						if (filter.ShowSummary == false)
						{
							if (filter.IncludeReclassified == false)
							{
								turnover = ExecuteCommandForSummary()
									.Sum(result =>
									{
										GLTranR tran = result.GetItem<GLTranR>();
										ReclassifyingGLTranAggregate gLTranAggregate = result.GetItem<ReclassifyingGLTranAggregate>();

										SetOrigAmounts(tran);

										if (tran.Reclassified == true)
										{
											ResetAmounts(gLTranAggregate, tran);
										}
		...
		private IEnumerable<PXResult> ExecuteCommandForRetrieveGLTran(bool isSyncPosition)
		{
			...
			return Command.View.Select(
						currents: null,
						parameters: new object[] { Filter.Current.OrgBAccountID },
				...
				.Cast<PXResult>()
				.Where(_ => GroupHelper.IsAccessibleToUser(Caches[typeof(ADL.Sub)], _.GetItem<ADL.Sub>(), Accessinfo.UserName, true));
		}
		...
		private IEnumerable<PXResult> ExecuteCommandForSummary()
		{
			...
			return Command.View.Select(
								...
								startRow: ref startRow,
								maximumRows: 0,
								totalRows: ref totalRows).Cast<PXResult>();
		}
