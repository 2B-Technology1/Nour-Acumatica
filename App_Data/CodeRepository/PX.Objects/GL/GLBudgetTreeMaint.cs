...
namespace PX.Objects.GL
{
	public class GLBudgetTreeMaint : PXGraph<GLBudgetTreeMaint>
	{
		...
		protected virtual void GLBudgetTree_SubMask_FieldUpdating(PXCache sender, PXFieldUpdatingEventArgs e)
		{
			if (e.NewValue != null && e.Row != null)
			{
				PXSegmentedState segmentedState = (PXSegmentedState)sender.GetStateExt(null, typeof(GLBudgetTree.subID).Name);
				e.NewValue = ((string)e.NewValue).PadRight(segmentedState.Segments.Sum(s => s.Length), '?').Replace(' ', '?');
			}
		}
	...
	public partial class AccountsPreloadFilter : IBqlTable
	{
		...
		[PXDimensionSelector(AccountAttribute.DimensionName, typeof(Search<Account.accountID,
			Where<Account.accountCD, Like<Current<SelectedNode.accountMaskWildcard>>, And<Account.accountingType, Equal<AccountEntityType.gLAccount>>>,
			OrderBy<Asc<Account.accountCD>>>), typeof(Account.accountCD), DescriptionField=typeof(Account.description))]
