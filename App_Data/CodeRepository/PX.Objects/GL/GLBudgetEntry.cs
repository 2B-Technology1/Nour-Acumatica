...
{
	public class SuppressDeletingDialogBoxScope : FlaggedModeScopeBase<SuppressDeletingDialogBoxScope> { }
	
	public class SuppressCascadeDeletingScope : FlaggedModeScopeBase<SuppressDeletingDialogBoxScope> { }
	
	public partial class GLBudgetEntry : PXGraph<GLBudgetEntry>, PXImportAttribute.IPXPrepareItems, PXImportAttribute.IPXProcess
	{
		#region Selects
		...
		private AccountIndex _accountIndex = null;
		private AccountIndex accountIndex => _accountIndex ?? (_accountIndex = new AccountIndex(this));

		private SubIndex _subIndex = null;
		private SubIndex subIndex => _subIndex ?? (_subIndex = new SubIndex(this));

		private LedgerIndex _ledgerIndex = null;
		private LedgerIndex ledgerIndex => _ledgerIndex ?? (_ledgerIndex = new LedgerIndex(this));

		private bool? _needSubDimensionValidate = null;
		private bool needSubDimensionValidate
		{
			get
			{
				if (_needSubDimensionValidate == null)
				{
					PXView subDimensionView = new PXView(
						this,
						true,
						SelectFrom<CS.Dimension>
							.Where<CS.Dimension.dimensionID.IsEqual<SubAccountAttribute.dimensionName>>
							.View
							.ReadOnly
							.GetCommand());
					using (new PXFieldScope(subDimensionView, typeof(CS.Dimension.validate)))
					{
						_needSubDimensionValidate = ((CS.Dimension)subDimensionView.SelectSingle()).Validate;
					}
				}
				return (bool)_needSubDimensionValidate;
			}
		}
		#endregion
		...
		private GLBudgetLine ComparisonRow(GLBudgetLine article, int sortOrder)
		{
			GLBudgetLine compare = new GLBudgetLine
			{
				...
				Description = PXLocalizer.Localize(Messages.BudgetArticleDescrCompared),
		...
		private GLBudgetLine LocateInBudgetArticlesCache(List<GLBudgetLine> existingArticles, GLBudgetLine line, bool IncludeGroups)
		...
		private void CheckBudgetLinesForDuplicates()
		{
			...
			foreach (GLBudgetLine item in FindDuplicates())
					{
				duplicateFound = true;
		...
		{
			Dictionary<GLBudgetLineKey, GLBudgetLine> listCached = new Dictionary<GLBudgetLineKey, GLBudgetLine>();
			List<BudgetKey> budgetsInIndex = new List<BudgetKey>();

			var items = BudgetArticles.Cache.Cached.RowCast<GLBudgetLine>().OrderBy(BudgetArticles.Cache.GetStatus);

			foreach (GLBudgetLine item in items)
			{
				if (item.IsGroup == false &&
					 BudgetArticles.Cache.GetStatus(item).IsNotIn(PXEntryStatus.Deleted, PXEntryStatus.InsertedDeleted) &&
					item.Comparison != true)
				{
					GLBudgetLineKey line = GLBudgetLineKey.Create(item);
					if (!budgetsInIndex.Any(line.Match))
					{
						BudgetKey budgetKey = new BudgetKey(line);
						IEnumerable<GLBudgetLine> gLBudgetLines = budgetKey.SelectLines(this).RowCast<GLBudgetLine>().OrderBy(BudgetArticles.Cache.GetStatus, PXEntryStatus.Held);
						listCached.AddRange(gLBudgetLines.GroupBy(GLBudgetLineKey.Create).ToDictionary(_ => _.Key, _ => _.First()));
						budgetsInIndex.Add(budgetKey);
					}

					if (listCached.TryGetValue(line, out GLBudgetLine existingItem))
					{
					...
					{
						listCached.Add(line, item);
					}
				}
			}
		}

		protected void RemoveDuplicates() => FindDuplicates().ForEach(_ => BudgetArticles.Cache.Delete(_));
		...
		{
			if (!isImportStarted && this.IsDirty)
			{
			...
			string val = values[nameof(GLBudgetLine.AccountID)] as string;
			if (!string.IsNullOrEmpty(val))
				accountID = accountIndex.GetID(val);

			int? subID = subIndex.GetID(values[nameof(GLBudgetLine.SubID)] as string ?? subIndex.DefaultCD);

			if ((accountID != null && (subID != null || !needSubDimensionValidate) && SubEnabled) || (accountID != null && !SubEnabled))
			{
				GLBudgetLine article = ArticleIndex.GetByKey(GLBudgetLineKey.Create(values, this), checkGLBudgetLine);
		...
		public virtual void distributeOK()
		{
			...
			{
				if (!(bool)article.IsGroup && (article.AccountID.HasValue && accountIndex.IsActive(article.AccountID.Value)
					&& article.SubID.HasValue && subIndex.IsActive(article.SubID.Value)))
				{
		...
		private void ConvertBudget()
		{
			...
			List<GLBudgetTree> budgetTree = PXSelect<GLBudgetTree>
				.Select(this)
				.RowCast<GLBudgetTree>()
				.ToList();

			//Check for conflicting lines
			foreach (GLBudgetLine oldArticle in oldArticles)
			{
				foreach (GLBudgetTree treeNode in budgetTree)
				{
					if ((bool)treeNode.IsGroup && treeNode.AccountID != null && treeNode.SubID != null)
					{
						string oldAccountCD = accountIndex.GetCD((int)oldArticle.AccountID);
						string newAccountCD = accountIndex.GetCD((int)treeNode.AccountID); 
						string oldSubCD = subIndex.GetCD((int)oldArticle.SubID);
						string newSubCD = subIndex.GetCD((int)treeNode.SubID);
			...
			List<GLBudgetLine> existingGroups = PXSelect<GLBudgetLine,
				Where<GLBudgetLine.branchID, Equal<Current<BudgetFilter.branchID>>,
				And<GLBudgetLine.ledgerID, Equal<Current<BudgetFilter.ledgerID>>,
				And<GLBudgetLine.finYear, Equal<Current<BudgetFilter.finYear>>>>>>
				.Select(this)
			...
			foreach (GLBudgetLine oldArticle in oldArticles)
			{
				...
				foreach (GLBudgetLine newGroup in existingGroups)
				{
					...
					{
						if (MatchMask(accountIndex.GetCD((int)oldArticle.AccountID), newGroup.AccountMask ?? String.Empty)
							&& MatchMask(subIndex.GetCD((int)oldArticle.SubID), newGroup.SubMask ?? String.Empty))
						{
							groupFound = true;
		...
		protected virtual void DoPreload()
		{
			...
			foreach (GLBudgetLine newArticle in articlesFromHistory)
			{
				...
				foreach (GLBudgetLine existingArticle in articlesNotInCurrentGroup)
				{
					...
					{
						if (MatchMask(accountIndex.GetCD(newArticle.AccountID.Value), existingArticle.AccountMask ?? String.Empty)
							&& MatchMask(subIndex.GetCD(newArticle.SubID.Value), existingArticle.SubMask ?? String.Empty))
						{
							skipArticle = true;
						}
					}
		...
		protected virtual void AllocationFieldSelecting(PXCache sender, PXFieldSelectingEventArgs e, int fieldNbr)
		{
			...
			if (article != null)
			{
				...
				state.Enabled = state.Enabled
					&& article.AccountID.HasValue && accountIndex.IsActive(article.AccountID.Value)
					&& article.SubID.HasValue && subIndex.IsActive(article.SubID.Value);
				e.ReturnValue = val;
			}
		...
		protected virtual void GLBudgetLine_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			{
				PXUIFieldAttribute.SetReadOnly(sender, article, !accountIndex.IsActive(article.AccountID.Value)
					|| !subIndex.IsActive(article.SubID.Value));
			}
		}
		...
		protected virtual void GLBudgetLine_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			GLBudgetLine_VerifyBranchLedgerLink(sender, article);
			...
			{
				string accountCD = accountIndex.GetCD(article.AccountID.Value);
				string subCD = subIndex.GetCD(article.SubID.Value);
		...
		protected virtual void GLBudgetLine_RowDeleting(PXCache sender, PXRowDeletingEventArgs e)
		{
			...
			if (article.GroupID != article.ParentGroupID
				&& SuppressCascadeDeletingScope.IsActive == false
				&& SelectFrom<GLBudgetLine>
				.Where<GLBudgetLine.parentGroupID.IsEqual<@P.AsGuid>>.View.Select(this, article.GroupID).Any())
			{
				if (SuppressDeletingDialogBoxScope.IsActive || BudgetArticles.Ask(Messages.BudgetTreeDeleteGroupTitle, Messages.BudgetTreeDeleteGroupMessage, MessageButtons.YesNo) == WebDialogResult.Yes)
				{
					deleteChildBudgetRecords(article.GroupID);
				}
				else
				{
					e.Cancel = true;
		...
		{
			foreach (GLBudgetLine item in SelectFrom<GLBudgetLine>
				.Where<GLBudgetLine.parentGroupID.IsEqual<@P.AsGuid>>.View.Select(this, groupID))
			{
				if (item.IsGroup == true)
				{
					deleteChildBudgetRecords(item.GroupID);
				}
				BudgetArticles.Cache.Delete(item);
			}
		}
			...
			Ledger ledger = PXSelectJoin<Ledger,
						LeftJoin<OrganizationLedgerLink,
							...
							Where<Ledger.balanceType, Equal<LedgerBalanceType.budget>,
								And<Branch.branchID, Equal<Required<BudgetFilter.branchID>>,
								And<Ledger.ledgerID, Equal<Required<Ledger.ledgerID>>>>>>.Select(this,
								budgetLine.BranchID,
								budgetLine.LedgerID
								);
			if (ledger == null) throw new PXSetPropertyException(Messages.BranchHasNotBeenAssociatedWithLedger, PXAccess.GetBranchCD(budgetLine.BranchID), budgetLine.LedgerID);
		}
		...
		protected virtual void GLBudgetLine_SubID_FieldVerifying(PXCache cache, PXFieldVerifyingEventArgs e)
		{
			...
			{
				subCD = subIndex.GetCD((int)e.NewValue);
				}
		...
		protected class GLBudgetLineIdx {
			...
			public bool Add(GLBudgetLine line) {
				...
				if (keys != null)
				{
					...
					GLBudgetLineKey key =GLBudgetLineKey.Create(line);
			...
			public int Add(IEnumerable<PXResult<GLBudgetLine>> lines) => Add(lines.RowCast<GLBudgetLine>());

			public int Add(PXResultset<GLBudgetLine> lines) => Add(lines.RowCast<GLBudgetLine>());
			...
			public bool Delete(Guid? GroupID)
			{
				...
				if (keys != null)
				{
					all.TryGetValue(id, out GLBudgetLine line);
					if (line!=null)
					{
						GLBudgetLineKey key = GLBudgetLineKey.Create(line);
						if (keys.TryGetValue(key, out Dictionary<Guid, GLBudgetLine> dkey))
						{
							dkey.Remove(id);
							if (dkey.Count == 0)
							{
			...
			public GLBudgetLine GetGroup(Guid? GroupID, checkGLBudgetLine check = null)
			{
				...
				groups.TryGetValue(GroupID.Value, out GLBudgetLine line);
			...
			{
				if (GroupID == null) return null;
				all.TryGetValue(GroupID.Value, out GLBudgetLine line);
				if (line != null && check != null)
					return check(line);
			...
			public GLBudgetLine GetByKey(GLBudgetLine keySource, checkGLBudgetLine check = null)
				{
				GLBudgetLineKey key = GLBudgetLineKey.Create(keySource);
			...
			public GLBudgetLine GetByKey(GLBudgetLineKey key, checkGLBudgetLine check = null)
			{
				...
				if (keys.TryGetValue(key, out Dictionary<Guid, GLBudgetLine> lines))
				{
					GLBudgetLine line = lines.Values.FirstOrDefault();
					if (line != null && check != null)
						return check(line);
					return line;
				}
			...
			{
				if (keys == null) return null;
				keys.TryGetValue(key, out Dictionary<Guid, GLBudgetLine> line);
				return line.Values;
			}
		}
		...
		{
			BudgetFilter filter = Filter.Current;
			if (filter.BranchID != null && filter.FinYear != null && filter.LedgerID != null)
			{
	...
	public partial class BudgetPreloadFilter : IBqlTable
	{
		...
		[PXDBInt]
		[PXDimensionSelector(AccountAttribute.DimensionName, typeof(Search<Account.accountID,
			Where<Account.accountCD, Like<Current<SelectedGroup.accountMaskWildcard>>, And<Account.accountingType, Equal<AccountEntityType.gLAccount>>>,
			OrderBy<Asc<Account.accountCD>>>),
			typeof(Account.accountCD), DescriptionField = typeof(Account.description))]
