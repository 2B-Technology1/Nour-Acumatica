...
namespace PX.Objects.GL
{
	...
	public class GeneralLedgerMaint : PXGraph<GeneralLedgerMaint, Ledger>
	{
		...
		public class OrganizationLedgerLinkMaint : OrganizationLedgerLinkMaintBase<GeneralLedgerMaint, Ledger>
		{
			...
			protected override void OrganizationLedgerLink_RowInserting(PXCache cache, PXRowInsertingEventArgs e)
			{
				var link = e.Row as OrganizationLedgerLink;
				if (link != null && !PXAccess.FeatureInstalled<FeaturesSet.multipleBaseCurrencies>()
					&& Base.LedgerRecords.Current != null && Base.LedgerRecords.Current.BalanceType == LedgerBalanceType.Actual
					&& Base.LedgerRecords.Current.BaseCuryID == null)
				{
					Organization org = GetUpdatingOrganization(link.OrganizationID);
					// Acuminator disable once PX1048 RowChangesInEventHandlersAllowedForArgsOnly [Required to prevent possible null base currency - fix under AC-275685 ]
		...
		{
			// Acuminator disable once PX1085 DatabaseQueriesInPXGraphInitialization [Use of PXSetup wich uses context to prevent unnecessary db calls]
			Organization organizationSetup = PXSetup<Organization>.Select(this);
			// Acuminator disable once PX1085 DatabaseQueriesInPXGraphInitialization [Use of PXSetup wich uses context to prevent unnecessary db calls]
			Ledger ledgerSetup = PXSetup<Ledger>.Where<Ledger.balanceType.IsEqual<LedgerBalanceType.actual>>.Select(this);
			if (organizationSetup == null && company.Current.BaseCuryID == null && ledgerSetup == null)
			{
