	...
	public class MasterFinPeriodMaint : PXGraph<MasterFinPeriodMaint, MasterFinYear>, IFinPeriodMaintenanceGraph
	{
		...
		public virtual MasterFinYear GeneratePreviousMasterFinYear()
		{
			...
			using (new YearsGenerationScope())
			{
				...
				MasterFinYear nextYear = PXSelect<
					MasterFinYear,
					Where<MasterFinYear.year, Greater<Required<MasterFinYear.year>>>,
					OrderBy<
						Asc<MasterFinYear.year>>>
					.SelectWindowed(this, 0, 1, insertedYear.Year);

				if (insertedYear.EndDate != nextYear.StartDate)
					throw new PXSetPropertyException(Messages.CalendarCanNotBeAdded, insertedYear.Year);
		...
		protected virtual void _(Events.CacheAttached<OrganizationFinYear.organizationID> e) { }
		...
		{
			if (IsMasterFinPeriodReferencedInGL(finPeriodID))
			{
				throw new PXException(Messages.FinancialPeriodCanNotBeDeleted, PeriodIDAttribute.FormatForError(finPeriodID));
			}
		}

		protected virtual void CheckMasterFinPeriodReferencedInFA(string finPeriodID)
		{
			if (IsMasterFinPeriodReferencedInFA(finPeriodID))
			{
				throw new PXException(Messages.FinancialPeriodCanNotBeDeletedFA, PeriodIDAttribute.FormatForError(finPeriodID));
			}
		}

		protected virtual void CheckMasterFinPeriodReferenced(string finPeriodID)
		{
			CheckMasterFinPeriodReferencedInGL(finPeriodID);
			CheckMasterFinPeriodReferencedInFA(finPeriodID);
		}
		...
		protected virtual bool IsMasterFinPeriodReferencedInFA(string finPeriodID)
		{
			using (new PXConnectionScope())
			{
				return SelectFrom<FATran>
					...
					.InnerJoin<Branch>
						.On<FATran.branchID.IsEqual<Branch.branchID>
							.Or<FATran.srcBranchID.IsEqual<Branch.branchID>>>
					.InnerJoin<OrganizationFinPeriod>
						.On<Branch.organizationID.IsEqual<OrganizationFinPeriod.organizationID>
							.And<FATran.finPeriodID.IsEqual<OrganizationFinPeriod.finPeriodID>>>
					.InnerJoin<MasterFinPeriod>
						.On<OrganizationFinPeriod.masterFinPeriodID.IsEqual<MasterFinPeriod.finPeriodID>>
					.Where<MasterFinPeriod.finPeriodID.IsEqual<@P.AsString>>
					.View
					.ReadOnly
					.Select(this, finPeriodID)
					.Any(); // Use Linq to Select TOP 1 without ORDER BY
			}
		}
		...
		protected virtual void MasterFinYear_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			Periods.Cache.AllowInsert = (year.CustomPeriods == true) && nextYear == null;
		...
		protected virtual void _(Events.CacheAttached<OrganizationFinPeriod.organizationID> e) { }
		...
		protected virtual void MasterFinPeriod_IsAdjustment_FieldUpdating(PXCache sender, PXFieldUpdatingEventArgs e)
		{
			...
			{
				// StartDate is temporarily set to null to prevent Cache Update from corrupting the record.
				// Logic in MasterFinPeriod.StartDateUI getter and setter yields different results depending on the current value of _EndDate field.
				// That makes MasterFinPeriod, and other FinPeriod records sensible to order in which their properties are set.
		...
		protected virtual void MasterFinPeriod_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			bool isLastOpenPeriod = customPeriodsEnabled && nextYear == null &&
				PXSelect<
					MasterFinPeriod,
			...
			bool isPeriodReferencedInFA = SelectFrom<FATran>
					.InnerJoin<FABook>
						.On<FABook.bookID.IsEqual<FATran.bookID>
							.And<FABook.updateGL.IsEqual<True>>>
					.InnerJoin<Branch>
					...
					.InnerJoin<MasterFinPeriod>
						.On<OrganizationFinPeriod.masterFinPeriodID.IsEqual<MasterFinPeriod.finPeriodID>>
					.Where<MasterFinPeriod.finPeriodID.IsGreaterEqual<@P.AsString>>
					.View
					.ReadOnly
					.Select(this, masterFinPeriod.FinPeriodID)
					.Any(); // Use Linq to Select TOP 1 without ORDER BY;

			PXUIFieldAttribute.SetEnabled<MasterFinPeriod.endDateUI>(cache, masterFinPeriod, isLastOpenPeriod && !isPeriodReferencedInFA);
		...
		protected virtual void MasterFinPeriod_RowDeleting(PXCache cache, PXRowDeletingEventArgs e)
		{
			...
			if (isIntialized)
			{
				...
				CheckMasterFinPeriodReferenced(period.FinPeriodID);
			}

			GLBudgetLine glBudgetLine = new SelectFrom<GLBudgetLine>
								.LeftJoin<GLBudgetLineDetail>
									.On<GLBudgetLineDetail.ledgerID.IsEqual<GLBudgetLine.ledgerID>
										.And<GLBudgetLineDetail.branchID.IsEqual<GLBudgetLine.branchID>
										.And<GLBudgetLineDetail.finYear.IsEqual<GLBudgetLine.finYear>>>>
								.Where<GLBudgetLineDetail.finPeriodID.IsEqual<@P.AsString>>
								.View(this).SelectSingle(period.FinPeriodID);

			if (glBudgetLine != null)
			{
				Branch branch = Branch.PK.Find(this, glBudgetLine.BranchID);
				Ledger ledger = Ledger.PK.Find(this, glBudgetLine.LedgerID);
				throw new PXException(Messages.FinPeriodExistsinBudget, PeriodIDAttribute.FormatForError(period.FinPeriodID), ledger.LedgerCD, branch.BranchCD);
			}
		}
		...
		protected virtual void MasterFinPeriod_RowPersisting(PXCache cache, PXRowPersistingEventArgs e)
		{
			...
			{
				CheckMasterFinPeriodReferenced(fp.FinPeriodID);
			}
		}
		...
		public void SynchronizeMasterAndOrganizationPeriods()
		{
			...
			if (PXAccess.FeatureInstalled<FeaturesSet.multipleCalendarsSupport>())
			{
				...
				{
					CheckMasterFinPeriodReferencedInGL(deletedMasterPeriod.FinPeriodID);
				...
				if (year != null) // not deleting
				{
					if (Periods.Cache.IsInsertedUpdatedDeleted)
					{
						if (!PXLongOperation.IsLongOperationContext() &&
							IsYearPeriodsNotMatch(year.Year))
						{
							Periods.Ask(Messages.FiscalYearCustomPeriodsMessageTitle, Messages.FinPeriodsChange, MessageButtons.OK, MessageIcon.Warning);
						}
					}
