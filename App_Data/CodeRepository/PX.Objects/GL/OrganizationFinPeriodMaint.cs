...
namespace PX.Objects.GL
{
	public class OrganizationFinPeriodMaint : PXGraph<OrganizationFinPeriodMaint>, IFinPeriodMaintenanceGraph
	{
		...
		{
		    OrgFinYear.Cache.AllowInsert = false;
		...
		{
			// Instead removed OrgFinYear.AllowUpdate = false (AC-253378)
			return string.Compare(viewName, nameof(OrgFinYear)) == 0
				? 0
				: base.ExecuteUpdate(viewName, keys, values, parameters);
		}
		...
		protected virtual void VerifyOrganizationFinYearForDelete(OrganizationFinYear organizationFinYear)
		{
			...
			{
				OrganizationFinPeriod firstPeriodWithFATran = SelectFrom<OrganizationFinPeriod>
					.InnerJoin<FATran>
						.On<FATran.finPeriodID.IsEqual<OrganizationFinPeriod.finPeriodID>>
					.InnerJoin<Branch>
						.On<Branch.organizationID.IsEqual<OrganizationFinPeriod.organizationID>
							.And<Branch.branchID.IsEqual<FATran.branchID>
								.Or<Branch.branchID.IsEqual<FATran.srcBranchID>>>>
					.InnerJoin<FABook>
						.On<FABook.bookID.IsEqual<FATran.bookID>
							.And<FABook.updateGL.IsEqual<True>>>
					.Where<OrganizationFinPeriod.organizationID.IsEqual<@P.AsInt>
						.And<OrganizationFinPeriod.finYear.IsEqual<@P.AsString>>>
				...
				if (firstPeriodWithFATran != null)
				{
					throw new PXException(
						Messages.FinancialPeriodCanNotBeDeletedFA,
						PeriodIDAttribute.FormatForError(firstPeriodWithFATran.FinPeriodID));
				}
			}
		}
		...
		protected virtual OrganizationFinYear GenerateSingleOrganizationFinYear(int organizationID, MasterFinYear startMasterYear, MasterFinPeriod startMasterFinPeriod)
		{
			...
			bool isCustomPeriods = startMasterYear.CustomPeriods == true || YearSetup.Current.PeriodType == FinPeriodType.CustomPeriodsNumber;
			bool hasAdjustmentPeriod = (!isCustomPeriods && YearSetup.Current.HasAdjustmentPeriod == true)
				|| (isCustomPeriods && FinPeriodRepository.GetAdjustmentFinPeriods(startMasterYear.Year, FinPeriod.organizationID.MasterValue).Any());
		...
		protected virtual OrganizationFinYear GenerateNextOrganizationFinYear(OrganizationFinYear year)
		{
			...
			if ((masterFinYear.CustomPeriods == false && YearSetup.Current.HasAdjustmentPeriod == true)
				|| (masterFinYear.CustomPeriods == true && FinPeriodRepository.GetAdjustmentFinPeriods(masterFinYear.Year, FinPeriod.organizationID.MasterValue).Any()))
			{
