...
namespace PX.Objects.GL
{
		...
		protected virtual void TYearSetupOnRowPersisting(PXCache cache, PXRowPersistingEventArgs e)
		{
			if (PXDBOperation.Insert == e.Operation || PXDBOperation.Update == e.Operation)
			{
				...
				if (FiscalPeriodSetupCreator.IsFixedLengthPeriod(fes.FPType) == false)
				{
					...
					TPeriodSetup previousPeriod = null;
					foreach (TPeriodSetup fps in this.Periods.Select())
					{
						...
						{
							if (previousPeriod != null && fps.EndDateUI.Value < previousPeriod.EndDateUI.Value)
							{
								this.Periods.Cache.RaiseExceptionHandling<PeriodSetup.endDateUI>(fps, fps.EndDateUI, new PXSetPropertyException(Messages.EndDateMustNotBeEarlierStartDateOfAdjustmentPeriod, PXErrorLevel.Error, previousPeriod.EndDateUI.Value.ToShortDateString()));
							}
						}
						else if (previousPeriod != null && fps.EndDateUI.Value == previousPeriod.EndDateUI.Value)
						{
							this.Periods.Cache.RaiseExceptionHandling<PeriodSetup.endDateUI>(fps, fps.EndDateUI, new PXSetPropertyException(Messages.FiscalPeriodAdjustmentPeriodError, PXErrorLevel.Error));
						}
						else if (previousPeriod != null && fps.EndDateUI.Value < previousPeriod.EndDateUI.Value)
						{
							this.Periods.Cache.RaiseExceptionHandling<PeriodSetup.endDateUI>(fps, fps.EndDateUI, new PXSetPropertyException(Messages.EndDateMustNotBeEarlierStartDateOfRegularPeriod, PXErrorLevel.Error));
						}
						previousPeriod = fps;
					}
					if (fes.FinPeriods != count)
				...
				if (oldRow.EndDate.Value != newRow.EndDate.Value)
				{
					...
					foreach (TPeriodSetup fps in this.Periods.Select())
					{
						if (fps.PeriodNbr == key)
						{
							...
							this.Periods.Cache.MarkUpdated(fps);
		...
		where TPeriodSetup : class, IPeriodSetup, new()
	{
		...
		public FiscalPeriodSetupCreator(string aFiscalYear, DateTime aPeriodsStartDate, FPType aPeriodType, bool aHasAdjPeriod, short? aPeriodLength, DateTime aYearStartDate, string aEndYearCalcMethod, int? aEndYearDayOfWeek)
		{
		...
		public virtual bool fillNextPeriod(TPeriodSetup result, TPeriodSetup current)
		{
			...
			if (current != null)
				...
				result.StartDate = this.periodsStartDate;
			periodNbr++;
