...
namespace PX.Objects.GL
{
	...
	public partial class GLBudgetLine : IBqlTable
	{
		...
		{
			public static GLBudgetLine Find(PXGraph graph, int? branchID, int? ledgerID, Guid? groupID, Guid? parentGroupID, string finYear, PKFindOptions options = PKFindOptions.None) => FindBy(graph, branchID, ledgerID, groupID, parentGroupID, finYear, options);
		}
		...
		[Account(null, 
			typeof(Search<Account.accountID,
				Where<Account.accountCD, Like<Current<SelectedGroup.accountMaskWildcard>>>,
				OrderBy<Asc<Account.accountCD>>>))]
		[PXRestrictor(typeof(Where<Account.active, Equal<True>>), Messages.AccountInactive, ReplaceInherited = true)]
		[PXDefault]
