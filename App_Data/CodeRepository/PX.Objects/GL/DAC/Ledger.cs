...
namespace PX.Objects.GL
{
    ...
    /// (which corresponds to the <see cref="GeneralLedgerMaint"/> graph).
    /// </summary>
	[PXPrimaryGraph(typeof(GeneralLedgerMaint))]
	[System.SerializableAttribute()]
    [PXCacheName(Messages.Ledger, CacheGlobal = true)]
	public partial class Ledger : PX.Data.IBqlTable
	{
		#region Keys
		public class PK : PrimaryKeyOf<Ledger>.By<ledgerID>
		{
			public static Ledger Find(PXGraph graph, Int32? ledgerID) => FindBy(graph, ledgerID, (ledgerID ?? 0) <= 0);
		}
		public class UK : PrimaryKeyOf<Ledger>.By<ledgerCD>
		{
			public static Ledger Find(PXGraph graph, string ledgerCD, PKFindOptions options = PKFindOptions.None) => FindBy(graph, ledgerCD, options);
		}
