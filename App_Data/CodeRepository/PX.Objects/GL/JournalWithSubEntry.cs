...
namespace PX.Objects.GL
{
	...
	public partial class JournalWithSubEntry : PXGraph<JournalWithSubEntry, GLDocBatch>, PXImportAttribute.IPXPrepareItems
	{
		...
		public JournalWithSubEntry()
		{
			...
			OpenPeriodAttribute.SetValidatePeriod<GLDocBatch.finPeriodID>(BatchModule.Cache, null, PeriodValidation.DefaultSelectUpdate);
		...
		[PXCopyPasteHiddenView]
		public PXSelect<GLTranCode, Where<GLTranCode.tranCode, Equal<Required<GLTranDoc.tranCode>>>> GLTranCode;
		...
		public PXSelectJoin<CashAccount, LeftJoin<CashAccountETDetail, On<CashAccountETDetail.cashAccountID, Equal<CashAccount.cashAccountID>,
											...
											Or<CashAccountETDetail.cashAccountID, IsNotNull>>>>>> cashAccountByAccountID;
		...
		protected virtual void _(Events.FieldUpdated<GLTranDoc, GLTranDoc.termsID> e)
		{
			Terms terms = (Terms)PXSelectorAttribute.Select<APInvoice.termsID>(e.Cache, e.Row);
			GLTranDoc row = (GLTranDoc)e.Row;
			bool isDocumentDebitAdjustment = row.DocType == APInvoiceType.DebitAdj;
			if (isDocumentDebitAdjustment && terms == null)
			{
				row.CuryDiscAmt = 0m;
				row.DiscDate = null;
				row.DueDate = null;
		...
		private CashAccountETDetail GetCashAccountETDetail(string entryTypeID, int? cashAccountID)
		{
			return PXSelect<CashAccountETDetail,
						...
						And<CashAccountETDetail.cashAccountID, Equal<Required<CashAccountETDetail.cashAccountID>>>>>.Select(this, entryTypeID, cashAccountID);
		}
		...
		protected virtual void GLTranDoc_ExtRefNbr_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			var row = (GLTranDoc)e.Row;
			var tranCode = GLTranCode.SelectSingle(row.TranCode);

			if (tranCode?.Module == GL.BatchModule.GL && tranCode?.TranType == GLTranType.GLEntry && ((e.NewValue ?? string.Empty) as string).Length > 15)
			{
				throw new PXSetPropertyException(Messages.ExtRefNumberLengthCannotExceed15Chars);
			}
		}
		...
		protected virtual void GLTranDoc_BAccountID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			bool isDocumentDebitAdjustment = ((GLTranDoc)e.Row).DocType == APInvoiceType.DebitAdj;
			bool isTermsInDebitAdjustments = ((APSetup)apsetup.Cache.Current).TermsInDebitAdjustments == true;

			if(!isDocumentDebitAdjustment || (isDocumentDebitAdjustment && isTermsInDebitAdjustments))
			{
		...
		protected virtual void GLTranDoc_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (row != null)
			{
				...
				bool isDocumentDebitAdjustment = row.DocType == APInvoiceType.DebitAdj;
				if (isDocumentDebitAdjustment)
				{
					bool isTermsInDebitAdjustments = apsetup.Current.TermsInDebitAdjustments == true;
					PXUIFieldAttribute.SetRequired<APInvoice.dueDate>(sender, isTermsInDebitAdjustments && row.TermsID != null);
					PXUIFieldAttribute.SetRequired<APInvoice.discDate>(sender, isTermsInDebitAdjustments && row.TermsID != null);

					PXUIFieldAttribute.SetEnabled<GLTranDoc.termsID>(sender, row,
						(isTermsInDebitAdjustments && row.Released == false));
					PXUIFieldAttribute.SetEnabled<GLTranDoc.dueDate>(sender, row, row.TermsID != null);
					PXUIFieldAttribute.SetEnabled<GLTranDoc.discDate>(sender, row, row.TermsID != null);

					Terms terms = (Terms)PXSelectorAttribute.Select<GLTranDoc.termsID>(sender, row);
					bool termsMultiple = terms?.InstallmentType == TermsInstallmentType.Multiple;
					PXUIFieldAttribute.SetEnabled<GLTranDoc.curyDiscAmt>(sender, row,
						!termsMultiple && row.TermsID != null && row.Released == false);
				}
			}
		...
		protected virtual void GLTranDoc_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if (e.Operation == PXDBOperation.Insert || e.Operation == PXDBOperation.Update)
			{
				...
				if (row.IsChildTran == false)
				{
					...
					validateTerms = (IsAPInvoice(row) || IsARInvoice(row)) && !IsARCreditMemo(row) && row.DocType != APInvoiceType.DebitAdj;
				}
	...
	public class GLBatchDocRelease : PXGraph<GLBatchDocRelease>
	{
		...
		protected virtual PXGraph CreateAP(GLTranDoc doc, List<GLTranDoc> aList, List<Batch> toPost)
		{
			...
			if (result == null && JournalWithSubEntry.IsAPInvoice(doc))
			{
				...
				if (copy.DocType == APDocType.CreditAdj)
				{
					SetCreditAdjustmentRequiredInfo(copy, doc);
				}
		...
		protected void SetCreditAdjustmentRequiredInfo(APInvoice doc, GLTranDoc voucherDetail)
		{
