...
namespace PX.Objects.GL
{
    ...
    public class PeriodIDAttribute : PXAggregateAttribute, IPXCommandPreparingSubscriber, IPXRowSelectingSubscriber
    {
		...
		public class PeriodResult
		{
			...
			public string PeriodIDForDisplay => PeriodID?.Length == 6 ? FinPeriodIDFormattingAttribute.FormatForDisplay(PeriodID) : PeriodID;
        ...
        protected virtual void SetPeriodIDBySources(PXCache cache, object row)
        {
            ...
            {
				DateTime? sourceDate = GetSourceDate(cache, row);
                ...
                try
                {
                    ...
                    if (pendingValue != null && pendingValue != PXCache.NotSetValue)
                    {
                        ...
                        if (!cache.Graph.UnattendedMode)
                        {
                            ...
                            {
                                cache.SetValuePending(row, _FieldName, result.PeriodIDForDisplay);
                            }
                        }
                ...
                finally
                {
                    ...
                    {
                        cache.SetDefaultExt(row, _FieldName, result.PeriodIDForDisplay);
                    }
                }
            }
            else
		...
		protected virtual PeriodResult GetPeriod(PXGraph graph,
			Type searchType,
			DateTime? date,
			OrganizationDependedPeriodKey periodKey,
			List<object> currents = null)
		{
			FinPeriod finPeriod = graph.GetService<IFinPeriodRepository>().FindFinPeriodByDate(date, periodKey?.OrganizationID);
			return new PeriodResult(finPeriod);
		}
	...
	public class OpenPeriodAttribute : FinPeriodSelectorAttribute, IPXFieldSelectingSubscriber, IPXRowSelectedSubscriber, IPXFieldUpdatedSubscriber
    {
		...
		protected virtual int? GetSuitableFinPeriodCountInCompanies(PXGraph graph, int?[] organizationIDs, string masterFinPeriodID)
		{
			PXView view = new PXView(graph, true, GetSuitableFinPeriodCountInCompaniesCmd);

			List<object> data = view.SelectMulti(organizationIDs, masterFinPeriodID);
		...
		protected virtual BqlCommand GenerateGetSuitableFinPeriodCountInCompaniesCmd(Type queryType)
		{
			...
			cmd = cmd.AggregateNew<Aggregate<GroupBy<FinPeriod.masterFinPeriodID,
		...
		public override void CacheAttached(PXCache sender)
		{
			...
			if (SourceFieldType != null)
			{
				Type itemType = BqlCommand.GetItemType(SourceFieldType);

				sender.Graph.FieldVerifying.AddHandler(
						itemType,
						SourceFieldType.Name,
						PeriodSourceDateFieldVerifying);

				sender.Graph.FieldUpdating.AddHandler(
						itemType,
						SourceFieldType.Name,
						(s, e) => dateSourceFieldAlreadyVerified = false
						);
		...
		{
			if (ValidatePeriod == PeriodValidation.Nothing || e.Row == null) return;

			DateTime? newDateValue = e.NewValue as DateTime?;

			if (newDateValue != null)
			{
				FinPeriod period = cache.Graph.GetService<IFinPeriodRepository>()
										.FindFinPeriodByDate(newDateValue, FinPeriod.organizationID.MasterValue);
				if (period == null)
				{
					throw new PXSetPropertyException(Messages.CorrespondingFinPeriodsDoseNotExist, newDateValue.Value.ToShortDateString());
				}

				PXFieldState filedState = cache.GetStateExt(e.Row, SourceFieldType.Name) as PXFieldState;
				if (filedState != null && filedState.ErrorLevel != PXErrorLevel.Warning)
					cache.RaiseExceptionHandling(SourceFieldType.Name, e.Row, newDateValue, null);
			}
		}
		...
		protected virtual void IsCurrentPeriod(PXCache sender, object row, object value)
		{
			...
			if (_ValidatePeriod != PeriodValidation.Nothing && _sourceDate != DateTime.MinValue)
			{
				...
				string finPeriodIDFromDate = GetPeriod(
					...
					_sourceDate,
					new FinPeriod.Key {OrganizationID = calendarOrganizationID },
					row.SingleToListOrNull()).PeriodID;
		...
		private bool dateSourceFieldAlreadyVerified = false;
		...
		public virtual void Document_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			{
				RiseFieldVerifyingForDocument_RowSelected(sender, e);
		...
		{
			if (SourceFieldType == null || dateSourceFieldAlreadyVerified) return;

			PXFieldState state =  sender.GetStateExt(e.Row, SourceFieldType.Name) as PXFieldState;

			if (state != null && state.Value != null && state.ErrorLevel == PXErrorLevel.Undefined)
			{
				object value = state.Value;
				try
				{
					sender.RaiseFieldVerifying(SourceFieldType.Name, e.Row, ref value);
				}
				catch (PXSetPropertyException ex)
				{					
					sender.RaiseExceptionHandling(SourceFieldType.Name, e.Row, value, ex);
				}
				dateSourceFieldAlreadyVerified = true;
			}
		}
		...
		protected virtual void ValidateFinPeriodID(PXCache sender, object row, string finPeriodID)
		{
			...
			if (CommonServiceLocator.ServiceLocator.IsLocationProviderSet)
			{
				...
				else
				{
					if (period == null)
					{
						throw new PXSetPropertyException(Messages.FinPeriodDoesNotExistForCompany, FormatForError(finPeriodID), PXAccess.GetOrganizationCD(calendarOrganizationID));
					}
					masterFinPeriodID = period.MasterFinPeriodID;
				}

				ValidateFinPeriodsStatus(sender, row, calendarOrganizationID, masterFinPeriodID, finPeriodID, CalendarOrganizationIDProvider);
			}
		}
		...
		protected virtual void ValidateFinPeriodsStatus(PXCache sender, object row, int? calendarOrganizationID, string masterFinPeriodID, string finPeriodID, ICalendarOrganizationIDProvider calendarOrganizationIDProvider)
		{
			CalendarOrganizationIDProvider.PeriodKeyWithSourceValuesCollection keyWithSourceValues =
				calendarOrganizationIDProvider.GetKeysWithBasisOrganizationIDs(sender.Graph, sender, row);
			...
			if (SelectionModeWithRestrictions == SelectionModesWithRestrictions.All
				|| SelectionModeWithRestrictions == SelectionModesWithRestrictions.Undefined
					&& calendarOrganizationID != FinPeriod.organizationID.MasterValue)
			{
				ValidateFinPeriodsStatusForAll(sender, row, calendarOrganizationID, masterFinPeriodID, finPeriodID, organizationIDs);
			}
			else
			{
				ValidateFinPeriodsStatusForAny(sender, row, masterFinPeriodID, organizationIDs);
			}
		}

		private void ValidateFinPeriodsStatusForAny(PXCache sender, object row, string masterFinPeriodID, int?[] organizationIDs)
		{
			int? availablePeriodCount;

			int? openPeriodCount = GetSuitableFinPeriodCountInCompanies(sender.Graph, organizationIDs, masterFinPeriodID);
			...
			if (periodUtils.CanPostToClosedPeriod())
			{
				availablePeriodCount = PXSelectGroupBy<OrganizationFinPeriod,
												Where<OrganizationFinPeriod.status, NotEqual<FinPeriod.status.locked>,
													...
													And<OrganizationFinPeriod.masterFinPeriodID, Equal<Required<OrganizationFinPeriod.masterFinPeriodID>>>>>>,
												Aggregate<GroupBy<OrganizationFinPeriod.masterFinPeriodID,
															Count<OrganizationFinPeriod.organizationID>>>>
												.Select(sender.Graph, organizationIDs, masterFinPeriodID)
												.RowCount;
			...
			{
				throw new PXSetPropertyException(Messages.FinPeriodCannotBeUsedForProcessing, FormatForError(masterFinPeriodID));
			}
			...
			{
				validationResult.AddMessage(PXErrorLevel.Warning, Messages.FinPeriodCannotBeUsedForProcessingForAtLeastOneCompany, FormatForError(masterFinPeriodID));
			}

			if (availablePeriodCount != openPeriodCount 
			    && openPeriodCount != organizationIDs.Length)
			{
				validationResult.AddMessage(PXErrorLevel.Warning, Messages.FinPeriodIsClosedInAtLeastOneCompany, FormatForError(masterFinPeriodID));
		}
		...
		private void ValidateFinPeriodsStatusForAll(PXCache sender, object row, string finPeriodID, int?[] organizationIDs)
		{
			...
			List<FinPeriod> finPeriods = new List<FinPeriod>();
			List<int?> unexistingForOrganizationIDs = new List<int?>();

			foreach (int? organizationID in organizationIDs)
			{
				FinPeriod finPeriod = finPeriodRepository.FindByID(organizationID, finPeriodID);
				if (finPeriod == null) unexistingForOrganizationIDs.Add(organizationID);
				else finPeriods.Add(finPeriod);
			}

			PeriodValidationResult validationResult = new PeriodValidationResult();

			if (unexistingForOrganizationIDs.Any())
			{
			...
			if (validationResult.HasWarningOrError)
			{
				...
				if (validationResult.MaxErrorLevel <= PXErrorLevel.Warning || sender.GetStatus(row) == PXEntryStatus.Notchanged)
				{
		...
		private void ValidateFinPeriodsStatusForAll(PXCache sender, object row, int? calendarOrganizationID, string masterFinPeriodID, string finPeriodID, int?[] organizationIDs)
		{
			List<FinPeriod> finPeriods =
				PXSelect<FinPeriod,
					...
					.Select(sender.Graph, masterFinPeriodID, organizationIDs)
					.RowCast<FinPeriod>()
					.ToList();
			...
			if (finPeriods.Count != organizationIDs.Length)
			{
				...
				validationResult.AddMessage(
					PXErrorLevel.Error,
					ExceptionType.Custom,
					Messages.FinPeriodCorrespondingToPeriodInCompanyCalendarDoesNotExistForCompanies,
					FinPeriodIDFormattingAttribute.FormatForError(finPeriodID),
					PXAccess.GetOrganizationCD(calendarOrganizationID),
					unexistingForOrganizationIDs.Select(PXAccess.GetOrganizationCD).ToArray().JoinIntoStringForMessageNoQuotes(5));
			}
	...
	public class AcctSubAttribute : PXEntityAttribute { }
	...
	public class AccountAttribute : PXEntityAttribute, IPXFieldVerifyingSubscriber, IPXRowPersistingSubscriber
	{	
				...
				for (int i = 0; i < items.Count; i++)
				{
					if (typeof(IBqlWhere).IsAssignableFrom(items[i]))
					{
						items[i] = BqlCommand.Compose(
							...
							typeof(And<,,>),
							typeof(Account.accountingType), GetAccountTypeValue(),
							typeof(And<>),
							items[i]);
		...
		protected virtual Type GetAccountTypeValue()
		{
			return typeof(Equal<AccountEntityType.gLAccount>);
		}
		...
		private Ledger GetLedger(PXCache sender, object row)
		{
			...
			return Ledger.PK.Find(sender.Graph, ledgerID);
		}
		...
		public static Account GetAccount(PXGraph graph, int? accountID)
		{
			...
			var account = Account.PK.Find(graph, accountID);
	...
	public sealed class UnboundSubAccountAttribute : PXEntityAttribute
	{
	...
	{        
		...
		public AccountCDWildcardAttribute()
			: base(_DimensionName,typeof(Search<Account.accountCD,
				Where<Account.accountingType, Equal<AccountEntityType.gLAccount>>>))
		{
			Initialize();	
		}
	...
	public class GLCashTranIDAttribute : CashTranIDAttribute, IPXRowInsertingSubscriber, IPXRowUpdatingSubscriber
	{
		...
		public virtual void RowUpdating(PXCache sender, PXRowUpdatingEventArgs e)
		{
			...
			if (CheckGLTranCashAcc(sender.Graph, tran, out cashAccountID) == false)
			{
				...
				if (e.ExternalCall)
				{
					if (!PXAccess.FeatureInstalled<FeaturesSet.subAccount>())
					{
						sender.RaiseExceptionHandling<GLTran.accountID>(tran, account.AccountCD,
						new PXSetPropertyException(Messages.CashAccountDoesNotExistForBranchAndAccount, branch.BranchCD, account.AccountCD));
						e.Cancel = true;
					}
					else
					{
		...
		{
			cashAccountID = null;
			Account acc = Account.PK.Find(graph, tran.AccountID);
				...
				where TBatchNbr : IBqlField
		{
			...
			{
				Ledger setup = Ledger.PK.Find(sender.Graph, parentTran.LedgerID);
				if (!(setup is { BalanceType: LedgerBalanceType.Actual }))
					return null;
			}
			else
			{
			...
			else
			{
				if ((parentTran.Released == true) && (catran_Row != null) && (catran_Row.TranID != null))
				{
					...
					if (catran_Row.VoidedTranID.HasValue)
					{
						SetOriginalTransactionVoided(sender, catran_Row.VoidedTranID, catran_Row);
					}
				}
				else
				{
	...
	public sealed class PersistErrorAttribute : PXEventSubscriberAttribute, IPXFieldSelectingSubscriber
	{
		...
		public static void SetError<TField>(PXCache sender, object data, string error)
			where TField : IBqlField
		{
			SetError(sender, data, typeof(TField).Name, error);
		}
		...
		public static void ClearError<TField>(PXCache sender, object data)
			where TField : IBqlField
		{
			ClearError(sender, data, typeof(TField).Name);
		}
