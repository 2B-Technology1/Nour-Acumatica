...
namespace PX.Objects.GL.FinPeriods
{
	public class FinPeriodUtils : IFinPeriodUtils
	{
		...
		{
			var recordsByPeriod = records.AsEnumerable().GroupBy(record => getFinPeriodID(record));
		...
		public void ValidateMasterFinPeriod<T>(IEnumerable<T> records, Func<T, string> getMasterFinPeriodID, Func<T, int?[]> getBranchIDs, Type fieldModuleClosed = null)
		{
			var recordsByPeriod = records.GroupBy(record => getMasterFinPeriodID(record));
			...
			foreach (var recordsByPeriodGroup in recordsByPeriod)
			{
				...
				ValidateMasterFinPeriod(recordsByPeriodGroup.Key, orgnizationIDs, fieldModuleClosed, generalResult);
			}

			generalResult.RaiseIfHasError();
		}

		protected virtual void ValidateMasterFinPeriod(string masterFinPeriodID,
			int?[] orgnizationIDs,
			Type fieldModuleClosed = null,
			ProcessingResult generalResult = null)
		{
			...
			ICollection<OrganizationFinPeriod> finPeriods =
								PXSelect<OrganizationFinPeriod,
										...
										.Select(Graph, orgnizationIDs, masterFinPeriodID)
										.RowCast<OrganizationFinPeriod>()
										.ToArray();
			...
			if (finPeriods.Count != orgnizationIDs.Length)
			{
				...
				generalResult.AddErrorMessage(Messages.RelatedFinPeriodsForMasterDoesNotExistForCompanies,
		...
		public virtual ProcessingResult CanPostToPeriod(IFinPeriod finPeriod, Type fieldModuleClosed = null)
		{
			...
			if (fieldModuleClosed != null)
			{
				...
				{
					result = HandleErrorThatPeriodIsClosed(finPeriod, GetMessageForModule(fieldModuleClosed.Name));
				}
			}
		...
		private string GetMessageForModule(string module)
		{
			switch (module)
			{
				case nameof(OrganizationFinPeriod.iNClosed): return IN.Messages.FinancialPeriodClosedInIN;
				case nameof(OrganizationFinPeriod.aRClosed): return AR.Messages.FinancialPeriodClosedInAR;
				case nameof(OrganizationFinPeriod.aPClosed): return AP.Messages.FinancialPeriodClosedInAP;
				case nameof(OrganizationFinPeriod.cAClosed): return CA.Messages.FinancialPeriodClosedInCA;

				default: return Messages.FinPeriodIsClosedInCompany;
			}
		}
		...
		public bool CanPostToClosedPeriod()
		{
			...
			GLSetup setup = new PXSetup<GLSetup>(Graph).Current;
		...
		{
			return HandleErrorThatPeriodIsClosed(finPeriod, Messages.FinPeriodIsClosedInCompany);
		}
		...
		public virtual void CopyPeriods<TDAC, TSourceFinPeriodID, TSourceMasterFinPeriodID, TDestFinPeriodID, TDestMasterFinPeriodID>(PXCache cache, TDAC src, TDAC dest)
			where TDAC : class, IBqlTable, new()
		...
		{
			var sourceFinPeriodID = cache.GetValue<TSourceFinPeriodID>(src);
			var sourceMasterFinPeriodID = cache.GetValue<TSourceMasterFinPeriodID>(src);
			var destFinPeriodID = cache.GetValue<TDestFinPeriodID>(src);
			var destMasterFinPeriodID = cache.GetValue<TDestMasterFinPeriodID>(src);

			if (sourceFinPeriodID != destFinPeriodID && sourceMasterFinPeriodID != destMasterFinPeriodID)
			{
				object origFinPeriodID = cache.GetValue<TSourceFinPeriodID>(src);
				string tranPeriodID = (string)cache.GetValue<TSourceMasterFinPeriodID>(src);
				try
				{
					FinPeriodIDAttribute.SetPeriodsByMaster<TDestFinPeriodID>(cache, dest, tranPeriodID);
					origFinPeriodID = cache.GetValue<TSourceFinPeriodID>(dest);
					cache.RaiseFieldVerifying<TDestFinPeriodID>(dest, ref origFinPeriodID);
				}
				catch (PXException e)
				{
					cache.RaiseExceptionHandling<TDestFinPeriodID>(dest, origFinPeriodID, e);
				}
			}
		}
	}
