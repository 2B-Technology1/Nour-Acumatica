...
namespace PX.Objects.GL.FinPeriods
{
	public class FinPeriodRepository : IFinPeriodRepository
	{
		...
		{
			CalendarByYearPrefetchParameters parameters = new CalendarByYearPrefetchParameters { OrgID = organizationID, Graph = Graph };
			CalendarByYear calendarByYear = PXDatabase.GetSlot<CalendarByYear, CalendarByYearPrefetchParameters>(nameof(CalendarByYear) + organizationID, parameters, typeof(FinPeriod));
			parameters.Graph = null; //make sure the graph will never be forgotten in the heap
			return calendarByYear;
		}

		public FinPeriod FindFinPeriodByDate(DateTime? date, int? organizationID)
		{
			if (organizationID == null || date == null) return null;
			else return GetCalendarCache(organizationID.Value).FetchFinPeriod(date.Value);
		}
		...
		public FinPeriod FindByID(int? organizationID, string finPeriodID)
		{
			...
			else return GetCalendarCache(organizationID.Value).FetchFinPeriod(finPeriodID);
		}
	    ...
	    {
			FinPeriod period = organizationID == null ? null : GetCalendarCache(organizationID.Value).FetchFinPeriodByMasterID(masterFinPeriodID);
