...
namespace PX.Objects.GL.Reclassification.UI
{
	public class ReclassifyTransactionsProcess : ReclassifyTransactionsBase<ReclassifyTransactionsProcess>
	{
		...
		protected virtual void GLTranForReclassification_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			bool isProjectOrNonProject = ProjectDefaultAttribute.IsProjectOrNonProject(this, tran.ProjectID);
			PXUIFieldAttribute.SetEnabled<GLTranForReclassification.newProjectID>(cache, tran, canEdit && isProjectOrNonProject);
			...
			if (tran.NewProjectID == null && !isProjectOrNonProject) //is contract
			{
				// Acuminator disable once PX1047 RowChangesInEventHandlersForbiddenForArgs : contract ID is not and should not be actually set to NewProjectID, but it should be shown
				tran.NewProjectID = tran.ProjectID;
			}
		...
		protected virtual void _(Events.FieldDefaulting<GLTranForReclassification, GLTranForReclassification.newFinPeriodID> e)
		{
			e.Cancel = true;
		}
		...
		{
			if (tran?.NewTranDate == null) return;
			if (FieldHasError(tran, nameof(GLTranForReclassification.newTranDate))) return;
		...
		private void ReplaceProjectAttributes(ReplaceOptions options, GLTranForReclassification tran)
		{
			if (options.NewProjectID != null && options.NewProjectID != tran.NewProjectID)
			{
				tran.NewProjectID = options.NewProjectID;

				if (options.NewTaskID == null)
				{
					tran.NewTaskID = GetNewTaskIdAfterProjectReplaceWithoutTaskReplace(tran.NewProjectID, tran.ProjectID, tran.TaskID);
				}
			}
		...
		private int? GetNewTaskIdAfterProjectReplaceWithoutTaskReplace(int? newProjectId, int? oldProjectId, int? oldTaskId)
		{
			...
			var oldTask = PMTask.PK.Find(this, oldProjectId, oldTaskId);

			var taskOfTheNewProjectWithTheSameCD = PMTask.UK.Find(this, newProjectId, oldTask.TaskCD);

			return taskOfTheNewProjectWithTheSameCD?.TaskID;
		}
		...
		{
			if (tran.ProjectID == nonProjectID || tran.ProjectID == null) return true;
			if (PMProject.PK.Find(graph, tran.ProjectID)?.BaseType == CT.CTPRType.Contract) return true;

			return PXAccess.FeatureInstalled<FeaturesSet.projectAccounting>() &&
				SelectFrom<PMProject>
				.LeftJoin<PMTran>
		...
		{
			tranForReclass.NewProjectID = ProjectDefaultAttribute.IsProjectOrNonProject(this, tranForReclass.ProjectID) ? tranForReclass.ProjectID : null;
        ...
        private IEnumerable<PXResult<GLTranForReclassification, Account, Sub, Batch, CurrencyInfo, Ledger>> GetTransForReclassByLoadOptions(LoadOptions loadOptions)
		{
			PXSelectBase<GLTranForReclassification> query = new PXSelectJoin<GLTranForReclassification,
																		...
																		Where<PMProject.contractID, IsNull, Or<PMProject.nonProject, Equal<True>,
																			Or<PMProject.isActive, Equal<True>, And<PMProject.status, Equal<ProjectStatus.active>,
																			And<Where<PMTran.tranID, IsNull, 
		...
		private void OptionallyApplyProjectOptions(LoadOptions loadOptions, PXSelectBase<GLTranForReclassification> query, List<object> pars)
		{
			...
			{
				var project = PMTask.PK.FindDirty(this, loadOptions.ProjectID, loadOptions.FromTaskID);
			...
			{
				var project = PMTask.PK.FindDirty(this, loadOptions.ProjectID, loadOptions.ToTaskID);
