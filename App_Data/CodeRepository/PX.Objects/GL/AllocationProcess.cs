...
namespace PX.Objects.GL
{
	...
	public class AllocationProcess : PXGraph<AllocationProcess>
	{
		...
		public AllocationProcess()
		{
			...
			OpenPeriodAttribute.SetValidatePeriod<AllocationFilter.finPeriodID>(Filter.Cache, null, validationValue);
		}
		...
		[PXUIVisible(typeof(FeatureInstalled<CS.FeaturesSet.branch>.Or<FeatureInstalled<CS.FeaturesSet.multiCompany>>))]
		protected virtual void _(Events.CacheAttached<AllocationExt.branchID> e) { }
		#endregion
		...
		protected virtual void ProcessAllocation(GLAllocation allocation, string masterPostPeriod, DateTime? aDate) 
		{
			...
			//Set Batch Currency
			Ledger allocLedger = FindLedger(allocation.AllocLedgerID);

			CurrencyInfo info = batchGraph.currencyinfo.Insert(new CurrencyInfo
			{
				CuryID = allocLedger.BaseCuryID,
				BaseCuryID = allocLedger.BaseCuryID
			});
			...
			bool zeroTotalForSourceAccounts = totalDebit == 0m && totalCredit == 0m;
			if (zeroTotalForSourceAccounts || difference != 0m && difference < MinAmountToDistribute) return; //Amount is too small to be destributed
			...
			foreach (PXResult<GLAllocationDestination> iRes in destSelect.Select(allocation.GLAllocationID))
			{
				...
				else
				{
					...
					for (int i = 1; i <= segCount; i++)
					{
						...
						{
								newSubCD = subSrc.SubCD;
								keylength = subSrc.SubCD.Length;
							}
						}
						else
						{
		...
		{
			return Ledger.PK.Find(this, aLedgerID);
		}
