...
namespace PX.Objects.GL
{
	public abstract class OrganizationDependedPeriodIDAttribute : PeriodIDAttribute, IPXFieldDefaultingSubscriber
    {
		...
		{
			return PeriodKeyProvider.GetKey(cache.Graph, cache, row);
		}
		...
		public virtual void FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			{
				e.NewValue = GetPeriod(sender.Graph,
					DefaultTypeRestrictedByOrganization ?? DefaultType,
					 _sourceDate == DateTime.MinValue ? (DateTime?)null : _sourceDate,
					PeriodKeyProvider.GetKey(sender.Graph, sender, e.Row),
					e.Row.SingleToListOrNull()).PeriodIDForDisplay;
			}
		}
            ...
            if (RedefaultOrRevalidateOnOrganizationSourceUpdated)
            {
                ...
                if (ShouldExecuteRedefaultFinPeriodIDonRowUpdated(errorValue, hasError, newPeriodKey, oldPeriodKey))
                {
                    RedefaultPeriodID(cache, e.Row);
						 ...
						 && (!IsFilterMode || e.ExternalCall == true))
                {
