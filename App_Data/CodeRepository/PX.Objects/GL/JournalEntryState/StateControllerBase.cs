...
namespace PX.Objects.GL.JournalEntryState
{
	public abstract class StateControllerBase
	{
        ...
        private void SetReclassifyButtonState(Batch batch, out bool alreadyReclassified)
		{
			...
			var ledger = Ledger.PK.Find(JournalEntry, batch.LedgerID);
		...
		public bool CanReverseBatch(Batch batch)
		{
			...
			if (batch.Module == GL.BatchModule.CM && batch.AutoReverse != true && batch.AutoReverseCopy != true)
			{
				gLTran = (GLTran)PXSelect<GLTran,
											...
											.Select(JournalEntry, batch.Module, batch.BatchNbr, "REV");
			}

			bool canReverse = (batch.Released == true)
				&& (batch.Module != GL.BatchModule.CM || gLTran != null)
