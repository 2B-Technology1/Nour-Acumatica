...
namespace PX.Objects.CN.Compliance.AP.GraphExtensions
{
    public class ApInvoiceEntryExt : PXGraphExtension<ComplianceViewEntityExtension<APInvoiceEntry, APInvoice>, APInvoiceEntry>
    {
        ...
        protected virtual void _(Events.RowDeleted<APInvoice> args)
        {
            ...
            {
				if (compliance.DocumentType == lwDocType.ComplianceAttributeTypeID && compliance.IsCreatedAutomatically != true)
				{
					ComplianceDocuments.Delete(compliance);
				}
				else
				{
        ...
        private IEnumerable<ComplianceDocument> GetComplianceDocuments()
        {
            ...
            {
				List<ComplianceDocument> complianceDoc = new PXSelectJoin<ComplianceDocument, LeftJoin<ComplianceDocumentReference,
								On<ComplianceDocumentReference.complianceDocumentReferenceId, Equal<ComplianceDocument.billID>>>,
							Where<ComplianceDocumentReference.type, Equal<Current<APInvoice.docType>>,
				...
				var linksToBill = new PXSelect<ComplianceDocumentBill,
						Where<ComplianceDocumentBill.docType, Equal<Current<APInvoice.docType>>,
						And<ComplianceDocumentBill.refNbr, Equal<Current<APInvoice.refNbr>>>>>(Base).Select();

				foreach(PXResult<ComplianceDocumentBill> res in linksToBill)
				{
					ComplianceDocumentBill link = (ComplianceDocumentBill)res;

					ComplianceDocument lw = PXSelect<ComplianceDocument,
						Where<ComplianceDocument.complianceDocumentID, Equal<Required<ComplianceDocument.complianceDocumentID>>>>
						.Select(Base, link.ComplianceDocumentID);

					if (!complianceDoc.Contains(lw))
					{
						complianceDoc.Add(lw);
					}
				}
