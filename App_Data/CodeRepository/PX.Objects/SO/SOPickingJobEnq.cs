...
namespace PX.Objects.SO
{
	public class SOPickingJobEnq : PXGraph<SOPickingJobEnq>
	{
		...
		[PXUIField(DisplayName = Messages.TimeInQueue, Enabled = false)]
		...
		public
			SelectPickingJobs<
				Where<SOPickingJob.status.IsNotIn<SOPickingJob.status.onHold, SOPickingJob.status.picked, SOPickingJob.status.completed, SOPickingJob.status.cancelled>>
			> PickingJobs;
			...
			SelectPickingJobs<
				Where<
					...
					Or<
						...
						And<SOPickingJob.status.IsNotIn<SOPickingJob.status.onHold, SOPickingJob.status.completed, SOPickingJob.status.cancelled>>>>
			> PackingJobs;
		...
		protected virtual void _(Events.RowUpdated<SOPickingJob> args)
		{
			...
			try
			{
				...
				processor.PickingJobs.Cache.MarkUpdated(job, assertError: true);
