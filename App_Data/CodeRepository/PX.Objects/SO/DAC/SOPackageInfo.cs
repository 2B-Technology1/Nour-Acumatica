...
namespace PX.Objects.SO
{
	...
	public partial class SOPackageInfo : PX.Data.IBqlTable
	{
		...
		{
			public static SOPackageInfo Find(PXGraph graph, string orderType, string orderNbr, int? lineNbr, PKFindOptions options = PKFindOptions.None)
				=> FindBy(graph, orderType, orderNbr, lineNbr, options);
		}
		...
		[PXSelector(typeof(Search5<CSBox.boxID,
			...
			Or<Where<Current<SOOrder.shipVia>, IsNotNull,
				And<Where<CarrierPackage.carrierID, Equal<Current<SOOrder.shipVia>>, Or<CSBox.activeByDefault, Equal<True>>>>>>>,
		...
		public abstract class allowOverrideDimension : PX.Data.BQL.BqlBool.Field<allowOverrideDimension> { }

		/// <summary>
		/// A Boolean value that specifies whether the <see cref="Width"/>, <see cref="Height"/> and <see cref="Length"/> dimension values of the package can be overridden.
		/// </summary>
		/// <value>
		/// The field always returns the value of <see cref="CSBox.AllowOverrideDimension"/>.
		/// </value>
		[PXBool]
		[PXUnboundDefault(typeof(Search<CSBox.allowOverrideDimension, Where<CSBox.boxID, Equal<Current<boxID>>>>), PersistingCheck = PXPersistingCheck.Nothing)]
		[PXFormula(typeof(Default<boxID>))]
	...
	[PXProjection(typeof(Select2<SOPackageInfo,
		InnerJoin<CSBox, On<CSBox.boxID, Equal<SOPackageInfo.boxID>>,
		CrossJoin<CommonSetup>>>), new Type[] { typeof(SOPackageInfo) })]
	[Serializable]
	public partial class SOPackageInfoEx : SOPackageInfo
	{
		...
		{
			public static SOPackageInfoEx Find(PXGraph graph, string orderType, string orderNbr, int? lineNbr, PKFindOptions options = PKFindOptions.None)
				=> FindBy(graph, orderType, orderNbr, lineNbr, options);
		}
