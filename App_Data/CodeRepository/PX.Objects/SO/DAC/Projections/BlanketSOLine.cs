...
namespace PX.Objects.SO.DAC.Projections
{
	...
	public class BlanketSOLine : IBqlTable
	{
		...
		{
			public static BlanketSOLine Find(PXGraph graph, string orderType, string orderNbr, int? lineNbr, PKFindOptions options = PKFindOptions.None) => FindBy(graph, orderType, orderNbr, lineNbr, options);
		}
		...
		[PXDBBool(BqlField = typeof(SOLine.isStockItem))]
		...
		[PXParent(typeof(Select<BlanketSOOrderSite, Where<BlanketSOOrderSite.orderType, Equal<Current<orderType>>, And<BlanketSOOrderSite.orderNbr, Equal<Current<orderNbr>>, And<BlanketSOOrderSite.siteID, Equal<Current2<siteID>>>>>>), LeaveChildren = true)]
		[PXUnboundFormula(typeof(IIf<Where<openLine.IsEqual<True>>, int1, int0>), typeof(SumCalc<BlanketSOOrderSite.openLineCntr>), ValidateAggregateCalculation = true)]
		...
		/// </summary>
		[PXDBBool(BqlField = typeof(SOLine.skipLineDiscounts))]
		[PXDefault(false)]
		...
		[DirtyFormula(typeof(Switch<Case<Where<openLine, Equal<True>, And<Where<isFree, NotEqual<True>, Or<manualDisc, Equal<True>>>>>, int1>, int0>),
			typeof(SumCalc<BlanketSOOrder.openLineCntr>), IsUnbound: true, SkipZeroUpdates = false)]
