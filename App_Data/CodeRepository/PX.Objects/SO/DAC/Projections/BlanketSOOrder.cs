...
namespace PX.Objects.SO.DAC.Projections
{
	...
	public class BlanketSOOrder : IBqlTable
	{
		...
		{
			public static BlanketSOOrder Find(PXGraph graph, string orderType, string orderNbr, PKFindOptions options = PKFindOptions.None) => FindBy(graph, orderType, orderNbr, options);
		}
		#endregion
		...
		[PXDBInt(BqlField = typeof(SOOrder.openSiteCntr))]
		...
		[PXDBTimestamp(BqlField = typeof(SOOrder.Tstamp), VerifyTimestamp = VerifyTimestampOptions.BothFromGraphAndRecord)]
