...
namespace PX.Objects.SO
{
	...
	public class SOPickingJob : WMSJob
	{
		...
		{
			public static SOPickingJob Find(PXGraph graph, int? jobID, PKFindOptions options = PKFindOptions.None) => FindBy(graph, jobID, options);
		}
		...
		public new abstract class status : BqlString.Field<status>
		{
			...
			public const string Cancelled = WMSJob.status.Cancelled;
			...
			public static class DisplayNames
			{
				...
				public const string Cancelled = WMSJob.status.DisplayNames.Cancelled;
			}
		...
		[PXUIEnabled(typeof(
			status.IsNotIn<status.picked, status.completed, status.cancelled>.
			And<SOPickingWorksheet.worksheetType.FromParent.IsEqual<SOPickingWorksheet.worksheetType.single>>))]
