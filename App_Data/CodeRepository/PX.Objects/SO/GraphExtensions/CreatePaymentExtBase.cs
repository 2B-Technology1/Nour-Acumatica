...
namespace PX.Objects.SO.GraphExtensions
{
		...
		where TPaymentAdjust : class, IBqlTable, new()
	{
		...
		protected virtual void ClearQuickPayment(SOQuickPayment quickPayment)
		{
			...
			quickPayment.NewAccount = null;
			quickPayment.SaveCard = null;
			quickPayment.SaveAccount = null;
		...
		protected virtual void _(Events.RowSelected<SOQuickPayment> eventArgs)
		{
			...
			bool isCCPaymentMethod = paymentMethod?.PaymentType == PaymentMethodType.CreditCard || paymentMethod?.PaymentType == PaymentMethodType.EFT;
			bool isEft = paymentMethod?.PaymentType == PaymentMethodType.EFT;
			bool isCreditCardRequired = isCCPaymentMethod && paymentMethod.IsAccountNumberRequired == true;
			...
			createPaymentAuthorize.SetVisible(CCProcessingHelper.PaymentMethodSupportsIntegratedProcessing(paymentMethod) && AllowsAuthorization(eventArgs.Row));
			...
			bool isActualFinPeriodIsClosed = IsActualFinPeriodClosed();
			...
			bool processingCenterFeatureDisabled = CCPluginTypeHelper.IsProcCenterFeatureDisabled(cCProcessingCenter?.ProcessingTypeName);
			...
			if (isActualFinPeriodIsClosed && isCCPaymentMethod)
			{
				eventArgs.Cache.RaiseExceptionHandling<SOQuickPayment.paymentMethodID>(eventArgs.Row, eventArgs.Row.PaymentMethodID,
			...
			createPaymentCapture.SetEnabled(!(hasError || externalAuthorizationOnly || isActualFinPeriodIsClosed || processingCenterFeatureDisabled));
			createPaymentAuthorize.SetEnabled(!(hasError || externalAuthorizationOnly || isActualFinPeriodIsClosed || isEft || processingCenterFeatureDisabled));
			createPaymentRefund.SetEnabled(!(hasError || externalAuthorizationOnly) || isActualFinPeriodIsClosed || processingCenterFeatureDisabled);
			...
			PXUIFieldAttribute.SetVisible<SOQuickPayment.newCard>(eventArgs.Cache, eventArgs.Row, useNewCard && !isEft);
			PXUIFieldAttribute.SetEnabled<SOQuickPayment.newCard>(eventArgs.Cache, eventArgs.Row, useNewCard && !isEft);
			PXUIFieldAttribute.SetVisible<SOQuickPayment.newAccount>(eventArgs.Cache, eventArgs.Row, useNewCard && isEft);
			PXUIFieldAttribute.SetEnabled<SOQuickPayment.newAccount>(eventArgs.Cache, eventArgs.Row, useNewCard && isEft);

			bool useSaveCard = useNewCard && GetSavePaymentProfileCode(eventArgs.Row) == SavePaymentProfileCode.Allow;
			PXUIFieldAttribute.SetVisible<SOQuickPayment.saveCard>(eventArgs.Cache, eventArgs.Row, useSaveCard && !isEft);
			PXUIFieldAttribute.SetEnabled<SOQuickPayment.saveCard>(eventArgs.Cache, eventArgs.Row, useSaveCard && !isEft);
			PXUIFieldAttribute.SetVisible<SOQuickPayment.saveAccount>(eventArgs.Cache, eventArgs.Row, useSaveCard && isEft);
			PXUIFieldAttribute.SetEnabled<SOQuickPayment.saveAccount>(eventArgs.Cache, eventArgs.Row, useSaveCard && isEft);

			if (eventArgs.Row.NewAccount == true && isEft)
			{
				UIState.RaiseOrHideError<SOQuickPayment.newAccount>(eventArgs.Cache, eventArgs.Row, true, AR.Messages.WarningEftThisPayment, PXErrorLevel.Warning, eventArgs.Row.NewAccount);
			}
		}

		protected virtual bool IsActualFinPeriodClosed()
		{
			var branch = PXAccess.GetBranch(PXContext.GetBranchID());

			if (branch != null)
			{
				var organizationId = branch.Organization.OrganizationID;

				FinPeriod finPeriod = FinPeriodRepository.FindFinPeriodByDate(DateTime.Now, organizationId);

				if (finPeriod == null || finPeriod.Status == FinPeriod.status.Closed)
					return true;
			}
		...
		protected virtual void _(Events.FieldUpdated<SOQuickPayment, SOQuickPayment.newCard> eventArgs) => NewCardAccountFieldUpdated(eventArgs.Row, eventArgs.Cache);

		protected virtual void _(Events.FieldUpdated<SOQuickPayment, SOQuickPayment.newAccount> eventArgs) => NewCardAccountFieldUpdated(eventArgs.Row, eventArgs.Cache);
		...
		private void NewCardAccountFieldUpdated(SOQuickPayment payment, PXCache cache)
		{
			...
			{
				payment.PMInstanceID = null;
			}
			else
			{
				cache.SetDefaultExt<SOQuickPayment.processingCenterID>(payment);
				cache.SetDefaultExt<SOQuickPayment.saveCard>(payment);
			}
		}
			...
			where AmountField: IBqlField
		{
			...
			{
				exc = new PXSetPropertyException<AmountField>(GetExceedingAmountErrorMessage(qp), maxValue);
			}
			else if (value <= 0m)
			{
		...
		protected virtual void FieldDefaulting(Events.FieldDefaulting<SOQuickPayment, SOQuickPayment.newCard> e)
		{
			...
			if (procCenter != null)
			{
				bool useNewCard = (paymentMethod.PaymentType == PaymentMethodType.CreditCard || paymentMethod.PaymentType == PaymentMethodType.EFT)
					&& procCenter.UseAcceptPaymentForm == true;
		...
		protected virtual void _(Events.RowSelected<TDocument> eventArgs)
		{
			bool isEFTFeatureInstalled = PXAccess.FeatureInstalled<CS.FeaturesSet.acumaticaPayments>();
			voidDocumentPayment.SetCaption(isEFTFeatureInstalled ? Messages.VoidCCEftPayment : Messages.VoidCCPayment);
			importDocumentPayment.SetCaption(isEFTFeatureInstalled ? Messages.ImportCardEftPayment : Messages.ImportCardPayment);
		}
		...
		protected virtual void CreatePayment(SOQuickPayment quickPayment, string paymentType, bool throwErrors)
		{
			...
			if (quickPayment.Capture == true || quickPayment.Authorize == true || quickPayment.Refund == true)
			{
				...
				{
					Document = GetCurrent<TDocument>(),
					CcPaymentConnectorUrl = CCPaymentProcessingHelper.GetCurrentSiteUrl(System.Web.HttpContext.Current),
		...
		protected virtual void ProcessCreatePayment(CreatePaymentParameter parameter)
		{
			...
			using (new ForcePaymentAppScope())
			{
				...
				{
					CCPaymentProcessingHelper.SetCurrentSiteUrl(null);
				}
			}
		}
		...
		protected virtual bool AllowsAuthorization(SOQuickPayment quickPayment) => quickPayment.IsRefund != true;

		protected virtual string GetExceedingAmountErrorMessage(SOQuickPayment quickPayment) => Messages.PaymentShouldBeLessUnpaidAmount;
		...
		public virtual void CapturePayment(TDocumentAdjust adjustment, ARPaymentEntry paymentEntry, ARPaymentEntryPaymentTransaction paymentTransaction)
		{
			...
			if (paymentEntry.Document.Current.Hold == true)
				throw new PXException(Messages.CapturePaymentWithHoldStatusError);
