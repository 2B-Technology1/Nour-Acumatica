...
namespace PX.Objects.SO.GraphExtensions.SOOrderEntryExt
{
	...
	public abstract class SOOrderLineSplittingAllocatedExtension : PXGraphExtension<SOOrderLineSplittingExtension, SOOrderEntry>
	{
		...
		[PXProtectedAccess] protected abstract SOLineSplit[] SelectAllSplits(SOLine line, bool compareInventoryID = true);
		[PXProtectedAccess] protected abstract SOLineSplit[] SelectSplits(SOLineSplit split, bool compareInventoryID = true);
		...
		[PXProtectedAccess] protected abstract SOLineSplit[] SelectSplitsReversedforTruncate(SOLineSplit split, bool excludeCompleted = true);

		[PXProtectedAccess] protected abstract IDisposable InvtMultModeScope(SOLine line);
		#endregion
		...
		public virtual void EventHandlerInternal(ManualEvent.Row<SOLine>.Inserted.Args e, Action<ManualEvent.Row<SOLine>.Inserted.Args> base_EventHandlerInternal)
		{
			...
			Availability.Check(e.Row, e.Row.CostCenterID);
		}
		...
		public virtual void EventHandlerInternal(ManualEvent.Row<SOLine>.Updated.Args e, Action<ManualEvent.Row<SOLine>.Updated.Args> base_EventHandlerInternal)
		{
			...
			else
			{
				...
				if (IsAllocationEntryEnabled)
				{
					...
					if (e.OldRow != null && (!e.Cache.ObjectsEqual<SOLine.inventoryID, SOLine.subItemID, SOLine.invtMult, SOLine.uOM, SOLine.projectID, SOLine.taskID, SOLine.costCenterID>(e.Row, e.OldRow)
						|| (e.Row.SiteID != e.OldRow.SiteID && e.Row.Operation == SOOperation.Issue)))
					{
						Base1.RaiseRowDeleted(e.OldRow);
						Base1.RaiseRowInserted(e.Row);
					}
					else if (e.OldRow != null && e.Row.SiteID != e.OldRow.SiteID && e.Row.Operation == SOOperation.Receipt)
					{
						foreach (SOLineSplit split in SelectSplits(e.Row))
						{
							split.SiteID = e.Row.SiteID;
		...
		#endregion
		#region SOLineSplit
		protected virtual void SOLineSplit_SiteID_Updated(Events.FieldUpdated<SOLineSplit, SOLineSplit.siteID> e)
		{
			if (e.Row.Operation == SOOperation.Receipt)
			{
				e.Row.ToSiteID = e.Row.SiteID;
			}
		}

		/// Overrides <see cref="SOOrderLineSplittingExtension.EventHandler(ManualEvent.Row{SOLineSplit}.Selected.Args)"/>
		[PXOverride]
		public void EventHandler(ManualEvent.Row<SOLineSplit>.Selected.Args e,
			Action<ManualEvent.Row<SOLineSplit>.Selected.Args> base_EventHandler)
		{
			base_EventHandler(e);

			if (e.Row != null)
			{
				var soline = PXParentAttribute.SelectParent<SOLine>(e.Cache, e.Row);
				PXUIFieldAttribute.SetEnabled<SOLineSplit.isAllocated>(e.Cache, e.Row, AllowToManualAllocate(soline, e.Row));
			}
		}
				...
				if (!e.ExternalCall && CurrentOperation == PXDBOperation.Update)
				{
					...
					foreach (SOLineSplit siblingSplit in SelectSplits(e.Row))
					{
						if (SchedulesEqual(e.Row, siblingSplit, PXDBOperation.Insert))
						{
							...
							e.Cache.MarkUpdated(siblingSplit, assertError: true);
							...
							if (siblingSplit.SplitLineNbr != e.Row.SplitLineNbr && SchedulesEqual(siblingSplit, e.Row, PXDBOperation.Update))
							{
								...
								if (newPlan != null)
								{
									...
									if (itemPlanCache.GetStatus(newPlan) != PXEntryStatus.Inserted)
										itemPlanCache.MarkUpdated(newPlan, assertError: true);
								}
			...
			Action<ManualEvent.FieldOf<SOLineSplit>.Defaulting.Args<string>> base_EventHandlerUOM)
		{
			...
			else
			{
				...
				if (item != null && ((INLotSerClass)item).LotSerTrack == INLotSerTrack.SerialNumbered
					&& Base.Transactions.Current?.IsSpecialOrder != true)
				{
		...
		protected virtual bool AllocatedUpdated(SOLineSplit split, bool externalCall)
		{
			...
			var stat = INSiteStatusByCostCenter.PK.Find(Base, split.InventoryID, split.SubItemID, split.SiteID, split.CostCenterID);
			if (stat != null)
			{
			...
			else if (INLotSerialNbrAttribute.IsTrack(item, split.TranType, split.InvtMult) && split.LotSerialNbr != null && !INLotSerialNbrAttribute.IsTrackSerial(item, split.TranType, split.InvtMult))
			{
				if (split.BaseQty > 0m)
				{
					...
					if (split.IsAllocated == true)
					{
						...
						Base.FindImplementation<SOLineSplitPlan>().RaiseRowUpdated(split);
		...
		/// </summary>
		[PXOverride]
		public virtual SOLine UpdateParent(SOLineSplit newSplit, SOLineSplit oldSplit,
			Func<SOLineSplit, SOLineSplit, SOLine> base_UpdateParent)
		{
			...
			Action<Counters, SOLineSplit> base_UpdateCounters)
		{
			...
			SOOrderType ordertype = PXSetup<SOOrderType>.Select(Base);
			if (IsAllocationEntryEnabled && ordertype.RequireLotSerial != true)
			{
		...
		public virtual void CompleteSchedules(SOLine line)
		{
			...
			var lastUnshippedSchedules = new HashSet<int?>();
			foreach (var split in SelectSplitsReversed(line, false))
			{
				...
				if (lastShipmentNbr != null && split.ShipmentNbr == lastShipmentNbr)
				{
					...
					lastUnshippedSchedules.Add(split.SplitLineNbr);
				}
			}

			TruncateSchedules(line, lastUnshippedQty.Value, lastUnshippedSchedules);
		...
		private void TruncateSchedules(SOLine line, decimal baseQty)
		{
			TruncateSchedules(line, baseQty, null);
		}

		public virtual void TruncateSchedules(SOLine line, decimal baseQty, HashSet<int?> lastUnshippedSchedules)
		{
			LineCounters.Remove(line);

			foreach (var split in SelectSplitsReversedforTruncate(line)
				.OrderBy(_=> lastUnshippedSchedules != null && lastUnshippedSchedules.Contains(_.ParentSplitLineNbr)? 0 : 1))
			{
		...
		public virtual bool SchedulesEqual(SOLineSplit a, SOLineSplit b, PXDBOperation operation)
		{
			if (a != null && b != null)
			{
				return
					...
					(string.Equals(a.LotSerialNbr, b.LotSerialNbr, StringComparison.OrdinalIgnoreCase)
						|| string.IsNullOrEmpty(a.LotSerialNbr)
							&& string.IsNullOrEmpty(b.LotSerialNbr)) &&
		...
		protected virtual void IssueAvailable(SOLine line, decimal? baseQty, bool isUncomplete)
		{
			...
			foreach (INSiteStatusByCostCenter avail in
				...
				View.ReadOnly.Select(Base, line.InventoryID, line.SubItemID, line.SiteID, line.CostCenterID))
			{
				SOLineSplit split = (SOLineSplit)line;
				if (Base1.UseBaseUnitInSplit(split, line, item))
				...
				{
					SiteStatusByCostCenter accumavail = new SiteStatusByCostCenter();
		...
		protected virtual bool IsSplitRequired(SOLine line, out InventoryItem item)
		{
			...
			if (item != null && item.StkItem == false && item.KitItem == true && line.Behavior.IsNotIn(SOBehavior.CM, SOBehavior.IN, SOBehavior.MO))
				skipSplitCreating = true;
		...
		public virtual SOLineSplit[] SelectSplitsForDropShip(SOLine line) => SelectAllSplits(line).Where(s => s.ShipmentNbr == null || s.ShippedQty != 0m).ToArray();

		public virtual bool HasMultipleSplitsOrAllocation(SOLine line)
		{
			var details = SelectSplitsForDropShip(line);
			return details.Length > 1 || details.FirstOrDefault()?.IsAllocated == true;
		}
		...
		{
			var line = SelectLine(copy);
			if (line?.IsSpecialOrder == true && line.Operation == SOOperation.Issue && line.Behavior != SOBehavior.TR)
				copy.POCreate = true;
		...
		public virtual bool AllowToManualAllocate(SOLine line, SOLineSplit split)
		{
			return
				split.LineType == SOLineType.Inventory //isLineTypeInventory
				&& split.Operation == SOOperation.Issue //isIssue
				&& split.Completed != true //!isCompleted
				&& (split.POCreate != true || line.IsSpecialOrder == true) && split.POCompleted != true //!isPOSchedule
				&& split.ChildLineCntr == 0;
		}
	}
}
