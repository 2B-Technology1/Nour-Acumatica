...
namespace PX.Objects.SO.GraphExtensions.SOOrderEntryExt
{
	public class DropShipLinksExt : PXGraphExtension<PurchaseSupplyBaseExt, SOOrderEntry>
	{
		...
		public virtual void _(Events.RowSelected<SOLine> e)
		{
			...
			if (e.Row.POCreate == true && link != null && link.POCompleted == true && e.Row.Completed != true && link.BaseReceivedQty < link.POBaseOrderQty)
			{
				e.Cache.RaiseExceptionHandling<SOLine.pOLinkActive>(e.Row, e.Row.POLinkActive,
					new PXSetPropertyException<SOLine.pOLinkActive>(Messages.LinkedPOLineIsCompleted, PXErrorLevel.Warning, link.POOrderNbr));
			}
		...
		#region DropShipLink events

		public virtual void _(Events.RowInserted<DropShipLink> e)
		{
			SupplyPOOrder supplyOrder = Base1.GetSupplyOrder(e.Row);
			if (supplyOrder == null)
				return;

			if (e.Row.POCompleted == false && e.Row.Active == true)
			{
				supplyOrder.DropShipNotLinkedLinesCntr--;
				Base1.SupplyPOOrders.Update(supplyOrder);
			}
		}

		public virtual void _(Events.RowUpdated<DropShipLink> e)
		{
			SupplyPOOrder supplyOrder = Base1.GetSupplyOrder(e.Row);
			if (supplyOrder == null)
				return;

			if (!Base1.DropShipLinks.Cache.ObjectsEqual<DropShipLink.poCompleted, DropShipLink.active>(e.Row, e.OldRow))
			{
				if (e.OldRow.POCompleted == false && e.OldRow.Active == true)
				{
					supplyOrder.DropShipNotLinkedLinesCntr++;
				}
