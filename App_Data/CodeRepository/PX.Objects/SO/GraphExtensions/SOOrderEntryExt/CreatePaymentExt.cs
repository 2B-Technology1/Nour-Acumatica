...
namespace PX.Objects.SO.GraphExtensions.SOOrderEntryExt
{
	public class CreatePaymentExt : CreatePaymentExtBase<SOOrderEntry, SOOrder, SOAdjust>
	{
		...
		protected override void _(Events.RowSelected<SOOrder> eventArgs)
		{
			...
			bool isMixedOrder = eventArgs.Row.Behavior == SOBehavior.MO;
			bool docStatusAllowsPayment = eventArgs.Row.Completed != true
				...
				&& (!isMixedOrder || eventArgs.Row.Status.IsIn(SOOrderStatus.Open, SOOrderStatus.CreditHold));

			bool paymentsAllowedByAmount = (!isMixedOrder || eventArgs.Row.CuryOrderTotal >= 0m);
			bool createPaymentEnabled = paymentsAllowed && !inserted && docStatusAllowsPayment && paymentsAllowedByAmount;
			bool refundsAllowedByAmount = (!isMixedOrder || eventArgs.Row.CuryOrderTotal < 0m);
			bool createRefundEnabled = refundsAllowed && !inserted && eventArgs.Row.Status == SOOrderStatus.Open
				&& Base.soordertype.Current?.AllowRefundBeforeReturn == true && refundsAllowedByAmount;

			var paymentType = PaymentMethod.PK.Find(Base, eventArgs.Row.PaymentMethodID)?.PaymentType;

			bool importPaymentEnabled = createPaymentEnabled && eventArgs.Row.PaymentMethodID != null &&
				(paymentType == PaymentMethodType.CreditCard || paymentType == PaymentMethodType.EFT);
			...
			createOrderPrepayment.SetVisible(paymentsAllowed && !isMixedOrder);
			createOrderPrepayment.SetEnabled(createPaymentEnabled && !isMixedOrder);
			importDocumentPayment.SetVisible(paymentsAllowed && !isMixedOrder);
			importDocumentPayment.SetEnabled(importPaymentEnabled && !isMixedOrder);

			captureDocumentPayment.SetVisible(paymentsAllowed && !isMixedOrder);
			voidDocumentPayment.SetVisible(paymentsAllowed);

			deletePayment.SetVisible(isMixedOrder);
			deleteRefund.SetVisible(isMixedOrder);
			...
			ARPaymentType.SOListAttribute.SetPaymentList<SOAdjust.adjgDocType>(Base.Adjustments.Cache, refundsAllowed && refundsAllowedByAmount);
