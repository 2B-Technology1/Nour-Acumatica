...
namespace PX.Objects.SO.GraphExtensions.SOOrderEntryExt
{
	...
	public class Blanket : PXGraphExtension<SOOrderEntry>
	{
		...
		public override void Initialize()
		{
			...
			Base.RowUpdated.AddHandler<SOOrder>(LateSOOrderUpdated);
		}
				...
				if (blanketLine.POCreated == true)
				{
						...
						.Where(split => split.LineNbr == soLine.LineNbr))
					{
						...
						if (plan != null && blanketPlan != null)
						{
							...
							Base.Caches<INItemPlan>().MarkUpdated(plan, assertError: true);
						}
						break;
					}
				}
		...
		protected virtual void _(Events.RowSelected<SOOrder> e)
		{
			...
			if (isBlanket)
			{
				e.Cache.Adjust<PXUIFieldAttribute>(e.Row)
					...
					.SameFor<SOOrder.curyFreightTot>()
					.SameFor<SOOrder.disableAutomaticTaxCalculation>()
			...
			{
				Base.Transactions.Cache.Adjust<PXUIFieldAttribute>()
						.For<SOLine.pOCreateDate>(a =>
							a.Visible = PXAccess.FeatureInstalled<FeaturesSet.sOToPOLink>())
						.SameFor<SOLine.pOCreate>()
						.SameFor<SOLine.pOSource>();

				Base.splits.Cache.Adjust<PXUIFieldAttribute>()
						.For<SOLineSplit.pOCreateDate>(a =>
						a.Enabled = a.Visible = PXAccess.FeatureInstalled<FeaturesSet.sOToPOLink>());
			}
		...
		protected virtual void _(Events.RowUpdated<SOOrder> e)
		{
			...
			if (e.Row.Behavior == SOBehavior.BL)
			{
				...
				if (taxZoneChanged)
				{
					foreach (SOLine soline in Base.Transactions.Select())
					{
						if (externalTaxZone) //no need to call any additional logic while setting external tax zone
						{
							...
							Base.Transactions.Cache.MarkUpdated(soline, assertError: true);
						}
						else
						{
				...
				if (dateUpdated)
				{
					...
					foreach (SOLine line in Base.Transactions.Select())
					{
						Base.Transactions.Cache.MarkUpdated(line, assertError: true);
					}
		...
		{
			if (!sender.ObjectsEqual<SOOrder.cancelled>(e.Row, e.OldRow))
			{
				foreach (SOLine line in PXParentAttribute.SelectSiblings(Base.Transactions.Cache, null, typeof(SOOrder)))
				{
					...
					line.Cancelled = ((SOOrder)e.Row).Cancelled;
					Base.Transactions.Cache.MarkUpdated(line, assertError: true);
