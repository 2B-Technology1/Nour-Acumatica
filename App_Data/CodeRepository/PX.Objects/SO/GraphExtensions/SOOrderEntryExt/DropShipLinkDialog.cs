...
namespace PX.Objects.SO.GraphExtensions.SOOrderEntryExt
{
	public class DropShipLinkDialog : PXGraphExtension<POLinkDialog, PurchaseSupplyBaseExt, SOOrderEntry>
	{
				...
				Where<SupplyPOLine.lineType, In3<POLineType.goodsForDropShip, POLineType.nonStockForDropShip>,
					...
					And<Where<DropShipLink.sOOrderType, Equal<Current<SOLine.orderType>>,
						...
						Or<DropShipLink.sOOrderNbr, IsNull,
							...
							And<SupplyPOLine.baseOrderQty, Equal<Minus<Mult<Current<SOLine.baseOrderQty>, Current<SOLine.invtMult>>>>,
		...
		public virtual void LinkPOSupply(SOLine currentSOLine)
		{
			...
			if (Base.LineSplittingAllocatedExt.HasMultipleSplitsOrAllocation(currentSOLine))
			...
			foreach (SupplyPOLine supply in Base2.SupplyPOLines.Cache.Updated)
			{
				...
				if (selected && link == null && currentSOSplit.IsAllocated == false && currentSOSplit.Completed == false && currentSOSplit.BaseQty > 0m
					&& string.IsNullOrEmpty(currentSOSplit.SOOrderNbr) && string.IsNullOrEmpty(currentSOSplit.PONbr)
					&& currentSOSplit.BaseQty == -currentSOLine.InvtMult * currentSOLine.BaseQty)
				{
		...
		public virtual SOLine CreateSOLineFromDropShipLine(POLine line, bool isLegacy)
		{
			SOLine newLine = PXCache<SOLine>.CreateCopy(Base.Transactions.Insert());
			...
			return PXCache<SOLine>.CreateCopy(Base.Transactions.Update(newLine));
		}
	}
}
