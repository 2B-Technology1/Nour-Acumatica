...
namespace PX.Objects.SO.GraphExtensions.SOShipmentEntryExt
{
	...
	public class SOShipmentLineSplittingExtension : LineSplittingExtension<SOShipmentEntry, SOShipment, SOShipLine, SOShipLineSplit>
	{
			...
			if (e.Row.Confirmed != e.OldRow.Confirmed && e.Row.Confirmed == true)
			{
				foreach (SOShipLine item in PXParentAttribute.SelectSiblings(LineCache, null, typeof(SOShipment)))
				{
					if (Math.Abs(item.BaseQty.Value) >= 0.0000005m && (item.UnassignedQty >= 0.0000005m || item.UnassignedQty <= -0.0000005m))
					{
						...
						LineCache.MarkUpdated(item, assertError: true);
					}
				}
			}
		}
		...
		{
			using (e.ExternalCall && e.Row.Operation == SOOperation.Issue
				? UpdateIfFieldsChangedScope.Create(e.Cache, e.OldRow, e.Row, typeof(SOShipLine.locationID), typeof(SOShipLine.lotSerialNbr), typeof(SOShipLine.expireDate))
				: null)
		...
		protected virtual void LocationUpdated(SOShipLineSplit split)
		{
			...
			{
				SOShipLine line = PXParentAttribute.SelectParent<SOShipLine>(Base.splits.Cache, split);

				INLotSerialStatusByCostCenter status =
					SelectFrom<INLotSerialStatusByCostCenter>.
					Where<
						INLotSerialStatusByCostCenter.inventoryID.IsEqual<@P.AsInt>.
						...
						And<INLotSerialStatusByCostCenter.costCenterID.IsEqual<@P.AsInt>>>.
					View.Select(Base, split.InventoryID, split.SubItemID, split.SiteID, split.LotSerialNbr, split.LocationID, line?.CostCenterID);
		...
		protected override PXSelectBase<INLotSerialStatusByCostCenter> GetSerialStatusCmdBase(SOShipLine line, PXResult<InventoryItem, INLotSerClass> item)
		{
		...
		protected override void AppendSerialStatusCmdWhere(PXSelectBase<INLotSerialStatusByCostCenter> cmd, SOShipLine line, INLotSerClass lotSerClass)
		{
		...
		protected override void SetUnassignedQty(SOShipLine line, decimal detailsBaseQty, bool allowNegative)
		{
			base.SetUnassignedQty(line, detailsBaseQty, allowNegative);

			UpdateUnassigned(line);
		}

		protected virtual void UpdateUnassigned(SOShipLine line)
		{
			if (line.UnassignedQty > 0 && line.Operation == SOOperation.Issue)
			{
				var manualEditableFields = new[] { typeof(SOShipLine.locationID), typeof(SOShipLine.lotSerialNbr), typeof(SOShipLine.expireDate) };
				var manualUpdate = UpdateIfFieldsChangedScope.Any(manualEditableFields.Contains);
