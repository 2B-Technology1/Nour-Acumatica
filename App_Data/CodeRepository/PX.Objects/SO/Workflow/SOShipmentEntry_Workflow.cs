			...
			context.AddScreenConfigurationFor(screen =>
					...
					.WithFlowStates(fss =>
					{
						...
						fss.Add<State.completed>(flowState =>
						{
							return flowState
								...
								.WithFieldStates(DisableWholeScreen)
								.WithOnEnterAssignments(eass => eass.Add<gotReadyForArchiveAt>(v => v.SetFromToday()))
								.WithOnLeaveAssignments(lass => lass.Add<gotReadyForArchiveAt>(v => v.SetFromValue(null)));
						});
				...
				.WithActions(actions =>
				{
					...
					actions.Add(g => g.printLabels, a => a
						...
						.InBatchMode());
					actions.Add(g => g.printCommercialInvoices, a => a
						.WithCategory(labelsCategory)
						.MassProcessingScreen<SOInvoiceShipment>()
						.InBatchMode());
				...
				{
					rules.Add(r => r.Archive<SOShipLine>().UsingItsParentAttribute());
					rules.Add(r => r.Archive<SOShipLineSplit>().UsingItsParentAttribute());
					rules.Add(r => r.Archive<SOPackageDetail>().UsingItsParentAttribute());
					rules.Add(r => r.Archive<SOShipLineSplitPackage>().UsingItsFK<SOShipLineSplitPackage.FK.Shipment>());
				}));
		}
