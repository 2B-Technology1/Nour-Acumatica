...
namespace PX.Objects.SO.Attributes
{
	...
	public class CustomerOrderNbrAttribute : CustomerOrderNbrBaseAttribute
	{
		...
		public override void RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (order?.OrderType == null || PXUIFieldAttribute.GetErrorOnly<SOOrder.customerOrderNbr>(sender, e.Row) != null)
				return;

			var soOrderType = SOOrderType.PK.Find(sender.Graph, order.OrderType);
			if (soOrderType?.CustomerOrderIsRequired != true || string.IsNullOrWhiteSpace(order.CustomerOrderNbr) ||
				soOrderType.CustomerOrderValidation != CustomerOrderValidationType.Warn)
				return;
			...
			var exception = new PXSetPropertyException(Messages.CustomerOrderHasDuplicateWarning, PXErrorLevel.Warning, duplicateOrder.OrderNbr);
			sender.RaiseExceptionHandling<SOOrder.customerOrderNbr>(e.Row, order.CustomerOrderNbr, exception);
		}
