...
namespace PX.Objects.SO
{
	...
	public class SOInvoiceTermsAttribute : TermsAttribute
	{
		public SOInvoiceTermsAttribute()
			: base(typeof(ARInvoice.docDate), typeof(ARInvoice.dueDate), typeof(ARInvoice.discDate), null, null, typeof(ARInvoice.curyTaxTotal))
		{
		}
	...
	public class SOTaxAttribute : TaxAttribute
	{
		...
		protected override List<object> SelectDocumentLines(PXGraph graph, object row)
		{
			...
			{
				ret.Add(new SOLine { OrderType = order.OrderType, OrderNbr = order.OrderNbr, LineNbr = 32000, TaxCategoryID = order.FreightTaxCategoryID, CuryExtPrice = order.CuryFreightTot, ExtPrice = order.FreightTot, CuryDiscAmt = 0, DiscAmt = 0, GroupDiscountRate = 1, DocumentDiscountRate = 1 });
			}
			return ret;
		}
		...
		{
			bool calc = true;
			if (SkipInternalTaxCalculation(sender, e))
				calc = false;
		...
		private bool SkipInternalTaxCalculation(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			string taxZoneID = (string)sender.GetValue(e.Row, _TaxZoneID);
			return IsExternalTax(sender.Graph, taxZoneID) || (e.Row != null && ((IsManualExternalTax(sender.Graph, taxZoneID) && ((SOOrder)e.Row).ExternalTaxesImportInProgress == true) || ((SOOrder)e.Row).DisableAutomaticTaxCalculation == true));
		}
