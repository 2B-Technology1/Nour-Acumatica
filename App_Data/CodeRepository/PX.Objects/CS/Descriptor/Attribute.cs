...
namespace PX.Objects.CS
{
	...
	public class BatchDocumentList<T, BatchField> : DocumentList<T>
		where T : class, IBqlTable
		where BatchField: IBqlField
	{
		public int BatchSize = 500;

		public BatchDocumentList(PXGraph Graph)
			: base(Graph)
		{
		}
		...
		protected override object GetValue(object data, Type field)
		{
			int? currentSize = _Graph.Caches[typeof(T)].GetValue<BatchField>(data) as int?;
			return currentSize < BatchSize || WebConfig.ParallelProcessingDisabled
				? base.GetValue(data, field)
				: null;
		}
	}
			...
			sender.Graph.OnBeforeCommit += (graph) =>
				...
				foreach (var row in sender.Inserted.Concat_(sender.Updated))
				{
					if ((int?)sender.GetValue(row, _FieldName) < 0 &&
						graph.Views.Caches.Contains(_RecordType))
					{
						var displayName = graph.Caches[_RecordType].DisplayName;
						var fieldName = !string.IsNullOrEmpty(displayName)
							? $"{PXUIFieldAttribute.GetDisplayName(sender, _FieldName)} ({displayName})"
							: PXUIFieldAttribute.GetDisplayName(sender, _FieldName);

						throw new PXException(Messages.NegativeValuedIdentifier, fieldName);
					}
				}
			};
		}
	...
	public abstract class ContactAttribute : SharedRecordAttribute
	{
		...
		public override void CacheAttached(PXCache sender)
		{
			...
			sender.Graph.OnBeforeCommit += (graph) => {
				foreach (var row in sender.Inserted)
				{
					if ((Convert.ToInt64(sender.GetValue(row, _FieldName)) < 0L) &&
						graph.Views.Caches.Contains(_RecordType))
					{
						var displayName = graph.Caches[_RecordType].DisplayName;
						var fieldName = !string.IsNullOrEmpty(displayName)
	...
	public abstract class SharedRecordAttribute : PXEventSubscriberAttribute, IPXRowInsertedSubscriber, IPXRowPersistingSubscriber, IPXRowPersistedSubscriber, IPXRowSelectedSubscriber, IPXRowDeletedSubscriber
	{
		...
		protected Dictionary<object, object> _persistedItems = null;
	...
	public class AutoNumberAttribute : PXEventSubscriberAttribute, IPXFieldDefaultingSubscriber, IPXFieldVerifyingSubscriber, IPXRowInsertingSubscriber, IPXRowPersistingSubscriber, IPXRowPersistedSubscriber, IPXFieldSelectingSubscriber
	{
		...
		private void getfields(PXCache sender, object row)
		{
			...
			if (_Select != null)
			{
				if(sender.Graph.Caches[_setupType].Keys.Count == 0)
				{
					Type Tsetup = BqlCommand.Compose(typeof(PXSetup<>), _setupType);
					...
					{
						_numberingID = (string)sender.Graph.Caches[_setupType].GetValue(
							PXResult.Unwrap(source[0], _setupType),
							_setupField);
					}
				}
				else
		...
		protected virtual void Parameter_CommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
		{
			...
			{
				//condition is expected to be false for "<NEW>" and true for " <NEW>"
        ...
        protected static string GetNextNumberInt(PXCache sender, object data, string numberingID, DateTime? dateTime, string lastAssigned, out string LastNbr, out string WarnNbr, out int? NumberingSEQ)
		{
			if (numberingID != null && dateTime != null)
			{
				...
				else if (lastAssigned != null && lastAssigned.CompareTo(newNumber) > 0)
				{
					newNumber = NextNumber(lastAssigned, sequence.NbrStep ?? 0);
				}
		...
		public static bool TryToGetNextNumber(string str, int count, out string nextNumber)
		{
			try
			{
				nextNumber = NextNumber(str, count);
				return true;
			}
		...
		public static bool TryToGetNextNumber(string str, out string nextNumber) => TryToGetNextNumber(str, 1, out nextNumber);

		public static bool TryToGetNextNumber(string str)
		{
			string nextNumber;
			return TryToGetNextNumber(str, out nextNumber);
		}

		public static bool CheckIfNumberEndsDigit(string str) => string.IsNullOrEmpty(str) ? false : char.IsDigit(str.Last());
		...
		public static string NextNumber(string str) => NextNumber(str, silent: true);
	...
	public class TermsAttribute : PXEventSubscriberAttribute, IPXFieldUpdatedSubscriber, IPXRowSelectedSubscriber
	{
		...
		public TermsAttribute(Type DocDate, Type DueDate, Type DiscDate, Type CuryDocBal, Type CuryDiscBal, Type CuryTaxTotal)
		{
			_DocDate = DocDate;
			_DueDate = DueDate;
			_DiscDate = DiscDate;
		...
		public override void CacheAttached(PXCache sender)
		{
			...
			if (_CuryDocBal != null)
			{
				if (sender.Graph.IsContractBasedAPI)
					sender.Graph.FieldUpdated.AddHandler(BqlCommand.GetItemType(_CuryDocBal), _CuryDocBal.Name, SetCuryDiscBalBefore1stBalanceUpdate);
			...
			{
				if (sender.Graph.IsContractBasedAPI)
					sender.Graph.RowPersisting.AddHandler(BqlCommand.GetItemType(_CuryDiscBal), (cache, args) =>
					{
						decimal? discBalValue = _CuryDiscBalBefore1stBalanceUpdate ?? (decimal?)cache.GetValue(args.Row, _CuryDiscBal.Name);
						if (discBalValue == 0m) return;
						else
						{
							PXFieldVerifyingEventArgs fieldVerifyingArgs = new PXFieldVerifyingEventArgs(args.Row, discBalValue, true);
							VerifyDiscount(cache, fieldVerifyingArgs);
							cache.SetValue(args.Row, _CuryDiscBal.Name, fieldVerifyingArgs.NewValue);
						}
					});
				else
		...
		private void SetCuryDiscBalBefore1stBalanceUpdate(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			{
				decimal? currentCuryDiscBal = (decimal?)sender.GetValue(e.Row, _CuryDiscBal.Name);
				_CuryDiscBalBefore1stBalanceUpdate = currentCuryDiscBal == 0 ? null : currentCuryDiscBal;
			}
			sender.Graph.FieldUpdated.RemoveHandler(BqlCommand.GetItemType(_CuryDocBal), _CuryDocBal.Name, SetCuryDiscBalBefore1stBalanceUpdate);
		}
		...
		public virtual void RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			if (_CuryDiscBal == null) return;

			Terms terms = SelectTerms(sender.Graph, (string)sender.GetValue(e.Row, _FieldName));
			if (terms != null)
		...
		protected virtual void VerifyDiscount(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			{
				CalcDisc(sender, new PXFieldUpdatedEventArgs(e.Row, null, true));

				decimal? CuryDocBal = (decimal?)sender.GetValue(e.Row, _CuryDocBal.Name);
				decimal? CuryDiscBal = (decimal?)sender.GetValue(e.Row, _CuryDiscBal.Name);
		...
		protected virtual void CalcDisc(PXCache sender, PXFieldUpdatedEventArgs e)
		{
				...
				!sender.Graph.IsCopyPasteContext)
			{
				...
				AR.Standalone.ARCashSale aRCashSale = e.Row as AR.Standalone.ARCashSale;
				AR.ARInvoice aRInvoice = e.Row as AR.ARInvoice;
				bool cashType = (aRCashSale?.DocType == ARDocType.CashSale || aRCashSale?.DocType == ARDocType.CashReturn)
						|| (aRInvoice?.DocType == ARDocType.CashSale || aRInvoice?.DocType == ARDocType.CashReturn);

				if (!cashType)
				{
					bool localizationEnabled = false;

					if (!string.IsNullOrEmpty(sender.Graph.PrimaryView))
					{
						localizationEnabled = LocalizationServiceExtensions.LocalizationEnabled<FeaturesSet.canadianLocalization>(sender.Graph);
					}
					else if (sender.Current != null && sender.Current.GetType() == typeof(ARInvoice))
					{
						if ((sender.Current as ARInvoice).BranchID != null)
						{
							localizationEnabled = LocalizationServiceExtensions.LocalizationEnabled<FeaturesSet.canadianLocalization>(
								OrganizationLocalizationHelper.GetCurrentLocalizationCodeForBranch((sender.Current as ARInvoice).BranchID));
						}
					}

					if (_CuryTaxTotal != null &&
						sender.GetValue(e.Row, _CuryTaxTotal.Name) != null && localizationEnabled)
					{
						CuryDocBal = (decimal)sender.GetValue(e.Row, _CuryDocBal.Name) - (decimal)sender.GetValue(e.Row, _CuryTaxTotal.Name);
					}
				}
			...
			if (docDate != null && terms != null)
			{
				...
				switch (terms.DueType)
				{
					...
					case TermsDueType.FixedNumberOfDaysPlusDayOfNextMonth:
						...
						break;
					case TermsDueType.ByDecadesInNextMonth:
						dueDate = DocDate.AddMonths(1).SetDateByEndOfDecade();
						break;
					default:
						break;
				}
				...
				switch (terms.DiscType)
				{
					...
					case TermsDueType.DayOfTheMonth:
						...
						if (terms.DueType == TermsDueType.DayOfNextMonth && DocDate.Day <= (int)terms.DayDue00)
                            ...
                            monthShift = DocDate.Day >= (int)terms.DayDisc ? 1 : 0;
						else if(terms.DueType == TermsDueType.ByDecadesInNextMonth || terms.DueType == TermsDueType.DayOfNextMonth)
							monthShift = DocDate.Day > (int)terms.DayDisc ? 1 : 0;
                        else
		...
		public static Terms SelectTerms(PXGraph graph, string TermsID)
		{
			...
			else return (Terms)PXSelect<Terms, Where<Terms.termsID, Equal<Required<Terms.termsID>>>>.Select(graph, TermsID);
		}
	...
	#region SalesTerritoryTypeAttribute

	[AttributeUsage(AttributeTargets.Property, AllowMultiple = false)]
	public class SalesTerritoryTypeAttribute : PXStringListAttribute, IPXFieldVerifyingSubscriber
	{
		public const string ByCountry = "C";
		public const string ByState = "S";
		public const string Other = "O";
		public SalesTerritoryTypeAttribute()
			: base(
				new string[] { ByCountry, ByState, Other },
				new string[] { Messages.SalesTerritoryTypeByCountry, Messages.SalesTerritoryTypeByState, Messages.SalesTerritoryTypeOther })
		{ }

		public class byCountry : PX.Data.BQL.BqlString.Constant<byCountry>
		{
			public byCountry() : base(ByCountry) {; }
		}
		public class byState : PX.Data.BQL.BqlString.Constant<byState>
		{
			public byState() : base(ByState) {; }
		}
		...
		{
			if (Other.Equals(e.NewValue))
			{
				sender.RaiseExceptionHandling(
							FieldName, e.Row, null,
							new PXSetPropertyException(Messages.SalesTerritoryOtherTypeWarning, PXErrorLevel.Warning));
			}
		}
		#endregion
	}
