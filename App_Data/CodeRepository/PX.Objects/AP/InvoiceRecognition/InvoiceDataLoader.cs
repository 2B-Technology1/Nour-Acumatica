...
namespace PX.Objects.AP.InvoiceRecognition
{
	internal class InvoiceDataLoader
	{
		...
		private void LoadPrimary(object row)
		{
			...
			if (_vendorId != null)
			{
				SetExtValue(_graph.Document.Cache, row, nameof(APRecognizedInvoice.VendorID), _vendorId, showExtErrorValue: true);
			}
		...
		private static void SetExtValue(PXCache cache, object row, string fieldName, object extValue, bool showExtErrorValue = false)
		{
			try
			{
			...
			{
				showExtErrorValue &= e.ErrorValue == null;
				var errorValue = e.ErrorValue ?? extValue;

				if (showExtErrorValue)
				{
					cache.RaiseFieldSelecting(fieldName, row, ref errorValue, false);
					errorValue = PXFieldState.UnwrapValue(errorValue);
				}

				cache.RaiseExceptionHandling(fieldName, row, errorValue, e);
			}
		}
