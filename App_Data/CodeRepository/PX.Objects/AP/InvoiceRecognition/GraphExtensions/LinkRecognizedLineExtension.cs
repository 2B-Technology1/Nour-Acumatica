...
namespace PX.Objects.AP.InvoiceRecognition
{
	...
	public class LinkRecognizedLineExtension : PXGraphExtension<APInvoiceRecognitionEntry>
	{
		...
		private PXResultset<LinkLineOrder> GetLinkLineOrders(string pONbr, int? inventoryID, string uOM, int? vendorLocationID)
		{
			PXSelectBase<LinkLineOrder> cmd = new PXSelect<LinkLineOrder,
				Where2<
					...
					And2<Where<LinkLineOrder.uOM, Equal<Required<APRecognizedTran.uOM>>,
						Or<Required<APRecognizedTran.uOM>, IsNull>>,
					And<LinkLineOrder.orderCuryID, Equal<Required<APRecognizedInvoice.curyID>>>>>>>(Base);
			...
			return cmd.Select(pONbr, pONbr, inventoryID, uOM, uOM, Base.Document.Current?.CuryID, vendorLocationID);
		}
