...
namespace PX.Objects.AP
{
	...
	public class APPriceWorksheetMaint : PXGraph<APPriceWorksheetMaint, APPriceWorksheet>, PXImportAttribute.IPXPrepareItems, PXImportAttribute.IPXProcess
	{
        ...
        [PXFilterable]
        [PXCopyPasteHiddenView]
        public APAddItemLookup<APAddItemSelected, AddItemFilter> addItemLookup;
        ...
        public virtual IEnumerable AddSelItems(PXAdapter adapter)
        {
            ...
            foreach (APAddItemSelected line in addItemLookup.Cache.Cached)
            {
		...
		public virtual IEnumerable AddAllItems(PXAdapter adapter)
		{
			...
			foreach (APAddItemSelected line in addItemLookup.Select())
			{
				InsertOrUpdateARAddItem(line);
			}
		...
		protected virtual void InsertOrUpdateARAddItem (APAddItemSelected line)
		{
			...
			var salesPrices =
			   ...
			   .SelectMultiBound(this, null, priceCode, line.InventoryID, addItemParameters.Current.CuryID,
					Document.Current.EffectiveDate, Document.Current.EffectiveDate, Document.Current.EffectiveDate).ToList();
       ...
       protected virtual decimal AdjustByUOM(InventoryItem inventoryItem, APPriceWorksheetDetail worksheetDetail, decimal correctedAmtInBaseUnit)
		...
		private decimal GetItemPrice(int? inventoryID, string toCuryID, DateTime? curyEffectiveDate, string uom = null)
        {
		...
		protected virtual decimal? GetItemPriceFromInventoryItem(InventoryItem inventoryItem, string uom)
        {
			...
			if (inventoryItem.StkItem == false || inventoryItem.ValMethod == INValMethod.Standard)
			{
				var itemCurySettngs = InventoryItemCurySettings.PK.Find(this, inventoryItem?.InventoryID, Accessinfo.BaseCuryID);

				lastCost = itemCurySettngs.StdCost;
			}
			else
			{
				INItemCost itemCost = INItemCost.PK.Find(this, inventoryItem.InventoryID, Accessinfo.BaseCuryID);
			...
			{
				return INUnitAttribute.ConvertToBase(Caches[typeof(InventoryItem)], inventoryItem.InventoryID, uom, lastCost ?? 0m, INPrecision.NOROUND);
			}
        }
        ...
        protected virtual void APPriceWorksheetDetail_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
        {
            ...
            {
				det.CurrentPrice = GetItemPrice(det.InventoryID, det.CuryID, Document.Current.EffectiveDate, det.UOM);
            }
        }
		...
		protected virtual Type[] GetAlternativeKeyFields()
		{
			...
			if (PXAccess.FeatureInstalled<FeaturesSet.supportBreakQty>())
				types.Add(typeof(APPriceWorksheetDetail.breakQty));
	...
	// Acuminator disable once PX1094 NoPXHiddenOrPXCacheNameOnDac [Legacy code. Renamed class, prefix changed from AR to AP]
	...
	public partial class APAddItemSelected : IBqlTable
    {
        #region Selected
        public abstract class selected : PX.Data.BQL.BqlBool.Field<selected> { }
        protected bool? _Selected = false;
		// Acuminator disable once PX1030 PXDefaultIncorrectUse [Legacy code. Renamed class name prefix from AR to AP]
        [PXBool]
        [PXDefault(false)]
		...
		// Acuminator disable once PX1030 PXDefaultIncorrectUse [Legacy code. Renamed class, prefix changed from AR to AP]
        ...
        where StatusFilter : AddItemFilter, new()
    {
		...
		protected override PXView CreateIntView(PXGraph graph)
		{
			...
			view.WhereAnd<Where<Current<AddItemFilter.myWorkGroup>, Equal<boolFalse>,
						 Or<APAddItemSelected.productWorkgroupID, TM.IsWorkgroupOfContact<CurrentValue<AddItemFilter.currentOwnerID>>>>>();
