...
namespace PX.Objects.AP
{
	...
	[PXProjection(typeof(
			SelectFrom<APInvoice>
				.InnerJoin<Location>
					.On<Location.bAccountID.IsEqual<APInvoice.vendorID>
					.And<Location.locationID.IsEqual<APInvoice.vendorLocationID>>>
				.InnerJoin<LocationAPPaymentInfo>
					.On<LocationAPPaymentInfo.bAccountID.IsEqual<Location.bAccountID>
					.And<LocationAPPaymentInfo.locationID.IsEqual<Location.vPaymentInfoLocationID>>>
				.LeftJoin<PaymentMethodAccount>
					.On<PaymentMethodAccount.paymentMethodID.IsEqual<LocationAPPaymentInfo.vPaymentMethodID>
					.And<PaymentMethodAccount.branchID.IsEqual<APInvoice.branchID>
					.And<PaymentMethodAccount.useForAP.IsEqual<True>
					.And<PaymentMethodAccount.aPIsDefault.IsEqual<True>>>>>
				.LeftJoin<CashAccount>
					.On<CashAccount.cashAccountID.IsEqual<APInvoice.payAccountID>
					.Or<APInvoice.payAccountID.IsNull.And<CashAccount.cashAccountID.IsEqual<LocationAPPaymentInfo.vCashAccountID>>>
					.Or<APInvoice.payAccountID.IsNull.And<LocationAPPaymentInfo.vCashAccountID.IsNull
						.And<CashAccount.cashAccountID.IsEqual<PaymentMethodAccount.cashAccountID>>>>>
				.InnerJoin<CurrencyInfo>
					.On<CurrencyInfo.curyInfoID.IsEqual<APInvoice.curyInfoID>>
				.LeftJoin<APAdjust>
					.On<APAdjust.adjdDocType.IsEqual<APInvoice.docType>
					.And<APAdjust.adjdRefNbr.IsEqual<APInvoice.refNbr>
					.And<APInvoice.docType.IsEqual<APDocType.prepayment>>>>
				.Where<APInvoice.openDoc.IsEqual<True>
					.And<APInvoice.released.IsEqual<True>.Or<APInvoice.prebooked.IsEqual<True>>>
					.And<APInvoice.dueDate.IsNotNull>
					.And<CashAccount.cashAccountID.IsNotNull>
					.And<APAdjust.curyAdjdAmt.IsNull.Or<APAdjust.curyAdjdAmt.IsLess<APInvoice.curyOrigDocAmt>>>
					.And<APInvoice.docBal.IsNotEqual<decimal0>>>
		))]
    [Serializable]
	public partial class APCashRequirementsReport : PX.Data.IBqlTable
	{
		...
		/// <summary>
		/// The cash discount balance of the invoice.
		/// </summary>
		protected Decimal? _DiscBal;
		[PXDBBaseCury(BqlField = typeof(APInvoice.discBal))]
		public virtual Decimal? DiscBal
		...
		#endregion
		#region InvoicePayTypeID
		public abstract class invoicePayTypeID : PX.Data.BQL.BqlString.Field<invoicePayTypeID> { }
