...
{ 
	public class APInvoiceFillFromRecognizedScope : FlaggedModeScopeBase<APInvoiceFillFromRecognizedScope> { }
	...
	public partial class APInvoiceEntry : APDataEntryGraph<APInvoiceEntry, APInvoice>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		[PXFormula(typeof(
			IIf<Where<Current<APInvoice.docType>, NotEqual<APDocType.prepayment>,
		...
		public virtual IEnumerable VendorRefund(PXAdapter adapter)
		{
			if (Document.Current != null && (bool)Document.Current.Released && Document.Current.DocType == APDocType.DebitAdj && (bool)Document.Current.OpenDoc)
			{
				...
				{
					pe.Clear();
					pe.CreatePayment(Document.Current, APDocType.Refund);
				}
		...
		protected virtual bool AskUserApprovalIfReversingDocumentAlreadyExists(APInvoice origDoc)
		{
			...
			if (reversingDoc != null)
			{
				string localizedMsg = PXMessages.LocalizeFormatNoPrefix(
					...
					APDocType.GetDisplayName(reversingDoc.DocType),
					reversingDoc.RefNbr);
		...
		[APMigrationModeDependentActionRestriction(
			...
			restrictForRegularDocumentInMigrationMode: true,
			restrictForUnreleasedMigratedDocumentInNormalMode: true)]
		[PXActionRestriction(typeof(Where<Current<APInvoice.isRetainageReversing>, Equal<True>,
			And<Where<Current<APInvoice.isRetainageDocument>, Equal<True>, Or<Current<APInvoice.retainageApply>, Equal<True>>>>>), Messages.ReverseRetainageReversingDocument)]
		public virtual IEnumerable ReverseInvoice(PXAdapter adapter)
		{
			...
			bool docTypeProhibitsReversal =
				origDocType != APDocType.Invoice &&
				origDocType != APDocType.CreditAdj &&
				origDocType != APDocType.DebitAdj;
					...
					origDoc.IsChildRetainageDocument())
			{
				...
				{
					InventoryItem item = InventoryItem.PK.Find(this, itr.InventoryID);
					if (item?.IsConverted == true && itr.IsStockItem != null && itr.IsStockItem != item.StkItem)
						continue;
			...
			if (origDoc.DocType == APDocType.DebitAdj)
			{
				if (isPOReferenced || Transactions.Select().RowCast<APTran>().Any(_ => !string.IsNullOrEmpty(_.LCRefNbr)))
					throw new PXException(Messages.DebitAdjustmentReferecesPOOrderOrPOReceipt);
		...
		public virtual IEnumerable PayInvoice(PXAdapter adapter)
		{
			if (Document.Current != null && (Document.Current.Released == true || Document.Current.Prebooked == true))
			{
				...
				bool isPayByLineDebitAdj = Document.Current.DocType == APDocType.DebitAdj && Document.Current.PaymentsByLinesAllowed == true;

				if (Document.Current.OpenDoc == true && (Document.Current.Payable == true || Document.Current.DocType == APInvoiceType.Prepayment || isPayByLineDebitAdj))
				{
					...
					{
						pe.Clear();
						pe.CreatePayment(Document.Current, APDocType.Check);
					}
					throw new PXRedirectRequiredException(pe, "PayInvoice");
				}
		...
		public SelectFrom<POOrder>.
			InnerJoin<APTran>.On<APTran.FK.POOrder>.
			Where<APTran.FK.Invoice.SameAsCurrent>.View POOrderForApproval;
		...
		public virtual void ReverseInvoiceProc(APRegister doc)
		{
			...
			foreach (PXResult<APInvoice, CurrencyInfo, Terms, Vendor> res in APInvoice_CurrencyInfo_Terms_Vendor.Select(this, (object)doc.DocType, doc.RefNbr))
			{
				...
				invoice.DocType = invoice.DocType == APDocType.DebitAdj
					? APDocType.CreditAdj
					: APDocType.DebitAdj;
				invoice.RefNbr = null;
				...
				if (APSetup.Current?.TermsInDebitAdjustments == false)
				{
				...
				invoice.Hold = (apsetup.Current.HoldEntry ?? false) || IsApprovalRequired(invoice);
			...
			foreach (APTran srcTran in PXSelect<APTran, Where<APTran.tranType, Equal<Required<APTran.tranType>>, And<APTran.refNbr, Equal<Required<APTran.refNbr>>>>>.Select(this, doc.DocType, doc.RefNbr))
			{
				...
				bool isConverted = (item?.IsConverted == true && srcTran.IsStockItem != null && srcTran.IsStockItem != item.StkItem);
				if (isConverted)
				{
					tran.LineType = null;

					tran.POAccrualLineNbr = null;
					tran.POAccrualRefNoteID = null;
					tran.POAccrualType = null;

					tran.POOrderType = null;
					tran.PONbr = null;
					tran.POLineNbr = null;

					tran.ReceiptType = null;
					tran.ReceiptNbr = null;
					tran.ReceiptLineNbr = null;
			...
			if (!IsExternalTax(Document.Current.TaxZoneID))
			{
				...
				OldNewTaxTranPair<APTaxTran>[] taxes = PXSelect<APTaxTran,
				...
				foreach (OldNewTaxTranPair<APTaxTran> taxPair in taxes)
				{
					APTaxTran tax = taxPair.OldTaxTran;
		...
		public APInvoiceEntry()
		{
			...
			APTaxAttribute.IncludeDirectTaxLine<APTran.taxCategoryID>(AllTransactions.Cache, null, true);
		}
		...
		{
			if (viewName.Equals(nameof(Document), StringComparison.InvariantCultureIgnoreCase))
			{
				values[nameof(APRegister.CuryDocBal)] = PXCache.NotSetValue;
			}
		...
		public override void Persist()
		{
			...
			foreach (APInvoice apdoc in Document.Cache.Cached)
			{
				...
				{
					PXUpdate<Set<APAdjust.pPDVATAdjRefNbr, Null,
						Set<APAdjust.pPDVATAdjDocType, Null>>, APAdjust,
						Where<APAdjust.pendingPPD, Equal<True>,
							And<APAdjust.pPDVATAdjRefNbr, Equal<Required<APAdjust.pPDVATAdjRefNbr>>>>>
			...
			//Taxes can be imported as-is, ignoring all the validation rules when certain conditions are met. See APInvoiceEntryExternalTaxImport extension.
			InsertImportedTaxes();
					...
					foreach (PXResult<Standalone.APRegisterAlias, CurrencyInfo, Branch, APAdjust, APPayment> res in 
						PXSelectJoin<
							...
							Where<
								...
								And<Standalone.APRegisterAlias.released, Equal<True>, 
								And<Standalone.APRegisterAlias.openDoc, Equal<True>,
								And<Standalone.APRegisterAlias.paymentsByLinesAllowed, Equal<False>,
		...
		protected virtual void APInvoice_PaymentsByLinesAllowed_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if ((bool?)e.OldValue == true && document.PaymentsByLinesAllowed != true)
			{
				...
				sender.RaiseExceptionHandling<APInvoice.curyOrderDiscTotal>(document, document.CuryOrderDiscTotal, null);
		...
		protected virtual void _(Events.FieldUpdated<APInvoice, APInvoice.retainageApply> e)
		{
			APInvoice document = e.Row as APInvoice;
			if (document == null) return;
			...
			{
				e.Cache.RaiseExceptionHandling<APInvoice.curyDiscTot>(document, document.CuryDiscTot, null);
				e.Cache.RaiseExceptionHandling<APInvoice.curyOrderDiscTotal>(document, document.CuryOrderDiscTotal, null);
			}
		}
		...
		protected virtual bool IsAllowWithholdingTax(PXCache sender, APInvoice doc) => !(doc.DocType == APDocType.Prepayment && HasWithholdingTax(sender, doc));

		protected virtual bool HasWithholdingTax(PXCache sender, APInvoice doc)
			{
			APTaxTran withholdingTaxes = PXSelectJoin<APTaxTran,
								InnerJoin<Tax, 
		...
		protected virtual void APInvoice_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if (HasTermsWithMultipleInstallments(sender, doc) && HasWithholdingTax(sender, doc))
			{
				throw new PXRowPersistingException(nameof(APInvoice), doc, Messages.WithholdingTaxesInMultipleInstallmentDocumentError);
			}
			...
			if (doc.CuryDiscTot != 0.0m && Transactions.Select().Where(a => ((APTran)a).IsDirectTaxLine != true).Count() == 0)
			{
				sender.RaiseExceptionHandling<APInvoice.curyDiscTot>(e.Row, doc.CuryDiscTot,
					new PXSetPropertyException(Messages.CanNotHaveDiscountForDirectTaxLineOnly, PXErrorLevel.Error));
			}
		}
		...
		protected virtual void APInvoice_DocDesc_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			{
				aPTaxTran.Description = invoice.DocDesc;
		...
		protected virtual void APInvoice_TermsID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			var invoiceState = GetDocumentState(sender, document);
			if (apsetup.Current.TermsInDebitAdjustments == true && invoiceState.IsDocumentDebitAdjustment && terms == null)
			{
		...
		public virtual APInvoiceState GetDocumentState(PXCache cache, APInvoice document)
        {
            ...
            {
                result.DontApprove = !IsApprovalRequired(document);
            }

            result.HasPOLink = IsPOLinkedAPBill.Ensure(cache, document);

			result.PaymentsByLinesAllowed = document.PaymentsByLinesAllowed == true;
			...
			result.IsRetainageReversing = (document.IsOriginalRetainageDocument() || document.IsChildRetainageDocument()) && document.IsRetainageReversing == true;

			result.IsRetainageApplyDocument =
				(result.IsDocumentInvoice || result.IsDocumentDebitAdjustment) &&
				document.OrigModule == BatchModule.AP &&
				!result.IsDocumentReleasedOrPrebooked &&
				!result.IsRetainageDocument ||
				result.RetainageApply;
			...
			result.IsAssignmentEnabled = !result.IsDocumentReleasedOrPrebooked && !result.IsDocumentVoided && !result.IsDocumentRejectedOrPendingApproval && !result.IsDocumentApprovedBalanced;
        ...
        protected virtual void APInvoice_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetRequired<APInvoice.dueDate>(cache, !invoiceState.IsDocumentDebitAdjustment
				|| (invoiceState.IsDocumentDebitAdjustment && apsetup.Current.TermsInDebitAdjustments == true && document.TermsID != null));
			...
			if (invoiceState.IsDocumentReleasedOrPrebooked || invoiceState.IsDocumentVoided)
			{
				...
				PXUIFieldAttribute.SetEnabled<APInvoice.dueDate>(cache, document,
					...
					|| (invoiceState.IsDocumentDebitAdjustment && document.TermsID != null))
					&& document.OpenDoc == true && document.PendingPPD != true);
				...
				PXUIFieldAttribute.SetEnabled<APInvoice.isJointPayees>(cache, document, document.OpenDoc == true);
				PXUIFieldAttribute.SetEnabled<APInvoice.payDate>(cache, document,
					(!invoiceState.IsDocumentDebitAdjustment
					|| (invoiceState.IsDocumentDebitAdjustment && document.TermsID != null))
					&& document.OpenDoc == true);
				PXUIFieldAttribute.SetEnabled<APInvoice.discDate>(cache, document,
					(!invoiceState.IsDocumentDebitAdjustment
					|| (invoiceState.IsDocumentDebitAdjustment && document.TermsID != null))
					&& !invoiceState.IsDocumentPrepayment && document.OpenDoc == true && document.PendingPPD != true);
			...
			else if (invoiceState.IsRetainageReversing)
			{
				...
				{
					PXUIFieldAttribute.SetEnabled<APInvoice.invoiceNbr>(cache, document, true);
				}
				DiscountDetails.Cache.SetAllEditPermissions(allowEdit: false);
			}
			else
			{
				...
				PXUIFieldAttribute.SetEnabled<APInvoice.termsID>(cache, document,
					(!invoiceState.IsDocumentDebitAdjustment || apsetup.Current.TermsInDebitAdjustments == true) && !invoiceState.IsDocumentPrepayment);
				PXUIFieldAttribute.SetEnabled<APInvoice.dueDate>(cache, document, !invoiceState.IsDocumentDebitAdjustment
					|| (invoiceState.IsDocumentDebitAdjustment && document.TermsID != null));
				PXUIFieldAttribute.SetEnabled<APInvoice.discDate>(cache, document,
					(!invoiceState.IsDocumentDebitAdjustment || (invoiceState.IsDocumentDebitAdjustment && document.TermsID != null))
					&& !invoiceState.IsDocumentPrepayment);
				...
				PXUIFieldAttribute.SetEnabled<APInvoice.curyDiscTot>(cache, document,
				...
				PXUIFieldAttribute.SetEnabled<APInvoice.retainageApply>(cache, document, !invoiceState.IsDocumentCreditAdjustment);
			...
			{
				PXUIFieldAttribute.SetEnabled<APInvoice.employeeWorkgroupID>(cache, document, invoiceState.IsAssignmentEnabled);
				PXUIFieldAttribute.SetEnabled<APInvoice.employeeID>(cache, document, invoiceState.IsAssignmentEnabled);
			}
			...
			Taxes.Cache.AllowDelete = Transactions.Cache.AllowDelete && !invoiceState.IsRetainageReversing && !invoiceState.IsRetainageDocument &&
				!invoiceState.IsDocumentReleasedOrPrebooked;
			Taxes.Cache.AllowInsert = Transactions.Cache.AllowInsert && !invoiceState.IsRetainageReversing && !invoiceState.IsRetainageDocument &&
				!invoiceState.IsDocumentReleasedOrPrebooked;
			Taxes.Cache.AllowUpdate = Transactions.Cache.AllowUpdate && !invoiceState.IsRetainageReversing && !invoiceState.IsRetainageDocument &&
				!invoiceState.IsDocumentReleasedOrPrebooked;

			Adjustments.Cache.AllowInsert = false;
			Adjustments.Cache.AllowDelete = false;
			Adjustments.Cache.AllowUpdate = !invoiceState.IsRetainageReversing &&
			...
			editVendor.SetEnabled(vendor?.Current != null && !invoiceState.IsRetainageReversing);
			...
			reclassifyBatch.SetEnabled(document.Released == true && !invoiceState.IsDocumentPrepayment && !invoiceState.IsRetainageReversing);
			...
			if (document.VendorLocationID != null && !invoiceState.IsRetainageReversing)
			{
			...
			Transactions.Cache.AllowDelete &= !invoiceState.IsRetainageDocument && !invoiceState.IsRetainageReversing && !IsTaxDocumentVendorNotBaseCuryID;
			Transactions.Cache.AllowInsert &= !invoiceState.IsRetainageDocument && !invoiceState.IsRetainageReversing && !IsTaxDocumentVendorNotBaseCuryID && !invoiceState.IsPrepaymentRequestFromPO;
			Transactions.Cache.AllowUpdate &= (!invoiceState.IsRetainageDocument || vendor1099) && !invoiceState.IsRetainageReversing && !IsTaxDocumentVendorNotBaseCuryID;
			...
			bool isSuppliedByVendorEnabled = !invoiceState.HasPOLink && document.Released != true && document.Prebooked != true && isPayToVendor && !invoiceState.IsRetainageReversing;
			...
			PXUIFieldAttribute.SetVisible<APInvoice.retainageApply>(cache, document, invoiceState.IsRetainageApplyDocument);
			PXUIFieldAttribute.SetVisible<APInvoice.isRetainageDocument>(cache, document, invoiceState.IsRetainageDocument);
			...
			PXUIFieldAttribute.SetVisible<APInvoice.isJointPayees>(cache, document, document.DocType == APDocType.Invoice);
			...
			if (PXAccess.FeatureInstalled<FeaturesSet.vATReporting>() &&
				document.CuryOrigDiscAmt > 0m &&
				(document.DocType != APDocType.DebitAdj
				|| (document.DocType == APDocType.DebitAdj && document.TermsID != null)))
			{
				Taxes.Select();
			...
			if (isPaymentsByLinesAllowed)
			{
				...
				{
					string errorMessage = PXAccess.FeatureInstalled<FeaturesSet.vendorDiscounts>() ? Messages.PaymentsByLinesDiscountsNotSupported :
						Messages.PaymentsByLinesDocumentDiscountsNotSupported;
			...
			{
				string errorMessage = PXAccess.FeatureInstalled<FeaturesSet.vendorDiscounts>() ? Messages.RetainageApplyDiscountsNotSupported :
						Messages.RetainageApplyDocumentDiscountsNotSupported;
				cache.RaiseExceptionHandling<APInvoice.curyDiscTot>(document, document.CuryDiscTot,
						new PXSetPropertyException(errorMessage, PXErrorLevel.Warning));
			}
		...
		protected virtual void APTran_SubID_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			if (documentLine == null || e.Cancel == true) return;

			InventoryItem item = nonStockItem.Select(documentLine.InventoryID);
			// Supress defaulting during paste or filling with stock item from recognized
			if (IsCopyPasteContext
				|| item?.StkItem == true
					&& APInvoiceFillFromRecognizedScope.IsActive
			...
			{
				PMTask task = PMTask.PK.FindDirty(this, documentLine.ProjectID, documentLine.TaskID);
				if (task != null)
		...
		protected virtual void _(Events.FieldVerifying<APTran.curyRetainageAmt> e)
		{
			APTran row = e.Row as APTran;
			if (row == null) return;

			decimal availableRetainageAmount = row.TranType == APDocType.Prepayment
				? (row.CuryPrepaymentAmt ?? 0m)
				: (row.CuryLineAmt ?? 0m) - (row.CuryDiscAmt ?? 0m);
		...
		protected virtual void APTran_ProjectID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			APInvoice doc = Document.Current;
			if(doc != null && doc.Released == true && doc.Status == APDocStatus.UnderReclassification && APSetup.Current.RequireSingleProjectPerDocument != true )
			{
				Document.Cache.SetValue<APInvoice.projectID>(Document.Current, ProjectDefaultAttribute.NonProject());
			}
		}
		...
		protected virtual void APTran_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			// reset the valus to with matches global level visibility check done in APInvoice Row selected event
			PXUIFieldAttribute.SetEnabled<APTran.retainagePct>(cache, row, Document.Current?.RetainageApply == true);
			PXUIFieldAttribute.SetEnabled<APTran.curyRetainageAmt>(cache, row, Document.Current?.RetainageApply == true);

			// rest values to enable state  
			PXUIFieldAttribute.SetEnabled<APTran.manualDisc>(cache, row, true);
			PXUIFieldAttribute.SetEnabled<APTran.curyDiscAmt>(cache, row, true);
			PXUIFieldAttribute.SetEnabled<APTran.discPct>(cache, row, true);
			PXUIFieldAttribute.SetEnabled<APTran.deferredCode>(cache, row, true);
			PXUIFieldAttribute.SetEnabled<APTran.discountID>(cache, row, true);
			...
			if (!isLCBasedTranAP)
			{
				...
				{
					PXUIFieldAttribute.SetEnabled<APTran.accountID>(cache, row, !isStockItem && !isDocumentReleased);
					PXUIFieldAttribute.SetEnabled<APTran.subID>(cache, row, !isStockItem && !isDocumentReleased);
				}
			}
			else
			{
			...
			PXUIFieldAttribute.SetEnabled<APTran.projectID>(cache, row, isProjectEditable && APSetup.Current.RequireSingleProjectPerDocument != true && !(isDocumentReleased && vendor.Current?.Vendor1099 == true));
			PXUIFieldAttribute.SetEnabled<APTran.taskID>(cache, row, isProjectEditable && !(isDocumentReleased && vendor.Current?.Vendor1099 == true));
			...
			if (doc.IsRetainageDocument == true && vendor.Current?.Vendor1099 == true)
			{
				PXUIFieldAttribute.SetEnabled(cache, row, false);
				PXUIFieldAttribute.SetEnabled<APTran.box1099>(cache, row, true);
			}
			...
			{
				PXUIFieldAttribute.SetEnabled<APTran.retainagePct>(cache, row, false);
				PXUIFieldAttribute.SetEnabled<APTran.curyRetainageAmt>(cache, row, false);
				PXUIFieldAttribute.SetEnabled<APTran.manualDisc>(cache, row, false);
				PXUIFieldAttribute.SetEnabled<APTran.curyDiscAmt>(cache, row, false);
				PXUIFieldAttribute.SetEnabled<APTran.discPct>(cache, row, false);
		...
		protected virtual void APTran_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			bool groupAndDocumentDiscountRecalculated = false;
			if (!sender.ObjectsEqual<APTran.isDirectTaxLine>(e.Row, e.OldRow))
			{
				if (row.LineType != APLineType.Discount && row.LineType != APLineType.LandedCostAP)
				{
					// Acuminator disable once PX1045 PXGraphCreateInstanceInEventHandlers [AC-101617 AC-254808:In order to re-calculate discounts on direct tax change]
					...
					groupAndDocumentDiscountRecalculated = true;
				}
			}
			...
			if ((!sender.ObjectsEqual<APTran.branchID>(e.Row, e.OldRow) || !sender.ObjectsEqual<APTran.inventoryID>(e.Row, e.OldRow) ||
					...
					!sender.ObjectsEqual<APTran.discountID>(e.Row, e.OldRow) || changedSuppliedByVendorLocation || groupAndDocumentDiscountRecalculated) 
		...
		protected virtual void APTaxTran_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			Tax tax = PXSelectorAttribute.Select<Tax.taxID>(sender, apTaxTran) as Tax;
		...
		public virtual void ProcessPOOrderLines(IEnumerable<IAPTranSource> orderlines, HashSet<APTran> duplicates = null, bool keepOrderTaxes = false)
		{
			...
			foreach (var orderline in orderlines)
			{
				...
				catch (PXException ex)
				{
					...
					isSubcontractLine = orderline.OrderType == POOrderType.RegularSubcontract;
				}
				finally
				{
					RowInserting.RemoveHandler<APTran>(handler);
			...
			if (failedToAddLine)
			{
				throw new PXException(isSubcontractLine ? PX.Objects.CN.Subcontracts.AP.Descriptor.Messages.FailedToAddSubcontractLinesError : PO.Messages.FailedToAddLine);
			}
		...
		public bool IsApprovalRequired(APInvoice doc)
	    {
			return EPApprovalSettings<APSetupApproval>.ApprovedDocTypes.Contains(doc.DocType) ||
			       (doc.DocType == APDocType.Prepayment &&
			        EPApprovalSettings<APSetupApproval>.ApprovedDocTypes.Contains(APDocType.PrepaymentRequest));
	    }
		...
		public virtual APTran AddPOReceiptLine(IAPTranSource aLine, HashSet<APTran> checkForDuplicates)
		{
			...
			if (deferralCode != null)
			{
				newtran.RequiresTerms = DoesRequireTerms(deferralCode);
			}
			...
			if (updTran == null)
			{
				...
				if (!keepAmounts && retainPct != 0m && !isSubcontractDebitAdj)
				{
					...
					Transactions.Cache.SetValueExt<APTran.retainagePct>(newtran, retainPct);
		...
		{
			return deferralCode.MultiDeliverableArrangement == false && DeferredMethodType.RequiresTerms(deferralCode.Method);
		}
		...
		public virtual void CheckProjectAccountRule(PXCache sender, APTran row)
		{
			...
			{
				Account account = Account.PK.Find(this, row.AccountID);
				if (account != null)
				{
		...
		public virtual APInvoice CreatePPDDebitAdj(APPPDVATAdjParameters filter, List<PendingPPDVATAdjApp> list)
		{
			bool firstApp = true;
			...
			debitAdj.CuryOrigDiscAmt = 0m;
			debitAdj.DiscDate = debitAdj.DueDate;
			debitAdj = Document.Update(debitAdj);
		...
		public virtual APInvoice CreatePPDCreditAdj(APPPDVATAdjParameters filter, List<PendingPPDVATAdjApp> list)
		{
			bool firstApp = true;
			APInvoice creditAdj = (APInvoice)Document.Cache.CreateInstance();
			...
			foreach (PendingPPDVATAdjApp doc in list)
			{
				if (firstApp)
				{
					...
					creditAdj.DocType = APDocType.CreditAdj;

					creditAdj = PXCache<APInvoice>.CreateCopy(Document.Insert(creditAdj));

					creditAdj.VendorID = doc.VendorID;
					creditAdj.VendorLocationID = doc.InvVendorLocationID;
					creditAdj.CuryInfoID = info.CuryInfoID;
					creditAdj.CuryID = info.CuryID;
					Vendor vendor = PXSelect<Vendor, Where<Vendor.bAccountID, Equal<Required<Vendor.bAccountID>>>>
						.Select(this, doc.VendorID);
					creditAdj.DocDesc = PXDBLocalizableStringAttribute.GetTranslation(Caches[typeof(APSetup)], APSetup.Current, nameof(AP.APSetup.pPDDebitAdjustmentDescr), vendor?.LocaleName);
					creditAdj.BranchID = doc.AdjdBranchID;
					creditAdj.DocDate = filter.GenerateOnePerVendor == true ? filter.DebitAdjDate : doc.AdjgDocDate;
					creditAdj.APAccountID = doc.AdjdAPAcct;
					creditAdj.APSubID = doc.AdjdAPSub;
					creditAdj.TaxZoneID = doc.InvTaxZoneID;
					creditAdj.PendingPPD = true;
					creditAdj.SuppliedByVendorLocationID = doc.InvVendorLocationID;
					creditAdj.Hold = false;
					creditAdj.TaxCalcMode = doc.InvTaxCalcMode;

					creditAdj = Document.Update(creditAdj);

					string masterPeriodID = filter.GenerateOnePerVendor == true
						 ? FinPeriodRepository.GetByID(filter.FinPeriodID, PXAccess.GetParentOrganizationID(filter.BranchID)).MasterFinPeriodID
						 : doc.AdjgTranPeriodID;
					FinPeriodIDAttribute.SetPeriodsByMaster<APInvoice.finPeriodID>(Document.Cache, creditAdj, masterPeriodID);

					if (creditAdj.TaxCalcMode != doc.InvTaxCalcMode)
					{
						creditAdj.TaxCalcMode = doc.InvTaxCalcMode;
						creditAdj = Document.Update(creditAdj);
					}
			...
			if (APSetup.Current.RequireControlTotal == true)
			{
				creditAdj.CuryOrigDocAmt = creditAdj.CuryDocBal;
				Document.Cache.Update(creditAdj);
			}

			if (APSetup.Current.RequireControlTaxTotal == true)
			{
				creditAdj.CuryTaxAmt = creditAdj.CuryTaxTotal;
				Document.Cache.Update(creditAdj);
			}

			autoApply.Press();

			foreach (var res in Adjustments.Select())
			{
				APAdjust adj = res;
				PendingPPDVATAdjApp pendingPPDDoc = list.Where(doc => doc.AdjdDocType == adj.AdjgDocType && doc.AdjdRefNbr == adj.AdjgRefNbr).FirstOrDefault();
				if (pendingPPDDoc != null)
				{
					adj.CuryAdjdAmt = pendingPPDDoc.InvCuryDocBal;
					adj = Adjustments.Update(adj);
				}
			}

			creditAdj.CuryOrigDiscAmt = 0m;
			creditAdj.DiscDate = creditAdj.DueDate;
			creditAdj = Document.Update(creditAdj);
	    ...
	    public virtual void AddPPDDebitAdjDetails(PendingPPDVATAdjApp doc, decimal? TaxTotal, decimal? InclusiveTotal, PXResultset<APTaxTran> taxes)
		{
			...
			{
				// Acuminator disable once PX1050 HardcodedStringInLocalizationMethod [Legacy code. DocType collects constants from APInvoiceType class. False alert]
				// Acuminator disable once PX1051 NonLocalizableString [Legacy code. DocType collects constants from APInvoiceType class. False alert]
