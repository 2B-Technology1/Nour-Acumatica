...
namespace PX.Objects.AP
{
	...
	[PXPrimaryGraph(
			...
			typeof(Select<
					AP.VendorR,
				Where2<
					Where<AP.VendorR.type, Equal<BAccountType.vendorType>,
						Or<AP.VendorR.type, Equal<BAccountType.combinedType>>>,
					And<AP.VendorR.bAccountID, Equal<Current<BAccount.bAccountID>>>>>),
			typeof(Select<
					EP.EPEmployee,
				Where<
	...
	public class VendorMaint : PXGraph<VendorMaint, VendorR>
	{
		...
		protected virtual void Vendor_TaxAgency_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			...
			if (vendor.TaxAgency != true && e.OldValue as bool? == true)
			{
				vendor.SalesTaxAcctID = null;
				vendor.PurchTaxAcctID = null;
				vendor.TaxExpenseAcctID = null;
				vendor.SalesTaxSubID = null;
				vendor.PurchTaxSubID = null;
				vendor.TaxExpenseSubID = null;
			}
		}
		...
		public class PaymentDetailsExt : PXGraphExtension<VendorMaint>
		{
			...
			protected virtual void _(Events.FieldSelecting<VendorPaymentMethodDetail.detailValue> e) => PaymentMethodDetailHelper.VendorDetailValueFieldSelecting(Base, e);
			#endregion
			...
			protected virtual void _(Events.RowPersisting<VendorPaymentMethodDetail> e)
			{
				...
				bool validationResult = ValidateLocationPaymentMethod(e.Cache, null, out errorMessage);
			...
			protected virtual bool ValidateLocationPaymentMethod(PXCache cache, CRLocation location, out string errorMessage)
			{
				bool res = true;
				errorMessage = null;
				VendorR acct = Base.BAccountAccessor.Current;
				if (acct != null && (acct.Type == BAccountType.VendorType || acct.Type == BAccountType.CombinedType))
				{
					foreach (VendorPaymentMethodDetail it in this.PaymentDetails.Select())
					{
						if (!ValidatePaymentDetail(it, out errorMessage))
						{
							res = false;
							break;
						}
			...
			protected virtual bool ValidatePaymentDetail(VendorPaymentMethodDetail aRow, out string errorMessage)
			{
				errorMessage = null;
				PaymentMethodDetail iTempl = this.FindTemplate(aRow);
				if (iTempl != null && (iTempl.IsRequired ?? false) && String.IsNullOrEmpty(aRow.DetailValue))
				{
					errorMessage = PXLocalizer.LocalizeFormat(CA.Messages.RequiredPaymentInstructionCanNotBeEmpty, iTempl.Descr);
					this.PaymentDetails.Cache.RaiseExceptionHandling<VendorPaymentMethodDetail.detailValue>(aRow, aRow.DetailValue, new PXSetPropertyException(CA.Messages.ERR_RequiredValueNotEnterd));
					return false;
				}
				return true;
						...
						if (paymentTypeDef != null)
						{
							...
							using (ReadOnlyScope rs = new ReadOnlyScope(PaymentDetails.Cache))
							{
								foreach (PaymentMethodDetail it in toAdd)
								{
									...
									{
										detail.DetailValue = it.DefaultValue;
									}
			...
			.WithCombinedTypeValidation
		{
			...
			public override void ValidateAddress()
			{
				...
				{
					PXAddressValidator.Validate<Address>(Base, remitAddress, true, true);
				}
			}
		...
		/// <exclude/>
		// Acuminator disable once PX1016 ExtensionDoesNotDeclareIsActiveMethod extension should be constantly active
		public class LocationDetailsExt : LocationDetailsExt<VendorMaint, VendorR, VendorR.bAccountID>
		{
			[PXOverride]
		...
		public class CreateContactFromVendorGraphExt : CRCreateContactActionBase<VendorMaint, VendorR>
		{
			...
			protected override PXSelectBase<CRPMTimeActivity> Activities => Base.GetExtension<VendorMaint_ActivityDetailsExt>().Activities;
