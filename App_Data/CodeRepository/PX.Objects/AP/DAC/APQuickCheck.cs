...
namespace PX.Objects.AP.Standalone
{
	[PXProjection(typeof(Select2<APRegister,
		...
		Where<APRegister.docType, Equal<APDocType.quickCheck>,
			Or<APRegister.docType, Equal<APDocType.voidQuickCheck>>>>), Persistent = true, CommerceTranType=typeof(APInvoice))]
	...
	public class APQuickCheck : APRegister, IPrintCheckControlable, IAssign, IApprovable, IApprovalDescription
	{
		...
		{
			public static APQuickCheck Find(PXGraph graph, string docType, string refNbr, PKFindOptions options = PKFindOptions.None) => FindBy(graph, docType, refNbr, options);
		}
		...
		[PXSelector(typeof(APContact.contactID), ValidateValue = false)]    //Attribute for showing contact email field on Automatic Notifications screen in the list of availible emails for
																			//Cash Purchases screen. Relies on the work of platform, which uses PXSelector to compose email list
		...
		[APTermsSelector]
		[Terms(typeof(APQuickCheck.docDate), null, null, typeof(APQuickCheck.curyDocBal), typeof(APQuickCheck.curyOrigDiscAmt), typeof(APQuickCheck.curyTaxTotal))]
		...
		[PXDBTimestamp(BqlField = typeof(APRegister.Tstamp), RecordComesFirst = true)]
