...
namespace PX.Objects.AP
{
	...
	public class APDocumentRelease : PXGraph<APDocumentRelease>
	{
		...
		protected virtual void _(Events.CacheAttached<BalancedAPDocument.branchID> e) { }
		#endregion
	...
	public class APReleaseProcess : PXGraph<APReleaseProcess>
	{
		...
		public PXSelectJoin<
			APAdjust,
				...
				LeftJoin<CM.CurrencyInfo2, 
					On<CM.CurrencyInfo2.curyInfoID, Equal<APRegisterAlias.curyInfoID>>>>>>>>>,
			Where<
				...
				And2<Where<
					...
					>,
				And<Where<
					APAdjust.adjgDocType, Equal<APAdjust.adjdDocType>,
					And<APAdjust.adjgRefNbr, Equal<APAdjust.adjdRefNbr>,
						Or<APRegisterAlias.released, Equal<True>,
		...
		public PXSelectReadonly<APTran, Where<APTran.tranType, Equal<Required<APTran.tranType>>, And<APTran.refNbr, Equal<Required<APTran.refNbr>>, And<APTran.box1099, IsNotNull>>>> AP1099Tran_Select;
		...
		public APSetup apsetup
		{
			get
			{
				return _apsetup ?? (_apsetup = PXSetup<APSetup>.Select(this));
				}
				}
		...
		public POSetup posetup
		{
			get
			{
				return _posetup ?? (_posetup = PXSetup<POSetup>.Select(this));
			}
		}
		...
		private List<APRegister> CreateInstallments(PXResult<APInvoice, CurrencyInfo, Terms, Vendor> res)
		{
			...
			foreach (TermsInstallments inst in installments)
			{
				...
				CurrencyInfo new_info = docgraph.GetExtension<APInvoiceEntry.MultiCurrency>().CloneCurrencyInfo(info);
				...
				new_apdoc.CuryDetailExtPriceTotal = 0m;
				new_apdoc.DetailExtPriceTotal = 0m;
				new_apdoc.CuryLineDiscTotal = 0m;
				new_apdoc.LineDiscTotal = 0m;
				...
				if (inst.InstallmentNbr == installments.Count)
				{
					...
					preCalculatedBaseAmt = new_apdoc.OrigDocAmt - BaseTotalInstallments;
				}
				else
				{
				...
				{
					new_apdoc.OrigDocAmt = preCalculatedBaseAmt;
					new_apdoc.DocBal = preCalculatedBaseAmt;
					new_apdoc.LineTotal = preCalculatedBaseAmt;
				}

				docgraph.Document.Update(new_apdoc);

				CuryTotalInstallments += (decimal)new_apdoc.CuryOrigDocAmt;
				BaseTotalInstallments += new_apdoc.OrigDocAmt.Value;
				TaxAttribute.SetTaxCalc<APTran.taxCategoryID>(docgraph.Transactions.Cache, null, TaxCalc.NoCalc);
				...
				new_aptran = docgraph.Transactions.Insert(new_aptran);
				if (preCalculatedBaseAmt.HasValue)
				{
					new_aptran.TranAmt = preCalculatedBaseAmt;
					docgraph.Transactions.Update(new_aptran);
			...
			if (installments.Count > 0)
			{
				PXDatabase.Update<APInvoice>(
					new PXDataFieldAssign<APInvoice.installmentCntr>(PXDbType.SmallInt, Convert.ToInt16(installments.Count)),
					new PXDataFieldRestrict<APInvoice.docType>(PXDbType.VarChar, apdoc.DocType),
					new PXDataFieldRestrict<APInvoice.refNbr>(PXDbType.NVarChar, apdoc.RefNbr));
			}
			...
			out List<INRegister> inDocs)
		{
			...
			if (info.BaseCuryID != info.CuryID && apdoc.InstallmentNbr != null)
				info.BaseCalc = false;
			...
			if (doc.Released != true && (!isPrebooking || doc.Prebooked != true))
			{
				...
				if ((doc.DocType == APDocType.Invoice || doc.DocType == APDocType.CreditAdj) && doc.Voided == true)
				...
				if (isPrebookCompletion == false)
				{
					...
					else if (apdoc.DocType != APDocType.Prepayment)
					{
						...
						if (apdoc.IsOriginalRetainageDocument() &&
							apdoc.IsRetainageReversing == true)
						{
						...
						if (apdoc.IsChildRetainageDocument())
						{
							...
							if (origRetainageDoc != null)
							{
								...
								decimal sign = origRetainageDoc.SignAmount * apdoc.SignAmount ?? 0m;

								origRetainageDoc.CuryRetainageUnreleasedAmt -= (apdoc.CuryOrigDocAmt + apdoc.CuryRoundDiff) * sign;
				...
				else
				{
					...
					if (!_IsIntegrityCheck)
					{
						foreach (APTran apTran in apTranTaxes)
						{
							if (apTran.Released != true)
								ConvertedInventoryItemAttribute.ValidateRow(APTran_TranType_RefNbr.Cache, apTran);
						}
					}
					...
					{
						APTran n = group.Key;
						PXCache<APTran>.StoreOriginal(this, n);
						...
						foreach (PXResult<APTran, APTax, Tax, DRDeferredCode, LandedCostCode, InventoryItem, APTaxTran> r in group)
						{
							...
							if (isFirstAPTaxRow)
							{
								...
								tran.SummPost = this.SummPost && n.TaskID == null;
							...
							!isMultipleInstallmentInvoice)
						{
								...
								apdoc.IsRetainageReversing == true)
							{
								...
								PXCache<APTran>.StoreOriginal(this, origRetainageTran);
					...
					else
					{
						foreach (PXResult<APTaxTran, Tax, APInvoice> r in APTaxTran_TranType_RefNbr.Select(apdoc.DocType, apdoc.RefNbr))
						{
							...
							if (salestax.TaxType == CSTaxType.Withholding) continue;

							if (salestax.DirectTax != true &&
								(apdoc.TaxCalcMode == TaxCalculationMode.Gross || salestax.TaxCalcLevel == CSTaxCalcLevel.Inclusive && apdoc.TaxCalcMode != TaxCalculationMode.Net))
				...
				if (!_IsIntegrityCheck && !IsInvoiceReclassification)
				{
					foreach (PXResult<APAdjust, APPayment> appres in PXSelectJoin<APAdjust, InnerJoin<APPayment, On<APPayment.docType, Equal<APAdjust.adjgDocType>, And<APPayment.refNbr, Equal<APAdjust.adjgRefNbr>>>>, Where<APAdjust.adjdDocType, Equal<Required<APAdjust.adjdDocType>>, And<APAdjust.adjdRefNbr, Equal<Required<APAdjust.adjdRefNbr>>, And<APAdjust.released, Equal<False>, And<APPayment.released, Equal<True>>>>>>.Select(this, doc.DocType, doc.RefNbr))
					{
						...
						PXCache<APAdjust>.StoreOriginal(this, adj);
						PXCache<APPayment>.StoreOriginal(this, payment);
					...
					//
					if (doc.DocType == APDocType.DebitAdj && _InstallmentType != TermsInstallmentType.Multiple)
					{
						ret.Add(doc);
					}
				}
		...
		private void AdjustOriginalRetainageLineBalance(APRegister document, APTran tran, decimal? curyAmount, decimal? baseAmount)
		{
			...
			if (origRetainageTran != null)
			{
				PXCache<APTran>.StoreOriginal(this, origRetainageTran);
				// We should consider amount sign both for Original and Child Retainage document
				// to cover all possible combinations between INV, ACR and ADR documents.
				decimal sign = APDocType.SignAmount(origRetainageTran.TranType) * document.SignAmount ?? 0m;
				...
				if (!IsIntegrityCheck)
					DecreaseRetainedAmount(origRetainageTran, tran.CuryOrigTranAmt.GetValueOrDefault() * sign);
		...
		protected virtual LineBalances CalculateInvoiceDetailsCashDiscBalance(APTran tran, APRegister doc)
		{
		...
		private bool IsPostUseAndSalesTaxesByProjectKey(PXGraph graph, Tax tax)
		{
			...
			{
				Account account = Account.PK.Find(graph, tax.ExpenseAccountID);
		...
		protected virtual void PostDirectTax(CurrencyInfo info, APTaxTran x, APInvoice orig_doc)
		{
			...
			if (tran == null)
			{
				...
				tran.Released = true;
				tran.TranDate = x.TranDate;
				tran.FinPeriodID = x.FinPeriodID;
			...
			{
				tran.CuryTaxableAmt = orig_info.CuryConvCury(tran.TaxableAmt.Value);
				tran.CuryTaxAmt = orig_info.CuryConvCury(tran.TaxAmt.Value);
			}
		...
		public virtual void ProcessRetainageTranPost(APInvoice doc)
		{
			...
			if (doc.PaymentsByLinesAllowed == true)
			{
				...
				PXResultset<APTran> apTranTaxes = APTran_TranType_RefNbr.Select(doc.DocType, doc.RefNbr);
				...
				foreach (var group in apTranTaxes.AsEnumerable().GroupBy(row => (APTran)row, apTranComparer))
				{
					...
					post.LineNbr = n.OrigLineNbr;
					post.Type = APTranPost.type.Retainage;
					post.CuryRetainageAmt = -n.CuryTranBal;
					post.RetainageAmt = -n.TranBal;
					if (IsNeedUpdateHistoryForTransaction(post.FinPeriodID))
						TranPost.Insert(post);
		...
		public static void Update1099Hist(PXGraph graph, decimal histMult, APAdjust adj, APTran tran, APRegister apdoc)
		{
			...
			Update1099Hist(graph, histMult, tran, apdoc, adj.AdjgDocDate, adj.AdjgBranchID, adj.AdjAmt);

		}

		public static void Update1099Hist(PXGraph graph, decimal histMult, APTran tran, APRegister apdoc, DateTime? docDate, int? branchID, decimal? adjAmt)
		{
            PXCache cache = graph.Caches[typeof(AP1099Hist)];
			string Year1099 = ((DateTime)docDate).Year.ToString();

			if (apdoc != null && tran.CuryLineAmt != 0m)
			{
				AP1099Yr year = new AP1099Yr
				{
					FinYear = Year1099,
					OrganizationID = PXAccess.GetParentOrganizationID(branchID)
				};
				...
				if (hist != null)
				{
					...
					decimal tranAmountPortion = 0;
					if (apdoc.OrigDocAmt == 0)
					{
						tranAmountPortion = tranAmountAfterDiscount - whTaxAmount;
					}
					else
					{
						tranAmountPortion = (tranAmountAfterDiscount - whTaxAmount) * (decimal)adjAmt / ((decimal)apdoc.OrigDocAmt - (decimal)apdoc.OrigWhTaxAmt);
					}
		...
		public virtual void VerifyAdjustedDocumentAndClose(APRegister adjddoc)
		{
			...
			if (cachedDoc != null)
			{
				PXCache<APRegister>.RestoreCopy(apdoc, cachedDoc);
			}
			else if (_IsIntegrityCheck) return;
			...
			if (_IsIntegrityCheck == false &&
				(apdoc.CuryDocBal < 0m || apdoc.CuryDocBal > apdoc.CuryOrigDocAmt))
			{
				string docType = PXStringListAttribute.GetLocalizedLabel<APRegister.docType>(APDocument.Cache, apdoc);
				throw new PXException(apdoc.CuryDocBal < 0m ?
					Messages.DocumentOutOfBalanceNegative :
					Messages.DocumentOutOfBalanceHigher, docType, apdoc.RefNbr);
			}
			...
			APRegister origRetainageDoc = null;
			...
			//
			if (apdoc.IsChildRetainageDocument() &&
				(origRetainageDoc = GetOriginalRetainageDocument(apdoc)) != null)
			{
		...
		protected virtual void ProcessPrepaymentRequestApplication(APRegister prepaymentRequest, APAdjust prepaymentAdj)
		{
			...
			else if (prepaymentAdj.AdjgDocType.IsIn(APDocType.Check, APDocType.Prepayment))
			{
				if (_IsIntegrityCheck)
				{
					...
					APDocument.Cache.Update(prepaymentRequest);
					VerifyAdjustedDocumentAndClose(prepaymentRequest);
					return;
				}
		...
		private void UpdateVoidedCheck(APRegister voidcheck)
		{
			foreach (string origDocType in voidcheck.PossibleOriginalDocumentTypes())
			{
					...
					.Search<APPayment.vendorID>(voidcheck.VendorID, origDocType, voidcheck.RefNbr))
				{
					...
					if (voidcheck.NoteID == apdoc.NoteID)
						APDocument.Cache.RestoreCopy(voidcheck, apdoc);
			...
			Tuple<APAdjust, CurrencyInfo> lastAdjustment)
		{
			...
			bool hasZeroBalance = paymentRegister.IsOriginalRetainageDocument()
				...
				: paymentRegister.HasZeroBalance<APRegister.curyDocBal, APTran.curyTranBal>(this);
			...
			if ((!paymentRegister.IsOriginalRetainageDocument() || paymentRegister.DocType != APDocType.DebitAdj || hasAnyApplications)
				&& (hasZeroBalance || payment.VoidAppl == true))
			{
				ClosePayment(paymentRegister, payment, paymentVendor);
			}
			else ReopenPayment(paymentRegister, payment);
			...
			Currency paycury)
		{
			...
			foreach (var group in adjustments
				.AsEnumerable()
				.Where(res => ((APAdjust)res).IsInitialApplication != true)
				.Cast<PXResult<APAdjust, CurrencyInfo, Currency, APInvoice, APPayment, Standalone.APRegisterAlias, APTran>>()
				.GroupBy(res => ((APInvoice)res), APDocument.Cache.GetComparer()))
			{
				APRegister adjustedDocument = (APInvoice)group.Key;
				bool verifyAdjustedDocument = false;

				foreach (PXResult<APAdjust, CurrencyInfo, Currency, APInvoice, APPayment, Standalone.APRegisterAlias, APTran> adjres in group)
				{
					APAdjust adj = adjres;
				...
				PXCache<APAdjust>.StoreOriginal(this, adj);
				PXCache<APRegister>.StoreOriginal(this, adjddoc);
				PXCache<APTran>.StoreOriginal(this, line);
				...
				if (adjddoc.RefNbr != null && _IsIntegrityCheck == false && paymentVendor.Vendor1099 == true
					|| AP1099Tran_Select.Any(adjddoc.DocType, adjddoc.RefNbr))
				{
					...
					if (adjddoc.RefNbr != null)
					{
					if (adjddoc.ReleasedOrPrebooked != true && adjddoc.DocType != APDocType.QuickCheck && adjddoc.DocType != APDocType.VoidQuickCheck)
						throw new PXException(Messages.Document_Applied_To_Unreleased_CannotRelease, payment.RefNbr, GetLabel.For<APDocType>(payment.DocType),
							adjddoc.RefNbr, GetLabel.For<APDocType>(adjddoc.DocType));
				...
				if (verifyAdjustedDocument && adjustedDocument.RefNbr != null)
				{
		...
		private void ProcessPayByLineDebitAdjAdjustment(APRegister paymentRegister, APAdjust adj)
		{
			...
			{
				PXCache<APTran>.StoreOriginal(this, tran);
		...
		protected virtual void PerformBasicReleaseChecks(APRegister document)
		{
			...
			List<Branch> branchesInactive = SelectFrom<Branch>
				.InnerJoin<APAdjust>
					.On<Branch.branchID.IsEqual<APAdjust.adjdBranchID>
						.Or<Branch.branchID.IsEqual<APAdjust.adjgBranchID>>>
		...
		protected virtual void RefillAPPrintCheckDetail(APRegister apdoc)
		{
			...
			pe.SelectTimeStamp();
			pe.RefillAPPrintCheckDetail(payment);
			pe.Save.Press();
		}
		...
		public virtual List<APRegister> ReleaseDocProc(JournalEntry je, APRegister doc, bool isPrebooking, out List<INRegister> inDocs)
		{
			...
			{
				PXCache<APRegister>.StoreOriginal(this, doc);
				...
				{
					if (((CurrencyInfo)res).CuryInfoID == null)
						throw new PXException(GL.Messages.DocumentNotReleased);
				...
				foreach (PXResult<APPayment, CurrencyInfo, Currency, Vendor, CashAccount> res in APPayment_DocType_RefNbr.Select(doc.DocType, doc.RefNbr))
				{
					...
					if (cashacct?.CashAccountID != null)
					{
						VerifyCashAccountInGL(cashacct);
					}

					if (payment != null)
					{
						VerifyPaymentData(payment);
					}
				...
				if (doc.DocType == APDocType.DebitAdj)
				{
					...
					else
					{
						...
						debitadj.CuryInfoID = doc.CuryInfoID;
				...
				//in case the doc is Debit Adj.it is included in ret itself. Need to update its timestump to make later operations possible
				if (!alreadyReleased && APPayment_DocType_RefNbr.Current?.NoteID == doc.NoteID && doc.DocType == APDocType.DebitAdj)
					PXCache<APRegister>.RestoreCopy(doc, APPayment_DocType_RefNbr.Current);
		...
		{
			if (payment != null && string.IsNullOrEmpty(payment.ExtRefNbr))
			{
				PaymentMethod paymentMethod = PaymentMethod.PK.Find(this, payment.PaymentMethodID);
				if (PaymentRefAttribute.PaymentRefMustBeUnique(paymentMethod))
				{
					bool isRequestPrepayment = payment.DocType == APDocType.Prepayment && (new PXSelectReadonly<APInvoice,
						Where<APInvoice.docType, Equal<Required<APPayment.docType>>,
							And<APInvoice.refNbr, Equal<Required<APPayment.refNbr>>,
							And<APInvoice.docType, Equal<APDocType.prepayment>>>>>(this)).SelectSingle(payment.DocType, payment.RefNbr) != null;

					if (payment.DocType == APDocType.Check
							|| payment.DocType == APDocType.Prepayment && !isRequestPrepayment
							|| payment.DocType == APDocType.Refund
							|| payment.DocType == APDocType.VoidCheck)
					{
						bool extRefNbrCanNotBeEmpty = true;
						if (payment.DocType == APDocType.VoidCheck || payment.DocType == APDocType.Refund)
						{
							APPayment originDoc = PXSelectReadonly<APPayment, Where<APPayment.docType, Equal<Required<APPayment.docType>>,
								And<APPayment.refNbr, Equal<Required<APPayment.refNbr>>>>>.SelectSingleBound(this, null, payment.OrigDocType, payment.OrigRefNbr);
						...
						if (extRefNbrCanNotBeEmpty && payment.Released != true)
						{
							throw new ReleaseException(ErrorMessages.FieldIsEmpty, PXUIFieldAttribute.GetDisplayName<APPayment.extRefNbr>(this.Caches[typeof(APPayment)]));
						}
					}
				}
			}
		...
		protected virtual void VerifyCashAccountInGL(CashAccount cashAccount)
		{
		...
		protected virtual void RaiseReleaseEvent(APRegister doc)
	    {
		    if (APDocument.Cache.ObjectsEqual(doc, APInvoice_DocType_RefNbr.Current))
		    {
			    ...
			    APDocument.Cache.Remove(doc);
			    PXCache<APRegister>.StoreOriginal(this, invoice);
			    APInvoice.Events
		    ...
		    else if (APDocument.Cache.ObjectsEqual(doc, APPayment_DocType_RefNbr.Current))
		    {
			    ...
			    APDocument.Cache.Remove(doc);
			    PXCache<APRegister>.StoreOriginal(this, payment);
			    APPayment.Events
				    .Select(e => e.ReleaseDocument)
				    .FireOn(this, payment);
		...
		/// </summary>
        public bool IsInvoiceReclassification
		{
			get;
			set;
		}
		...
		public virtual void IntegrityCheckProc(Vendor vend, string startPeriod)
		{
			...
			using (new PXConnectionScope())
			{
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					...
					List<APRegister> invoices = new List<APRegister>();
					List<APRegister> payments = new List<APRegister>();

					foreach (APRegister venddoc in venddocs)
					{
						if (venddoc.Payable == true || APDocType.HasBothInvoiceAndPaymentParts(venddoc.DocType))
						{
							invoices.Add(venddoc);
						}

						if (venddoc.Paying == true || APDocType.HasBothInvoiceAndPaymentParts(venddoc.DocType))
						{
							payments.Add(venddoc);
						}
					}

					invoices.Sort((APRegister docA, APRegister docB) =>
					{
						Func<APRegister, short?> retainageSortOrder = new Func<APRegister, short?>((doc) =>
						{
							// Sort order for the Retainage documents validation:
					...
					foreach (APRegister venddoc in invoices)
					{
						...
						bool
							released = (doc.Released ==
							            true); //Save state of the document - prebooked & released flags will be altered during release process
						if (prebooked)
						{
							doc.Prebooked = false;
						}
						
						foreach (PXResult<APInvoice, CurrencyInfo, Terms, Vendor> res in APInvoice_DocType_RefNbr.Select(
							         doc.DocType, doc.RefNbr))
						{
							doc.Released = false;
							...
							{
								SegregateBatch(je, doc.BranchID, doc.CuryID, doc.DocDate, doc.FinPeriodID, doc.DocDesc,
									(CurrencyInfo) res);
							}
					...
					payments.Sort(SortVendDocs);

					foreach (APRegister venddoc in payments)
					{
						APRegister doc = venddoc;
						...
						doc.Released = venddoc is APInvoice;
						foreach (PXResult<APPayment, CurrencyInfo, Currency, Vendor, CashAccount> res in
						         APPayment_DocType_RefNbr.Select(doc.DocType, doc.RefNbr, doc.VendorID))
						{
							APPayment payment = res;
							CurrencyInfo info = res;
							Currency paycury = res;
							...
							if (doc.Released == false && doc.DocType == APDocType.DebitAdj || doc.DocType == APDocType.CreditAdj)
							{
								//We don't need payment part processing on release;
								if (doc.Prebooked == true) continue;
							}

							SegregateBatch(je, doc.BranchID, doc.CuryID, payment.AdjDate, payment.AdjFinPeriodID, doc.DocDesc,
								info);
							...
							while (doc.AdjCntr < OrigAdjCntr)
							{
								...
								if (doc.AdjCntr == 0 || (doc.DocType != APDocType.QuickCheck &&
								                         doc.DocType != APDocType.VoidQuickCheck))
								{
									ProcessPayment(je, doc,
										new PXResult<APPayment, CurrencyInfo, Currency, Vendor, CashAccount>(payment,
											CurrencyInfo.GetEX(new_info), paycury, vendor, cashacct));
								}

								PXResultset<APAdjust> adjustments =
									APAdjust_AdjgDocType_RefNbr_VendorID.Select(doc.DocType, doc.RefNbr, _IsIntegrityCheck,
										doc.AdjCntr);
								foreach (PXResult<APAdjust, CurrencyInfo, Currency, APInvoice, APPayment,
									         Standalone.APRegisterAlias> result in adjustments)
								{
								...
								Tuple<APAdjust, CurrencyInfo> lastAdjustment =
									ProcessAdjustments(je, adjustments, doc, payment, vendor, new_info, paycury);
								...
								doc.AdjCntr++;
								VerifyPaymentRoundAndClose(je, doc, payment, vendor, CurrencyInfo.GetEX(new_info), paycury,
									lastAdjustment);
							}
							...
							APAdjust reversal = APAdjust_AdjgDocType_RefNbr_VendorID.Select(doc.DocType, doc.RefNbr,
								_IsIntegrityCheck, OrigAdjCntr);
							if (reversal != null && reversal.IsInitialApplication != true)
							{
								ReopenPayment(doc, payment);
							}
						}

						if (doc.DocType == APDocType.Prepayment)
						{
							// Restore Released flag for Prepayment
							doc.Released = released;
							if (doc.Status == APDocStatus.Voided)
							{
								if (doc.OpenDoc == true)
								// Restore Open flag for voided Prepayment without application
								doc.OpenDoc = false;
								//way to detect Voided Prepayment requests  without application
								if ((doc as APInvoice)?.Released == true && !TranPost.Select(doc.DocType, doc.RefNbr).Any())
								{
									doc.CuryDocBal = 0m;
									doc.DocBal = 0m;
								}
					...
					foreach (PXResult<APAdjust, APTran, APInvoice> res in PXSelectReadonly2<APAdjust,
						         InnerJoin<APTran, On<APTran.tranType, Equal<APAdjust.adjdDocType>,
								         And<APTran.refNbr, Equal<APAdjust.adjdRefNbr>>>,
							         InnerJoin<APInvoice, On<APInvoice.docType, Equal<APAdjust.adjdDocType>,
								         And<APInvoice.refNbr, Equal<APAdjust.adjdRefNbr>>>>>,
						         Where<APAdjust.vendorID, Equal<Required<APAdjust.vendorID>>, And<APAdjust.adjgDocDate,
							         GreaterEqual<Required<APAdjust.adjgDocDate>>,
							         And<APAdjust.released, Equal<True>,
								         And<APAdjust.voided, Equal<False>, And<APTran.box1099, IsNotNull>>>>>>.Select(this,
						         vend.BAccountID, new DateTime(Convert.ToInt32(minPeriod.Substring(0, 4)), 1, 1)))
					{
						APAdjust adj = res;
						APTran tran = res;
						APInvoice doc = res;
		...
		protected virtual HashedList<APRegister> GetDocumentsForIntegrityCheckProc(string minPeriod)
		{
			...
			PXResultset<APRegister> adjustments;
			if (PXAccess.FeatureInstalled<FeaturesSet.retainage>())
			{
				...
				foreach (var retdoc in retainage)
				{
					if (!venddocs.Contains((APRegister)retdoc))
					{
						...
						foreach (PXResult<APAdjust, APRegister, APInvoice, APPayment> docadj in SelectFrom<APAdjust>
								.InnerJoin<APRegister>.On<APAdjust.adjgDocType.IsEqual<APRegister.docType>
									.And<APAdjust.adjgRefNbr.IsEqual<APRegister.refNbr>>>
								.LeftJoin<APInvoice>
									.On<APInvoice.docType.IsEqual<APRegister.docType>
									.And<APInvoice.refNbr.IsEqual<APRegister.refNbr>>>
								.SingleTableOnly
                        .LeftJoin<APPayment>
									.On<APPayment.docType.IsEqual<APRegister.docType>
									.And<APPayment.refNbr.IsEqual<APRegister.refNbr>>>
								.SingleTableOnly
						...
						{
							APRegister pDoc = GetFullDocument(new PXResult<APRegister,APInvoice,APPayment>(docadj, docadj, docadj));
							if (!venddocs.Contains(pDoc))
							{
								venddocs.Add(pDoc);
								var allAdjlist = new PXResultset<APRegister>();
								allAdjlist.Add(new PXResult<APRegister>(pDoc));
								GetAllReleasedAdjustments(venddocs, allAdjlist, minPeriod);
							}
						}
					}
				}
			...
			adjustments = GetDirectAdjustmentsForIntegrityCheckProc(minPeriod);
			...
			var adjgs =
				...
				Where<APRegister.vendorID, Equal<Current<Vendor.bAccountID>>,
					...
					And<Exists<Select2<APAdjust,
						...
						Where<APAdjust.adjgRefNbr, Equal<APRegister.refNbr>,
							And2<Where<APAdjust.adjgDocType, Equal<APRegister.docType>,
									Or<Where<APAdjust.adjgDocType, Equal<APDocType.prepayment>,
										And<APAdjust.voided, Equal<True>,
										And<APRegister.docType, Equal<APDocType.voidCheck>>>>>>,
				...
				foreach (var source in currentList)
				{
					var adjustments =
						...
						Where<APRegister.vendorID, Equal<Current<Vendor.bAccountID>>,
							...
							And<APAdjust.adjdDocType, Equal<Required<APAdjust.adjdDocType>>,
								And<APAdjust.adjdRefNbr, Equal<Required<APAdjust.adjdRefNbr>>,
								And<Where<APAdjust.adjgDocType, Equal<APRegister.docType>,
									Or<Where<APAdjust.adjgDocType, Equal<APDocType.prepayment>,
										And<APAdjust.voided, Equal<True>,
										And<APRegister.docType, Equal<APDocType.voidCheck>>>>>>>>>>>>>,
							And<APRegister.tranPeriodID, Less<Required<APRegister.tranPeriodID>>,
			...
			//using (new PXConnectionScope())
			{
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					...
					foreach (APTaxTran iTaxTran in this.APTaxTran_TranType_RefNbr.Select(doc.DocType, doc.RefNbr))
					{
						...
						PXCache<APTaxTran>.StoreOriginal(this,iTaxTran);
		...
		public virtual bool IsFullyProcessedOriginalRetainageDocument(APRegister origRetainageInvoice)
		{
			...
			{
				if (!childRetainageBill.HasZeroBalance<APRegister.curyDocBal, APTran.curyTranBal>(this))
				{
			...
			return
				origRetainageInvoice.HasZeroBalance<APRegister.curyDocBal, APTran.curyTranBal>(this) &&
				origRetainageInvoice.HasZeroBalance<APRegister.curyRetainageUnreleasedAmt, APTran.curyRetainageBal>(this) &&
