...
namespace PX.Objects.AP
{
	...
	public class PaymentRefAttribute : PXEventSubscriberAttribute, IPXRowPersistingSubscriber, IPXFieldDefaultingSubscriber, IPXFieldVerifyingSubscriber 
	{
	    ...
	    public static void DeleteCheck(int CashAccountID, string PaymentMethodID, string CheckNbr)
	    {
			PXDatabase.Delete<CashAccountCheck>(
				new PXDataFieldRestrict<CashAccountCheck.cashAccountID>(CashAccountID),
		...
		public class LastCashAccountCheckSelect : PXSelectReadonly<CashAccountCheck,
			Where<CashAccountCheck.cashAccountID, Equal<Optional<PaymentMethodAccount.cashAccountID>>,
		...
		public static string GetMaxCashAccountCheckNumberByPrefix(PXGraph graph, int? cashAccountID, string paymentMethodID, string prefix)
		{
			return PXSelectReadonly<CashAccountCheck,
				Where<CashAccountCheck.cashAccountID, Equal<Required<CashAccountCheck.cashAccountID>>,
		...
		void IPXRowPersistingSubscriber.RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if (string.IsNullOrEmpty(newNumber))
			{
				if (APPaymentEntry.IsCheckReallyPrinted(check))
				{
					...
					Char.IsDigit(newNumber[newNumber.Length - 1]) &&
					!string.Equals(oldNumber, newNumber) &&
				!string.Equals(lastCheckNbr, newNumber) &&
					det.APAutoNextNbr == true)
				{
					WebDialogResult result = (check.IsReleaseCheckProcess == true || sender.Graph.UnattendedMode || sender.Graph.IsContractBasedAPI)
						? WebDialogResult.Yes
		...
		public static string GetNextPaymentRef(PXGraph graph, int? CashAccountID, string PaymentTypeID)
		{
			...
			if (PXSelect<CashAccountCheck,
				Where<CashAccountCheck.cashAccountID, Equal<Current<PaymentMethodAccount.cashAccountID>>,
	...
	{
		public VendorNonEmployeeActiveOrHoldPaymentsAttribute(Type search)
			: base(search)
		{
		}
	...
	public class VendorAttribute : PXEntityAttribute
	{
		...
		protected virtual void PopulateFields(PXCache sender)
		{
			...
			for (int i = 0; i < this._fields.Length; i++)
			{
				...
				var key = "PopulateFields";
				var depends = new object[]
				{
					_FieldList.GetHashCodeOfSequence(), 
					sender.GetItemType(),
					System.Threading.Thread.CurrentThread.CurrentCulture.Name
				};
				bool b = sender.Graph.Prototype.TryGetValue(out _HeaderList, key, depends);
				if (!b)
				{
					...
					sender.Graph.Prototype.SetValue(_HeaderList, key, depends);
				}
			}
	...
	public class APTaxAttribute : ManualVATAttribute
	{
		...
		{
			base.CacheAttached(sender);
			sender.Graph.FieldUpdated.AddHandler(_ChildType, _TaxCategoryID, TaxCategoryUpdated);
		}
	    ...
	    private void Init()
	    {
			...
			_Attributes.Add(new PXUnboundFormulaAttribute(typeof(Switch<Case<Where<APTran.lineType,
				NotIn3<SO.SOLineType.discount, SO.SOLineType.freight>>, APTran.curyDiscAmt>, decimal0>),
				typeof(SumCalc<APInvoice.curyLineDiscTotal>)));
		}
		...
		{
			if (_IncludeDirectTaxLine)
			{
				APTaxTran row = e.Row as APTaxTran;
				Tax tax = Tax.PK.Find(sender.Graph, row?.TaxID);
				if (tax?.DirectTax == true)
				{
					PXUIFieldAttribute.SetEnabled<APTaxTran.curyRetainedTaxableAmt>(sender, row, false);
					PXUIFieldAttribute.SetEnabled<APTaxTran.curyRetainedTaxAmt>(sender, row, false);
				}
			}
		...
		protected override void _CalcDocTotals(
			...
			decimal CuryTaxDiscountTotal)
		{
			if (true == (bool?)ParentGetValue<APRegister.releasedOrPrebooked>(sender.Graph)) return;
		...
		protected override bool ShouldRecalculateTaxesOnRowUpdate(PXCache rowCache, object newRow, object oldRow)
		{
			return !rowCache.ObjectsEqual<APTran.lCDocType, APTran.lCRefNbr,
				APTran.lCLineNbr, APTran.pONbr, APTran.receiptNbr>(newRow, oldRow)
				|| base.ShouldRecalculateTaxesOnRowUpdate(rowCache, newRow, oldRow);
		}

		protected override bool SkipDirectTax(PXCache sender, object row, string applicableDirectTaxId)
		{
			APTran tran = row as APTran;
			if (tran == null) return false;

			if (!string.IsNullOrEmpty(tran.PONbr) || !string.IsNullOrEmpty(tran.ReceiptNbr))
			{
				sender.RaiseExceptionHandling(_TaxCategoryID, row, tran.TaxCategoryID,
					new PXSetPropertyException(Messages.ImportTaxCanNotBeAppliedPo, PXErrorLevel.Warning,
					applicableDirectTaxId));
				return true;
			}

			if (!string.IsNullOrEmpty(tran.LCRefNbr)
				&& POLandedCostDetail.PK.Find(sender.Graph, tran.LCDocType, tran.LCRefNbr,
				tran.LCLineNbr)?.AllocationMethod != LandedCostAllocationMethod.None)
			{
				sender.RaiseExceptionHandling(_TaxCategoryID, row, tran.TaxCategoryID,
					new PXSetPropertyException(Messages.ImportTaxCanBeAppliedOnlyToLCNonAlocationMethod,
					PXErrorLevel.Warning, applicableDirectTaxId));
				return true;
			}
		...
		{
			if (e.Row == null || !_IncludeDirectTaxLine) return;

			DefualtDirectTaxRealtedFieldValue(sender, e.Row, IsDirectTaxLine(sender, e.Row));
		}

		protected override void UpdateIsDirectTaxLineFeildValue(PXCache cache, object row, bool newValue, bool oldValue)
		{
			APTran tran = row as APTran;
			if (tran == null) return;
			...
			if (newValue != oldValue)
			{
				...
				DefualtDirectTaxRealtedFieldValue(cache, tran, newValue);
				cache.RaiseRowUpdated(tran, copy);
			}
		}

		protected virtual void DefualtDirectTaxRealtedFieldValue(PXCache cache, object row, bool isDirectTax)
		{
			APTran tran = row as APTran;
			if (tran == null) return;

			if (tran.IsDirectTaxLine != true && isDirectTax)
			{
				cache.SetValueExt<APTran.isDirectTaxLine>(row, true);
				cache.SetValueExt<APTran.retainagePct>(row, 0.0m);
				cache.SetValueExt<APTran.curyRetainageAmt>(row, 0.0m);
				cache.SetValueExt<APTran.manualDisc>(row, true);
				cache.SetValueExt<APTran.curyDiscAmt>(row, 0.0m);
				cache.SetValueExt<APTran.discPct>(row, 0.0m);
				cache.SetValueExt<APTran.skipDisc>(row, true);
				cache.SetValueExt<APTran.automaticDiscountsDisabled>(row, true);
				cache.SetValueExt<APTran.deferredCode>(row, null);
				cache.SetValueExt<APTran.dRTermStartDate>(row, null);
				cache.SetValueExt<APTran.dRTermEndDate>(row, null);
			}
			else if(tran.IsDirectTaxLine == true && !isDirectTax)
			{
				cache.SetValueExt<APTran.isDirectTaxLine>(row, false);
				cache.SetValueExt<APTran.skipDisc>(row, false);
				cache.SetValueExt<APTran.manualDisc>(row, false);
				cache.SetValueExt<APTran.automaticDiscountsDisabled>(row, false);
			}
		}
		...
		public override void FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			if (project != null && 
				...
				project.BaseType == CT.CTPRType.Project && 
					...
					{
						Account account =
							Account.PK.Find(sender.Graph,
								sender.GetValue(e.Row, AccountFieldType.Name) as int?);
						if (account != null && account.AccountGroupID == null)
						{
							...
							throw new PXSetPropertyException(PM.Messages.AccountGroupNotFound, PXErrorLevel.Error, project.ContractCD, account.AccountCD);
						}
					}
        ...
        where WhereSelect : IBqlWhere, new()
	{
		...
		protected virtual void ReverseCharges(CM.IRegister oldDoc, CM.IRegister newDoc, bool reverseSign)
		{
			foreach (PXResult<ChargeTable, CashAccountETDetail> paychargeresult in PXSelectJoin<ChargeTable,
				LeftJoin<CashAccountETDetail, 
					On<CashAccountETDetail.entryTypeID, Equal<EntryTypeID>,
							And<CashAccountETDetail.cashAccountID, Equal<Current<CashAccountID>>>>>,
				...
				And<ChargeRefNbr, Equal<Required<ChargeRefNbr>>>>>.Select(this._Graph, oldDoc.DocType, oldDoc.RefNbr))
			{
				...
				charge.TranDate = newDoc.DocDate;
				this.Insert(charge);
    ...
    public abstract class BaseVendorRefNbrAttribute : PXEventSubscriberAttribute, IPXRowUpdatedSubscriber, IPXFieldVerifyingSubscriber, IPXRowDeletedSubscriber, IPXRowInsertedSubscriber, IPXRowPersistedSubscriber
	{
        ...
        public virtual void RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
        {
            ...
            {
	            var status = Cache.GetStatus(vrn);
	            if( Cache.Locate(vrn) != null &&
        ...
        protected virtual APVendorRefNbr InitVendorRefObject(PXCache sender, object row)
        {
            ...
            if (located == null)
                located = vrn;
					...
					PXCache<APVendorRefNbr>.StoreOriginal(_Graph, located);
	...
	public class APVendorRefNbrAttribute : BaseVendorRefNbrAttribute
	{
        ...
        protected override bool IsIgnored(PXCache sender, object row)
		{
            ...
            bool originatesFromExpenseClaim =
	            r.OrigRefNbr == null
		            ? false
		            : SelectFrom<EP.EPExpenseClaim>
			            .Where<EP.EPExpenseClaim.refNbr.IsEqual<@P.AsString>>.View.Select(sender.Graph, r.OrigRefNbr)
			            .AsEnumerable()
			            .Any();
