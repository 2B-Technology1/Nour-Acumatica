...
namespace PX.Objects.AP
{
	public class APSetupMaint : PXGraph<APSetupMaint>
	{
		...
		public PXSelectOrderBy<AP1099Box, OrderBy<AP1099Box.boxCD.Asc>> Boxes1099;
		...
		{
			foreach (AP1099Box item in SelectFrom<AP1099Box>.OrderBy<AP1099Box.boxCD.Asc>.View.Select(this))
			{
				...
				Account acct = Account_Box1099.Select(item.BoxNbr);

				if (acct != null)
				{
					item.AccountID = acct?.AccountID;
					item.OldAccountID = acct?.AccountID;
				}
