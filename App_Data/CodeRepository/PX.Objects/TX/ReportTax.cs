...
#pragma warning disable PX1015
	...
	public class ReportTax : PXGraph<ReportTax>
	{
		...
		public static int?[] GetBranchesForProcessing(PXGraph graph, int? organizationID, int? branchID, int? taxAgencyID, string taxPeriodID=null)
		{
			...
			using (new PXReadBranchRestrictedScope(organizationID.SingleToArrayOrNull(), null, requireAccessForAllSpecified:false))
			{
				_branches = PXSelectJoinGroupBy<TaxHistory,
							...
							Where<TaxHistory.vendorID, Equal<Required<TaxHistory.vendorID>>, And<Current<TaxPeriodFilter.organizationID>, IsNotNull>>,
				...
				.Union(PXSelectGroupBy<TaxTran,
					Where<TaxTran.vendorID, Equal<Required<TaxPeriodFilter.vendorID>>,
						And<Current<TaxPeriodFilter.organizationID>, IsNotNull,
						And<TaxTran.released, Equal<True>,
						And<TaxTran.voided, Equal<False>,
						And<TaxTran.taxPeriodID, IsNull,
						And<TaxTran.taxType, NotEqual<TaxType.pendingPurchase>,
													...
													requireAccessForAllSpecified: true))
			{
				return PXSelectJoin<TaxTran,
									...
									Where<Tax.taxVendorID, Equal<Required<TaxPeriodFilter.vendorID>>,
											...
											And<TaxTran.origRefNbr, Equal<Empty>,
											And<Exists<
													Select2<TaxReportLine,
													InnerJoin<TaxBucketLine,
														On<TaxBucketLine.vendorID, Equal<Required<TaxPeriodFilter.vendorID>>,
														And<TaxBucketLine.lineNbr, Equal<TaxReportLine.lineNbr>,
														And<TaxBucketLine.lineNbr, Equal<TaxReportLine.lineNbr>>>>>,
													Where<TaxBucketLine.bucketID, Equal<TaxTran.taxBucketID>>>>>>>>>>>>>,
									OrderBy<Asc<TOrderByField>>>
									.SelectWindowed(graph, 0, 1, taxAgencyID, taxAgencyID);
			}
		}
		protected virtual void TaxPeriodFilter_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			fileTax.SetEnabled(CheckProcessAllowable(filter, taxper, vendor));

			CheckGLSychronizedPeriods(sender, filter, vendor);
		}

		protected virtual void CheckGLSychronizedPeriods(PXCache sender, TaxPeriodFilter filter, Vendor vendor)
		{
			string existingError = PXUIFieldAttribute.GetErrorOnly<TaxPeriodFilter.taxPeriodID>(sender, filter);
		...
		protected virtual bool CheckProcessAllowable(TaxPeriodFilter filter, TaxPeriod taxper, Vendor vendor)
		{
			PXCache filterCache = Period_Header.Cache;

			bool allowProcess = (filter != null && taxper.VendorID != null);
			...
			{
				int? maxRevision = ReportTaxProcess.CurrentRevisionId(filterCache.Graph,
				...
				using (new PXReadBranchRestrictedScope(filter.OrganizationID.SingleToArray(), filter.BranchID.SingleToArrayOrNull(), restrictByAccessRights: false))
				{
					...
					// Acuminator disable once PX1015 IncorrectNumberOfSelectParameters [The two extra arguments passed to the BQL Select are required to avoid platfrom bug. Query cache doesn't take into consideration PXReadBranchRestrictedScope. See AC-57858 for more details.]
			...
			{
				filterCache.RaiseExceptionHandling<TaxPeriodFilter.taxPeriodID>(filter, taxPeriodIDForMessage,
			...
			if (allowProcess)
			{
				...
				{
					filterCache.RaiseExceptionHandling<TaxPeriodFilter.vendorID>(filter, filter.VendorID,
				...
				switch (taxper.Status)
				{
					case TaxPeriodStatus.Dummy:
						filterCache.RaiseExceptionHandling<TaxPeriodFilter.vendorID>(filter, taxper.VendorID,
					...
					case TaxPeriodStatus.Closed:
						{
									...
									vendor.TaxReportFinPeriod == true && taxper.EndDate <= tran.FinDate))
							{
								...
								filterCache.RaiseExceptionHandling<TaxPeriodFilter.taxPeriodID>(filter, taxPeriodIDForMessage, exception);
								allowProcess = false;
							}
						}
						break;
					default:
						{
							...
							if (allowProcess)
							{
									...
									vendor.TaxReportFinPeriod == true && taxper.StartDate > tran.FinDate))
								{
									var exception = new PXSetPropertyException<TaxPeriodFilter.taxPeriodID>(
															Messages.OneOrMoreTaxTransactionsFromPreviousPeriodsWillBeReported, PXErrorLevel.Warning);
									Period_Header.Cache.RaiseExceptionHandling<TaxPeriodFilter.taxPeriodID>(filter, taxPeriodIDForMessage, exception);
								}
							}
						}
						break;
		...
		protected virtual IEnumerable period_Details()
		{
			...
			using (new PXReadBranchRestrictedScope(filter.OrganizationID.SingleToArray(), filter.BranchID.SingleToArrayOrNull()))
			{
				var results = Period_Details_Expanded
					.Select()
					.AsEnumerable()
					.Where(row => ((TaxReportLine)row).TaxReportRevisionID == filter.TaxReportRevisionID
						&& ShowTaxReportLine((PXResult<TaxReportLine, TaxBucketLine, Currency, TaxTranRevReport>)row, Period_Header.Current.TaxPeriodID));

				IEnumerable<TaxTranRevReport> trans = results.RowCast<TaxTranRevReport>();
				IOrderedEnumerable<DateTime> dates = trans.Where(t => t.TranDate.HasValue).Select(t => t.TranDate.Value).OrderBy(t => t);
			...
			ret = (vendor != null && vendor.TaxUseVendorCurPrecision != true) ?
				TaxHistorySumManager.GetPreviewReport(
					this,
					vendor,
					(int) filter.TaxReportRevisionID,
					ret,
					(line) => ShowTaxReportLine(line, filter.TaxPeriodID))
				: ret;
		...
		public virtual bool ShowTaxReportLine(PXResult<TaxReportLine, TaxBucketLine, Currency, TaxTranRevReport> taxReportLineDetails, string taxPeriodID)
		{
			return true;
		}
	...
	public class ReportTaxProcess : PXGraph<ReportTaxProcess>
	{
		...
		[PXDate()]
		[PXDBScalar(typeof(Search<CM.CurrencyRate.curyEffDate,

			Where<CM.CurrencyRate.fromCuryID, Equal<TaxTran.curyID>,
			And<CM.CurrencyRate.toCuryID, Equal<CurrentValue<AP.Vendor.curyID>>,
			And<CM.CurrencyRate.curyRateType, Equal<CurrentValue<AP.Vendor.curyRateTypeID>>,
			And<CM.CurrencyRate.curyEffDate, LessEqual<TaxTran.tranDate>>>>>,
			OrderBy<Desc<CM.CurrencyRate.curyEffDate>>>))]
		...
		{
			HashSet<int> branchesToSelect = new HashSet<int>();
			IEnumerable<int?> taxHistoryBranchIds = null;

			if (filter.OrganizationID != null)
			{
				branchesToSelect.Add(PXAccess.GetChildBranchIDs(filter.OrganizationID, false));

				taxHistoryBranchIds = PXSelectGroupBy<TaxHistory,
					Where<TaxHistory.branchID, In<Required<TaxHistory.branchID>>>,
					Aggregate<
						GroupBy<TaxHistory.branchID>>>
					.Select(this, branchesToSelect)
					.Select(t => ((TaxHistory)t).BranchID);
			}

			using (new PXReadBranchRestrictedScope(filter.OrganizationID.SingleToArray(), taxHistoryBranchIds.ToArray(), requireAccessForAllSpecified: true))
			{
				...
				foreach (PXResult<Branch, TaxHistory, TaxTranReport, TaxBucketLine> taxTrans in SelectTaxTransForVerifying(filter, taxPeriod, vendor))
				{
		...
		protected virtual PXResultset<Branch, TaxHistory, TaxTranReport, TaxBucketLine> SelectTaxTransForVerifying(TaxPeriodFilter filter, TaxPeriod taxPeriod, Vendor vendor)
		{
			return PXSelectJoin<Branch,
						LeftJoin<TaxHistory,
							On<TaxHistory.branchID, Equal<Branch.branchID>,
					...
					.SelectMultiBound<PXResultset<Branch, TaxHistory, TaxTranReport, TaxBucketLine>>(this, new object[] { filter, taxPeriod, vendor });
		}
		...
		public virtual string FileTaxProc(TaxPeriodFilter filter)
		{
			...
			using (new PXConnectionScope())
			{
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					...
					PXAccess.MasterCollection.Organization organization = PXAccess.GetOrganizationByID(filter.OrganizationID);
					...
					using (new PXReadBranchRestrictedScope(filter.OrganizationID.SingleToArray(), _branches, requireAccessForAllSpecified: true))
					{
						...
						string reportCuryID = GetReportCuryID(rmanager, organization);
						...
						UpdateTaxTranBucketIDsFromTaxRev(taxPeriod);

						PXResultset<TaxTran> taxTransWOTaxRevs = SelectTaxTransWOTaxRevs(taxPeriod);

						CalculateAndUpdateTaxTranTaxableAmounts(taxPeriod, organization, vendor, revisionId);

						PXResultset<TaxTran> unreportedTaxTrans =
							SelectUnreportedTaxTrans(taxPeriod, vendor);

						UpdateTaxPeriodInTaxAdjustments();

						TaxTran rateNotFound = SelectTaxTranRateNotFound(taxPeriod, revisionId);
					...
					prev_hist = UpdateTaxHistory(filter, taxPeriod, prev_hist, _branches, revisionId);
		...
		protected virtual TaxHistory UpdateTaxHistory(TaxPeriodFilter filter, TaxPeriod taxPeriod, TaxHistory prev_hist, int?[] _branches, int revisionId )
		{
			RoundingManager rmanager = new RoundingManager(this, taxPeriod.VendorID, (int)filter.TaxReportRevisionID);
			using (new PXReadBranchRestrictedScope(filter.OrganizationID.SingleToArray(), _branches, requireAccessForAllSpecified: true))
			{
						...
						object.Equals(prev_hist.LineNbr, line.LineNbr) == false)
					{
						...
						if (prev_hist == null)
						{
							prev_hist = new TaxHistory
							{
								...
								TaxID = tran.TaxID,
								LineNbr = line.LineNbr,
								CuryID = rmanager.CurrentVendor.CuryID ?? PXAccess.GetBranch(tran.BranchID).BaseCuryID,
		...
		protected virtual PXResultset<TaxTran> SelectTaxTranRateNotFound(TaxPeriod taxPeriod, int revisionId)
		{
			return PXSelect<TaxTran,
					Where<TaxTran.taxPeriodID, Equal<Required<TaxTran.taxPeriodID>>,
						...
						And<TaxTran.taxType, NotEqual<TaxType.pendingSales>,
						And<
							Where<TaxTran.reportTaxAmt, IsNull,
			...
			PXUpdateJoin<
				...
				Where2<
					Where<TaxAdjustment.taxPeriod, NotEqual<TaxTran.taxPeriodID>,
						Or<Where<TaxAdjustment.taxPeriod, IsNull,
							And<TaxTran.taxPeriodID, IsNotNull>>>>,
					And<TaxAdjustment.released, Equal<True>>>>
				.Update(this);
		}

		protected virtual PXResultset<TaxTran> SelectUnreportedTaxTrans(TaxPeriod taxPeriod, Vendor vendor)
		{
			return PXSelectJoin<TaxTran,
					InnerJoin<Branch,
						On<TaxTran.branchID, Equal<Branch.branchID>>,
					...
					Where<TaxTran.vendorID, Equal<Required<TaxPeriod.vendorID>>,
						...
						And<TaxTran.taxType, NotEqual<TaxType.pendingSales>,
						And2<
							Where<Required<Vendor.taxReportFinPeriod>, Equal<True>,
								Or<TaxTran.tranDate, Less<Required<TaxPeriod.endDate>>>>,
								And2<
									Where<Required<Vendor.taxReportFinPeriod>, Equal<False>,
		...
		protected virtual void CalculateAndUpdateTaxTranTaxableAmounts(TaxPeriod taxPeriod, PXAccess.MasterCollection.Organization organization, Vendor vendor, int revisionId)
		{
			RoundingManager rmanager = new RoundingManager(this, taxPeriod.VendorID, revisionId);
			string reportCuryID = GetReportCuryID(rmanager, organization);
			...
			PXUpdateJoin<Set<TaxTran.taxPeriodID, Required<TaxPeriod.taxPeriodID>,
				...
				Set<TaxTran.reportTaxAmt, Switch<
						...
						Case<Where<CM.CurrencyRate.curyMultDiv, Equal<CuryMultDivType.mult>>, Round<Mult<TaxTran.curyTaxAmt, CurrencyRate.curyRate>, Currency.decimalPlaces>,
						Case<Where<CM.CurrencyRate.curyMultDiv, Equal<CuryMultDivType.div>>, Round<Div<TaxTran.curyTaxAmt, CurrencyRate.curyRate>, Currency.decimalPlaces>>>>>>,
				Set<TaxTran.reportTaxableAmt, Switch<
						...
						Case<Where<CM.CurrencyRate.curyMultDiv, Equal<CuryMultDivType.mult>>, Round<Mult<TaxTran.curyTaxableAmt, CurrencyRate.curyRate>, Currency.decimalPlaces>,
						Case<Where<CM.CurrencyRate.curyMultDiv, Equal<CuryMultDivType.div>>, Round<Div<TaxTran.curyTaxableAmt, CurrencyRate.curyRate>, Currency.decimalPlaces>>>>>>,
					Set<TaxTran.reportExemptedAmt, Switch<
							...
							Case<Where<CM.CurrencyRate.curyMultDiv, Equal<CuryMultDivType.mult>>, Round<Mult<TaxTran.curyExemptedAmt, CurrencyRate.curyRate>, Currency.decimalPlaces>,
							Case<Where<CM.CurrencyRate.curyMultDiv, Equal<CuryMultDivType.div>>, Round<Div<TaxTran.curyExemptedAmt, CurrencyRate.curyRate>, Currency.decimalPlaces>>>>>>,
				...
				Set<TaxTran.reportCuryRate, Switch<Case<Where<CM.CurrencyRate.curyRate, IsNotNull>, CurrencyRate.curyRate>, decimal1>,
					Set<TaxTran.reportCuryMultDiv, Switch<Case<Where<CM.CurrencyRate.curyMultDiv, IsNotNull>, CurrencyRate.curyMultDiv>, CuryMultDivType.mult>>>>>>>>>>>,
				TaxTran,
					InnerJoin<Branch,
						On<TaxTran.branchID, Equal<Branch.branchID>>,
					InnerJoin<OrganizationFinPeriod,
				...
				InnerJoin<TaxBucketLine,
						On<TaxTran.vendorID, Equal<TaxBucketLine.vendorID>,
							And<TaxTran.taxBucketID, Equal<TaxBucketLine.bucketID>>>,
						LeftJoin<TaxReportLine, On<TaxReportLine.vendorID, Equal<TaxBucketLine.vendorID>,
							And<TaxReportLine.taxReportRevisionID, Equal<TaxBucketLine.taxReportRevisionID>,
							And<TaxReportLine.lineNbr, Equal<TaxBucketLine.lineNbr>>>>,
						LeftJoin<Currency,
							On<Currency.curyID, Equal<Required<Currency.curyID>>>,
							LeftJoin<CM.CurrencyRate,
								On<CM.CurrencyRate.fromCuryID, Equal<TaxTran.curyID>,
								And<CM.CurrencyRate.toCuryID, Equal<Required<CM.CurrencyRate.toCuryID>>,
								And<CM.CurrencyRate.curyRateType, Equal<Required<CM.CurrencyRate.curyRateType>>,
								And<CM.CurrencyRate.curyEffDate, Equal<TaxTran.curyEffDate>>>>>>>>>>>,
				Where<TaxTran.vendorID, Equal<Required<TaxPeriod.vendorID>>,
					And<TaxTran.released, Equal<True>,
					And<TaxTran.voided, Equal<False>,
					And<TaxTran.taxPeriodID, IsNull,
					...
					And<TaxTran.taxType, NotEqual<TaxType.pendingPurchase>,
					And<TaxTran.taxType, NotEqual<TaxType.pendingSales>,
					And2<Where<TaxReportLine.taxZoneID, IsNull, Or<TaxReportLine.taxZoneID, Equal<TaxTran.taxZoneID>>>,
				...
				.Update(this, taxPeriod.TaxPeriodID, revisionId,
								organization.BaseCuryID, organization.BaseCuryID, reportCuryID, reportCuryID, reportCuryID, rmanager.CurrentVendor.CuryRateTypeID,
							taxPeriod.VendorID,
								vendor.TaxReportFinPeriod, taxPeriod.EndDate, vendor.TaxReportFinPeriod, taxPeriod.EndDate);
		}

		private static string GetReportCuryID(RoundingManager rmanager, PXAccess.MasterCollection.Organization organization)
		{
			return rmanager.CurrentVendor.CuryID ?? organization.BaseCuryID;
		}
