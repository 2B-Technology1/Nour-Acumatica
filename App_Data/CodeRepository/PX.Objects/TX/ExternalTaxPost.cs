	...
	public class ExternalTaxPost : PXGraph<ExternalTaxPost>
	{
		...
		public class ExternalTaxPostProcess : PXGraph<ExternalTaxPostProcess>
		{
			...
			public void Post(Document doc)
			{
				...
				var request = BuildCommitTaxRequest(doc);
			...
			public virtual CommitTaxRequest BuildCommitTaxRequest(Document doc)
			{
				...
				if (doc.Module == BatchModule.AP)
				{
					...
					if (apgraph.Document.Current != null)
					{
						APInvoiceEntryExternalTax apgraphExt = apgraph.GetExtension<APInvoiceEntryExternalTax>();
						request = apgraphExt.BuildCommitTaxRequest(apgraph.Document.Current);
					}
				}
				else if (doc.Module == BatchModule.AR)
				{
					...
					if (argraph.Document.Current != null)
					{
						ARInvoiceEntryExternalTax argraphExt = argraph.GetExtension<ARInvoiceEntryExternalTax>();
						request = argraphExt.BuildCommitTaxRequest(argraph.Document.Current);
					}
				}
				else if (doc.Module == BatchModule.CA)
				{
					...
					if (cagraph.CAAdjRecords.Current != null)
					{
						CATranEntryExternalTax cagraphExt = cagraph.GetExtension<CATranEntryExternalTax>();
						request = cagraphExt.BuildCommitTaxRequest(cagraph.CAAdjRecords.Current);
					}
				}
				return request;
			}
