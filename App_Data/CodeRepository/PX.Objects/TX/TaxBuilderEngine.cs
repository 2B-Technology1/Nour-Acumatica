...
namespace PX.Objects.TX
{
	public class TaxBuilderEngine
	{
		...
		public static string GetTaxZoneByAddress(PXGraph graph, IAddressBase adrress)
		{
			...
			if (adrress == null)
				throw new ArgumentNullException("adrress");

			if (string.IsNullOrWhiteSpace(adrress.CountryID))
				return null;

			PXResultset<TaxZoneAddressMapping> resultset = null;

			if (!string.IsNullOrWhiteSpace(adrress.PostalCode))
			{
				resultset = SelectFrom<TaxZoneAddressMapping>
					.Where<TaxZoneAddressMapping.countryID.IsEqual<P.AsString>
					.And<TaxZoneAddressMapping.stateID.IsEqual<Empty>>
					.And<P.AsString.IsBetween<TaxZoneAddressMapping.fromPostalCode, TaxZoneAddressMapping.toPostalCodeSuffixed>>>
					.View.Select(graph, adrress.CountryID, adrress.PostalCode);
			}

			if ((resultset == null || resultset.Count == 0) && !string.IsNullOrWhiteSpace(adrress.State))
			{
				resultset = SelectFrom<TaxZoneAddressMapping>
					.Where<TaxZoneAddressMapping.countryID.IsEqual<P.AsString>
					.And<TaxZoneAddressMapping.stateID.IsEqual<P.AsString>
					.And<TaxZoneAddressMapping.fromPostalCode.IsEqual<Empty>>>>
					.View.Select(graph, adrress.CountryID, adrress.State);
			}
			...
			if (resultset == null || resultset.Count == 0)
			{
				resultset = SelectFrom<TaxZoneAddressMapping>
					...
					.And<TaxZoneAddressMapping.stateID.IsEqual<Empty>
					.And<TaxZoneAddressMapping.fromPostalCode.IsEqual<Empty>>>>
					.View.Select(graph, adrress.CountryID);
			}

			if ((resultset == null || resultset.Count == 0) && !string.IsNullOrWhiteSpace(adrress.PostalCode))
			{
				resultset = SelectFrom<TaxZoneAddressMapping>
					.Where<TaxZoneAddressMapping.countryID.IsEqual<Empty>
					.And<TaxZoneAddressMapping.stateID.IsEqual<Empty>>
					.And<P.AsString.IsBetween<TaxZoneAddressMapping.fromPostalCode, TaxZoneAddressMapping.toPostalCodeSuffixed>>>
					.View.Select(graph, adrress.PostalCode);
			}

			if (resultset.Count == 0)
			{
				PXTrace.WriteWarning("Failed to find record in TaxZoneAddressMapping for the given address data => country: {0}, state: {1}, postal code: {2}", adrress.CountryID, adrress.State, adrress.PostalCode);
				return null;
			}
			else if (resultset.Count > 1)
			{
				PXTrace.WriteWarning("{0} records returned from TaxZoneAddressMapping for the given address data => country: {0}, state: {1}, postal code: {2}", adrress.CountryID, adrress.State, adrress.PostalCode);
			}

			return ((TaxZoneAddressMapping)resultset).TaxZoneID;
		}
	}
