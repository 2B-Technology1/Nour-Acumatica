...
namespace PX.Objects.TX
{
	public abstract class TaxAttribute : TaxBaseAttribute
	{
		...
		public override void CacheAttached(PXCache sender)
		{
			...
			sender.Graph.RowSelected.AddHandler(_TaxSumType, Tax_RowSelected);
		}
			...
			if ((_TaxCalc != TaxCalc.NoCalc && e.ExternalCall || _TaxCalc == TaxCalc.ManualCalc))
			{
				if (e.OldRow != null && e.Row != null)
				{
					if (taxSumOld.TaxID != taxSum.TaxID || IsTaxRowQuantityUpdated(sender, e))
					{
						...
						{
							ITaxDetail taxzonedet = MatchesCategory(cache, det, (ITaxDetail)e.OldRow);
							if (taxzonedet != null)
							{
								DelOneTax(taxcache, det, taxzonedet);
							}
						}
		...
		protected virtual void Tax_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			if (_IncludeDirectTaxLine)
			{
				...
				{
					PXUIFieldAttribute.SetEnabled(sender, row, _TaxID, false);
					PXUIFieldAttribute.SetEnabled(sender, row, _CuryTaxableAmt, false);
					PXUIFieldAttribute.SetEnabled(sender, row, _CuryTaxAmt, false);
					PXUIFieldAttribute.SetEnabled(sender, row, _CuryExpenseAmt, false);
				}
			}
		}
