...
namespace PX.Objects.TX
{
	...
	{		
		...
		protected virtual void _(Events.RowPersisting<TaxRev> e)
		{
			if ((Tax.Current != null) && !Tax.Current.DeductibleVAT.GetValueOrDefault())
			{
				if (e.Row != null)
					e.Cache.SetDefaultExt<TaxRev.nonDeductibleTaxRate>(e.Row);
			}
		}
		...
		public virtual void SetWarningsOnRowUpdate(PXCache cache, Tax newTax, Tax oldTax)
		{
			...
			if (newTax.DeductibleVAT == true && newTax.DirectTax == true)
			{
				cache.RaiseExceptionHandling<Tax.deductibleVAT>(newTax, newTax.DeductibleVAT, new PXSetPropertyException(Messages.TheseTwoOptionsCantBeCombined));
				cache.RaiseExceptionHandling<Tax.directTax>(newTax, newTax.DirectTax, new PXSetPropertyException(Messages.TheseTwoOptionsCantBeCombined));
				return;
			}

			if (newTax.ReverseTax == true && newTax.DirectTax == true)
			{
			...
			if (newTax.TaxCalcLevel == "0" && newTax.TaxCalcLevel2Exclude == true && newTax.DirectTax != true)
			{
			...
			if (newTax.DirectTax != (cache.GetValueOriginal<Tax.directTax>(newTax) as bool?) && !TryValidateTaxCombinations(cache, newTax, oldTax, out string errorMessage))
			{
				cache.RaiseExceptionHandling<Tax.directTax>(newTax, newTax.DirectTax, new PXSetPropertyException(errorMessage));
				return;
			}
		...
		protected virtual void ResetTaxFieldsOnTaxUpdate(PXCache cache, Tax newTax, Tax oldTax)
		{
			...
			{
				ResetPendingSalesTax(cache, newTax);
		...
		protected virtual void ResetPendingSalesTax(PXCache cache, Tax newTax)
		{
		...
		protected virtual void _(Events.FieldVerifying<TaxCategoryDet, TaxCategoryDet.taxCategoryID> e)
		{
			TaxCategoryDet taxCategoryDetail = e.Row;
			string taxCategoryId = (string)e.NewValue;
			if (!string.IsNullOrEmpty(taxCategoryId))
			{
				if (!TryValidateDirectIndirectTaxCombinationWithCategories(new string[] { taxCategoryId }, CurrentTax.Current?.TaxID, CurrentTax.Current?.DirectTax == true,
					out PXResultset<TaxCategoryDet> invalidCategoryCombinations))
				{
					e.Cancel = true;
					throw new PXSetPropertyException(Messages.NonImportAndImportCanNotExistTogether, taxCategoryId);
				}

				string[] allTaxZones = this.Zones.Select().RowCast<TaxZoneDet>().Select(a => a.TaxZoneID).ToArray();

				if (CurrentTax.Current?.DirectTax == true && !TryValidteDirectTaxCombinationWithTaxZoneAndTaxCategory(allTaxZones, new string[] { taxCategoryId },
					Tax.Current?.TaxID, out PXResultset<TaxCategoryDet, TaxZoneDet> invalidZoneCategoryCombinations))
				{
					e.Cancel = true;
					throw new PXSetPropertyException(Messages.SeveralImportTaxesCanNotBeInSameZoneAndCategory,
					taxCategoryId, ((TaxZoneDet)invalidZoneCategoryCombinations)?.TaxZoneID);
				}
			}
		}

		protected virtual void _(Events.FieldVerifying<TaxZoneDet, TaxZoneDet.taxZoneID> e)
		{
			TaxZoneDet taxZoneDetail = e.Row;
			string taxZoneId = (string)e.NewValue;
			if (!string.IsNullOrEmpty(taxZoneId))
			{
				string [] allTaxCategories = this.Categories.Select().RowCast<TaxCategoryDet>().Select(a => a.TaxCategoryID).ToArray();

				if (!TryValidteDirectTaxCombinationWithTaxZoneAndTaxCategory(new string[] { taxZoneId  }, allTaxCategories,
					Tax.Current?.TaxID, out PXResultset<TaxCategoryDet, TaxZoneDet> invalidZoneCategoryCombinations))
				{
					e.Cancel = true;

					if (CurrentTax.Current.DirectTax == true)
					{
						throw new PXSetPropertyException(Messages.SeveralImportTaxesCanNotBeInSameZoneAndCategory,
					((TaxCategoryDet)invalidZoneCategoryCombinations)?.TaxCategoryID, taxZoneId);
					}
					else
					{
						throw new PXSetPropertyException(Messages.NonImportAndImportCanNotExistTogether,
							((TaxCategoryDet)invalidZoneCategoryCombinations)?.TaxCategoryID);
					}
				}
			}
		}

		protected virtual bool TryValidateDirectIndirectTaxCombinationWithCategories(string[] taxCategoryIds, string taxId, bool isDirectTax, out PXResultset<TaxCategoryDet> invalidCategoryCombinations)
		{
			...
			.Where<TaxCategoryDet.taxCategoryID.IsIn<P.AsString>
				.And<TaxCategory.taxCatFlag.IsEqual<False>>
				.And<Tax.taxID.IsNotEqual<P.AsString>>
				.And<Tax.directTax.IsNotEqual<P.AsBool>>>
			.View.Select(this, new object[] { taxCategoryIds, taxId, isDirectTax });

			return invalidCategoryCombinations.Count == 0;
		}

		protected virtual bool TryValidteDirectTaxCombinationWithTaxZoneAndTaxCategory(string[] taxZoneIds,string[] taxCategoryIds, string taxId,
			out PXResultset<TaxCategoryDet, TaxZoneDet> invalidZoneCategoryCombinations)
		{
			...
			foreach (PXResult<TaxCategoryDet,TaxZoneDet, TaxCategory, Tax> row in SelectFrom<TaxCategoryDet>
			.LeftJoin<TaxZoneDet>
			.On<TaxZoneDet.taxID.IsEqual<TaxCategoryDet.taxID>>
			.InnerJoin<TaxCategory>
			.On<TaxCategory.taxCategoryID.IsEqual<TaxCategoryDet.taxCategoryID>>
			.InnerJoin<Tax>
			.On<Tax.taxID.IsEqual<TaxCategoryDet.taxID>>
			.Where<TaxZoneDet.taxZoneID.IsIn<P.AsString>
				.And<TaxCategoryDet.taxCategoryID.IsIn<P.AsString>>
				.And<TaxCategory.taxCatFlag.IsEqual<False>>
				.And<Tax.directTax.IsEqual<True>>
				.And<TaxZoneDet.taxID.IsNotNull>
				.And<TaxCategoryDet.taxID.IsNotEqual<P.AsString>>>
			.AggregateTo<GroupBy<TaxCategoryDet.taxCategoryID>, GroupBy<TaxZoneDet.taxZoneID>>
			.View.Select(this, new object[] { taxZoneIds, taxCategoryIds, taxId }))
			{
				invalidZoneCategoryCombinations.Add(row);
			}

			return invalidZoneCategoryCombinations.Count == 0;
		}

		protected bool TryValidateTaxCombinations(PXCache cache, Tax newTax, Tax oldTax, out string errorMessage)
		{
			errorMessage = string.Empty;
			string[] allCategoryIds = Categories.Select().RowCast<TaxCategoryDet>().Select(a => a.TaxCategoryID).ToArray();

			if (!TryValidateDirectIndirectTaxCombinationWithCategories(allCategoryIds, newTax.TaxID, newTax.DirectTax == true,
				out PXResultset<TaxCategoryDet> invalidCategoryCombinations))
			{
				TaxCategoryDet category = (TaxCategoryDet)invalidCategoryCombinations;
				errorMessage = PXMessages.LocalizeFormat(Messages.NonImportAndImportCanNotExistTogether, category?.TaxCategoryID);

				return false;
			}

			string[] allTaxZoneIds = this.Zones.Select().RowCast<TaxZoneDet>().Select(a => a.TaxZoneID).ToArray();

			if (!TryValidteDirectTaxCombinationWithTaxZoneAndTaxCategory(allTaxZoneIds, allCategoryIds,
				Tax.Current?.TaxID, out PXResultset<TaxCategoryDet, TaxZoneDet> invalidZoneCategoryCombinations))
			{
				TaxCategoryDet category = (TaxCategoryDet)invalidZoneCategoryCombinations;
				TaxZoneDet zone = (TaxZoneDet)invalidZoneCategoryCombinations;
				errorMessage = PXMessages.LocalizeFormat(Messages.SeveralImportTaxesCanNotBeInSameZoneAndCategory,
				category?.TaxCategoryID, zone?.TaxZoneID);
		...
		protected virtual void _(Events.FieldUpdated<Tax, Tax.directTax> e)
		{
			...
			{
				e.Cache.RaiseExceptionHandling<Tax.taxApplyTermsDisc>(tax, CSTaxTermsDiscount.NoAdjust, null);

				e.Cache.SetValueExt<Tax.taxCalcLevel2Exclude>(tax, true);
				e.Cache.SetValueExt<Tax.taxApplyTermsDisc>(tax, CSTaxTermsDiscount.NoAdjust);

				string inclusiveLineLevel = CSTaxCalcType.Item + CSTaxCalcLevel.Inclusive;
				e.Cache.SetValueExt<Tax.taxCalcLevel>(tax, CSTaxCalcLevel.Inclusive);
				e.Cache.SetValueExt<Tax.taxCalcType>(tax, CSTaxCalcType.Item);
				e.Cache.SetValueExt<Tax.taxCalcRule>(tax, inclusiveLineLevel);
			}
			else
			{
				e.Cache.SetValueExt<Tax.taxCalcLevel2Exclude>(tax, false);
			}
		}
		...
		where Field : IBqlField
		{
			...
			sender.SetValue<Field>(tax, newAccountOrSubValue);
		}
		...
		protected virtual void Tax_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			bool isDirectTax = tax.DirectTax == true;
			...
			PXUIFieldAttribute.SetEnabled<Tax.taxCalcRule>(cache, tax, !isExternal && !isDirectTax);
			PXUIFieldAttribute.SetEnabled<Tax.taxCalcLevel2Exclude>(cache, tax, !isExternal && !isPerUnitTax && !isDirectTax);
			PXUIFieldAttribute.SetEnabled<Tax.taxApplyTermsDisc>(cache, tax, !isExternal && !isPerUnitTax && !isDirectTax);
		...
		{
			bool isPerUnitTax = tax.TaxType == CSTaxType.PerUnit;
			bool enabled = tax.ReportExpenseToSingleAccount == true && (tax.TaxType == CSTaxType.Use || tax.TaxType == CSTaxType.Sales || tax.DeductibleVAT == true);
		...
		public void CheckFieldIsEmpty<Field>(PXCache sender, PXRowPersistingEventArgs e)
		where Field : IBqlField
		{
