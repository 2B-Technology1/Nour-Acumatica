...
namespace PX.Objects.TX
{
	...
	{	
		...
		[PXButton(Category = CS.ActionCategories.ReportManagement)]
		...
		public class TaxBucketAnalizer
		{
			...
			private PXSelectJoin<TaxBucketLine,
				...
				LeftJoinSingleTable<Vendor,
					On<TaxBucketLine.vendorID, Equal<Vendor.bAccountID>>,
			...
			public Func<PXResult<TaxReportLine, TaxBucketLine, Currency, TaxTranRevReport>, bool> showTaxReportLine
			{
				get;
                set;
			}
			...
			{
				return _vendorBucketLines
					.Select(VendorId, TaxReportVersion, LineType)
					.AsEnumerable()
					.Cast<PXResult<TaxBucketLine, TaxReportLine, Vendor, Currency>> ()
					.Where(set => showTaxReportLine( new PXResult<TaxReportLine, TaxBucketLine, Currency, TaxTranRevReport>(set, set, set, new TaxTranRevReport())));
			}
			...
			public TaxBucketAnalizer(PXGraph graph, int BAccountID, string TaxLineType, int TaxreportRevisionID)
			{
				...
				_graph = graph;
				_vendorBucketLines = new PXSelectJoin<TaxBucketLine,
					LeftJoin<TaxReportLine,
						On<TaxBucketLine.lineNbr, Equal<TaxReportLine.lineNbr>,
						And<TaxBucketLine.taxReportRevisionID, Equal<TaxReportLine.taxReportRevisionID>,
						And<TaxBucketLine.vendorID, Equal<TaxReportLine.vendorID>>>>,
		...
		public static Dictionary<int, List<int>> AnalyseBuckets(
			...
			string TaxLineType,
			bool CalcWithZones,
			Func<PXResult<TaxReportLine, TaxBucketLine, Currency, TaxTranRevReport>, bool> ShowTaxReportLine = null)
        {
