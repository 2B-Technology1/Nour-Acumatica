	...
	public class OpportunityWorkflow : PX.Data.PXGraphExtension<OpportunityMaint>
	{
		...
		protected static void Configure(WorkflowContext<OpportunityMaint, CROpportunity> context)
		{
			...
			var actionCreateTask = context.ActionDefinitions.CreateExisting(
				OpportunityMaint_ActivityDetailsExt_Actions.ActionNames.NewTask_Workflow,
				a => a.WithCategory(categoryActivities));
			var actionCreateNote = context.ActionDefinitions.CreateExisting(
				OpportunityMaint_ActivityDetailsExt_Actions.ActionNames.NewActivity_Note_Workflow,
				a => a.WithCategory(categoryActivities));
			...
			context.AddScreenConfigurationFor(screen =>
			{
				return screen
					...
					.WithActions(actions =>
					{
						...
						actions.Add<CreateSOOrder>(g => g.CreateSalesOrder, c => c.WithCategory(categoryRecordCreation).WithPersistOptions(ActionPersistOptions.NoPersist));
