...
namespace PX.Objects.CR.Workflow.Quote
{
	...
	using CreateContactExt = QuoteMaint_CRCreateContactAction;
	using CreateAccountExt = QuoteMaint_CRCreateBothContactAndAccountAction;
	...
	public class QuoteWorkflow : PXGraphExtension<QuoteMaint>
	{
		...
		protected static void Configure(WorkflowContext<QuoteMaint, CRQuote> context)
		{
			...
			var actionReassign = context.ActionDefinitions
				.CreateExisting(nameof(QuoteMaint.Approval.ReassignApproval), a => a
					.WithCategory(categoryApproval)
					.IsHiddenWhen(
						!(conditions.IsApprovalMapEnabled
						&& conditions.IsPendingApprovalState)
			...
			var actionCreateTask = context.ActionDefinitions.CreateExisting(
				QuoteMaint_ActivityDetailsExt_Actions.ActionNames.NewTask_Workflow,
				a => a.WithCategory(categoryActivities));

			var actionCreateEmail = context.ActionDefinitions.CreateExisting(
				QuoteMaint_ActivityDetailsExt_Actions.ActionNames.NewMailActivity_Workflow,
				a => a.WithCategory(categoryActivities));

			var actionCreatePhoneCall = context.ActionDefinitions.CreateExisting(
				QuoteMaint_ActivityDetailsExt_Actions.ActionNames.NewActivity_Phonecall_Workflow,
				a => a.WithCategory(categoryActivities));
			...
			Workflow.IConfigured DefaultSalesQuoteNoApprovalMapFlow(Workflow.INeedStatesFlow flow)
			{
				...
				void DisableQuoteMain(FieldState.IContainerFillerFields fields)
				{
					...
					fields.AddField<ownerID>(f => f.IsDisabled());
					fields.AddField<bAccountID>(f => f.IsDisabled());
					fields.AddField<branchID>(f => f.IsDisabled());
					fields.AddField<projectID>(f => f.IsDisabled());
					fields.AddField<externalRef>(f => f.IsDisabled());
					fields.AddField<workgroupID>(f => f.IsDisabled());
