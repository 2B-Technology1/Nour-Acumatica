...
namespace PX.Objects.CR
{
	...
	public partial class CRCase : IBqlTable, IAssign, INotable, IPXSelectable
	{
		#region Keys
		...
		{
			public static CRCase Find(PXGraph graph, string caseCD, PKFindOptions options = PKFindOptions.None) => FindBy(graph, caseCD, options);
		}
		...
		public static class FK
		{
			...
			/// Owner
			/// </summary>
			public class Owner : CR.Contact.PK.ForeignKeyOf<CRCase>.By<ownerID> { }
				...
				IsNull<
					Minus1<
						Search<CRActivity.startDate,
							Where<CRActivity.refNoteID, Equal<CRCase.noteID>,
							And2<
								Where<CRActivity.isPrivate, IsNull,
									Or<CRActivity.isPrivate, Equal<False>>>,
							And<CRActivity.ownerID, IsNotNull,
							And2<
								Where<CRActivity.incoming, IsNull,
									Or<CRActivity.incoming, Equal<False>>>,
							And<
								Where<CRActivity.isExternal, IsNull,
									Or<CRActivity.isExternal, Equal<False>>>>>>>>,
							OrderBy<Asc<CRActivity.startDate>>>,
						CRCase.createdDateTime>,
					int0>
			)
		...
		public virtual int? TimeResolution
		{
			get
			{
				...
				var span = startTime.Value - CreatedDateTime.Value;
        ...
        [PXFormula(typeof(DateDiff<CRCase.createdDateTime, PXDateAndTimeAttribute.now, DateDiff.minute>))]
