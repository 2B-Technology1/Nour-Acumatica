	...
	public static class UDFHelper
	{
		...
		public static IEnumerable<PX.CS.KeyValueHelper.ScreenAttribute> GetUDFFields(string screenID, string udfTypeField = null)
		{
			...
			foreach (var attr in KeyValueHelper.Def
									?.GetAttributes(ScreenHelper.UnmaskScreenID(screenID))
									?.Where(f =>
		...
		public static void FillfromPopupUDF(PXCache destCache, PXCache sourceUDFPopupCache, Type targetGraphType, object destRow)
		{
			...
			foreach (PopupUDFAttributes attr in sourceUDFPopupCache.Cached.OfType<PopupUDFAttributes>())
			{
		...
		{
			graph.FieldVerifying.AddHandler<PopupUDFAttributes.displayName>((s, e) =>
			{
				if (e.Row is PopupUDFAttributes row
				    && (row.Value == null || string.IsNullOrEmpty(row.Value)))
				{
					throw new PXSetPropertyException(Messages.FillReqiredUDFAttributes, PXErrorLevel.Error);
				}
			});
			graph.FieldSelecting.AddHandler<PopupUDFAttributes.value>((s, e) =>
			{
				var row = e.Row as PopupUDFAttributes;
				if (row == null)
					return;
				var state = GetGraphUDFFieldState(s.Graph.GetType(), row.AttributeID);
				if (state != null)
				{
					state.Required = true;
