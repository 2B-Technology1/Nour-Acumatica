	...
	public class QuoteMaintExternalTax : ExternalTaxBase<QuoteMaint, CRQuote>
	{
		...
		public void Persist()
		{
			if (Base.Quote.Current != null && IsExternalTax(Base.Quote.Current.TaxZoneID) && !skipExternalTaxCalcOnSave && Base.Quote.Current.IsTaxValid != true)
			{
				if (!PXLongOperation.IsLongOperationContext())
				{
					...
					var ext = graph.GetProcessingExtension<QuoteMaintExternalTax>();

					PXLongOperation.StartOperation(Base, delegate ()
					{
						ext.CalculateExternalTax(graph.Quote.Current);
					});
				}
				else
				{
		...
		protected virtual void _(Events.RowUpdated<CRQuote> e)
		{
			if (IsExternalTax(e.Row.TaxZoneID) && !e.Cache.ObjectsEqual<CRQuote.contactID, CRQuote.taxZoneID, CRQuote.branchID, CRQuote.locationID,
					CRQuote.curyAmount, CRQuote.shipAddressID, CRQuote.avalaraCustomerUsageType>(e.Row, e.OldRow))
			{
		...
		protected virtual GetTaxRequest BuildGetTaxRequest(CRQuote quote)
		{
			...
			TaxZone taxZone = (TaxZone)PXSetup<TaxZone, Where<TaxZone.taxZoneID, Equal<Required<CROpportunity.taxZoneID>>>>.Select(Base, quote.TaxZoneID);
			...
			request.CustomerUsageType = quote.AvalaraCustomerUsageType;
			request.ExemptionNo = quote.ExternalTaxExemptionNumber;
