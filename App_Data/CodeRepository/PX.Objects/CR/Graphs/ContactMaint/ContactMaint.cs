...
namespace PX.Objects.CR
{
	public class ContactMaint : PXGraph<ContactMaint, Contact, Contact.displayName>, ICaptionable
	{
		...
		public SelectContactEmailSyncBase<
			SelectFrom<Contact>
			.LeftJoin<BAccount>
		...
		public SelectFrom<CRCampaignMembers>
					...
					.Where<CRCampaignMembers.contactID.IsEqual<Contact.contactID.FromCurrent>>
				.View Members;
		...
		protected virtual void _(Events.RowSelected<Contact> e)
		{
			...
			NWatchers.Cache.AllowInsert = isNotInserted;
			...
			PXUIFieldAttribute.SetEnabled<Contact.bAccountID>(e.Cache, row, true);
			PXUIFieldAttribute.SetEnabled<Contact.classID>(e.Cache, row, true);
		...
		protected virtual void _(Events.FieldUpdated<Contact, Contact.status> e)
		{
			...
			bool IsContactInactive = row.Status is ContactStatus.Inactive;

			row.IsActive = IsContactInactive
				? false
				: true;

			SetLinkedUserStatus(row);

			BAccount account = BAccount.PK.Find(this, row.BAccountID);
		...
		public class PrimaryContactGraphExt : PXGraphExtension<ContactMaint>
		{
			...
			protected virtual void _(Events.RowPersisted<Contact> e)
			{
				...
				BAccount account = BAccount.PK.Find(Base, oldBAID);

				if (account == null) return;

				if (account.PrimaryContactID == contact.ContactID)
				{
		...
		public class CRDuplicateEntitiesForContactGraphExt : CRDuplicateEntities<ContactMaint, Contact>
		{
			...
			public class Workflow : PXGraphExtension<CRDuplicateEntitiesForContactGraphExt, ContactWorkflow, ContactMaint>
			{
				...
				protected static void Configure(WorkflowContext<ContactMaint, Contact> context)
				{
			...
			{
				return new PXResult<Contact, Address>(
					duplicateDocument.Base as Contact,
					Base.AddressCurrent.SelectSingle()
				);
			}
		...
		public class UpdateRelatedContactInfoFromContactGraphExt : CRUpdateRelatedContactInfoGraphExt<ContactMaint>
		{
			...
			protected virtual void _(Events.RowPersisted<Address> e)
			{
				...
				// Acuminator disable once PX1043 SavingChangesInEventHandlers [ISV]
				UpdateAddress(e.Cache, row, ContactRelatedAddresses, contact.ContactID);
			}
		...
		public class CreateAccountFromContactGraphExt : CRCreateAccountAction<ContactMaint, Contact>
		{
			...
			protected override PXSelectBase<CRPMTimeActivity> Activities => Base.GetExtension<ContactMaint_ActivityDetailsExt>().Activities;
