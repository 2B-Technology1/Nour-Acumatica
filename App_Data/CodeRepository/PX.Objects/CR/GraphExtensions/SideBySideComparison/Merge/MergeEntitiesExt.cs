...
namespace PX.Objects.CR.Extensions.SideBySideComparison.Merge
{
		...
		where TMain : class, IBqlTable, INotable, new()
	{
		...
		public override string LeftValueDescription => MessagesNoPrefix.MergeEntitiesLeftValueDescriptionCaption;
		public override string RightValueDescription => MessagesNoPrefix.MergeEntitiesRightValueDescriptionCaption;
		...
		{
			var graphMainEntityCache = Base.Caches[typeof(TMain)];

			Guid[] duplicateEntityFiles = PXNoteAttribute.GetFileNotes(graphMainEntityCache, duplicateEntity);
			PXNoteAttribute.SetFileNotes(graphMainEntityCache, targetEntity, duplicateEntityFiles);

			var targetNote = PXNoteAttribute.GetNote(graphMainEntityCache, targetEntity);
			var duplicateNote = PXNoteAttribute.GetNote(graphMainEntityCache, duplicateEntity);
			if (!String.IsNullOrWhiteSpace(duplicateNote))
			{
				if (!String.IsNullOrWhiteSpace(targetNote)) targetNote += "\r\n\r\n";
				targetNote += duplicateNote;
				PXNoteAttribute.SetNote(graphMainEntityCache, targetEntity, targetNote);
			}
		}
		...
		public override IEnumerable<MergeComparisonRow> UpdateComparisons(IEnumerable<MergeComparisonRow> comparisons)
		{
			foreach (var comparison in base.UpdateComparisons(comparisons))
			{
				...
				if (comparison.LeftFieldState.IsReadOnly
					|| !comparison.LeftFieldState.Enabled
					|| comparison.RightFieldState.IsReadOnly
					|| !comparison.RightFieldState.Enabled)
				{
					comparison.Hidden = true;
					comparison.Selection = Filter.Current.TargetRecord is MergeEntitiesFilter.targetRecord.SelectedRecord
						? ComparisonSelection.Right
						: ComparisonSelection.Left;
				}
		...
		public virtual (TMain Target, TMain Duplicate) ProcessMerge()
		{
			...
			Base.Caches[typeof(TMain)].Current = target;
		...
		protected virtual void _(Events.FieldSelecting<MergeEntitiesFilter.caption> e)
		{
			if (e?.Row == null) return;

			e.ReturnValue = PXMessages.Localize(MessagesNoPrefix.MergeEntitiesCaption);
		}
