...
namespace PX.Objects.CR.Extensions.SideBySideComparison
{
		...
		where TComparisonRow : ComparisonRow, new()
	{
		...
		/// </remarks>
		public virtual string LeftValueDescription => MessagesNoPrefix.MergeEntitiesLeftColumnDefaultCaption;
		...
		/// The string that is provided here should be localized, but the class itself doesn't collect strings.
		/// </remarks>
		public virtual string RightValueDescription => MessagesNoPrefix.MergeEntitiesRightColumnDefaultCaption;
		...
		protected virtual void PrimaryRowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			// Acuminator disable once PX1050 HardcodedStringInLocalizationMethod [no other way to fix]
			// Acuminator disable once PX1051 NonLocalizableString [no other way to fix]
			Base.Caches[typeof(TComparisonRow)]
				.AdjustUIReadonly()
				.For<ComparisonRow.leftValue>(a => a.DisplayName = PXMessages.LocalizeNoPrefix(LeftValueDescription))
				.For<ComparisonRow.rightValue>(a => a.DisplayName = PXMessages.LocalizeNoPrefix(RightValueDescription));
		}
		...
		protected virtual void _(Events.FieldSelecting<TComparisonRow, ComparisonRow.leftValue> e)
		{
			if (e.Row != null)
			{
				e.ReturnState = PXFieldState.CreateInstance(
					FieldStateExtensions.Copy(e.Row.LeftFieldState),
		...
		protected virtual void _(Events.FieldSelecting<TComparisonRow, ComparisonRow.rightValue> e)
		{
			...
			{
				e.ReturnState = PXFieldState.CreateInstance(
					FieldStateExtensions.Copy(e.Row.RightFieldState),
