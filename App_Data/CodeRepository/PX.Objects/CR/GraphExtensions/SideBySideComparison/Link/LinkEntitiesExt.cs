...
namespace PX.Objects.CR.Extensions.SideBySideComparison.Link
{
		...
		where TFilter : LinkFilter, new()
	{
		...
		public override IEnumerable<LinkComparisonRow> FilterComparisons(IEnumerable<LinkComparisonRow> comparisons)
		{
			foreach (var comparison in base.FilterComparisons(comparisons))
			{
				if (comparison.LeftFieldState.IsReadOnly
					|| !comparison.LeftFieldState.Enabled)
				{
					// field is disabled on the target record - skip
					continue;
				}
		...
		protected virtual void _(Events.FieldSelecting<LinkFilter.caption> e)
		{
			...
			e.ReturnValue = PXMessages.Localize(MessagesNoPrefix.LinkEntitiesCaption);
		}
