...
namespace PX.Objects.CR.Extensions.CRDuplicateEntities
{
		...
		where TMain : class, IBqlTable, INotable, new()
	{
		...
		public virtual IEnumerable duplicates(bool? forceSelect = null)
		{
			...
			if (DuplicateDocuments.Cache.GetStatus(entity) == PXEntryStatus.Inserted && !PXTransactionScope.IsScoped)
			{
				using (new PXTransactionScope())
				{
					...
					PersistGrams(GetGramContext(entity));
			...
			foreach (PXResult rec in possibleDuplicates)
			{
				...
				var location = dupRecord.DuplicateContactType switch
				{
					ContactTypesAttribute.Person => null,
					ContactTypesAttribute.Lead => null,
					ContactTypesAttribute.BAccountProperty => rec.GetItem<Standalone.Location>()
				};
		...
		protected virtual IEnumerable duplicatesForLinking()
		{
				...
				.Where(WhereLinkingMet))
			{
				...
				yield return new CRDuplicateResult(
					...
					rec.GetItem<Standalone.Location>());
			}
		}
		...
		public override void Initialize()
		{
			...
			BqlCommand bqlCommand = BqlTemplate.OfCommand<
					...
					.InnerJoin<CRDuplicateGrams>
						...
						.And<CRDuplicateGrams.entityName.IsEqual<CRGrams.entityName>
					...
					.LeftJoin<Standalone.Location>
						.On<Standalone.Location.locationID.IsEqual<BAccountR.defLocationID>>
					.Where<
			...
			where T : CRDuplicateRecord
		{
			...
			var screenId = Base.Accessinfo.GetNormalizedScreenID();
		...
		protected virtual void _(Events.RowPersisted<DuplicateDocument> e)
		{
			...
			// Acuminator disable once PX1043 SavingChangesInEventHandlers [tran status is ok]
			var result = PersistGrams(GetGramContext(row));

			if (result.IsGramsCreated)
			{
				DuplicateDocuments.Cache.SetValue<DuplicateDocument.duplicateStatus>(row, result.NewDuplicateStatus);
				DuplicateDocuments.Cache.SetValue<DuplicateDocument.grammValidationDateTime>(row, result.GramValidationDate);
		...
		public abstract PXResult<Contact> GetGramContext(DuplicateDocument duplicateDocument);
		...
		public virtual void CheckBlockingOnEntry(DuplicateDocument duplicateDocument)
		{
			...
			if (this.Documents.View.Answer == WebDialogResult.None
				...
				&& !Processor.GramSourceUpdated(GetGramContext(duplicateDocument)))
			{
				(bool anyFound, var duplicates) = CheckIfAnyDuplicates(duplicateDocument);

				if (anyFound)
				{
					var blocked = Processor.CheckIsBlocked(GetGramContext(duplicateDocument), duplicates)?.ToList();
		...
		public virtual (bool IsGramsCreated, string NewDuplicateStatus, DateTime? GramValidationDate) PersistGrams(PXResult<Contact> entities)
		{
			try
			{
				return Processor.PersistGrams(entities);
			}
			catch (Exception e)
			{
