...
namespace PX.Objects.CR.Extensions.CRCreateInvoice
{
		...
		where TMaster : class, IBqlTable, new()
	{
		...
		public virtual void _(Events.RowSelected<CreateInvoicesFilter> e)
		{
			...
			bool isManualNumbering = Numbering.PK.Find(Base, setup?.InvoiceNumberingID)?.UserNumbering == true;

			PXUIFieldAttribute.SetVisible<CreateInvoicesFilter.refNbr>(e.Cache, row, isManualNumbering);
			PXDefaultAttribute.SetPersistingCheck<CreateInvoicesFilter.refNbr>(e.Cache, row, isManualNumbering ? PXPersistingCheck.NullOrBlank : PXPersistingCheck.Nothing);
		}
				...
				if (baccount.Type == BAccountType.VendorType || baccount.Type == BAccountType.ProspectType)
				{
					...
					if (result == WebDialogResult.Yes)
					{
						...
						PXLongOperation.StartOperation(Base, () =>
						{
							...
							extension.ConvertToCustomerAccount(baccount, masterEntity);
						});
					}
		...
		protected virtual void DoCreateInvoice(ARInvoiceEntry docgraph, Document masterEntity, CreateInvoicesFilter filter)
		{
			...
			invoice.CuryDocumentDiscTotal = masterEntity.ManualTotalEntry == true
				? 0
			...
			if (filter.RecalculateDiscounts != true && filter.OverrideManualDiscounts != true)
			{
				...
				if (isCustomerDiscountsFeatureInstalled == false && masterEntity.ManualTotalEntry == false)
				{
					docgraph.Document.Cache.SetValueExt<ARInvoice.curyDiscTot>(docgraph.Document.Current, masterEntity.CuryDiscTot);
				}
