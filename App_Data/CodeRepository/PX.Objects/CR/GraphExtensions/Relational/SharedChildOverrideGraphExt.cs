		...
		public virtual void UpdateRelated(Document row, Document oldRow)
		{
			...
			if (row.RelatedID != oldRow.RelatedID)
			{
				...
				if (row.RelatedID == null)
				{
					...
					if (relatedChild != null && oldRelated.ChildID == row.ChildID)
					{
						...
						PrimaryDocument.Cache.SetValueExt(row, nameof(row.ChildID), child.ChildID);
					}
				}
				...
				if (related != null && row.IsOverrideRelated == false)
				{
					...
					PrimaryDocument.Cache.SetValueExt(row, nameof(row.ChildID), relatedChild.ChildID);
				}
				else
				{
			...
			if (row.IsOverrideRelated != oldRow.IsOverrideRelated)
			{
				...
				if (row.IsOverrideRelated == true)
				{
					...
					PrimaryDocument.Cache.SetValueExt(row, nameof(row.ChildID), null);
				}
