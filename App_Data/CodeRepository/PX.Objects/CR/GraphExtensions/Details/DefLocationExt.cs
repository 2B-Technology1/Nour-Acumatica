...
namespace PX.Objects.CR.Extensions
{
	...
	/// </summary>
	public abstract class DefLocationExt<TGraph, TDefContactAddress, TLocationDetails, TMaster, TBAccountID, TDefLocationID> : PXGraphExtension<TGraph>, IAddressValidationHelper
		...
		where TDefLocationID : class, IBqlField
	{
		...
		public override void Initialize()
		{
			...
			LocationDetailsExtension = Base.GetExtension<TLocationDetails>()
						?? throw new PXException(Messages.GraphHaveNoExt, typeof(TLocationDetails).Name);
			DefContactAddressExtension = Base.GetExtension<TDefContactAddress>()
						?? throw new PXException(Messages.GraphHaveNoExt, typeof(TDefContactAddress).Name);
		...
		public virtual void setDefaultLocation()
		{
			...
			var row = this.LocationDetailsExtension.Locations.Current;
		...
		public virtual void Persist(Action del)
		{
			...
			if (master != null && Base.Caches[typeof(TMaster)].GetStatus(master) == PXEntryStatus.Updated)
			{
				...
				foreach (CRLocation location in this.LocationDetailsExtension.Locations.Select())
				{
		...
		public virtual void ValidateAddress()
		{
			...
			{
				PXAddressValidator.Validate<Address>(Base, locAddress, true, true);
			}
		}
		...
		public virtual void SetLocationAsDefault(CRLocation row)
		{
			...
			foreach (CRLocation other in this.LocationDetailsExtension.Locations.Select()) 
			{
