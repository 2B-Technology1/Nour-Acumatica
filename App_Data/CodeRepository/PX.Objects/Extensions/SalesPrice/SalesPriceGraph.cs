...
namespace PX.Objects.Extensions.SalesPrice
{
			...
			where TPrimary : class, IBqlTable, new()
	{
		...
		protected class DetailMapping : IBqlMapping
		{
			...
			/// <exclude />
			public Type SkipLineDiscounts = typeof(Detail.skipLineDiscounts);
		}
		...
		protected virtual decimal GetPrice(PXCache sender, Detail row)
		{
			...
			decimal? price = GetSalesPriceItemAndCalculatedPrice(sender, customerPriceClass, doc.BAccountID,
					row.InventoryID, row.SiteID, info.GetCM(),
				  row.UOM, row.Quantity, doc.DocumentDate ?? sender.Graph.Accessinfo.BusinessDate.Value, row.CuryUnitPrice, taxCalcMode).Item2;
			return price ?? 0m;
		}
		...
		protected virtual (ARSalesPriceMaint.SalesPriceItem, decimal?) GetSalesPriceItemAndCalculatedPrice(
			...
			string taxCalcMode)
		{
			return ARSalesPriceMaint.SingleARSalesPriceMaint.GetSalesPriceItemAndCalculatedPrice(
				 sender,
				 custPriceClass,
				 customerID,
				 inventoryID,
        ...
        /// <summary>The FieldUpdated2 event handler for the <see cref="Detail.SiteID" /> field. When the Warehouse field value is changed, <see cref="Detail.CuryUnitPrice" /> is assinged a new value.</summary>
        /// <param name="e">Parameters of the event.</param>
        protected virtual void _(Events.FieldUpdated<Detail, Detail.siteID> e)
        {
            if (e.Row.ManualPrice != true && e.Row.IsFree != true && !e.Cache.Graph.IsCopyPasteContext && !e.Cache.Graph.IsImportFromExcel)
            {
                e.Cache.SetDefaultExt<Detail.curyUnitPrice>(e.Row);
            }
        }
		...
		protected virtual void _(Events.RowUpdated<Detail> e)
		{
			...
			if ((e.ExternalCall || e.Cache.Graph.IsImport)
                    ...
                    && e.Cache.ObjectsEqual<Detail.siteID>(e.Row, e.OldRow)
			...
			if (
				...
				&& e.Row.IsFree != true
				&& !e.Cache.Graph.IsCopyPasteContext
			)
			{
				string customerPriceClass = ARPriceClass.EmptyPriceClass;
				PriceClassSource source = PriceClassSource.Select();
				...
				Document doc = Documents.Current;
				CurrencyInfo currencyInfo = SelectCurrencyInfo(doc);
				string taxCalcMode = string.IsNullOrEmpty(doc.TaxCalcMode) ? TaxCalculationMode.TaxSetting : doc.TaxCalcMode;

				ARSalesPriceMaint salesPriceMaint = ARSalesPriceMaint.SingleARSalesPriceMaint;
				bool alwaysFromBaseCury = salesPriceMaint.GetAlwaysFromBaseCurrencySetting(e.Cache);
				...
				try
				{
					priceItem = salesPriceMaint.FindSalesPrice(
						...
						Math.Abs(e.Row.Quantity ?? 0m),
						e.Row.UOM,
						doc.DocumentDate ?? e.Cache.Graph.Accessinfo.BusinessDate.Value,
						taxCalcMode);

					// Acuminator disable once PX1047 RowChangesInEventHandlersForbiddenForArgs [Regular C# field used for optimization]
					e.Cache.SetValue<Detail.skipLineDiscounts>(e.Row, priceItem?.SkipLineDiscounts ?? false);
				}
