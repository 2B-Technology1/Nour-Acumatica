		...
		where TPrimary : class, IBqlTable, new()
	{
		...
		protected override TPrimary DoValidateCCPayment(TPrimary doc)
		{
			if (!RunPendingOperations(doc))
			{
				...
				ICCPaymentTransaction cCProcTran = GetProcTrans().FirstOrDefault();
				ARPaymentEntry aRPaymentEntry = Base as ARPaymentEntry;
				if (aRPaymentEntry._IsLocalValidation)
				{
					V2.TransactionData tranData = new V2.TransactionData()
					{
						AuthCode = cCProcTran.AuthNumber,
						TranID = cCProcTran.PCTranNumber,
						Amount = cCProcTran.Amount.GetValueOrDefault(),
						CcvVerificationStatus = V2Converter.ConvertCardVerificationStatus(CVVVerificationStatusCode.GetCcvVerificationStatus(storedTran.CVVVerification)),
						TranStatus = V2Converter.ConvertTranStatusToV2(CCTranStatusCode.GetCCTranStatus(cCProcTran.TranStatus)),
						SubmitTime = storedTran.LastActivityDate.Value,
						TranType = V2Converter.ConvertTranTypeToV2(CCTranTypeCode.GetTranTypeByTranTypeStr(cCProcTran.TranType)),
						ResponseReasonText = ((PaymentTransactionDetail)cCProcTran).PCResponseReasonText,
						TranUID = storedTran.NoteID,
						IsLocalValidation = true
					};
					RecordTranData(tranData.TranType.Value, GetPaymentDoc(doc), tranData, true);
				...
				if (storedTran != null)
				{
					bool synced = TrySyncByStoredIds(doc, storedTran);
		...
		protected override bool TrySyncByStoredIds(TPrimary doc, IExternalTransaction extTran)
		{
			...
			if (extTran.Active == false && extTran.ProcStatus == ExtTransactionProcStatusCode.Unknown)
			{
				if (!string.IsNullOrEmpty(extTran.TranNumber))
				{
					// We may have TranNumber from the start, like in Stripe
					try
					{
				...
				if (!synced && extTran.NoteID != null
					&& IsFeatureSupported(SelectedProcessingCenter, CCProcessingFeature.TransactionFinder, false)
					&& IsPaymentHostedFormSupported(SelectedProcessingCenter))
				{
					// We may be able to search by our ID, like in Fortis
					V2.TransactionData tranData = GetPaymentProcessing().FindTransaction(extTran.NoteID.Value, SelectedProcessingCenter);
					var extTranDetail = GetExtTranDetails().FirstOrDefault(i => i.TransactionID == extTran.TransactionID);
					if (tranData != null && extTranDetail != null)
					{
						extTranDetail.TranNumber = tranData.TranID;
						extTranDetail = ExternalTransaction.Update(extTranDetail);
		...
		public virtual IEnumerable SyncPaymentTransaction(PXAdapter adapter)
		{
			...
			{
				PXLongOperation.StartOperation(Base, () => SyncImpl(doc, tranId));
			}
			...
			{
				var processingGraph = GetProcessingGraph();
				var currDoc = SetCurrentDocument(processingGraph, localDoc);
				var ext = GetPaymentTransactionAcceptFormExt(processingGraph);
				ext.SyncPaymentTransactionById(currDoc, new List<string>() { localTranId });
			};
		}
			...
			foreach (TPrimary doc in adapter.Get<TPrimary>())
			{
				...
				else
				{
					...
					try
					{
						...
						if (CCProcessingFeatureHelper.IsFeatureSupported(procCenter, CCProcessingFeature.PaymentForm))
						{
							string meansOfPayment = PaymentMethod.PK.Find(Base, SelectedPaymentMethod)?.PaymentType;

							var options = GetPaymentProcessing().PreparePaymentForm(pDoc, SelectedProcessingCenter, SelectedBAccount, NeedSaveCard(), V2.CCTranType.AuthorizeOnly, extTran.NoteID, meansOfPayment);
							extTran = UpdateExtTran(extTran, options);
							throw new PXPluginRedirectException<PXPluginRedirectOptions>(options);
						}
						else
						{
							ShowAcceptPaymentForm(V2.CCTranType.AuthorizeOnly, extTran.NoteID);
						}
					}
				...
				else
				{
					...
					try
					{
						...
						if (CCProcessingFeatureHelper.IsFeatureSupported(procCenter, CCProcessingFeature.PaymentForm))
						{
							...
							var options = GetPaymentProcessing().PreparePaymentForm(pDoc, SelectedProcessingCenter, SelectedBAccount, NeedSaveCard(), V2.CCTranType.AuthorizeAndCapture, extTran.NoteID, meansOfPayment);
							extTran = UpdateExtTran(extTran, options);
							throw new PXPluginRedirectException<PXPluginRedirectOptions>(options);
						}
						else
						{
							ShowAcceptPaymentForm(V2.CCTranType.AuthorizeAndCapture, extTran.NoteID);
						}
					}
					catch (PXBaseRedirectException ex)
					{
		...
		private void ProcessMobilePayment(TPrimary doc, ExternalTransactionDetail extTran, V2.CCTranType tranType)
		{
			...
			if (pDoc.PMInstanceID == PaymentTranExtConstants.NewPaymentProfile && !TranHeldForReview())
			{
				...
				{
					string meansOfPayment = PaymentMethod.PK.Find(Base, SelectedPaymentMethod)?.PaymentType;
					options = GetPaymentProcessing().PreparePaymentForm(pDoc, SelectedProcessingCenter, SelectedBAccount, NeedSaveCard(), tranType, extTran.NoteID, meansOfPayment);
					extTran = UpdateExtTran(extTran, options);
		...
		{
			bool isAllow = GetProcessingCenterById(SelectedProcessingCenter)?.UseAcceptPaymentForm ?? false;
			if (!isAllow)
			{
				var customer = Customer.PK.Find(Base, SelectedBAccount);
				throw new PXException(AR.Messages.ProcessingCenterDoesNotAllowNewCards, SelectedProcessingCenter, customer?.AcctCD);
			}
		}
				...
				foreach (string tranId in tranIds)
				{
					...
					if (tranData.TranUID != null)
					{
						...
						else
						{
							...
							if (storedExtTran != null && storedExtTran.NeedSync == false)
							{
								recordTran = true;
							}
						}
		...
		public void ShowAcceptPaymentForm(V2.CCTranType tranType, Guid? noteId)
		{
			TPrimary doc = Base.Caches[typeof(TPrimary)].Current as TPrimary;
			ICCPayment pDoc = GetPaymentDoc(doc);
			GetPaymentProcessing().ShowAcceptPaymentForm(tranType, pDoc, SelectedProcessingCenter, SelectedBAccount, noteId);
		}
		...
		protected virtual void CheckAndRecordTransaction(TPrimary doc, ExternalTransactionDetail storedExtTran, V2.TransactionData tranData)
		{
			...
			RecordTranData(tranType, pDoc, tranData);
		}

		protected virtual void RecordTranData(V2.CCTranType tranType, ICCPayment pDoc, V2.TransactionData tranData, bool isLocalValidation = false)
		{
			switch (tranType)
			{
		...
		protected void UpdateExpirationDate(V2.TransactionData tranData)
		{
			if (tranData.ExpireAfterDays != null)
			{
				...
				storedTran.ExpirationDate = expirationDate;
				PaymentTransactionDetail procTran = GetPaymentTranDetails().First(i => i.TransactionID == storedTran.TransactionID);
				procTran.ExpirationDate = storedTran.ExpirationDate;
