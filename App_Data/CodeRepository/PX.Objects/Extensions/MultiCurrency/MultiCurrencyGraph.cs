...
namespace PX.Objects.Extensions.MultiCurrency
{
			...
			where TPrimary : class, IBqlTable, new()
	{
		...
		{
			if (Documents.Current == null && currencyinfo.Cache.Inserted.Count() == 1)
				return currencyinfo.Cache.Inserted.Cast<CurrencyInfo>().Single();
			else
				return GetCurrencyInfo(Documents.Current?.CuryInfoID);
		}
		#endregion
		...
		protected bool IsModified(PXCache cache, TPrimary item)
			...
			TPrimary oldItem = _oldRow?.Base as TPrimary;
			if (oldItem == null) return true;
			return IsModified(cache, item, oldItem);
			}
		...
		public virtual bool IsModified(PXCache cache, IBqlTable newRow, IBqlTable oldRow)
			{
			foreach (string field in cache.Fields.Where(CurrencyInfoAttribute.IsDifferenceEssential))
							{
				object oldV = cache.GetValue(oldRow, field);
				object newV = cache.GetValue(newRow, field);
		...
		public override void Initialize()
		{
			...
			//prevent values resetting when user press ViewBase/View Cury buttons of the PXCurrencyRate control
		...
		protected virtual void CuryFieldUpdating(PXCache sender, PXFieldUpdatingEventArgs e, CuryField curyField)
		{
			...
			if (value != 0.0m)
			{
				e.NewValue = Math.Round(value, 
					curyField.CustomPrecision ?? 
				   GetCurrencyInfo(sender, e.Row, curyField.CuryInfoIDName)?.CuryPrecision ?? 
				   2,
		...
		protected virtual void BaseFieldUpdating(PXCache sender, PXFieldUpdatingEventArgs e, CuryField curyField)
		{
			...
			if (value != 0.0m)
			{
				e.NewValue = Math.Round((decimal)e.NewValue, 
					curyField.CustomPrecision ?? 
					GetCurrencyInfo(sender, e.Row, curyField.CuryInfoIDName)?.BasePrecision ?? 2, 
					MidpointRounding.AwayFromZero);
			}
		}
		...
		{
			 if(!Base.Caches<TPrimary>().IsDirty) _oldRow = e.Row;
