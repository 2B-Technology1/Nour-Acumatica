...
namespace PX.Objects.Extensions.CustomerCreditHold
{
	...
	{
		public enum EnforceType { None, AdminHold, CreditHold }
		...
		where TStatusField : IBqlField
	{
		...
		protected virtual void PlaceOnHold(PXCache sender, TDoc Row, EnforceType enforceType)
		{
			...
			Customer customer, CustomerClass customerclass)
		{
			...
			if (res.Failed == false && customer.Status == CustomerStatus.CreditHold)
			{
				...
				{
					res.Enforce = EnforceType.AdminHold;
					res.Failed = true;
				}
			}
			...
			Customer customer, CreditVerificationResult res)
		{
			...
			{
				if (e is PXRowUpdatedEventArgs && (res.Enforce != EnforceType.None || GetCreditCheckError(sender, Row) == true))
				{
					ApplyCreditVerificationResult(sender, Row, res, arbalancescache);
						 ...
						 (res.Enforce != EnforceType.None || GetCreditCheckError(sender, Row) == true))
				{
