...
namespace PX.Objects.Extensions.Discount
{
            ...
            where TPrimary : class, IBqlTable, new()
    {
		...
		public virtual DiscountCalculationOptions DefaultCalculationOptions => DiscountEngine.DefaultARDiscountCalculationParameters;
		...
		protected virtual void _(Events.FieldUpdated<Detail, Detail.discountID> e)
        {
            ...
            if (e.ExternalCall && e.Row != null)
            {
				...
				// Acuminator disable once PX1003 NonSpecificPXGraphCreateInstance [Legacy code]
		...
		protected virtual void _(Events.RowUpdated<Detail> e)
		{
			...
			{
				// Acuminator disable once PX1045 PXGraphCreateInstanceInEventHandlers [Legacy code]
				ProcessDiscountsOnDetailRowUpdated(e.Row, e.OldRow, e.Cache);
			}
        }

		protected virtual void ProcessDiscountsOnDetailRowUpdated(Detail row, Detail oldRow, PXCache cache)
		{
			if (row.ManualDisc != true)
			{
				if (oldRow.ManualDisc == true)//Manual Discount Unckecked
				{
					if (row.IsFree == true)
					{
						ResetQtyOnFreeItem(cache.Graph, row);
					}
				}
				if (row.IsFree == true)
				{
					DiscountEngineGraph.ClearDiscount(cache, row);
				}
			}

			RecalculateDiscounts(cache, row, row);
		}
		...
		protected virtual void _(Events.RowDeleted<Detail> e)
        {
            ...
            if (documentCache.Current != null && documentCache.GetStatus(documentCache.Current) != PXEntryStatus.Deleted)
            {
				...
				DiscountEngineGraph.RecalculateGroupAndDocumentDiscounts(Details.Cache,
                    Details, null, Discounts, doc.BranchID, doc.LocationID ?? 0, doc.DocumentDate ?? (new PXGraph()).Accessinfo.BusinessDate, DefaultCalculationOptions);
		...
		protected virtual void _(Events.RowUpdated<Document> e)
        {
            ...
            if (e.ExternalCall && (!Documents.Cache.ObjectsEqual<Document.documentDate>(e.OldRow, e.Row)))
            {
				...
				DiscountEngineGraph.AutoRecalculatePricesAndDiscounts(Details.Cache,
                    Details, null, Discounts, e.Row.LocationID ?? 0, e.Row.DocumentDate ?? (new PXGraph()).Accessinfo.BusinessDate, DefaultCalculationOptions);
            }
            ...
            bool locationChanged = !Documents.Cache.ObjectsEqual<Document.locationID>(e.OldRow, e.Row);
            bool dateChanged = !Documents.Cache.ObjectsEqual<Document.documentDate>(e.OldRow, e.Row);
            bool bAccountChanged = !Documents.Cache.ObjectsEqual<Document.bAccountID>(e.OldRow, e.Row);
			if (locationChanged || dateChanged || bAccountChanged)
            {
                RecalcDiscountsParamFilter recalcFilter =
                    new RecalcDiscountsParamFilter
                    {
						...
						RecalcUnitPrices = bAccountChanged,
                        RecalcTarget = RecalcDiscountsParamFilter.AllLines
                    };
				// Acuminator disable once PX1045 PXGraphCreateInstanceInEventHandlers [Legacy code]
				// Acuminator disable once PX1003 NonSpecificPXGraphCreateInstance [Legacy code]
				DiscountEngineGraph.RecalculatePricesAndDiscounts(Details.Cache,
                    Details, null, Discounts, e.Row.LocationID ?? 0, e.Row.DocumentDate ?? (new PXGraph()).Accessinfo.BusinessDate, recalcFilter, DefaultCalculationOptions);
                RecalculateTotalDiscount();
            ...
            if (!DiscountEngineGraph.IsInternalDiscountEngineCall && e.Row != null)
            {
				if (e.Row.DiscountID != null)
				{
					...
					DiscountEngineGraph.InsertManualDocGroupDiscount(Details.Cache, Details, Discounts, e.Row, e.Row.DiscountID, null,
						Documents.Current.BranchID, Documents.Current.LocationID ?? 0, Documents.Current.DocumentDate, DefaultCalculationOptions);
					RefreshTotalsAndFreeItems(Discounts.Cache);
					(Discounts.Cache as PXModelExtension<Discount>)?.UpdateExtensionMapping(e.Row);
				}

				// Acuminator disable once PX1045 PXGraphCreateInstanceInEventHandlers [Legacy code]
				if (DiscountEngineGraph.SetExternalManualDocDiscount(Details.Cache, Details, Discounts, e.Row, null, DefaultCalculationOptions))
				{
					RefreshTotalsAndFreeItems(Discounts.Cache);
					(Discounts.Cache as PXModelExtension<Discount>)?.UpdateExtensionMapping(e.Row);
				}
        ...
        protected virtual void _(Events.RowUpdated<Discount> e)
        {
            if (!DiscountEngineGraph.IsInternalDiscountEngineCall && e.Row != null)
            {
                ...
                {
					// Acuminator disable once PX1045 PXGraphCreateInstanceInEventHandlers [Legacy code]
					DiscountEngineGraph.UpdateDocumentDiscount(Details.Cache, Details, Discounts, Documents.Current.BranchID, Documents.Current.LocationID ?? 0, Documents.Current.DocumentDate, e.Row.Type != DiscountType.Document, DefaultCalculationOptions);
                    RefreshTotalsAndFreeItems(Discounts.Cache);
                    (Discounts.Cache as PXModelExtension<Discount>)?.UpdateExtensionMapping(e.Row);
                }
				...
				// Acuminator disable once PX1045 PXGraphCreateInstanceInEventHandlers [Legacy code]
				if (DiscountEngineGraph.SetExternalManualDocDiscount(Details.Cache, Details, Discounts, e.Row, e.OldRow, DefaultCalculationOptions))
				{
					RefreshTotalsAndFreeItems(Discounts.Cache);
					(Discounts.Cache as PXModelExtension<Discount>)?.UpdateExtensionMapping(e.Row);
				}
        ...
        protected virtual void _(Events.RowDeleted<Discount> e)
        {
            ...
            {
				// Acuminator disable once PX1045 PXGraphCreateInstanceInEventHandlers [Legacy code]
				DiscountEngineGraph.UpdateDocumentDiscount(Details.Cache, Details, Discounts, Documents.Current.BranchID, Documents.Current.LocationID ?? 0, Documents.Current.DocumentDate, (e.Row.Type != null && e.Row.Type != DiscountType.Document && e.Row.Type != DiscountType.ExternalDocument), DefaultCalculationOptions);
                (Discounts.Cache as PXModelExtension<Discount>)?.UpdateExtensionMapping(e.Row);
            }
        ...
        protected virtual void RecalculateDiscounts(PXCache sender, Detail line, object source)
        {
            if (line.Quantity != null && line.CuryLineAmount != null && line.IsFree != true)
            {
                ...
                DiscountEngineGraph.SetDiscounts(Details.Cache,
                    Details, line, Discounts, doc.BranchID ?? 0, doc.LocationID ?? 0, doc.CuryID, doc.DocumentDate ?? Base.Accessinfo.BusinessDate, null, DefaultCalculationOptions);
