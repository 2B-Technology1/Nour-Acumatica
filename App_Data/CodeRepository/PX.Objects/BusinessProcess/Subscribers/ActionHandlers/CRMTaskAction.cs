...
namespace PX.Objects.BusinessProcess.Subscribers.ActionHandlers
{
    internal class CRMTaskAction : TemplateMessageAction
    {
        ...
        public CRMTaskAction(Guid id, IPushNotificationDefinitionProvider graphProvider, IEventDefinitionsProvider eventDefinitionProvider, IPXPageIndexingService pageIndexingService)
            : base(id, graphProvider, eventDefinitionProvider, pageIndexingService)
        {
            Id = id;
        }
            ...
            using (new PXLocaleScope(taskTemplate.LocaleName ?? PXLocalesProvider.GetCurrentLocale()))
            {
                ...
                if (entityGraph is PXGenericInqGrph)
                {
                    ...
                    fieldValueGetter = (names) => 
                    {
                        if (names.Length > 0)
                        {
                            return eventRows[0].NewRow.TryGetValue(new PX.PushNotifications.KeyWithAlias(names.Last()), out object value)
                                ? PX.PushNotifications.ValueWithInternal.UnwrapInternalValue(value)
                                : null;
                        }
