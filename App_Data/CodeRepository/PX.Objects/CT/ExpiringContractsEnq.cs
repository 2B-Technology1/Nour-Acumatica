	...
	public class ExpiringContractsEng : PXGraph<ExpiringContractsEng>
	{
		...
		public virtual IEnumerable contracts()
		{
			...
			if (filter != null)
			{
				...
				{
					ContractsView.WhereAnd<Where<Customer.customerClassID, Equal<Current<ExpiringContractFilter.customerClassID>>>>();
				}

				if (filter.TemplateID != null)
				{
					ContractsView.WhereAnd<Where<Contract.templateID, Equal<Current<ExpiringContractFilter.templateID>>>>();
				}
				...
				*/
				foreach ( PXResult<Contract> result in ContractsView.Select())
				{
					bool skipItem = false;
