...
namespace PX.Objects.CT
{
	public class CTBillEngine : PXGraph<CTBillEngine>, IContractInformation
	{
		...
		private string GetDescriptionMessageForDetail(InventoryAction action, ContractDetail detail)
		{
			return PXDBLocalizableStringAttribute.GetTranslation<ContractDetail.description>(Caches[typeof(ContractDetail)], detail);
		}
		/// <summary>
		...
		public override string PrimaryView => nameof(Contracts);
		...
		public virtual void Setup(int? contractID, DateTime? date)
		{
			...
			ValidateCustomerInfo(contract, customer, location);
		...
		private static DateTime? ShiftDateIfPeriodClosed(IFinPeriodRepository periodRepository, DateTime date, int? organizationID)
		{
			FinPeriod originalDatePeriod = periodRepository.GetFinPeriodByDate(date, organizationID);
			if (originalDatePeriod.ARClosed == true)
				return periodRepository.FindFirstOpenFinPeriod(originalDatePeriod.FinPeriodID, organizationID, typeof(OrganizationFinPeriod.aRClosed)).StartDate;
			else
				return date;
		}
		...
		private void CreateInvoice(Contract contract, Contract template, List<InvoiceData> invoices, Customer customer, Location location, string action, Dictionary<int, List<TranNotePair>> sourceTran = null, List<UsageData> tranData = null)
		{
			...
			foreach (InvoiceData invData in invoices)
			{
				...
				copy.BranchID = location.CBranchID ?? Accessinfo.BranchID;
				...
				copy.DocDate = ShiftDateIfPeriodClosed(invoiceEntry.FinPeriodRepository, invData.InvoiceDate, PXAccess.GetParentOrganizationID(copy.BranchID));
				...
				if (invoices.Count <= 0 || (
					    action != ContractAction.Bill &&
					    contract.EffectiveFrom != null &&
					    ((contract.EffectiveFrom == schedule.NextDate || contract.EffectiveFrom == schedule.LastDate) &&
					     invoice.CuryOrigDocAmt == 0))) continue;
		...
		private void UndoInvoices(Contract contract)
		{
			...
			foreach (ARInvoice d in GetDocuments(invoiceEntry, contract))
			{
				if (d.Released != true)
				{
					toRemove.Add(d);
			...
			Remove(invoiceEntry, toRemove);
			Unlink(toUnlink);
		}

		private IEnumerable<ARInvoice> GetDocuments(ARInvoiceEntry invoicegraph, Contract contract)
		{
			return PXSelectJoin<
				ARInvoice, 
				...
				.Select(invoicegraph, contract.ContractID)
				.RowCast<ARInvoice>()
				.ToArray();
		}

		private static void Remove(ARInvoiceEntry invoiceEntry, IEnumerable<ARInvoice> documents)
		{
		...
		public virtual void Bill(int? contractID, DateTime? date = null)
		{
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				if (data.Count > 0 && data.Any(IsBillable))
				{
					InvoiceData invoiceData = new InvoiceData(billingDate.Value);
					invoiceData.UsageData.AddRange(data);
					CreateInvoice(contract, template, new List<InvoiceData> { invoiceData }, customer, location, ContractAction.Bill, sourceTran, tranData);
		...
		protected virtual List<UsageData> GetSetupFee(Contract contract)
		{
				...
				.Select(this, contract.ContractID))
			{
				...
				data.Description = GetDescriptionMessageForDetail(InventoryAction.Setup, det);
		...
		protected virtual void ValidateCustomerInfo(Contract contract, Customer customer, Location location)
		{
		}
		...
		protected virtual bool ProcessDollarUsage(Contract contract, TranNotePair tran, string UOM, decimal? BillableQty, int? EmployeeID, PXResult<ContractDetail, ContractItem, InventoryItem> res, List<UsageData> addedData)
		{
			...
			{
				billing = (ContractDetail)ContractDetails.Cache.Locate(new ContractDetail { ContractID = contract.ContractID, ContractItemID = billing.ContractItemID, RevID = billing.RevID });
				billing = (ContractDetail)ContractDetails.Cache.CreateCopy(billing);
				...
				if (availableDeposit.TryGetValue(item.DepositItemID, out available))
				{
					...
					if (deposit != null)
					{
						deposit = (ContractDetail)ContractDetails.Cache.CreateCopy(deposit);
		...
		public virtual decimal? CalculatePrice(Contract contract, UsageData tranData)
		{
			...
			{
				CuryRateTypeID = PXAccess.FeatureInstalled<FeaturesSet.multicurrency>() ? customer.CuryRateTypeID ?? cmsetup.Current.ARRateTypeDflt : cmsetup.Current.ARRateTypeDflt,
