...
namespace PX.Objects.CT
{
	public class ContractItemMaint : PXGraph<ContractItemMaint, ContractItem>
	{
		...
		{
			if (item != null && item.BaseItemID != null && (item.BasePriceVal ?? 0m) == 0m)
			{
		...
		{
			if (item != null && item.RecurringItemID != null && ((item.FixedRecurringPriceVal ?? 0m) == 0m || (item.UsagePriceOption != PriceOption.Manually && (item.UsagePriceVal ?? 0m) == 0m)))
			{
		...
		{
			if (item != null && item.RenewalItemID != null && (item.RenewalPriceVal ?? 0m) == 0m)
			{
