...
{
	/// <summary>The base class for the <see cref="PMProforma">pro forma invoice</see> line.
	/// The class provides the fields that are common to the <see cref="PMProformaProgressLine" /> 
	/// and <see cref="PMProformaTransactLine" /> types.</summary>
	...
	public class PMProformaProgressLine : PMProformaLine
	{
		...
		[PXDefault(typeof(Search<PMTask.taskID, Where<PMTask.projectID, Equal<Current<projectID>>, And<PMTask.isDefault, Equal<True>>>>), PersistingCheck = PXPersistingCheck.Nothing)]
		[ActiveProjectTask(typeof(PMProformaLine.projectID), BatchModule.AR, DisplayName = "Project Task", AllowCompleted = true, Enabled = false)]
		[PXRestrictor(typeof(Where<PMTask.type, NotEqual<ProjectTaskType.cost>>), CN.Subcontracts.PM.Descriptor.Messages.PmTask.TaskTypeIsNotAvailable, typeof(PMTask.taskCD))]
		...
		/// <inheritdoc/>
		[PXDefault(typeof(Search<Account.accountGroupID, Where<Account.accountID, Equal<Current<accountID>>>>))]
		[AccountGroup]
		[PXForeignReference(typeof(Field<accountGroupID>.IsRelatedTo<PMAccountGroup.groupID>))]
		...
		/// <inheritdoc/>
		[Account(typeof(PMProformaProgressLine.branchID), typeof(Search<Account.accountID, Where<Account.accountGroupID, IsNotNull>>),
