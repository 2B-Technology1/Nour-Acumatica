...
namespace PX.Objects.PM
{
	...
	public class PMProjectRevenueTotal : PX.Data.IBqlTable
	{
		...
		{
			[PXDependsOnFields(typeof(curyAmount), typeof(curyInvoicedAmount), typeof(PMBudget.curyActualAmount), typeof(PMBudget.curyInclTaxAmount))]
			get
			{
				if (CuryAmount.HasValue && CuryAmount != 0)
					return (CuryInvoicedAmount.GetValueOrDefault() + CuryActualAmount.GetValueOrDefault() + CuryInclTaxAmount.GetValueOrDefault()) / CuryAmount * 100m;
				else
					return 0m;
			}
		}
		...
		{
			[PXDependsOnFields(typeof(curyRevisedAmount), typeof(curyInvoicedAmount), typeof(PMBudget.curyActualAmount), typeof(PMBudget.curyInclTaxAmount))]
			get
			{
				if (CuryRevisedAmount.HasValue && CuryRevisedAmount != 0)
					return (CuryInvoicedAmount.GetValueOrDefault() + CuryActualAmount.GetValueOrDefault() + CuryInclTaxAmount.GetValueOrDefault()) / CuryRevisedAmount * 100m;
				else
					return 0m;
			}
		}
	...
	/// <summary>
	/// Aggregate Total Sum of a Project for Cost Forecast.
	/// </summary>
	[PXCacheName(Messages.ContractTotal)]
	[PXProjection(typeof(Select4<PMBudget,
				...
				Sum<PMBudget.curyCostProjectionCostAtCompletion,
				Sum<PMBudget.curyChangeOrderAmount,
				Sum<PMBudget.curyAmount,
				Sum<PMBudget.curyCommittedOrigAmount>>>>>>>>>>>))]
	public class PMProjectCostForecastTotal : PX.Data.IBqlTable
	{
		#region ProjectID
		/// <exclude/>
		public abstract class projectID : PX.Data.BQL.BqlInt.Field<projectID> { }
		...
		/// <summary>
		/// Total Revised Commited Amount for Project
		/// </summary>
		...
		/// <summary>
		/// Total Committed Invoiced Amount for Project
		/// </summary>
		...
		[PXFormula(typeof(Add<curyActualAmount, Sub<curyCommittedAmount, curyCommittedInvoicedAmount>>))]
		[PXBaseCury()]
		[PXDefault(TypeCode.Decimal, "0.0", PersistingCheck = PXPersistingCheck.Nothing)]
		[PXUIField(DisplayName = "Actual + Committed Costs", Enabled = false)]
		public virtual Decimal? CompletedAmount
		{
			//[PXDependsOnFields(typeof(curyActualAmount), typeof(curyCommittedAmount), typeof(curyCommittedInvoicedAmount))]
			get; //{ return CuryActualAmount.GetValueOrDefault() + CuryCommittedAmount.GetValueOrDefault() - CuryCommittedInvoicedAmount.GetValueOrDefault(); }
			set;
		}
		#endregion
		#region CuryChangeOrderAmount
		...
		/// <inheritdoc cref="PMBudget.CuryCommittedOrigAmount"/>
