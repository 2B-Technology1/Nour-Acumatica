...
namespace PX.Objects.PM
{
	...
	[PXBreakInheritance]
	public partial class PMQuote : IBqlTable, IAssign, IPXSelectable, INotable
	{
		#region Selected
		...
		[PXSelector(
			typeof(SelectFrom<PMQuote>
				.LeftJoin<BAccount>.On<BAccount.bAccountID.IsEqual<bAccountID>>
				.LeftJoin<Contact>.On<Contact.contactID.IsEqual<contactID>>
				.LeftJoin<PMProject>.On<PMProject.contractID.IsEqual<quoteProjectID>>
				.Where<quoteType.IsEqual<CRQuoteTypeAttribute.project>
					.And<quoteProjectID.IsNull.Or<MatchUserFor<PMProject>>>
					.And<BAccount.bAccountID.IsNull.Or<MatchUserFor<BAccount>>>>
				.SearchFor<quoteNbr>),
			new[]
			{
				typeof(quoteNbr),
		...
		/// </summary>
		[LocationActive(typeof(Where<CR.Location.bAccountID, Equal<Current<PMQuote.bAccountID>>, Or<Current<PMQuote.bAccountID>, IsNull>>),
		...
		#region IsFirstQuote
		public abstract class isFirstQuote : PX.Data.BQL.BqlBool.Field<isFirstQuote> { }
		...
		{
			[PXDependsOnFields(typeof(amount), typeof(taxTotal), typeof(taxInclTotal))]
			get
			{
				return Amount + TaxTotal - TaxInclTotal.GetValueOrDefault();
			}
		...
		{
			[PXDependsOnFields(typeof(curyAmount), typeof(curyTaxTotal), typeof(curyTaxInclTotal))]
			get
			{
				return CuryAmount + CuryTaxTotal - CuryTaxInclTotal.GetValueOrDefault();
			}
		...
		/// <summary>
		/// The total inclusive tax amount in base currency.
		/// </summary>
		...
		[PXDefault(TaxCalculationMode.TaxSetting, typeof(Search<Location.cTaxCalcMode, Where<Location.bAccountID, Equal<Current<PMQuote.bAccountID>>,
																						 And<Location.locationID, Equal<Current<PMQuote.locationID>>>>>))]
