...
namespace PX.Objects.PM
{
	...
	public class RateMaint : PXGraph<RateMaint, PMRateSequence>
	{
		...
		protected virtual IEnumerable Cancel(PXAdapter adapter)
		{
			...
			if (hasResult == false && RateSequence.Current != null)
			{
				int? rateTableIDIndex = null;
				string rateTableID = null;
				bool rateTableIDChange = false;
				int? rateTypeIDIndex = null;
				string rateTypeID = null;
				bool rateTypeIDChange = false;
				int? rateSequenceIndex = null;
				int? rateSequence = null;
				bool rateSequenceChange = false;
				...
				for (int i = 0; i <= adapter.SortColumns.Length - 1; i++)
				{
					...
					{
						rateTableIDIndex = i;
						rateTableID = adapter.Searches[i] as string;
						rateTableIDChange = PXLocalesProvider.CollationComparer.Compare(rateTableID, RateSequence.Current.RateTableID) != 0;
					}
					else if (adapter.SortColumns[i].ToLower() == typeof(PMRateSequence.rateTypeID).Name.ToLower())
					{
						rateTypeIDIndex = i;
						rateTypeID = adapter.Searches[i] as string;
						rateTypeIDChange = PXLocalesProvider.CollationComparer.Compare(rateTypeID, RateSequence.Current.RateTypeID) != 0;
					}
					else if (adapter.SortColumns[i].ToLower() == typeof(PMRateSequence.sequence).Name.ToLower())
					{
						rateSequenceIndex = i;
						rateSequence = adapter.Searches[i] as int?;
						rateSequenceChange = rateSequence != RateSequence.Current.Sequence;
					}
				...
				if (rateTableID != null && rateTypeID != null && rateSequence != null && rateSequenceChange)
				{
					rs = PXSelect<PMRateSequence,
							Where<PMRateSequence.rateTableID, Equal<Required<PMRateSequence.rateTableID>>,
								And<PMRateSequence.rateTypeID, Equal<Required<PMRateSequence.rateTypeID>>,
								And<PMRateSequence.sequence, Equal<Required<PMRateSequence.sequence>>>>>,
							OrderBy<Asc<PMRateSequence.sequence, Asc<PMRateSequence.rateCodeID>>>>
						.SelectWindowed(this, 0, 1, rateTableID, rateTypeID, rateSequence).RowCast<PMRateSequence>().FirstOrDefault();
					if (rs == null && this.Caches<PMRateSequence>().GetStatus(RateSequence.Current) == PXEntryStatus.Notchanged)
					{
						PMRateSequence newRate = new PMRateSequence();
						newRate.RateTableID = rateTableID;
						newRate.RateTypeID = rateTypeID;
						newRate.Sequence = rateSequence; 
						this.Caches<PMRateSequence>().Insert(newRate);
						this.Caches<PMRateSequence>().IsDirty = false;
						return new object[] { newRate };
					}
				}

				if (rs == null && rateTableID != null && rateTypeID != null && (rateTableIDChange || rateTypeIDChange))
				{
					rs = PXSelect<PMRateSequence,
							Where<PMRateSequence.rateTableID, Equal<Required<PMRateSequence.rateTableID>>,
								And<PMRateSequence.rateTypeID, Equal<Required<PMRateSequence.rateTypeID>>>>,
							OrderBy<Asc<PMRateSequence.sequence, Asc<PMRateSequence.rateCodeID>>>>
						.SelectWindowed(this, 0, 1, rateTableID, rateTypeID).RowCast<PMRateSequence>().FirstOrDefault();
					if (rs == null && this.Caches<PMRateSequence>().GetStatus(RateSequence.Current) == PXEntryStatus.Notchanged)
					{
						PMRateSequence newRate = new PMRateSequence();
						newRate.RateTableID = rateTableID;
				...
				if (rs != null)
				{
					...
					if (rateSequenceIndex != null)
					{
						adapter.Searches[rateSequenceIndex.Value] = rs.Sequence;
					}
