...
namespace PX.Objects.PM
{
	...
	public class TransactionInquiry : PXGraph<TransactionInquiry>
	{
		...
		protected virtual void _(Events.CacheAttached<PMRegister.origDocNbr> _)
		{
		}
		...
		[PXMergeAttributes(Method = MergeMethod.Merge)]
		[PXUIField(DisplayName = Messages.EmployeeName, Visible = false)]
		...
		[PXCopyPasteHiddenView]
		[PXHidden]
		public PXSelect<BAccount> dummyBAccount;
		...
		public virtual IEnumerable transactions()
		{
			...
			if (IsNotEmpty(filter))
			{
				...
				{
					select.WhereAnd<Where<PMTran.refNbr, Equal<Current<TranFilter.refNbr>>>>();
				}
				...
				{
					select.WhereAnd<Where<PMTran.accountGroupID, Equal<Current<TranFilter.accountGroupID>>, Or<PMTran.offsetAccountGroupID, Equal<Current<TranFilter.accountGroupID>>>>>();
				}
		...
		private static bool IsNotEmpty(TranFilter filter)
			=> filter != null && (
				...
				filter.TranID != null ||
				!string.IsNullOrWhiteSpace(filter.RefNbr));
		...
		{
			PXUIFieldAttribute.SetVisible<PMTran.earningType>(e.Cache, null, false);
			PXUIFieldAttribute.SetVisible<PMTran.overtimeMultiplier>(e.Cache, null, false);
		}
		...
		public class TranFilter : IBqlTable
		{
			...
			/// <summary>
			/// The reference number of the PM Document.
			/// </summary>
			[PXDBString(PMRegister.refNbr.Length, IsUnicode = true, InputMask = ">CCCCCCCCCCCCCCC")]
