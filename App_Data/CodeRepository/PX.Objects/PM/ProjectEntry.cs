...
namespace PX.Objects.PM
{
	...
	public class ProjectEntry : PXGraph<ProjectEntry, PMProject>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
			...
			where TGraph: PO.POOrderEntry, new()
		{
			...
			{
				if (RequireSingleProjectPerDocument() == true)
				{
		...
		public ProjectEntry()
		{
			...
			createChangeOrder.SetVisible(ChangeOrderFeatureEnabled());
		...
		protected virtual void _(Events.FieldUpdated<PMRevenueBudget, PMRevenueBudget.curyAmountToInvoice> e)
		{
			if (!_IsRecalculatingRevenueBudgetScope && e.Row?.ProgressBillingBase == ProgressBillingBase.Amount) //The value is manually edited by the user on the screen
			{
				...
				if (budgetedAmount != 0)
				{
					decimal invoicedOrPendingPrepayment = e.Row.CuryAmountToInvoice.GetValueOrDefault() + GetCuryActualAmountWithTaxes(e.Row) + e.Row.CuryInvoicedAmount.GetValueOrDefault() + (e.Row.CuryPrepaymentAmount.GetValueOrDefault() - e.Row.CuryPrepaymentInvoiced.GetValueOrDefault());
		...
		{
			return APSetup.Current?.RequireSingleProjectPerDocument == true;
		}
		...
		{
			return ChangeOrderFeatureEnabled()
				&& !IsUserNumberingOn(Setup.Current?.ChangeOrderNumbering);
		}
		...
		{
			return CostCommitmentTrackingEnabled() && ChangeOrderEnabled();
		}
		...
		protected virtual bool CheckTaskBeforeImport(IDictionary keys, IDictionary values, string key)
		{
			...
			if (keys.Contains(key))
			{
				...
				{
					var task = Tasks.Select().RowCast<PMTask>()
						.FirstOrDefault(x => string.Equals(x.TaskCD, taskCd, StringComparison.OrdinalIgnoreCase));
