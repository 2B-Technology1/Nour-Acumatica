...
namespace PX.Objects.PM
{
	public class PMBillEngine : PXGraph<PMBillEngine>, IRateTable
	{
		...
		public virtual BillingResult Bill(int? projectID, DateTime? invoiceDate, string finPeriod)
		{
			...
			var currencyInfo = GetCurrencyInfo(project, billingDate);
			...
			BillingResult result = new BillingResult(billingDate);
		...
		public virtual void InsertTransactionsInInvoice(PMProject project, List<BillingData> unbilled, int mult)
		{
			...
			foreach (BillingData data in unbilled)
			{
				ARTran tran = new ARTran
				{
					...
					RetainagePct = (project.RetainageMode == RetainageModes.Contract || project.SteppedRetainage == true) ? project.RetainagePct : data.Tran.RetainagePct,
			...
			DateTime billingDate, string docDesc, int? locationID)
		{
			...
			if (invoice.RetainageApply == false)
			{
				InvoiceEntry.Document.Cache.SetValue<ARInvoice.retainageSubID>(invoice, null);
				InvoiceEntry.Document.Cache.SetValue<ARInvoice.retainageAcctID>(invoice, null);
			}
			...
			if (contactPM != null && contactPM.IsDefaultContact != true)
			{
				...
				if (contactAR == null)
				{
					ARContact billContact = (ARContact)InvoiceEntry.Billing_Contact.Cache.CreateInstance();
					ContactAttribute.CopyContact(billContact, contactPM);

					billContact.BAccountContactID = contactPM.BAccountContactID;
					billContact.IsDefaultContact = true;

					InvoiceEntry.Billing_Contact.Current = billContact = InvoiceEntry.Billing_Contact.Insert(billContact);
					InvoiceEntry.Document.Current.BillContactID= billContact.ContactID;
				}
				else
				{
		...
		public virtual List<BillingData> BillFixPriceTask(PMProject project, Customer customer, PMTask task, PMBillingRule rule, DateTime billingDate)
		{
			...
			var budgetLines = select.Select(task.TaskID).AsEnumerable()
		...
		public virtual int? CalculateTargetSalesAccountID(PMBillingRule rule, PMProject project, PMTask task, PMTran tran, PMProformaLine line, Customer customer)
		{
			...
			{
				Account account = GetLineAccount(tran.AccountID);
				if (account != null)
				{
				...
				{
					throw new PXException(Messages.SourceTransactionAccountIsNotConfiguredForBilling, rule.StepID, rule.BillingID, tran.TranType, tran.RefNbr);
				}
			}
		...
		public virtual string CalculateTargetSalesSubaccountCD(PMBillingRule rule, PMProject project, PMTask task, int? subIDTran, int? employeeID, int? branchID, int? inventoryID, Customer customer)
		{
			...
			if (rule.SubMaskBudget.Contains(AcctSubDefault.Branch))
			{
		...
		public virtual void Transform(PMProject project, List<PMTran> billingBase, PMBillingRule rule, PMTask task)
		{
			foreach (PMTran tran in billingBase)
			{
				...
				Transactions.SetValueExt<PMTran.projectCuryInvoicedAmount>(tran, amt);
		...
		public virtual IList<PMTran> ReverseTranNonGL(PMTran tran)
		{
			...
			debit.SubID = null;
			debit.OffsetAccountID = null;
			debit.OffsetSubID = null;
			...
			if (tran.OffsetAccountGroupID != null)
			{
				...
				credit.SubID = null;
				credit.OffsetAccountID = null;
				credit.OffsetSubID = null;
		...
		public class BillingResult
		{
			...
			/// </summary>
			public List<ARRegister> Invoices { get; } = new List<ARRegister>();

			/// <summary>
			/// Nothing created
			/// </summary>
			public bool IsEmpty => Proformas.Count + Invoices.Count == 0;

			/// <summary>
			/// Only one documant created
			/// </summary>
			public bool IsSingle => Proformas.Count + Invoices.Count == 1;

			/// <summary>
			/// Ctor
			/// </summary>
			/// <param name="billingDate">Date of billing</param>
			public BillingResult(DateTime billingDate)
			{
