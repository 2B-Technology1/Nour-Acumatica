...
namespace PX.Objects.PM
{
	...
	{
		public SelectFrom<PMForecast>
			.LeftJoin<PMProject>
				.On<PMProject.contractID.IsEqual<PMForecast.projectID>>
		...
		public virtual IEnumerable AddPeriods(PXAdapter adapter)
		{
			...
			{
				//Get key from filter.
				var budgetKey = new BudgetKeyTuple(Revisions.Current.ProjectID.GetValueOrDefault(),
				  Filter.Current.ProjectTaskID.GetValueOrDefault(),
				  Filter.Current.AccountGroupID.GetValueOrDefault(),
				  Filter.Current.InventoryID.GetValueOrDefault(),
				  Filter.Current.CostCodeID.GetValueOrDefault());

				PMBudgetInfo budget = GetBudgetInfoFromKey(budgetKey);
				TryGeneratePeriodsForBudgetFromBudgetInfo(budget);
			}
			else if (Items.Current != null)
			{
		...
		public virtual IEnumerable SettleBalances(PXAdapter adapter)
		{
			...
			{			
				...
				budget = GetBudgetInfoFromKey(budgetKey);
			}
			else if (Items.Current != null)
			{			
				budget = GetBudgetInfoFromKey(BudgetKeyTuple.Create(Items.Current));
			}
		...
		protected virtual string GetMinStartPeriodID(PMBudgetInfo row)
		{
			...
			{
				DateTime? firstCR = GetFirstChangeRequestDate(row.ProjectID, row.ProjectTaskID);
				if (firstCR != null)
				{
					if (minDate == null)
					{
			...
			var selectFirstTran = new PXSelect<PMTran, Where<PMTran.projectID, Equal<Required<PMTran.projectID>>, And<PMTran.taskID, Equal<Required<PMTran.taskID>>>>, OrderBy<Asc<PMTran.date>>>(this);

			PMTran first = selectFirstTran.SelectWindowed(0, 1, row.ProjectID, row.ProjectTaskID);
			if (first != null)
			{
				if (minDate == null)
				{
			...
			{
				PMTask task = PMTask.PK.FindDirty(this, row.ProjectID, row.ProjectTaskID);
		...
		protected virtual DateTime? GetFirstChangeRequestDate(int? projectID, int? projectTaskID)
		{
			var selectFirstChangeRequest = new PXSelectJoin<PMChangeRequest,
				...
				Where<PMChangeRequestLine.projectID, Equal<Required<PMChangeRequestLine.projectID>>,
					And<Where<PMChangeRequestLine.costTaskID, Equal<Required<PMChangeRequestLine.costTaskID>>,
			...
			PMChangeRequest doc = selectFirstChangeRequest.SelectWindowed(0, 1, projectID, projectTaskID, projectTaskID);
			if (doc != null)
			{
				return doc.Date;
			}
		...
		protected virtual DateTime? GetLastChangeRequestDate(int? projectID, int? projectTaskID)
		{
			...
			PMChangeRequest doc = selectLastChangeRequest.SelectWindowed(0, 1, projectID, projectTaskID, projectTaskID);
			if (doc != null)
			{
				return doc.Date;
			}
		...
		protected virtual string GetMaxEndPeriodID(PMBudgetInfo row)
		{
			...
			var selectLastTran = new PXSelect<PMTran, Where<PMTran.projectID, Equal<Required<PMTran.projectID>>, And<PMTran.taskID, Equal<Required<PMTran.taskID>>>>, OrderBy<Desc<PMTran.date>>>(this);

			PMTran last = selectLastTran.SelectWindowed(0, 1, row.ProjectID, row.ProjectTaskID);
			if (last != null)
			{
				if (maxDate == null)
				{
			...
			if (PXAccess.FeatureInstalled<FeaturesSet.changeRequest>())
			{
				DateTime? lastCR = GetLastChangeRequestDate(row.ProjectID, row.ProjectTaskID);
				if (lastCR != null)
				{
					if (maxDate == null)
					{
		...
		public class PMForecastFilter : IBqlTable
		{
			...
			[AccountGroup(typeof(Where<PMAccountGroup.groupID.IsNotNull
				.And<Brackets<PMAccountGroup.type.IsEqual<GL.AccountType.expense>
					.Or<Brackets<PMAccountGroup.type.IsEqual<PMAccountType.offBalance>
