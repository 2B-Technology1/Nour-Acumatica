...
namespace PX.Objects.PM
{
	...
	public class ProjectMultiCurrency : IProjectMultiCurrency
	{
		...
		public virtual void CalculateCurrencyValues(PXGraph graph, GLTran tran, PMTran pmt, Batch batch, PMProject project, Ledger ledger)
		{
			...
			PMTran origProjectTran = null;

			if (batch.AutoReverseCopy == true)
			{
				GLTran original = GLTran.PK.Find(graph, tran.OrigModule, tran.OrigBatchNbr, tran.OrigLineNbr);
				if (original != null)
				{
					origProjectTran = PMTran.PK.Find(graph, original.PMTranID);
				}
			}
			...
			if (PXAccess.FeatureInstalled<FeaturesSet.projectMultiCurrency>())
			{
				...
				{
					if (origProjectTran != null)
					{
						pmt.BaseCuryInfoID = origProjectTran.BaseCuryInfoID;
					}
					else
					{
				...
				else
				{
					...
					{
						pmt.BaseCuryInfoID = origProjectTran.BaseCuryInfoID;
						pmt.Amount = CM.TemporaryHelpers.MultiCurrencyCalculator.GetCurrencyInfo<PMTran.baseCuryInfoID>(graph, pmt).CuryConvBase(tranCuryAmount.GetValueOrDefault());
					}
					else
					{
				...
				if (project.CuryID == batch.CuryID)
				{
					if (origProjectTran != null)
					{
						pmt.ProjectCuryInfoID = origProjectTran.ProjectCuryInfoID;
					}
					else
					{
				...
				else
				{
					...
					{
						pmt.ProjectCuryInfoID = origProjectTran.ProjectCuryInfoID;
						pmt.ProjectCuryAmount = CM.TemporaryHelpers.MultiCurrencyCalculator.GetCurrencyInfo<PMTran.projectCuryInfoID>(graph, pmt).CuryConvBase(pmt.TranCuryAmount.GetValueOrDefault());
					}
					else
					{
