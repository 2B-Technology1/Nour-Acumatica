...
namespace PX.Objects.IN
{
	public class INTransferEntry : INRegisterEntryBase
	{
		...
		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXDBScalar(typeof(Search5<INLocationStatusInTransit.qtyOnHand,
			...
			Where<INLocationStatusInTransit.siteID, Equal<SiteAttribute.transitSiteID>,
				...
				And<INTransitLine.transferNbr, Equal<INTran.refNbr>>>>,
			Aggregate<
		...
		public INTransferEntry()
		{
			...
			OpenPeriodAttribute.SetValidatePeriod<INRegister.finPeriodID>(this.transfer.Cache, null, PeriodValidation.DefaultSelectUpdate);
			...
			FieldDefaulting.AddHandler<SiteStatusByCostCenter.negAvailQty>(cancelHandler);
		}
		#endregion
		...
		protected virtual void _(Events.RowUpdated<INRegister> e)
		{
			...
			ValidateTransferTrans(e.Row, e.OldRow);
		}

		protected virtual void ValidateTransferTrans(INRegister row, INRegister oldRow)
		{
			if (!transfer.Cache.ObjectsEqual<INRegister.transferType>(row, oldRow))
			{
				foreach (INTran tran in transactions.Select())
				{
					if (!IsTransferTranValid(tran))
					{
		...
		{
			if (transfer.Current.TransferType == INTransferType.OneStep && tran.ToLocationID == null)
			{
				return false;
			}
		...
		protected virtual void _(Events.RowSelected<INRegister> e)
		{
			...
			LineSplittingExt.lsselect.AllowInsert = isEditableDocument && e.Row.SiteID != null;
			LineSplittingExt.lsselect.AllowUpdate = e.Row.Released == false;
			LineSplittingExt.lsselect.AllowDelete = isEditableDocument;
			
			PXUIFieldAttribute.SetEnabled<INTran.toLocationID>(transactions.Cache, null, isEditableDocument);
			...
			PXUIFieldAttribute.SetEnabled<INRegister.transferType>(e.Cache, e.Row, isEditableDocument && (e.Row.SiteID == null || e.Row.SiteID != e.Row.ToSiteID));
			...
			PXUIFieldAttribute.SetVisible<INTran.toCostLayerType>(transactions.Cache, null, e.Row.TransferType == INTransferType.OneStep);
		}
		#endregion
		...
		{
			if (e.Row == null || e.Cancel == true) return;
		...
		protected override void _(Events.FieldUpdated<INTran, INTran.inventoryID> e)
		{
			...
			e.Cache.SetDefaultExt<INTran.locationID>(e.Row);
		}
		...
		public class INOpenPeriodTransferAttribute : INOpenPeriodAttribute
		{
			...
			public INOpenPeriodTransferAttribute() :
				base(
					  ...
					  branchSourceType: typeof(AccessInfo.branchID),
