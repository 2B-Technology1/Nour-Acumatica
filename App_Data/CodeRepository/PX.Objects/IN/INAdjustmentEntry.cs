...
namespace PX.Objects.IN
{
	public class INAdjustmentEntry : INRegisterEntryBase
	{
		...
		[PXCopyPasteHiddenFields(typeof(INRegister.ignoreAllocationErrors))]
		public
			PXSelect<INRegister,
			Where<
		...
		public INAdjustmentEntry()
		{
			...
			OpenPeriodAttribute.SetValidatePeriod<INRegister.finPeriodID>(this.adjustment.Cache, null, PeriodValidation.DefaultSelectUpdate);
		...
		protected virtual void _(Events.FieldUpdated<INTran, INTran.costCenterID> e)
		{
			if (!object.Equals(e.OldValue, e.NewValue) && e.Row.OrigRefNbr != null)
				e.Cache.VerifyFieldAndRaiseException<INTran.origRefNbr>(e.Row);
		}
		...
		protected virtual void _(Events.RowSelected<INTran> e)
		{
			...
			bool uomEnabled = GetUOMEnabled(isPIAdjustment, e.Row);
			PXUIFieldAttribute.SetEnabled<INTran.uOM>(e.Cache, e.Row, uomEnabled);
		...
		protected virtual bool GetUOMEnabled(bool isPIAdjustment, INTran tran)
		{
			return !isPIAdjustment;
		}
		...
		protected virtual void _(Events.RowPersisting<INTran> e)
		{
			...
			if (adjustment.Current != null && adjustment.Current.OrigModule != INRegister.origModule.PI && item != null && item.ValMethod == INValMethod.FIFO && e.Row.OrigRefNbr == null)
			{
				...
				if (e.Row != null && e.Row.POReceiptNbr != null && e.Row.POReceiptLineNbr != null)
				{
					...
					dropShipPO = pOReceiptLine != null && PO.POLineType.IsDropShip(pOReceiptLine.LineType);
				}
		...
		public class LineSplittingExtension : GraphExtensions.INRegisterLineSplittingExtension<INAdjustmentEntry>
		{
			...
			protected override void ClearLotSerial(ManualEvent.Row<INTran>.Updated.Args e)
			{
				if (e.Row.OrigModule != INRegister.origModule.PI)
				...
				if (newLine != null)
				{
					...
					foreach (INTran tran in Base.transactions.Select())
					{
						if (tran.SortOrder >= newLine.SortOrder)
						{
							...
							Base.transactions.Cache.MarkUpdated(tran, assertError: true);
						}
					}
