...
namespace PX.Objects.IN.PhysicalInventory
{
	public class PILocksInspector
	{
		...
		public virtual bool IsInventoryLocationLocked(int? inventoryID, int? locationID, string excludePIID, out string PIID)
		{
			PIID = null;
			if (inventoryID == null || locationID == null)
				return false;

			PILocks locks = GetLocks().FirstOrDefault(l
				=> l.IsActive
				&& l.PIID != excludePIID
				&& l.HasCollision(inventoryID.Value, locationID.Value));
			PIID = locks?.PIID;
		...
		{
			return IsInventoryLocationLocked(inventoryID, locationID, excludePIID, out _);
		}
