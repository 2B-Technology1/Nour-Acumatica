...
{
	public class INItemClassMaint : PXGraph<INItemClassMaint, INItemClass>, ICreateMatrixHelperFactory
	{
		...
		[PXDimensionSelector(INItemClass.Dimension, typeof(SearchFor<INItemClass.itemClassCD>.Where<INItemClass.stkItem.IsEqual<False>.Or<INItemClass.stkItem.IsEqual<True>.And<FeatureInstalled<FeaturesSet.distributionModule>>>>), typeof(INItemClass.itemClassCD), DescriptionField = typeof(INItemClass.descr))]
		protected virtual void _(Events.CacheAttached<INItemClass.itemClassCD> e) { }
		#endregion
		...
		[PXButton(CommitChanges = true), PXUIField(Visible = false, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Update)]
		...
		public sealed override void Configure(PXScreenConfiguration config) =>
			Configure(config.GetScreenConfigurationContext<INItemClassMaint, INItemClass>());
		protected static void Configure(WorkflowContext<INItemClassMaint, INItemClass> context)
		{
			#region Categories
		...
		protected virtual void _(Events.RowSelected<INItemClass> e)
		{
			...
			CommodityCodeTypes.CustomListAttribute ccStringList = StockItem
				? new CommodityCodeTypes.StockCommodityCodeListAttribute()
				: (CommodityCodeTypes.CustomListAttribute)new CommodityCodeTypes.NonStockCommodityCodeListAttribute();
			...
			PXStringListAttribute.SetList<INItemClass.commodityCodeType>(itemclass.Cache, e.Row, ccStringList.AllowedValues, ccStringList.AllowedLabels);
		...
		protected virtual void _(Events.FieldUpdated<INItemClass, INItemClass.stkItem> e)
		{
			...
			e.Cache.SetDefaultExt<INItemClass.postToExpenseAccount>(e.Row);
		...
		protected virtual void InitDetailsFromParentItemClass(INItemClass itemClass)
		{
			...
			using (new ReadOnlyScope(UnitsOfMeasureExt.classunits.Cache, replenishment.Cache, Mapping.Cache))
			{
				UnitsOfMeasureExt.classunits.Cache.ClearQueryCache();
				UnitsOfMeasureExt.classunits.Cache.Clear();
				foreach (INUnit conv in UnitsOfMeasureExt.classunits.Select(itemClass.ParentItemClassID))
				{
					var convCopy = UnitsOfMeasureExt.classunits.Cache.CreateCopy(conv);
					UnitsOfMeasureExt.classunits.Cache.SetValue<INUnit.recordID>(convCopy, null);
					UnitsOfMeasureExt.classunits.Cache.SetValue<INUnit.itemClassID>(convCopy, itemClass.ItemClassID);
					convCopy = UnitsOfMeasureExt.classunits.Cache.Insert(convCopy);
					if (convCopy == null)
			...
			if (graph.itemclass.Current != null)
			{
				...
				if (children.Any())
				{
					using (PXTransactionScope ts = new PXTransactionScope())
					{
						...
						var unitConversionsTemplate = graph.UnitsOfMeasureExt.classunits.Select().RowCast<INUnit>();
		...
		protected virtual void UpdateChildItemClass(INItemClass child)
		{
			...
			PXDatabase.Update<INItemClass>(
				...
				new PXDataFieldAssign<INItemClass.postToExpenseAccount>(itemclass.Current.PostToExpenseAccount),
		...
		protected virtual void MergeUnitConversions(INItemClass child, IEnumerable<INUnit> unitConversionsTemplate)
		{
			...
			var unitConversionsExisting = UnitsOfMeasureExt.classunits.Select(child.ItemClassID).RowCast<INUnit>();
