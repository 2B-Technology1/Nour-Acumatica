...
namespace PX.Objects.IN
{
	...
	public class INUpdateReplenishmentRules : PXGraph<INUpdateReplenishmentRules>
	{
		...
		public partial class InventoryItemRO : IBqlTable
		{
			...
			{
				public static InventoryItemRO Find(PXGraph graph, int? inventoryID, PKFindOptions options = PKFindOptions.None) => FindBy(graph, inventoryID, options);
			}
		...
		public IEnumerable records() 
		{
			...
			if (filt.Action == Filter.Actions.Calculate)
			{
				PXSelectBase<INItemSiteRS> select = new PXSelectJoin<INItemSiteRS, InnerJoin<InventoryItemRO, On<InventoryItemRO.inventoryID, Equal<INItemSiteRS.inventoryID>,
																...
																And<InventoryItemRO.planningMethod, Equal<INPlanningMethod.inventoryReplenishment>>>>,
		...
		protected static void RecalcVendorLeadTime(PXGraph aGraph, INItemSiteRS aItemInfo, out Decimal? aLeadTimeAve, out Decimal? aLeadTimeMSE)
		{
			...
			PXSelectBase<POLine> select = new PXSelectReadonly2<POLine,
				InnerJoin<POReceiptLine, On<POReceiptLine.pOType, Equal<POLine.orderType>,
				...
				InnerJoin<POReceipt, On<POReceiptLine.FK.Receipt>>>,
					Where<POReceiptLine.receiptDate, IsNotNull,
						...
						And<POLine.costCenterID, Equal<CostCenter.freeStock>,
													...
													out Dictionary<int, List<PeriodSalesStatInfo>> periodValues)
		{
			...
			var histRecords = Lazy.By(() => PXSelectJoin<INItemSiteHistByCostCenterD,
					LeftJoin<INItemSiteReplenishment, On<INItemSiteHistByCostCenterD.FK.ItemSiteReplenishment>>,
					Where<INItemSiteHistByCostCenterD.siteID, Equal<Required<INItemSiteHistByCostCenterD.siteID>>,
						And<INItemSiteHistByCostCenterD.inventoryID, Equal<Required<INItemSiteHistByCostCenterD.inventoryID>>,
						And<INItemSiteHistByCostCenterD.costCenterID, Equal<CostCenter.freeStock>,
						And<INItemSiteHistByCostCenterD.sDate, Less<Required<INItemSiteHistByCostCenterD.sDate>>>>>>,
					OrderBy<Asc<INItemSiteHistByCostCenterD.sDate, Asc<INItemSiteHistByCostCenterD.subItemID>>>>
				.Select(aGraph, aItemInfo.SiteID, aItemInfo.InventoryID, maxDate).AsEnumerable()
				.Cast<PXResult<INItemSiteHistByCostCenterD, INItemSiteReplenishment>>()
				.ToArray());
			...
			{
				INItemSiteHistByCostCenterD firstSale = histRecords.Value
					.Select(h => (INItemSiteHistByCostCenterD)h)
			...
			foreach (PXResult<INItemSiteHistByCostCenterD, INItemSiteReplenishment> iSalesStats in histRecords.Value)
			{
				INItemSiteHistByCostCenterD iSales = iSalesStats;
		...
		public class PeriodSalesStatInfo : IComparable<PeriodSalesStatInfo>//, DataPoint, DemandForecastModel.IDataPoint
		{
			...
			public PeriodSalesStatInfo(string aPeriodType, INItemSiteHistByCostCenterD aSalesInfo, DateTime aSalesStartingDate)
			{
