...
namespace PX.Objects.IN
{
	...
	public class INReplenishmentProjection : TM.OwnedFilter.ProjectionAttribute
	{
		...
		protected override Type GetSelect(PXCache sender)
		{
			...
			commands.AddRange(new [] {
				typeof(Select2<,,>), typeof(S.INItemSite),
				typeof(InnerJoin<,,>), typeof(InventoryItem),
				typeof(On2<S.INItemSite.FK.InventoryItem,
							And<InventoryItem.itemStatus, NotEqual<InventoryItemStatus.inactive>,
							And<InventoryItem.itemStatus, NotEqual<InventoryItemStatus.markedForDeletion>,
							And<InventoryItem.planningMethod, Equal<INPlanningMethod.inventoryReplenishment>>>>>),
				typeof(InnerJoin<,,>), typeof(INSite),
					typeof(On<S.INItemSite.FK.Site>),
				typeof(LeftJoin<,,>), typeof(Vendor), typeof(On<S.INItemSite.FK.PreferredVendor>) });

			if (subItems)
			{
				commands.AddRange(new [] {
				typeof(LeftJoin<,,>), typeof(INSiteStatusByCostCenter),
					typeof(On<INSiteStatusByCostCenter.inventoryID, Equal<S.INItemSite.inventoryID>,
						And<INSiteStatusByCostCenter.siteID, Equal<S.INItemSite.siteID>,
						And<INSiteStatusByCostCenter.costCenterID, Equal<CostCenter.freeStock>>>>),
				typeof(LeftJoin<,,>), typeof(POVendorInventoryRepo),
				typeof(On<POVendorInventoryRepo.inventoryID, Equal<S.INItemSite.inventoryID>,
					And<POVendorInventoryRepo.vendorID, Equal<S.INItemSite.preferredVendorID>,
					And<IsNull<POVendorInventoryRepo.vendorLocationID, int_1>, Equal<IsNull<S.INItemSite.preferredVendorLocationID, int_1>>,
					And<IsNull<POVendorInventoryRepo.subItemID, int_1>, Equal<IsNull<INSiteStatusByCostCenter.subItemID, int_1>>>>>>),

				typeof(LeftJoin<,,>), typeof(INSubItem), typeof(On<INSiteStatusByCostCenter.FK.SubItem>),
				...
				typeof(On<INItemSiteReplenishment.inventoryID, Equal<INItemSite.inventoryID>,
							...
							And<INItemSiteReplenishment.subItemID, Equal<INSiteStatusByCostCenter.subItemID>,
			...
			else
			{
				...
				typeof(On<INSiteStatusByCostCenter.inventoryID, Equal<S.INItemSite.inventoryID>,
							And<INSiteStatusByCostCenter.siteID, Equal<S.INItemSite.siteID>,
							And<INSiteStatusByCostCenter.subItemID, Equal<INSubItem.subItemID>,
							And<INSiteStatusByCostCenter.costCenterID, Equal<CostCenter.freeStock>>>>>),
				typeof(LeftJoin<,,>), typeof(POVendorInventoryRepo),
				typeof(On<POVendorInventoryRepo.inventoryID, Equal<S.INItemSite.inventoryID>,
					And<POVendorInventoryRepo.vendorID, Equal<S.INItemSite.preferredVendorID>,
					And<IsNull<POVendorInventoryRepo.vendorLocationID, int_1>, Equal<IsNull<S.INItemSite.preferredVendorLocationID, int_1>>,
					And<IsNull<POVendorInventoryRepo.subItemID, int_1>, Equal<IsNull<INSubItem.subItemID, int_1>>>>>>),
	...
	public class INReplenishmentItem : S.INItemSite
	{
		#region Keys
		public new class PK : PrimaryKeyOf<INReplenishmentItem>.By<inventoryCD, siteCD, subItemCD>
		{
			public static INReplenishmentItem Find(PXGraph graph, string inventoryID, string siteID, string subItemCD, PKFindOptions options = PKFindOptions.None) => FindBy(graph, inventoryID, siteID, subItemCD, options);
		}
		...
		#endregion
		#region PurchaseUnit
		[INUnit(typeof(inventoryID), DisplayName = "Purchase UOM", BqlField = typeof(InventoryItem.purchaseUnit))]
		...
		[PXQuantity]
		[PXDBCalced(typeof(IsNull<INSiteStatusByCostCenter.qtyPOOrders, decimal0>), typeof(decimal))]
		[PXDefault(TypeCode.Decimal, "0.0")]
			...
			foreach (INReplenishmentItem rec in list)
			{
				...
				if (rec.Selected == true || rec.QtyProcess > 0)
					...
					And<INItemPlan.siteID, Equal<Required<INItemPlan.siteID>>,
						...
						graph.Caches<INItemPlan>().Delete(e);
				...
				if (rec.QtyProcess > 0)
				{
					...
					plan.Hold = false;
					graph.Caches<INItemPlan>().Update(plan);
					updated = true;
				}
			}
			if (updated)
