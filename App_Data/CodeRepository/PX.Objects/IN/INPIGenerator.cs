...
namespace PX.Objects.IN
{
	...
	public class PIGenerator : PXGraph<PIGenerator>
	{
		...
		{
			PIGeneratorSettings currentSettings = this.GeneratorSettings.Current;
			List<ExcludedLocation> excludedLocations = this.ExcludedLocations.Select().RowCast<ExcludedLocation>().ToList();
			List<ExcludedInventoryItem> excludedInventories = this.ExcludedInventoryItems.Select().RowCast<ExcludedInventoryItem>().ToList();

			PXLongOperation.StartOperation(this.UID, delegate ()
			{
				PIGenerator graph = PXGraph.CreateInstance<PIGenerator>();
				graph.GeneratorSettings.Current = currentSettings;
				foreach (ExcludedLocation location in excludedLocations)
				{
					graph.ExcludedLocations.Cache.MarkUpdated(location, assertError: true);
				}
				foreach (ExcludedInventoryItem inventory in excludedInventories)
				{
					graph.ExcludedInventoryItems.Cache.MarkUpdated(inventory, assertError: true);
				}
				...
				target.Clear();
				target.PIHeader.Current = PXSelect<INPIHeader, Where<INPIHeader.pIID, Equal<Current<INPIHeader.pIID>>>>.Select(target);
		...
		public virtual List<PIPreliminaryResult> CalcPIRows(bool updateDB, bool saveEmpty)
		{
			...
			var piTypeLocations = GetPiTypeLocations().RowCast<INLocation>().ToList();
			...
			foreach (var il in intermediateResult)
			{
				...
				if (lss_rec?.LotSerialNbr != null) // nulls left-joined - non-lot-serial
				{
					...
					item.BookQty = lss_rec.GetBookQty();
				}
				else
				{
					...
					if (ls_rec != null)
					{
						item.BookQty = ls_rec.GetBookQty();
					}
				}
