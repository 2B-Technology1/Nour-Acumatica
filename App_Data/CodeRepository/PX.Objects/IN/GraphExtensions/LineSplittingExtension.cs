...
namespace PX.Objects.IN.GraphExtensions
{
		...
		where TSplit : class, IBqlTable, ILSDetail, new()
	{
		...
		protected virtual bool TryInferLineFieldFromAttribute(PXEventSubscriberAttribute attr)
		{
			...
			if (LineLotSerialNbrField == null && attr is INLotSerialNbrAttribute)
			{
				LineLotSerialNbrField = SplitCache.GetBqlField(attr.FieldName);
				return true;
			}
		...
		public virtual IEnumerable GetLotSerialOpts()
		{
			...
			opt.ExtensionName = TypePrefixed(nameof(LotSerOptions));
		...
		protected PXAction AddAction(string name, string displayName, bool visible, PXButtonDelegate handler, PXCacheRights enableRights)
		{
			...
			if (name == TypePrefixed(nameof(ShowSplits)))
				action.SetIgnoresArchiveDisabling(true);
			...
			else if (item != null)
			{
				...
				try
				{
						...
						View.Select(Base, e.Row.InventoryID))
					{
						...
						TLine copy = CreateVirtualStockComponentLine(e.Row, kitItem);

						ConvertedInventoryItemAttribute.ValidateRow(LineCache, copy);

						try
						{
							EventHandlerInternal(new ManualEvent.Row<TLine>.Inserted.Args(e.Cache, copy, e.ExternalCall));

							if (copy.UnassignedQty > 0m && unassignedKitQty < copy.UnassignedQty / kitItem.DfltCompQty)
							{
								unassignedKitQty = copy.UnassignedQty / kitItem.DfltCompQty;
							}
						}
					...
					View.Select(Base, e.Row.InventoryID))
				{
					...
					TLine copy = CreateVirtualNonStockComponentLine(e.Row, kitItem);
				...
				e.Row.UnassignedQty = unassignedKitQty;
				if (unassignedKitQty > 0m)
		...
		protected virtual void EventHandler(ManualEvent.Row<TLine>.Updated.Args e) // former Master_RowUpdated
		{
			...
			using (SuppressedModeScope())
				using (InvtMultModeScope(e.Row, e.OldRow))
					EventHandlerInternal(e);
		}
		...
		protected virtual void EventHandlerInternal(ManualEvent.Row<TLine>.Updated.Args e) // former _Master_RowUpdated
		{
			...
			if (e.OldRow != null && (
				...
				|| e.OldRow.CostCenterID != e.Row.CostCenterID))
			{
			...
			else
			{
				...
				if (e.Row.ExpireDate == e.OldRow.ExpireDate
					&& !string.Equals(e.OldRow.LotSerialNbr, e.Row.LotSerialNbr, StringComparison.OrdinalIgnoreCase))
				{
					e.Row.ExpireDate = null;
				}
				...
				if (item.StkItem == true || item.KitItem != true)
				{
					...
					if (mode == INLotSerTrack.Mode.None || (mode & INLotSerTrack.Mode.Create) > 0)
					{
						...
						else if (IsTransferReceipt(e.Row))
						{
							UpdateNumbers(e.Row);
						}
					...
					{
						if (IsPrimaryFieldsUpdated(e.Row, e.OldRow) || !string.Equals(e.Row.LotSerialNbr, e.OldRow.LotSerialNbr, StringComparison.OrdinalIgnoreCase))
						{
							RaiseRowDeleted(e.OldRow);
							RaiseRowInserted(e.Row);
						}
				...
				else
				{
					...
					try
					{
							...
							View.Select(Base, e.Row.InventoryID))
						{
							...
							TLine oldcopy = CreateVirtualStockComponentLine(e.OldRow, kititem);
						...
						View.Select(Base, e.Row.InventoryID))
					{
						...
						TLine oldcopy = CreateVirtualNonStockComponentLine(e.OldRow, kitItem);
		...
		protected virtual void ClearLotSerial(ManualEvent.Row<TLine>.Updated.Args e)
		{
		...
		protected virtual void EventHandlerInternal(ManualEvent.Row<TLine>.Deleted.Args e) // former _Master_RowDeleted
		{
			...
			if (e.Row.InventoryID != null)
			{
				...
				if (item != null && (((InventoryItem)item).StkItem == false && ((InventoryItem)item).KitItem == true))
				{
					foreach (PXResult<INKitSpecStkDet, InventoryItem> res in
						SelectFrom<INKitSpecStkDet>.
						InnerJoin<InventoryItem>.On<INKitSpecStkDet.FK.ComponentInventoryItem>.
						...
						View.Select(Base, e.Row.InventoryID))
					{
						...
						copy.UnassignedQty = 0m;

						try
						{
							EventHandlerInternal(new ManualEvent.Row<TLine>.Deleted.Args(e.Cache, copy, false));
						}
						catch (PXException ex)
						{
							throw new PXException(ex, Messages.FailedToProcessComponent, compItem.InventoryCD, item.InventoryCD, ex.MessageNoPrefix);
					...
					foreach (PXResult<INKitSpecNonStkDet, InventoryItem> res in
						SelectFrom<INKitSpecNonStkDet>.
							InnerJoin<InventoryItem>.On<INKitSpecNonStkDet.FK.ComponentInventoryItem>.
							Where<INKitSpecNonStkDet.kitInventoryID.IsEqual<@P.AsInt>.And<InventoryItem.kitItem.IsEqual<True>.Or<InventoryItem.nonStockShip.IsEqual<True>>>>.
							View.Select(Base, e.Row.InventoryID))
					{
						(var kititem, var compItem) = res;
		...
		protected virtual void EventHandler(ManualEvent.Row<TLine>.Persisting.Args e) // former Master_RowPersisting
		{
			if (e.Operation.Command().IsIn(PXDBOperation.Insert, PXDBOperation.Update)
				&& GenerateLotSerialNumberOnPersist(e.Row))
			{
				TSplit[] splits = SelectSplits(e.Row, false);
				if (e.Row != null)
				...
				foreach (TSplit split in splits)
				{
					...
					if (string.IsNullOrEmpty(e.Row.LotSerialNbr) == false && split.InventoryID == e.Row.InventoryID)
					{
			...
			if (e.TranStatus == PXTranStatus.Aborted)
			{
				...
				if (e.Operation.Command().IsIn(PXDBOperation.Insert, PXDBOperation.Update))
				{
					if (e.Row == null || !PersistedLinesToRelatedSplits.TryGetValue(e.Row, out TSplit[] lineSplits))
						lineSplits = SelectSplits(e.Row, false);
		...
		protected virtual void EventHandler(ManualEvent.Row<TSplit>.Inserting.Args e) // former Detail_RowInserting
		{
			...
			if (SuppressedMode && CurrentOperation == PXDBOperation.Update)
			{
				...
				if (sameSibling != null)
				{
					...
					if (lsClass.LotSerTrack != INLotSerTrack.SerialNumbered || sameSibling.BaseQty == 0m || GetTranTrackMode(e.Row, lsClass) == INLotSerTrack.Mode.None)
					{
						...
						e.Cache.MarkUpdated(sameSibling, assertError: true);
		...
		protected virtual void EventHandler(ManualEvent.Row<TSplit>.Inserted.Args e) // former Detail_RowInserted
		{
			...
			using (SuppressedModeScope())
			{
				var parent = UpdateParent(e.Row, null);

				if (!UnattendedMode)
					Availability?.Check(e.Row, parent.CostCenterID);
			}
		}
		...
		protected virtual void EventHandler(ManualEvent.Row<TSplit>.Updated.Args e) // former Detail_RowUpdated
		{
			...
			using (SuppressedModeScope())
			{
				var parent = UpdateParent(e.Row, e.OldRow);
		...
		public virtual void EventHandler(ManualEvent.Row<TSplit>.Persisting.Args e) // former Detail_RowPersisting
		{
			if (e.Operation.Command().IsIn(PXDBOperation.Insert, PXDBOperation.Update))
			{
				if (string.IsNullOrEmpty(e.Row.AssignedNbr) == false && INLotSerialNbrAttribute.StringsEqual(e.Row.AssignedNbr, e.Row.LotSerialNbr))
				{
					...
					try
					{
						numVal = AutoNumberAttribute.NextNumber(lotSerNum?.LotSerNumVal, silent: false);
					}
		...
		protected virtual void EventHandler(ManualEvent.Row<LotSerOptions>.Selected.Args e)
		{
			...
			if (String.IsNullOrEmpty(e.Row?.ExtensionName) || TypePrefixed(nameof(LotSerOptions)) == e.Row.ExtensionName)
		...
		{
			IssueNumbers(line, deltaBaseQty, Base.Caches<INLotSerialStatusByCostCenter>(), Base.Caches<LotSerialStatusByCostCenter>());
		}
		...
		protected void IssueNumbers(TLine line, decimal deltaBaseQty, PXCache statusCache, PXCache statusAccumCache)
		{
			...
			(decimal remainderBaseQty, TSplit lastSplit) = IssueLotSerials(line, deltaBaseQty, statusCache, statusAccumCache, item);
			HandleRemainder(line, remainderBaseQty, lastSplit, (INLotSerClass)item);
		}

		protected virtual (decimal, TSplit) IssueLotSerials(TLine line, decimal deltaBaseQty, PXCache statusCache, PXCache statusAccumCache, PXResult<InventoryItem, INLotSerClass> item)
		{
			TSplit split = LineToSplit(line);
			...
			if ((GetTranTrackMode(line, lotSerClass) & INLotSerTrack.Mode.Issue) > 0)
			{
				...
				foreach (ILotSerial lsmaster in lotSerialStatuses)
				{
					...
					var signs = Availability.GetAvailabilitySigns<SiteLotSerial>(split);
					if (signs.SignQtyAvail.IsMinus)
					{
						...
						decimal? availableQty = GetSerialStatusAvailableQty(lsmaster, accumavail);
						if (availableQty <= 0m)
						{
							continue;
						}
					...
					else
					{
						...
						decimal? qtyOnHand = GetSerialStatusQtyOnHand(lsmaster);
						if (qtyOnHand <= 0m)
							continue;

						if (qtyOnHand <= deltaBaseQty)
						{
							split.BaseQty = qtyOnHand;
							deltaBaseQty -= qtyOnHand.Value;
						}
						else
						{
							if (lotSerClass.LotSerTrack == INLotSerTrack.SerialNumbered)
		...
		protected virtual void HandleRemainder(TLine line, decimal deltaBaseQty, TSplit lastSplit, INLotSerClass lotSerClass)
		{
			if (deltaBaseQty > 0m && line.InventoryID != null && line.SubItemID != null && line.SiteID != null && line.LocationID != null && !string.IsNullOrEmpty(line.LotSerialNbr))
			{
				...
				{
					lastSplit.SplitLineNbr = null;
					lastSplit.BaseQty = deltaBaseQty;

					SetSplitQtyWithLine(lastSplit, line);
					lastSplit.ExpireDate = ExpireDateByLot(lastSplit, null);

					try
					{
						PXCache<TSplit>.Insert(Base, Clone(lastSplit));
					}
					catch
					{
						line.UnassignedQty += deltaBaseQty;
		...
		{
			return ((IStatus)lsmaster).QtyAvail + accumavail.QtyAvail;
		}

		protected virtual decimal? GetSerialStatusQtyOnHand(ILotSerial lsmaster)
		{
			return lsmaster.QtyOnHand;
		}

		internal virtual List<ILotSerial> PerformSelectSerial<TLotSerialStatus>(PXSelectBase cmd, object[] pars)
			where TLotSerialStatus : ILotSerial, IBqlTable
		{
			return cmd.View.SelectMultiBound(pars).RowCast<TLotSerialStatus>().Cast<ILotSerial>().ToList();
		}
		...
		{
			var ret = new INLotSerialStatusByCostCenter
			{
				InventoryID = item.InventoryID,
				SiteID = item.SiteID,
				LocationID = item.LocationID,
			...
			if (item is TLine line)
			{
				ret.CostCenterID = line.CostCenterID;
			}
			else if (item is TSplit split)
			{
				// TODO: Special: check if can do without select
				line = SelectLine(split);
				ret.CostCenterID = line?.CostCenterID ?? CostCenter.FreeStock;
			}
		...
		protected virtual void ExpireLotSerialStatusCacheFor(TSplit split)
		{
			...
			ExpireCached(MakeINLotSerialStatusByCostCenter(split));
		}
		#endregion
		...
		public virtual TLine UpdateParent(TSplit newSplit, TSplit oldSplit)
		{
			...
			if (parent != null && anySplit != null && SameInventoryItem(anySplit, parent))
			{
				...
				using (InvtMultModeScope(parent))
				{
					SetUnassignedQty(parent, baseQty, false);
				}

				LineCache.MarkUpdated(parent, assertError: true);
				...
				if (Math.Abs(oldParent.BaseQty.Value - parent.BaseQty.Value) >= 0.0000005m ||
				    Math.Abs(oldParent.Qty.Value - parent.Qty.Value) >= 0.0000005m)
				{
		...
		protected virtual TSplit[] SelectSplits(TLine line, bool compareInventoryID = true)
		{
			return PXParentAttribute
				...
				.Cast<TSplit>()
				.Where(split => !compareInventoryID || SameInventoryItem(split, line))
				.ToArray();
		}
		...
		{
			PXSelectBase<INLotSerialStatusByCostCenter> cmd = GetSerialStatusCmd(line, item);
			INLotSerialStatusByCostCenter pars = MakeINLotSerialStatusByCostCenter(line);
			return PerformSelectSerial<INLotSerialStatusByCostCenter>(cmd, new object[] { pars });
		}

		public virtual PXSelectBase<INLotSerialStatusByCostCenter> GetSerialStatusCmd(TLine line, PXResult<InventoryItem, INLotSerClass> item)
		{
			PXSelectBase<INLotSerialStatusByCostCenter> cmd = GetSerialStatusCmdBase(line, item);
		...
		protected virtual PXSelectBase<INLotSerialStatusByCostCenter> GetSerialStatusCmdBase(TLine line, PXResult<InventoryItem, INLotSerClass> item)
		{
			return new
		...
		protected virtual void AppendSerialStatusCmdWhere(PXSelectBase<INLotSerialStatusByCostCenter> cmd, TLine line, INLotSerClass lotSerClass)
		{
		...
		public virtual void AppendSerialStatusCmdOrderBy(PXSelectBase<INLotSerialStatusByCostCenter> cmd, TLine line, INLotSerClass lotSerClass)
		{
		...
		{
			return line.TranType == INTranType.Transfer && line.InvtMult == 1;
		}
			...
			if (a != null && b != null)
			{
				return a.InventoryID == b.InventoryID &&
					(a.IsStockItem != true ||
						...
						(string.Equals(a.LotSerialNbr, b.LotSerialNbr, StringComparison.OrdinalIgnoreCase) || string.IsNullOrEmpty(a.LotSerialNbr) && string.IsNullOrEmpty(b.LotSerialNbr)) &&
		...
		{
			if (SkipUnassignedException || PXUIFieldAttribute.GetErrorOnly(LineCache, line, LineQtyField.Name) != null)
				return;
		...
		protected virtual bool GenerateLotSerialNumberOnPersist(TLine line) => !LineCounters.TryGetValue(line, out Counters counters) || counters.UnassignedNumber != 0;
		#endregion
		...
		public class InvtMultScope : IDisposable
		{
			...
			protected virtual bool IsReverse(TLine line) => line.Qty < 0m;
		...
		protected virtual IDisposable InvtMultModeScope(TLine line) => new InvtMultScope(line);
		protected virtual IDisposable InvtMultModeScope(TLine line, TLine oldLine) => new InvtMultScope(line, oldLine);
		...
		#region SkipUnassignedExceptionScope
		public virtual IDisposable SkipUnassignedExceptionScope()
		{
			SkipUnassignedException = true;
			return Disposable.Create(() => SkipUnassignedException = false);
		}
		#endregion
		#endregion
	}
