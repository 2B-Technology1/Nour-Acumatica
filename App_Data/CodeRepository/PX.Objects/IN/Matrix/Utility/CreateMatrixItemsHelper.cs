	...
	public class CreateMatrixItemsHelper
	{
			...
			Action<MatrixInventoryItem, InventoryItem> beforeSave = null)
		{
			...
			IEnumerable<INUnit> templateItemConvs =
				graph.UnitsOfMeasureExt.itemunits.View.SelectMultiBound(new[] { templateItem })
				.RowCast<INUnit>()
				.ToArray();
			...
			MatrixInventoryItem itemToCreateUpdate, bool create)
		{
			...
			AssignConversionsSettings(graph, item, templateItem, userExcludedFields, create);
		...
		protected virtual void AssignConversionsSettings(InventoryItemMaintBase graph, InventoryItem item, InventoryItem templateItem,
			HashSet<string> userExcludedFields, bool create)
		{
			...
			{
				if (!userExcludedFields.Contains(nameof(InventoryItem.salesUnit)))
					cache.SetValueExt<InventoryItem.salesUnit>(item, templateItem.SalesUnit);

				if (!userExcludedFields.Contains(nameof(InventoryItem.purchaseUnit)))
					cache.SetValueExt<InventoryItem.purchaseUnit>(item, templateItem.PurchaseUnit);

				if (!userExcludedFields.Contains(nameof(InventoryItem.decimalSalesUnit)))
					cache.SetValueExt<InventoryItem.decimalSalesUnit>(item, templateItem.DecimalSalesUnit);

				if (!userExcludedFields.Contains(nameof(InventoryItem.decimalPurchaseUnit)))
					cache.SetValueExt<InventoryItem.decimalPurchaseUnit>(item, templateItem.DecimalPurchaseUnit);
			}
		}
			...
			foreach (POVendorInventory templateVendorInv in templateVendorInvs)
			{
				...
				if (excludeFields == null)
				{
					excludeFields = new string[]
					{
						...
						nameof(POVendorInventory.noteID),
					};
		...
		protected virtual void AssignInventoryConversions(InventoryItemMaintBase graph, IEnumerable<INUnit> templateItemConvs)
		{
			...
			INUnit[] itemConvs = graph.UnitsOfMeasureExt.itemunits.Select().RowCast<INUnit>().ToArray();
			foreach (INUnit templateItemConv in templateItemConvs)
			{
				...
				if (createItemConv)
				{
					...
					itemConv = graph.UnitsOfMeasureExt.itemunits.Insert(new INUnit { FromUnit = templateItemConv.FromUnit });
				}
				var copy = (INUnit)graph.UnitsOfMeasureExt.itemunits.Cache.CreateCopy(itemConv);
				graph.UnitsOfMeasureExt.itemunits.Cache.RestoreCopy(copy, templateItemConv);

				foreach (string excludeField in excludeFields)
				{
					graph.UnitsOfMeasureExt.itemunits.Cache.SetValue(copy, excludeField,
						graph.UnitsOfMeasureExt.itemunits.Cache.GetValue(itemConv, excludeField));
				}

				itemConv = graph.UnitsOfMeasureExt.itemunits.Update(copy);
			}
		}
