			...
			where MainItemType : class, IBqlTable, new()
	{
		...
		protected virtual IDictionary<InventoryMapKey, InventoryMapValue> GetInventoryMap()
		{
			...
			foreach (PXResult<CSAnswers, InventoryItem> result in SelectInventoryWithAttributes())
			{
				...
				{
					var key = new InventoryMapKey(columnAttributeValue, rowAttributeValue);

					if (inventoryMatrix.TryGetValue(key, out var duplicate))
					{
						var template = InventoryItem.PK.FindDirty(Base, GetTemplateID());
						var duplicateItem = InventoryItem.PK.Find(Base, duplicate.InventoryID);
						var item = InventoryItem.PK.Find(Base, lastInventoryId);

						throw new PXSetPropertyException(Messages.TemplateHasDuplicatesByAttributes,
							item.InventoryCD?.TrimEnd(), duplicateItem.InventoryCD?.TrimEnd(), template.InventoryCD?.TrimEnd());
					}

					inventoryMatrix.Add(key, CreateInventoryMapValue(lastInventoryId, result));
		...
		protected virtual void FillInventoryMatrix(IDictionary<InventoryMapKey, InventoryMapValue> inventoryMap, int? selectedColumn)
		{
			...
			for (int rowAttributeIndex = 0; rowAttributeIndex < matrixValues.RowValues.Length; rowAttributeIndex++)
			{
				...
				newRow.UOMs = new string[matrixValues.ColumnValues.Length];
				newRow.BaseQuantities = new decimal?[matrixValues.ColumnValues.Length];
				...
				for (int colAttributeIndex = 0; colAttributeIndex < matrixValues.ColumnValues.Length; colAttributeIndex++)
				{
					...
					newRow.BaseUOM = template.BaseUnit;
