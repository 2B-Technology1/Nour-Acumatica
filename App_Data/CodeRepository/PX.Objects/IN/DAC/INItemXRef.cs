...
namespace PX.Objects.IN
{
	...
	public partial class INItemXRef : PX.Data.IBqlTable
	{
		...
		{
			public static INItemXRef Find(PXGraph graph, int? inventoryID, string alternateType, int? bAccountID, string alternateID, int? subItemID, PKFindOptions options = PKFindOptions.None)
				=> FindBy(graph, inventoryID, alternateType, bAccountID, alternateID, subItemID, options);
		}
		...
		[PXRestrictor(typeof(
				Where<
					BAccountR.type, In3<BAccountType.customerType, BAccountType.vendorType, BAccountType.combinedType>,
					And<
						Where<Optional<INItemXRef.alternateType>, Equal<INAlternateType.vPN>,
							Or<Optional<INItemXRef.alternateType>, Equal<INAlternateType.cPN>>>>>),
			Messages.DoesNotMatchWithAlternateType, typeof(BAccountR.acctCD), typeof(BAccountR.type),
			ShowWarning = true)]
		[PXRestrictor(typeof(
				Where<
					BAccountR.type, In3<BAccountType.vendorType, BAccountType.combinedType>,
					And<BAccountR.vStatus, NotEqual<AP.VendorStatus.inactive>,
					And<Optional<INItemXRef.alternateType>, Equal<INAlternateType.vPN>,
				Or<Optional<INItemXRef.alternateType>, Equal<INAlternateType.cPN>>>>>),
			Messages.VendorIsInactive,
			ShowWarning = true)]
		[PXRestrictor(typeof(
				Where<
					BAccountR.type, In3<BAccountType.customerType, BAccountType.combinedType>,
					And<BAccountR.status, NotEqual<AR.CustomerStatus.inactive>,
					And<Optional<INItemXRef.alternateType>, Equal<INAlternateType.cPN>,
				Or<Optional<INItemXRef.alternateType>, Equal<INAlternateType.vPN>>>>>),
			Messages.CustomerIsInactive,
			ShowWarning = true)]
	...
	public class INAlternateType
	{
		...
		public class gIN : PX.Data.BQL.BqlString.Constant<gIN>
		{
			public gIN() : base(GIN) {; }
		}
