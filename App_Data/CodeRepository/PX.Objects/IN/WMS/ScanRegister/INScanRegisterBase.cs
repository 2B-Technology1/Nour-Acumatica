		...
		public abstract class ReleaseCommand : ScanCommand
		{
			...
			protected override bool Process()
			{
				if (Basis.Document != null)
				{
					...
					Basis
					.AwaitFor<INRegister>(async (basis, doc, ct) =>
					{
						INDocumentRelease.ReleaseDoc(new List<INRegister>() { doc }, false);
						await basis.CurrentMode.Commands.OfType<ReleaseCommand>().FirstOrDefault()?.OnAfterRelease(doc, ct);
					})
			...
			public virtual void ConfigureOnSuccessAction(ScanLongRunAwaiter<TSelf, INRegister>.ISuccessProcessor onSuccess)
			{
				onSuccess
