	...
	public class INScanReceive : WMSBase
	{
		...
		public sealed class ReceiptMode : ScanMode
		{
			...
			public new sealed class ReleaseCommand : WMSBase.ReleaseCommand
			{
				...
				protected override async System.Threading.Tasks.Task OnAfterRelease(INRegister doc, CancellationToken cancellationToken)
				{
					await base.OnAfterRelease(doc, cancellationToken);
					...
					if (PXAccess.FeatureInstalled<CS.FeaturesSet.deviceHub>() && doc.RefNbr != null)
					{
						...
						if (printInventory && !string.IsNullOrEmpty(printLabelsReportID))
						{
							...
							await DeviceHubTools.PrintReportViaDeviceHub<CR.BAccount>(Basis, printLabelsReportID, reportParameters, INNotificationSource.None, null, cancellationToken);
						}
					}
				}
