...
namespace PX.Objects.IN.WMS
{
	...
	public class INScanTransfer : WMSBase
	{
		...
		public virtual bool UseDefaultLotSerialNbr => UserSetup.For(Graph).UseDefaultLotSerialNbr == true;
		...
		#region TransferToSiteID
		public int? TransferToSiteID
		{
			get => TransferHeader.TransferToSiteID;
			set => TransferSetter.Set(h => h.TransferToSiteID, value);
		}
		#endregion
		#region TransferToLocationID
		public int? TransferToLocationID
		{
			get => TransferHeader.TransferToLocationID;
			set => TransferSetter.Set(h => h.TransferToLocationID, value);
		}
		#endregion
		...
		public sealed class TransferMode : ScanMode
		{
			...
			protected override IEnumerable<ScanState<INScanTransfer>> CreateStates()
			{
				...
				yield return new WarehouseState()
					.Intercept.ClearState.ByAppend(basis => basis.TransferToSiteID = null);
				...
				yield return new LotSerialState()
					.Intercept.IsStateActive.ByConjoin(basis => basis.UseDefaultLotSerialNbr
						? basis.Remove != true && basis.SelectedLotSerialClass?.LotSerIssueMethod == INLotSerIssueMethod.UserEnterable
						: basis.IsEnterableLotSerial(isForIssue: false));
			...
			public abstract class AmbiguousLocationState : EntityState<INLocation[]>
			{
				...
				protected override bool IsStateSkippable() => !Basis.PromptLocationForEveryLine && (_isSource ? Basis.LocationID != null : Basis.TransferToLocationID != null);
				private int? SiteID => Basis.Document == null
					? _isSource ? Basis.SiteID : Basis.TransferToSiteID
					: _isSource ? Basis.Document.SiteID : Basis.Document.ToSiteID;
				...
				protected override INLocation[] GetByBarcode(string barcode)
				{
					if (Basis.Document == null)
					{
						...
						{
							var defaultSite = UserPreferenceExt.GetDefaultSite(Basis.Graph);
							if (defaultSite == null)
								locations = locations.Where(r => r.site.SiteID == SiteID).ToArray();
						}

						if (locations.Length == 0)
							return null;

						ambiguousLocation = locations.Where(t => t.location.Active == true).Count() > 1;
						return locations.Select(t => t.location).ToArray();
					}
					else
					{
						if (Basis.SiteID != Basis.Document.SiteID)
							Basis.SiteID = Basis.Document.SiteID;

						if (Basis.TransferToSiteID != Basis.Document.ToSiteID)
							Basis.TransferToSiteID = Basis.Document.ToSiteID;

						return ReadLocationByBarcode(_isSource ? Basis.Document.SiteID : Basis.Document.ToSiteID, barcode).With(loc => new[] { loc });
					}
				}
					...
					else
					{
						...
						if (_isSource)
						{
							if (Basis.Document == null)
							{
								...
								Basis.TransferToSiteID = location.SiteID;
							}
						...
						else
						{
							if (Basis.Document == null)
								Basis.TransferToSiteID = location.SiteID;
							Basis.TransferToLocationID = location.LocationID;
						}
					}
				}
				...
				protected virtual IEnumerable<(INLocation location, INSite site)> ReadLocationsByBarcode(int? siteID, string locationCD)
				{
					...
					var locations =
						...
						Where<
							...
							And<INLocation.locationCD.IsEqual<@P.AsString>>.
							And<INSite.active.IsEqual<True>>>.
						OrderBy<
			...
			public sealed class ConfirmState : ConfirmationState
			{
				...
				public class Logic : ScanExtension
				{
					...
					protected virtual bool CanConfirm(out FlowStatus error)
					{
						...
						if (Basis.CurrentMode.HasActive<TargetLocationState>() && Basis.TransferToLocationID == null)
						{
					...
					protected virtual FlowStatus ConfirmAdd()
					{
						...
						if (existTransaction != null)
						{
							...
							using (string.IsNullOrEmpty(Basis.LotSerialNbr) ? null : Basis.Base.LineSplittingExt.SkipUnassignedExceptionScope())
						...
						else
						{
							...
							setter.Set(tr => tr.SiteID, Basis.Document.SiteID);
							setter.Set(tr => tr.ToSiteID, Basis.Document.ToSiteID);
							existTransaction = Basis.Details.Update(existTransaction);
					...
					protected virtual INRegister EnsureDocument()
					{
						...
						Basis.DocumentView.SetValueExt<INRegister.toSiteID>(Basis.Document, Basis.TransferToSiteID);
						}
					...
					{
						if (Basis.HasActive<LotSerialState>() && !string.IsNullOrEmpty(Basis.LotSerialNbr) && Basis.Graph.splits.SelectMain().Any(s => !string.Equals(s.LotSerialNbr, Basis.LotSerialNbr, StringComparison.OrdinalIgnoreCase)))
						{
					...
					protected virtual INTran FindTransferRow()
					{
						var existTransactions = Basis.Details.SelectMain().Where(t =>
							...
							t.ToLocationID == (Basis.TransferToLocationID ?? t.ToLocationID) &&
							t.ReasonCode == (Basis.ReasonCodeID ?? t.ReasonCode) &&
							t.UOM == Basis.UOM);

						INTran existTransaction;
						...
						{
							if (Basis.ItemHasLotSerial
								&& Basis.UseDefaultLotSerialNbr == true)
							{
								if (Basis.Remove == true)
									existTransaction = existTransactions.LastOrDefault();
								else if (Basis.IsEnterableLotSerial(isForIssue: false))
									existTransaction = null;
								else
			...
			public new sealed class ReleaseCommand : WMSBase.ReleaseCommand
			{
				...
				public override void ConfigureOnSuccessAction(ScanLongRunAwaiter<INScanTransfer, INRegister>.ISuccessProcessor onSuccess)
				{
					base.ConfigureOnSuccessAction(onSuccess);

					onSuccess
						.ResetFull()
						.Do((basis, doc) => basis.ReportComplete(DocumentIsReleased));//to keep the msg after set state
				}
			}
			#endregion
	...
	public sealed class TransferScanHeader : PXCacheExtension<RegisterScanHeader, WMSScanHeader, QtyScanHeader, ScanHeader>
	{
		...
		[PXUIVisible(typeof(transferToSiteID.IsNotNull.And<transferToSiteID.IsNotEqual<WMSScanHeader.siteID>>))]
		public int? TransferToSiteID { get; set; }
		public abstract class transferToSiteID : BqlInt.Field<transferToSiteID> { }
		#endregion
		#region TransferToLocationID
		[Location(typeof(transferToSiteID))]
		public int? TransferToLocationID { get; set; }
		public abstract class transferToLocationID : BqlInt.Field<transferToLocationID> { }
		#endregion
		#region AmbiguousLocationCD
		[PXString]
