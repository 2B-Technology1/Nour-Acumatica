...
namespace PX.Objects.IN.WMS
{
		...
		where TGraph : PXGraph, new()
	{
		...
		public class InventoryItemState : EntityState<PXResult<INItemXRef, InventoryItem>>
		{
			...
			public override string Code => Value;
			protected override string StatePrompt => Basis.InventoryID != null && Basis.HasActive<LotSerialState>()
				? Basis.Localize(Msg.PromptWithLotSerialNbr, Basis.SightOf<WMSScanHeader.inventoryID>())
				: Msg.Prompt;
			...
			protected override Validation Validate(PXResult<INItemXRef, InventoryItem> entity)
			{
				...
				if(!SuppressModuleItemStatusCheck)
				{
					...
					{
						return Validation.Fail(Msg.InvalidItemStatus, inventoryItem.InventoryCD, Basis.SightOf<InventoryItem.itemStatus>(inventoryItem));
					}
			...
			protected override void Apply(PXResult<INItemXRef, InventoryItem> entity)
			{
				...
				if (Basis.Get<QtySupport>()?.IsQtySetAutomatically != true && Basis.Get<GS1Support>()?.IsWithinCompositeBarcodeProcessing != true)
			...
			protected PXResult<INItemXRef, InventoryItem> ReadItemByBarcode(string barcode, INPrimaryAlternateType? additionalAlternateType = null)
			{
				...
				if (additionalAlternateType == INPrimaryAlternateType.CPN)
					view.WhereAnd<Where<INItemXRef.alternateType.IsIn<INAlternateType.barcode, INAlternateType.gIN, INAlternateType.cPN>>>();
				else if (additionalAlternateType == INPrimaryAlternateType.VPN)
					view.WhereAnd<Where<INItemXRef.alternateType.IsIn<INAlternateType.barcode, INAlternateType.gIN, INAlternateType.vPN>>>();
				else
					view.WhereAnd<Where<INItemXRef.alternateType.IsIn<INAlternateType.barcode, INAlternateType.gIN>>>();

				var item = view
					.Select(barcode).AsEnumerable()
					.OrderByDescending(r => r.GetItem<INItemXRef>().AlternateType.IsIn<string>(INAlternateType.Barcode, INAlternateType.GIN))
			...
			private AbsenceHandling.Of<PXResult<INItemXRef, InventoryItem>> TryHandleByLotSerialNbr(string barcode, Func<string, AbsenceHandling.Of<PXResult<INItemXRef, InventoryItem>>> base_HandleAbsence)
			{
				var result = base_HandleAbsence(barcode);
				if (!result.IsHandled && Basis.InventoryID != null && Basis.TryProcessBy<LotSerialState>(barcode, StateSubstitutionRule.KeepPositiveReports | StateSubstitutionRule.KeepApplication | StateSubstitutionRule.KeepStateChange))
		...
		public class ExpireDateState : EntityState<DateTime?>
		{
			...
			protected override bool IsStateActive()
			{
				return
					...
					Basis.IsEnterableLotSerial(IsForIssue, IsForTransfer);
			}
