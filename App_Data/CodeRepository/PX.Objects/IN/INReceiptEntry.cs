...
namespace PX.Objects.IN
{
	public class INReceiptEntry : INRegisterEntryBase
	{
		...
		public INReceiptEntry()
		{
			...
			OpenPeriodAttribute.SetValidatePeriod<INRegister.finPeriodID>(this.receipt.Cache, null, PeriodValidation.DefaultSelectUpdate);
		...
		protected virtual void _(Events.RowSelected<INRegister> e)
		{
			...
			LineSplittingExt.lsselect.AllowInsert = isEditableDocument && !isTransfer;
			LineSplittingExt.lsselect.AllowUpdate = e.Row.Released == false;
			LineSplittingExt.lsselect.AllowDelete = isEditableDocument && !isTransfer;
		...
		protected virtual void _(Events.FieldUpdated<INRegister, INRegister.transferNbr> e)
		{
			...
			var splitLineNbrAttribute =
				splits.Cache.GetAttributes<INTranSplit.splitLineNbr>()
				.OfType<PXLineNbrAttribute>()
				.FirstOrDefault();

			using (new PXReadBranchRestrictedScope())
			{
				foreach (PXResult<INTransitLine, INLocationStatusInTransit, INTransitLineLotSerialStatus, INSite, InventoryItem, INTran> res in
					...
					InnerJoin<InventoryItem, On<INLocationStatusInTransit.FK.InventoryItem>,
					InnerJoin<INTran,
					...
					.Select(this, transferNbr))
				{
					...
					if (prev_linenbr != transitline.TransferLineNbr)
					{
						...
						newtran.CostCenterID = tran.ToCostCenterID;
						newtran.CostLayerType = tran.ToCostLayerType;
						newtran.SpecialOrderCostCenterID = tran.ToSpecialOrderCostCenterID;
						newtran.ProjectID = tran.ToProjectID;
						newtran.TaskID = tran.ToTaskID;
					...
					splitLineNbrAttribute?.ClearLastDefaultValue();
				}
			}
			UpdateTranCostQty(newtran, newtranqty, newtrancost);
		}
		...
		public class TransferNbrSelectorAttribute : PXSelectorAttribute
		{
			...
			public override void CacheAttached(PXCache sender)
			{
				...
				PXView view = sender.Graph.Views[_ViewName] = new PXView(sender.Graph, true, _Select, (PXSelectDelegate)delegate ()
				{
					...
					using (new PXReadBranchRestrictedScope())
					{
						res = _outerview.Select(PXView.Currents, PXView.Parameters, PXView.Searches, PXView.SortColumns, PXView.Descendings, PXView.Filters, ref startRow, 0, ref totalRows);
						PXView.StartRow = 0;
					}
