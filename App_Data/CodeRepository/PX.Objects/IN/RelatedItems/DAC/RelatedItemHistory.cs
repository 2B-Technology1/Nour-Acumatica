...
namespace PX.Objects.IN.RelatedItems
{
	...
	{
		#region Keys
		// Acuminator disable once PX1035 MultipleKeyDeclarationsWithSameFields [this key is regular, while another one is dirty]
		public class PK : PrimaryKeyOf<RelatedItemHistory>.By<lineID>
		{
			public static RelatedItemHistory Find(PXGraph graph, int? lineID, PKFindOptions options = PKFindOptions.None) => FindBy(graph, lineID, options);

			// Acuminator disable once PX1035 MultipleKeyDeclarationsWithSameFields [this key is dirty, while another one is regular]
