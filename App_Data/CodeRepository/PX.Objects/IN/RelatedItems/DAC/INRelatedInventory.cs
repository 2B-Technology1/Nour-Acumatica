...
namespace PX.Objects.IN.RelatedItems
{
	...
	/// the Non-Stock Items (IN202000) form (which corresponds to the <see cref="NonStockItemMaint"/> graph) screens.
	...
	public class INRelatedInventory: IBqlTable
	{
		...
		{
			public static INRelatedInventory Find(PXGraph graph, int? inventoryID, int? lineID, PKFindOptions options = PKFindOptions.None) 
				=> FindBy(graph, inventoryID, lineID, options);
		}
