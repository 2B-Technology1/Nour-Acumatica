	...
	public class INPICountEntry : PXGraph<INPICountEntry>, PXImportAttribute.IPXPrepareItems, PXImportAttribute.IPXProcess
	{
		...
		protected virtual void INBarCodeItem_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (row != null && row.AutoAddLine == true && this.AddByBarCode.VerifyRequired(true) && row.Qty > 0)
				// Acuminator disable once PX1043 SavingChangesInEventHandlers [Live update]
				// Acuminator disable once PX1045 PXGraphCreateInstanceInEventHandlers [Live update]
				// Acuminator disable once PX1071 PXActionExecutionInEventHandlers [Live update]
				UpdatePhysicalQty();
		}
		...
		private void UpdatePhysicalQty()
		{
			...
			INPIDetail detail =
				PXSelectReadonly<INPIDetail,
				Where<
					INPIDetail.pIID, Equal<Current<INPIHeader.pIID>>,
					And<INPIDetail.inventoryID, Equal<Current<INBarCodeItem.inventoryID>>,
					...
					And<Where<INPIDetail.lotSerialNbr, IsNull,
						Or<INPIDetail.lotSerialNbr, Equal<Current<INBarCodeItem.lotSerialNbr>>>>>>>>>>
				.SelectWindowed(this, 0, 1);

			detail = UpdatePhysicalQty(detail, (PIHeader.Current.PIID, item.LocationID, item.InventoryID, item.SubItemID, item.LotSerialNbr, item.ExpireDate, item.BaseQty));
		...
		protected INPIDetail UpdatePhysicalQty(INPIDetail detail, (string PIID, int? LocationID, int? InventoryID, int? SubItemID, string LotSerialNbr, DateTime? ExpireDate, decimal? DeltaBaseQty) item)
		{
			...
			if (detail == null)
			{
				...
				entry.PIHeader.Current = entry.PIHeader.Search<INPIHeader.pIID>(item.PIID);
				...
				var detailId = entry.PIDetail.Cache.GetValue<INPIDetail.pIID>(detail);
				var lineNbr = entry.PIDetail.Cache.GetValue<INPIDetail.lineNbr>(detail);

				PIHeader.View.RequestRefresh();
				PIDetail.Cache.Locate(new Dictionary<string, object>()
				{
					[nameof(INPIDetail.PIID)] = detailId,
					[nameof(INPIDetail.LineNbr)] = lineNbr
				});
			}
			else
			{
				detail = PXCache<INPIDetail>.CreateCopy(detail);
				detail.PhysicalQty = detail.PhysicalQty.GetValueOrDefault() + item.DeltaBaseQty.GetValueOrDefault();
		...
		protected virtual void INPIDetail_RowUpdating(PXCache sender, PXRowUpdatingEventArgs e)
		{
			...
			{
				// Acuminator disable once PX1043 SavingChangesInEventHandlers [Live updates, the default update is suppressed]
				if (!PXDatabase.Update<INPIDetail>(
					...
					new PXDataFieldRestrict(typeof(INPIDetail.Tstamp).Name, PXDbType.Timestamp, 8, d.tstamp, PXComp.LE)))
				{
			...
			PIHeader.Current.TotalPhysicalQty += (d.PhysicalQty ?? 0) - (o.PhysicalQty ?? 0);
			PIHeader.Current.TotalVarQty += (d.VarQty ?? 0) - (o.VarQty ?? 0);
			PIHeader.Current.tstamp = d.tstamp = PXDatabase.SelectTimeStamp();
		}
