...
namespace PX.Objects.IN
{
    ...
    public partial class InventoryTranDetEnqFilter : PX.Data.IBqlTable
    {
        ...
        [PXDefault()]
        [AnyInventory(typeof(Search<InventoryItem.inventoryID, Where2<Where<InventoryItem.stkItem, NotEqual<boolFalse>, Or<InventoryItem.isConverted, Equal<True>>>, And<Where<Match<Current<AccessInfo.userName>>>>>>), typeof(InventoryItem.inventoryCD), typeof(InventoryItem.descr))] // ??? zzz stock / nonstock ?
    ...
    public partial class InventoryTranDetEnqResult : PX.Data.IBqlTable
    {
		...
		[PO.POReceiptType.List]
		[PXUIField(DisplayName = "PO Receipt Type", Visibility = PXUIVisibility.Visible, Visible = false)]
    ...
    public class InventoryTranDetEnq : PXGraph<InventoryTranDetEnq>
    {
		...
		[PXSelector(typeof(Search<PO.POReceipt.receiptNbr, Where<PO.POReceipt.receiptType.IsEqual<InventoryTranDetEnqResult.pOReceiptType.FromCurrent>>>))]
		...
		protected virtual IEnumerable resultRecords()
        {
            ...
            {
				return new List<PXResult<InventoryTranDetEnqResult, INTran>>();//empty
            }
			...
			foreach (PXResult<InventoryTranDetEnqResult, INTran> item in list)
			{
		...
		protected virtual IEnumerable internalResultRecords()
        {
			...
			PXSelectBase<INTran> cmdBeginning = new PXSelectJoinGroupBy<INTran,
					...
					, Where<INTran.inventoryID, Equal<Current<InventoryTranDetEnqFilter.inventoryID>>, And2<Match<INSite, Current<AccessInfo.userName>>,
						And<Where<INTran.isStockItem, IsNull, Or<INTran.isStockItem, Equal<True>>>>>>
				, Aggregate<
			...
			PXSelectBase<INTran> cmd = new PXSelectReadonly2<INTran,
                    ...
                    InnerJoin<INSite, On<INTran.FK.Site>>>>,
					Where<INTran.inventoryID, Equal<Current<InventoryTranDetEnqFilter.inventoryID>>, And2<Match<INSite, Current<AccessInfo.userName>>,
						And<Where<INTran.isStockItem, IsNull, Or<INTran.isStockItem, Equal<True>>>>>>>(this);
