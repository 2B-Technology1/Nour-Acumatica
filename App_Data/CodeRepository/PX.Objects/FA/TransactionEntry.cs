...
namespace PX.Objects.FA
{
	...
	public class TransactionEntry : PXGraph<TransactionEntry, FARegister>
	{
		...
		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXRestrictor(typeof(Where<FixedAsset.status, NotEqual<FixedAssetStatus.disposed>,
			And <FixedAsset.status, NotEqual<FixedAssetStatus.reversed>>>),
			Messages.CannotUseDisabledOrReversedFixedAsset)]
		protected virtual void _(Events.CacheAttached<FATran.assetID> e) { }
		#endregion
		...
		public TransactionEntry()
		{
			...
			_created = new BatchDocumentList<FARegister, FARegister.lineCntr>(this);
		}
		#endregion
		...
		public static void SegregateRegister(PXGraph graph, int BranchID, string Origin, string PeriodID, DateTime? DocDate, string descr, DocumentList<FARegister> created)
		{
			...
			if (register.RefNbr != null && (WebConfig.ParallelProcessingDisabled || register.LineCntr <= 500))
			{
				FARegister newreg = register;
		...
		protected virtual void FATran_RowInserting(PXCache sender, PXRowInsertingEventArgs e)
		{
			...
			e.Cancel |= tran.TranAmt == 0m && (!string.IsNullOrEmpty(tran.MethodDesc) || tran.TranType != "C+" && tran.TranType != "P+" && tran.TranType != "P-" && tran.TranType != "S+" && tran.TranType != "S-" && tran.Origin != FARegister.origin.Adjustment);
		...
		protected virtual void FATran_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if (tran != null
				...
				&& tran.Origin != FARegister.origin.DisposalReversal
		...
		{
			FATran tran = (FATran) e.Row;
			if(tran?.AssetID == null || tran.BookID == null || tran.TranDate == null || e.Cancel == true) return;
