...
namespace PX.Objects.FA
{
	public class GenerationPeriods : PXGraph<GenerationPeriods>
	{
		...
		protected virtual void BoundaryYears_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			int earliestYear = GL.FinPeriods.FinPeriodUtils.EarliestYear;
			int fromYear = int.Parse(filter.FromYear);
			int toYear = int.Parse(filter.ToYear);

			sender.RaiseExceptionHandling<BoundaryYears.fromYear>(filter, filter.FromYear,
				fromYear < earliestYear
					? new PXSetPropertyException(
							CS.Messages.Entry_GT,
							PXErrorLevel.Error,
							earliestYear - 1)
					: null);

			PXSetPropertyException toYearException = null;
			if (toYear < earliestYear)
			{
				toYearException = new PXSetPropertyException(
					CS.Messages.Entry_GT,
					PXErrorLevel.Error,
					earliestYear - 1);
			}
			...
			{
				toYearException = new PXSetPropertyException(
					Messages.ToYearMustNotBeEarlierThanFromYear,
					PXErrorLevel.Error);
			}
			sender.RaiseExceptionHandling<BoundaryYears.toYear>(filter, filter.ToYear, toYearException);

			Books.SetProcessAllEnabled(fromYear >= earliestYear && toYear >= earliestYear && toYear >= fromYear);
			Books.SetProcessEnabled(fromYear >= earliestYear && toYear >= earliestYear && toYear >= fromYear);
		...
		protected virtual FABookYear GenerateSingleOrganizationFABookYear(FAOrganizationBook book, FABookYear startMasterYear, FABookPeriod startMasterPeriod)
		{
			...
			bool hasAdjustmentPeriod = (yearSetup.HasAdjustmentPeriod == true && yearSetup.FPType != FiscalPeriodSetupCreator.FPType.Custom)
				|| (yearSetup.FPType == FiscalPeriodSetupCreator.FPType.Custom
					&& FABookPeriodRepository.FindFABookPeriodSetup(book).Where(periodSetup => periodSetup.StartDate == periodSetup.EndDate).Any());

			if (hasAdjustmentPeriod)
			{
				periodsCountForCopy--;
			}
	...
	{
		public new abstract class selected : BqlBool.Field<selected> { }
		public new abstract class bookID : BqlInt.Field<bookID> { }
		public new abstract class updateGL : BqlBool.Field<updateGL> { }


		#region BookCode
		public new abstract class bookCode : BqlString.Field<bookCode> { }
		/// <summary>
		...
		public abstract class rawOrganizationID : BqlInt.Field<rawOrganizationID> { }
		...
		public abstract class organizationCD : BqlString.Field<organizationCD> { }
		...
		public abstract class firstCalendarYear : BqlString.Field<firstCalendarYear> { }
		/// <summary>
		...
		*/
		...
		public abstract class lastCalendarYear : BqlString.Field<lastCalendarYear> { }
		/// <summary>
