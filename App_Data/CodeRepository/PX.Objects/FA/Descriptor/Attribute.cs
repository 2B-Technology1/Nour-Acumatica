...
namespace PX.Objects.FA
{
	...
	public class FABookPeriodIDAttribute : OrganizationDependedPeriodIDAttribute
	{
		...
		protected override PeriodResult GetPeriod(PXGraph graph, Type searchType, DateTime? date, OrganizationDependedPeriodKey periodKey, List<object> currents = null)
		{
			...
			List<object> queryCurrents = new List<object>() { new QueryParams { SourceDate = date } };
			...
			List<object> source = view.Select(
				...
				periodKey.ToListOfObjects(true).ToArray(),
				null,
				null,
				null,
				null,
			...
			if (source != null && source.Count > 0)
			{
				...
				return new PeriodResult(item as IPeriod);
			}
			else return new PeriodResult(null);
		}
	}
	#endregion
