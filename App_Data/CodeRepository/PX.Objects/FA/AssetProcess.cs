...
namespace PX.Objects.FA
{
	...
	public class AssetTranRelease : PXGraph<AssetTranRelease>
	{
		...
		public AssetTranRelease()
		{
			...
			FADocumentList.ParallelProcessingOptions = settings =>
			{
		...
		public static DocumentList<Batch> ReleaseDoc(List<FARegister> list, bool isMassProcess, bool AutoPost)
		{
			...
			DocumentList<Batch> batchlist = new BatchDocumentList<Batch, Batch.lineCntr>(rg);
	...
	public class AssetProcess : PXGraph<AssetProcess>
	{
		...
		public static void SuspendBalanceForPeriods(TransactionEntry graph, FABookBalance bookbal, int Periods)
		{
			...
			copy.NoteID = Guid.NewGuid(); //because copy is updating later
		...
		public static void CloseBalanceHistoryForPeriods(TransactionEntry graph, FABookBalance bookbal, int periodsCount)
		{
			if (periodsCount < 1)
			{
				return;
			...
			hist = new FABookHist
			{
				AssetID = bookbal.AssetID,
				BookID = bookbal.BookID,
				FinPeriodID = fABookPeriodUtils.PeriodPlusPeriodsCount(bookbal.CurrDeprPeriod, periodsCount, bookbal.BookID, bookbal.AssetID)
			};
			hist = FAHelper.InsertFABookHist(graph, hist, ref bookbal);
		}
		...
		public static void CloseAssetHistoryToPeriod(IEnumerable<FABookBalance> balances, DateTime? Date, string PeriodID)
		{
			TransactionEntry docgraph = CreateInstance<TransactionEntry>();
			...
			foreach (FABookBalance bookbal in balances)
			{
				...
				string BookPeriodID = bookbal.UpdateGL == true ? PeriodID : docgraph.GetService<IFABookPeriodRepository>().GetFABookPeriodIDOfDate(Date, bookbal.BookID, bookbal.AssetID);
				int periodsCount = docgraph.GetService<IFABookPeriodUtils>().PeriodMinusPeriod(BookPeriodID, bookbal.CurrDeprPeriod ?? BookPeriodID, bookbal.BookID, bookbal.AssetID) ?? 0;
				CloseBalanceHistoryForPeriods(docgraph, bookbal, periodsCount);
			}
		...
		public static void CheckAssetTransferInformation(PXGraph graph, FixedAsset asset, FADetails det, string transferPeriodId = null)
		{
			if (det == null || asset == null) return;

			if (det.Status == FixedAssetStatus.Hold || det.Status == FixedAssetStatus.Suspended)
			{
				throw new PXException(Messages.InvalidAssetStatusInTransferProcess);
			}

			if (asset.UnderConstruction == true ||  asset.Depreciable != true || transferPeriodId == null) return;
			...
			if (postingBookBalance != null && postingBookBalance.DeprToPeriod != null
				&& postingBookBalance.Status != FixedAssetStatus.FullyDepreciated
				&& postingBookBalance.Status == FixedAssetStatus.Active
				&& string.Compare(postingBookBalance.DeprToPeriod, transferPeriodId) < 0)
			{
				throw new PXException(Messages.TransferPeriodCannotBeGreaterThanDeprToPeriodInTransferProcess);
			}
		}
		...
		public static void ThrowDisabled_Transfer(PXGraph graph, TransferProcess.TransferFilter filter, FixedAsset asset, FADetailsTransfer det)
		{
			...
			CheckAssetTransferInformation(graph, asset, det, filter.PeriodID);
		...
		public static List<FABookBalance> PrepareDisposal(TransactionEntry docgraph, FixedAsset asset, FADetails assetdet, bool IsMassProcess, bool deprBeforeDisposal)
		{
			...
			foreach (FABookBalance bookbal in bookbalSelect.Select(assetdet.AssetID))
			{
				if (bookbal.Depreciate != true || !deprBeforeDisposal)
				{
					...
					{
						CloseAssetHistoryToPeriod(new List<FABookBalance>() { bookbal }, assetdet.DisposalDate, disposalPeriod);
					}
				}
			...
			books.ForEach(docgraph.EnsureRowPersistence); // (AC-211595) so we must persist restored DeprTo values
			docgraph.Actions.PressSave();
		...
		public static void AcquireAsset(TransactionEntry docgraph, int BranchID, IDictionary<FABookBalance, OperableTran> booktrn, FARegister register)
		{
			if (register != null)
			{
				...
				if (register.BranchID != BranchID)
				{
					...
					register = docgraph.Document.Update(registerCopy);
				}
			}
			else
			{
							...
							if (!string.IsNullOrEmpty(book.Key.LastDeprPeriod))
							{
								...
								if (string.CompareOrdinal(book.Key.LastDeprPeriod, book.Key.DeprToPeriod) < 0)
								{
									...
									if (!IsFullyDepreciatedAsset(docgraph, bookBal.AssetID))
									{
										...
										FixedAsset.Events
											.Select(ev => ev.NotFullyDepreciateAsset)
											.FireOn(docgraph, asset);
									}
								}
		...
		public static bool CalculateAsset(IEnumerable<FABookBalance> books, string maxPeriodID)
		{
			...
			{
				calc.Clear();
				calc.SelectTimeStamp();
				PXProcessing<FABookBalance>.SetCurrentItem(bookbal);
				calc.Caches<FABookBalance>().SetStatus(bookbal, PXEntryStatus.Notchanged);
				try
				{
		...
		public static bool DepreciateAsset(IEnumerable<FABookBalance> books, DateTime? DateTo, string PeriodTo, bool IsMassProcess, bool IncludeLastPeriod)
		{
			...
			docgraph.FieldDefaulting.AddHandler<FATran.finPeriodID>((cache, e) => e.Cancel = true);
			docgraph.FieldVerifying.AddHandler<FATran.finPeriodID>((cache, e) => e.Cancel = true);
			docgraph.Trans.Cache.Adjust<FABookPeriodSelectorAttribute>()
				.For<FATran.finPeriodID>(atr => atr.RedefaultOnDateChanged = false);
			...
			foreach (FABookBalance item in books)
			{
				...
				try
				{
					...
					{
						var result 
							= (PXResult<FABookBalance,FixedAsset,FAClass,FADetails,FADepreciationMethod,Branch,OrganizationFinPeriod>)
							PXSelectJoin<FABookBalance, 
								InnerJoin<FixedAsset, On<FixedAsset.assetID, Equal<FABookBalance.assetID>>,
								InnerJoin<FAClass, On<FAClass.classID, Equal<FixedAsset.classID>>,	
								InnerJoin<FADetails, 
						...
						PXSelect<FAClass, 
								Where<FAClass.assetID, Equal<Required<FixedAsset.assetID>>>>
							.StoreResult(calc, new List<object>{assetClass},
								PXQueryParameters.ExplicitParameters(cache.ClassID));
						
						PXCache<FixedAsset>.StoreOriginal(calc, cache);	
						PXCache<FADetails>.StoreOriginal(calc, detail);	
						PXCache<FABookBalance>.StoreOriginal(calc, bookbal);	
						...
						foreach (PXResult<FABookHistory, FixedAsset, Branch, FABook, FABookPeriod, FADetails> res in PXSelectJoin<
							...
							LeftJoin<FABookPeriod,
								...
								And<FABookPeriod.organizationID, 
									Equal<IIf<Where<FABook.updateGL, Equal<True>>, Branch.organizationID, FinPeriod.organizationID.masterValue>>,
							...
							.SelectMultiBound(docgraph, new object[] { bookbal }, bookbal.CurrDeprPeriod, BookPeriodTo))
						{
							...
							if (period.BookID == null) continue;
							...
							PXSelect<FixedAsset, 
									Where<FixedAsset.assetID, Equal<Required<FixedAsset.assetID>>>>
								.StoreResult(docgraph, asset);
							// 596019361	
							PXSelect<FixedAsset, 
									Where<FixedAsset.assetID, Equal<Current<FATran.assetID>>>>
								.StoreResult(docgraph, asset);
						
							PXSelectorAttribute.StoreCached<FABookBalance.assetID>(docgraph.bookbalances.Cache, bookbal, asset );
		...
		public static void SetLastDeprPeriod(PXSelectBase<FABookBalance> bookBalances, FABookBalance bookBal, string LastDeprPeriod)
		{
			...
			LastDeprPeriod = bookBal.DeprToPeriod != null && string.CompareOrdinal(LastDeprPeriod, bookBal.DeprToPeriod) > 0
				? bookBal.DeprToPeriod
				: LastDeprPeriod;
			...
			if (string.CompareOrdinal(bookBal.LastDeprPeriod, LastDeprPeriod) < 0)
			{
				...
				bookBal.CurrDeprPeriod = bookBal.DeprToPeriod == null || string.CompareOrdinal(bookBal.LastDeprPeriod, bookBal.DeprToPeriod) < 0 
		...
		public virtual void DataChecking(FARegister doc)
		{
			...
			foreach (PXResult<FATran, FABook, Branch, FABookPeriod> tranPeriod in
				SelectFrom<FATran>
					...
					.AggregateTo<GroupBy<FABookPeriod.finPeriodID>>
					.View.Select(this, doc.RefNbr))
			{
				...
				.View
				.Select<PXResultset<FABookPeriod, FinPeriod>>(this, period.FinPeriodID, period.OrganizationID, period.BookID);

				FABookPeriod faPeriod = synchronizedPeriod;
				FinPeriod finPeriod = synchronizedPeriod;

				PXException exception = new PXException(Messages.PostingBookNotMatchFinPeriodInGL, FinPeriodIDAttribute.FormatForError(period.FinPeriodID), PXAccess.GetOrganizationCD(period.OrganizationID));
				...
				{
					FinPeriod firstFinPeriod = FinPeriodRepository.FindFirstPeriod(period.OrganizationID);
																																	...
																																	And<FATran.released, Equal<False>>>>.Select(this, doc.RefNbr))
				{
					...
					switch (doc.Origin)
					{
						...
						case FARegister.origin.DisposalReversal:
							...
							string nondepreciableCurrDeprPeriod = bookbal.DeprFromPeriod;
							string nondepreciableLastDeprPeriod = null;
							if (bookbal.Depreciate != true)
							{
								FABookHistory lastClosedHistory = SelectFrom<FABookHistory>
									.Where<FABookHistory.assetID.IsEqual<FABookBalance.assetID.FromCurrent>
										.And<FABookHistory.bookID.IsEqual<FABookBalance.bookID.FromCurrent>>
										.And<FABookHistory.closed.IsEqual<True>>>
									.OrderBy<Desc<FABookHistory.finPeriodID>>
									.View
									.SelectSingleBound(this, new object[] { bookbal});
								if(lastClosedHistory != null)
								{
									nondepreciableLastDeprPeriod = lastClosedHistory.FinPeriodID;
									nondepreciableCurrDeprPeriod = FABookPeriodUtils.PeriodPlusPeriodsCount(nondepreciableLastDeprPeriod, 1, bookbal.BookID, bookbal.AssetID);
								}
							}


							bookbal.CurrDeprPeriod = bookbal.Depreciate != true 
														? nondepreciableCurrDeprPeriod
														: isFullyDepreciated 
															? null 
						...
						case FARegister.origin.Transfer:
							bookbal = (FABookBalance)bookbalance.Cache.Locate(bookbal) ?? bookbal;
							if (asset.Status != FixedAssetStatus.FullyDepreciated &&  string.IsNullOrEmpty(bookbal.CurrDeprPeriod))
							{
		...
		private static void SegregateBatch(JournalEntry je, int? BranchID, DateTime? DocDate, string FinPeriodID, string descr, DocumentList<Batch> created)
		{
			...
			if (fabatch != null && (WebConfig.ParallelProcessingDisabled || fabatch.LineCntr < 500))
			{
			...
			else
			{
				...
				OpenPeriodAttribute.SetValidatePeriod<Batch.finPeriodID>(je.BatchModule.Cache, fabatch, PeriodValidation.DefaultUpdate);
				fabatch = je.BatchModule.Insert(fabatch);
				OpenPeriodAttribute.SetValidatePeriod<Batch.finPeriodID>(je.BatchModule.Cache, fabatch, PeriodValidation.DefaultSelectUpdate);
		...
		public static void TransferAsset(PXGraph graph, FixedAsset asset, FALocationHistory location, ref FARegister register)
		{
					...
					.Select(graph, asset.AssetID)).Count > 0)
			{
				...
				{
					CheckAssetTransferInformation(graph, asset, detailsCache.Current as FADetails);
		...
		protected static FAAccounts GetNewAccounts(PXGraph graph, PXCache assetCache, PXCache locationsCache, FASetup fasetup, FixedAsset asset, FALocationHistory location)
		{
			...
			asset.UnderConstruction = prevClass?.UnderConstruction ?? false;
	...
	public static class FAHelper
	{
			...
			ref FABookBalance bookBalance)
		{
					...
					|| string.Compare(keyedHistory.FinPeriodID, bookBalance.MaxHistoryPeriodID) > 0))
			{
				...
				//Former EnsureRowPersistence
				graph.EnsureCachePersistence(bookBalance.GetType());
				graph.Caches<FABookBalance>().MarkUpdated(bookBalance, false);
			}
			return result;
		}
