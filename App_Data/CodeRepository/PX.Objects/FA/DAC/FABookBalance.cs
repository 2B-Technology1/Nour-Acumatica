...
namespace PX.Objects.FA
{
	...
	[PXProjection(typeof(SelectFrom<FABookBalance>
		...
		.LeftJoin<FABookHistory>
			...
			.And<FABookHistory.finPeriodID.IsEqual<FABookBalance.maxHistoryPeriodID.When<FixedAsset.underConstruction.IsEqual<True>>
				.Else<FABookBalance.currDeprPeriod.IfNullThen<FABookBalance.lastPeriod>>>>
			>>
		.InnerJoin<FABook>
			.On<FABook.bookID.IsEqual<FABookBalance.bookID>>), new Type[] { typeof(FABookBalance) })]
	...
	public partial class FABookBalance : PX.Data.IBqlTable
	{
		...
		{
			public static FABookBalance Find(PXGraph graph, int? assetID, int? bookID, PKFindOptions options = PKFindOptions.None) => FindBy(graph, assetID, bookID, options);
		}
		...
		/// <inheritdoc cref="FABook.BookCode"/>
		[PXDBString(BqlField = typeof(FABook.bookCode))]
		[PXDefault("", typeof(Search<FABook.bookCode, Where<FABook.bookID, Equal<Current<FABookBalance.bookID>>>>), PersistingCheck = PXPersistingCheck.Nothing)]
		...
		[PXForeignReference(typeof(Field<bonusID>.IsRelatedTo<FABonus.bonusID>))]
		...
		/// <summary>
		/// Identifier of the <see cref="PX.Data.Note">Note</see> object, associated with the book balance.
		/// </summary>
		/// <value>
