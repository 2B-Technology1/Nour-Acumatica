	...
	public class CABankTransactionsEnq : PXGraph<CABankTransactionsEnq>
    {
		...
		public PXSelectJoin<CABankTran, LeftJoin<CABankTranMatch,
					 On<CABankTranMatch.tranID, Equal<CABankTran.tranID>,
						...
						LeftJoin<CABatch, On<CABatch.batchNbr, Equal<CABankTranMatch.docRefNbr>,
		...
		protected virtual IEnumerable result()
		{
			...
			foreach (PXResult<CABankTran, CABankTranMatch, CATran, CABatch> res in Views[nameof(CATran_History)].Select(PXView.Currents,null, mapper.Searches,mapper.SortColumns,mapper.Descendings, mapper.Filters, ref startRow, PXView.MaximumRows, ref totalRows))
			{
				...
				if(match.DocType == CATranType.CABatch && cABatch.CuryDetailTotal != null)
				{
					UpdateCATranWithCABatchInfo(catran, cABatch.CuryDetailTotal);
				}
		...
		{
			catran.DrCr = CADrCr.CACredit;
			catran.CuryTranAmt = -curyDetailTotal;
		}
