...
namespace PX.Objects.CA
{
	public class EntryTypeMaint : PXGraph<EntryTypeMaint, CAEntryType>
	{
		...
		protected virtual void CAEntryType_RowDeleting(PXCache sender, PXRowDeletingEventArgs e)
		{
			try
			{
				PXSelectorAttribute.CheckAndRaiseForeignKeyException(sender, e.Row, typeof(CABankTranRule.documentEntryTypeID), customMessage: Messages.EntryTypeCannotBeDeletedBecauseItsUsedByBankTransactionRules);
			}
			catch(PXException ex)
			{
				var row = e.Row as CAEntryType;

				sender.RaiseExceptionHandling<CAEntryType.entryTypeId>(e.Row, row.EntryTypeId, new PXSetPropertyException(ex.MessageNoPrefix, PXErrorLevel.Error));
				throw;
			}
		}
