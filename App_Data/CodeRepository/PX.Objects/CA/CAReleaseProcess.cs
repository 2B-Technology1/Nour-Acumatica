...
namespace PX.Objects.CA
{
	...
	public class CAReleaseProcess : PXGraph<CAReleaseProcess>
	{
		...
		public PXSelectJoin<CATran,
				...
				OrderBy<Asc<CATran.origRefNbr, Desc<CATran.origTranType>>>> CATranFundsTransfers_Ordered;
		...
		protected virtual void _(Events.CacheAttached<CATran.origTranType> e) { }
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				foreach (PXResult<CATran, CashAccount, Currency, CurrencyInfo> res in resultset)
				{
					...
					if (isTransfer && branchID == transferBatch?.BranchID &&
						ShouldNotSegregate(catran, transferTran))
					{
					...
					if (object.Equals(prev_tran, catran) == false)
					{
						...
						foreach (PXResult<CATaxTran, Tax> r in CATaxTran_TranType_RefNbr.Select(caadj.AdjTranType, caadj.AdjRefNbr))
						{
							...
							FinPeriodIDAttribute.SetPeriodsByMaster<CATaxTran.finPeriodID>(CATaxTran_TranType_RefNbr.Cache, x, caadj.TranPeriodID);
								...
								(x.TaxType == TaxType.PendingPurchase || x.TaxType == TaxType.PendingSales))
							{
								...
								FinPeriodIDAttribute.SetPeriodsByMaster<SVATConversionHist.adjdFinPeriodID>(
													SVATConversionHistory.Cache, histSVAT, caadj.TranPeriodID);
					...
					foreach (var casplit in splits)
					{
						...
						{
							AdjustTaxCalculationLevelForNetGrossEntryModeFeature(caadj, ref firstTax);
						}
		...
		{
			Account caAcc = AccountAttribute.GetAccount(this, cashacct.AccountID);
		...
		protected virtual void AdjustTaxCalculationLevelForNetGrossEntryModeFeature(CAAdj document, ref Tax taxCorrespondingToLine)
			=> AdjustTaxCalculationLevelForNetGrossEntryMode(document, ref taxCorrespondingToLine);
			...
			where TCADocument : class, ICADocument, new()
		{
			...
			foreach (PXResult<CAExpense> result in CAExpense_RefNbr.Select(doc.RefNbr))
			{
				...
				if (ShouldNotSegregate(transferBatchOut.BranchID, transferTranOut, charge))
				{
					je.BatchModule.Current = transferBatchOut;
					shouldSegregateWithOut = false;
				}

				if (ShouldNotSegregate(transferBatchIn.BranchID, transferTranIn, charge))
				{
		...
		protected virtual void GenerateGLTranPairForExpense(JournalEntry je, CAExpense charge)
		{
			...
			VerifyCashAccountInGL(expCashcacount);
		...
		{
			var voidType = ARPaymentType.GetVoidingARDocType(detail.OrigDocType);

			var exist = PXSelect<ARRegister,
						   ...
						   >>>>.Select(this, voidType, detail.OrigRefNbr).Any();

			if(exist)
			{
				throw new PXException(Messages.VoidedDepositCannotBeReleasedDueToIncludedPaymentHasUnreleasedVoidingEntry);
			}
		}
		...
		public virtual void ReleaseDeposit(JournalEntry je, ref List<Batch> batchlist, CADeposit doc)
		{
			...
			foreach (PXResult<CATran, CashAccount, Currency, CurrencyInfo, CADepositDetail, CADepositEntry.ARPaymentUpdate, CADepositEntry.APPaymentUpdate, CADepositEntry.CAAdjUpdate, CCBatch> res in resultset)
			{
				...
				{
					CheckIfUnreleasedVoidedEntryExist(detail);

					if (IsVoidedEntryWithoutPair(detail))
				{
				...
				if (object.Equals(prev_tran, catran) == false)
				{
					...
					if (!String.IsNullOrEmpty(detail.OrigRefNbr))
					{
						...
						if (rgol != Decimal.Zero)
						{
							...
							if (!rgols.ContainsKey(detail.CashAccountID.Value))
							{
								...
								rgols[detail.CashAccountID.Value] = rgol_tran;
							}
							else
							{
								rgol_tran = rgols[detail.CashAccountID.Value];
							}
		...
		protected void CheckMultipleGLPosting(HashSet<long> caTranIDs)
		{
			foreach (long id in caTranIDs)
			{
				...
				ValidateIfMappedGLAccountIsCashAccount(id, count);
		...
		private void ValidateIfMappedGLAccountIsCashAccount(long id, int count)
		{
			...
			using (new PXReadBranchRestrictedScope())
			{
				...
				Account account = PXSelectReadonly2<Account,
								InnerJoin<CashAccount, On<Account.accountCD, Equal<CashAccount.accountID>>,
								InnerJoin<CATran, On<CashAccount.cashAccountCD, Equal<CATran.cashAccountID>>>>,
								Where<CATran.tranID, Equal<Required<CATran.tranID>>,
									And<Account.isCashAccount, Equal<boolFalse>>>>.Select(this, id);

				if ((account?.IsCashAccount ?? true) != true)
				{
