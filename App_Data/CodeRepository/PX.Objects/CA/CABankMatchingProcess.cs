...
namespace PX.Objects.CA
{
	public class CABankMatchingProcess : PXGraph<CABankMatchingProcess>, ICABankTransactionsDataProvider
	{
		...
		public static CABankTranInvoiceMatch CreateCABankTranInvoiceMatch(Light.ARInvoice doc, Light.BAccount bAccount, CABankTran aDetail)
		{
			CABankTranInvoiceMatch tranMatch = new CABankTranInvoiceMatch();
			tranMatch.Copy(doc);

			tranMatch.ReferenceName = bAccount.AcctName;

			if (tranMatch.DrCr != aDetail.DrCr)
			{
				tranMatch.CuryTranAmt = -1 * tranMatch.CuryTranAmt;
				tranMatch.TranAmt = -1 * tranMatch.TranAmt;
				tranMatch.CuryDiscAmt = -1 * tranMatch.CuryDiscAmt;
				tranMatch.DiscAmt = -1 * tranMatch.DiscAmt;
			}

			return tranMatch;
		}

		public static CABankTranInvoiceMatch CreateCABankTranInvoiceMatch(Light.APInvoice doc, Light.BAccount bAccount, CABankTran aDetail)
		{
			CABankTranInvoiceMatch tranMatch = new CABankTranInvoiceMatch();
			tranMatch.Copy(doc);
		...
		public static List<CABankTranInvoiceMatch> FindDetailMatchingInvoicesProc(PXGraph graph, CABankTran aDetail, CashAccount cashAccount, decimal aRelevanceTreshold, CABankTranInvoiceMatch[] aBestMatches)
		{
				...
				amount > 0)
			{
					...
					sel.Select(bqlParams.ToArray()))
				{
					...
					CABankTranInvoiceMatch tran = CreateCABankTranInvoiceMatch(iDoc, iCustomer, aDetail);
				...
				PXSelectBase<CABankTranMatch> selMatched = new PXSelectJoin<CABankTranMatch,
					LeftJoin<Light.ARInvoice, On<CABankTranMatch.docModule, Equal<GL.BatchModule.moduleAR>,
						And<CABankTranMatch.docType, Equal<Light.ARInvoice.docType>,
						And<CABankTranMatch.docRefNbr, Equal<Light.ARInvoice.refNbr>>>>,
					LeftJoin<Light.BAccount, On<Light.BAccount.bAccountID, Equal<Light.ARInvoice.customerID>>>>,
					Where<CABankTranMatch.tranID, Equal<Required<CABankTranMatch.tranID>>>>(graph);

				foreach (PXResult<CABankTranMatch, Light.ARInvoice, Light.BAccount> it in selMatched.Select(aDetail.TranID))
				{
					Light.ARInvoice iDoc = it;
					if (!string.IsNullOrEmpty(iDoc?.RefNbr) && !result.Any(r => r.OrigModule.Equals(BatchModule.AR)
										&& r.OrigTranType.Equals(iDoc.DocType) && r.OrigRefNbr.Equals(iDoc.RefNbr)))
					{
						Light.BAccount iCustomer = it;
						CABankTranMatch iMatch = it;
						CABankTranInvoiceMatch tran = CreateCABankTranInvoiceMatch(iDoc, iCustomer, aDetail);
			...
			if (aDetail.DrCr == CA.CADrCr.CACredit && !clearingAccount)
			{
					...
					sel.Select(bqlParams.ToArray()))
				{
					...
					CABankTranMatch iMatch = it;
					Light.CABankTranAdjustment iAdj = it;
					CABankTranInvoiceMatch tran = CreateCABankTranInvoiceMatch(iDoc, iPayee, aDetail);
				...
				PXSelectBase<CABankTranMatch> selMatched = new PXSelectJoin<CABankTranMatch,
					LeftJoin<Light.APInvoice, On<CABankTranMatch.docModule, Equal<GL.BatchModule.moduleAP>,
						And<CABankTranMatch.docType, Equal<Light.APInvoice.docType>,
						And<CABankTranMatch.docRefNbr, Equal<Light.APInvoice.refNbr>>>>,
					LeftJoin<Light.BAccount, On<Light.BAccount.bAccountID, Equal<Light.APInvoice.vendorID>>>>,
					Where<CABankTranMatch.tranID, Equal<Required<CABankTranMatch.tranID>>>>(graph);

				foreach (PXResult<CABankTranMatch, Light.APInvoice, Light.BAccount> it in selMatched.Select(aDetail.TranID))
				{
					Light.APInvoice iDoc = it;
					if (!string.IsNullOrEmpty(iDoc?.RefNbr) && !result.Any(r => r.OrigModule.Equals(BatchModule.AP)
										&& r.OrigTranType.Equals(iDoc.DocType) && r.OrigRefNbr.Equals(iDoc.RefNbr)))
					{
						Light.BAccount iPayee = it;
						CABankTranMatch iMatch = it;
		...
		protected static PXSelectBase<Light.ARInvoice> CreateARInvoiceQuery(PXGraph graph, CABankTran aDetail, CashAccount cashAccount, decimal? tranAmount, out List<object> bqlParams)
		{
			...
			if (PXAccess.FeatureInstalled<FeaturesSet.interBranch>() == false)
			{
				...
				var organizationID = PXAccess.GetParentOrganizationID(cashAcc.BranchID);
				int[] childBranches = PXAccess.GetChildBranchIDs(organizationID);
				sel.WhereAnd<Where<Light.ARInvoice.branchID, In<Required<Light.ARInvoice.branchID>>>>();
				bqlParams.Add(childBranches);
			}
		...
		protected static PXSelectBase<Light.APInvoice> CreateAPInvoiceQuery(PXGraph graph, CABankTran aDetail, CashAccount cashAccount, decimal? tranAmount, out List<object> bqlParams)
		{
			...
			if (PXAccess.FeatureInstalled<FeaturesSet.interBranch>() == false)
			{
				...
				var organizationID = PXAccess.GetParentOrganizationID(cashAcc.BranchID);
				int[] childBranches = PXAccess.GetChildBranchIDs(organizationID);
				sel.WhereAnd<Where<Light.APInvoice.branchID, In<Required<Light.APInvoice.branchID>>>>();
				bqlParams.Add(childBranches);
			}
		...
		private void TryToSetDefaultTaxInfo(CABankTran row, CashAccount cashAccount)
		{
			if (!string.IsNullOrEmpty(row.EntryTypeID))
			{
				CashAccountETDetail entryType = PXSelect<CashAccountETDetail, Where<CashAccountETDetail.cashAccountID, Equal<Required<CABankTran.cashAccountID>>,
		...
		{
			CABankTranDetail split = e.Row as CABankTranDetail;
			CABankTran adj = CABankTran.Current;
