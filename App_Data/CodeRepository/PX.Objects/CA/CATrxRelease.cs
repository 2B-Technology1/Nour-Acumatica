...
namespace PX.Objects.CA
{
	...
	public class CATrxRelease : PXGraph<CATrxRelease>
	{
				...
				try
				{
					...
					switch (tran.OrigModule)
					{
						...
						case GL.BatchModule.AP:
							...
							{
								CATrxRelease.ReleaseCATran(tran, ref searchGraph, out error);
		...
		{
			ReleaseCATran(aTran, ref aGraph, null, out PXSetPropertyException error);
			if (error?.InnerException != null) throw error.InnerException;
		}

		public static void ReleaseCATran(CATran aTran, ref PXGraph aGraph, out PXSetPropertyException error)
		{
			ReleaseCATran(aTran, ref aGraph, null, out error);
		}

		public static void ReleaseCATran(CATran aTran, ref PXGraph aGraph, List<Batch> externalPostList)
		{
			ReleaseCATran(aTran, ref aGraph, externalPostList, out PXSetPropertyException error);
		}

		public static void ReleaseCATran(CATran aTran, ref PXGraph aGraph, List<Batch> externalPostList, out PXSetPropertyException error)
		{
			error = null;
			int i = 0;
			if (aTran != null)
			{
				...
				switch (aTran.OrigModule)
				{
					case GL.BatchModule.AP:
						...
						{
							var ex = new PXException(Messages.CannotReleasePendingApprovalDocument);
							error = new PXSetPropertyException(ex, PXErrorLevel.RowError, ex.Message);
							break;
						}
					...
					case GL.BatchModule.AR:
						...
						if (arReg.PendingProcessing == true) throw new PXException(Messages.CannotReleasePendingprocessingDocument);
		...
		protected virtual void _(Events.CacheAttached<CARegister.branchID> e) { }
		#endregion
