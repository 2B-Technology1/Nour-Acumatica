...
namespace PX.Objects.CA
{
	public class CAAPARTranType : ILabelProvider
	{
		...
		protected static readonly ValueLabelList listAllUI = new ValueLabelList
		{
			...
			new GLTranType().ValueLabelPairs,
			{ CATranType.CABatch,   AP.Messages.APBatch } ,
			new APDocType().ValueLabelPairsUI.Except(listARAPIntersect, Comparer),
			new ARDocType().ValueLabelPairs.Except(listARAPIntersect, Comparer),
			listARAPIntersect,
			new EPDocumentType().ValueLabelList
		};
		...
		public class ListByModuleUIAttribute : LabelListAttribute
		{
			...
			protected Type _ModuleField;
			protected string lastModule;
			protected ListByModuleUIAttribute(Type moduleField, ValueLabelList listAll) : base(listAll)
			{
				_ModuleField = moduleField;
				_listAll = listAll;
			}
			public ListByModuleUIAttribute(Type moduleField)
				: this(moduleField, listAllUI)
			{ }
			...
			protected void SetNewList(PXCache sender, ValueLabelList valueLabelList)
			{
				List<ListByModuleUIAttribute> list = new List<ListByModuleUIAttribute>();
				list.Add(this);
				SetListInternal(list, valueLabelList.Select(pair => pair.Value).ToArray(), valueLabelList.Select(pair => pair.Label).ToArray(), sender);
			}
			...
			{
				base.TryLocalize(sender);
				RipDynamicLabels(_listAll.Select(pair => pair.Label).ToArray(), sender);
			}
		}
	}
}
