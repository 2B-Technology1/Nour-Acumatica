...
{
	public partial class CABankTransactionsMaint : PXGraph<CABankTransactionsMaint>, ICABankTransactionsDataProvider, IAddARTransaction, IAddAPTransaction
	{
		#region Constructor
		...
		public virtual IEnumerable LoadInvoices(PXAdapter adapter)
		{
			...
			if (currentDoc.OrigModule == GL.BatchModule.AR)
			{
				...
				if (dialogResult == WebDialogResult.OK || dialogResult == WebDialogResult.Yes)
				{
					...
					loadOpts.Current.LoadChildDocuments = ARPaymentEntry.LoadOptions.loadChildDocuments.IncludeCRM;
					PXResultset<ARInvoice> custdocs = ARPaymentEntry.GetCustDocs(loadOpts.Current,
					new ARPayment
					{
						Released = false,
						OpenDoc = true,
		...
		protected virtual void ClearMatchProc(CABankTran detail)
		{
			...
			{
				CABankTran copy = (CABankTran)Details.Cache.CreateCopy(detail);
				...
				foreach (var adj in Adjustments.Select(copy.TranID))
				{
					Adjustments.Delete(adj);
				}
				foreach (var split in TranSplit.Select(copy.TranID))
				{
					TranSplit.Delete(split);
				}

				ClearFields(copy);
				ClearChargeFields(copy);
				Details.Update(copy);
			}
		}
		...
		public virtual void CreateRule()
		{
			...
			var rule = new CABankTranRulePopup
			{
				...
				PayeeName = currentTran.PayeeName
			};
			...
			if (!e.Cache.ObjectsEqual<CABankTranExpenseDetailMatch.isMatched>(e.Row, e.OldRow))
			{
				...
				if (e.Row.IsMatched == true)
				{
					CABankTranMatch match = new CABankTranMatch()
					{
						...
						CuryApplAmt = receipt.ClaimCuryTranAmtWithTaxes,
					};
		...
		protected virtual void CABankTran_CreateDocument_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			{
				sender.SetDefaultExt<CABankTran.curyApplAmt>(row);
		...
		protected virtual string GetUnrecognizedReceiptDefaultEntryType(CABankTran row)
		{
			...
			CAEntryType entryType = PXSelectJoin<
				...
				Where<CashAccountETDetail.cashAccountID, Equal<Required<CABankTran.cashAccountID>>,
		...
		protected virtual void CABankTran_EntryTypeId_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (row != null)
			{
				...
				if (row.OrigModule == GL.BatchModule.CA && row.EntryTypeID == null && row.EntryTypeID != (string)e.OldValue)
				{
					foreach (CABankTranDetail split in TranSplit.Select())
					{
						TranSplit.Delete(split);
		...
		public static void CABankTranRowUpdated(CABankTransactionsMaint graph, PXCache sender, PXRowUpdatedEventArgs e, PXSelectBase<CABankTranDetail> tranSplit, PXSelectBase<CABankTranMatch> tranMatch, PXSelectBase<CABankTranAdjustment> adjustments, PXSelectBase<CABankTranMatch> tranMatchCharge, bool matchInvoiceProcess)
		{
			...
			if (row.CreateDocument == true)
			{
				if(oldRow.TaxCalcMode != row.TaxCalcMode)
				{
					foreach (CABankTranDetail split in tranSplit.Select())
					{
						tranSplit.Delete(split);
			...
			ProcessChargeOnRowUpdated(row, oldRow, tranMatchCharge, matchInvoiceProcess);
		}

		private static void ProcessChargeOnRowUpdated(CABankTran row, CABankTran oldRow, PXSelectBase<CABankTranMatch> tranMatchCharge, bool matchInvoiceProcess)
		{
		...
		private void VerifyMatchingPaymentDate(PXCache sender, CABankTran row)
		{
			...
			DateTime originalTranDate = (DateTime)row.TranDate;
			int? tranDaysBefore = ((CashAccount)cashAccount.Select())?.DisbursementTranDaysBefore ?? 0;
		...
		private void EnableCreateTab(PXCache sender, CABankTran row, bool needsPMInstance)
		{
			...
			bool noAdjustmentsYet = (row?.CountAdjustments ?? 0) == 0;
			...
			Adjustments.View.AllowInsert = row?.PayeeBAccountID != null;
		}
		...
		public static bool ValidateTranFields(CABankTransactionsMaint graph, PXCache sender, CABankTran row, PXSelectBase<CABankTranAdjustment> adjustments)
		{
			...
			RaiseOrHideError<CABankTran.curyUnappliedBalCA>(sender, row, unappliedBalCA, Messages.AmountDiscrepancy, PXErrorLevel.RowWarning, row.CuryDetailsWithTaxesTotal, row.CuryTotalAmt);
		...
		public static void CheckExternalTaxProviders(PXCache sender, CABankTran row)
		{
			...
			if (row.ChargeTypeID != null)
			{
				string defaultTaxZoneID = ((CashAccountETDetail)SelectFrom<CashAccountETDetail>
															.Where<CashAccountETDetail.cashAccountID.IsEqual<@P.AsInt>
		...
		protected virtual void _(Events.FieldVerifying<Filter.cashAccountID> e)
		{
			...
			{
				Account account = Account.PK.Find(this, (int?)cashAccount?.AccountID);
				if (account?.IsCashAccount != true)
				{
					PXTrace.WriteError(new PXException(Messages.GLAccountIsNotCashAccount, account.AccountCD));
					e.Cache.RaiseExceptionHandling<Filter.cashAccountID>(e.Row, e.NewValue,
					new PXSetPropertyException(Messages.GLAccountIsNotCashAccount, PXErrorLevel.RowError, account.AccountCD));
					e.Cancel = true;
				}
			}
		}
		...
		protected virtual void _(Events.RowPersisting<CABankTaxTran> e)
		{
			var parentRecord = (CABankTran)PXParentAttribute.SelectParent(e.Cache, e.Row);

			if (parentRecord != null && (e.Operation == PXDBOperation.Insert || e.Operation == PXDBOperation.Update))
			{
				e.Row.TaxZoneID = parentRecord.TaxZoneID;
				PXDefaultAttribute.SetPersistingCheck<CABankTaxTran.taxZoneID>(e.Cache, e.Row, PXPersistingCheck.NullOrBlank);
			}
			else
			{
				PXDefaultAttribute.SetPersistingCheck<CABankTaxTran.taxZoneID>(e.Cache, e.Row, PXPersistingCheck.Nothing);
			}
		}
		...
		protected virtual void CABankTranDetail_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (rowStatus != PXEntryStatus.InsertedDeleted && rowStatus != PXEntryStatus.Deleted)
			{
			if (row.AccountID == null)
			{
				...
				try
				{
					...
					using (PXTransactionScope ts = new PXTransactionScope())
					{
						...
						else
						{
							...
							graph.MatchCATran(det, toPost, out error);
						}

						det = (CABankTran)graph.Details.Cache.CreateCopy(det);
						det.Processed = (error == null);
					...
					listMessages[det.TranID.Value] = error ?? new PXSetPropertyException(Messages.DeatsilProcess, PXErrorLevel.RowInfo);
					hasErrors = hasErrors || error?.ErrorLevel == PXErrorLevel.RowError;
				}
		...
		{
			MatchCATran(det, externalPostList, out PXSetPropertyException error);
		}

		protected virtual void MatchCATran(CABankTran det, List<Batch> externalPostList, out PXSetPropertyException error)
		{
			error = null;
			...
			{
				matchFound = ProcessCABankTranMatches(det, externalPostList, match, out error);
			}
		...
		private bool ProcessCABankTranMatches(CABankTran det, List<Batch> externalPostList, CABankTranMatch match, out PXSetPropertyException error)
		{
			error = null;
			bool matchFound;
			if (match == null)
			...
			if (match.DocModule == BatchModule.AP && match.DocType == CATranType.CABatch)
			{
				...
				foreach (CATranExt tran in PXSelectJoin<
					...
					.Select(this, match.DocRefNbr))
				{
					if (ProcessCATran(det, externalPostList, tran.TranID, out error, false) != true)
					{
						cleared = false;
					}
				}
				...
				if (batch.Released != true)
				{
					...
					ReleaseCABatch(batchEntryGraph, batch, det);
				}
			}
			else
			{
				ProcessCATran(det, externalPostList, match.CATranID, out error);
			}

			return matchFound;
		}

		protected virtual void ReleaseCABatch(CABatchEntry batchEntryGraph, CABatch batch, CABankTran detail)
		{
			if (batch.SkipExport != true && batch.Exported != true)
			{
				throw new PXException(Messages.CABatchShouldBeExportedBeforeRelease, batch.BatchNbr);
			}
		...
		protected virtual bool ProcessCATran(CABankTran det, List<Batch> externalPostList, Int64? tranID)
		{
			return ProcessCATran(det, externalPostList, tranID, out PXSetPropertyException error);
		}

		protected virtual bool ProcessCATran(CABankTran det, List<Batch> externalPostList, Int64? tranID, out PXSetPropertyException error)
		{
			return ProcessCATran(det, externalPostList, tranID, out error, true);
		}

		private bool ProcessCATran(CABankTran det, List<Batch> externalPostList, Int64? tranID, out PXSetPropertyException error, bool checkAmt = true)
		{
			error = null;
				...
				if (tran.Released != true)
				{
					...
					switch (tran.OrigModule)
					{
						case GL.BatchModule.AP:
							...
							{
								CATrxRelease.ReleaseCATran(tran, ref searchGraph, externalPostList, out error);
								TryToReleaseBatchPayment(tran, tranID);
							}
							break;
						case GL.BatchModule.AR:
		...
		protected virtual void TryToReleaseBatchPayment(CATranExt tran, Int64? tranID)
		{
			var batchResult = (PXResult<CABatch, CABatchDetail>)PXSelectReadonly2<CABatch,
					InnerJoin<CABatchDetail, On<CABatch.batchNbr, Equal<CABatchDetail.batchNbr>>>,
					Where<CABatchDetail.origModule, Equal<Required<CABatchDetail.origModule>>,
						...
						And<CABatchDetail.origLineNbr, Equal<CABatchDetail.origLineNbr.defaultValue>>>>>>
					.Select(this, GL.BatchModule.AP, tran.OrigTranType, tran.OrigRefNbr);

			CABatch batch = batchResult;
			CABatchDetail detail = batchResult;

			if (batch != null && batch?.Released != true && !CheckIfBatchHasUnreleasedPayments(detail, tranID) && !CheckIfBatchHasUnreleasedPaymentsInCache(detail, tranID))
			{
				CABatchEntry batchEntry = PXGraph.CreateInstance<CABatchEntry>();
				batchEntry.Document.Current = batch;

				try
				{
					ReleaseCABatch(batchEntry, batch, null);
				}
				catch { }
			}
		}

		private bool CheckIfBatchHasUnreleasedPayments(CABatchDetail detail, Int64? tranID)
		{
			return PXSelectJoin<CABatchDetail,
						InnerJoin<APPayment, On<CABatchDetail.origModule, Equal<BatchModule.moduleAP>,
						...
						Where<CABatchDetail.batchNbr, Equal<Required<CABatch.batchNbr>>,
							And<APPayment.released, NotEqual<True>,
							And<APPayment.cATranID, NotEqual<Required<APPayment.cATranID>>>>>>.Select(this, detail.BatchNbr, tranID).Any();
		}

		private bool CheckIfBatchHasUnreleasedPaymentsInCache(CABatchDetail detail, Int64? tranID)
		{
			foreach (var result in PXSelectJoin<CABankTran,
						InnerJoin<CABankTranMatch,
							On<CABankTran.tranID, Equal<CABankTranMatch.tranID>>,
						InnerJoin<APPayment, On<APPayment.docType, Equal<CABankTranMatch.docType>,
							And<APPayment.refNbr, Equal<CABankTranMatch.docRefNbr>,
							And<CABankTranMatch.docModule, Equal<BatchModule.moduleAP>>>>,
						InnerJoin<CABatchDetail, On<CABatchDetail.origModule, Equal<BatchModule.moduleAP>,
							And<CABatchDetail.origDocType, Equal<APPayment.docType>,
							And<CABatchDetail.origRefNbr, Equal<APPayment.refNbr>,
							And<CABatchDetail.origLineNbr, Equal<CABatchDetail.origLineNbr.defaultValue>>>>>>>>,
						Where<CABatchDetail.batchNbr, Equal<Required<CABatch.batchNbr>>,
							And<CABankTran.processed, NotEqual<True>,
							And<APPayment.cATranID, NotEqual<Required<APPayment.cATranID>>>>>>.Select(this, detail.BatchNbr, tranID))
			{
				var caBankTran = (CABankTran)result;
				caBankTran = Details.Locate(caBankTran);

				if (caBankTran.Processed == true) continue;
				return true;
			}
		...
		protected virtual void ValidateDataForChargeCreation(CABankTran aRow)
		{
			...
			else
			{
				...
				Account account = PXSelectJoin<Account,
					InnerJoin<CAEntryType, On<CAEntryType.accountID, Equal<Account.accountID>>>,
					Where<CAEntryType.entryTypeId, Equal<Required<CAEntryType.entryTypeId>>>>.Select(this, aRow.ChargeTypeID);

				if (account?.AccountGroupID != null)
				{
					if (aRow.DrCr == CADrCr.CADebit)
					{
						throw new PXSetPropertyException(Messages.ChargeTypeRequiresDefinedProjectAR, aRow.ChargeTypeID);
					}
					else
					{
						throw new PXSetPropertyException(Messages.ChargeTypeRequiresDefinedProjectAP, aRow.ChargeTypeID);
					}
				}
			}
		}
		...
		protected virtual CATran AddARTransaction(ICADocSource parameters, CurrencyInfo aCuryInfo, IEnumerable<ICADocAdjust> aAdjustments, bool aOnHold)
		{
			...
			CM.CurrencyInfo refInfo = aCuryInfo.GetCM();
			return PaymentReclassifyProcess.AddARTransaction(this, parameters, refInfo, aAdjustments, aOnHold);
		}
		...
		public virtual ARPayment InitializeARPayment(ARPaymentEntry graph, ICADocSource parameters, CM.CurrencyInfo aCuryInfo, bool aOnHold)
		{
			return AddARTransactionHelper.InitializeARPayment(graph, parameters, aCuryInfo, aOnHold, Descriptor.BranchSource.CustomerVendorLocation);
		}

		public virtual void InitializeCurrencyInfo(ARPaymentEntry graph, ICADocSource parameters, CM.CurrencyInfo aCuryInfo, ARPayment doc)
		{
		...
		protected virtual CATran AddAPTransaction(ICADocSource parameters, CurrencyInfo aCuryInfo, IList<ICADocAdjust> aAdjustments, bool aOnHold)
		{
			...
			CM.CurrencyInfo refInfo = aCuryInfo.GetCM();
			return PaymentReclassifyProcess.AddAPTransaction(this, parameters, refInfo, aAdjustments, aOnHold);
		}

		#region IAddAPTransaction
		public virtual APPayment InitializeAPPayment(APPaymentEntry graph, ICADocSource parameters, CM.CurrencyInfo aCuryInfo, IList<ICADocAdjust> aAdjustments, bool aOnHold)
		{
			return AddAPTransactionHelper.InitializeAPPayment(graph, parameters, aCuryInfo, aAdjustments, aOnHold, Descriptor.BranchSource.CustomerVendorLocation);
		}

		public virtual void InitializeCurrencyInfo(APPaymentEntry graph, ICADocSource parameters, CM.CurrencyInfo aCuryInfo, APPayment doc)
		{
		...
		protected virtual void ApplyRule(CABankTran transaction, CABankTranRule rule)
		{
			...
			{
				if (transaction.CuryInfoID == null)
				{
					SetDefaultCurrencyInfoID(transaction);
				}
		...
		{
			CurrencyInfo refInfo = base.GetExtension<CA.MultiCurrency.CABankTransactionsMaintMultiCurrency>().CreateCurrencyInfo();
			Details.Cache.SetValueExt<CABankTran.curyInfoID>(transaction, refInfo.CuryInfoID);
		}
			...
			if (parameters.OrigModule == GL.BatchModule.CA)
			{
				...
				if (refInfo == null)
				{
					refInfo = PXSelectReadonly<
						CurrencyInfo,
						Where<CM.Extensions.CurrencyInfo.curyInfoID, Equal<Required<CurrencyInfo.curyInfoID>>>>
				...
				PXNoteAttribute.CopyNoteAndFiles(sourceCache, parameters, te.CAAdjRecords.Cache, adj);
		...
		public static CATran AddChargeTransaction(PXGraph graph, ICADocSource bankTran, CurrencyInfo aCuryInfo, decimal? amount, string taxCategotyID)
		{
			...
			CurrencyInfo refInfo = aCuryInfo;
			refInfo = (CurrencyInfo)teMultiCurrency.currencyinfo.Cache.CreateCopy(refInfo);
		...
		public virtual PXResultset<CABatch> SearchForMatchingCABatches(CABankTran aDetail, IMatchSettings aSettings, Pair<DateTime, DateTime> tranDateRange, string curyID, bool allowUnreleased)
		{
			return CABankTransactionsRepository.SearchForMatchingCABatches(this, aDetail, aSettings, tranDateRange, curyID, allowUnreleased);
		}
