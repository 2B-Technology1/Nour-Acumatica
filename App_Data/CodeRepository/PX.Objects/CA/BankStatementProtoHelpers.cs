...
namespace PX.Objects.CA.BankStatementProtoHelpers
{
	...
	public static class StatementsMatchingProto
	{
			...
			where T : PXGraph, ICABankTransactionsDataProvider
		{
			...
			foreach (PXResult<CATranExt, Light.BAccount, CATran2, CABankTranMatch2, CABatchDetailOrigDocAggregate> iRes in graph.SearchForMatchingTransactions(aDetail, aSettings, tranDateRange, curyID))
			{
				...
				{
					PXResultset<CABatchDetailOrigDocAggregate> matches = graph.SearchForMatchesInCABatches(aDetail.TranType, batchDetail.BatchNbr);
				...
				iTran.Status = graph.GetStatus(iTran);
				...
				Where<CABankTranMatch.tranID, Equal<Required<CABankTranMatch.tranID>>>>.Select(graph, aDetail.TranID))
			{
				...
				if (matchedTran != null && matchedTran.TranID != null)
				{
					if (matchList.Find((PXResult<CATranExt, Light.BAccount> result) => { CATranExt tran = result;  return tran.TranID == matchedTran.TranID; }) == null)
					{
						...
						matchedTran.Status = graph.GetStatus(matchedTran);
		...
		public static string GetStatus(CATran tran)
		{
			...
			else if (tran.Released == true && tran.Posted != true)
			{
				return GL.BatchStatus.Released;
			}
			else if (tran.Hold == true)
			{
			...
			where T : PXGraph, ICABankTransactionsDataProvider
		{
			...
			foreach (PXResult<CABatch, CABatchDetail, Light.APPayment, Light.BAccount, CABankTranMatch> iRes in graph.SearchForMatchingCABatches(aDetail, aSettings, tranDateRange, curyID, allowUnreleased))
			{
		...
		public static Pair<DateTime, DateTime> GetDateRangeForMatch(CABankTran aDetail, IMatchSettings aSettings)
		{
			...
			tranDateStart = tranDateStart.AddDays(-(isReceipt ? (aSettings.ReceiptTranDaysBefore ?? 0) : (aSettings.DisbursementTranDaysBefore ?? 0)));
			tranDateEnd = tranDateEnd.AddDays((isReceipt ? (aSettings.ReceiptTranDaysAfter ?? 0) : (aSettings.DisbursementTranDaysAfter ?? 0)));
		...
		public static bool CheckRuleMatches(CABankTran transaction, CABankTranRule rule)
		{
			...
			if (String.IsNullOrEmpty(rule.PayeeName) == false && match)
			{
				match &= GetRegexPayeeName(rule).IsMatch(transaction.PayeeName ?? "");
			}
			return match;
		}

        private static Regex GetRegexPayeeName(CABankTranRule rule)
        {
			var escaped = Regex.Escape(rule.PayeeName ?? "");
			var  PatternPayeeName = rule.UsePayeeNameWildcards == true ?
            					"^" + escaped.Replace("\\*", ".*").Replace("\\?", ".") + "$"
            					: escaped;
	        return new Regex(PatternPayeeName,
		        rule.MatchDescriptionCase == true ? new RegexOptions() : RegexOptions.IgnoreCase);
        }
	}
}
