	...
	public class CAReconEntry : PXGraph<CAReconEntry>, ICaptionable
	{
				...
				if (doc.SkipVoided == true)
				{
					voidingTrans.AddRange(PXSelectReadonly<CATranVoiding,
												  ...
												  And<Where<CATranVoiding.tranDate, LessEqual<Current2<CARecon.loadDocumentsTill>>,
														...
														Or<CATranVoiding.reconNbr, Equal<Current2<CARecon.reconNbr>>>>>>>>>
		...
		private List<CATranExt> GetBatchTransactions(HashSet<long> catrans)
		{
			...
			foreach (PXResult<CABatch, CABatchDetail, CATran, CABankTranMatch, CABankTran, CABankTranMatch2> result in lines)
			{
				...
				if (SkipTransaction(catran, batch))
				{
					continue;
				}
		...
		protected virtual bool SkipTransaction(CATran tran, CABatch batch)
		{
			//Please be advised that SkipTransaction method is also implemented in PRxCAReconEntry Graph Extension
			return false;
		}
		#endregion
		...
		protected virtual void CARecon_RowInserted(PXCache cache, PXRowInsertedEventArgs e)
		{
			...
			if (PXAccess.FeatureInstalled<FeaturesSet.multicurrency>())
			{
				...
				var error = PXUIFieldAttribute.GetErrorWithLevel<CurrencyInfo.curyID>(currencyinfo.Cache, info);
				string message = error.errorMessage;
				if (string.IsNullOrEmpty(message) != true && error.errorLevel >= PXErrorLevel.Error)
				{
					throw new PXSetPropertyException(message, PXErrorLevel.Error);
				}

				error = PXUIFieldAttribute.GetErrorWithLevel<CurrencyInfo.curyEffDate>(currencyinfo.Cache, info);
		...
		protected virtual void CARecon_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			SetLoadDocumentsTillWarning(cache, header);
			}

		private void SetLoadDocumentsTillWarning(PXCache cache, CARecon header)
			{
			PXSetPropertyException loadDocumentsTillWarning = null;
			if (header.ReconDate != null &&
			    header.LoadDocumentsTill != null &&
			    header.LoadDocumentsTill < header.ReconDate)
			{
				loadDocumentsTillWarning =
					new PXSetPropertyException(
						header.IsUserLoadDocumentsTill == true
							? Messages.LastDateToLoadNotAvailable
							: Messages.LoadDocumentsUpToSetLessReconciledDateBySystem, PXErrorLevel.Warning);
			}

			cache.RaiseExceptionHandling<CARecon.loadDocumentsTill>(header, header.LoadDocumentsTill, loadDocumentsTillWarning);
		}
		...
		protected virtual void _(Events.FieldUpdated<CARecon.loadDocumentsTill> e)
		{
			CARecon header = (CARecon)e.Row;
			e.Cache.SetValue<CARecon.isUserLoadDocumentsTill>(header, true);
		}
		#endregion
