...
namespace PX.Objects.CA
{
    ...
    public class CashAccountMaint : PXGraph<CashAccountMaint, CashAccount>
	{
		...
		public PXSelectReadonly<Account, Where<Account.accountID, Equal<Optional<CashAccount.accountID>>, And<Match<Current<AccessInfo.userName>>>>> Account_AccountID_Readonly;
		public PXSelect<CashAccountCheck, Where<CashAccountCheck.cashAccountID, Equal<Optional<CashAccount.cashAccountID>>>> CashAccountChecks;
        ...
        public PXSelectJoin<CashAccountETDetail,
                    InnerJoin<CAEntryType, On<CAEntryType.entryTypeId, Equal<CashAccountETDetail.entryTypeID>>>,
                        Where<CashAccountETDetail.cashAccountID, Equal<Current<CashAccount.cashAccountID>>>> ETDetails;
        public PXSelect<CAEntryType> EntryTypes;
        [PXCopyPasteHiddenFields(new Type[] {typeof(CashAccountDeposit.cashAccountID) })]
        public PXSelect<CashAccountDeposit, Where<CashAccountDeposit.cashAccountID, Equal<Current<CashAccount.cashAccountID>>>> Deposits;
        ...
        public PXSelectJoin<CashAccountPaymentMethodDetail,
                               ...
                               Where<CashAccountPaymentMethodDetail.cashAccountID, Equal<Optional2<CashAccount.cashAccountID>>,
        ...
        protected virtual void CashAccount_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
        {
			...
			if (CashAccount.Cache.GetStatus(cashacct) == PXEntryStatus.Inserted || CashAccount.Cache.GetStatus(cashacct) == PXEntryStatus.Updated)
			{
				Account account = Account_AccountID_Readonly.SelectSingle(cashacct.AccountID);

				if (account.IsCashAccount != true)
				{
					MarkGLAccoutAsCashAccount(cashacct.AccountID);
				}
			}
		}
			...
			if (!sender.ObjectsEqual<CashAccount.accountID>(updatedCashAccount, oldCashAccount))
			{
				...
				foreach (CashAccount cashAccount in accountsToProcess)
				{
					if (cashAccount.AccountID == null)
						continue;
					MarkGLAccoutAsCashAccount(cashAccount.AccountID);
				}
			}
		}
		...
		{
			Account generalLedgerAccount = Account_AccountID.SelectSingle(accountID);

			using (new PXReadBranchRestrictedScope())
			{
				bool haveCashAccounts =
				PXSelect<CashAccount, Where<CashAccount.accountID, Equal<Required<CashAccount.accountID>>>>.Select(this, accountID).Count > 0;
		...
		protected virtual void PaymentMethodAccount_APQuickBatchGeneration_FieldDefaulting(PXCache cache, PXFieldDefaultingEventArgs e)
		{
			...
			PaymentMethod pm = PXSelect<PaymentMethod, Where<PaymentMethod.paymentMethodID, Equal<Required<PaymentMethod.paymentMethodID>>>>.Select(this, row.PaymentMethodID);
			e.NewValue = row?.UseForAP == true && pm?.APCreateBatchPayment == true;
			e.Cancel = true;
		}
        ...
        protected virtual void PaymentMethodAccount_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
        {
			...
			if (row != null && string.IsNullOrEmpty(row.PaymentMethodID) == false)
			{
				pt = PXSelect<PaymentMethod,
									  Where<PaymentMethod.paymentMethodID, Equal<Required<PaymentMethod.paymentMethodID>>>>.
								   Select(this, row.PaymentMethodID);

				var createBatchPayment = pt?.APCreateBatchPayment == true;
				PXUIFieldAttribute.SetEnabled<PaymentMethodAccount.aPBatchLastRefNbr>(cache, row, EnableAP && createBatchPayment);
				PXUIFieldAttribute.SetEnabled<PaymentMethodAccount.aPQuickBatchGeneration>(cache, e.Row, EnableAP && createBatchPayment);
				PaymentMethodAccountHelper.VerifyAPAutoNextNbr(cache, row, pt);
				PXException ex = null;
				if (row.APQuickBatchGeneration == true)
				{
					PaymentMethodAccountHelper.TryToVerifyAPLastReferenceNumber<PaymentMethodAccount.aPQuickBatchGeneration>(row.APAutoNextNbr, row.APLastRefNbr, null, out ex);
				}
				cache.RaiseExceptionHandling<PaymentMethodAccount.aPQuickBatchGeneration>(row, row.APQuickBatchGeneration, ex);
			}
		...
		private void SetupDefaultEntryType(PXCache sender, PXRowUpdatedEventArgs e)
		{
            ...
            CashAccountETDetail defaultDetail = PXSelect<CashAccountETDetail, Where<CashAccountETDetail.isDefault, Equal<True>,
                            And<CashAccountETDetail.cashAccountID, Equal<Required<CashAccountETDetail.cashAccountID>>,
                            And<CashAccountETDetail.entryTypeID, NotEqual<Required<CashAccountETDetail.entryTypeID>>>>>>.Select(this, detail.CashAccountID, detail.EntryTypeID);
		...
		public virtual bool CheckIfCashAccountUsedForBatchPayments(CashAccount aAcct)
		{
			PaymentMethod paymentMethod = PXSelectJoin<PaymentMethod,
				InnerJoin<PaymentMethodAccount, On<PaymentMethod.paymentMethodID, Equal<PaymentMethodAccount.paymentMethodID>>>,
				Where<PaymentMethod.aPCreateBatchPayment, Equal<True>,
					And<PaymentMethodAccount.cashAccountID, Equal<Required<PaymentMethodAccount.cashAccountID>>>>>.SelectWindowed(this, 0, 1, aAcct.CashAccountID);
			if (string.IsNullOrEmpty(paymentMethod?.PaymentMethodID)) return false;
			return true;
		}
