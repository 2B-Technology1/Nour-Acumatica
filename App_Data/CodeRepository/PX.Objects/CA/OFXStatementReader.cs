...
//        #endregion
		...
		protected virtual void ReadHeader100(string aHeader)
		{
			...
			// Collecting the start positions for header-value pairs
			var headerPositions = new SortedDictionary<int, string>();
			foreach (string headerValue in headerValues)
			{
				int headerIndex = aHeader.IndexOf(headerValue);
				if (headerIndex >= 0)
					headerPositions.Add(aHeader.IndexOf(headerValue), headerValue);
			}

			// Extract the header-value pairs by position - extract the substring between header start index and next header start position
			for (int i = 0; i < headerPositions.Count; i++)
			{
				int startPosition = headerPositions.ElementAt(i).Key;
				string[] valueSplitted = aHeader.Substring(startPosition, (i != headerPositions.Count - 1 ? headerPositions.ElementAt(i + 1).Key : aHeader.Length) - startPosition).Split(':');
				// Save only needed values
				switch (valueSplitted[0].Trim())
				{
					case "OFXHEADER": this.header.OFXHEADER = ExtractValueFromHeader(valueSplitted[1].Trim()); break;
					case "VERSION": this.header.VERSION = ExtractValueFromHeader(valueSplitted[1].Trim()); break;
					case "CHARSET": this.header.CHARSET = ExtractValueFromHeader(valueSplitted[1].Trim()); break;
					case "ENCODING": this.header.ENCODING = ExtractValueFromHeader(valueSplitted[1].Trim()); break;
					case "DATA": this.header.DATA = ExtractValueFromHeader(valueSplitted[1].Trim()); break;
				}
			}
		}

		// We do not need invoke the string.Trim() because the whitespaces will be removed in string.Split(...)
		private string ExtractValueFromHeader(string input) => input.Split(Splitters, StringSplitOptions.RemoveEmptyEntries)[0];
...
//		protected abstract void ReadOFXMessage(string aOFXMessage);
	...
	public class OFXStatementReader : OFXReaderBase, IStatementReader
	{
			...
			where T : CABankTransactionsImport
		{
			...
			var notExistCashAccounts = string.Empty;
			var notSelectedCashAccounts = string.Empty;
			var cashAccountsLinkedToBankFeeds = string.Empty; 

			foreach (STMTRS iAcctStatement in this.Content.Stmtrs)
			{
				ExportNew(graph, iAcctStatement, updateCurrent, ref notExistCashAccounts, ref notSelectedCashAccounts, ref cashAccountsLinkedToBankFeeds, allowEmptyFITID);
			...
			if (!string.IsNullOrEmpty(cashAccountsLinkedToBankFeeds))
			{
				throw new PXException(cashAccountsLinkedToBankFeeds);
			}
			...
			where T : CABankTransactionsImport
		{
			...
			ExportNew(aGraph, aAcctStatement, updateCurrent, ref notExistCashAccounts, ref notSelectedCashAccounts, ref cashAccountsLinkedToBankFeeds, allowEmptyFITID);
		...
		protected void ExportNew<T>(T aGraph, STMTRS aAcctStatement, bool updateCurrent, ref string notExistCashAccounts, ref string notSelectedCashAccounts,
			ref string linkedToBankFeed, bool allowEmptyFITID=false)
			where T : CABankTransactionsImport
		{
			...
			else if (aGraph.CheckCashAccountIsLinkedToBankFeed(acct, out var message))
			{
				linkedToBankFeed += "\n" + message;
			}
			else
