...
namespace PX.Objects.CA
{
	public class CABankTransactionsImport : PXGraph<CABankTransactionsImport, CABankTranHeader>, PXImportAttribute.IPXPrepareItems, ICaptionable
	{
		...
		public virtual IEnumerable UploadFile(PXAdapter adapter)
		{
			...
			if (CASetup.Current.ImportToSingleAccount == true)
			{
				...
				else
				{
					...
					if (CheckCashAccountIsLinkedToBankFeed(acct, out string errMessage))
					{
						throw new PXException(errMessage);
					}
		...
		[PXButton(ConfirmationMessage = ActionsMessages.ConfirmationMsg,
		...
		public CABankTransactionsImport()
        {
			...
			PXUIFieldAttribute.SetReadOnly<CABankTran.userDesc>(Details.Cache, null, true);
		...
		public virtual bool CheckCashAccountIsLinkedToBankFeed(CashAccount cashAccount, out string message)
		{
			var ret = false;
			message = null;
			if (cashAccount != null)
			{
				var ext = GetExtension<CABankTransactionsImportBankFeed>();
				if (ext != null)
				{
					var bankFeed = ext.BankFeedDetail.SelectSingle(cashAccount.CashAccountID);
					if (bankFeed != null)
					{
						ret = true;
						message = PXMessages.LocalizeFormatNoPrefix(Messages.CashAccountAlreadyLinked, cashAccount.CashAccountCD, bankFeed.AccountName, bankFeed.BankFeedID);
					}
				}
			}
			return ret;
		...
		public bool IsAlreadyImported(int? aCashAccountID, string aExtTranID, out string aRefNbr)
		{
			...
			if (detail != null)
				aRefNbr = detail.HeaderRefNbr.ToString();
			return (detail != null);
		}
