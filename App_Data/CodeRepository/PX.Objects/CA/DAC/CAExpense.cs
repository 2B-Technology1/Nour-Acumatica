...
namespace PX.Objects.CA
{
	...
	public class CAExpense : AP.IPaymentCharge, IBqlTable
	{
		...
		{
			public static CAExpense Find(PXGraph graph, string refNbr, int? lineNbr, PKFindOptions options = PKFindOptions.None) => FindBy(graph, refNbr, lineNbr, options);
		}
		...
		[CashAccount(null, typeof(Search<CashAccount.cashAccountID, Where<Match<Current<AccessInfo.userName>>>>), DisplayName = "Cash Account", Visibility = PXUIVisibility.SelectorVisible, DescriptionField = typeof(CashAccount.descr), Required = true)]
		[PXDefault]
		...
		[PXSelector(typeof(Search2<CAEntryType.entryTypeId,
			...
			Where<CashAccountETDetail.cashAccountID, Equal<Current<cashAccountID>>,
		...
		[Account(
			...
			typeof(Search<Account.accountID,
				Where2<Where<Account.curyID, Equal<Current<curyID>>, Or<Account.curyID, IsNull>>,
	...
	public abstract class taxableAmt : PX.Data.BQL.BqlDecimal.Field<taxableAmt> { }
		...
		[PXDefault(TypeCode.Decimal, "0.0", typeof(curyTaxableAmt))]
		...
		[PXDefault(typeof(Search<CashAccountETDetail.taxZoneID, Where<CashAccountETDetail.cashAccountID, Equal<Current<CAExpense.cashAccountID>>, And<CashAccountETDetail.entryTypeID, Equal<Current<CAExpense.entryTypeID>>>>>), PersistingCheck = PXPersistingCheck.Nothing)]
		[PXFormula(typeof(Default<cashAccountID, entryTypeID>))]
		...
		[PXDefault(TaxCalculationMode.TaxSetting, typeof(Search<CashAccountETDetail.taxCalcMode,
			Where<CashAccountETDetail.cashAccountID, Equal<Current<CAExpense.cashAccountID>>,
