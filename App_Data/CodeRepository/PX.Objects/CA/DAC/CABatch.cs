...
namespace PX.Objects.CA
{
	...
	/// (which corresponds to the <see cref="CABatchEntry"/> graph).
    /// </summary>
    [PXPrimaryGraph(typeof(CABatchEntry))]
	[PXCacheName(Messages.CABatch)]
    [Serializable]
	public partial class CABatch : IBqlTable, INotable
	{
		#region Keys
		public class PK : PrimaryKeyOf<CABatch>.By<batchNbr>
		{
			public static CABatch Find(PXGraph graph, string batchNbr, PKFindOptions options = PKFindOptions.None) => FindBy(graph, batchNbr, options);
		}
        ...
        /// </summary>
		[CashAccount(null, typeof(Search<CashAccount.cashAccountID, Where2<Match<Current<AccessInfo.userName>>, And<CashAccount.clearingAccount, Equal<CS.boolFalse>>>>),
		...
		/// </summary>
		/// <value>
		/// Corresponds to the <see cref="Branch.BranchCD"/> field.
		/// </value>
		[PXString]
		[PXDBScalar(typeof(Search2<Branch.branchCD, InnerJoin<CashAccount, On<CashAccount.branchID, Equal<Branch.branchID>>>,
			Where<CashAccount.cashAccountID, Equal<CABatch.cashAccountID>>>))]
		...
		/// </summary>
		[PXDBBool]
		[PXDBDefault(typeof(Search<PaymentMethod.skipExport, Where<PaymentMethod.paymentMethodID, Equal<Current<paymentMethodID>>>>))]
		...
		#endregion
		#region Exported
		public abstract class exported : PX.Data.BQL.BqlBool.Field<exported> { }
		...
		{
			[PXDependsOnFields(typeof(hold), typeof(released), typeof(exported), typeof(canceled), typeof(voided))]
			get
			{
				...
				if (Exported.HasValue && Exported == true && Canceled.HasValue && Canceled != true && Released.HasValue && Released != true)
				{
					return CABatchStatus.Exported;
				}
				if (Canceled.HasValue && Canceled == true && Released != true)
				{
					return CABatchStatus.Canceled;
				}
				if (Released.HasValue && Released == true && Voided != true)
				{
					return CABatchStatus.Released;
				}
				if (Voided.HasValue && Voided == true)
				{
					return CABatchStatus.Voided;
				}
		...
		/// <summary>
		/// The date when the batch payments was voided.
		/// </summary>
		[PXDBDate]
		...
		public abstract class countOfPayments : PX.Data.BQL.BqlInt.Field<countOfPayments> { }
		[PXDBInt]
		[PXDefault(0, PersistingCheck = PXPersistingCheck.Nothing)]
		[PXUIField(DisplayName = "Payment Count", Visibility = PXUIVisibility.Invisible, Enabled = false)]
		...
		public CATran CopyTo(CATran destination)
		{
			...
			destination.OrigTranTypeUI = CATranType.CABatch;
	...
	public class CABatchStatus
	{
		public class ListAttribute : PXStringListAttribute
		{
			public ListAttribute() : base(
				new string[] { Balanced, Hold, Released, Exported, Canceled, Voided },
				new string[] { Messages.Balanced, Messages.Hold, Messages.Released, Messages.Exported, Messages.Canceled, Messages.Voided })
			{ }
		}
