...
using ARCCCardType = PX.Objects.AR.CCPaymentProcessing.Common.CCCardType;
	...
	public class CCBatchEnq : PXGraph
	{
		...
		private static void DoImportTransactions(IEnumerable<CCBatch> batches)
		{
			...
			foreach (CCBatch batch in batches.OrderBy(batch => batch.SettlementTimeUTC))
			{
				...
				if (PXSelect<CCBatchStatistics, Where<CCBatchStatistics.batchID, Equal<Required<CCBatch.batchID>>>>.Select(graph, batch.BatchID).Empty_())
				{
					CalculateStatistics(graph, batch);
				}
		...
		{
			var batchTransaction = PXSelectGroupBy<CCBatchTransaction,
							Where<CCBatchTransaction.batchID, Equal<Required<CCBatch.batchID>>>,
							Aggregate<GroupBy<CCBatchTransaction.procCenterCardTypeCode, GroupBy<CCBatchTransaction.settlementStatus, Sum<CCBatchTransaction.amount, Count<CCBatchTransaction.pCTranNumber>>>>>>
							.Select(graph, batch.BatchID);

			List<CCBatchStatistics> statistics = ConvertBatchTransactionsToBatchStatistics(batchTransaction);
			using (var scope = new PXTransactionScope())
			{
				foreach (CCBatchStatistics batchStatistics in statistics)
				{
					graph.CardTypeSummary.Insert(batchStatistics);
				}
				graph.Actions.PressSave();
				scope.Complete();
			}
		...
		private static List<CCBatchStatistics> ConvertBatchTransactionsToBatchStatistics(PXResultset<CCBatchTransaction> batchTransactions)
		{
			var statistics = new List<CCBatchStatistics>();

			foreach (var result in batchTransactions)
			{
				CCBatchTransaction batchTransaction = result;
				CCBatchStatistics batchStatistic = statistics.FirstOrDefault(item => item.ProcCenterCardTypeCode == batchTransaction.ProcCenterCardTypeCode);

				if (batchStatistic == null)
				{
					batchStatistic = new CCBatchStatistics
					{
					...
					SetCCBatchStatistics(result, batchTransaction, batchStatistic);
		...
		private static void SetCCBatchStatistics(PXResult<CCBatchTransaction> result, CCBatchTransaction batchTransaction, CCBatchStatistics batchStatistic)
		{
		...
		private static CCBatchStatistics TransformBatchStatistics(int? batchID, BatchStatisticsData statData)
		{
			return new CCBatchStatistics
			{
				...
				CardTypeCode = GetCardTypeCode(statData.CardType),
		...
		private static CCBatchTransaction TransformBatchTransaction(int? batchID, TransactionData tranData)
		{
			return new CCBatchTransaction
			{
				...
				CardTypeCode = GetCardTypeCode(tranData.CardTypeCode),
		...
		{
			ARCCCardType cardType = V2Converter.ConvertCardType(ccCardType);
