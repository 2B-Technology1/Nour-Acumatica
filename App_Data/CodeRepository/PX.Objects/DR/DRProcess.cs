...
namespace PX.Objects.DR
{
		...
		: PXGraph<DRProcess>, IDREntityStorage, IBusinessAccountProvider, IInventoryItemProvider
	{
		...
		protected virtual void DRScheduleDetail_TotalAmt_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (scheduleDetail.TotalAmt != null && scheduleDetail.CuryTotalAmt == null)
			{
		...
		public List<Batch> RunRecognition(List<DRRecognition.DRBatch> list, DateTime? recDate)
		{
			...
			Exceptions.Clear();
				...
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					...
					foreach (DRScheduleTran drTran in drTranList)
					{
						...
						scheduleDetail.DefAmt -= Math.Sign(tranAmt) * Math.Min(Math.Abs(tranAmt), Math.Abs(scheduleDetail.DefAmt ?? 0));
						decimal curyDefAmt = 0m;
						CM.PXCurrencyAttribute.CuryConvCury<DRScheduleDetail.curyInfoID>(ScheduleDetail.Cache, scheduleDetail, scheduleDetail.DefAmt ?? 0m, out curyDefAmt);
						scheduleDetail.CuryDefAmt = curyDefAmt;
			...
			PXDBCurrencyAttribute.SetBaseCalc<DRScheduleDetail.curyDefAmt>(ScheduleDetail.Cache, null, false);
			this.Actions.PressSave();
			PXDBCurrencyAttribute.SetBaseCalc<DRScheduleDetail.curyDefAmt>(ScheduleDetail.Cache, null, true);
		...
		public virtual void CreateSchedule(ARTran arTransaction, DRDeferredCode deferralCode, ARRegister originalDocument, decimal amount, bool isDraft)
		{
			...
			else
			{
				...
				else
				{
					...
					else if (originalDocument.OrigModule == BatchModule.SO
						&& originalDocument.DocType == ARDocType.Invoice
						&& !string.IsNullOrEmpty(originalDocument.OrigRefNbr))
					{
						if (existingSchedule == null)
						{
							DRScheduleParameters scheduleParameters = GetScheduleParameters(originalDocument, arTransaction);
		...
		public virtual void CreateSchedule(APTran tran, DRDeferredCode defCode, APInvoice document, decimal amount, bool isDraft)
		{
			...
			{
				if (document.DocType == APDocType.DebitAdj || document.DocType == APDocType.CreditAdj)
				{
					if (existingSchedule != null)
					{
						DRScheduleParameters scheduleParams = GetScheduleParameters(document, tran);
					...
					else
					{
						...
						CreateRelatedSchedule(scheduleParams, tran.BranchID, tran.DefScheduleID,
							amount, defCode, inventoryItem, tran.AccountID, tran.SubID, isDraft, document.DocType == APDocType.DebitAdj);
					}
				}
			}
		}
		...
		{
			return InsertScheduleDetail(branchID, sc, component, compItem, defCode, amount, defAcctID, defSubID, isDraft, null, null);
		}

		protected DRScheduleDetail InsertScheduleDetail(int? branchID, DRSchedule sc, INComponent component, InventoryItem compItem, DRDeferredCode defCode, decimal amount, int? defAcctID, int? defSubID, bool isDraft,
			DateTime? termStartDate, DateTime? termEndDate)
		{
		...
		{
			return InsertScheduleDetail(branchID, sc, componentID, defCode, amount, defAcctID, defSubID, acctID, subID, isDraft, null, null);
		}
			...
			DateTime? termStartDate , DateTime? termEndDate)
		{
			...
			sd.IsCustom = false;
			sd.TermStartDate = termStartDate;
			sd.TermEndDate = termEndDate;
