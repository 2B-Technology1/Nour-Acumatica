...
namespace PX.Objects.DR
{
	...
	/// of the document line (such as <see cref="ARTran"/>) that has a 
	...
	public partial class DRSchedule : PX.Data.IBqlTable
	{
		...
		{
			public static DRSchedule Find(PXGraph graph, int? scheduleID, PKFindOptions options = PKFindOptions.None) => FindBy(graph, scheduleID, options);
		}
		public class UK : PrimaryKeyOf<DRSchedule>.By<scheduleNbr>
		{
			public static DRSchedule Find(PXGraph graph, string scheduleNbr, PKFindOptions options = PKFindOptions.None) => FindBy(graph, scheduleNbr, options);
		}
		...
		[PXFormula(typeof(Default<DRSchedule.documentType>))]
		[PXForeignReference(typeof(Field<bAccountID>.IsRelatedTo<BAccountR.bAccountID>))]
		...
		[PXUnboundDefault(CR.BAccountType.CustomerType, PersistingCheck=PXPersistingCheck.Nothing)]
