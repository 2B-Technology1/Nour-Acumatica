...
namespace PX.Objects.DR
{
	public class SingleScheduleCreator
	{
		...
		private void CreateScheduleDetails(DRProcess.DRScheduleParameters scheduleParameters, Amount lineTotal)
		{
			...
			if (!_isDraft)
			{
				foreach (PXResult<DRScheduleDetail, DRDeferredCode, INComponent> detail in _dataProvider.GetScheduleDetailsResultset(_schedule.ScheduleID))
				{
					DRScheduleDetail scheduleDetail = detail;
					DRDeferredCode deferralCode = detail;
					INComponent component = detail;

					//residual component without DefCode should be recognized immediately, it doesn't need CreditLineTransaction
					if (deferralCode.MultiDeliverableArrangement != true || component.AmtOptionASC606 != INAmountOptionASC606.Residual
						|| deferralCode != null && deferralCode.DeferredCodeID != null)
					{
