	...
	public class TransactionsGenerator
	{
			...
			ref short lineCounter)
		{
			...
			foreach (DRScheduleTran originalTransaction in originalTransactions)
			{
				...
				DateTime? maxDate;
				string maxPeriod;
				DRSetup dRSetup = new PXSelect<DRSetup>(_graph).Select();

				if (relatedScheduleDetail.Module == BatchModule.AR
					&& PXAccess.FeatureInstalled<FeaturesSet.aSC606>()
					&& dRSetup.RecognizeAdjustmentsInPreviousPeriods == true
					&& DeferredMethodType.RequiresTerms(_code)
					&& _code.RecognizeInPastPeriods == true)
				{
					maxDate = originalTransaction.RecDate;
					maxPeriod = originalTransaction.FinPeriodID;
				}
				else if (_code.StartOffset < 0)
				{
					maxDate = originalTransaction.RecDate;
					maxPeriod = originalTransaction.FinPeriodID;
				}
				...
				{
					maxDate = originalTransaction.RecDate.Value < relatedScheduleDetail.DocDate.Value
								? relatedScheduleDetail.DocDate
								: originalTransaction.RecDate;
					maxPeriod = string.CompareOrdinal(originalTransaction.FinPeriodID, relatedScheduleDetail.FinPeriodID) < 0
