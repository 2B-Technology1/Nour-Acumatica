...
namespace PX.Objects.DR
{
	...
	public class DRRecognition: PXGraph<DRRecognition>
	{
		...
		protected virtual PXFilterRow[] AlterSheduledFilters(PXView.PXFilterRowCollection filters)
		{
			List<PXFilterRow> newFilters = new List<PXFilterRow>();
			foreach (PXFilterRow filter in filters)
			{
				if (filter.DataField == nameof(ScheduledTran.DocType))
				{
					if (filter.Value?.ToString() == DRScheduleDocumentType.Invoice)
					{
						filter.Value = ARDocType.Invoice;
					}
					else if (filter.Value?.ToString() == DRScheduleDocumentType.Bill)
					{
						filter.Value = AP.APDocType.Invoice;
					}
				}
		...
		public virtual IEnumerable items()
		{
			...
			IEnumerable drTrans = hasCashReceipt ? select.View.SelectMulti()
				: GraphHelper.QuickSelect(select.View.Graph, select.View.BqlSelect, AlterSheduledFilters(PXView.Filters));
		...
		public partial class ScheduledTranRec : IBqlTable
		{
			...
			[Account(DisplayName = "Account", Visibility = PXUIVisibility.Visible,
				...
				BqlField = typeof(DRScheduleTran.accountID))]
