	...
	public partial class ReceivePutAway : WMSBase
	{
		public sealed class PutAwayMode : ScanMode
		{
			...
			public class Logic : ScanExtension
			{
				...
				public Logic() => _transferEntryLazy = Lazy.By(CreateTransferEntry);
				...
				public
					...
					View PutAway;
				public virtual IEnumerable putAway() => Basis.SortedResult(Basis.GetSplits(Basis.Receipt));
				...
				private INTransferEntry CreateTransferEntry()
				{
					...
					Basis.Graph.Caches[typeof(SiteStatusByCostCenter)] = transferEntry.Caches[typeof(SiteStatusByCostCenter)];
					Basis.Graph.Caches[typeof(LocationStatusByCostCenter)] = transferEntry.Caches[typeof(LocationStatusByCostCenter)];
					Basis.Graph.Caches[typeof(LotSerialStatusByCostCenter)] = transferEntry.Caches[typeof(LotSerialStatusByCostCenter)];
					...
					Basis.Graph.Views.Caches.Remove(typeof(SiteStatusByCostCenter));
					Basis.Graph.Views.Caches.Remove(typeof(LocationStatusByCostCenter));
					Basis.Graph.Views.Caches.Remove(typeof(LotSerialStatusByCostCenter));
				...
				/// Overrides <see cref="BarcodeDrivenStateMachine{TSelf, TGraph}.HandleScan(string)"/>
				[PXOverride]
				public virtual bool HandleScan(string barcode, Func<string, bool> base_HandleScan)
				{
					using (Basis.Header?.Mode == PutAwayMode.Value ? GetReleaseModeScope() : null)
						return base_HandleScan(barcode);
				}

				private IDisposable GetReleaseModeScope()
					=> Graph.FindImplementation<IItemPlanHandler<POReceiptLineSplit>>().ReleaseModeScope();
			}
			#endregion
			...
			public new sealed class ReceiptState : ReceivePutAway.ReceiptState
			{
				protected override POReceipt GetByBarcode(string barcode)
				{
					POReceipt receipt =
						...
						LeftJoin<AP.Vendor>.On<POReceipt.vendorID.IsEqual<AP.Vendor.bAccountID>>.SingleTableOnly.
						Where<
							...
							And<
								AP.Vendor.bAccountID.IsNull.
								Or<Match<AP.Vendor, AccessInfo.userName.FromCurrent>>>>.
			...
			public sealed class TargetLocationState : LocationState
			{
				...
				protected override bool IsStateSkippable() => Basis.PutAwayToLocationID != null;

				protected override void ReportSuccess(INLocation location) => Basis.Reporter.Info(Msg.Ready, location.LocationCD);

				protected override void Apply(INLocation location) => Basis.PutAwayToLocationID = location.LocationID;
				protected override void ClearState() => Basis.PutAwayToLocationID = null;
			...
			public sealed class LotSerialState : WMSBase.LotSerialState
			{
				protected override Validation Validate(string lotSerial)
				{
					...
					if (Get<PutAwayMode.Logic>().PutAway.SelectMain().All(t => !string.Equals(t.LotSerialNbr, lotSerial, StringComparison.OrdinalIgnoreCase)))
					...
					public virtual FlowStatus SyncWithTransfer(POReceiptLineSplit receivedSplit, decimal qty)
					{
						...
						INTranSplit[] linkedSplits = isNewDocument
							...
							: SelectFrom<POReceiptSplitToTransferSplitLink>.
								...
								View.ReadOnly.SelectMultiBound(Basis, new object[] { receivedSplit }, transfer.Current.RefNbr, Basis.PutAwayToLocationID)
								.RowCast<INTranSplit>()
								.ToArray();
						...
						if (existingINSplits.Length == 0)
						{
							...
							tran.ToLocationID = Basis.PutAwayToLocationID;
							...
							if (tranSplit == null)
							{
								...
								tranSplit.ToLocationID = Basis.PutAwayToLocationID;
						...
						{
							tranSplit = existingINSplits.FirstOrDefault(s => string.Equals(s.LotSerialNbr, Basis.LotSerialNbr ?? s.LotSerialNbr, StringComparison.OrdinalIgnoreCase));
							if (tranSplit != null)
							{
			...
			public sealed class ReleaseTransferCommand : ScanCommand
			{
				...
				public class Logic : ScanExtension
				{
					...
					private static void ReleaseTransferImpl(INRegister transfer)
					{
						...
						te.transfer.Current = INRegister.PK.Find(te, transfer, PKFindOptions.IncludeDirty);
			...
			public class ToLocationID : FieldAttached.To<POReceiptLineSplit>.AsInteger
			{
				...
				protected override PXFieldState AdjustStateByRow(PXFieldState state, POReceiptLineSplit row)
				{
					...
					else
					{
						...
						{
							state = (PXFieldState)tranCache.GetStateExt<INTran.toLocationID>(null);
							state.Value = Base.WMS.Localize(IN.Messages.EmptyLS);
						}
					}
