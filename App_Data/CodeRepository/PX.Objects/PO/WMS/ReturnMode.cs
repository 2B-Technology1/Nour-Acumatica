		...
		public sealed class ReturnMode : ScanMode
		{
			...
			public class Logic : ScanExtension
			{
				...
				public
					...
					View Returned;
				public virtual IEnumerable returned() => Basis.SortedResult(Basis.GetSplits(Basis.Receipt, includeUnassigned: true));

				public
					SelectFrom<POReceiptLineSplit>.
					InnerJoin<POReceiptLine>.On<POReceiptLineSplit.FK.ReceiptLine>.
					View ReturnedNotZero;
				public virtual IEnumerable returnedNotZero() => Basis.SortedResult(Basis.GetSplits(Basis.Receipt, includeUnassigned: true).Where(s => ((POReceiptLineSplit)s).ReceivedQty > 0));
				#endregion
				...
				protected override POReceipt GetByBarcode(string barcode)
				{
					POReceipt returnReceipt =
						...
						Where<
							...
							And<POReceipt.receiptType.IsEqual<POReceiptType.poreturn>>.
							And<
