...
namespace PX.Objects.PO
{
	...
	public class POCreate : PXGraph<POCreate>
	{
		...
		public POCreate()
		{
			...
			PXUIFieldAttribute.SetDisplayName<INPlanType.localizedDescr>(this.Caches[typeof(INPlanType)], Messages.PlanTypeDescr);
		...
		public virtual void EnumerateAndPrepareFixedDemandRow(PXResult<POFixedDemand> rec)
		{
			...
			var soline = PXResult.Unwrap<SOLine>(rec);

			demand.CuryID = vendor.CuryID;

			if (demand.IsSpecialOrder == true)
			{
				demand.CuryID = soorder?.CuryID;
				demand.EffPrice = demand.CuryUnitCost;
			}
			else if (demand?.InventoryID != null && demand.DemandUOM != null && demand.VendorID != null && vendor?.CuryID != null &&
			...
			if (FixedDemand.Locate(demand) == null)
			{
				FixedDemand.Cache.SetStatus(demand, PXEntryStatus.Notchanged); // It's necessary because we use PXFieldScope and the platform doesn't add the rows to the cache.
			}
		}
		...
		protected virtual void POFixedDemand_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (row.IsSpecialOrder == true)
			{
				var message = GetSpecialOrderCurrencyError(row, false);

				if (PXUIFieldAttribute.GetErrorOnly<POFixedDemand.vendorID>(sender, row) == null)
					PXUIFieldAttribute.SetWarning<POFixedDemand.vendorID>(sender, row, message);
			}
		}
		...
		protected virtual string GetSpecialOrderCurrencyError(POFixedDemand row, bool onlyErrors)
		{
			...
			var vendor = Vendor.PK.Find(this, row.VendorID);
			var curyID = vendor?.CuryID;
			if (curyID == null)
			{
				int? branchID = PXAccess.FeatureInstalled<FeaturesSet.multipleBaseCurrencies>() ? Filter.Current.BranchID : null;
				curyID = GetBaseCuryID(branchID ?? Accessinfo.BranchID);
			}
			...
			if (curyID != row.CuryID)
			{
				...
				{
					message = Messages.CannotCreatePOOrderDueToVendorCurrency; 
				}
				else if (onlyErrors != true)
				{
					message = Messages.CreatedPOOrderWillHaveSOOrderCurrency;
				}
			}
		...
		protected virtual void POFixedDemand_RecordID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (demand == null || Filter.Current == null || demand.IsSpecialOrder == true)
				return;

			decimal? vendorUnitCost = null;

			if (demand.InventoryID != null && demand.DemandUOM != null && demand.VendorID != null && Filter.Current.PurchDate != null)
			{
				...
				if (vendor?.CuryID != null)
				{
					vendorUnitCost = APVendorPriceMaint.CalculateCuryUnitCost(sender, demand.VendorID, demand.VendorLocationID, demand.InventoryID,
																			  demand.SiteID, vendor.CuryID, demand.DemandUOM, demand.OrderQty,
		...
		public virtual PXRedirectRequiredException CreatePOOrders(List<POFixedDemand> list, DateTime? PurchDate, bool extSort, int? branchID = null)
		{
			...
			foreach (POFixedDemand demand in list)
			{
				...
				{
					PXProcessing<POFixedDemand>.SetWarning(Messages.MissingVendorOrLocation);
					continue;
				}
				...
				try
				{
					var message = GetSpecialOrderCurrencyError(demand, true);
					...
					if (docgraph.Transactions.Cache.IsInsertedUpdatedDeleted)
					{
						...
						{
							PXProcessing<POFixedDemand>.SetInfo(PXMessages.LocalizeFormatNoPrefixNLA(Messages.PurchaseOrderCreated, docgraph.Document.Current.OrderNbr) + "\r\n" + ErrorText);
						}
						else
						{
							PXProcessing<POFixedDemand>.SetWarning(PXMessages.LocalizeFormatNoPrefixNLA(Messages.PurchaseOrderCreated, docgraph.Document.Current.OrderNbr) + "\r\n" + ErrorText);
						}
		...
		protected virtual POOrder FindOrCreatePOOrder(DocumentList<POOrder> created, POOrder previousOrder, POFixedDemand demand, SOOrder soorder, SOLineSplit3 soline, bool requireSingleProject)
		{
			...
			if (OrderType == POOrderType.RegularOrder)
			{
				...
				{
					int? project = demand.DemandProjectID ?? PM.ProjectDefaultAttribute.NonProject();
