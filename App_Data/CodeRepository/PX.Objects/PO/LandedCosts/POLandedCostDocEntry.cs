...
namespace PX.Objects.PO
{
	...
	public class POLandedCostDocEntry : PXGraph<POLandedCostDocEntry, POLandedCostDoc>
	{
		...
		public POLandedCostDocEntry()
		{
			...
			OpenPeriodAttribute.SetValidatePeriod<POLandedCostDoc.finPeriodID>(this.Document.Cache, null, PeriodValidation.DefaultSelectUpdate);
			...
			TaxAttribute.IncludeDirectTaxLine<POLandedCostDetail.taxCategoryID>(Details.Cache, null, true);
		}
		...
		protected virtual void _(Events.RowSelected<POLandedCostReceiptLine> e)
		{
			if (e.Row == null)
				return;

			if (e.Cache.GetStatus(e.Row) == PXEntryStatus.Inserted && e.Row.IsStockItem != null)
			{
				InventoryItem item = InventoryItem.PK.Find(this, e.Row.InventoryID);
				if (item?.IsConverted == true && item.StkItem != e.Row.IsStockItem)
				{
					var inventoryCD = item.InventoryCD.Trim();
					var message = item.StkItem == true ? Messages.CannotCreateLandedCostConvertedStock : Messages.CannotCreateLandedCostConvertedNonStock;

					e.Cache.RaiseExceptionHandling<POReceiptLine.inventoryID>(e.Row, inventoryCD, new PXSetPropertyException(message, inventoryCD));
				}
			}
		}
		...
		public virtual IEnumerable CreateAPInvoice(PXAdapter adapter)
		{
			if (Document.Current != null)
			{
				...
				if (landedCostDoc.Released == true)
				{
					...
					var billHeader = factory.CreateLandedCostBillHeader(landedCostDoc, details, new APInvoice());
					var currentBill = new APInvoice()
					{
						CuryInfoID = newCuryInfo.CuryInfoID,
						CuryID = newCuryInfo.CuryID,
						BranchID = billHeader.BranchID,
						DocType = billHeader.DocType,
						DocDate = billHeader.DocDate
					};
					...
					apGraph.Caches<POLandedCostDoc>().MarkUpdated(landedCostDoc, assertError: true);
		...
		public virtual void AddPurchaseReceiptLines(IEnumerable<POReceiptLineAdd> lines)
		{
			...
			foreach (var line in newLines)
			{
				...
				landedCostReceiptLine = ReceiptLines.Insert(landedCostReceiptLine);

				landedCostReceiptLine.IsStockItem = line.IsStockItem;
			}
		}
		...
		public virtual void ReleaseDoc(POLandedCostDoc doc)
		{
			...
			ValidateReceiptLines(receiptLines);
		...
		{
			foreach (var detail in details)
				ConvertedInventoryItemAttribute.ValidateRow(ReceiptLines.Cache, detail);
		}
			...
			IEnumerable<LandedCostAllocationService.POLandedCostReceiptLineAdjustment> adjustments)
		{
			...
			var forReleaseIN = new List<INRegister>();
				...
				foreach (var inAdjustmentPair in inAdjustmentsDictionary)
				{
					...
					{
						inGraph.CostCenterDispatcherExt?.SetCostLayerType(inTran);
			...
			IEnumerable<POLandedCostSplit> splits, IEnumerable<POLandedCostTaxTran> taxes)
		{
			...
			var billHeader = apBillFactory.CreateLandedCostBillHeader(doc, details, new APInvoice());
			var currentBill = new APInvoice()
			{
				CuryInfoID = newCuryInfo.CuryInfoID,
				CuryID = newCuryInfo.CuryID,
		...
		protected virtual APTaxTran InsertAPTaxTran(APInvoiceEntry apGraph, APTaxTran newTax)
		{
			...
			taxTran.CuryTaxAmtSumm = newTax.CuryTaxAmt;
			taxTran.NonDeductibleTaxRate = newTax.NonDeductibleTaxRate;
			taxTran.CuryExpenseAmt = newTax.CuryExpenseAmt;
