...
namespace PX.Objects.PO
{
	...
	[Serializable]
	public partial class POLineUOpen : IBqlTable, ISortOrder, ICommitmentSource, IItemPlanPOSource
	{
		#region Keys
		public class PK : PrimaryKeyOf<POLineUOpen>.By<orderType, orderNbr, lineNbr>
		{
			public static POLineUOpen Find(PXGraph graph, string orderType, string orderNbr, int? lineNbr, PKFindOptions options = PKFindOptions.None) => FindBy(graph, orderType, orderNbr, lineNbr, options);
		}
		...
		[PXUnboundFormula(
			typeof(Switch<Case<Where<POLine.lineType, In3<POLineType.goodsForDropShip, POLineType.nonStockForDropShip>,
				And<POLine.completed, Equal<False>>>, int1>, int0>),
			typeof(SumCalc<POOrder.dropShipOpenLinesCntr>))]
		...
		[PXFormula(typeof(Switch<Case<Where<POLineUOpen.completed, Equal<True>, Or<POLineUOpen.cancelled, Equal<True>>>, decimal0>,
			Maximum<Sub<POLineUOpen.orderQty, Maximum<POLineUOpen.completedQty, POLineUOpen.orderedQty>>, decimal0>>),
		...
		[PXFormula(typeof(Switch<Case<Where<POLineUOpen.closed, Equal<True>, Or<POLineUOpen.cancelled, Equal<True>>>, decimal0>,
			Maximum<Sub<Maximum<POLineUOpen.orderQty, Maximum<POLineUOpen.completedQty, POLineUOpen.orderedQty>>, POLineUOpen.billedQty>, decimal0>>),
