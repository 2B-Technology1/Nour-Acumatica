...
namespace PX.Objects.PO
{
	...
	public partial class POLine : PX.Data.IBqlTable, ISortOrder, ITaxableDetail, IMatrixItemLine, ICommitmentSource, IItemPlanPOSource
	{
		#region Keys
		public class PK : PrimaryKeyOf<POLine>.By<orderType, orderNbr, lineNbr>
		{
			public static POLine Find(PXGraph graph, string orderType, string orderNbr, int? lineNbr, PKFindOptions options = PKFindOptions.None) => FindBy(graph, orderType, orderNbr, lineNbr, options);
		}
		public static class FK
		{
			...
			public class Task : PMTask.PK.ForeignKeyOf<POLine>.By<projectID, taskID> { }
		...
		[PXDefault(typeof(
			Switch<
				Case<Where<
					POLine.orderType, Equal<POOrderType.projectDropShip>>,
					False,
				Case<Where<
					InventoryItem.postToExpenseAccount.FromSelectorOf<POLine.inventoryID>.
					IfNullThen<InventoryItem.postToExpenseAccount.purchases>.
					IsEqual<InventoryItem.postToExpenseAccount.purchases>>,
					False>>,
				True>))]
		...
		#region OrderedQty
		[PXDBQuantity(typeof(POLine.uOM), typeof(POLine.baseOrderedQty), HandleEmptyKey = true, MinValue = 0)]
		[PXDefault(TypeCode.Decimal, "0.0")]
		...
		[PXDBBool()]
		[PXDefault(false)]
		[PXUIField(DisplayName = "Special Order", FieldClass = FeaturesSet.specialOrders.FieldClass, Enabled = false)]
		[PXUnboundFormula(typeof(IIf<Where<isSpecialOrder, Equal<True>>, int1, int0>), typeof(SumCalc<POOrder.specialLineCntr>))]
		...
		[PXFormula(typeof(Switch<Case<Where<POLine.completed, Equal<True>, Or<POLine.cancelled, Equal<True>>>, decimal0>,
			Maximum<Sub<POLine.orderQty, Maximum<POLine.completedQty, POLine.orderedQty>>, decimal0>>),
		...
		[PXFormula(typeof(Switch<Case<Where<POLine.closed, Equal<True>, Or<POLine.cancelled, Equal<True>>>, decimal0>,
			Maximum<Sub<Maximum<POLine.orderQty, Maximum<POLine.completedQty, POLine.orderedQty>>, POLine.billedQty>, decimal0>>),
		...
		[PXFormula(typeof(Maximum<Sub<orderQty, receivedQty>, decimal0>))]
		...
		#region NonOrderedQty
		[PXQuantity]
		[PXFormula(typeof(openQty))]
		[PXUIField(DisplayName = "Blanket Open Qty.", Enabled = false)]
		public virtual decimal? NonOrderedQty { get; set; }
		public abstract class nonOrderedQty : BqlDecimal.Field<nonOrderedQty> { }
		#endregion
		#endregion
	...
	public static class POLineType
	{
		...
		public static bool IsStockNonDropShip(string lineType)
			=> IsStock(lineType) && !IsDropShip(lineType) && !IsProjectDropShip(lineType);

		public static bool IsNonStockNonServiceNonDropShip(string lineType)
			=> IsNonStock(lineType) && !IsService(lineType) && !IsDropShip(lineType) && !IsProjectDropShip(lineType);
	}
