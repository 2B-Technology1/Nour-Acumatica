...
namespace PX.Objects.PO.GraphExtensions.POReceiptEntryExt
{
	public class UpdatePOOnRelease: BaseUpdatePOAccrual<POReceiptEntry.MultiCurrency, POReceiptEntry, POReceipt>
	{
		...
		public virtual POLineUOpen UpdatePOLineOnReceipt(PXResult<POReceiptLine> res, POLineUOpen poLine)
		{
			...
			if (line.InventoryID != null && !String.IsNullOrEmpty(line.UOM) && !String.IsNullOrEmpty(poLine.UOM) && !string.Equals(line.UOM, poLine.UOM, StringComparison.OrdinalIgnoreCase))
			{
		...
		{
			if (rctLine.IsKit == true && rctLine.IsStockItem != true)
			{
			...
			Base.transactions.Cache.MarkUpdated(rctLine, assertError: true);
		}
