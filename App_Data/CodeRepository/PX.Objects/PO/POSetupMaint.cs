...
namespace PX.Objects.PO
{
	public class POSetupMaint : PXGraph<POSetupMaint>
	{
		...
		protected virtual void _(Events.RowSelected<POReceivePutAwaySetup> e)
		{
			if (e.Row != null && e.Row.DefaultLotSerialNumber == true && e.Row.DefaultExpireDate == false)
			{
				e.Cache.RaiseExceptionHandling<POReceivePutAwaySetup.defaultExpireDate>(
					e.Row, e.Row.DefaultExpireDate,
					new PXException(WMS.ReceivePutAway.Msg.SetupLotSerialFlagsInconsistency,
						PXUIFieldAttribute.GetDisplayName<POReceivePutAwaySetup.defaultExpireDate>(e.Cache),
						PXUIFieldAttribute.GetDisplayName<POReceivePutAwaySetup.defaultLotSerialNumber>(e.Cache)));
			}
			else
				e.Cache.RaiseExceptionHandling<POReceivePutAwaySetup.defaultExpireDate>(e.Row, e.Row?.DefaultExpireDate, null);
		}
	}
}
