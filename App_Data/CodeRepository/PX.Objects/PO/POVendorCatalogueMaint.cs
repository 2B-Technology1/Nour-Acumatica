...
{
	[PXProjection(typeof(
		SelectFrom<CRLocation>.
		InnerJoin<Vendor>.On<Vendor.bAccountID.IsEqual<CRLocation.bAccountID>>.
		Where<
			Vendor.type.IsNotEqual<BAccountType.employeeType>.
			And<
				Vendor.vStatus.IsEqual<VendorStatus.active>.
				Or<Vendor.vStatus.IsEqual<VendorStatus.oneTime>>>>))]
	[PXCacheName(Messages.VendorLocation)]
	public class VendorLocation : IBqlTable
	{		
		#region BAccountID
	...
	public class POVendorCatalogueMaint : PXGraph<POVendorCatalogueMaint>, PXImportAttribute.IPXPrepareItems	
	{
        ...
        [PXUIField(DisplayName = Messages.ShowVendorPrices, MapEnableRights = PXCacheRights.Select)]
        [PXButton]
