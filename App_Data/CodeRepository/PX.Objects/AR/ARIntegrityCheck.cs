	...
	public class ARIntegrityCheck : PXGraph<ARIntegrityCheck>
	{
			...
			else
			{
				ARCustomerList.SetProcessDelegate<ARReleaseProcess>(
					delegate (ARReleaseProcess arReleaseGraph, Customer cust)
					{
						...
						ReopenDocumentsHavingPendingApplications(arReleaseGraph, cust, filter.FinPeriodID);
					}
				);
		...
		protected virtual void IntegrityCheckProc(Customer cust, ARIntegrityCheckFilter filter)
		{
			...
			if (filter.RecalcDocumentBalances == true)
			{
				...
				ReopenDocumentsHavingPendingApplications(this, cust, finPeriod);
			}
		...
		protected virtual void UpdateDocumentBalances(Customer cust, string finPeriod)
		{
			...
			PXUpdateJoin<
					...
					Set<ARRegister.curyRetainageUnpaidTotal,
						ARRegisterTranPostGLGrouped.calcCuryRetainageUnpaidTotal,
					Set<ARRegister.retainageUnpaidTotal,
		...
		private static void ReopenDocumentsHavingPendingApplications(PXGraph graph, Customer customer, string finPeriod)
		{
			PXUpdate<Set<ARRegister.openDoc, True>,
				ARRegister,
				Where<ARRegister.openDoc, Equal<False>,
					And<ARRegister.customerID, Equal<Required<ARRegister.customerID>>,
					And<ARRegister.tranPeriodID, GreaterEqual<Required<ARRegister.tranPeriodID>>,
					And<Exists<Select<ARAdjust, Where<
						...
						>>>>>>>>
				.Update(graph, customer.BAccountID, finPeriod);


			PXUpdate<Set<ARRegister.status, ARDocStatus.open>,
				ARRegister,
				Where<ARRegister.status, Equal<ARDocStatus.closed>,
					And<ARRegister.customerID, Equal<Required<ARRegister.customerID>>,
					And<ARRegister.tranPeriodID, GreaterEqual<Required<ARRegister.tranPeriodID>>,
