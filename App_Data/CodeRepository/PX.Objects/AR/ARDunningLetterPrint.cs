...
namespace PX.Objects.AR
{
    public class ARDunningLetterPrint : PXGraph<ARDunningLetterPrint>
    {
        ...
        public ARDunningLetterPrint()
        {
			...
			PXUIFieldAttribute.SetVisible<PrintParameters.orgBAccountID>(Filter.Cache, null, setup.PrepareDunningLetters != AR.ARSetup.prepareDunningLetters.ConsolidatedForAllCompanies);
		}
        ...
        protected virtual IEnumerable details()
        {
			...
			var par = new List<object>() { header.BeginDate, header.EndDate };

			ARSetup arPrefs = ARSetup.Current;
			bool allowFilterByBranch = arPrefs.PrepareDunningLetters != AR.ARSetup.prepareDunningLetters.ConsolidatedForAllCompanies;
			if (allowFilterByBranch && header.OrgBAccountID != null)
			{
				cmd.WhereAnd<Where<ARDunningLetter.branchID,
						Inside<Required<PrintParameters.orgBAccountID>>>>();
				par.Add(header.OrgBAccountID);
			}

			foreach (PXResult<ARDunningLetter, ARDunningLetterDetail> res in cmd.Select(par.ToArray()))
            {
				...
				row.DunningFee = dunningLetter.DunningFee;
				row.CuryID = PXAccess.GetBranch(dunningLetter.BranchID)?.BaseCuryID;
			...
			if (row != null)
            {
				...
				switch (row.Action)
                {
					case ActionTypes.Print:
                        Details.SetAsyncProcessDelegate((list, ct) => Print(filter, list, false, ct));
                        break;
					...
					case ActionTypes.MarkDontPrint:
                        Details.SetAsyncProcessDelegate((list, ct) => Print(filter, list, true, ct));
                        break;
		...
		protected virtual void _(Events.FieldDefaulting<PrintParameters.orgBAccountID> e)
		{
			e.NewValue = null;
			e.Cancel = true;
		}
        ...
        public static async  System.Threading.Tasks.Task Print(PrintParameters filter, List<DetailsResult> list, bool markOnly, CancellationToken cancellationToken)
        {
			bool failed = false;
    ...
    public class ARDunningLetterUpdate : PXGraph<ARDunningLetterUpdate, Customer>
	{
		...
		public virtual void EMailDL(int dunningLetterID, bool markOnly, bool showAll)
        {
            ...
            else
            {
				...
				this.GetExtension<ARDunningLetterUpdate_ActivityDetailsExt>().SendNotification(ARNotificationSource.Customer, notificationCD, dunningLetter.BranchID, new Dictionary<string, string>() { { "DunningLetterID", dunningLetterID.ToString() } }, true);
