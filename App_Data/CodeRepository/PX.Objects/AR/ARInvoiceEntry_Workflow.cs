...
namespace PX.Objects.AR
{
	...
	public class ARInvoiceEntry_Workflow : PXGraphExtension<ARInvoiceEntry>
	{
		...
		public class Conditions : Condition.Pack
		{
			...
			public Condition IsEmailedRequired => GetOrCreate(c =>
			...
			public Condition IsPendingPayment => GetOrCreate(c => c.FromBql<
				pendingPayment.IsEqual<True>
			>());
			...
			public Condition IsPrepaymentInvoiceReversing => GetOrCreate(c => c.FromBql<
				docType.IsEqual<ARDocType.creditMemo>.And<origDocType.IsEqual<ARDocType.prepaymentInvoice>>
			>());
				...
				screen
					...
					.AddDefaultFlow(flow =>
						flow
							.WithFlowStates(fss =>
							{
									...
									seq.WithStates(sss =>
										{
											...
											{
												return flowState
													.IsSkippedWhen(conditions.IsEmailed || !conditions.IsEmailedRequired)
													.WithActions(actions =>
													{
														actions.Add(g => g.putOnHold);
														actions.Add(g => g.createSchedule);
								...
								{
									return flowState
										.WithActions(actions => { actions.Add(g => g.customerDocuments); });
								});
								fss.Add<State.voided>(flowState =>
								{
									return flowState
										.WithActions(actions => { actions.Add(g => g.sendEmail); })
										.WithActions(actions => { actions.Add(g => g.customerDocuments); })
										.WithActions(actions => { actions.Add(g => g.printARRegister); });
								});
							})
							.WithTransitions(transitions =>
								{
									...
									transitions.AddGroupFrom<State.HoldToBalance>(ts =>
									{
										...
										ts.Add(t => t
											.To<State.pendingPayment>()
											.IsTriggeredOn(g => g.OnReleaseDocument)
											.When(conditions.IsPrepaymentInvoice && conditions.IsOpen)
											.WithFieldAssignments(fas =>
											{
												fas.Add<pendingPayment>(e => e.SetFromValue(true));
											}));
										ts.Add(t => t
											.To<State.open>()
											.IsTriggeredOn(g => g.OnReleaseDocument)
									...
									transitions.AddGroupFrom<State.pendingPayment>(ts =>
									{
										ts.Add(t => t
											.To<State.open>()
											.IsTriggeredOn(g => g.OnReleaseDocument)
											...
											.WithFieldAssignments(fas =>
											{
												fas.Add<pendingPayment>(e => e.SetFromValue(false));
											}));
										ts.Add(t => t
											.To<State.open>()
					...
					.WithActions(actions =>
					{
						...
						actions.Add(g => g.releaseFromCreditHold, c => c
							...
							.IsHiddenWhen(conditions.IsCreditMemo || conditions.IsPrepaymentInvoice)
						...
						actions.Add(g => g.printInvoice, c => c
							...
							.IsDisabledWhen(conditions.IsPrepaymentInvoiceReversing)
						...
						actions.Add(g => g.reverseInvoice, c => c
							...
							.IsDisabledWhen(conditions.IsPrepaymentInvoice && !conditions.IsPendingPayment || conditions.IsPrepaymentInvoiceReversing)
							.IsHiddenWhen(conditions.IsFinCharge || conditions.IsSmallCreditWO));
						actions.Add(g => g.reverseInvoiceAndApplyToMemo, c => c
							.WithCategory(correctionsCategory)
							.IsDisabledWhen(conditions.IsPrepaymentInvoiceReversing)
							.IsHiddenWhen(conditions.IsFinCharge || conditions.IsPrepaymentInvoice));
						actions.Add(g => g.customerRefund, c => c
							.WithCategory(processingCategory)
							.IsDisabledWhen(conditions.IsPrepaymentInvoiceReversing)
							.IsHiddenWhen(conditions.IsNotCreditMemo || conditions.IsPrepaymentInvoice));
						actions.Add(g => g.writeOff, c => c
							.WithCategory(correctionsCategory)
							.IsDisabledWhen(conditions.IsPrepaymentInvoiceReversing)
							.IsHiddenWhen(conditions.IsMigrationMode || conditions.IsPrepaymentInvoice));
						...
						actions.Add(g => g.recalculateDiscountsAction, c => c
							.WithCategory(customOtherCategory)
							.IsDisabledWhen(conditions.IsNotAllowRecalcPrice || conditions.IsPrepaymentInvoiceReversing)
							.IsHiddenWhen(conditions.IsMigrationMode || conditions.IsPrepaymentInvoice));
						...
						actions.Add(g => g.sendEmail, c => c
							.IsDisabledWhen(conditions.IsPrepaymentInvoiceReversing && conditions.IsClosed)
							.WithCategory(customOtherCategory));

						actions.Add(g => g.customerDocuments, c => c.WithCategory(PredefinedCategory.Inquiries));
						actions.Add(g => g.sOInvoice,
							c => c.WithCategory(relatedDocumentsCategory)
								.IsHiddenWhen(conditions.IsFinCharge || conditions.IsSmallCreditWO || conditions.IsARInvoice || conditions.IsPrepaymentInvoice));
