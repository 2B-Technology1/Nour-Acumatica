...
namespace PX.Objects.AR
{
	...
	public class ARDocumentEnq : PXGraph<ARDocumentEnq>
	{
			...
			public abstract class refNbr : PX.Data.BQL.BqlString.Field<refNbr>
		{
			...
			[Account(typeof(branchID), DisplayName = "Prepayment Account",
				BqlTable = typeof(ARRegister))]
			...
			[PXDefault(PersistingCheck = PXPersistingCheck.Nothing)]
			[SubAccount(typeof(prepaymentAccountID), DescriptionField = typeof(Sub.description), DisplayName = "Prepayment Subaccount", Visibility = PXUIVisibility.Visible,
				BqlTable = typeof(ARRegister))]
		...
		public partial class ARDocumentPeriodResult : IBqlTable
		{
			...
			[PXDefault]
			[Account(typeof(branchID), DisplayName = "Account",
			...
			[PXDefault]
			[SubAccount(typeof(accountID), DescriptionField = typeof(Sub.description), DisplayName = "Subaccount", Visibility = PXUIVisibility.Visible,
				BqlTable = typeof(ARTranPostGL))]
		...
		public virtual IEnumerable ViewOriginalDocument(PXAdapter adapter)
		{
			if (Documents.Current != null)
			{
				...
				AR.ARRegister origDoc = PXSelect<AR.ARRegister,
					Where<AR.ARRegister.refNbr, Equal<Required<AR.ARRegister.origRefNbr>>,
						And<AR.ARRegister.docType, Equal<Required<AR.ARRegister.origDocType>>>>>
		...
		[PXButton(Category = CS.ActionCategories.DocumentProcessing)]
		...
		public virtual IEnumerable ARBalanceByCustomerReport(PXAdapter adapter)
		{
			...
			if (filter != null)
			{
				...
				{
					BAccountR bAccount = SelectFrom<BAccountR>.Where<BAccountR.bAccountID.IsEqual<@P.AsInt>>.View.Select(this, filter.OrgBAccountID);
		...
		protected virtual PXDelegateResult SelectDetails(bool summarize = false)
		{
			...
			{
				if (byPeriod && PXAccess.FeatureInstalled<FeaturesSet.vATRecognitionOnPrepayments>()
					&& !(header.IncludeGLTurnover == true && !summarize))
				{
					sel = sel.WhereAnd<Where<ARDocumentPeriodResult.accountID, Equal<Current<ARDocumentFilter.aRAcctID>>>>();
				}
				else
			...
			if (header.ARSubID != null)
			{
				if (byPeriod && PXAccess.FeatureInstalled<FeaturesSet.vATRecognitionOnPrepayments>()
					&& !(header.IncludeGLTurnover == true && !summarize))
				{
					sel = sel.WhereAnd<Where<ARDocumentPeriodResult.subID, Equal<Current<ARDocumentFilter.aRSubID>>>>();
				}
				else
			...
			if(byPeriod)
			{
				...
				else if (PXAccess.FeatureInstalled<FeaturesSet.vATRecognitionOnPrepayments>())
				{
					aggregate = typeof(Aggregate<
						GroupBy<ARDocumentPeriodResult.docType,
						GroupBy<ARDocumentPeriodResult.refNbr,
						// added fields to (!summarize) version
						GroupBy<ARDocumentPeriodResult.accountID,
						GroupBy<ARDocumentPeriodResult.subID,
						//---
				...
				types[0] = types[0].GetGenericArguments().Count() == 3
					? typeof(Select4<,,,>)
					: typeof(Select5<,,,,>);
				int indexOfOrderBy = types.IndexOf(typeof(OrderBy<>));
				types.Insert(indexOfOrderBy, aggregate);
				if (summarize)
				{
					types[0] = types.Where(t => typeof(IBqlJoin).IsAssignableFrom(t)).Any()
						? typeof(Select5<,,,>)
						: typeof(Select4<,,>);
					RemoveOrderBy(types);
				}
			...
			using(new PXFieldScope(documentView, restrictedFields))
							...
							ref startRow, PXView.MaximumRows, ref totalRows))
				{
					...
					else
					{
						...
						ARDocumentPeriodResult res = rec as ARDocumentPeriodResult;
						if (res != null)
						{
							item.ARAccountID = res.AccountID;
							item.ARSubID = res.SubID;
						}
						item.GLTurnover = 0m;
		...
		protected virtual void RemoveOrderBy(List<Type> types)
		{
			int indexOfOrderBy = types.IndexOf(typeof(OrderBy<>));
			types.RemoveRange(indexOfOrderBy, types.Count - indexOfOrderBy);
		}
		#endregion
