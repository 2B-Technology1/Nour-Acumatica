	...
	public class ARPaymentType : ARDocType
	{
		...
		public new class SOListAttribute : PXStringListAttribute
		{
			public SOListAttribute()
				: base(
				new string[] { Payment, CreditMemo, Prepayment, Refund, PrepaymentInvoice },
				new string[] { Messages.Payment, Messages.CreditMemo, Messages.Prepayment, Messages.Refund, Messages.PrepaymentInvoice })
			{ }
				...
				where Field : IBqlField
			{
				SetList<Field>(cache, null,
					refund ? new string[] { Refund } : new string[] { Payment, CreditMemo, Prepayment, PrepaymentInvoice },
					refund ? new string[] { Messages.Refund } : new string[] { Messages.Payment, Messages.CreditMemo, Messages.Prepayment, Messages.PrepaymentInvoice });
			}
		}
