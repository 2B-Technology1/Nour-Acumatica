...
namespace PX.Objects.AR
{
	...
	public class ARTaxAttribute : TaxAttribute
	{
			...
			: base(ParentType, TaxType, TaxSumType, TaxCalculationMode, parentBranchIDField)
		{
			...
			this._Attributes.Add(new PXUnboundFormulaAttribute(typeof(Switch<Case<
				Where<ARTran.lineType, NotEqual<SO.SOLineType.discount>>, ARTran.curyTranAmt>,
				decimal0>),
				typeof(SumCalc<ARInvoice.curyLineTotal>)));
			this._Attributes.Add(new PXUnboundFormulaAttribute(typeof(Switch<
				Case<Where<ARTran.lineType, NotIn3<SO.SOLineType.discount, SO.SOLineType.freight>>, ARTran.curyDiscAmt>,
				decimal0>),
		...
		protected override void DateUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (!isApplyRetainageCreditMemo && document.IsRetainageDocument != true)
			{
		...
		{
			if (true == (bool?)ParentGetValue<ARRegister.released>(sender.Graph)) return;
		...
		{
			return ((row == null || (bool?)sender.GetValue(row, _DisableAutomaticTaxCalculation) != true) &&
				(sender.Current == null || (bool?)sender.GetValue(sender.Current, _DisableAutomaticTaxCalculation) != true));
		}
	}
		...
		public virtual void FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			if (pm != null && det != null)
			{
				if (det.ARAutoNextNbr == true)
				{
					...
					while (PXSelect<CashAccountCheck, Where<CashAccountCheck.cashAccountID, Equal<Current<PaymentMethodAccount.cashAccountID>>, And<CashAccountCheck.paymentMethodID, Equal<Current<PaymentMethodAccount.paymentMethodID>>, And<CashAccountCheck.checkNbr, Equal<Required<CashAccountCheck.checkNbr>>>>>>.SelectSingleBound(sender.Graph, new object[] { det }, new object[] { e.NewValue }).Count == 1);
	...
	[Obsolete]
	public class ARActiveProjectAttibute : ARActiveProjectAttribute
	{
		public ARActiveProjectAttibute() : base()
		{
		}
	}
	...
	public class ARActiveProjectAttribute : PM.ProjectBaseAttribute
	{
		public ARActiveProjectAttribute() : base(null)
		{
		}
	}
}
