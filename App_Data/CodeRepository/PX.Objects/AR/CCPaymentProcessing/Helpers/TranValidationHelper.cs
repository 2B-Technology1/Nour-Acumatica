	...
	public static class TranValidationHelper
	{
		...
		public static void CheckPmInstance(TransactionData tranData, Tuple<CustomerPaymentMethod, CustomerPaymentMethodDetail> cpmData)
		{
			if (tranData != null && cpmData != null)
			{
				...
				if (tranData.CustomerId != null && tranData.PaymentId != null && (tranData.CustomerId != cpm.CustomerCCPID || tranData.PaymentId != cpmd.Value))
				{
		...
		public static void CheckTransactionByNoteId(TransactionData tranData, AdditionalParams inputParams)
		{
			var query = new PXSelect<ExternalTransaction, Where<ExternalTransaction.noteID, Equal<Required<ExternalTransaction.noteID>>>>(inputParams.Repo.Graph);
			ExternalTransaction storedTran = query.SelectSingle(tranData.TranUID);
			
			if (storedTran == null)
			{
				throw new TranValidationException(Messages.ERR_TransactionProcessedOnAnotherInstanceOrTenant, tranData.TranID);
			}

			if ((!string.IsNullOrEmpty(storedTran.TranNumber) && !storedTran.TranNumber.Equals(tranData.TranID)) ||
				!storedTran.ProcessingCenterID.Equals(inputParams.ProcessingCenter))
			{
				throw new TranValidationException(Messages.ERR_TransactionProcessedOnAnotherInstance, tranData.TranID);
			}	
		}
		...
		public static string GenerateTranAlreadyRecordedErrMsg(string tranId, string docType, string refNbr, AdditionalParams inputParams)
		{
			...
			if (inputParams.PMInstanceId > 0)
			{
				...
				CustomerPaymentMethod cpm = CustomerPaymentMethod.PK.Find(inputParams.Repo.Graph, inputParams.PMInstanceId);
