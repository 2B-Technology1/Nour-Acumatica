...
namespace PX.Objects.AR
{
	...
	public partial class ARPaymentEntry : ARDataEntryGraph<ARPaymentEntry, ARPayment>
		...
		public class MultiCurrency : ARMultiCurrencyGraph<ARPaymentEntry, ARPayment>
		{
			...
			protected virtual void _(Events.FieldUpdated<ARPayment, ARPayment.cashAccountID> e)
			{
				...
				else
				{
					...
					SourceFieldUpdated<ARPayment.curyInfoID, ARPayment.curyID, ARPayment.adjDate>(e.Cache, e.Row);
			...
			protected override void _(Events.FieldUpdated<Document, Document.documentDate> e)
			{
				...
				else Base.LoadInvoicesProc(true, null);

				ARPayment payment = (ARPayment)e.Row.Base;

				foreach (ARAdjust adj in Base.Adjustments.View.SelectExternal().RowCast<ARAdjust>()
						.Where(adj => (adj.Selected == true)))
				{
					ARInvoice invoice = ARInvoice.PK.Find(Base, adj.AdjdDocType, adj.AdjdRefNbr);

					if (payment.AdjDate <= invoice?.DiscDate && invoice.OrigModule == BatchModule.AR)
					{
						bool checkBalance = !(payment.Status == ARDocStatus.Hold || payment.Status == ARDocStatus.Balanced);   // Payment Amount is editable
						Base.AutoApplyProc(payment.CuryOrigDocAmt != 0m);
					}
				}
			}
		...
		public class AutoApplyProcess : Common.Scopes.FlaggedModeScopeBase<AutoApplyProcess>
		{
			public AutoApplyProcess() : base() { }
		}
		...
		[PXCopyPasteHiddenFields(typeof(ARPayment.clearDate), typeof(ARPayment.cleared))]
		...
		public PXSelectJoin<Standalone.ARAdjust,
					LeftJoinSingleTable<ARInvoice, On<ARInvoice.docType, Equal<Standalone.ARAdjust.adjdDocType>,
					...
					LeftJoin<CurrencyInfo,
						On<CurrencyInfo.curyInfoID, Equal<Standalone.ARAdjust.adjdCuryInfoID>>,
					LeftJoin<CM.CurrencyInfo2,
                  On<CM.CurrencyInfo2.curyInfoID, Equal<Standalone.ARRegisterAlias.curyInfoID>>>>>>>,
				Where<Standalone.ARAdjust.adjgDocType, Equal<Current<ARPayment.docType>>,
					...
					And<Standalone.ARAdjust.released, Equal<Required<Standalone.ARAdjust.released>>>>>>
				Adjustments_Balance;
		...
		{
			if (Adjustments_Raw.Cache.Inserted.Empty_()) return;

			IEnumerable<ARAdjust> cachedAdjustments = Adjustments_Raw.Cache.Inserted.OfType<ARAdjust>();

			LoadOptions loadOptions = new LoadOptions
			{
				StartRefNbr = cachedAdjustments.Select(_ => _.AdjdRefNbr).Min(),
				EndRefNbr = cachedAdjustments.Select(_ => _.AdjdRefNbr).Max(),
				FromDate = cachedAdjustments.Select(_ => _.AdjdDocDate).Min(),
				TillDate = cachedAdjustments.Select(_ => _.AdjdDocDate).Max(),
			};

			Adjustments_Raw.Cache.Cached.OfType<ARAdjust>().Join(
					GetCustDocs(loadOptions, Document.Current, arsetup.Current, this).AsEnumerable().Cast<PXResult<ARInvoice, CurrencyInfo, Customer, ARTran>>(),
		...
		public PXSelectJoin<ARTranPostBal,
				...
				Where<ARTranPostBal.docType, Equal<Current<ARPayment.docType>>,
					...
					And<ARTranPostBal.type, NotEqual<ARTranPost.type.prepaymentInvoice>,
		...
		public EPApprovalAutomationWithReservedDoc<ARPayment, ARPayment.approved, ARPayment.rejected, ARPayment.hold, ARSetupApproval> Approval;
		...
		[PXUnboundFormula(
			...
			ForceAggregateRecalculation = true)]
		[PXRestrictor(typeof(Where<PX.Objects.SO.SOInvoice.refNbr, IsNull, Or<PX.Objects.SO.SOInvoice.released, Equal<True>>>),
			Messages.NotReleasedSOInvoiceCannotBeSelectedForPayment,
			typeof(PX.Objects.SO.SOInvoice.docType),
			typeof(PX.Objects.SO.SOInvoice.refNbr),
			SuppressVerify = true)]
		...
		public override IEnumerable Release(PXAdapter adapter)
		{
			...
			foreach (ARPayment ardoc in adapter.Get<ARPayment>())
			{
				if (!(bool)ardoc.Hold)
				{
					list.Add((ARRegister)cache.Update(ardoc) ?? ardoc);
				}
			}
			if (list.Count == 0)
			{
		...
		{
			LoadInvoicesProc(LoadExistingOnly, new LoadOptions { Apply = true });
		}
		...
		public virtual void LoadInvoicesProc(bool LoadExistingOnly, LoadOptions opts)
		{
			...
			try
			{
				...
				foreach (PXResult<ARInvoice, CurrencyInfo, Customer, ARTran> res in custdocs)
				{
					...
					if (existing.ContainsKey(ToKeyString(invoice, tran)) == false)
					{
						...
						adjustmentsAmount += invoice.CuryDocBal * invoice.SignBalance ?? 0m;
			...
			if (opts?.Apply == true)
			{
				if (currentDoc.CuryDocBal != 0 && adjustmentsAmount + currentDoc.CuryDocBal < 0)
				{
					...
					credits.Sort((a, b) =>
					{
						return ((IComparable)a.CuryDocBal).CompareTo(b.CuryDocBal);
					});

					foreach (ARAdjust adj in credits)
					{
						if (adjustmentsAmount + currentDoc.CuryDocBal < 0 && currentDoc.CuryDocBal < adj.CuryDocBal)
						{
							Adjustments.Delete(adj);
							adjustmentsAmount += adj.CuryDocBal ?? 0m;
						}
						else
						{
							ARAdjust copy = PXCache<ARAdjust>.CreateCopy(adj);
							copy.CuryAdjgAmt += currentDoc.CuryDocBal;
		...
		public static int GetCustDocsCount(LoadOptions opts, ARPayment currentARPayment, ARSetup currentARSetup, PXGraph graph)
		{
			...
			object[] parametrs = AddParametersFromOptions(opts, currentARPayment, graph, cmd).ToArray();

			using (new PXFieldScope(cmd.View, typeof(ARInvoice.docType), typeof(ARInvoice.refNbr), typeof(ARTran.lineNbr)))
			{
				return cmd.Select(parametrs).RowCount ?? 0;
			}
		}

		private static IEnumerable<object> AddParametersFromOptions(LoadOptions opts, ARPayment currentARPayment, PXGraph graph, PXSelectBase<ARInvoice> cmd)
			{
			yield return currentARPayment.DocType;
			yield return currentARPayment.RefNbr;
			yield return currentARPayment.DocType;

			if (opts?.FromDate != null)
				{
					cmd.WhereAnd<Where<ARInvoice.docDate, GreaterEqual<Required<LoadOptions.fromDate>>>>();
				yield return opts.FromDate;
				}
			if (opts?.TillDate != null)
				{
					cmd.WhereAnd<Where<ARInvoice.docDate, LessEqual<Required<LoadOptions.tillDate>>>>();
				yield return opts.TillDate;
				}
			...
			switch (loadChildDocs)
			{
				case LoadOptions.loadChildDocuments.IncludeCRM:
				case LoadOptions.loadChildDocuments.ExcludeCRM:
					yield return currentARPayment.CustomerID;
					yield return currentARPayment.CustomerID;
					break;
				default:
					yield return currentARPayment.CustomerID;
			...
			if (!PXAccess.FeatureInstalled<FeaturesSet.interBranch>() && currentARPayment.BranchID != null)
			{
				...
				yield return GL.Helpers.BranchHelper.GetBranchesToApplyDocuments(graph, currentARPayment.BranchID);
				}
				}
		...
		public static PXResultset<ARInvoice> GetCustDocs(LoadOptions opts, ARPayment currentARPayment, ARSetup currentARSetup, PXGraph graph)
		{
			...
			var cmd = new PXSelectReadonly2<ARInvoice,
				...
				Where<ARInvoice.docType, NotEqual<Required<ARPayment.docType>>,
					And<ARInvoice.released, Equal<True>,
					And<ARInvoice.openDoc, Equal<True>,
					And<ARInvoice.hold, NotEqual<True>,
					And<ARAdjust.adjgRefNbr, IsNull,
					And<ARAdjust2.adjgRefNbr, IsNull,
					And<ARInvoice.pendingPPD, NotEqual<True>,
					And<ARInvoice.isUnderCorrection, NotEqual<True>,
			...
			PXResultset<ARInvoice> custdocs = opts?.MaxDocs == null 
				? cmd.Select(parametrs) 
				: cmd.SelectWindowed(0, (int)opts.MaxDocs, parametrs);
		...
		public virtual IEnumerable LoadInvoices(PXAdapter adapter)
		{
			...
			if (res == WebDialogResult.OK || res == WebDialogResult.Yes)
			{
				...
				loadOpts.ClearDialog();

				if (IsImport) LoadInvoicesProc(false, loadOpts.Current);
				else
				{
					ARPaymentEntry clone = this.Clone();
					PXLongOperation.StartOperation(this, () =>
					{
						clone.LoadInvoicesProc(false, loadOpts.Current);
		...
		protected virtual void ReverseApplicationProc(ARAdjust application, ARPayment payment)
		{
			...
			if (application.AdjdDocType == ARDocType.PrepaymentInvoice && application.AdjgDocType == ARDocType.CreditMemo)
			{
				throw new PXException(Messages.CannotReverseApplicationOfCreditMemoToPrepaymentInvoice);
			}
		...
		protected virtual void ARPayment_CustomerID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
				 ...
				 && arsetup.Current.AutoLoadMaxDocs > 0)
			{
				...
				else
				{
					...
					if (count > 0)
					{
						LoadInvoicesProc(false, opt); // load existing and invoices of new customers                    
					}
				}
			}
		}
		...
		{
			return doc.DocType == ARDocType.Refund 
			       && EPApprovalSettings<ARSetupApproval>.ApprovedDocTypes.Contains(doc.DocType);;
		}
		#endregion
		...
		private ARRegister GetAdjdInvoiceToVerifyArePaymentsByLineAllowed(PXCache cache, ARAdjust adj)
		{
			...
			return
				Caches[typeof(ARInvoice)].Cached.Cast<ARInvoice>().FirstOrDefault(_ => _.DocType == adj.AdjdDocType && _.RefNbr == adj.AdjdRefNbr)
				?? (ARRegister)PXSelectorAttribute.Select<ARAdjust.adjdRefNbr>(cache, adj);
		}
		...
		{
			ARPaymentChargeTran chargeTran = e.Row as ARPaymentChargeTran;

			if (chargeTran == null)
				return;

			var account = Account.PK.Find(this, chargeTran.AccountID);

			if (account != null && chargeTran.ProjectID != NonProject.ID)
			{
				if (account.AccountGroupID == null)
				{
					PaymentCharges.Cache.RaiseExceptionHandling<ARPaymentChargeTran.accountID>(chargeTran, account.AccountCD,
								new PXSetPropertyException(PM.Messages.NoOffsetAccountGroup, PXErrorLevel.Error, account.AccountCD));
				}
			}
		}

		protected virtual void ARPaymentChargeTran_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			ARPaymentChargeTran chargeTran = e.Row as ARPaymentChargeTran;
			...
			if (account != null && chargeTran.ProjectID != NonProject.ID)
			{
				if (account.AccountGroupID == null)
				{
					throw new PXRowPersistingException(typeof(ARPaymentChargeTran.accountID).Name, null, PM.Messages.NoOffsetAccountGroup, nameof(ARPaymentChargeTran.accountID));
				}
			}
		}
		...
		protected virtual void ARPayment_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			bool isEft = string.Equals(paymentmethod.Current?.PaymentType, PaymentMethodType.EFT);

			bool useNewCard = (paymentmethod.Current?.PaymentType == PaymentMethodType.CreditCard ||
							  paymentmethod.Current?.PaymentType == PaymentMethodType.EFT) &&
							  doc.DocType != ARDocType.Refund &&
							  !existsOpenCCTran;
			...
			PXUIFieldAttribute.SetEnabled<ARPayment.newCard>(cache, doc, useNewCard && !isEft);
			PXUIFieldAttribute.SetVisible<ARPayment.newCard>(cache, doc, newCardVisible && !isEft);
			PXUIFieldAttribute.SetEnabled<ARPayment.newAccount>(cache, doc, useNewCard && isEft);
			PXUIFieldAttribute.SetVisible<ARPayment.newAccount>(cache, doc, newCardVisible && isEft);
			...
			PXUIFieldAttribute.SetEnabled<ARPayment.saveCard>(cache, doc, useSaveCard && !isEft);
			PXUIFieldAttribute.SetVisible<ARPayment.saveCard>(cache, doc, saveCardVisible && !isEft);
			PXUIFieldAttribute.SetEnabled<ARPayment.saveAccount>(cache, doc, useSaveCard && isEft);
			PXUIFieldAttribute.SetVisible<ARPayment.saveAccount>(cache, doc, saveCardVisible && isEft);
			...
			bool hasAdjustments = doc.CustomerID != null && 
			                      Adjustments_Raw.Select().Any();
			...
			else if (docReleased && docOpen)
			{
				...
				holdAdj = Adjustments_Raw.Select().RowCast<ARAdjust>()
				...
				release.SetEnabled(docNotOnHold && !holdAdj && hasAdjustments);
			}
			else if (HasUnreleasedSOInvoice)
			{
				PXUIFieldAttribute.SetEnabled(cache, doc, false);
				PXUIFieldAttribute.SetEnabled<ARPayment.newCard>(cache, doc, false);
				PXUIFieldAttribute.SetEnabled<ARPayment.newAccount>(cache, doc, false);
				PXUIFieldAttribute.SetEnabled<ARPayment.hold>(cache, doc, true);
				PXUIFieldAttribute.SetEnabled<ARPayment.docDesc>(cache, doc, true);
		...
		protected virtual void CheckForUnreleasedIncomingApplications(PXCache sender, ARPayment document)
		{
			...
			ARAdjust unreleasedIncomingApplication = PXSelectJoin<ARAdjust,
			...
			{
				unreleasedIncomingApplication =
					BQL.HasApplicationToUnreleasedCM<Required<ARAdjust.adjgDocType>, Required<ARAdjust.adjgRefNbr>>.Select
					.SelectSingle<ARAdjust>(this, true, document.DocType, document.RefNbr);
		...
		public virtual void ARPayment_RowSelecting(PXCache sender, PXRowSelectingEventArgs e)
		{
			...
			if (row != null && !e.IsReadOnly)
			{
				using (new PXConnectionScope())
				{
					...
					RecalcWriteOffAmounts(sender, row);
				}
			}
		}
		...
		public virtual void RecalcWriteOffAmounts(PXCache sender, ARPayment row)
		{
			...
			if (row.CuryWOAmt == null)
			{
				row.CuryWOAmt = 0m;
			}
			sender.RaiseFieldUpdated<ARPayment.curyWOAmt>(row, null);
		}
		...
		public static void SetDocTypeList(PXCache cache, string docType)
		{
			...
			else if (docType == ARDocType.Payment || docType == ARDocType.VoidPayment || docType == ARDocType.Prepayment)
			{
				...
				{
					values.Add(ARDocType.PrepaymentInvoice);
					labels.Add(Messages.PrepaymentInvoice);
				}
			}
		...
		protected virtual void ARPayment_RowDeleting(PXCache sender, PXRowDeletingEventArgs e)
		{
			...
			PaymentMethod pm = this.paymentmethod.Current;
			var voidPaymentWithoutRefund = ((ARPayment)e.Row).DocType == ARDocType.VoidPayment && !state.IsRefunded && !state.IsVoided;
			if (pm?.PaymentType == CA.PaymentMethodType.CreditCard && pm?.ARIsProcessingRequired == true && state?.IsActive == true && !voidPaymentWithoutRefund)
			{
			...
			string paymentType)
		{
			...
			{
				throw new PXException(ErrorMessages.ElementDoesntExist, Messages.ARInvoice);
			}
		...
		internal bool _IsLocalValidation = false;
		...
		{
			string errMsg;
			return VerifyAdjFinPeriodID(doc, newValue, out errMsg);
		}
		...
		internal virtual void SetLocalValidation(ARPayment doc)
		{
			_IsLocalValidation = IsLocalValidation(doc);
		}
		...
		private ARPayment CopyRefundFromPayment(ARPayment doc, CurrencyInfo origCurrencyInfo, ARPayment origPayment)
		{
			...
			ExternalTransaction actualExtTran = ExternalTransaction.PK.Find(this, origPayment.CCActualExternalTransactionID);
		...
		protected virtual void ValidatePaymentForRefund(ARPayment document)
		{
			...
			ARAdjust unreleasedIncomingApplication = PXSelectJoin<
				ARAdjust,
				InnerJoin<ARRegisterAlias,
					On<ARAdjust.adjgDocType, Equal<ARRegisterAlias.docType>,
					And<ARAdjust.adjgRefNbr, Equal<ARRegisterAlias.refNbr>>>>,
		...
		{
			if (payment.Released != true || payment.OpenDoc != true)
			{
				return;
			}
			...
			if (!ARPaymentType.IsSelfVoiding(payment.DocType))
			{
				ARPayment voidPayment =
				SelectFrom<ARPayment>
					.Where<ARPayment.docType.IsEqual<@P.AsString>
						.And<ARPayment.origRefNbr.IsEqual<@P.AsString>>
						.And<ARPayment.released.IsNotEqual<True>>>
					.View.Select(this, ARDocType.VoidPayment, payment.RefNbr);
				if (voidPayment != null)
				{
					var errorMsg = PXLocalizer.LocalizeFormat(
						...
						GetLabel.For<ARDocType>(voidPayment.DocType),
						GetLabel.For<ARDocType>(payment.DocType),
						GetLabel.For<ARDocType>(voidPayment.DocType),
						voidPayment.RefNbr);
					throw new PXException(errorMsg);
