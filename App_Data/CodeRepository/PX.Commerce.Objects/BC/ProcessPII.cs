...
namespace PX.Commerce.Objects
{
	...
	public class ProcessPII : PXGraph<ProcessPII>
	{
		...
		protected virtual void _(PX.Data.Events.RowSelected<PIIFilter> e)
		{
			...
			AddWarningsAndErrorsForFilter(e);
		}

		/// <summary>
		/// This method is called at the end of the Select event handler of the PIIFilter.
		/// It displays a warning message when the user chooses Erase action but can be overritten to display other type of messages for the filter section.
		/// </summary>
		/// <param name="e"></param>

		protected virtual void AddWarningsAndErrorsForFilter(Events.RowSelected<PIIFilter> e)
		{
			PIIFilter filter = e.Row;
			if (filter == null) return;
			if (filter.Action == PIIActionAttribute.Erase)
				e.Cache.RaiseExceptionHandling<PIIFilter.action>(e.Row, filter.Action,
						new PXSetPropertyException<PIIFilter.action>(BCObjectsMessages.EraseDataWarning, PXErrorLevel.Warning));
			else
				PXUIFieldAttribute.SetError<PIIFilter.action>(e.Cache, e.Row, null);
		}
