...
namespace PX.Objects.PJ.Submittals.PJ.GraphExtensions
{
	public class SubmittalEntryWorkflowExtension : PXGraphExtension<SubmittalEntry>
	{
		...
		public sealed override void Configure(PXScreenConfiguration configuration) =>
			Configure(configuration.GetScreenConfigurationContext<SubmittalEntry, PJSubmittal>());
		protected static void Configure(WorkflowContext<SubmittalEntry, PJSubmittal> context)
		{
						...
						config => config.IsSidePanelScreen(
							sidePanelAction =>
								sidePanelAction.NavigateToScreen<DrawingLogsMaint>()
									...
									{
										containerFiller.Add<DrawingLogFilter.projectId>(c => c.SetFromField<PJSubmittal.projectId>());
