...
namespace PX.Objects.PJ.Common.Services
{
	...
	public abstract class BaseEmailFileAttachService
	{
		...
		public void AttachAllFilesLinkedToRelatedEntities(PXCache cache, object target)
			=> AttachAllLinkedNoteDocFiles(cache, target, GetFilesLinkedToRelatedEntities);

		protected void AttachAllLinkedNoteDocFiles(PXCache cache, object target, Func<IEnumerable<NoteDoc>> getLinkedNoteDocs)
		{
			var fileIds = getLinkedNoteDocs()
				.Where(note => note.FileID.HasValue)
				.Select(note => note.FileID.Value)
				.Distinct()
				.ToArray();
			...
			PXNoteAttribute.SetFileNotes(cache, target, fileIds);
		}
