...
{
	public class ProjectEntrySidePanelsExtension : PXGraphExtension<ProjectEntry_Workflow, ProjectEntry>
	{
		...
		public override void Configure(PXScreenConfiguration configuration)
		{
			...
			{
				WorkflowContext<ProjectEntry, PMProject> context =
				configuration.GetScreenConfigurationContext<ProjectEntry, PMProject>();
								...
								config => config.IsSidePanelScreen(
									sidePanelAction =>
										sidePanelAction.NavigateToScreen<DrawingLogsMaint>()
											...
											{
												containerFiller.Add<DrawingLogFilter.projectId>(c => c.SetFromField<PMProject.contractID>());
								...
								config => config.IsSidePanelScreen(
									sidePanelAction =>
										sidePanelAction.NavigateToScreen("PJ3020PL")
											...
											.WithAssignments(containerFiller =>
											{
												containerFiller.Add("ProjectIssue_projectId", c => c.SetFromField<PMProject.contractID>());
											}
											))
								...
								config => config.IsSidePanelScreen(
									sidePanelAction =>
										sidePanelAction.NavigateToScreen("PJ3010PL")
											...
											.WithAssignments(containerFiller =>
											{
												containerFiller.Add("RFI_projectId", c => c.SetFromField<PMProject.contractID>());
											}
											))
