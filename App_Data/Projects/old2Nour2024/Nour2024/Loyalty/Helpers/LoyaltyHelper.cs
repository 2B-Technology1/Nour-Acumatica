...
namespace Nour2025.Loyalty.Helpers
{
    public static class LoyaltyHelper
    {
        ...
        public static class LoyaltyStatus
        {
            ...
            public class redeemed : PX.Data.BQL.BqlString.Constant<redeemed>
            {
                public redeemed() : base(Redeemed) { }
            }
        ...
        public static void StoreEarnedLoyaltyPoints(Customer customer, SOOrder order, decimal earnedPoints)
        {
            ...
            //{
                try
                {
                    // Create a new instance of the DAC (LoyaltyActivity)
                    ...
                    // Use PXDatabase.Insert with explicit parameter mapping
                    PXDatabase.Insert<LoyaltyActivity>(
                        new PXDataFieldAssign<LoyaltyActivity.lineNbr>(loyaltyActivity.LineNbr),
                        new PXDataFieldAssign<LoyaltyActivity.customerID>(loyaltyActivity.CustomerID),
                        new PXDataFieldAssign<LoyaltyActivity.transactionID>(loyaltyActivity.TransactionID),
                        new PXDataFieldAssign<LoyaltyActivity.transactionDate>(loyaltyActivity.TransactionDate),
                        new PXDataFieldAssign<LoyaltyActivity.transactionType>(loyaltyActivity.TransactionType),
                        new PXDataFieldAssign<LoyaltyActivity.orderInvoiceReference>(loyaltyActivity.OrderInvoiceReference),
                        new PXDataFieldAssign<LoyaltyActivity.pointsEarned>(loyaltyActivity.PointsEarned),
                        new PXDataFieldAssign<LoyaltyActivity.consumedPoints>(loyaltyActivity.ConsumedPoints),
                        new PXDataFieldAssign<LoyaltyActivity.remainingPoints>(loyaltyActivity.RemainingPoints),
                        new PXDataFieldAssign<LoyaltyActivity.type>(loyaltyActivity.Type),
                        new PXDataFieldAssign<LoyaltyActivity.status>(loyaltyActivity.Status),
                        new PXDataFieldAssign<LoyaltyActivity.activationDate>(loyaltyActivity.ActivationDate),
                        new PXDataFieldAssign<LoyaltyActivity.expirationDate>(loyaltyActivity.ExpirationDate)
                    );
                ...
                catch (Exception ex)
                {
                    ...
                    PXTrace.WriteError($"Error inserting LoyaltyActivity: {ex.Message}");
                    throw; // Re-throw the exception to ensure the transaction is rolled back
                }
            //}
