...
using Nour2025.Loyalty.Helpers;
using PX.Data.BQL.Fluent;
using PX.Data.BQL;
using PX.Common;
using System.Linq;
using Nour20241217;
using static PX.Objects.CN.Compliance.CL.DAC.ComplianceDocument;
    ...
    public class SOOrderEntry_Extension : PXGraphExtension<SOOrderEntry>
    {
        ...
        protected void SOOrder_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
        {
            ...
            try
            {
                ...
                //#region Current Loyaltypoint
                //int? customerID = order.CustomerID;
                //if (customerID != null)
                //{
                //    // Query loyalty points from the LoyaltyActivity table
                //    var loyaltyPoints = PXSelect<LoyaltyActivity,
                //        Where<LoyaltyActivity.customerID, Equal<Required<LoyaltyActivity.customerID>>>>
                //        .Select(Base, customerID)
                //        .RowCast<LoyaltyActivity>()
                //        .Sum(activity => activity.RemainingPoints ?? 0);

                //    // Assign the calculated points to the unbounded field
                //    sender.SetValueExt<SOOrderExt.loyaltyPoints>(order, loyaltyPoints);

                //    var customer = SelectFrom<Customer>
                //   .Where<Customer.bAccountID.IsEqual<@P.AsInt>>
                //   .View.Select(Base, Base.Document.Current.CustomerID).RowCast<Customer>().FirstOrDefault();

                //    var earnedPoints = LoyaltyHelper.CalculateEarnedLoyaltyPoints(customer, order);
                //    sender.SetValueExt<SOOrderExt.earnedLoyaltyPoints>(order, earnedPoints);
        ...
        protected virtual void _(Events.RowPersisted<SOOrder> e)
        {
            ...
            if (IsSavingForFirtTime == true)
            {
                ...
                if (order.OrderNbr != null && order.OrderNbr != "" && order.OrderNbr != " <NEW>" && order.OrderType == "QT")
                {
                    ...
                    if (phone != null)
                    {
                        if (phone.Length == 11 || phone.Length == 12)
                        {
                            ...
                            string orderNumber = order.OrderNbr;
                            string orderDate = order.OrderDate.Value.ToShortDateString();

                            string message = $".شكرًا لاختيارك مركز نور الدين الشريف \\n" +
                                                               $".تم إصدار فاتورة برقم: {orderNumber} بتاريخ: {orderDate} \\n" +
                                                               "bit.ly/4g3lFl2: نرجو تقييم تجربتك عبر الرابط \\n" +
                                                               ".للاستفسارات أو الشكاوى، يُرجى الاتصال على: 19943 \\n" +
                                                               ".سعداء بخدمتك";
                ...
                //#region Loyalty Calculations
                //var soorder = base.Base.Document.Current;
                //var customer = SelectFrom<Customer>
                //    .Where<Customer.bAccountID.IsEqual<@P.AsInt>>
                //    .View.Select(Base, Base.Document.Current.CustomerID).RowCast<Customer>().FirstOrDefault();

                //var earnedPoints = LoyaltyHelper.CalculateEarnedLoyaltyPoints(customer, soorder);
                //LoyaltyHelper.StoreEarnedLoyaltyPoints(customer, soorder, earnedPoints);
                //#endregion
            }
