...
{
    public class CUCProcess : PXGraph<CUCProcess>
    {
        [Serializable]
            ...
            
        {
            ...
            [PXSelector
                (typeof(Search<CashAccount.cashAccountID, Where<CashAccountExt.usrCUC, Equal<True>>>),
                        new Type[]
                        {
            ...
            //        Where<CashAccountExt.usrCUC, Equal<True>,Or<CashAccountExt.usrNR,Equal<True>>>,
            ...
            [PXSelector
               (typeof(Search2<CashAccount.cashAccountID,
                           InnerJoin<CashAccountAccess, On<CashAccountAccess.cashAccountID, Equal<CashAccount.cashAccountID>>>,
                   Where2<Where<CashAccountExt.usrNR, Equal<True>>,
          ...
          Where<ARRegisterExt.usrCUC, Equal<False>,
              And<CashAccountExt.usrNR, Equal<True>,
                  And2<Where<ARRegister.docType, Equal<ARDocType.payment>, Or<ARRegister.docType, Equal<ARDocType.prepayment>>>,
                      And2<Where<ARRegister.status, Equal<ARDocStatus.open>, Or<ARRegister.status, Equal<ARDocStatus.closed>>>,
                          And<ARRegisterExt.usrRefunded, Equal<False>,
            //; Or<ARRegisterExt.usrRefunded, Equal<true>,
           // And<ARPayment.curyOrigDocAmt, Less<ARRegister.curyOrigDocAmt>,
                          And<CashAccountAccess.userID, Equal<Current<AccessInfo.userID>>/*,
                              And<CAAdjExt.usrReversed, Equal<False>>*/>>>>>>> Records; //this line should havn't been added. CUC doesnt read data from the CAAdj table
        ...
        private void FilterTransactions(ref BqlCommand query, ProcessFilter filter)
        {
            if (filter != null)
            {
                ...
                {
                    query = query.WhereAnd<Where<CashAccountAccess.cashAccountID, Equal<Current<ProcessFilter.cashAccountFilter>>>>();
                }
            }
        }
        ...
        {
            #region CalcTotal and GetRecords
        ...
        public static void ReleaseDocs(List<ARRegister> payments,int? cashAccount,DateTime? transactionDate,string documentRef)
        {
            ...
            foreach (ARRegister payment in payments)
            {
                ...
                CashAccount NRAccount = PXSelect<CashAccount,
                    Where<CashAccount.cashAccountCD,Equal<Required<CashAccount.cashAccountCD>>>>
                    .Select(bankDeposit,payment.GetExtension<ARRegisterExt>().UsrNRAccount);

                bankDeposit.Document.Insert();
                bankDeposit.Document.Current.CashAccountID = cashAccount;
                bankDeposit.Document.Current.ExtRefNbr = documentRef;
                ...
                if (NRAccount == null)
                    ...
                    bankDeposit.Details.Current.AccountID = NRAccount.CashAccountID;
                ...
                entry.Document.Cache.SetValueExt<ARPaymentExt.usrCUC>(entry.Document.Current, true);
                entry.Document.Cache.SetValueExt<ARPaymentExt.usrBankDeposited>(entry.Document.Current, false);
                entry.Document.Cache.SetValueExt<ARPaymentExt.usrDepositRefNbr>(entry.Document.Current, bankDeposit.Document.Current.RefNbr);
                entry.Document.Cache.SetValue<ARPayment.extRefNbr>(entry.Document.Current, "0000");
