...
public partial class Frames_Login : System.Web.UI.Page
{
	...
	{
		_loginUiService.ClearCompanyIdInCookie(Response);
		//it is much easier to redirect to force a clean slate
		//than to manually reset everything everywhere on the page (and in some dependencies)
		//but before that we also need to clear company from query string
		Response.Redirect(_loginUiService.GetPathWithoutCompanyId(Request));
	}
	#endregion
		...
		if (_multifactorService.IsAccessCodeValid(userName, txtPass.Text, oneTimePasswordText.Text, (object)Request, out user, out reason))
		{
			try
			{
				...
				else
				{
					...
					{
						_oidcUiService.TryAssociate(HttpContext.Current);
	...
	protected void SecureLogin(string[] companies)
	{
		...
		else if (TryGetCompanyIdFromCookie(out var companyIdFromCookie) &&
			this.cmbCompany.Items.FindByValue(companyIdFromCookie) != null)
		{
			this.cmbCompany.SelectedValue = companyIdFromCookie;
		}
	...
	private void FillCompanyCombo()
	{
		...
		else
		{
			...
			else
			{
				...
				if (TryGetCompanyIdFromCookie(out var companyIdFromCookie))
					this.cmbCompany.SelectedValue = companyIdFromCookie;
	...
	private bool TryGetCompanyIdFromCookie(out string companyId)
	{
		companyId = _loginUiService.GetCompanyIdFromCookie(Request);
	...
	private void InitialiseExternalLogins()
	{
		...
		try
		{
			...
			var oidc = GetOidcLogin(GetTenantName());
			var phExtControls = this.Master.FindControl("phExt").Controls;
			var oldOidcControls = phExtControls.OfType<HyperLink>()
				.Where(b => b.ID.IsPresent() && b.ID.StartsWith(OidcCommandPrefix))
				.ToArray();
			oldOidcControls.ForEach(c => phExtControls.Remove(c));

			foreach (var oidcProvider in oidc.Providers)
			{
				var providerBtn = new HyperLink
				{
					ID = OidcCommandPrefix + oidcProvider.Id.ToString().Replace('-', '_'),
					CssClass = "extlogin_wide_button",
					NavigateUrl = oidcProvider.InitiateSignInUrl,
					Text = $"<img src='{oidcProvider.IconUrl}' alt='logo' /><span>{oidcProvider.Name}</span>",
				};
				providerBtn.Attributes.Add("onclick", "return wrapClick(this, 'isRealOAuth', realFlagContainer, function() {return true}, true);");
	...
	private LoginPageOidcViewModel GetOidcLogin(string tenant)
		=> _oidcUiService.GetLoginPage(Context, tenant, cmbLang.SelectedValue, Request.Params.Get("ReturnUrl"));
	...
	private string GetTenantName()
	{
		if (PXDatabase.SecureCompanyID)
		{
			...
			if (TryGetCompanyIdFromCookie(out var companyId))
			{
				return companyId;
			}
				...
				case SecureTenantFlowStates.Initial:
					{
						...
						this.btnMoreOpenIdProviders.Visible = GetOidcLogin(null).HasMoreProviders;
						break;
					}
