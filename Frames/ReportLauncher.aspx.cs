...
public partial class Pages_ReportLauncher : PX.Web.UI.PXPage
{
	...
	private const string PreserveSessionParam = "preserveSession";
	...
	protected void Page_Init(object sender, EventArgs e)
	{
		...
		var node = (PXSiteMapNode) PXSiteMap.Provider.FindSiteMapNode(url)
			?? (PXSiteMapNode) PXSiteMap.Provider.FindSiteMapNode(Request.RawUrl);
		if (node != null)
		{
	...
	{
        object passed = PXContext.Session.PageInfo[Page.Request.Path.ToRelativeUrl(Page.Request.ApplicationPath)];
		if (passed == null)
		{
			object passedByRawUrl = PXContext.Session.PageInfo[Page.Request.RawUrl.ToRelativeUrl(Page.Request.ApplicationPath)];
		...
		object reportParams = reports == null ? passed : reports[0].Value;
		Dictionary<string, string> parsDict = (reportParams as PXReportRedirectParameters)?.ReportParameters ?? reportParams as Dictionary<string, string>;
		if (parsDict != null)
		{
			string enabledLocalization;

			if (parsDict.TryGetValue(ReportLauncherHelper._ENABLED_LOCALIZATION, out enabledLocalization)
				&& !string.IsNullOrEmpty(enabledLocalization))
			{
				if (viewer.Report != null)
				{
					viewer.Report.Localization = enabledLocalization;
				}
				parsDict.Remove(ReportLauncherHelper._ENABLED_LOCALIZATION);
			}
		}
		...
		if (provider != null && reports != null)
		{
			if (reports.SeparateWindows)
			{
				if (reports.Count > 1)
				{
					...
					{
						url = $"{PXUrl.ToAbsoluteUrl(url)}&{PreserveSessionParam}=true";
			...
			else
			{
				foreach (KeyValuePair<string, object> t in reports)
				{
					...
					if (reportNode != null && !string.IsNullOrEmpty(reportName = PXUrl.GetParameter(reportNode.Url, "ID")))
					{
						...
						Dictionary<string, string> pars = (t.Value as PXReportRedirectParameters)?.ReportParameters ?? t.Value as Dictionary<string, string>;
						if (pars != null)
						{
							string enabledLocalization;

							if (pars.TryGetValue(ReportLauncherHelper._ENABLED_LOCALIZATION, out enabledLocalization)
							    && !string.IsNullOrEmpty(enabledLocalization))
							{
								report.Localization = enabledLocalization;
								pars.Remove(ReportLauncherHelper._ENABLED_LOCALIZATION);
							}
						}
	...
	protected void viewer_ReportLoaded(object sender, EventArgs e)
	{
		...
		if ((Request.QueryString[ReportLauncherHelper._AUTO_EXPORT_PDF] != null)) systemParamsCount++;
        ...
        if (pars == null && Request.QueryString.Count > systemParamsCount && !(Request.AppRelativeCurrentExecutionFilePath ?? "").StartsWith("~/rest/"))
		{
			...
			{
				if (String.Compare(key, ReportLauncherHelper._AUTO_EXPORT_PDF, StringComparison.OrdinalIgnoreCase) == 0)
				{
					viewer.AutoExportPDF = true;
					continue;
				}
				if (String.Compare(key, "ID", StringComparison.OrdinalIgnoreCase) == 0 ||
					...
					String.Compare(key, ReportLauncherHelper._ENABLED_LOCALIZATION, StringComparison.OrdinalIgnoreCase) == 0 ||
					string.Compare(key, PreserveSessionParam, StringComparison.OrdinalIgnoreCase) == 0 ||
		...
		{
			pars = null;
			ClearReportParametersFromSession();
		}
	...
	protected void ClearReportParametersFromSession()
	{
		PXContext.Session.PageInfo[Page.Request.Path.ToRelativeUrl(Page.Request.ApplicationPath)] = null;		
	}
	...
	protected void viewer_ReportPreRender(object sender, EventArgs e)
	{
		...
		if (NextReport != null)
		{
			...
			PXContext.Session.PageInfo[url.ToRelativeUrl(Page.Request.ApplicationPath)] = NextReport.Value.Value;
		}
	}
}
