...
public class InventoryMatrixEntry
{
	...
	public const string TotalField = "Extra";
	...
	public static void SetUOMFormat(PXGridRow row, PXGridColumnCollection columns)
	{
		EntryMatrix item = row.DataItem as EntryMatrix;
		if (item?.UOMs == null)
			return;
		...
		for (int columnIndex = 0; columnIndex < row.Cells.Count; columnIndex++)
		{
			...
			if (column.DataField?.StartsWith(Template) == true && char.IsDigit(column.DataField.Last()))
			{
				uom = item.UOMs[attributeIndex++];
			}
			else if (column.DataField == TotalField)
			{
				uom = item.BaseUOM;	
			}
			...
			if (uom != null)
			{
				...
				cell.DisplayText = $"{cell.FormattedText} {uom}";
			}
		}
	}
